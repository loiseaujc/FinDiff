module stdlib_math
  ...
  !!([Specification](../page/specs/stdlib_math.html#logspace-create-a-logarithmically-spaced-rank-one-array))
    ...
    interface meshgrid
        ...
        module subroutine meshgrid_1_rqp_rqp(&
                x1,  &
                xm1,  &
                indexing &
        )
            real(qp), intent(in) :: x1(:)
            real(qp), intent(out) :: xm1 (:)
            integer, intent(in), optional :: indexing
        end subroutine meshgrid_1_rqp_rqp
        module subroutine meshgrid_2_rqp_rqp(&
                x1, x2,  &
                xm1, xm2,  &
                indexing &
        )
            ...
            real(qp), intent(out) :: xm2 (:,:)
            integer, intent(in), optional :: indexing
        end subroutine meshgrid_2_rqp_rqp
        module subroutine meshgrid_3_rqp_rqp(&
                x1, x2, x3,  &
                xm1, xm2, xm3,  &
                indexing &
        )
            ...
            real(qp), intent(in) :: x2(:)
            real(qp), intent(out) :: xm2 (:,:,:)
            real(qp), intent(in) :: x3(:)
            real(qp), intent(out) :: xm3 (:,:,:)
            integer, intent(in), optional :: indexing
        end subroutine meshgrid_3_rqp_rqp
        module subroutine meshgrid_4_rqp_rqp(&
                x1, x2, x3, x4,  &
                xm1, xm2, xm3, xm4,  &
                indexing &
        )
            ...
            real(qp), intent(in) :: x3(:)
            real(qp), intent(out) :: xm3 (:,:,:,:)
            real(qp), intent(in) :: x4(:)
            real(qp), intent(out) :: xm4 (:,:,:,:)
            integer, intent(in), optional :: indexing
        end subroutine meshgrid_4_rqp_rqp
...
contains
    ...
    elemental function deg2rad_qp(theta) result(result)
        real(qp), intent(in) :: theta
        real(qp) :: result
        result = theta * PI_qp / 180.0_qp
        
    end function deg2rad_qp
    
    elemental function rad2deg_qp(theta) result(result)
        real(qp), intent(in) :: theta
        real(qp) :: result
        result = theta * 180.0_qp / PI_qp
        
    end function rad2deg_qp
    ...
    elemental subroutine swap_qp(lhs, rhs)
        real(qp), intent(inout) :: lhs, rhs
        real(qp) :: temp
        temp = lhs; lhs = rhs; rhs = temp
    end subroutine
    ...
    elemental subroutine swap_cqp(lhs, rhs)
        complex(qp), intent(inout) :: lhs, rhs
        complex(qp) :: temp
        temp = lhs; lhs = rhs; rhs = temp
    end subroutine
