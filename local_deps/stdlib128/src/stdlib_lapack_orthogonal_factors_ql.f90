  ...
  contains
     ...
     !! 0 is a M-by-(N-M) zero matrix, if M < N.
        ...
        ! =====================================================================
           ...
           ! the lq decomposition
           if( ( n<=m ) .or. ( nb<=m ) .or. ( nb>=n ) ) then
             call stdlib_qgelqt( m, n, mb, a, lda, t( 6_ilp ), mb, work, info )
           else
             call stdlib_qlaswlq( m, n, mb, nb, a, lda, t( 6_ilp ), mb, work,lwork, info )
           end if
           work( 1_ilp ) = lwreq
           return
     ...
     !! 0 is a M-by-(N-M) zero matrix, if M < N.
        ...
        ! =====================================================================
           ...
           ! the lq decomposition
           if( ( n<=m ) .or. ( nb<=m ) .or. ( nb>=n ) ) then
             call stdlib_wgelqt( m, n, mb, a, lda, t( 6_ilp ), mb, work, info )
           else
             call stdlib_wlaswlq( m, n, mb, nb, a, lda, t( 6_ilp ), mb, work,lwork, info )
           end if
           work( 1_ilp ) = lwreq
           return
     ...
     !! factorization (DGELQ)
       ...
       ! =====================================================================
           ...
           if( ( left .and. m<=k ) .or. ( right .and. n<=k ).or. ( nb<=k ) .or. ( nb>=max( m, n, &
                     k ) ) ) then
             call stdlib_qgemlqt( side, trans, m, n, k, mb, a, lda,t( 6_ilp ), mb, c, ldc, work, info &
                       )
           else
             call stdlib_qlamswlq( side, trans, m, n, k, mb, nb, a, lda, t( 6_ilp ),mb, c, ldc, work, &
                       lwork, info )
           end if
           work( 1_ilp ) = lw
           return
     ...
     !! LQ factorization (ZGELQ)
       ...
       ! =====================================================================
           ...
           if( ( left .and. m<=k ) .or. ( right .and. n<=k ).or. ( nb<=k ) .or. ( nb>=max( m, n, &
                     k ) ) ) then
             call stdlib_wgemlqt( side, trans, m, n, k, mb, a, lda,t( 6_ilp ), mb, c, ldc, work, info &
                       )
           else
             call stdlib_wlamswlq( side, trans, m, n, k, mb, nb, a, lda, t( 6_ilp ),mb, c, ldc, work, &
                       lwork, info )
           end if
           work( 1_ilp ) = lw
           return
        ...
        ! =====================================================================
           ...
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
              ...
              do i = 1, k - nx, nb
                 ...
                 ! compute the lq factorization of the current block
                 ! a(i:i+ib-1,i:n)
                 call stdlib_qgelq2( ib, n-i+1, a( i, i ), lda, tau( i ), work,iinfo )
                 if( i+ib<=m ) then
                    ! form the triangular factor of the block reflector
                    ! h = h(i) h(i+1) . . . h(i+ib-1)
                    call stdlib_qlarft( 'FORWARD', 'ROWWISE', n-i+1, ib, a( i, i ),lda, tau( i ), &
                              work, ldwork )
                    ! apply h to a(i+ib:m,i:n) from the right
        ...
        ! =====================================================================
           ...
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
              ...
              do i = 1, k - nx, nb
                 ...
                 ! compute the lq factorization of the current block
                 ! a(i:i+ib-1,i:n)
                 call stdlib_wgelq2( ib, n-i+1, a( i, i ), lda, tau( i ), work,iinfo )
                 if( i+ib<=m ) then
                    ! form the triangular factor of the block reflector
                    ! h = h(i) h(i+1) . . . h(i+ib-1)
                    call stdlib_wlarft( 'FORWARD', 'ROWWISE', n-i+1, ib, a( i, i ),lda, tau( i ), &
                              work, ldwork )
                    ! apply h to a(i+ib:m,i:n) from the right
     ...
     !! 0 is a m-by-(n-m) zero matrix, if m < n.
        ...
        ! =====================================================================
           ...
           do i = 1, k
              ! generate elementary reflector h(i) to annihilate a(i,i+1:n)
              call stdlib_qlarfg( n-i+1, a( i, i ), a( i, min( i+1, n ) ), lda,tau( i ) )
              if( i<m ) then
                 ! apply h(i) to a(i+1:m,i:n) from the right
                 aii = a( i, i )
                 a( i, i ) = one
                 call stdlib_qlarf( 'RIGHT', m-i, n-i+1, a( i, i ), lda, tau( i ),a( i+1, i ), &
                           lda, work )
                 a( i, i ) = aii
              end if
           end do
     ...
     !! 0 is a m-by-(n-m) zero matrix, if m < n.
        ...
        ! =====================================================================
           ...
           do i = 1, k
              ...
              alpha = a( i, i )
              call stdlib_wlarfg( n-i+1, alpha, a( i, min( i+1, n ) ), lda,tau( i ) )
              if( i<m ) then
                 ! apply h(i) to a(i+1:m,i:n) from the right
                 a( i, i ) = cone
                 call stdlib_wlarf( 'RIGHT', m-i, n-i+1, a( i, i ), lda, tau( i ),a( i+1, i ), &
                           lda, work )
              end if
              a( i, i ) = alpha
              call stdlib_wlacgv( n-i+1, a( i, i ), lda )
     ...
     !! as returned by ZGELQF.
        ...
        ! =====================================================================
           ...
           ! use unblocked code for the last or only block.
           if( kk<m )call stdlib_wungl2( m-kk, n-kk, k-kk, a( kk+1, kk+1 ), lda,tau( kk+1 ), work,&
                      iinfo )
           if( kk>0_ilp ) then
              ! use blocked code
              do i = ki + 1, 1, -nb
                 ...
                 if( i+ib<=m ) then
                    ...
                    call stdlib_wlarft( 'FORWARD', 'ROWWISE', n-i+1, ib, a( i, i ),lda, tau( i ), &
                              work, ldwork )
                    ! apply h**h to a(i+ib:m,i:n) from the right
                    call stdlib_wlarfb( 'RIGHT', 'CONJUGATE TRANSPOSE', 'FORWARD','ROWWISE', m-i-&
                    ib+1, n-i+1, ib, a( i, i ),lda, work, ldwork, a( i+ib, i ), lda,work( ib+1 ), &
                              ldwork )
                 end if
                 ! apply h**h to columns i:n of current block
                 call stdlib_wungl2( ib, n-i+1, ib, a( i, i ), lda, tau( i ), work,iinfo )
                 ! set columns 1:i-1 of current block to czero
                 do j = 1, i - 1
                    do l = i, i + ib - 1
                       a( l, j ) = czero
        ...
        ! =====================================================================
           ...
           do i = k, 1, -1
              ...
              if( i<n ) then
                 ...
                 if( i<m ) then
                    a( i, i ) = cone
                    call stdlib_wlarf( 'RIGHT', m-i, n-i+1, a( i, i ), lda,conjg( tau( i ) ), a( &
                              i+1, i ), lda, work )
                 end if
                 call stdlib_wscal( n-i, -tau( i ), a( i, i+1 ), lda )
                 call stdlib_wlacgv( n-i, a( i, i+1 ), lda )
              end if
              a( i, i ) = cone - conjg( tau( i ) )
              ! set a(i,1:i-1) to czero
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( nb<nbmin .or. nb>=k ) then
              ! use unblocked code
              call stdlib_wunml2( side, trans, m, n, k, a, lda, tau, c, ldc, work,iinfo )
           else
              ! use blocked code
              iwt = 1_ilp + nw*nb
              if( ( left .and. notran ) .or.( .not.left .and. .not.notran ) ) then
              ...
              do i = i1, i2, i3
                 ...
                 ! form the triangular factor of the block reflector
                 ! h = h(i) h(i+1) . . . h(i+ib-1)
                 call stdlib_wlarft( 'FORWARD', 'ROWWISE', nq-i+1, ib, a( i, i ),lda, tau( i ), &
                           work( iwt ), ldt )
                 if( left ) then
                    ! h or h**h is applied to c(i:m,1:n)
                    mi = m - i + 1_ilp
                 ...
                 ! apply h or h**h
                 call stdlib_wlarfb( side, transt, 'FORWARD', 'ROWWISE', mi, ni, ib,a( i, i ), &
                           lda, work( iwt ), ldt,c( ic, jc ), ldc, work, ldwork )
              end do
           end if
           work( 1_ilp ) = lwkopt
     ...
     !! as returned by DGELQF.
        ...
        ! =====================================================================
           ...
           ! use unblocked code for the last or only block.
           if( kk<m )call stdlib_qorgl2( m-kk, n-kk, k-kk, a( kk+1, kk+1 ), lda,tau( kk+1 ), work,&
                      iinfo )
           if( kk>0_ilp ) then
              ! use blocked code
              do i = ki + 1, 1, -nb
        ...
        ! =====================================================================
           ...
           do i = k, 1, -1
              ...
              if( i<n ) then
                 ...
                 call stdlib_qscal( n-i, -tau( i ), a( i, i+1 ), lda )
              end if
              a( i, i ) = one - tau( i )
              ! set a(i,1:i-1) to zero
              do l = 1, i - 1
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( nb<nbmin .or. nb>=k ) then
              ! use unblocked code
              call stdlib_qorml2( side, trans, m, n, k, a, lda, tau, c, ldc, work,iinfo )
           else
              ! use blocked code
              iwt = 1_ilp + nw*nb
              if( ( left .and. notran ) .or.( .not.left .and. .not.notran ) ) then
              ...
              do i = i1, i2, i3
                 ...
                 ! form the triangular factor of the block reflector
                 ! h = h(i) h(i+1) . . . h(i+ib-1)
                 call stdlib_qlarft( 'FORWARD', 'ROWWISE', nq-i+1, ib, a( i, i ),lda, tau( i ), &
                           work( iwt ), ldt )
                 if( left ) then
                    ! h or h**t is applied to c(i:m,1:n)
                    mi = m - i + 1_ilp
                 ...
                 ! apply h or h**t
                 call stdlib_qlarfb( side, transt, 'FORWARD', 'ROWWISE', mi, ni, ib,a( i, i ), &
                           lda, work( iwt ), ldt,c( ic, jc ), ldc, work, ldwork )
              end do
           end if
           work( 1_ilp ) = lwkopt
     ...
     !! using the compact WY representation of Q.
       ...
       ! =====================================================================
           ...
           do i = 1, k,  mb
              ib = min( k-i+1, mb )
           ! compute the lq factorization of the current block a(i:m,i:i+ib-1)
              call stdlib_qgelqt3( ib, n-i+1, a(i,i), lda, t(1_ilp,i), ldt, iinfo )
              if( i+ib<=m ) then
           ! update by applying h**t to a(i:m,i+ib:n) from the right
              call stdlib_qlarfb( 'R', 'N', 'F', 'R', m-i-ib+1, n-i+1, ib,a( i, i ), lda, t( 1_ilp, i &
                        ), ldt,a( i+ib, i ), lda, work , m-i-ib+1 )
              end if
           end do
           return
     ...
     !! using the compact WY representation of Q.
       ...
       ! =====================================================================
           ...
           do i = 1, k,  mb
              ib = min( k-i+1, mb )
           ! compute the lq factorization of the current block a(i:m,i:i+ib-1)
              call stdlib_wgelqt3( ib, n-i+1, a(i,i), lda, t(1_ilp,i), ldt, iinfo )
              if( i+ib<=m ) then
           ! update by applying h**t to a(i:m,i+ib:n) from the right
              call stdlib_wlarfb( 'R', 'N', 'F', 'R', m-i-ib+1, n-i+1, ib,a( i, i ), lda, t( 1_ilp, i &
                        ), ldt,a( i+ib, i ), lda, work , m-i-ib+1 )
              end if
           end do
           return
     ...
     !! IBM J. Res. Develop. Vol 44 No. 4 July 2000.
        ...
        ! =====================================================================
           ...
           if( m==1_ilp ) then
              ! compute householder transform when m=1
              call stdlib_qlarfg( n, a(1_ilp,1_ilp), a( 1_ilp, min( 2_ilp, n ) ), lda, t(1_ilp,1_ilp) )
           else
              ! otherwise, split a into blocks...
              m1 = m/2_ilp
              m2 = m-m1
              ...
              j1 = min( m+1, n )
              ! compute a(1:m1,1:n) <- (y1,r1,t1), where q1 = i - y1 t1 y1^h
              call stdlib_qgelqt3( m1, n, a, lda, t, ldt, iinfo )
              ! compute a(j1:m,1:n) = q1^h a(j1:m,1:n) [workspace: t(1:n1,j1:n)]
              do i=1,m2
                 do j=1,m1
                    t(  i+m1, j ) = a( i+m1, j )
              ...
              call stdlib_qtrmm( 'R', 'U', 'T', 'U', m2, m1, one,a, lda, t( i1, 1_ilp ), ldt )
              call stdlib_qgemm( 'N', 'T', m2, m1, n-m1, one, a( i1, i1 ), lda,a( 1_ilp, i1 ), lda, &
                        one, t( i1, 1_ilp ), ldt)
              call stdlib_qtrmm( 'R', 'U', 'N', 'N', m2, m1, one,t, ldt, t( i1, 1_ilp ), ldt )
              call stdlib_qgemm( 'N', 'N', m2, n-m1, m1, -one, t( i1, 1_ilp ), ldt,a( 1_ilp, i1 ), lda, &
                        one, a( i1, i1 ), lda )
              call stdlib_qtrmm( 'R', 'U', 'N', 'U', m2, m1 , one,a, lda, t( i1, 1_ilp ), ldt )
              ...
              ! compute a(j1:m,j1:n) <- (y2,r2,t2) where q2 = i - y2 t2 y2^h
              call stdlib_qgelqt3( m2, n-m1, a( i1, i1 ), lda,t( i1, i1 ), ldt, iinfo )
              ! compute t3 = t(j1:n1,1:n) = -t1 y1^h y2 t2
              do i=1,m2
                 do j=1,m1
                    t( j, i+m1  ) = (a( j, i+m1 ))
              ...
              call stdlib_qtrmm( 'R', 'U', 'T', 'U', m1, m2, one,a( i1, i1 ), lda, t( 1_ilp, i1 ), &
                        ldt )
              call stdlib_qgemm( 'N', 'T', m1, m2, n-m, one, a( 1_ilp, j1 ), lda,a( i1, j1 ), lda, &
                        one, t( 1_ilp, i1 ), ldt )
              call stdlib_qtrmm( 'L', 'U', 'N', 'N', m1, m2, -one, t, ldt,t( 1_ilp, i1 ), ldt )
                        
              call stdlib_qtrmm( 'R', 'U', 'N', 'N', m1, m2, one,t( i1, i1 ), ldt, t( 1_ilp, i1 ), &
                        ldt )
              ! y = (y1,y2); l = [ l1            0  ];  t = [t1 t3]
                               ! [ a(1:n1,j1:n)  l2 ]       [ 0 t2]
           end if
     ...
     !! IBM J. Res. Develop. Vol 44 No. 4 July 2000.
        ...
        ! =====================================================================
           ...
           if( m==1_ilp ) then
              ! compute householder transform when m=1
              call stdlib_wlarfg( n, a(1_ilp,1_ilp), a( 1_ilp, min( 2_ilp, n ) ), lda, t(1_ilp,1_ilp) )
              t(1_ilp,1_ilp)=conjg(t(1_ilp,1_ilp))
           else
              ! otherwise, split a into blocks...
              m1 = m/2_ilp
              ...
              j1 = min( m+1, n )
              ! compute a(1:m1,1:n) <- (y1,r1,t1), where q1 = i - y1 t1 y1^h
              call stdlib_wgelqt3( m1, n, a, lda, t, ldt, iinfo )
              ! compute a(j1:m,1:n) =  a(j1:m,1:n) q1^h [workspace: t(1:n1,j1:n)]
              do i=1,m2
                 do j=1,m1
                    t(  i+m1, j ) = a( i+m1, j )
              ...
              call stdlib_wtrmm( 'R', 'U', 'C', 'U', m2, m1, cone,a, lda, t( i1, 1_ilp ), ldt )
                        
              call stdlib_wgemm( 'N', 'C', m2, m1, n-m1, cone, a( i1, i1 ), lda,a( 1_ilp, i1 ), lda, &
                        cone, t( i1, 1_ilp ), ldt)
              call stdlib_wtrmm( 'R', 'U', 'N', 'N', m2, m1, cone,t, ldt, t( i1, 1_ilp ), ldt )
                        
              call stdlib_wgemm( 'N', 'N', m2, n-m1, m1, -cone, t( i1, 1_ilp ), ldt,a( 1_ilp, i1 ), lda, &
                        cone, a( i1, i1 ), lda )
              call stdlib_wtrmm( 'R', 'U', 'N', 'U', m2, m1 , cone,a, lda, t( i1, 1_ilp ), ldt )
              ...
              ! compute a(j1:m,j1:n) <- (y2,r2,t2) where q2 = i - y2 t2 y2^h
              call stdlib_wgelqt3( m2, n-m1, a( i1, i1 ), lda,t( i1, i1 ), ldt, iinfo )
              ! compute t3 = t(j1:n1,1:n) = -t1 y1^h y2 t2
              do i=1,m2
                 do j=1,m1
                    t( j, i+m1  ) = (a( j, i+m1 ))
              ...
              call stdlib_wtrmm( 'R', 'U', 'C', 'U', m1, m2, cone,a( i1, i1 ), lda, t( 1_ilp, i1 ), &
                        ldt )
              call stdlib_wgemm( 'N', 'C', m1, m2, n-m, cone, a( 1_ilp, j1 ), lda,a( i1, j1 ), lda, &
                        cone, t( 1_ilp, i1 ), ldt )
              call stdlib_wtrmm( 'L', 'U', 'N', 'N', m1, m2, -cone, t, ldt,t( 1_ilp, i1 ), ldt )
                        
              call stdlib_wtrmm( 'R', 'U', 'N', 'N', m1, m2, cone,t( i1, i1 ), ldt, t( 1_ilp, i1 ), &
                        ldt )
              ! y = (y1,y2); l = [ l1            0  ];  t = [t1 t3]
                               ! [ a(1:n1,j1:n)  l2 ]       [ 0 t2]
           end if
     ...
     !! Q is of order M if SIDE = 'L' and of order N  if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( left .and. notran ) then
              do i = 1, k, mb
                 ib = min( mb, k-i+1 )
                 call stdlib_qlarfb( 'L', 'T', 'F', 'R', m-i+1, n, ib,v( i, i ), ldv, t( 1_ilp, i ), &
                           ldt,c( i, 1_ilp ), ldc, work, ldwork )
              end do
           else if( right .and. tran ) then
              do i = 1, k, mb
                 ib = min( mb, k-i+1 )
                 call stdlib_qlarfb( 'R', 'N', 'F', 'R', m, n-i+1, ib,v( i, i ), ldv, t( 1_ilp, i ), &
                           ldt,c( 1_ilp, i ), ldc, work, ldwork )
              end do
           else if( left .and. tran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ib = min( mb, k-i+1 )
                 call stdlib_qlarfb( 'L', 'N', 'F', 'R', m-i+1, n, ib,v( i, i ), ldv, t( 1_ilp, i ), &
                           ldt,c( i, 1_ilp ), ldc, work, ldwork )
              end do
           else if( right .and. notran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ib = min( mb, k-i+1 )
                 call stdlib_qlarfb( 'R', 'T', 'F', 'R', m, n-i+1, ib,v( i, i ), ldv, t( 1_ilp, i ), &
                           ldt,c( 1_ilp, i ), ldc, work, ldwork )
              end do
           end if
           return
     ...
     !! Q is of order M if SIDE = 'L' and of order N  if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( left .and. notran ) then
              do i = 1, k, mb
                 ib = min( mb, k-i+1 )
                 call stdlib_wlarfb( 'L', 'C', 'F', 'R', m-i+1, n, ib,v( i, i ), ldv, t( 1_ilp, i ), &
                           ldt,c( i, 1_ilp ), ldc, work, ldwork )
              end do
           else if( right .and. tran ) then
              do i = 1, k, mb
                 ib = min( mb, k-i+1 )
                 call stdlib_wlarfb( 'R', 'N', 'F', 'R', m, n-i+1, ib,v( i, i ), ldv, t( 1_ilp, i ), &
                           ldt,c( 1_ilp, i ), ldc, work, ldwork )
              end do
           else if( left .and. tran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ib = min( mb, k-i+1 )
                 call stdlib_wlarfb( 'L', 'N', 'F', 'R', m-i+1, n, ib,v( i, i ), ldv, t( 1_ilp, i ), &
                           ldt,c( i, 1_ilp ), ldc, work, ldwork )
              end do
           else if( right .and. notran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ib = min( mb, k-i+1 )
                 call stdlib_wlarfb( 'R', 'C', 'F', 'R', m, n-i+1, ib,v( i, i ), ldv, t( 1_ilp, i ), &
                           ldt,c( 1_ilp, i ), ldc, work, ldwork )
              end do
           end if
           return
     ...
     !! 0 is a M-by-(N-M) zero matrix, if M < N, and is not stored.
        ...
        ! =====================================================================
           ...
           ! the lq decomposition
            if((m>=n).or.(nb<=m).or.(nb>=n)) then
             call stdlib_qgelqt( m, n, mb, a, lda, t, ldt, work, info)
             return
            end if
            kk = mod((n-m),(nb-m))
            ii=n-kk+1
            ! compute the lq factorization of the first block a(1:m,1:nb)
            call stdlib_qgelqt( m, nb, mb, a(1_ilp,1_ilp), lda, t, ldt, work, info)
            ctr = 1_ilp
            do i = nb+1, ii-nb+m , (nb-m)
            ! compute the qr factorization of the current block a(1:m,i:i+nb-m)
              call stdlib_qtplqt( m, nb-m, 0_ilp, mb, a(1_ilp,1_ilp), lda, a( 1_ilp, i ),lda, t(1_ilp, ctr * m + 1_ilp),&
                        ldt, work, info )
              ctr = ctr + 1_ilp
            end do
           ! compute the qr factorization of the last block a(1:m,ii:n)
            if (ii<=n) then
             call stdlib_qtplqt( m, kk, 0_ilp, mb, a(1_ilp,1_ilp), lda, a( 1_ilp, ii ),lda, t(1_ilp, ctr * m + 1_ilp), &
                       ldt,work, info )
            end if
           work( 1_ilp ) = m * mb
           return
     ...
     !! 0 is a M-by-(N-M) zero matrix, if M < N, and is not stored.
        ...
        ! =====================================================================
           ...
           ! the lq decomposition
            if((m>=n).or.(nb<=m).or.(nb>=n)) then
             call stdlib_wgelqt( m, n, mb, a, lda, t, ldt, work, info)
             return
            end if
            kk = mod((n-m),(nb-m))
            ii=n-kk+1
            ! compute the lq factorization of the first block a(1:m,1:nb)
            call stdlib_wgelqt( m, nb, mb, a(1_ilp,1_ilp), lda, t, ldt, work, info)
            ctr = 1_ilp
            do i = nb+1, ii-nb+m , (nb-m)
            ! compute the qr factorization of the current block a(1:m,i:i+nb-m)
              call stdlib_wtplqt( m, nb-m, 0_ilp, mb, a(1_ilp,1_ilp), lda, a( 1_ilp, i ),lda, t(1_ilp, ctr * m + 1_ilp),&
                        ldt, work, info )
              ctr = ctr + 1_ilp
            end do
           ! compute the qr factorization of the last block a(1:m,ii:n)
            if (ii<=n) then
             call stdlib_wtplqt( m, kk, 0_ilp, mb, a(1_ilp,1_ilp), lda, a( 1_ilp, ii ),lda, t(1_ilp, ctr * m + 1_ilp), &
                       ldt,work, info )
            end if
           work( 1_ilp ) = m * mb
           return
     ...
     !! factorization (DLASWLQ)
       ...
       ! =====================================================================
           ...
           if(left.and.tran) then
               ...
               if (kk>0_ilp) then
                 ii=m-kk+1
                 call stdlib_qtpmlqt('L','T',kk , n, k, 0_ilp, mb, a(1_ilp,ii), lda,t(1_ilp,ctr*k+1), ldt, c(&
                           1_ilp,1_ilp), ldc,c(ii,1_ilp), ldc, work, info )
               else
                 ii=m+1
               end if
               ...
               ! multiply q to the current block of c (1:m,i:i+nb)
                 ctr = ctr - 1_ilp
                 call stdlib_qtpmlqt('L','T',nb-k , n, k, 0_ilp,mb, a(1_ilp,i), lda,t(1_ilp, ctr*k+1),ldt, c(&
                           1_ilp,1_ilp), ldc,c(i,1_ilp), ldc, work, info )
               end do
               ! multiply q to the first block of c (1:m,1:nb)
               call stdlib_qgemlqt('L','T',nb , n, k, mb, a(1_ilp,1_ilp), lda, t,ldt ,c(1_ilp,1_ilp), ldc, work, &
                         info )
           else if (left.and.notran) then
               ! multiply q to the first block of c
              kk = mod((m-k),(nb-k))
              ii=m-kk+1
              ctr = 1_ilp
              call stdlib_qgemlqt('L','N',nb , n, k, mb, a(1_ilp,1_ilp), lda, t,ldt ,c(1_ilp,1_ilp), ldc, work, &
                        info )
              do i=nb+1,ii-nb+k,(nb-k)
               ! multiply q to the current block of c (i:i+nb,1:n)
               call stdlib_qtpmlqt('L','N',nb-k , n, k, 0_ilp,mb, a(1_ilp,i), lda,t(1_ilp,ctr*k+1), ldt, c(1_ilp,&
                         1_ilp), ldc,c(i,1_ilp), ldc, work, info )
               ctr = ctr + 1_ilp
              end do
              if(ii<=m) then
               ! multiply q to the last block of c
               call stdlib_qtpmlqt('L','N',kk , n, k, 0_ilp, mb, a(1_ilp,ii), lda,t(1_ilp,ctr*k+1), ldt, c(1_ilp,&
                         1_ilp), ldc,c(ii,1_ilp), ldc, work, info )
              end if
           else if(right.and.notran) then
               ! multiply q to the last block of c
               ...
               if (kk>0_ilp) then
                 ii=n-kk+1
                 call stdlib_qtpmlqt('R','N',m , kk, k, 0_ilp, mb, a(1_ilp, ii), lda,t(1_ilp,ctr *k+1), ldt, &
                           c(1_ilp,1_ilp), ldc,c(1_ilp,ii), ldc, work, info )
               else
                 ii=n+1
               end if
               ...
               ! multiply q to the current block of c (1:m,i:i+mb)
                  ctr = ctr - 1_ilp
                  call stdlib_qtpmlqt('R','N', m, nb-k, k, 0_ilp, mb, a(1_ilp, i), lda,t(1_ilp,ctr*k+1), ldt, &
                            c(1_ilp,1_ilp), ldc,c(1_ilp,i), ldc, work, info )
               end do
               ! multiply q to the first block of c (1:m,1:mb)
               call stdlib_qgemlqt('R','N',m , nb, k, mb, a(1_ilp,1_ilp), lda, t,ldt ,c(1_ilp,1_ilp), ldc, work, &
                         info )
           else if (right.and.tran) then
             ! multiply q to the first block of c
              kk = mod((n-k),(nb-k))
              ctr = 1_ilp
              ii=n-kk+1
              call stdlib_qgemlqt('R','T',m , nb, k, mb, a(1_ilp,1_ilp), lda, t,ldt ,c(1_ilp,1_ilp), ldc, work, &
                        info )
              do i=nb+1,ii-nb+k,(nb-k)
               ! multiply q to the current block of c (1:m,i:i+mb)
               call stdlib_qtpmlqt('R','T',m , nb-k, k, 0_ilp,mb, a(1_ilp,i), lda,t(1_ilp,ctr*k+1), ldt, c(1_ilp,&
                         1_ilp), ldc,c(1_ilp,i), ldc, work, info )
               ctr = ctr + 1_ilp
              end do
              if(ii<=n) then
             ! multiply q to the last block of c
               call stdlib_qtpmlqt('R','T',m , kk, k, 0_ilp,mb, a(1_ilp,ii), lda,t(1_ilp,ctr*k+1),ldt, c(1_ilp,1_ilp),&
                          ldc,c(1_ilp,ii), ldc, work, info )
              end if
           end if
           work(1_ilp) = lw
     ...
     !! factorization (ZLASWLQ)
       ...
       ! =====================================================================
           ...
           if(left.and.tran) then
               ...
               if (kk>0_ilp) then
                 ii=m-kk+1
                 call stdlib_wtpmlqt('L','C',kk , n, k, 0_ilp, mb, a(1_ilp,ii), lda,t(1_ilp,ctr*k+1), ldt, c(&
                           1_ilp,1_ilp), ldc,c(ii,1_ilp), ldc, work, info )
               else
                 ii=m+1
               end if
               ...
               ! multiply q to the current block of c (1:m,i:i+nb)
                 ctr = ctr - 1_ilp
                 call stdlib_wtpmlqt('L','C',nb-k , n, k, 0_ilp,mb, a(1_ilp,i), lda,t(1_ilp,ctr*k+1),ldt, c(1_ilp,&
                           1_ilp), ldc,c(i,1_ilp), ldc, work, info )
               end do
               ! multiply q to the first block of c (1:m,1:nb)
               call stdlib_wgemlqt('L','C',nb , n, k, mb, a(1_ilp,1_ilp), lda, t,ldt ,c(1_ilp,1_ilp), ldc, work, &
                         info )
           else if (left.and.notran) then
               ! multiply q to the first block of c
              kk = mod((m-k),(nb-k))
              ii=m-kk+1
              ctr = 1_ilp
              call stdlib_wgemlqt('L','N',nb , n, k, mb, a(1_ilp,1_ilp), lda, t,ldt ,c(1_ilp,1_ilp), ldc, work, &
                        info )
              do i=nb+1,ii-nb+k,(nb-k)
               ! multiply q to the current block of c (i:i+nb,1:n)
               call stdlib_wtpmlqt('L','N',nb-k , n, k, 0_ilp,mb, a(1_ilp,i), lda,t(1_ilp, ctr * k + 1_ilp), ldt, &
                         c(1_ilp,1_ilp), ldc,c(i,1_ilp), ldc, work, info )
               ctr = ctr + 1_ilp
              end do
              if(ii<=m) then
               ! multiply q to the last block of c
               call stdlib_wtpmlqt('L','N',kk , n, k, 0_ilp, mb, a(1_ilp,ii), lda,t(1_ilp, ctr * k + 1_ilp), ldt, &
                         c(1_ilp,1_ilp), ldc,c(ii,1_ilp), ldc, work, info )
              end if
           else if(right.and.notran) then
               ! multiply q to the last block of c
               ...
               if (kk>0_ilp) then
                 ii=n-kk+1
                 call stdlib_wtpmlqt('R','N',m , kk, k, 0_ilp, mb, a(1_ilp, ii), lda,t(1_ilp, ctr * k + 1_ilp), &
                           ldt, c(1_ilp,1_ilp), ldc,c(1_ilp,ii), ldc, work, info )
               else
                 ii=n+1
               end if
               ...
               ! multiply q to the current block of c (1:m,i:i+mb)
               ctr = ctr - 1_ilp
               call stdlib_wtpmlqt('R','N', m, nb-k, k, 0_ilp, mb, a(1_ilp, i), lda,t(1_ilp, ctr * k + 1_ilp), &
                         ldt, c(1_ilp,1_ilp), ldc,c(1_ilp,i), ldc, work, info )
               end do
               ! multiply q to the first block of c (1:m,1:mb)
               call stdlib_wgemlqt('R','N',m , nb, k, mb, a(1_ilp,1_ilp), lda, t,ldt ,c(1_ilp,1_ilp), ldc, work, &
                         info )
           else if (right.and.tran) then
             ! multiply q to the first block of c
              kk = mod((n-k),(nb-k))
              ii=n-kk+1
              call stdlib_wgemlqt('R','C',m , nb, k, mb, a(1_ilp,1_ilp), lda, t,ldt ,c(1_ilp,1_ilp), ldc, work, &
                        info )
              ctr = 1_ilp
              do i=nb+1,ii-nb+k,(nb-k)
               ! multiply q to the current block of c (1:m,i:i+mb)
               call stdlib_wtpmlqt('R','C',m , nb-k, k, 0_ilp,mb, a(1_ilp,i), lda,t(1_ilp,ctr *k+1), ldt, c(1_ilp,&
                         1_ilp), ldc,c(1_ilp,i), ldc, work, info )
               ctr = ctr + 1_ilp
              end do
              if(ii<=n) then
             ! multiply q to the last block of c
               call stdlib_wtpmlqt('R','C',m , kk, k, 0_ilp,mb, a(1_ilp,ii), lda,t(1_ilp, ctr * k + 1_ilp),ldt, c(&
                         1_ilp,1_ilp), ldc,c(1_ilp,ii), ldc, work, info )
              end if
           end if
           work(1_ilp) = lw
     ...
     !! WY representation for Q.
       ...
       ! =====================================================================
           ...
           ! compute the qr factorization of the current block
              ...
              call stdlib_qtplqt2( ib, nb, lb, a(i,i), lda, b( i, 1_ilp ), ldb,t(1_ilp, i ), ldt, iinfo )
                        
           ! update by applying h**t to b(i+ib:m,:) from the right
              if( i+ib<=m ) then
                 call stdlib_qtprfb( 'R', 'N', 'F', 'R', m-i-ib+1, nb, ib, lb,b( i, 1_ilp ), ldb, t( &
                           1_ilp, i ), ldt,a( i+ib, i ), lda, b( i+ib, 1_ilp ), ldb,work, m-i-ib+1)
              end if
           end do
           return
     ...
     !! WY representation for Q.
       ...
       ! =====================================================================
           ...
           ! compute the qr factorization of the current block
              ...
              call stdlib_wtplqt2( ib, nb, lb, a(i,i), lda, b( i, 1_ilp ), ldb,t(1_ilp, i ), ldt, iinfo )
                        
           ! update by applying h**t to b(i+ib:m,:) from the right
              if( i+ib<=m ) then
                 call stdlib_wtprfb( 'R', 'N', 'F', 'R', m-i-ib+1, nb, ib, lb,b( i, 1_ilp ), ldb, t( &
                           1_ilp, i ), ldt,a( i+ib, i ), lda, b( i+ib, 1_ilp ), ldb,work, m-i-ib+1)
              end if
           end do
           return
     ...
     !! using the compact WY representation for Q.
        ...
        ! =====================================================================
           ...
           do i = 1, m
              ! generate elementary reflector h(i) to annihilate b(i,:)
              p = n-l+min( l, i )
              call stdlib_qlarfg( p+1, a( i, i ), b( i, 1_ilp ), ldb, t( 1_ilp, i ) )
              if( i<m ) then
                 ! w(m-i:1) := c(i+1:m,i:n) * c(i,i:n) [use w = t(m,:)]
                 do j = 1, m-i
                    t( m, j ) = (a( i+j, i ))
                 end do
                 call stdlib_qgemv( 'N', m-i, p, one, b( i+1, 1_ilp ), ldb,b( i, 1_ilp ), ldb, one, t( m, &
                           1_ilp ), ldt )
                 ! c(i+1:m,i:n) = c(i+1:m,i:n) + alpha * c(i,i:n)*w(m-1:1)^h
                 alpha = -(t( 1_ilp, i ))
                 do j = 1, m-i
                 ...
                 call stdlib_qger( m-i, p, alpha,  t( m, 1_ilp ), ldt,b( i, 1_ilp ), ldb, b( i+1, 1_ilp ), &
                           ldb )
              end if
           end do
           do i = 2, m
              ...
              call stdlib_qtrmv( 'L', 'N', 'N', p, b( 1_ilp, np ), ldb,t( i, 1_ilp ), ldt )
              ! rectangular part of b2
              call stdlib_qgemv( 'N', i-1-p, l,  alpha, b( mp, np ), ldb,b( i, np ), ldb, zero, t(&
                         i,mp ), ldt )
              ! b1
              call stdlib_qgemv( 'N', i-1, n-l, alpha, b, ldb, b( i, 1_ilp ), ldb,one, t( i, 1_ilp ), ldt &
                        )
              ! t(1:i-1,i) := t(1:i-1,1:i-1) * t(i,1:i-1)
             call stdlib_qtrmv( 'L', 'T', 'N', i-1, t, ldt, t( i, 1_ilp ), ldt )
              ! t(i,i) = tau(i)
              t( i, i ) = t( 1_ilp, i )
              t( 1_ilp, i ) = zero
           end do
     ...
     !! using the compact WY representation for Q.
        ...
        ! =====================================================================
           ...
           do i = 1, m
              ! generate elementary reflector h(i) to annihilate b(i,:)
              p = n-l+min( l, i )
              call stdlib_wlarfg( p+1, a( i, i ), b( i, 1_ilp ), ldb, t( 1_ilp, i ) )
              t(1_ilp,i)=conjg(t(1_ilp,i))
              if( i<m ) then
                 do j = 1, p
                    b( i, j ) = conjg(b(i,j))
                 ...
                 call stdlib_wgemv( 'N', m-i, p, cone, b( i+1, 1_ilp ), ldb,b( i, 1_ilp ), ldb, cone, t( &
                           m, 1_ilp ), ldt )
                 ! c(i+1:m,i:n) = c(i+1:m,i:n) + alpha * c(i,i:n)*w(m-1:1)^h
                 alpha = -(t( 1_ilp, i ))
                 do j = 1, m-i
                 ...
                 call stdlib_wgerc( m-i, p, (alpha),  t( m, 1_ilp ), ldt,b( i, 1_ilp ), ldb, b( i+1, 1_ilp ), &
                           ldb )
                 do j = 1, p
                    b( i, j ) = conjg(b(i,j))
                 end do
           ...
           do i = 2, m
              ...
              call stdlib_wtrmv( 'L', 'N', 'N', p, b( 1_ilp, np ), ldb,t( i, 1_ilp ), ldt )
              ! rectangular part of b2
              call stdlib_wgemv( 'N', i-1-p, l,  alpha, b( mp, np ), ldb,b( i, np ), ldb, czero, &
                        t( i,mp ), ldt )
              ! b1
              call stdlib_wgemv( 'N', i-1, n-l, alpha, b, ldb, b( i, 1_ilp ), ldb,cone, t( i, 1_ilp ), &
                        ldt )
              ! t(1:i-1,i) := t(1:i-1,1:i-1) * t(i,1:i-1)
              do j = 1, i-1
                 t(i,j)=conjg(t(i,j))
              end do
              call stdlib_wtrmv( 'L', 'C', 'N', i-1, t, ldt, t( i, 1_ilp ), ldt )
              do j = 1, i-1
                 t(i,j)=conjg(t(i,j))
              end do
              do j = 1, n-l+p
        ...
        ! =====================================================================
           ...
           if( left .and. notran ) then
              do i = 1, k, mb
                 ...
                 call stdlib_qtprfb( 'L', 'T', 'F', 'R', nb, n, ib, lb,v( i, 1_ilp ), ldv, t( 1_ilp, i ), &
                           ldt,a( i, 1_ilp ), lda, b, ldb, work, ib )
              end do
           else if( right .and. tran ) then
              do i = 1, k, mb
                 ...
                 call stdlib_qtprfb( 'R', 'N', 'F', 'R', m, nb, ib, lb,v( i, 1_ilp ), ldv, t( 1_ilp, i ), &
                           ldt,a( 1_ilp, i ), lda, b, ldb, work, m )
              end do
           else if( left .and. tran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ...
                 call stdlib_qtprfb( 'L', 'N', 'F', 'R', nb, n, ib, lb,v( i, 1_ilp ), ldv, t( 1_ilp, i ), &
                           ldt,a( i, 1_ilp ), lda, b, ldb, work, ib )
              end do
           else if( right .and. notran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ...
                 call stdlib_qtprfb( 'R', 'T', 'F', 'R', m, nb, ib, lb,v( i, 1_ilp ), ldv, t( 1_ilp, i ), &
                           ldt,a( 1_ilp, i ), lda, b, ldb, work, m )
              end do
           end if
           return
        ...
        ! =====================================================================
           ...
           if( left .and. notran ) then
              do i = 1, k, mb
                 ...
                 call stdlib_wtprfb( 'L', 'C', 'F', 'R', nb, n, ib, lb,v( i, 1_ilp ), ldv, t( 1_ilp, i ), &
                           ldt,a( i, 1_ilp ), lda, b, ldb, work, ib )
              end do
           else if( right .and. tran ) then
              do i = 1, k, mb
                 ...
                 call stdlib_wtprfb( 'R', 'N', 'F', 'R', m, nb, ib, lb,v( i, 1_ilp ), ldv, t( 1_ilp, i ), &
                           ldt,a( 1_ilp, i ), lda, b, ldb, work, m )
              end do
           else if( left .and. tran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ...
                 call stdlib_wtprfb( 'L', 'N', 'F', 'R', nb, n, ib, lb,v( i, 1_ilp ), ldv, t( 1_ilp, i ), &
                           ldt,a( i, 1_ilp ), lda, b, ldb, work, ib )
              end do
           else if( right .and. notran ) then
              kf = ((k-1)/mb)*mb+1
              do i = kf, 1, -mb
                 ...
                 call stdlib_wtprfb( 'R', 'C', 'F', 'R', m, nb, ib, lb,v( i, 1_ilp ), ldv, t( 1_ilp, i ), &
                           ldt,a( 1_ilp, i ), lda, b, ldb, work, m )
              end do
           end if
           return
        ...
        ! =====================================================================
           ...
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
              ...
              do i = k - kk + ki + 1, k - kk + 1, -nb
                 ...
                 ! compute the ql factorization of the current block
                 ! a(1:m-k+i+ib-1,n-k+i:n-k+i+ib-1)
                 call stdlib_qgeql2( m-k+i+ib-1, ib, a( 1_ilp, n-k+i ), lda, tau( i ),work, iinfo )
                           
                 if( n-k+i>1_ilp ) then
                    ! form the triangular factor of the block reflector
                    ! h = h(i+ib-1) . . . h(i+1) h(i)
                    call stdlib_qlarft( 'BACKWARD', 'COLUMNWISE', m-k+i+ib-1, ib,a( 1_ilp, n-k+i ), &
                              lda, tau( i ), work, ldwork )
                    ! apply h**t to a(1:m-k+i+ib-1,1:n-k+i-1) from the left
           ...
           ! use unblocked code to factor the last or only block
           if( mu>0_ilp .and. nu>0_ilp )call stdlib_qgeql2( mu, nu, a, lda, tau, work, iinfo )
           work( 1_ilp ) = iws
           return
        ...
        ! =====================================================================
           ...
           if( nb>=nbmin .and. nb<k .and. nx<k ) then
              ...
              do i = k - kk + ki + 1, k - kk + 1, -nb
                 ...
                 ! compute the ql factorization of the current block
                 ! a(1:m-k+i+ib-1,n-k+i:n-k+i+ib-1)
                 call stdlib_wgeql2( m-k+i+ib-1, ib, a( 1_ilp, n-k+i ), lda, tau( i ),work, iinfo )
                           
                 if( n-k+i>1_ilp ) then
                    ! form the triangular factor of the block reflector
                    ! h = h(i+ib-1) . . . h(i+1) h(i)
                    call stdlib_wlarft( 'BACKWARD', 'COLUMNWISE', m-k+i+ib-1, ib,a( 1_ilp, n-k+i ), &
                              lda, tau( i ), work, ldwork )
                    ! apply h**h to a(1:m-k+i+ib-1,1:n-k+i-1) from the left
                    call stdlib_wlarfb( 'LEFT', 'CONJUGATE TRANSPOSE', 'BACKWARD','COLUMNWISE', m-&
                    k+i+ib-1, n-k+i-1, ib,a( 1_ilp, n-k+i ), lda, work, ldwork, a, lda,work( ib+1 ), &
                              ldwork )
                 end if
              end do
           ...
           ! use unblocked code to factor the last or only block
           if( mu>0_ilp .and. nu>0_ilp )call stdlib_wgeql2( mu, nu, a, lda, tau, work, iinfo )
           work( 1_ilp ) = iws
           return
     ...
     !! A = Q * L.
        ...
        ! =====================================================================
           ...
           do i = k, 1, -1
              ! generate elementary reflector h(i) to annihilate
              ! a(1:m-k+i-1,n-k+i)
              call stdlib_qlarfg( m-k+i, a( m-k+i, n-k+i ), a( 1_ilp, n-k+i ), 1_ilp,tau( i ) )
              ! apply h(i) to a(1:m-k+i,1:n-k+i-1) from the left
              aii = a( m-k+i, n-k+i )
              a( m-k+i, n-k+i ) = one
              call stdlib_qlarf( 'LEFT', m-k+i, n-k+i-1, a( 1_ilp, n-k+i ), 1_ilp, tau( i ),a, lda, work )
     ...
     !! A = Q * L.
        ...
        ! =====================================================================
           ...
           do i = k, 1, -1
              ...
              ! a(1:m-k+i-1,n-k+i)
              alpha = a( m-k+i, n-k+i )
              call stdlib_wlarfg( m-k+i, alpha, a( 1_ilp, n-k+i ), 1_ilp, tau( i ) )
              ! apply h(i)**h to a(1:m-k+i,1:n-k+i-1) from the left
              a( m-k+i, n-k+i ) = cone
              call stdlib_wlarf( 'LEFT', m-k+i, n-k+i-1, a( 1_ilp, n-k+i ), 1_ilp,conjg( tau( i ) ), a, &
                        lda, work )
              a( m-k+i, n-k+i ) = alpha
           end do
           return
     ...
     !! as returned by ZGEQLF.
        ...
        ! =====================================================================
           ...
           ! use unblocked code for the first or only block.
           call stdlib_wung2l( m-kk, n-kk, k-kk, a, lda, tau, work, iinfo )
           if( kk>0_ilp ) then
              ! use blocked code
              do i = k - kk + 1, k, nb
                 ib = min( nb, k-i+1 )
                 ...
                 ! apply h to rows 1:m-k+i+ib-1 of current block
                 call stdlib_wung2l( m-k+i+ib-1, ib, ib, a( 1_ilp, n-k+i ), lda,tau( i ), work, iinfo &
                           )
                 ! set rows m-k+i+ib:m of current block to czero
                 do j = n - k + i, n - k + i + ib - 1
                    do l = m - k + i + ib, m
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( nb<nbmin .or. nb>=k ) then
              ! use unblocked code
              call stdlib_wunm2l( side, trans, m, n, k, a, lda, tau, c, ldc, work,iinfo )
           else
              ! use blocked code
              iwt = 1_ilp + nw*nb
              if( ( left .and. notran ) .or.( .not.left .and. .not.notran ) ) then
              ...
              do i = i1, i2, i3
                 ...
                 ! form the triangular factor of the block reflector
                 ! h = h(i+ib-1) . . . h(i+1) h(i)
                 call stdlib_wlarft( 'BACKWARD', 'COLUMNWISE', nq-k+i+ib-1, ib,a( 1_ilp, i ), lda, &
                           tau( i ), work( iwt ), ldt )
                 if( left ) then
                    ! h or h**h is applied to c(1:m-k+i+ib-1,1:n)
                    mi = m - k + i + ib - 1_ilp
                 ...
                 ! apply h or h**h
                 call stdlib_wlarfb( side, trans, 'BACKWARD', 'COLUMNWISE', mi, ni,ib, a( 1_ilp, i ), &
                           lda, work( iwt ), ldt, c, ldc,work, ldwork )
              end do
           end if
           work( 1_ilp ) = lwkopt
     ...
     !! as returned by ZGEQLF.
        ...
        ! =====================================================================
           ...
           do i = 1, k
              ...
              ! apply h(i) to a(1:m-k+i,1:n-k+i) from the left
              a( m-n+ii, ii ) = cone
              call stdlib_wlarf( 'LEFT', m-n+ii, ii-1, a( 1_ilp, ii ), 1_ilp, tau( i ), a,lda, work )
                        
              call stdlib_wscal( m-n+ii-1, -tau( i ), a( 1_ilp, ii ), 1_ilp )
              a( m-n+ii, ii ) = cone - tau( i )
              ! set a(m-k+i+1:m,n-k+i) to czero
              do l = m - n + ii + 1, m
                 a( l, ii ) = czero
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           do i = i1, i2, i3
              ...
              aii = a( nq-k+i, i )
              a( nq-k+i, i ) = cone
              call stdlib_wlarf( side, mi, ni, a( 1_ilp, i ), 1_ilp, taui, c, ldc, work )
              a( nq-k+i, i ) = aii
           end do
           return
     ...
     !! as returned by DGEQLF.
        ...
        ! =====================================================================
           ...
           ! use unblocked code for the first or only block.
           call stdlib_qorg2l( m-kk, n-kk, k-kk, a, lda, tau, work, iinfo )
           if( kk>0_ilp ) then
              ! use blocked code
              do i = k - kk + 1, k, nb
                 ib = min( nb, k-i+1 )
                 ...
                 ! apply h to rows 1:m-k+i+ib-1 of current block
                 call stdlib_qorg2l( m-k+i+ib-1, ib, ib, a( 1_ilp, n-k+i ), lda,tau( i ), work, iinfo &
                           )
                 ! set rows m-k+i+ib:m of current block to zero
                 do j = n - k + i, n - k + i + ib - 1
                    do l = m - k + i + ib, m
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           if( nb<nbmin .or. nb>=k ) then
              ! use unblocked code
              call stdlib_qorm2l( side, trans, m, n, k, a, lda, tau, c, ldc, work,iinfo )
           else
              ! use blocked code
              iwt = 1_ilp + nw*nb
              if( ( left .and. notran ) .or.( .not.left .and. .not.notran ) ) then
              ...
              do i = i1, i2, i3
                 ...
                 ! form the triangular factor of the block reflector
                 ! h = h(i+ib-1) . . . h(i+1) h(i)
                 call stdlib_qlarft( 'BACKWARD', 'COLUMNWISE', nq-k+i+ib-1, ib,a( 1_ilp, i ), lda, &
                           tau( i ), work( iwt ), ldt )
                 if( left ) then
                    ! h or h**t is applied to c(1:m-k+i+ib-1,1:n)
                    mi = m - k + i + ib - 1_ilp
                 ...
                 ! apply h or h**t
                 call stdlib_qlarfb( side, trans, 'BACKWARD', 'COLUMNWISE', mi, ni,ib, a( 1_ilp, i ), &
                           lda, work( iwt ), ldt, c, ldc,work, ldwork )
              end do
           end if
           work( 1_ilp ) = lwkopt
     ...
     !! as returned by DGEQLF.
        ...
        ! =====================================================================
           ...
           do i = 1, k
              ...
              ! apply h(i) to a(1:m-k+i,1:n-k+i) from the left
              a( m-n+ii, ii ) = one
              call stdlib_qlarf( 'LEFT', m-n+ii, ii-1, a( 1_ilp, ii ), 1_ilp, tau( i ), a,lda, work )
                        
              call stdlib_qscal( m-n+ii-1, -tau( i ), a( 1_ilp, ii ), 1_ilp )
              a( m-n+ii, ii ) = one - tau( i )
              ! set a(m-k+i+1:m,n-k+i) to zero
              do l = m - n + ii + 1, m
                 a( l, ii ) = zero
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           do i = i1, i2, i3
              ...
              aii = a( nq-k+i, i )
              a( nq-k+i, i ) = one
              call stdlib_qlarf( side, mi, ni, a( 1_ilp, i ), 1_ilp, tau( i ), c, ldc,work )
              a( nq-k+i, i ) = aii
           end do
           return
           ...
           if( left ) then
              if( notran ) then
                 do i = 1, n, nb
                    ...
                    ldwork = m
                    ! multiply bottom part of c by q12.
                    call stdlib_wlacpy( 'ALL', n1, len, c( n2+1, i ), ldc, work,ldwork )
                    call stdlib_wtrmm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'NON-UNIT',n1, len, cone, &
                              q( 1_ilp, n2+1 ), ldq, work,ldwork )
                    ! multiply top part of c by q11.
                    call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', n1, len, n2,cone, q, ldq, &
                              c( 1_ilp, i ), ldc, cone, work,ldwork )
                    ! multiply top part of c by q21.
                    call stdlib_wlacpy( 'ALL', n2, len, c( 1_ilp, i ), ldc,work( n1+1 ), ldwork )
                              
                    call stdlib_wtrmm( 'LEFT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT',n2, len, cone, &
                              q( n1+1, 1_ilp ), ldq,work( n1+1 ), ldwork )
                    ! multiply bottom part of c by q22.
                    call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', n2, len, n1,cone, q( n1+1, &
                              n2+1 ), ldq, c( n2+1, i ), ldc,cone, work( n1+1 ), ldwork )
                    ! copy everything back.
                    call stdlib_wlacpy( 'ALL', m, len, work, ldwork, c( 1_ilp, i ),ldc )
                 end do
              else
                 do i = 1, n, nb
                    len = min( nb, n-i+1 )
                    ldwork = m
                    ! multiply bottom part of c by q21**h.
                    call stdlib_wlacpy( 'ALL', n2, len, c( n1+1, i ), ldc, work,ldwork )
                    call stdlib_wtrmm( 'LEFT', 'UPPER', 'CONJUGATE', 'NON-UNIT',n2, len, cone, q( &
                              n1+1, 1_ilp ), ldq, work,ldwork )
                    ! multiply top part of c by q11**h.
                    call stdlib_wgemm( 'CONJUGATE', 'NO TRANSPOSE', n2, len, n1,cone, q, ldq, c( &
                              1_ilp, i ), ldc, cone, work,ldwork )
                    ! multiply top part of c by q12**h.
                    call stdlib_wlacpy( 'ALL', n1, len, c( 1_ilp, i ), ldc,work( n2+1 ), ldwork )
                              
                    call stdlib_wtrmm( 'LEFT', 'LOWER', 'CONJUGATE', 'NON-UNIT',n1, len, cone, q( &
                              1_ilp, n2+1 ), ldq,work( n2+1 ), ldwork )
                    ! multiply bottom part of c by q22**h.
                    call stdlib_wgemm( 'CONJUGATE', 'NO TRANSPOSE', n1, len, n2,cone, q( n1+1, n2+&
                              1_ilp ), ldq, c( n1+1, i ), ldc,cone, work( n2+1 ), ldwork )
                    ! copy everything back.
                    call stdlib_wlacpy( 'ALL', m, len, work, ldwork, c( 1_ilp, i ),ldc )
                 end do
              ...
              if( notran ) then
                 do i = 1, m, nb
                    ...
                    ldwork = len
                    ! multiply right part of c by q21.
                    call stdlib_wlacpy( 'ALL', len, n2, c( i, n1+1 ), ldc, work,ldwork )
                    call stdlib_wtrmm( 'RIGHT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT',len, n2, cone,&
                               q( n1+1, 1_ilp ), ldq, work,ldwork )
                    ! multiply left part of c by q11.
                    call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', len, n2, n1,cone, c( i, 1_ilp )&
                              , ldc, q, ldq, cone, work,ldwork )
                    ! multiply left part of c by q12.
                    call stdlib_wlacpy( 'ALL', len, n1, c( i, 1_ilp ), ldc,work( 1_ilp + n2*ldwork ), &
                              ldwork )
                    call stdlib_wtrmm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', 'NON-UNIT',len, n1, cone,&
                               q( 1_ilp, n2+1 ), ldq,work( 1_ilp + n2*ldwork ), ldwork )
                    ! multiply right part of c by q22.
                    call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', len, n1, n2,cone, c( i, n1+&
                              1_ilp ), ldc, q( n1+1, n2+1 ), ldq,cone, work( 1_ilp + n2*ldwork ), ldwork )
                    ! copy everything back.
                    call stdlib_wlacpy( 'ALL', len, n, work, ldwork, c( i, 1_ilp ),ldc )
                 end do
              else
                 do i = 1, m, nb
                    len = min( nb, m-i+1 )
                    ldwork = len
                    ! multiply right part of c by q12**h.
                    call stdlib_wlacpy( 'ALL', len, n1, c( i, n2+1 ), ldc, work,ldwork )
                    call stdlib_wtrmm( 'RIGHT', 'LOWER', 'CONJUGATE', 'NON-UNIT',len, n1, cone, q(&
                               1_ilp, n2+1 ), ldq, work,ldwork )
                    ! multiply left part of c by q11**h.
                    call stdlib_wgemm( 'NO TRANSPOSE', 'CONJUGATE', len, n1, n2,cone, c( i, 1_ilp ), &
                              ldc, q, ldq, cone, work,ldwork )
                    ! multiply left part of c by q21**h.
                    call stdlib_wlacpy( 'ALL', len, n2, c( i, 1_ilp ), ldc,work( 1_ilp + n1*ldwork ), &
                              ldwork )
                    call stdlib_wtrmm( 'RIGHT', 'UPPER', 'CONJUGATE', 'NON-UNIT',len, n2, cone, q(&
                               n1+1, 1_ilp ), ldq,work( 1_ilp + n1*ldwork ), ldwork )
                    ! multiply right part of c by q22**h.
                    call stdlib_wgemm( 'NO TRANSPOSE', 'CONJUGATE', len, n2, n1,cone, c( i, n2+1 )&
                              , ldc, q( n1+1, n2+1 ), ldq,cone, work( 1_ilp + n1*ldwork ), ldwork )
                    ! copy everything back.
                    call stdlib_wlacpy( 'ALL', len, n, work, ldwork, c( i, 1_ilp ),ldc )
                 end do
