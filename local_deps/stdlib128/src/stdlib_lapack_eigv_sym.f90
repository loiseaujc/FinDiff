  ...
  contains
     ...
     !! positive definite.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           ! form a cholesky factorization of b.
           call stdlib_qpotrf( uplo, n, b, ldb, info )
           if( info/=0_ilp ) then
              info = n + info
              return
           end if
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_qsygst( itype, uplo, n, a, lda, b, ldb, info )
           call stdlib_qsyev( jobz, uplo, n, a, lda, w, work, lwork, info )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              neig = n
              if( info>0_ilp )neig = info - 1_ilp
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_qsygst( itype, uplo, n, a, lda, b, ldb, info )
           call stdlib_qsyevd( jobz, uplo, n, a, lda, w, work, lwork, iwork, liwork,info )
           lopt = max( real( lopt,KIND=qp), real( work( 1_ilp ),KIND=qp) )
           liopt = max( real( liopt,KIND=qp), real( iwork( 1_ilp ),KIND=qp) )
           if( wantz .and. info==0_ilp ) then
              ! backtransform eigenvectors to the original problem.
              if( itype==1_ilp .or. itype==2_ilp ) then
                 ! for a*x=(lambda)*b*x and a*b*x=(lambda)*x;
     ...
     !! range of values or a range of indices for the desired eigenvalues.
       ...
       ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_qsygst( itype, uplo, n, a, lda, b, ldb, info )
           call stdlib_qsyevx( jobz, range, uplo, n, a, lda, vl, vu, il, iu, abstol,m, w, z, ldz, &
                     work, lwork, iwork, ifail, info )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              if( info>0_ilp )m = info - 1_ilp
     ...
     !! and B is also positive definite.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           ! form a cholesky factorization of b.
           call stdlib_qpptrf( uplo, n, bp, info )
           if( info/=0_ilp ) then
              info = n + info
              return
           end if
           ...
           call stdlib_qspev( jobz, uplo, n, ap, w, z, ldz, work, info )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              neig = n
              if( info>0_ilp )neig = info - 1_ilp
              if( itype==1_ilp .or. itype==2_ilp ) then
                 ...
                 do j = 1, neig
                    call stdlib_qtpsv( uplo, trans, 'NON-UNIT', n, bp, z( 1_ilp, j ),1_ilp )
                 end do
              else if( itype==3_ilp ) then
                 ! for b*a*x=(lambda)*x;
                 ! backtransform eigenvectors: x = l*y or u**t*y
                 ...
                 do j = 1, neig
                    call stdlib_qtpmv( uplo, trans, 'NON-UNIT', n, bp, z( 1_ilp, j ),1_ilp )
                 end do
              end if
           end if
           return
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           lwmin = max( real( lwmin,KIND=qp), real( work( 1_ilp ),KIND=qp) )
           liwmin = max( real( liwmin,KIND=qp), real( iwork( 1_ilp ),KIND=qp) )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              neig = n
              if( info>0_ilp )neig = info - 1_ilp
     ...
     !! for the desired eigenvalues.
       ...
       ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_qspgst( itype, uplo, n, ap, bp, info )
           call stdlib_qspevx( jobz, range, uplo, n, ap, vl, vu, il, iu, abstol, m,w, z, ldz, &
                     work, iwork, ifail, info )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              if( info>0_ilp )m = info - 1_ilp
     ...
     !! and banded, and B is also positive definite.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           ! form a split cholesky factorization of b.
           call stdlib_qpbstf( uplo, n, kb, bb, ldbb, info )
           if( info/=0_ilp ) then
              info = n + info
              return
           end if
           ...
           inde = 1_ilp
           indwrk = inde + n
           call stdlib_qsbgst( jobz, uplo, n, ka, kb, ab, ldab, bb, ldbb, z, ldz,work( indwrk ), &
                     iinfo )
           ! reduce to tridiagonal form.
           if( wantz ) then
              vect = 'U'
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           if( .not.wantz ) then
              ...
              call stdlib_qstedc( 'I', n, w, work( inde ), work( indwrk ), n,work( indwk2 ), &
                        llwrk2, iwork, liwork, info )
              call stdlib_qgemm( 'N', 'N', n, n, n, one, z, ldz, work( indwrk ), n,zero, work( &
                        indwk2 ), n )
              call stdlib_qlacpy( 'A', n, n, work( indwk2 ), n, z, ldz )
           end if
           work( 1_ilp ) = lwmin
           iwork( 1_ilp ) = liwmin
           return
     ...
     !! a range of values or a range of indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           if( ( alleig .or. test ) .and. ( abstol<=zero ) ) then
              call stdlib_qcopy( n, work( indd ), 1_ilp, w, 1_ilp )
              indee = indwrk + 2_ilp*n
              call stdlib_qcopy( n-1, work( inde ), 1_ilp, work( indee ), 1_ilp )
              if( .not.wantz ) then
           ...
           ! otherwise, call stdlib_qstebz and, if eigenvectors are desired,
           ! call stdlib_qstein.
           if( wantz ) then
              order = 'B'
           else
           ...
           if( wantz ) then
              call stdlib_qstein( n, work( indd ), work( inde ), m, w,iwork( indibl ), iwork( &
                        indisp ), z, ldz,work( indwrk ), iwork( indiwo ), ifail, info )
              ! apply transformation matrix used in reduction to tridiagonal
              ! form to eigenvectors returned by stdlib_qstein.
              do j = 1, m
                 call stdlib_qcopy( n, z( 1_ilp, j ), 1_ilp, work( 1_ilp ), 1_ilp )
                 call stdlib_qgemv( 'N', n, n, one, q, ldq, work, 1_ilp, zero,z( 1_ilp, j ), 1_ilp )
              end do
           end if
           30 continue
           ! if eigenvalues are not in order, then sort them, along with
           ...
           if( wantz ) then
              do j = 1, m - 1
                 ...
                 if( i/=0_ilp ) then
                    ...
                    w( j ) = tmp1
                    iwork( indibl+j-1 ) = itmp1
                    call stdlib_qswap( n, z( 1_ilp, i ), 1_ilp, z( 1_ilp, j ), 1_ilp )
                    if( info/=0_ilp ) then
                       itmp1 = ifail( i )
                       ifail( i ) = ifail( j )
                       ifail( j ) = itmp1
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do i = n - nb + 1, kk + 1, -nb
                 ...
                 ! matrix w which is needed to update the unreduced part of
                 ! the matrix
                 call stdlib_qlatrd( uplo, i+nb-1, nb, a, lda, e, tau, work,ldwork )
                 ! update the unreduced submatrix a(1:i-1,1:i-1), using an
                 ! update of the form:  a := a - v*w**t - w*v**t
                 call stdlib_qsyr2k( uplo, 'NO TRANSPOSE', i-1, nb, -one, a( 1_ilp, i ),lda, work, &
                           ldwork, one, a, lda )
                 ! copy superdiagonal elements back into a, and diagonal
                 ! elements into d
                 do j = i, i + nb - 1
              ...
              ! use unblocked code to reduce the last or only block
              call stdlib_qsytd2( uplo, kk, a, lda, d, e, tau, iinfo )
           else
              ! reduce the lower triangle of a
              do i = 1, n - nx, nb
                 ! reduce columns i:i+nb-1 to tridiagonal form and form the
                 ! matrix w which is needed to update the unreduced part of
                 ! the matrix
                 call stdlib_qlatrd( uplo, n-i+1, nb, a( i, i ), lda, e( i ),tau( i ), work, &
                           ldwork )
                 ! update the unreduced submatrix a(i+ib:n,i+ib:n), using
                 ! an update of the form:  a := a - v*w**t - w*v**t
                 call stdlib_qsyr2k( uplo, 'NO TRANSPOSE', n-i-nb+1, nb, -one,a( i+nb, i ), lda, &
                           work( nb+1 ), ldwork, one,a( i+nb, i+nb ), lda )
                 ! copy subdiagonal elements back into a, and diagonal
                 ! elements into d
                 do j = i, i + nb - 1
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do i = n - 1, 1, -1
                 ! generate elementary reflector h(i) = i - tau * v * v**t
                 ! to annihilate a(1:i-1,i+1)
                 call stdlib_qlarfg( i, a( i, i+1 ), a( 1_ilp, i+1 ), 1_ilp, taui )
                 e( i ) = a( i, i+1 )
                 if( taui/=zero ) then
                    ! apply h(i) from both sides to a(1:i,1:i)
                    a( i, i+1 ) = one
                    ! compute  x := tau * a * v  storing x in tau(1:i)
                    call stdlib_qsymv( uplo, i, taui, a, lda, a( 1_ilp, i+1 ), 1_ilp, zero,tau, 1_ilp )
                              
                    ! compute  w := x - 1/2 * tau * (x**t * v) * v
                    alpha = -half*taui*stdlib_qdot( i, tau, 1_ilp, a( 1_ilp, i+1 ), 1_ilp )
                    call stdlib_qaxpy( i, alpha, a( 1_ilp, i+1 ), 1_ilp, tau, 1_ilp )
                    ! apply the transformation as a rank-2 update:
                       ! a := a - v * w**t - w * v**t
                    call stdlib_qsyr2( uplo, i, -one, a( 1_ilp, i+1 ), 1_ilp, tau, 1_ilp, a,lda )
                    a( i, i+1 ) = e( i )
                 end if
                 d( i+1 ) = a( i+1, i+1 )
                 tau( i ) = taui
              ...
              do i = 1, n - 1
                 ! generate elementary reflector h(i) = i - tau * v * v**t
                 ! to annihilate a(i+2:n,i)
                 call stdlib_qlarfg( n-i, a( i+1, i ), a( min( i+2, n ), i ), 1_ilp,taui )
                 e( i ) = a( i+1, i )
                 if( taui/=zero ) then
                    ! apply h(i) from both sides to a(i+1:n,i+1:n)
                    a( i+1, i ) = one
                    ! compute  x := tau * a * v  storing y in tau(i:n-1)
                    call stdlib_qsymv( uplo, n-i, taui, a( i+1, i+1 ), lda,a( i+1, i ), 1_ilp, zero, &
                              tau( i ), 1_ilp )
                    ! compute  w := x - 1/2 * tau * (x**t * v) * v
                    alpha = -half*taui*stdlib_qdot( n-i, tau( i ), 1_ilp, a( i+1, i ),1_ilp )
                    call stdlib_qaxpy( n-i, alpha, a( i+1, i ), 1_ilp, tau( i ), 1_ilp )
                    ! apply the transformation as a rank-2 update:
                       ! a := a - v * w**t - w * v**t
                    call stdlib_qsyr2( uplo, n-i, -one, a( i+1, i ), 1_ilp, tau( i ), 1_ilp,a( i+1, i+1 ),&
                               lda )
                    a( i+1, i ) = e( i )
                 end if
                 d( i ) = a( i, i )
     ...
     !! if UPLO = 'L', Q = H(1) H(2) . . . H(n-1).
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ! q was determined by a call to stdlib_qsytrd with uplo = 'u'
              ! shift the vectors which define the elementary reflectors one
              ! column to the left, and set the last row and column of q to
              ! those of the unit matrix
              do j = 1, n - 1
              ...
              a( n, n ) = one
              ! generate q(1:n-1,1:n-1)
              call stdlib_qorgql( n-1, n-1, n-1, a, lda, tau, work, lwork, iinfo )
           else
              ! q was determined by a call to stdlib_qsytrd with uplo = 'l'.
              ! shift the vectors which define the elementary reflectors one
              ! column to the right, and set the first row and column of q to
              ! those of the unit matrix
              do j = n, 2, -1
              ...
              if( n>1_ilp ) then
                 ! generate q(2:n,2:n)
                 call stdlib_qorgqr( n-1, n-1, n-1, a( 2_ilp, 2_ilp ), lda, tau, work,lwork, iinfo )
     ...
     !! if UPLO = 'L', Q = H(1) H(2) . . . H(nq-1).
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ! q was determined by a call to stdlib_qsytrd with uplo = 'u'
              call stdlib_qormql( side, trans, mi, ni, nq-1, a( 1_ilp, 2_ilp ), lda, tau, c,ldc, work, &
                        lwork, iinfo )
           else
              ! q was determined by a call to stdlib_qsytrd with uplo = 'l'
              if( left ) then
              ...
              call stdlib_qormqr( side, trans, mi, ni, nq-1, a( 2_ilp, 1_ilp ), lda, tau,c( i1, i2 ), ldc,&
                         work, lwork, iinfo )
           end if
           work( 1_ilp ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( upper ) then
               ...
               if( ttype==1_ilp ) then
                   ...
                   ctmp = ( a( ofdpos, st ) )
                   call stdlib_qlarfg( lm, ctmp, v( vpos+1 ), 1_ilp,tau( taupos ) )
                   a( ofdpos, st ) = ctmp
                   lm = ed - st + 1_ilp
                   call stdlib_qlarfy( uplo, lm, v( vpos ), 1_ilp,( tau( taupos ) ),a( dpos, st ), &
                             lda-1, work)
               endif
               if( ttype==3_ilp ) then
                   lm = ed - st + 1_ilp
               ...
               if( ttype==2_ilp ) then
                   ...
                   lm = j2-j1+1
                   if( lm>0_ilp) then
                       call stdlib_qlarfx( 'LEFT', ln, lm, v( vpos ),( tau( taupos ) ),a( dpos-nb,&
                                  j1 ), lda-1, work)
                       if( wantz ) then
                           vpos   = mod( sweep-1, 2_ilp ) * n + j1
                           taupos = mod( sweep-1, 2_ilp ) * n + j1
                       ...
                       call stdlib_qlarfg( lm, ctmp, v( vpos+1 ), 1_ilp, tau( taupos ) )
                       a( dpos-nb, j1 ) = ctmp
                       call stdlib_qlarfx( 'RIGHT', ln-1, lm, v( vpos ),tau( taupos ),a( dpos-nb+&
                                 1_ilp, j1 ), lda-1, work)
                   endif
               endif
           ! lower case
               ...
               if( ttype==1_ilp ) then
                   ...
                   call stdlib_qlarfg( lm, a( ofdpos, st-1 ), v( vpos+1 ), 1_ilp,tau( taupos ) )
               ...
               if( ttype==2_ilp ) then
                   ...
                   lm = j2-j1+1
                   if( lm>0_ilp) then
                       call stdlib_qlarfx( 'RIGHT', lm, ln, v( vpos ),tau( taupos ), a( dpos+nb, &
                                 st ),lda-1, work)
                       if( wantz ) then
                           vpos   = mod( sweep-1, 2_ilp ) * n + j1
                           taupos = mod( sweep-1, 2_ilp ) * n + j1
                       ...
                       call stdlib_qlarfg( lm, a( dpos+nb, st ), v( vpos+1 ), 1_ilp,tau( taupos ) )
     ...
     !! positive definite.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           ! form a cholesky factorization of b.
           call stdlib_wpotrf( uplo, n, b, ldb, info )
           if( info/=0_ilp ) then
              info = n + info
              return
           end if
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_whegst( itype, uplo, n, a, lda, b, ldb, info )
           call stdlib_wheev( jobz, uplo, n, a, lda, w, work, lwork, rwork, info )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              neig = n
              if( info>0_ilp )neig = info - 1_ilp
              if( itype==1_ilp .or. itype==2_ilp ) then
                 ...
                 call stdlib_wtrsm( 'LEFT', uplo, trans, 'NON-UNIT', n, neig, cone,b, ldb, a, lda &
                           )
              else if( itype==3_ilp ) then
                 ! for b*a*x=(lambda)*x;
                 ! backtransform eigenvectors: x = l*y or u**h *y
                 ...
                 call stdlib_wtrmm( 'LEFT', uplo, trans, 'NON-UNIT', n, neig, cone,b, ldb, a, lda &
                           )
              end if
           end if
           work( 1_ilp ) = lwkopt
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_whegst( itype, uplo, n, a, lda, b, ldb, info )
           call stdlib_wheevd( jobz, uplo, n, a, lda, w, work, lwork, rwork, lrwork,iwork, liwork,&
                      info )
           lopt = max( real( lopt,KIND=qp), real( work( 1_ilp ),KIND=qp) )
           lropt = max( real( lropt,KIND=qp), real( rwork( 1_ilp ),KIND=qp) )
           liopt = max( real( liopt,KIND=qp), real( iwork( 1_ilp ),KIND=qp) )
           if( wantz .and. info==0_ilp ) then
              ! backtransform eigenvectors to the original problem.
              if( itype==1_ilp .or. itype==2_ilp ) then
     ...
     !! range of values or a range of indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_whegst( itype, uplo, n, a, lda, b, ldb, info )
           call stdlib_wheevx( jobz, range, uplo, n, a, lda, vl, vu, il, iu, abstol,m, w, z, ldz, &
                     work, lwork, rwork, iwork, ifail,info )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              if( info>0_ilp )m = info - 1_ilp
     ...
     !! and B is also positive definite.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           ! form a cholesky factorization of b.
           call stdlib_wpptrf( uplo, n, bp, info )
           if( info/=0_ilp ) then
              info = n + info
              return
           end if
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_whpgst( itype, uplo, n, ap, bp, info )
           call stdlib_whpev( jobz, uplo, n, ap, w, z, ldz, work, rwork, info )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              neig = n
              if( info>0_ilp )neig = info - 1_ilp
              if( itype==1_ilp .or. itype==2_ilp ) then
                 ...
                 do j = 1, neig
                    call stdlib_wtpsv( uplo, trans, 'NON-UNIT', n, bp, z( 1_ilp, j ),1_ilp )
                 end do
              else if( itype==3_ilp ) then
                 ! for b*a*x=(lambda)*x;
                 ! backtransform eigenvectors: x = l*y or u**h *y
                 ...
                 do j = 1, neig
                    call stdlib_wtpmv( uplo, trans, 'NON-UNIT', n, bp, z( 1_ilp, j ),1_ilp )
                 end do
              end if
           end if
           return
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_whpgst( itype, uplo, n, ap, bp, info )
           call stdlib_whpevd( jobz, uplo, n, ap, w, z, ldz, work, lwork, rwork,lrwork, iwork, &
                     liwork, info )
           lwmin = max( real( lwmin,KIND=qp), real( work( 1_ilp ),KIND=qp) )
           lrwmin = max( real( lrwmin,KIND=qp), real( rwork( 1_ilp ),KIND=qp) )
           liwmin = max( real( liwmin,KIND=qp), real( iwork( 1_ilp ),KIND=qp) )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              neig = n
     ...
     !! desired eigenvalues.
        ...
        ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem and solve.
           call stdlib_whpgst( itype, uplo, n, ap, bp, info )
           call stdlib_whpevx( jobz, range, uplo, n, ap, vl, vu, il, iu, abstol, m,w, z, ldz, &
                     work, rwork, iwork, ifail, info )
           if( wantz ) then
              ! backtransform eigenvectors to the original problem.
              if( info>0_ilp )m = info - 1_ilp
     ...
     !! and banded, and B is also positive definite.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           ! form a split cholesky factorization of b.
           call stdlib_wpbstf( uplo, n, kb, bb, ldbb, info )
           if( info/=0_ilp ) then
              info = n + info
              return
           end if
           ...
           inde = 1_ilp
           indwrk = inde + n
           call stdlib_whbgst( jobz, uplo, n, ka, kb, ab, ldab, bb, ldbb, z, ldz,work, rwork( &
                     indwrk ), iinfo )
           ! reduce to tridiagonal form.
           if( wantz ) then
              vect = 'U'
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           if( .not.wantz ) then
              ...
              call stdlib_wstedc( 'I', n, w, rwork( inde ), work, n, work( indwk2 ),llwk2, rwork( &
                        indwrk ), llrwk, iwork, liwork,info )
              call stdlib_wgemm( 'N', 'N', n, n, n, cone, z, ldz, work, n, czero,work( indwk2 ), &
                        n )
              call stdlib_wlacpy( 'A', n, n, work( indwk2 ), n, z, ldz )
           end if
           work( 1_ilp ) = lwmin
           rwork( 1_ilp ) = lrwmin
           iwork( 1_ilp ) = liwmin
     ...
     !! a range of values or a range of indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           ! transform problem to standard eigenvalue problem.
           call stdlib_whbgst( jobz, uplo, n, ka, kb, ab, ldab, bb, ldbb, q, ldq,work, rwork, &
                     iinfo )
           ! solve the standard eigenvalue problem.
           ! reduce hermitian band matrix to tridiagonal form.
           indd = 1_ilp
           ...
           call stdlib_whbtrd( vect, uplo, n, ka, ab, ldab, rwork( indd ),rwork( inde ), q, ldq, &
                     work( indwrk ), iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal
           ! to zero, then call stdlib_qsterf or stdlib_wsteqr.  if this fails for some
           ! eigenvalue, then try stdlib_qstebz.
           test = .false.
           if( indeig ) then
              if( il==1_ilp .and. iu==n ) then
           ...
           if( ( alleig .or. test ) .and. ( abstol<=zero ) ) then
              call stdlib_qcopy( n, rwork( indd ), 1_ilp, w, 1_ilp )
              indee = indrwk + 2_ilp*n
              call stdlib_qcopy( n-1, rwork( inde ), 1_ilp, rwork( indee ), 1_ilp )
              if( .not.wantz ) then
           ...
           indisp = indibl + n
           indiwk = indisp + n
           call stdlib_qstebz( range, order, n, vl, vu, il, iu, abstol,rwork( indd ), rwork( inde &
           ), m, nsplit, w,iwork( indibl ), iwork( indisp ), rwork( indrwk ),iwork( indiwk ), &
                     info )
           if( wantz ) then
              call stdlib_wstein( n, rwork( indd ), rwork( inde ), m, w,iwork( indibl ), iwork( &
                        indisp ), z, ldz,rwork( indrwk ), iwork( indiwk ), ifail, info )
              ! apply unitary matrix used in reduction to tridiagonal
              ! form to eigenvectors returned by stdlib_wstein.
              do j = 1, m
                 call stdlib_wcopy( n, z( 1_ilp, j ), 1_ilp, work( 1_ilp ), 1_ilp )
                 call stdlib_wgemv( 'N', n, n, cone, q, ldq, work, 1_ilp, czero,z( 1_ilp, j ), 1_ilp )
              end do
           end if
           30 continue
           ! if eigenvalues are not in order, then sort them, along with
           ...
           if( wantz ) then
              do j = 1, m - 1
                 ...
                 if( i/=0_ilp ) then
                    ...
                    w( j ) = tmp1
                    iwork( indibl+j-1 ) = itmp1
                    call stdlib_wswap( n, z( 1_ilp, i ), 1_ilp, z( 1_ilp, j ), 1_ilp )
                    if( info/=0_ilp ) then
                       itmp1 = ifail( i )
                       ifail( i ) = ifail( j )
                       ifail( j ) = itmp1
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do i = n - nb + 1, kk + 1, -nb
                 ...
                 ! matrix w which is needed to update the unreduced part of
                 ! the matrix
                 call stdlib_wlatrd( uplo, i+nb-1, nb, a, lda, e, tau, work,ldwork )
                 ! update the unreduced submatrix a(1:i-1,1:i-1), using an
                 ! update of the form:  a := a - v*w**h - w*v**h
                 call stdlib_wher2k( uplo, 'NO TRANSPOSE', i-1, nb, -cone,a( 1_ilp, i ), lda, work, &
                           ldwork, one, a, lda )
                 ! copy superdiagonal elements back into a, and diagonal
                 ! elements into d
                 do j = i, i + nb - 1
              ...
              ! use unblocked code to reduce the last or only block
              call stdlib_whetd2( uplo, kk, a, lda, d, e, tau, iinfo )
           else
              ! reduce the lower triangle of a
              do i = 1, n - nx, nb
                 ! reduce columns i:i+nb-1 to tridiagonal form and form the
                 ! matrix w which is needed to update the unreduced part of
                 ! the matrix
                 call stdlib_wlatrd( uplo, n-i+1, nb, a( i, i ), lda, e( i ),tau( i ), work, &
                           ldwork )
                 ! update the unreduced submatrix a(i+nb:n,i+nb:n), using
                 ! an update of the form:  a := a - v*w**h - w*v**h
                 call stdlib_wher2k( uplo, 'NO TRANSPOSE', n-i-nb+1, nb, -cone,a( i+nb, i ), lda, &
                           work( nb+1 ), ldwork, one,a( i+nb, i+nb ), lda )
                 ! copy subdiagonal elements back into a, and diagonal
                 ! elements into d
                 do j = i, i + nb - 1
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do i = n - 1, 1, -1
                 ...
                 ! to annihilate a(1:i-1,i+1)
                 alpha = a( i, i+1 )
                 call stdlib_wlarfg( i, alpha, a( 1_ilp, i+1 ), 1_ilp, taui )
                 e( i ) = real( alpha,KIND=qp)
                 if( taui/=czero ) then
                    ! apply h(i) from both sides to a(1:i,1:i)
                    a( i, i+1 ) = cone
                    ! compute  x := tau * a * v  storing x in tau(1:i)
                    call stdlib_whemv( uplo, i, taui, a, lda, a( 1_ilp, i+1 ), 1_ilp, czero,tau, 1_ilp )
                              
                    ! compute  w := x - 1/2 * tau * (x**h * v) * v
                    alpha = -chalf*taui*stdlib_wdotc( i, tau, 1_ilp, a( 1_ilp, i+1 ), 1_ilp )
                    call stdlib_waxpy( i, alpha, a( 1_ilp, i+1 ), 1_ilp, tau, 1_ilp )
                    ! apply the transformation as a rank-2 update:
                       ! a := a - v * w**h - w * v**h
                    call stdlib_wher2( uplo, i, -cone, a( 1_ilp, i+1 ), 1_ilp, tau, 1_ilp, a,lda )
                 else
              ...
              d( 1_ilp ) = real( a( 1_ilp, 1_ilp ),KIND=qp)
           else
              ! reduce the lower triangle of a
              a( 1_ilp, 1_ilp ) = real( a( 1_ilp, 1_ilp ),KIND=qp)
              do i = 1, n - 1
                 ! generate elementary reflector h(i) = i - tau * v * v**h
                 ! to annihilate a(i+2:n,i)
                 alpha = a( i+1, i )
                 call stdlib_wlarfg( n-i, alpha, a( min( i+2, n ), i ), 1_ilp, taui )
                 e( i ) = real( alpha,KIND=qp)
                 if( taui/=czero ) then
                    ! apply h(i) from both sides to a(i+1:n,i+1:n)
                    a( i+1, i ) = cone
                    ! compute  x := tau * a * v  storing y in tau(i:n-1)
                    call stdlib_whemv( uplo, n-i, taui, a( i+1, i+1 ), lda,a( i+1, i ), 1_ilp, czero, &
                              tau( i ), 1_ilp )
                    ! compute  w := x - 1/2 * tau * (x**h * v) * v
                    alpha = -chalf*taui*stdlib_wdotc( n-i, tau( i ), 1_ilp, a( i+1, i ),1_ilp )
                    call stdlib_waxpy( n-i, alpha, a( i+1, i ), 1_ilp, tau( i ), 1_ilp )
                    ! apply the transformation as a rank-2 update:
                       ! a := a - v * w**h - w * v**h
                    call stdlib_wher2( uplo, n-i, -cone, a( i+1, i ), 1_ilp, tau( i ), 1_ilp,a( i+1, i+1 )&
                              , lda )
                 else
     ...
     !! if UPLO = 'L', Q = H(1) H(2) . . . H(n-1).
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ! q was determined by a call to stdlib_whetrd with uplo = 'u'
              ! shift the vectors which define the elementary reflectors cone
              ! column to the left, and set the last row and column of q to
              ! those of the unit matrix
              do j = 1, n - 1
              ...
              a( n, n ) = cone
              ! generate q(1:n-1,1:n-1)
              call stdlib_wungql( n-1, n-1, n-1, a, lda, tau, work, lwork, iinfo )
           else
              ! q was determined by a call to stdlib_whetrd with uplo = 'l'.
              ! shift the vectors which define the elementary reflectors cone
              ! column to the right, and set the first row and column of q to
              ! those of the unit matrix
              do j = n, 2, -1
              ...
              if( n>1_ilp ) then
                 ! generate q(2:n,2:n)
                 call stdlib_wungqr( n-1, n-1, n-1, a( 2_ilp, 2_ilp ), lda, tau, work,lwork, iinfo )
     ...
     !! if UPLO = 'L', Q = H(1) H(2) . . . H(nq-1).
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ! q was determined by a call to stdlib_whetrd with uplo = 'u'
              call stdlib_wunmql( side, trans, mi, ni, nq-1, a( 1_ilp, 2_ilp ), lda, tau, c,ldc, work, &
                        lwork, iinfo )
           else
              ! q was determined by a call to stdlib_whetrd with uplo = 'l'
              if( left ) then
              ...
              call stdlib_wunmqr( side, trans, mi, ni, nq-1, a( 2_ilp, 1_ilp ), lda, tau,c( i1, i2 ), ldc,&
                         work, lwork, iinfo )
           end if
           work( 1_ilp ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( n<=kd+1 ) then
               if( upper ) then
                   do i = 1, n
                       llk = min( kd+1, i )
                       call stdlib_wcopy( llk, a( i-llk+1, i ), 1_ilp,ab( kd+1-llk+1, i ), 1_ilp )
                   end do
               else
                   do i = 1, n
                       llk = min( kd+1, n-i+1 )
                       call stdlib_wcopy( llk, a( i, i ), 1_ilp, ab( 1_ilp, i ), 1_ilp )
                   end do
               endif
               work( 1_ilp ) = 1_ilp
               return
           ...
           ! set the workspace of the triangular matrix t to czero once such a
           ! way every time t is generated the upper/lower portion will be always czero
           call stdlib_wlaset( "A", ldt, kd, czero, czero, work( tpos ), ldt )
           if( upper ) then
               do i = 1, n - kd, kd
                  pn = n-i-kd+1
                  pk = min( n-i-kd+1, kd )
                  ! compute the lq factorization of the current block
                  call stdlib_wgelqf( kd, pn, a( i, i+kd ), lda,tau( i ), work( s2pos ), ls2, &
                            iinfo )
                  ! copy the upper portion of a into ab
                  do j = i, i+pk-1
                     llk = min( kd, n-j ) + 1_ilp
                  ...
                  ! form the matrix t
                  call stdlib_wlarft( 'FORWARD', 'ROWWISE', pn, pk,a( i, i+kd ), lda, tau( i ),&
                            work( tpos ), ldt )
                  ! compute w:
                  call stdlib_wgemm( 'CONJUGATE', 'NO TRANSPOSE', pk, pn, pk,cone,  work( tpos ), &
                            ldt,a( i, i+kd ), lda,czero, work( s2pos ), lds2 )
                  call stdlib_whemm( 'RIGHT', uplo, pk, pn,cone,  a( i+kd, i+kd ), lda,work( &
                            s2pos ), lds2,czero, work( wpos ), ldw )
                  call stdlib_wgemm( 'NO TRANSPOSE', 'CONJUGATE', pk, pk, pn,cone,  work( wpos ), &
                            ldw,work( s2pos ), lds2,czero, work( s1pos ), lds1 )
                  call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', pk, pn, pk,-chalf, work( &
                            s1pos ), lds1,a( i, i+kd ), lda,cone,   work( wpos ), ldw )
                  ! update the unreduced submatrix a(i+kd:n,i+kd:n), using
                  ! an update of the form:  a := a - v'*w - w'*v
                  call stdlib_wher2k( uplo, 'CONJUGATE', pn, pk,-cone, a( i, i+kd ), lda,work( &
                            wpos ), ldw,one, a( i+kd, i+kd ), lda )
               end do
              ! copy the upper band to ab which is the band storage matrix
              do j = n-kd+1, n
               ...
               loop_40: do i = 1, n - kd, kd
                  ...
                  pk = min( n-i-kd+1, kd )
                  ! compute the qr factorization of the current block
                  call stdlib_wgeqrf( pn, kd, a( i+kd, i ), lda,tau( i ), work( s2pos ), ls2, &
                            iinfo )
                  ! copy the upper portion of a into ab
                  do j = i, i+pk-1
                     llk = min( kd, n-j ) + 1_ilp
                     call stdlib_wcopy( llk, a( j, j ), 1_ilp, ab( 1_ilp, j ), 1_ilp )
                  end do
                  call stdlib_wlaset( 'UPPER', pk, pk, czero, cone,a( i+kd, i ), lda )
                  ! form the matrix t
                  call stdlib_wlarft( 'FORWARD', 'COLUMNWISE', pn, pk,a( i+kd, i ), lda, tau( i ),&
                            work( tpos ), ldt )
                  ! compute w:
                  call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', pn, pk, pk,cone, a( i+kd, i )&
                            , lda,work( tpos ), ldt,czero, work( s2pos ), lds2 )
                  call stdlib_whemm( 'LEFT', uplo, pn, pk,cone, a( i+kd, i+kd ), lda,work( s2pos )&
                            , lds2,czero, work( wpos ), ldw )
                  call stdlib_wgemm( 'CONJUGATE', 'NO TRANSPOSE', pk, pk, pn,cone, work( s2pos ), &
                            lds2,work( wpos ), ldw,czero, work( s1pos ), lds1 )
                  call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', pn, pk, pk,-chalf, a( i+kd, &
                            i ), lda,work( s1pos ), lds1,cone, work( wpos ), ldw )
                  ! update the unreduced submatrix a(i+kd:n,i+kd:n), using
                  ! an update of the form:  a := a - v*w' - w*v'
                  call stdlib_wher2k( uplo, 'NO TRANSPOSE', pn, pk,-cone, a( i+kd, i ), lda,work( &
                            wpos ), ldw,one, a( i+kd, i+kd ), lda )
                  ! ==================================================================
                  ! restore a for comparison and checking to be removed
                   ! do 45 j = i, i+pk-1
           ...
           if( kd==1_ilp ) then
               ...
               if( upper ) then
                   do i = 1, n - 1
                       ...
                       if( i<n-1 )ab( abofdpos, i+2 ) = ab( abofdpos, i+2 )*tmp
                        ! if( wantz ) then
                           ! call stdlib_wscal( n, conjg( tmp ), q( 1, i+1 ), 1 )
                        ! end if
                   end do
               else
                   do i = 1, n - 1
                      ...
                      if( i<n-1 )ab( abofdpos, i+1 ) = ab( abofdpos, i+1 )*tmp
                       ! if( wantq ) then
                          ! call stdlib_wscal( n, tmp, q( 1, i+1 ), 1 )
                       ! end if
                   end do
               endif
               hous( 1_ilp ) = 1_ilp
        ...
        ! =====================================================================
           ...
           if( upper ) then
               ...
               if( ttype==1_ilp ) then
                   ...
                   ctmp = conjg( a( ofdpos, st ) )
                   call stdlib_wlarfg( lm, ctmp, v( vpos+1 ), 1_ilp,tau( taupos ) )
                   a( ofdpos, st ) = ctmp
                   lm = ed - st + 1_ilp
                   call stdlib_wlarfy( uplo, lm, v( vpos ), 1_ilp,conjg( tau( taupos ) ),a( dpos, st )&
                             , lda-1, work)
               endif
               if( ttype==3_ilp ) then
                   lm = ed - st + 1_ilp
               ...
               if( ttype==2_ilp ) then
                   ...
                   lm = j2-j1+1
                   if( lm>0_ilp) then
                       call stdlib_wlarfx( 'LEFT', ln, lm, v( vpos ),conjg( tau( taupos ) ),a( &
                                 dpos-nb, j1 ), lda-1, work)
                       if( wantz ) then
                           vpos   = mod( sweep-1, 2_ilp ) * n + j1
                           taupos = mod( sweep-1, 2_ilp ) * n + j1
                       ...
                       call stdlib_wlarfg( lm, ctmp, v( vpos+1 ), 1_ilp, tau( taupos ) )
                       a( dpos-nb, j1 ) = ctmp
                       call stdlib_wlarfx( 'RIGHT', ln-1, lm, v( vpos ),tau( taupos ),a( dpos-nb+&
                                 1_ilp, j1 ), lda-1, work)
                   endif
               endif
           ! lower case
               ...
               if( ttype==1_ilp ) then
                   ...
                   call stdlib_wlarfg( lm, a( ofdpos, st-1 ), v( vpos+1 ), 1_ilp,tau( taupos ) )
               ...
               if( ttype==2_ilp ) then
                   ...
                   lm = j2-j1+1
                   if( lm>0_ilp) then
                       call stdlib_wlarfx( 'RIGHT', lm, ln, v( vpos ),tau( taupos ), a( dpos+nb, &
                                 st ),lda-1, work)
                       if( wantz ) then
                           vpos   = mod( sweep-1, 2_ilp ) * n + j1
                           taupos = mod( sweep-1, 2_ilp ) * n + j1
                       ...
                       call stdlib_wlarfg( lm, a( dpos+nb, st ), v( vpos+1 ), 1_ilp,tau( taupos ) )
        ...
        ! =====================================================================
           ...
           if( n<=kd+1 ) then
               if( upper ) then
                   do i = 1, n
                       llk = min( kd+1, i )
                       call stdlib_qcopy( llk, a( i-llk+1, i ), 1_ilp,ab( kd+1-llk+1, i ), 1_ilp )
                   end do
               else
                   do i = 1, n
                       llk = min( kd+1, n-i+1 )
                       call stdlib_qcopy( llk, a( i, i ), 1_ilp, ab( 1_ilp, i ), 1_ilp )
                   end do
               endif
               work( 1_ilp ) = 1_ilp
               return
           ...
           ! set the workspace of the triangular matrix t to zero once such a
           ! way every time t is generated the upper/lower portion will be always zero
           call stdlib_qlaset( "A", ldt, kd, zero, zero, work( tpos ), ldt )
           if( upper ) then
               do i = 1, n - kd, kd
                  pn = n-i-kd+1
                  pk = min( n-i-kd+1, kd )
                  ! compute the lq factorization of the current block
                  call stdlib_qgelqf( kd, pn, a( i, i+kd ), lda,tau( i ), work( s2pos ), ls2, &
                            iinfo )
                  ! copy the upper portion of a into ab
                  do j = i, i+pk-1
                     llk = min( kd, n-j ) + 1_ilp
                  ...
                  ! form the matrix t
                  call stdlib_qlarft( 'FORWARD', 'ROWWISE', pn, pk,a( i, i+kd ), lda, tau( i ),&
                            work( tpos ), ldt )
                  ! compute w:
                  call stdlib_qgemm( 'CONJUGATE', 'NO TRANSPOSE', pk, pn, pk,one,  work( tpos ), &
                            ldt,a( i, i+kd ), lda,zero, work( s2pos ), lds2 )
                  call stdlib_qsymm( 'RIGHT', uplo, pk, pn,one,  a( i+kd, i+kd ), lda,work( s2pos &
                            ), lds2,zero, work( wpos ), ldw )
                  call stdlib_qgemm( 'NO TRANSPOSE', 'CONJUGATE', pk, pk, pn,one,  work( wpos ), &
                            ldw,work( s2pos ), lds2,zero, work( s1pos ), lds1 )
                  call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', pk, pn, pk,-half, work( &
                            s1pos ), lds1,a( i, i+kd ), lda,one,   work( wpos ), ldw )
                  ! update the unreduced submatrix a(i+kd:n,i+kd:n), using
                  ! an update of the form:  a := a - v'*w - w'*v
                  call stdlib_qsyr2k( uplo, 'CONJUGATE', pn, pk,-one, a( i, i+kd ), lda,work( &
                            wpos ), ldw,one, a( i+kd, i+kd ), lda )
               end do
              ! copy the upper band to ab which is the band storage matrix
              do j = n-kd+1, n
               ...
               loop_40: do i = 1, n - kd, kd
                  ...
                  pk = min( n-i-kd+1, kd )
                  ! compute the qr factorization of the current block
                  call stdlib_qgeqrf( pn, kd, a( i+kd, i ), lda,tau( i ), work( s2pos ), ls2, &
                            iinfo )
                  ! copy the upper portion of a into ab
                  do j = i, i+pk-1
                     llk = min( kd, n-j ) + 1_ilp
                     call stdlib_qcopy( llk, a( j, j ), 1_ilp, ab( 1_ilp, j ), 1_ilp )
                  end do
                  call stdlib_qlaset( 'UPPER', pk, pk, zero, one,a( i+kd, i ), lda )
                  ! form the matrix t
                  call stdlib_qlarft( 'FORWARD', 'COLUMNWISE', pn, pk,a( i+kd, i ), lda, tau( i ),&
                            work( tpos ), ldt )
                  ! compute w:
                  call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', pn, pk, pk,one, a( i+kd, i ),&
                             lda,work( tpos ), ldt,zero, work( s2pos ), lds2 )
                  call stdlib_qsymm( 'LEFT', uplo, pn, pk,one, a( i+kd, i+kd ), lda,work( s2pos ),&
                             lds2,zero, work( wpos ), ldw )
                  call stdlib_qgemm( 'CONJUGATE', 'NO TRANSPOSE', pk, pk, pn,one, work( s2pos ), &
                            lds2,work( wpos ), ldw,zero, work( s1pos ), lds1 )
                  call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', pn, pk, pk,-half, a( i+kd, i &
                            ), lda,work( s1pos ), lds1,one, work( wpos ), ldw )
                  ! update the unreduced submatrix a(i+kd:n,i+kd:n), using
                  ! an update of the form:  a := a - v*w' - w*v'
                  call stdlib_qsyr2k( uplo, 'NO TRANSPOSE', pn, pk,-one, a( i+kd, i ), lda,work( &
                            wpos ), ldw,one, a( i+kd, i+kd ), lda )
                  ! ==================================================================
                  ! restore a for comparison and checking to be removed
                   ! do 45 j = i, i+pk-1
