  ...
  contains
     ...
     !! in routine DTRSNA.
       ...
       ! =====================================================================
           ...
           real(qp) :: bignum, eps, rec, scaloc, si, smin, sminw, smlnum, sr, tjj, tmp, xj, xmax, &
                     xnorm, z
           ! Local Arrays 
           real(qp) :: d(2_ilp,2_ilp), v(2_ilp,2_ilp)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! do not test the input parameters for errors
           notran = .not.ltran
           ...
           ! set constants to control overflow
           eps = stdlib_qlamch( 'P' )
           smlnum = stdlib_qlamch( 'S' ) / eps
           bignum = one / smlnum
           xnorm = stdlib_qlange( 'M', n, n, t, ldt, d )
           if( .not.lreal )xnorm = max( xnorm, abs( w ), stdlib_qlange( 'M', n, 1_ilp, b, n, d ) )
           ...
           work( 1_ilp ) = zero
           do j = 2, n
              work( j ) = stdlib_qasum( j-1, t( 1_ilp, j ), 1_ilp )
           end do
           if( .not.lreal ) then
              do i = 2, n
                 work( i ) = work( i ) + abs( b( i ) )
           ...
           n1 = n
           if( .not.lreal )n1 = n2
           k = stdlib_iqamax( n1, x, 1_ilp )
           xmax = abs( x( k ) )
           scale = one
           if( xmax>bignum ) then
              scale = bignum / xmax
              call stdlib_qscal( n1, scale, x, 1_ilp )
              xmax = bignum
           end if
           if( lreal ) then
              if( notran ) then
                 ...
                 loop_30: do j = n, 1, -1
                    ...
                    if( j1==j2 ) then
                       ...
                       if( tjj<one ) then
                          if( xj>bignum*tjj ) then
                             rec = one / xj
                             call stdlib_qscal( n, rec, x, 1_ilp )
                             scale = scale*rec
                             xmax = xmax*rec
                          end if
                       end if
                       ...
                       if( j1>1_ilp ) then
                          call stdlib_qaxpy( j1-1, -x( j1 ), t( 1_ilp, j1 ), 1_ilp, x, 1_ilp )
                          k = stdlib_iqamax( j1-1, x, 1_ilp )
                          xmax = abs( x( k ) )
                       end if
                    else
                       ! meet 2 by 2 diagonal block
                       ...
                       d( 1_ilp, 1_ilp ) = x( j1 )
                       d( 2_ilp, 1_ilp ) = x( j2 )
                       call stdlib_qlaln2( .false., 2_ilp, 1_ilp, smin, one, t( j1, j1 ),ldt, one, one, d,&
                                  2_ilp, zero, zero, v, 2_ilp,scaloc, xnorm, ierr )
                       if( ierr/=0_ilp )info = 2_ilp
                       if( scaloc/=one ) then
                          call stdlib_qscal( n, scaloc, x, 1_ilp )
                          scale = scale*scaloc
                       end if
                       x( j1 ) = v( 1_ilp, 1_ilp )
                       x( j2 ) = v( 2_ilp, 1_ilp )
                       ...
                       if( j1>1_ilp ) then
                          call stdlib_qaxpy( j1-1, -x( j1 ), t( 1_ilp, j1 ), 1_ilp, x, 1_ilp )
                          call stdlib_qaxpy( j1-1, -x( j2 ), t( 1_ilp, j2 ), 1_ilp, x, 1_ilp )
                          k = stdlib_iqamax( j1-1, x, 1_ilp )
                          xmax = abs( x( k ) )
                       end if
                    end if
                 ...
                 loop_40: do j = 1, n
                    ...
                    if( j1==j2 ) then
                       ...
                       x( j1 ) = x( j1 ) - stdlib_qdot( j1-1, t( 1_ilp, j1 ), 1_ilp, x, 1_ilp )
                       xj = abs( x( j1 ) )
                       tjj = abs( t( j1, j1 ) )
                       tmp = t( j1, j1 )
                       if( tjj<smin ) then
                       ...
                       d( 1_ilp, 1_ilp ) = x( j1 ) - stdlib_qdot( j1-1, t( 1_ilp, j1 ), 1_ilp, x,1_ilp )
                       d( 2_ilp, 1_ilp ) = x( j2 ) - stdlib_qdot( j1-1, t( 1_ilp, j2 ), 1_ilp, x,1_ilp )
                       call stdlib_qlaln2( .true., 2_ilp, 1_ilp, smin, one, t( j1, j1 ),ldt, one, one, d, &
                                 2_ilp, zero, zero, v, 2_ilp,scaloc, xnorm, ierr )
                       if( ierr/=0_ilp )info = 2_ilp
                       if( scaloc/=one ) then
                          call stdlib_qscal( n, scaloc, x, 1_ilp )
              ...
              if( notran ) then
                 ...
                 loop_70: do j = n, 1, -1
                    ...
                    if( j1==j2 ) then
                       ...
                       if( tjj<one ) then
                          if( xj>bignum*tjj ) then
                             rec = one / xj
                             call stdlib_qscal( n2, rec, x, 1_ilp )
                             scale = scale*rec
                             xmax = xmax*rec
                          end if
                       end if
                       call stdlib_qladiv( x( j1 ), x( n+j1 ), tmp, z, sr, si )
                       x( j1 ) = sr
                       x( n+j1 ) = si
                       xj = abs( x( j1 ) ) + abs( x( n+j1 ) )
                       ! scale x if necessary to avoid overflow when adding a
                       ...
                       if( j1>1_ilp ) then
                          call stdlib_qaxpy( j1-1, -x( j1 ), t( 1_ilp, j1 ), 1_ilp, x, 1_ilp )
                          call stdlib_qaxpy( j1-1, -x( n+j1 ), t( 1_ilp, j1 ), 1_ilp,x( n+1 ), 1_ilp )
                          x( 1_ilp ) = x( 1_ilp ) + b( j1 )*x( n+j1 )
                          x( n+1 ) = x( n+1 ) - b( j1 )*x( j1 )
                          xmax = zero
                          do k = 1, j1 - 1
                       ...
                       d( 1_ilp, 2_ilp ) = x( n+j1 )
                       d( 2_ilp, 2_ilp ) = x( n+j2 )
                       call stdlib_qlaln2( .false., 2_ilp, 2_ilp, sminw, one, t( j1, j1 ),ldt, one, one, &
                                 d, 2_ilp, zero, -w, v, 2_ilp,scaloc, xnorm, ierr )
                       if( ierr/=0_ilp )info = 2_ilp
                       if( scaloc/=one ) then
                          call stdlib_qscal( 2_ilp*n, scaloc, x, 1_ilp )
                          scale = scaloc*scale
                       end if
                       x( j1 ) = v( 1_ilp, 1_ilp )
                       x( j2 ) = v( 2_ilp, 1_ilp )
                       ...
                       if( j1>1_ilp ) then
                          ...
                          call stdlib_qaxpy( j1-1, -x( j2 ), t( 1_ilp, j2 ), 1_ilp, x, 1_ilp )
                          call stdlib_qaxpy( j1-1, -x( n+j1 ), t( 1_ilp, j1 ), 1_ilp,x( n+1 ), 1_ilp )
                          call stdlib_qaxpy( j1-1, -x( n+j2 ), t( 1_ilp, j2 ), 1_ilp,x( n+1 ), 1_ilp )
                          x( 1_ilp ) = x( 1_ilp ) + b( j1 )*x( n+j1 ) +b( j2 )*x( n+j2 )
                          x( n+1 ) = x( n+1 ) - b( j1 )*x( j1 ) -b( j2 )*x( j2 )
                          xmax = zero
                          do k = 1, j1 - 1
                 ...
                 loop_80: do j = 1, n
                    ...
                    if( j1==j2 ) then
                       ...
                       x( j1 ) = x( j1 ) - stdlib_qdot( j1-1, t( 1_ilp, j1 ), 1_ilp, x, 1_ilp )
                       x( n+j1 ) = x( n+j1 ) - stdlib_qdot( j1-1, t( 1_ilp, j1 ), 1_ilp,x( n+1 ), 1_ilp )
                       ...
                       d( 1_ilp, 1_ilp ) = x( j1 ) - stdlib_qdot( j1-1, t( 1_ilp, j1 ), 1_ilp, x,1_ilp )
                       d( 2_ilp, 1_ilp ) = x( j2 ) - stdlib_qdot( j1-1, t( 1_ilp, j2 ), 1_ilp, x,1_ilp )
                       d( 1_ilp, 2_ilp ) = x( n+j1 ) - stdlib_qdot( j1-1, t( 1_ilp, j1 ), 1_ilp,x( n+1 ), 1_ilp )
                                 
                       d( 2_ilp, 2_ilp ) = x( n+j2 ) - stdlib_qdot( j1-1, t( 1_ilp, j2 ), 1_ilp,x( n+1 ), 1_ilp )
                                 
                       d( 1_ilp, 1_ilp ) = d( 1_ilp, 1_ilp ) - b( j1 )*x( n+1 )
                       d( 2_ilp, 1_ilp ) = d( 2_ilp, 1_ilp ) - b( j2 )*x( n+1 )
                       d( 1_ilp, 2_ilp ) = d( 1_ilp, 2_ilp ) + b( j1 )*x( 1_ilp )
                       d( 2_ilp, 2_ilp ) = d( 2_ilp, 2_ilp ) + b( j2 )*x( 1_ilp )
                       call stdlib_qlaln2( .true., 2_ilp, 2_ilp, sminw, one, t( j1, j1 ),ldt, one, one, d,&
                                  2_ilp, zero, w, v, 2_ilp,scaloc, xnorm, ierr )
                       if( ierr/=0_ilp )info = 2_ilp
                       if( scaloc/=one ) then
                          call stdlib_qscal( n2, scaloc, x, 1_ilp )
                          scale = scaloc*scale
                       end if
                       x( j1 ) = v( 1_ilp, 1_ilp )
                       x( j2 ) = v( 2_ilp, 1_ilp )
     ...
     !! IHI.
        ...
        ! =========================================================
           ...
           safmax = one / safmin
           call stdlib_qlabad( safmin, safmax )
           ulp = stdlib_qlamch( 'PRECISION' )
           ...
           loop_140: do its = 0, itmax
              ...
              loop_130: do k = m, i - 1
                 ...
                 ! submatrix. nr is the order of g.
                 nr = min( 3_ilp, i-k+1 )
                 if( k>m )call stdlib_qcopy( nr, h( k, k-1 ), 1_ilp, v, 1_ilp )
                 call stdlib_qlarfg( nr, v( 1_ilp ), v( 2_ilp ), 1_ilp, t1 )
                 if( k>m ) then
                    h( k, k-1 ) = v( 1_ilp )
                    h( k+1, k-1 ) = zero
                    if( k<i-1 )h( k+2, k-1 ) = zero
           ...
           if( l==i ) then
              ...
              ! transform the 2-by-2 submatrix to standard schur form,
              ! and compute and store the eigenvalues.
              call stdlib_qlanv2( h( i-1, i-1 ), h( i-1, i ), h( i, i-1 ),h( i, i ), wr( i-1 ), &
                        wi( i-1 ), wr( i ), wi( i ),cs, sn )
              if( wantt ) then
                 ! apply the transformation to the rest of h.
                 if( i2>i )call stdlib_qrot( i2-i, h( i-1, i+1 ), ldh, h( i, i+1 ), ldh,cs, sn )
                           
                 call stdlib_qrot( i-i1-1, h( i1, i-1 ), 1_ilp, h( i1, i ), 1_ilp, cs, sn )
              end if
              if( wantz ) then
                 ! apply the transformation to z.
                 call stdlib_qrot( nz, z( iloz, i-1 ), 1_ilp, z( iloz, i ), 1_ilp, cs, sn )
              end if
           end if
           ! reset deflation counter
           kdefl = 0_ilp
        ...
        ! =========================================================
           ...
           do i = ilo + 1, ihi
              if( aimag( h( i, i-1 ) )/=zero ) then
                 ...
                 sc = conjg( sc ) / abs( sc )
                 h( i, i-1 ) = abs( h( i, i-1 ) )
                 call stdlib_wscal( jhi-i+1, sc, h( i, i ), ldh )
                 call stdlib_wscal( min( jhi, i+1 )-jlo+1, conjg( sc ),h( jlo, i ), 1_ilp )
                 if( wantz )call stdlib_wscal( ihiz-iloz+1, conjg( sc ), z( iloz, i ), 1_ilp )
              end if
           end do
           nh = ihi - ilo + 1_ilp
           nz = ihiz - iloz + 1_ilp
           ...
           loop_130: do its = 0, itmax
              ...
              loop_120: do k = m, i - 1
                 ...
                 ! chases the bulge cone step toward the bottom of the active
                 ! submatrix.
                 ! v(2) is always real before the call to stdlib_wlarfg, and hence
                 ! after the call t2 ( = t1*v(2) ) is also real.
                 if( k>m )call stdlib_wcopy( 2_ilp, h( k, k-1 ), 1_ilp, v, 1_ilp )
                 call stdlib_wlarfg( 2_ilp, v( 1_ilp ), v( 2_ilp ), 1_ilp, t1 )
                 if( k>m ) then
                    h( k, k-1 ) = v( 1_ilp )
                    h( k+1, k-1 ) = czero
                 end if
                 ...
                 if( k==m .and. m>l ) then
                    ...
                    do j = m, i
                       if( j/=m+1 ) then
                          if( i2>j )call stdlib_wscal( i2-j, temp, h( j, j+1 ), ldh )
                          call stdlib_wscal( j-i1, conjg( temp ), h( i1, j ), 1_ilp )
                          if( wantz ) then
                             call stdlib_wscal( nz, conjg( temp ), z( iloz, j ),1_ilp )
                          end if
                       end if
                    end do
                 end if
              ...
              if( aimag( temp )/=zero ) then
                 ...
                 h( i, i-1 ) = rtemp
                 temp = temp / rtemp
                 if( i2>i )call stdlib_wscal( i2-i, conjg( temp ), h( i, i+1 ), ldh )
                 call stdlib_wscal( i-i1, temp, h( i1, i ), 1_ilp )
                 if( wantz ) then
                    call stdlib_wscal( nz, temp, z( iloz, i ), 1_ilp )
                 end if
              end if
           end do loop_130
           ! failure to converge in remaining number of iterations
     ...
     !! by the orthogonal matrix Q:  A = Q*H*Q**T = (QZ)*T*(QZ)**T.
        ...
        ! ================================================================
           ...
           if( n<=ntiny ) then
              ! ==== tiny matrices must use stdlib_qlahqr. ====
              lwkopt = 1_ilp
              if( lwork/=-1_ilp )call stdlib_qlahqr( wantt, wantz, n, ilo, ihi, h, ldh, wr, wi,iloz, &
                        ihiz, z, ldz, info )
           else
              ! ==== use small bulge multi-shift qr with aggressive early
              ! .    deflation on larger-than-tiny matrices. ====
              ...
              nsr = max( 2_ilp, nsr-mod( nsr, 2_ilp ) )
              ! ==== estimate optimal workspace ====
              ! ==== workspace query call to stdlib_qlaqr3 ====
              call stdlib_qlaqr3( wantt, wantz, n, ilo, ihi, nwr+1, h, ldh, iloz,ihiz, z, ldz, ls,&
                         ld, wr, wi, h, ldh, n, h, ldh,n, h, ldh, work, -1_ilp )
              ! ==== optimal workspace = max(stdlib_qlaqr5, stdlib_qlaqr3) ====
              lwkopt = max( 3_ilp*nsr / 2_ilp, int( work( 1_ilp ),KIND=ilp) )
              ! ==== quick return in case of workspace query. ====
              if( lwork==-1_ilp ) then
                 work( 1_ilp ) = real( lwkopt,KIND=qp)
                 return
              end if
              ...
              loop_80: do it = 1, itmax
                 ...
                 nve = ( n-nw ) - kwv + 1_ilp
                 ! ==== aggressive early deflation ====
                 call stdlib_qlaqr3( wantt, wantz, n, ktop, kbot, nw, h, ldh, iloz,ihiz, z, ldz, &
                 ls, ld, wr, wi, h( kv, 1_ilp ), ldh,nho, h( kv, kt ), ldh, nve, h( kwv, 1_ilp ), ldh,&
                           work, lwork )
                 ! ==== adjust kbot accounting for new deflations. ====
                 kbot = kbot - ld
                 ...
                 if( ( ld==0_ilp ) .or. ( ( 100_ilp*ld<=nw*nibble ) .and. ( kbot-ktop+1>min( nmin, nwmax )&
                    ...
                    ! ==== ns = nominal number of simultaneous shifts.
                    ! .    this may be lowered (slightly) if stdlib_qlaqr3
                    ! .    did not provide that many shifts. ====
                    ns = min( nsmax, nsr, max( 2_ilp, kbot-ktop ) )
                    ns = ns - mod( ns, 2_ilp )
                    ! ==== if there have been no deflations
                    ...
                    ! .    then try exceptional shifts.
                    ! .    otherwise use shifts provided by
                    ! .    stdlib_qlaqr3 above or from the eigenvalues
                    ! .    of a trailing principal submatrix. ====
                    if( mod( ndfl, kexsh )==0_ilp ) then
                       ks = kbot - ns + 1_ilp
                       do i = kbot, max( ks+1, ktop+2 ), -2
                          ...
                          cc = wilk2*ss
                          dd = aa
                          call stdlib_qlanv2( aa, bb, cc, dd, wr( i-1 ), wi( i-1 ),wr( i ), wi( i &
                                    ), cs, sn )
                       end do
                       if( ks==ktop ) then
                          wr( ks+1 ) = h( ks+1, ks+1 )
                       ...
                       ! ==== got ns/2 or fewer shifts? use stdlib_qlaqr4 or
                       ! .    stdlib_qlahqr on a trailing principal submatrix to
                       ! .    get more. (since ns<=nsmax<=(n-3)/6,
                       ! .    there is enough space below the subdiagonal
                       ! .    to fit an ns-by-ns scratch array.) ====
                       if( kbot-ks+1<=ns / 2_ilp ) then
                          ks = kbot - ns + 1_ilp
                          kt = n - ns + 1_ilp
                          call stdlib_qlacpy( 'A', ns, ns, h( ks, ks ), ldh,h( kt, 1_ilp ), ldh )
                                    
                          if( ns>nmin ) then
                             call stdlib_qlaqr4( .false., .false., ns, 1_ilp, ns,h( kt, 1_ilp ), ldh, wr( &
                                       ks ),wi( ks ), 1_ilp, 1_ilp, zdum, 1_ilp, work,lwork, inf )
                          else
                             call stdlib_qlahqr( .false., .false., ns, 1_ilp, ns,h( kt, 1_ilp ), ldh, wr( &
                                       ks ),wi( ks ), 1_ilp, 1_ilp, zdum, 1_ilp, inf )
                          end if
                          ks = ks + inf
                          ! ==== in case of a rare qr failure use
                          ...
                          if( ks>=kbot ) then
                             ...
                             bb = h( kbot-1, kbot )
                             dd = h( kbot, kbot )
                             call stdlib_qlanv2( aa, bb, cc, dd, wr( kbot-1 ),wi( kbot-1 ), wr( &
                                       kbot ),wi( kbot ), cs, sn )
                             ks = kbot - 1_ilp
                          end if
                       end if
                    ...
                    nve = n - kdu - kwv + 1_ilp
                    ! ==== small-bulge multi-shift qr sweep ====
                    call stdlib_qlaqr5( wantt, wantz, kacc22, n, ktop, kbot, ns,wr( ks ), wi( ks )&
                    , h, ldh, iloz, ihiz, z,ldz, work, 3_ilp, h( ku, 1_ilp ), ldh, nve,h( kwv, 1_ilp ), ldh, &
                              nho, h( ku, kwh ), ldh )
                 end if
                 ! ==== note progress (or the lack of it). ====
     ...
     !! by the unitary matrix Q:  A = Q*H*Q**H = (QZ)*H*(QZ)**H.
        ...
        ! ================================================================
           ...
           if( n<=ntiny ) then
              ! ==== tiny matrices must use stdlib_wlahqr. ====
              lwkopt = 1_ilp
              if( lwork/=-1_ilp )call stdlib_wlahqr( wantt, wantz, n, ilo, ihi, h, ldh, w, iloz,ihiz, &
                        z, ldz, info )
           else
              ! ==== use small bulge multi-shift qr with aggressive early
              ! .    deflation on larger-than-tiny matrices. ====
              ...
              nsr = max( 2_ilp, nsr-mod( nsr, 2_ilp ) )
              ! ==== estimate optimal workspace ====
              ! ==== workspace query call to stdlib_wlaqr3 ====
              call stdlib_wlaqr3( wantt, wantz, n, ilo, ihi, nwr+1, h, ldh, iloz,ihiz, z, ldz, ls,&
                         ld, w, h, ldh, n, h, ldh, n, h,ldh, work, -1_ilp )
              ! ==== optimal workspace = max(stdlib_wlaqr5, stdlib_wlaqr3) ====
              lwkopt = max( 3_ilp*nsr / 2_ilp, int( work( 1_ilp ),KIND=ilp) )
              ! ==== quick return in case of workspace query. ====
              if( lwork==-1_ilp ) then
                 work( 1_ilp ) = cmplx( lwkopt, 0_ilp,KIND=qp)
                 return
              end if
              ...
              loop_70: do it = 1, itmax
                 ...
                 nve = ( n-nw ) - kwv + 1_ilp
                 ! ==== aggressive early deflation ====
                 call stdlib_wlaqr3( wantt, wantz, n, ktop, kbot, nw, h, ldh, iloz,ihiz, z, ldz, &
                 ls, ld, w, h( kv, 1_ilp ), ldh, nho,h( kv, kt ), ldh, nve, h( kwv, 1_ilp ), ldh, work,&
                           lwork )
                 ! ==== adjust kbot accounting for new deflations. ====
                 kbot = kbot - ld
                 ...
                 if( ( ld==0_ilp ) .or. ( ( 100_ilp*ld<=nw*nibble ) .and. ( kbot-ktop+1>min( nmin, nwmax )&
                    ...
                    ! ==== ns = nominal number of simultaneous shifts.
                    ! .    this may be lowered (slightly) if stdlib_wlaqr3
                    ! .    did not provide that many shifts. ====
                    ns = min( nsmax, nsr, max( 2_ilp, kbot-ktop ) )
                    ns = ns - mod( ns, 2_ilp )
                    ! ==== if there have been no deflations
                    ...
                    ! .    then try exceptional shifts.
                    ! .    otherwise use shifts provided by
                    ! .    stdlib_wlaqr3 above or from the eigenvalues
                    ! .    of a trailing principal submatrix. ====
                    if( mod( ndfl, kexsh )==0_ilp ) then
                       ks = kbot - ns + 1_ilp
                       do i = kbot, ks + 1, -2
                       ...
                       ! ==== got ns/2 or fewer shifts? use stdlib_wlaqr4 or
                       ! .    stdlib_wlahqr on a trailing principal submatrix to
                       ! .    get more. (since ns<=nsmax<=(n-3)/6,
                       ! .    there is enough space below the subdiagonal
                       ! .    to fit an ns-by-ns scratch array.) ====
                       if( kbot-ks+1<=ns / 2_ilp ) then
                          ks = kbot - ns + 1_ilp
                          kt = n - ns + 1_ilp
                          call stdlib_wlacpy( 'A', ns, ns, h( ks, ks ), ldh,h( kt, 1_ilp ), ldh )
                                    
                          if( ns>nmin ) then
                             call stdlib_wlaqr4( .false., .false., ns, 1_ilp, ns,h( kt, 1_ilp ), ldh, w( &
                                       ks ), 1_ilp, 1_ilp,zdum, 1_ilp, work, lwork, inf )
                          else
                             call stdlib_wlahqr( .false., .false., ns, 1_ilp, ns,h( kt, 1_ilp ), ldh, w( &
                                       ks ), 1_ilp, 1_ilp,zdum, 1_ilp, inf )
                          end if
                          ks = ks + inf
                          ! ==== in case of a rare qr failure use
                    ...
                    nve = n - kdu - kwv + 1_ilp
                    ! ==== small-bulge multi-shift qr sweep ====
                    call stdlib_wlaqr5( wantt, wantz, kacc22, n, ktop, kbot, ns,w( ks ), h, ldh, &
                    iloz, ihiz, z, ldz, work,3_ilp, h( ku, 1_ilp ), ldh, nve, h( kwv, 1_ilp ), ldh,nho, h( ku,&
                               kwh ), ldh )
                 end if
                 ! ==== note progress (or the lack of it). ====
     ...
     !! entries.
        ...
        ! ================================================================
           ...
           if( jw<=2_ilp ) then
              ...
              ! ==== workspace query call to stdlib_qgehrd ====
              call stdlib_qgehrd( jw, 1_ilp, jw-1, t, ldt, work, work, -1_ilp, info )
              lwk1 = int( work( 1_ilp ),KIND=ilp)
              ! ==== workspace query call to stdlib_qormhr ====
              call stdlib_qormhr( 'R', 'N', jw, jw, 1_ilp, jw-1, t, ldt, work, v, ldv,work, -1_ilp, info )
           ...
           ! .    the deflation window that converged using infqr
           ! .    here and there to keep track.) ====
           call stdlib_qlacpy( 'U', jw, jw, h( kwtop, kwtop ), ldh, t, ldt )
           call stdlib_qcopy( jw-1, h( kwtop+1, kwtop ), ldh+1, t( 2_ilp, 1_ilp ), ldt+1 )
           call stdlib_qlaset( 'A', jw, jw, zero, one, v, ldv )
           call stdlib_qlahqr( .true., .true., jw, 1_ilp, jw, t, ldt, sr( kwtop ),si( kwtop ), 1_ilp, jw, &
                     v, ldv, infqr )
           ! ==== stdlib_qtrexc needs a clean margin near the diagonal ====
           do j = 1, jw - 3
              t( j+2, j ) = zero
              t( j+3, j ) = zero
           end do
           ...
           if( ilst<=ns ) then
              ...
              if( .not.bulge ) then
                 ...
                 if( abs( s*v( 1_ilp, ns ) )<=max( smlnum, ulp*foo ) ) then
                    ...
                    ! ==== undeflatable.   move it up out of the way.
                    ! .    (stdlib_qtrexc can not fail in this case.) ====
                    ifst = ns
                    call stdlib_qtrexc( 'V', jw, t, ldt, v, ldv, ifst, ilst, work,info )
                    ilst = ilst + 1_ilp
                 end if
              else
                 ! ==== complex conjugate pair ====
                 ...
                 if( max( abs( s*v( 1_ilp, ns ) ), abs( s*v( 1_ilp, ns-1 ) ) )<=max( smlnum, ulp*foo ) ) &
                    ...
                    ! ==== undeflatable. move them up out of the way.
                    ! .    fortunately, stdlib_qtrexc does the right thing with
                    ! .    ilst in case of a rare exchange failure. ====
                    ifst = ns
                    call stdlib_qtrexc( 'V', jw, t, ldt, v, ldv, ifst, ilst, work,info )
                    ilst = ilst + 2_ilp
           ...
           if( i>=infqr+1 ) then
              if( i==infqr+1 ) then
                 ...
                 bb = t( i-1, i )
                 dd = t( i, i )
                 call stdlib_qlanv2( aa, bb, cc, dd, sr( kwtop+i-2 ),si( kwtop+i-2 ), sr( kwtop+i-&
                           1_ilp ),si( kwtop+i-1 ), cs, sn )
                 i = i - 2_ilp
              end if
              go to 60
           ...
           if( ns<jw .or. s==zero ) then
              if( ns>1_ilp .and. s/=zero ) then
                 ! ==== reflect spike back into lower triangle ====
                 call stdlib_qcopy( ns, v, ldv, work, 1_ilp )
                 beta = work( 1_ilp )
                 call stdlib_qlarfg( ns, beta, work( 2_ilp ), 1_ilp, tau )
                 work( 1_ilp ) = one
                 call stdlib_qlaset( 'L', jw-2, jw-2, zero, zero, t( 3_ilp, 1_ilp ), ldt )
                 call stdlib_qlarf( 'L', ns, jw, work, 1_ilp, tau, t, ldt,work( jw+1 ) )
                 call stdlib_qlarf( 'R', ns, ns, work, 1_ilp, tau, t, ldt,work( jw+1 ) )
                 call stdlib_qlarf( 'R', jw, ns, work, 1_ilp, tau, v, ldv,work( jw+1 ) )
                 call stdlib_qgehrd( jw, 1_ilp, ns, t, ldt, work, work( jw+1 ),lwork-jw, info )
                           
              end if
              ! ==== copy updated reduced window into place ====
              if( kwtop>1_ilp )h( kwtop, kwtop-1 ) = s*v( 1_ilp, 1_ilp )
              call stdlib_qlacpy( 'U', jw, jw, t, ldt, h( kwtop, kwtop ), ldh )
              call stdlib_qcopy( jw-1, t( 2_ilp, 1_ilp ), ldt+1, h( kwtop+1, kwtop ),ldh+1 )
              ! ==== accumulate orthogonal matrix in order update
              ! .    h and z, if requested.  ====
              if( ns>1_ilp .and. s/=zero )call stdlib_qormhr( 'R', 'N', jw, ns, 1_ilp, ns, t, ldt, work, &
                        v, ldv,work( jw+1 ), lwork-jw, info )
              ! ==== update vertical slab in h ====
              if( wantt ) then
                 ltop = 1_ilp
              ...
              do krow = ltop, kwtop - 1, nv
                 kln = min( nv, kwtop-krow )
                 call stdlib_qgemm( 'N', 'N', kln, jw, jw, one, h( krow, kwtop ),ldh, v, ldv, &
                           zero, wv, ldwv )
                 call stdlib_qlacpy( 'A', kln, jw, wv, ldwv, h( krow, kwtop ), ldh )
              end do
              ! ==== update horizontal slab in h ====
              if( wantt ) then
                 do kcol = kbot + 1, n, nh
                    kln = min( nh, n-kcol+1 )
                    call stdlib_qgemm( 'C', 'N', jw, kln, jw, one, v, ldv,h( kwtop, kcol ), ldh, &
                              zero, t, ldt )
                    call stdlib_qlacpy( 'A', jw, kln, t, ldt, h( kwtop, kcol ),ldh )
                 end do
              end if
              ! ==== update vertical slab in z ====
              if( wantz ) then
                 do krow = iloz, ihiz, nv
                    kln = min( nv, ihiz-krow+1 )
                    call stdlib_qgemm( 'N', 'N', kln, jw, jw, one, z( krow, kwtop ),ldz, v, ldv, &
                              zero, wv, ldwv )
                    call stdlib_qlacpy( 'A', kln, jw, wv, ldwv, z( krow, kwtop ),ldz )
                 end do
              end if
           end if
           ! ==== return the number of deflations ... ====
     ...
     !! entries.
        ...
        ! ================================================================
           ...
           if( jw<=2_ilp ) then
              ...
              ! ==== workspace query call to stdlib_wgehrd ====
              call stdlib_wgehrd( jw, 1_ilp, jw-1, t, ldt, work, work, -1_ilp, info )
              lwk1 = int( work( 1_ilp ),KIND=ilp)
              ! ==== workspace query call to stdlib_wunmhr ====
              call stdlib_wunmhr( 'R', 'N', jw, jw, 1_ilp, jw-1, t, ldt, work, v, ldv,work, -1_ilp, info )
           ...
           ! .    the deflation window that converged using infqr
           ! .    here and there to keep track.) ====
           call stdlib_wlacpy( 'U', jw, jw, h( kwtop, kwtop ), ldh, t, ldt )
           call stdlib_wcopy( jw-1, h( kwtop+1, kwtop ), ldh+1, t( 2_ilp, 1_ilp ), ldt+1 )
           call stdlib_wlaset( 'A', jw, jw, czero, cone, v, ldv )
           call stdlib_wlahqr( .true., .true., jw, 1_ilp, jw, t, ldt, sh( kwtop ), 1_ilp,jw, v, ldv, &
                     infqr )
           ! ==== deflation detection loop ====
           ns = jw
           ilst = infqr + 1_ilp
           do knt = infqr + 1, jw
              ...
              if( cabs1( s )*cabs1( v( 1_ilp, ns ) )<=max( smlnum, ulp*foo ) )then
                 ...
                 ! ==== cone undeflatable eigenvalue.  move it up out of the
                 ! .    way.   (stdlib_wtrexc can not fail in this case.) ====
                 ifst = ns
                 call stdlib_wtrexc( 'V', jw, t, ldt, v, ldv, ifst, ilst, info )
                 ilst = ilst + 1_ilp
              end if
           end do
              ! ==== return to hessenberg form ====
           ...
           if( ns<jw .or. s==czero ) then
              if( ns>1_ilp .and. s/=czero ) then
                 ! ==== reflect spike back into lower triangle ====
                 call stdlib_wcopy( ns, v, ldv, work, 1_ilp )
                 do i = 1, ns
                    work( i ) = conjg( work( i ) )
                 end do
                 beta = work( 1_ilp )
                 call stdlib_wlarfg( ns, beta, work( 2_ilp ), 1_ilp, tau )
                 work( 1_ilp ) = cone
                 call stdlib_wlaset( 'L', jw-2, jw-2, czero, czero, t( 3_ilp, 1_ilp ), ldt )
                 call stdlib_wlarf( 'L', ns, jw, work, 1_ilp, conjg( tau ), t, ldt,work( jw+1 ) )
                           
                 call stdlib_wlarf( 'R', ns, ns, work, 1_ilp, tau, t, ldt,work( jw+1 ) )
                 call stdlib_wlarf( 'R', jw, ns, work, 1_ilp, tau, v, ldv,work( jw+1 ) )
                 call stdlib_wgehrd( jw, 1_ilp, ns, t, ldt, work, work( jw+1 ),lwork-jw, info )
                           
              end if
              ! ==== copy updated reduced window into place ====
              if( kwtop>1_ilp )h( kwtop, kwtop-1 ) = s*conjg( v( 1_ilp, 1_ilp ) )
              call stdlib_wlacpy( 'U', jw, jw, t, ldt, h( kwtop, kwtop ), ldh )
              call stdlib_wcopy( jw-1, t( 2_ilp, 1_ilp ), ldt+1, h( kwtop+1, kwtop ),ldh+1 )
              ! ==== accumulate orthogonal matrix in order update
              ! .    h and z, if requested.  ====
              if( ns>1_ilp .and. s/=czero )call stdlib_wunmhr( 'R', 'N', jw, ns, 1_ilp, ns, t, ldt, work, &
                        v, ldv,work( jw+1 ), lwork-jw, info )
              ! ==== update vertical slab in h ====
              if( wantt ) then
                 ltop = 1_ilp
              ...
              do krow = ltop, kwtop - 1, nv
                 kln = min( nv, kwtop-krow )
                 call stdlib_wgemm( 'N', 'N', kln, jw, jw, cone, h( krow, kwtop ),ldh, v, ldv, &
                           czero, wv, ldwv )
                 call stdlib_wlacpy( 'A', kln, jw, wv, ldwv, h( krow, kwtop ), ldh )
              end do
              ! ==== update horizontal slab in h ====
              if( wantt ) then
                 do kcol = kbot + 1, n, nh
                    kln = min( nh, n-kcol+1 )
                    call stdlib_wgemm( 'C', 'N', jw, kln, jw, cone, v, ldv,h( kwtop, kcol ), ldh, &
                              czero, t, ldt )
                    call stdlib_wlacpy( 'A', jw, kln, t, ldt, h( kwtop, kcol ),ldh )
                 end do
              end if
              ! ==== update vertical slab in z ====
              if( wantz ) then
                 do krow = iloz, ihiz, nv
                    kln = min( nv, ihiz-krow+1 )
                    call stdlib_wgemm( 'N', 'N', kln, jw, jw, cone, z( krow, kwtop ),ldz, v, ldv, &
                              czero, wv, ldwv )
                    call stdlib_wlacpy( 'A', kln, jw, wv, ldwv, z( krow, kwtop ),ldz )
                 end do
              end if
           end if
           ! ==== return the number of deflations ... ====
     ...
     !! entries.
        ...
        ! ================================================================
           ...
           if( jw<=2_ilp ) then
              ...
              call stdlib_qlaqr4( .true., .true., jw, 1_ilp, jw, t, ldt, sr, si, 1_ilp, jw,v, ldv, work, -&
                        1_ilp, infqr )
              lwk3 = int( work( 1_ilp ),KIND=ilp)
              ! ==== optimal workspace ====
              lwkopt = max( jw+max( lwk1, lwk2 ), lwk3 )
           ...
           nmin = stdlib_ilaenv( 12_ilp, 'DLAQR3', 'SV', jw, 1_ilp, jw, lwork )
           if( jw>nmin ) then
              call stdlib_qlaqr4( .true., .true., jw, 1_ilp, jw, t, ldt, sr( kwtop ),si( kwtop ), 1_ilp, &
                        jw, v, ldv, work, lwork, infqr )
           else
              call stdlib_qlahqr( .true., .true., jw, 1_ilp, jw, t, ldt, sr( kwtop ),si( kwtop ), 1_ilp, &
                        jw, v, ldv, infqr )
           end if
           ! ==== stdlib_qtrexc needs a clean margin near the diagonal ====
           do j = 1, jw - 3
     ...
     !! entries.
        ...
        ! ================================================================
           ...
           if( jw<=2_ilp ) then
              ...
              call stdlib_wlaqr4( .true., .true., jw, 1_ilp, jw, t, ldt, sh, 1_ilp, jw, v,ldv, work, -1_ilp, &
                        infqr )
              lwk3 = int( work( 1_ilp ),KIND=ilp)
              ! ==== optimal workspace ====
              lwkopt = max( jw+max( lwk1, lwk2 ), lwk3 )
           ...
           nmin = stdlib_ilaenv( 12_ilp, 'ZLAQR3', 'SV', jw, 1_ilp, jw, lwork )
           if( jw>nmin ) then
              call stdlib_wlaqr4( .true., .true., jw, 1_ilp, jw, t, ldt, sh( kwtop ), 1_ilp,jw, v, ldv, &
                        work, lwork, infqr )
           else
              call stdlib_wlahqr( .true., .true., jw, 1_ilp, jw, t, ldt, sh( kwtop ), 1_ilp,jw, v, ldv, &
                        infqr )
     ...
     !! by the orthogonal matrix Q:  A = Q*H*Q**T = (QZ)*T*(QZ)**T.
        ...
        ! ================================================================
           ...
           if( n<=ntiny ) then
              ...
              nsr = max( 2_ilp, nsr-mod( nsr, 2_ilp ) )
              ! ==== estimate optimal workspace ====
              ! ==== workspace query call to stdlib_qlaqr2 ====
              call stdlib_qlaqr2( wantt, wantz, n, ilo, ihi, nwr+1, h, ldh, iloz,ihiz, z, ldz, ls,&
                         ld, wr, wi, h, ldh, n, h, ldh,n, h, ldh, work, -1_ilp )
              ! ==== optimal workspace = max(stdlib_qlaqr5, stdlib_qlaqr2) ====
              lwkopt = max( 3_ilp*nsr / 2_ilp, int( work( 1_ilp ),KIND=ilp) )
              ! ==== quick return in case of workspace query. ====
              if( lwork==-1_ilp ) then
                 work( 1_ilp ) = real( lwkopt,KIND=qp)
              ...
              loop_80: do it = 1, itmax
                 ...
                 nve = ( n-nw ) - kwv + 1_ilp
                 ! ==== aggressive early deflation ====
                 call stdlib_qlaqr2( wantt, wantz, n, ktop, kbot, nw, h, ldh, iloz,ihiz, z, ldz, &
                 ls, ld, wr, wi, h( kv, 1_ilp ), ldh,nho, h( kv, kt ), ldh, nve, h( kwv, 1_ilp ), ldh,&
                           work, lwork )
                 ! ==== adjust kbot accounting for new deflations. ====
                 kbot = kbot - ld
                 ...
                 if( ( ld==0_ilp ) .or. ( ( 100_ilp*ld<=nw*nibble ) .and. ( kbot-ktop+1>min( nmin, nwmax )&
                    ...
                    ! ==== ns = nominal number of simultaneous shifts.
                    ! .    this may be lowered (slightly) if stdlib_qlaqr2
                    ! .    did not provide that many shifts. ====
                    ns = min( nsmax, nsr, max( 2_ilp, kbot-ktop ) )
                    ns = ns - mod( ns, 2_ilp )
                    ! ==== if there have been no deflations
                    ...
                    ! .    then try exceptional shifts.
                    ! .    otherwise use shifts provided by
                    ! .    stdlib_qlaqr2 above or from the eigenvalues
                    ! .    of a trailing principal submatrix. ====
                    if( mod( ndfl, kexsh )==0_ilp ) then
                       ks = kbot - ns + 1_ilp
                       do i = kbot, max( ks+1, ktop+2 ), -2
                       ...
                       ! ==== got ns/2 or fewer shifts? use stdlib_qlahqr
                       ! .    on a trailing principal submatrix to
                       ! .    get more. (since ns<=nsmax<=(n-3)/6,
                       ! .    there is enough space below the subdiagonal
                       ! .    to fit an ns-by-ns scratch array.) ====
                       if( kbot-ks+1<=ns / 2_ilp ) then
                          ...
                          call stdlib_qlahqr( .false., .false., ns, 1_ilp, ns,h( kt, 1_ilp ), ldh, wr( ks &
                                    ), wi( ks ),1_ilp, 1_ilp, zdum, 1_ilp, inf )
                          ks = ks + inf
                          ! ==== in case of a rare qr failure use
                          ! .    eigenvalues of the trailing 2-by-2
     ...
     !! by the unitary matrix Q:  A = Q*H*Q**H = (QZ)*H*(QZ)**H.
        ...
        ! ================================================================
           ...
           if( n<=ntiny ) then
              ...
              nsr = max( 2_ilp, nsr-mod( nsr, 2_ilp ) )
              ! ==== estimate optimal workspace ====
              ! ==== workspace query call to stdlib_wlaqr2 ====
              call stdlib_wlaqr2( wantt, wantz, n, ilo, ihi, nwr+1, h, ldh, iloz,ihiz, z, ldz, ls,&
                         ld, w, h, ldh, n, h, ldh, n, h,ldh, work, -1_ilp )
              ! ==== optimal workspace = max(stdlib_wlaqr5, stdlib_wlaqr2) ====
              lwkopt = max( 3_ilp*nsr / 2_ilp, int( work( 1_ilp ),KIND=ilp) )
              ! ==== quick return in case of workspace query. ====
              if( lwork==-1_ilp ) then
                 work( 1_ilp ) = cmplx( lwkopt, 0_ilp,KIND=qp)
              ...
              loop_70: do it = 1, itmax
                 ...
                 nve = ( n-nw ) - kwv + 1_ilp
                 ! ==== aggressive early deflation ====
                 call stdlib_wlaqr2( wantt, wantz, n, ktop, kbot, nw, h, ldh, iloz,ihiz, z, ldz, &
                 ls, ld, w, h( kv, 1_ilp ), ldh, nho,h( kv, kt ), ldh, nve, h( kwv, 1_ilp ), ldh, work,&
                           lwork )
                 ! ==== adjust kbot accounting for new deflations. ====
                 kbot = kbot - ld
                 ...
                 if( ( ld==0_ilp ) .or. ( ( 100_ilp*ld<=nw*nibble ) .and. ( kbot-ktop+1>min( nmin, nwmax )&
                    ...
                    ! ==== ns = nominal number of simultaneous shifts.
                    ! .    this may be lowered (slightly) if stdlib_wlaqr2
                    ! .    did not provide that many shifts. ====
                    ns = min( nsmax, nsr, max( 2_ilp, kbot-ktop ) )
                    ns = ns - mod( ns, 2_ilp )
                    ! ==== if there have been no deflations
                    ...
                    ! .    then try exceptional shifts.
                    ! .    otherwise use shifts provided by
                    ! .    stdlib_wlaqr2 above or from the eigenvalues
                    ! .    of a trailing principal submatrix. ====
                    if( mod( ndfl, kexsh )==0_ilp ) then
                       ks = kbot - ns + 1_ilp
                       do i = kbot, ks + 1, -2
                       ...
                       ! ==== got ns/2 or fewer shifts? use stdlib_wlahqr
                       ! .    on a trailing principal submatrix to
                       ! .    get more. (since ns<=nsmax<=(n-3)/6,
                       ! .    there is enough space below the subdiagonal
                       ! .    to fit an ns-by-ns scratch array.) ====
                       if( kbot-ks+1<=ns / 2_ilp ) then
                          ...
                          call stdlib_wlahqr( .false., .false., ns, 1_ilp, ns,h( kt, 1_ilp ), ldh, w( ks )&
                                    , 1_ilp, 1_ilp, zdum,1_ilp, inf )
                          ks = ks + inf
                          ! ==== in case of a rare qr failure use
                          ! .    eigenvalues of the trailing 2-by-2
     ...
     !! single small-bulge multi-shift QR sweep.
        ...
        ! ================================================================
           ...
           loop_180: do incol = ktop - 2*nbmps + 1, kbot - 2, 2*nbmps
              ...
              ndcol = incol + kdu
              if( accum )call stdlib_qlaset( 'ALL', kdu, kdu, zero, one, u, ldu )
              ! ==== near-the-diagonal bulge chase.  the following loop
              ! .    performs the near-the-diagonal part of a small bulge
              ! .    multi-shift qr sweep.  each 4*nbmps column diagonal
              ! .    chunk extends from column incol to column ndcol
              ...
              loop_145: do krcol = incol, min( incol+2*nbmps-1, kbot-2 )
                 ...
                 if ( bmp22 ) then
                    ...
                    k = krcol + 2_ilp*( m22-1 )
                    if( k==ktop-1 ) then
                       call stdlib_qlaqr1( 2_ilp, h( k+1, k+1 ), ldh, sr( 2_ilp*m22-1 ),si( 2_ilp*m22-1 ), sr(&
                                  2_ilp*m22 ), si( 2_ilp*m22 ),v( 1_ilp, m22 ) )
                       beta = v( 1_ilp, m22 )
                       call stdlib_qlarfg( 2_ilp, beta, v( 2_ilp, m22 ), 1_ilp, v( 1_ilp, m22 ) )
                    else
                       beta = h( k+1, k )
                       v( 2_ilp, m22 ) = h( k+2, k )
                       call stdlib_qlarfg( 2_ilp, beta, v( 2_ilp, m22 ), 1_ilp, v( 1_ilp, m22 ) )
                 ...
                 loop_80: do m = mbot, mtop, -1
                    k = krcol + 2_ilp*( m-1 )
                    if( k==ktop-1 ) then
                       call stdlib_qlaqr1( 3_ilp, h( ktop, ktop ), ldh, sr( 2_ilp*m-1 ),si( 2_ilp*m-1 ), sr( &
                                 2_ilp*m ), si( 2_ilp*m ),v( 1_ilp, m ) )
                       alpha = v( 1_ilp, m )
                       call stdlib_qlarfg( 3_ilp, alpha, v( 2_ilp, m ), 1_ilp, v( 1_ilp, m ) )
                    else
                       ! ==== perform delayed transformation of row below
                       ! .    mth bulge. exploit fact that first two elements
                       ! .    of row are actually zero. ====
                       ...
                       v( 2_ilp, m ) = h( k+2, k )
                       v( 3_ilp, m ) = h( k+3, k )
                       call stdlib_qlarfg( 3_ilp, beta, v( 2_ilp, m ), 1_ilp, v( 1_ilp, m ) )
                       ! ==== a bulge may collapse because of vigilant
                       ! .    deflation or destructive underflow.  in the
                       ! .    underflow case, try the two-small-subdiagonals
                       ! .    trick to try to reinflate the bulge.  ====
                       if( h( k+3, k )/=zero .or. h( k+3, k+1 )/=zero .or. h( k+3, k+2 )==zero ) &
                          ...
                          ! .    reflector is too large, then abandon it.
                          ! .    otherwise, use the new one. ====
                          call stdlib_qlaqr1( 3_ilp, h( k+1, k+1 ), ldh, sr( 2_ilp*m-1 ),si( 2_ilp*m-1 ), sr( &
                                    2_ilp*m ), si( 2_ilp*m ),vt )
                          alpha = vt( 1_ilp )
                          call stdlib_qlarfg( 3_ilp, alpha, vt( 2_ilp ), 1_ilp, vt( 1_ilp ) )
                          refsum = vt( 1_ilp )*( h( k+1, k )+vt( 2_ilp )*h( k+2, k ) )
                          if( abs( h( k+2, k )-refsum*vt( 2_ilp ) )+abs( refsum*vt( 3_ilp ) )>ulp*( abs( &
                                    h( k, k ) )+abs( h( k+1,k+1 ) )+abs( h( k+2, k+2 ) ) ) ) then
                             ! ==== starting a new bulge here would
     ...
     !! single small-bulge multi-shift QR sweep.
        ...
        ! ================================================================
           ...
           loop_180: do incol = ktop - 2*nbmps + 1, kbot - 2, 2*nbmps
              ...
              ndcol = incol + kdu
              if( accum )call stdlib_wlaset( 'ALL', kdu, kdu, czero, cone, u, ldu )
              ! ==== near-the-diagonal bulge chase.  the following loop
              ! .    performs the near-the-diagonal part of a small bulge
              ! .    multi-shift qr sweep.  each 4*nbmps column diagonal
              ! .    chunk extends from column incol to column ndcol
              ...
              loop_145: do krcol = incol, min( incol+2*nbmps-1, kbot-2 )
                 ...
                 if ( bmp22 ) then
                    ...
                    k = krcol + 2_ilp*( m22-1 )
                    if( k==ktop-1 ) then
                       call stdlib_wlaqr1( 2_ilp, h( k+1, k+1 ), ldh, s( 2_ilp*m22-1 ),s( 2_ilp*m22 ), v( 1_ilp, &
                                 m22 ) )
                       beta = v( 1_ilp, m22 )
                       call stdlib_wlarfg( 2_ilp, beta, v( 2_ilp, m22 ), 1_ilp, v( 1_ilp, m22 ) )
                    else
                       beta = h( k+1, k )
                       v( 2_ilp, m22 ) = h( k+2, k )
                       call stdlib_wlarfg( 2_ilp, beta, v( 2_ilp, m22 ), 1_ilp, v( 1_ilp, m22 ) )
                 ...
                 loop_80: do m = mbot, mtop, -1
                    k = krcol + 2_ilp*( m-1 )
                    if( k==ktop-1 ) then
                       call stdlib_wlaqr1( 3_ilp, h( ktop, ktop ), ldh, s( 2_ilp*m-1 ),s( 2_ilp*m ), v( 1_ilp, m )&
                                  )
                       alpha = v( 1_ilp, m )
                       call stdlib_wlarfg( 3_ilp, alpha, v( 2_ilp, m ), 1_ilp, v( 1_ilp, m ) )
                    else
                       ! ==== perform delayed transformation of row below
                       ! .    mth bulge. exploit fact that first two elements
                       ! .    of row are actually czero. ====
                       ...
                       v( 2_ilp, m ) = h( k+2, k )
                       v( 3_ilp, m ) = h( k+3, k )
                       call stdlib_wlarfg( 3_ilp, beta, v( 2_ilp, m ), 1_ilp, v( 1_ilp, m ) )
                       ! ==== a bulge may collapse because of vigilant
                       ! .    deflation or destructive underflow.  in the
                       ! .    underflow case, try the two-small-subdiagonals
                       ! .    trick to try to reinflate the bulge.  ====
                       if( h( k+3, k )/=czero .or. h( k+3, k+1 )/=czero .or. h( k+3, k+2 )==czero &
                          ...
                          ! .    reflector is too large, then abandon it.
                          ! .    otherwise, use the new cone. ====
                          call stdlib_wlaqr1( 3_ilp, h( k+1, k+1 ), ldh, s( 2_ilp*m-1 ),s( 2_ilp*m ), vt )
                                    
                          alpha = vt( 1_ilp )
                          call stdlib_wlarfg( 3_ilp, alpha, vt( 2_ilp ), 1_ilp, vt( 1_ilp ) )
                          refsum = conjg( vt( 1_ilp ) )*( h( k+1, k )+conjg( vt( 2_ilp ) )*h( k+2, k ) )
     ...
     !! multipole rational QZ method with agressive early deflation"
           ...
           nsr = max( 2_ilp, nsr-mod( nsr, 2_ilp ) )
           rcost = stdlib_ilaenv( 17_ilp, 'DLAQZ0', jbcmpz, n, ilo, ihi, lwork )
           itemp1 = int( nsr/sqrt( 1_ilp+2*nsr/( real( rcost,KIND=qp)/100_ilp*n ) ),KIND=ilp)
           itemp1 = ( ( itemp1-1 )/4_ilp )*4_ilp+4
           nbr = nsr+itemp1
           if( n < nmin .or. rec >= 2_ilp ) then
              call stdlib_qhgeqz( wants, wantq, wantz, n, ilo, ihi, a, lda, b, ldb,alphar, alphai,&
                         beta, q, ldq, z, ldz, work,lwork, info )
              return
           end if
           ! find out required workspace
           ! workspace query to stdlib_qlaqz3
           nw = max( nwr, nmin )
           call stdlib_qlaqz3( ilschur, ilq, ilz, n, ilo, ihi, nw, a, lda, b, ldb,q, ldq, z, ldz, &
           n_undeflated, n_qeflated, alphar,alphai, beta, work, nw, work, nw, work, -1_ilp, rec,&
                     aed_info )
           itemp1 = int( work( 1_ilp ),KIND=ilp)
           ! workspace query to stdlib_qlaqz4
           call stdlib_qlaqz4( ilschur, ilq, ilz, n, ilo, ihi, nsr, nbr, alphar,alphai, beta, a, &
                     lda, b, ldb, q, ldq, z, ldz, work,nbr, work, nbr, work, -1_ilp, sweep_info )
           itemp2 = int( work( 1_ilp ),KIND=ilp)
           lworkreq = max( itemp1+2*nw**2_ilp, itemp2+2*nbr**2_ilp )
           if ( lwork ==-1_ilp ) then
              work( 1_ilp ) = real( lworkreq,KIND=qp)
              return
           else if ( lwork < lworkreq ) then
              info = -19_ilp
           end if
           ...
           ! initialize q and z
           if( iwantq==3_ilp ) call stdlib_qlaset( 'FULL', n, n, zero, one, q, ldq )
           if( iwantz==3_ilp ) call stdlib_qlaset( 'FULL', n, n, zero, one, z, ldz )
           ! get machine constants
           safmin = stdlib_qlamch( 'SAFE MINIMUM' )
           safmax = one/safmin
           call stdlib_qlabad( safmin, safmax )
           ...
           do iiter = 1, maxit
              ...
              do while ( k>=istart2 )
                 ...
                 if( abs( b( k, k ) ) < max( smlnum, ulp*temp ) ) then
                    ...
                    ! to the top and deflate it
                    do k2 = k, istart2+1, -1
                       call stdlib_qlartg( b( k2-1, k2 ), b( k2-1, k2-1 ), c1, s1,temp )
                       b( k2-1, k2 ) = temp
                       b( k2-1, k2-1 ) = zero
                       call stdlib_qrot( k2-2-istartm+1, b( istartm, k2 ), 1_ilp,b( istartm, k2-1 ), &
                                 1_ilp, c1, s1 )
                       call stdlib_qrot( min( k2+1, istop )-istartm+1, a( istartm,k2 ), 1_ilp, a( &
                                 istartm, k2-1 ), 1_ilp, c1, s1 )
                       if ( ilz ) then
                          call stdlib_qrot( n, z( 1_ilp, k2 ), 1_ilp, z( 1_ilp, k2-1 ), 1_ilp, c1,s1 )
                       end if
                       if( k2<istop ) then
                          ...
                          call stdlib_qrot( istopm-k2+1, a( k2, k2 ), lda, a( k2+1,k2 ), lda, c1, &
                                    s1 )
                          call stdlib_qrot( istopm-k2+1, b( k2, k2 ), ldb, b( k2+1,k2 ), ldb, c1, &
                                    s1 )
                          if( ilq ) then
                             call stdlib_qrot( n, q( 1_ilp, k2 ), 1_ilp, q( 1_ilp, k2+1 ), 1_ilp,c1, s1 )
                          end if
                       end if
                    end do
                    if( istart2<istop )then
                       call stdlib_qlartg( a( istart2, istart2 ), a( istart2+1,istart2 ), c1, s1, &
                                 temp )
                       a( istart2, istart2 ) = temp
                       a( istart2+1, istart2 ) = zero
                       call stdlib_qrot( istopm-( istart2+1 )+1_ilp, a( istart2,istart2+1 ), lda, a( &
                                 istart2+1,istart2+1 ), lda, c1, s1 )
                       call stdlib_qrot( istopm-( istart2+1 )+1_ilp, b( istart2,istart2+1 ), ldb, b( &
                                 istart2+1,istart2+1 ), ldb, c1, s1 )
                       if( ilq ) then
                          call stdlib_qrot( n, q( 1_ilp, istart2 ), 1_ilp, q( 1_ilp,istart2+1 ), 1_ilp, c1, s1 )
              ...
              if ( istop-istart2+1 < nmin ) then
                 ! setting nw to the size of the subblock will make aed deflate
                 ! all the eigenvalues. this is slightly more efficient than just
                 ! using stdlib_qhgeqz because the off diagonal part gets updated via blas.
                 if ( istop-istart+1 < nmin ) then
                    nw = istop-istart+1
                    istart2 = istart
                 else
              ...
              ! time for aed
              call stdlib_qlaqz3( ilschur, ilq, ilz, n, istart2, istop, nw, a, lda,b, ldb, q, ldq,&
               z, ldz, n_undeflated, n_qeflated,alphar, alphai, beta, work, nw, work( nw**2_ilp+1 ),&
                         nw, work( 2_ilp*nw**2_ilp+1 ), lwork-2*nw**2_ilp, rec,aed_info )
              if ( n_qeflated > 0_ilp ) then
                 istop = istop-n_qeflated
                 ld = 0_ilp
                 eshift = zero
              end if
              if ( 100_ilp*n_qeflated > nibble*( n_qeflated+n_undeflated ) .or.istop-istart2+1 < nmin &
                        ) then
                 ! aed has uncovered many eigenvalues. skip a qz sweep and run
                 ! aed again.
                 cycle
              ...
              ! time for a qz sweep
              call stdlib_qlaqz4( ilschur, ilq, ilz, n, istart2, istop, ns, nblock,alphar( &
              shiftpos ), alphai( shiftpos ),beta( shiftpos ), a, lda, b, ldb, q, ldq, z, ldz,&
              work, nblock, work( nblock**2_ilp+1 ), nblock,work( 2_ilp*nblock**2_ilp+1 ), lwork-2*nblock**2_ilp,&
                        sweep_info )
           end do
           ! call stdlib_qhgeqz to normalize the eigenvalue blocks and set the eigenvalues
           ! if all the eigenvalues have been found, stdlib_qhgeqz will not do any iterations
           ! and only normalize the blocks. in case of a rare convergence failure,
           ! the single shift might perform better.
        80 continue
           call stdlib_qhgeqz( wants, wantq, wantz, n, ilo, ihi, a, lda, b, ldb,alphar, alphai, &
     ...
     !! multipole rational QZ method with agressive early deflation"
           ...
           nbr = nsr+itemp1
           if( n < nmin .or. rec >= 2_ilp ) then
              call stdlib_whgeqz( wants, wantq, wantz, n, ilo, ihi, a, lda, b, ldb,alpha, beta, q,&
                         ldq, z, ldz, work, lwork, rwork,info )
              return
           end if
           ! find out required workspace
           ! workspace query to stdlib_wlaqz2
           nw = max( nwr, nmin )
           call stdlib_wlaqz2( ilschur, ilq, ilz, n, ilo, ihi, nw, a, lda, b, ldb,q, ldq, z, ldz, &
           n_undeflated, n_qeflated, alpha,beta, work, nw, work, nw, work, -1_ilp, rwork, rec,&
                     aed_info )
           itemp1 = int( work( 1_ilp ),KIND=ilp)
           ! workspace query to stdlib_wlaqz3
           call stdlib_wlaqz3( ilschur, ilq, ilz, n, ilo, ihi, nsr, nbr, alpha,beta, a, lda, b, &
                     ldb, q, ldq, z, ldz, work, nbr,work, nbr, work, -1_ilp, sweep_info )
           itemp2 = int( work( 1_ilp ),KIND=ilp)
           lworkreq = max( itemp1+2*nw**2_ilp, itemp2+2*nbr**2_ilp )
           if ( lwork ==-1_ilp ) then
           ...
           ! initialize q and z
           if( iwantq==3_ilp ) call stdlib_wlaset( 'FULL', n, n, czero, cone, q,ldq )
           if( iwantz==3_ilp ) call stdlib_wlaset( 'FULL', n, n, czero, cone, z,ldz )
           ! get machine constants
           safmin = stdlib_qlamch( 'SAFE MINIMUM' )
           safmax = one/safmin
           call stdlib_qlabad( safmin, safmax )
           ...
           do iiter = 1, maxit
              ...
              do while ( k>=istart2 )
                 ...
                 if( abs( b( k, k ) ) < max( smlnum, ulp*tempr ) ) then
                    ...
                    ! to the top and deflate it
                    do k2 = k, istart2+1, -1
                       call stdlib_wlartg( b( k2-1, k2 ), b( k2-1, k2-1 ), c1, s1,temp )
                       b( k2-1, k2 ) = temp
                       b( k2-1, k2-1 ) = czero
                       call stdlib_wrot( k2-2-istartm+1, b( istartm, k2 ), 1_ilp,b( istartm, k2-1 ), &
                                 1_ilp, c1, s1 )
                       call stdlib_wrot( min( k2+1, istop )-istartm+1, a( istartm,k2 ), 1_ilp, a( &
                                 istartm, k2-1 ), 1_ilp, c1, s1 )
                       if ( ilz ) then
                          call stdlib_wrot( n, z( 1_ilp, k2 ), 1_ilp, z( 1_ilp, k2-1 ), 1_ilp, c1,s1 )
                       end if
                       if( k2<istop ) then
                          ...
                          call stdlib_wrot( istopm-k2+1, a( k2, k2 ), lda, a( k2+1,k2 ), lda, c1, &
                                    s1 )
                          call stdlib_wrot( istopm-k2+1, b( k2, k2 ), ldb, b( k2+1,k2 ), ldb, c1, &
                                    s1 )
                          if( ilq ) then
                             call stdlib_wrot( n, q( 1_ilp, k2 ), 1_ilp, q( 1_ilp, k2+1 ), 1_ilp,c1, conjg( s1 ) )
                                       
                          end if
                       end if
                    end do
                    if( istart2<istop )then
                       call stdlib_wlartg( a( istart2, istart2 ), a( istart2+1,istart2 ), c1, s1, &
                                 temp )
                       a( istart2, istart2 ) = temp
                       a( istart2+1, istart2 ) = czero
                       call stdlib_wrot( istopm-( istart2+1 )+1_ilp, a( istart2,istart2+1 ), lda, a( &
                                 istart2+1,istart2+1 ), lda, c1, s1 )
                       call stdlib_wrot( istopm-( istart2+1 )+1_ilp, b( istart2,istart2+1 ), ldb, b( &
                                 istart2+1,istart2+1 ), ldb, c1, s1 )
                       if( ilq ) then
                          call stdlib_wrot( n, q( 1_ilp, istart2 ), 1_ilp, q( 1_ilp,istart2+1 ), 1_ilp, c1, conjg(&
                                     s1 ) )
                       end if
                    end if
                    istart2 = istart2+1
              ...
              ! time for aed
              call stdlib_wlaqz2( ilschur, ilq, ilz, n, istart2, istop, nw, a, lda,b, ldb, q, ldq,&
               z, ldz, n_undeflated, n_qeflated,alpha, beta, work, nw, work( nw**2_ilp+1 ), nw,work( &
                         2_ilp*nw**2_ilp+1 ), lwork-2*nw**2_ilp, rwork, rec,aed_info )
              if ( n_qeflated > 0_ilp ) then
                 istop = istop-n_qeflated
                 ld = 0_ilp
              ...
              ! time for a qz sweep
              call stdlib_wlaqz3( ilschur, ilq, ilz, n, istart2, istop, ns, nblock,alpha( &
              shiftpos ), beta( shiftpos ), a, lda, b,ldb, q, ldq, z, ldz, work, nblock, work( &
                        nblock**2_ilp+1 ), nblock, work( 2_ilp*nblock**2_ilp+1 ),lwork-2*nblock**2_ilp, sweep_info )
           end do
           ! call stdlib_whgeqz to normalize the eigenvalue blocks and set the eigenvalues
           ! if all the eigenvalues have been found, stdlib_whgeqz will not do any iterations
           ! and only normalize the blocks. in case of a rare convergence failure,
           ! the single shift might perform better.
        80 call stdlib_whgeqz( wants, wantq, wantz, n, ilo, ihi, a, lda, b, ldb,alpha, beta, q, &
                  ldq, z, ldz, work, lwork, rwork,norm_info )
           info = norm_info
     ...
     !! in the QZ algorithm.
           ...
           ! ================================================================
           ! local scalars
           real(qp) :: w(2_ilp), safmin, safmax, scale1, scale2
           safmin = stdlib_qlamch( 'SAFE MINIMUM' )
           safmax = one/safmin
           ! calculate first shifted vector
           w( 1_ilp ) = beta1*a( 1_ilp, 1_ilp )-sr1*b( 1_ilp, 1_ilp )
           ...
           ! check for overflow
           if( abs( v( 1_ilp ) )>safmax .or. abs( v( 2_ilp ) ) > safmax .or.abs( v( 3_ilp ) )>safmax .or. &
           stdlib_qisnan( v( 1_ilp ) ) .or.stdlib_qisnan( v( 2_ilp ) ) .or. stdlib_qisnan( v( 3_ilp ) ) ) &
                     then
              v( 1_ilp ) = zero
              v( 2_ilp ) = zero
              v( 3_ilp ) = zero
     ...
     !! ZLAQZ1: chases a 1x1 shift bulge in a matrix pencil down a single position
           ...
           if( k+1 == ihi ) then
              ! shift is located on the edge of the matrix, remove it
              call stdlib_wlartg( b( ihi, ihi ), b( ihi, ihi-1 ), c, s, temp )
              b( ihi, ihi ) = temp
              b( ihi, ihi-1 ) = czero
              call stdlib_wrot( ihi-istartm, b( istartm, ihi ), 1_ilp, b( istartm,ihi-1 ), 1_ilp, c, s )
                        
              call stdlib_wrot( ihi-istartm+1, a( istartm, ihi ), 1_ilp, a( istartm,ihi-1 ), 1_ilp, c, s )
                        
              if ( ilz ) then
                 call stdlib_wrot( nz, z( 1_ilp, ihi-zstart+1 ), 1_ilp, z( 1_ilp, ihi-1-zstart+1 ), 1_ilp, c, s )
                           
              end if
           else
              ! normal operation, move bulge down
              ! apply transformation from the right
              call stdlib_wlartg( b( k+1, k+1 ), b( k+1, k ), c, s, temp )
              b( k+1, k+1 ) = temp
              b( k+1, k ) = czero
              call stdlib_wrot( k+2-istartm+1, a( istartm, k+1 ), 1_ilp, a( istartm,k ), 1_ilp, c, s )
                        
              call stdlib_wrot( k-istartm+1, b( istartm, k+1 ), 1_ilp, b( istartm, k ),1_ilp, c, s )
                        
              if ( ilz ) then
                 call stdlib_wrot( nz, z( 1_ilp, k+1-zstart+1 ), 1_ilp, z( 1_ilp, k-zstart+1 ),1_ilp, c, s )
              ...
              a( k+1, k ) = temp
              a( k+2, k ) = czero
              call stdlib_wrot( istopm-k, a( k+1, k+1 ), lda, a( k+2, k+1 ), lda, c,s )
              call stdlib_wrot( istopm-k, b( k+1, k+1 ), ldb, b( k+2, k+1 ), ldb, c,s )
              if ( ilq ) then
                 call stdlib_wrot( nq, q( 1_ilp, k+1-qstart+1 ), 1_ilp, q( 1_ilp, k+2-qstart+1 ), 1_ilp, c, conjg(&
                            s ) )
              end if
           end if
     ...
     !! DLAQZ2: chases a 2x2 shift bulge in a matrix pencil down a single position
           ...
           ! ================================================================
           ! local variables
           real(qp) :: h(2_ilp,3_ilp), c1, s1, c2, s2, temp
           if( k+2 == ihi ) then
              ! shift is located on the edge of the matrix, remove it
              h = b( ihi-1:ihi, ihi-2:ihi )
              ! make h upper triangular
              call stdlib_qlartg( h( 1_ilp, 1_ilp ), h( 2_ilp, 1_ilp ), c1, s1, temp )
              h( 2_ilp, 1_ilp ) = zero
              h( 1_ilp, 1_ilp ) = temp
              call stdlib_qrot( 2_ilp, h( 1_ilp, 2_ilp ), 2_ilp, h( 2_ilp, 2_ilp ), 2_ilp, c1, s1 )
              call stdlib_qlartg( h( 2_ilp, 3_ilp ), h( 2_ilp, 2_ilp ), c1, s1, temp )
              call stdlib_qrot( 1_ilp, h( 1_ilp, 3_ilp ), 1_ilp, h( 1_ilp, 2_ilp ), 1_ilp, c1, s1 )
              call stdlib_qlartg( h( 1_ilp, 2_ilp ), h( 1_ilp, 1_ilp ), c2, s2, temp )
              call stdlib_qrot( ihi-istartm+1, b( istartm, ihi ), 1_ilp, b( istartm,ihi-1 ), 1_ilp, c1, &
                        s1 )
              call stdlib_qrot( ihi-istartm+1, b( istartm, ihi-1 ), 1_ilp, b( istartm,ihi-2 ), 1_ilp, c2, &
                        s2 )
              b( ihi-1, ihi-2 ) = zero
              b( ihi, ihi-2 ) = zero
              call stdlib_qrot( ihi-istartm+1, a( istartm, ihi ), 1_ilp, a( istartm,ihi-1 ), 1_ilp, c1, &
                        s1 )
              call stdlib_qrot( ihi-istartm+1, a( istartm, ihi-1 ), 1_ilp, a( istartm,ihi-2 ), 1_ilp, c2, &
                        s2 )
              if ( ilz ) then
                 call stdlib_qrot( nz, z( 1_ilp, ihi-zstart+1 ), 1_ilp, z( 1_ilp, ihi-1-zstart+1 ), 1_ilp, c1, s1 &
                           )
                 call stdlib_qrot( nz, z( 1_ilp, ihi-1-zstart+1 ), 1_ilp, z( 1_ilp,ihi-2-zstart+1 ), 1_ilp, c2, &
                           s2 )
              end if
              call stdlib_qlartg( a( ihi-1, ihi-2 ), a( ihi, ihi-2 ), c1, s1,temp )
              a( ihi-1, ihi-2 ) = temp
              a( ihi, ihi-2 ) = zero
              call stdlib_qrot( istopm-ihi+2, a( ihi-1, ihi-1 ), lda, a( ihi,ihi-1 ), lda, c1, s1 &
                        )
              call stdlib_qrot( istopm-ihi+2, b( ihi-1, ihi-1 ), ldb, b( ihi,ihi-1 ), ldb, c1, s1 &
                        )
              if ( ilq ) then
                 call stdlib_qrot( nq, q( 1_ilp, ihi-1-qstart+1 ), 1_ilp, q( 1_ilp, ihi-qstart+1 ), 1_ilp, c1, s1 &
                           )
              end if
              call stdlib_qlartg( b( ihi, ihi ), b( ihi, ihi-1 ), c1, s1, temp )
              b( ihi, ihi ) = temp
              b( ihi, ihi-1 ) = zero
              call stdlib_qrot( ihi-istartm, b( istartm, ihi ), 1_ilp, b( istartm,ihi-1 ), 1_ilp, c1, s1 )
              ...
              call stdlib_qlartg( h( 1_ilp, 2_ilp ), h( 1_ilp, 1_ilp ), c2, s2, temp )
              ! apply transformations from the right
              call stdlib_qrot( k+3-istartm+1, a( istartm, k+2 ), 1_ilp, a( istartm,k+1 ), 1_ilp, c1, s1 )
                        
              call stdlib_qrot( k+3-istartm+1, a( istartm, k+1 ), 1_ilp, a( istartm,k ), 1_ilp, c2, s2 )
                        
              call stdlib_qrot( k+2-istartm+1, b( istartm, k+2 ), 1_ilp, b( istartm,k+1 ), 1_ilp, c1, s1 )
                        
              call stdlib_qrot( k+2-istartm+1, b( istartm, k+1 ), 1_ilp, b( istartm,k ), 1_ilp, c2, s2 )
                        
              if ( ilz ) then
                 call stdlib_qrot( nz, z( 1_ilp, k+2-zstart+1 ), 1_ilp, z( 1_ilp, k+1-zstart+1 ), 1_ilp, c1, s1 )
                           
                 call stdlib_qrot( nz, z( 1_ilp, k+1-zstart+1 ), 1_ilp, z( 1_ilp, k-zstart+1 ),1_ilp, c2, s2 )
                           
              end if
              b( k+1, k ) = zero
              b( k+2, k ) = zero
              ! calculate q1 and q2
              call stdlib_qlartg( a( k+2, k ), a( k+3, k ), c1, s1, temp )
              a( k+2, k ) = temp
              a( k+3, k ) = zero
              call stdlib_qlartg( a( k+1, k ), a( k+2, k ), c2, s2, temp )
              a( k+1, k ) = temp
              a( k+2, k ) = zero
              ! apply transformations from the left
              call stdlib_qrot( istopm-k, a( k+2, k+1 ), lda, a( k+3, k+1 ), lda,c1, s1 )
              call stdlib_qrot( istopm-k, a( k+1, k+1 ), lda, a( k+2, k+1 ), lda,c2, s2 )
              call stdlib_qrot( istopm-k, b( k+2, k+1 ), ldb, b( k+3, k+1 ), ldb,c1, s1 )
              call stdlib_qrot( istopm-k, b( k+1, k+1 ), ldb, b( k+2, k+1 ), ldb,c2, s2 )
              if ( ilq ) then
                 call stdlib_qrot( nq, q( 1_ilp, k+2-qstart+1 ), 1_ilp, q( 1_ilp, k+3-qstart+1 ), 1_ilp, c1, s1 )
                           
                 call stdlib_qrot( nq, q( 1_ilp, k+1-qstart+1 ), 1_ilp, q( 1_ilp, k+2-qstart+1 ), 1_ilp, c2, s2 )
     ...
     !! ZLAQZ2: performs AED
           ...
           ifst = 1_ilp
           ilst = jw
           call stdlib_wlaqz0( 'S', 'V', 'V', jw, 1_ilp, jw, a( kwtop, kwtop ), lda,b( kwtop, kwtop ),&
                      ldb, alpha, beta, qc, ldqc, zc,ldzc, work, -1_ilp, rwork, rec+1, qz_small_info )
           lworkreq = int( work( 1_ilp ),KIND=ilp)+2_ilp*jw**2_ilp
           lworkreq = max( lworkreq, n*nw, 2_ilp*nw**2_ilp+n )
           if ( lwork ==-1_ilp ) then
           ...
           call stdlib_wlacpy( 'ALL', jw, jw, b( kwtop, kwtop ), ldb, work( jw**2_ilp+1 ), jw )
           ...
           if( qz_small_info /= 0_ilp ) then
              ...
              call stdlib_wlacpy( 'ALL', jw, jw, work( jw**2_ilp+1 ), jw, b( kwtop,kwtop ), ldb )
           ...
           if ( kwtop == ilo .or. s == czero ) then
              ...
              do while ( k <= jw )
                    ...
                    if ( ( abs( s*qc( 1_ilp, kwbot-kwtop+1 ) ) ) <= max( ulp*tempr, smlnum ) ) &
                       ...
                       ifst = kwbot-kwtop+1
                       ilst = k2
                       call stdlib_wtgexc( .true., .true., jw, a( kwtop, kwtop ),lda, b( kwtop, &
                                 kwtop ), ldb, qc, ldqc,zc, ldzc, ifst, ilst, ztgexc_info )
                       k2 = k2+1
                    end if
                    k = k+1
           ...
           if ( kwtop /= ilo .and. s /= czero ) then
              ...
              a( kwtop:kwbot, kwtop-1 ) = a( kwtop, kwtop-1 ) *conjg( qc( 1_ilp,1_ilp:jw-nd ) )
              do k = kwbot-1, kwtop, -1
                 call stdlib_wlartg( a( k, kwtop-1 ), a( k+1, kwtop-1 ), c1, s1,temp )
                 a( k, kwtop-1 ) = temp
                 a( k+1, kwtop-1 ) = czero
                 k2 = max( kwtop, k-1 )
                 call stdlib_wrot( ihi-k2+1, a( k, k2 ), lda, a( k+1, k2 ), lda, c1,s1 )
                 call stdlib_wrot( ihi-( k-1 )+1_ilp, b( k, k-1 ), ldb, b( k+1, k-1 ),ldb, c1, s1 )
                           
                 call stdlib_wrot( jw, qc( 1_ilp, k-kwtop+1 ), 1_ilp, qc( 1_ilp, k+1-kwtop+1 ),1_ilp, c1, conjg( &
                           s1 ) )
              end do
              ! chase bulges down
              istartm = kwtop
              ...
              do while ( k >= kwtop )
                 ! move bulge down and remove it
                 do k2 = k, kwbot-1
                    call stdlib_wlaqz1( .true., .true., k2, kwtop, kwtop+jw-1,kwbot, a, lda, b, &
                              ldb, jw, kwtop, qc, ldqc,jw, kwtop, zc, ldzc )
                 end do
                 k = k-1
              end do
           ...
           if ( istopm-ihi > 0_ilp ) then
              call stdlib_wgemm( 'C', 'N', jw, istopm-ihi, jw, cone, qc, ldqc,a( kwtop, ihi+1 ), &
                        lda, czero, work, jw )
              call stdlib_wlacpy( 'ALL', jw, istopm-ihi, work, jw, a( kwtop,ihi+1 ), lda )
              call stdlib_wgemm( 'C', 'N', jw, istopm-ihi, jw, cone, qc, ldqc,b( kwtop, ihi+1 ), &
                        ldb, czero, work, jw )
              call stdlib_wlacpy( 'ALL', jw, istopm-ihi, work, jw, b( kwtop,ihi+1 ), ldb )
           end if
           if ( ilq ) then
              call stdlib_wgemm( 'N', 'N', n, jw, jw, cone, q( 1_ilp, kwtop ), ldq, qc,ldqc, czero, &
                        work, n )
              call stdlib_wlacpy( 'ALL', n, jw, work, n, q( 1_ilp, kwtop ), ldq )
           end if
           if ( kwtop-1-istartm+1 > 0_ilp ) then
              call stdlib_wgemm( 'N', 'N', kwtop-istartm, jw, jw, cone, a( istartm,kwtop ), lda, &
                        zc, ldzc, czero, work,kwtop-istartm )
             call stdlib_wlacpy( 'ALL', kwtop-istartm, jw, work, kwtop-istartm,a( istartm, kwtop )&
                       , lda )
              call stdlib_wgemm( 'N', 'N', kwtop-istartm, jw, jw, cone, b( istartm,kwtop ), ldb, &
                        zc, ldzc, czero, work,kwtop-istartm )
             call stdlib_wlacpy( 'ALL', kwtop-istartm, jw, work, kwtop-istartm,b( istartm, kwtop )&
                       , ldb )
           end if
           if ( ilz ) then
              call stdlib_wgemm( 'N', 'N', n, jw, jw, cone, z( 1_ilp, kwtop ), ldz, zc,ldzc, czero, &
                        work, n )
              call stdlib_wlacpy( 'ALL', n, jw, work, n, z( 1_ilp, kwtop ), ldz )
           end if
     ...
     !! DLAQZ3: performs AED
           ...
           ifst = 1_ilp
           ilst = jw
           call stdlib_qtgexc( .true., .true., jw, a, lda, b, ldb, qc, ldqc, zc,ldzc, ifst, ilst, &
                     work, -1_ilp, dtgexc_info )
           lworkreq = int( work( 1_ilp ),KIND=ilp)
           call stdlib_qlaqz0( 'S', 'V', 'V', jw, 1_ilp, jw, a( kwtop, kwtop ), lda,b( kwtop, kwtop ),&
                      ldb, alphar, alphai, beta, qc,ldqc, zc, ldzc, work, -1_ilp, rec+1, qz_small_info )
           lworkreq = max( lworkreq, int( work( 1_ilp ),KIND=ilp)+2_ilp*jw**2_ilp )
           lworkreq = max( lworkreq, n*nw, 2_ilp*nw**2_ilp+n )
           if ( lwork ==-1_ilp ) then
           ...
           call stdlib_qlacpy( 'ALL', jw, jw, b( kwtop, kwtop ), ldb, work( jw**2_ilp+1 ), jw )
           ...
           if( qz_small_info /= 0_ilp ) then
              ...
              call stdlib_qlacpy( 'ALL', jw, jw, work( jw**2_ilp+1 ), jw, b( kwtop,kwtop ), ldb )
           ...
           do while ( k <= ihi )
              ...
              if ( bulge ) then
                 ! 2x2 eigenvalue block
                 call stdlib_qlag2( a( k, k ), lda, b( k, k ), ldb, safmin,beta( k ), beta( k+1 ),&
                            alphar( k ),alphar( k+1 ), alphai( k ) )
                 alphai( k+1 ) = -alphai( k )
                 k = k+2
              else
           ...
           if ( kwtop /= ilo .and. s /= zero ) then
              ...
              a( kwtop:kwbot, kwtop-1 ) = a( kwtop, kwtop-1 )*qc( 1_ilp,1_ilp:jw-nd )
              do k = kwbot-1, kwtop, -1
                 call stdlib_qlartg( a( k, kwtop-1 ), a( k+1, kwtop-1 ), c1, s1,temp )
                 a( k, kwtop-1 ) = temp
                 a( k+1, kwtop-1 ) = zero
                 k2 = max( kwtop, k-1 )
                 call stdlib_qrot( ihi-k2+1, a( k, k2 ), lda, a( k+1, k2 ), lda, c1,s1 )
                 call stdlib_qrot( ihi-( k-1 )+1_ilp, b( k, k-1 ), ldb, b( k+1, k-1 ),ldb, c1, s1 )
                           
                 call stdlib_qrot( jw, qc( 1_ilp, k-kwtop+1 ), 1_ilp, qc( 1_ilp, k+1-kwtop+1 ),1_ilp, c1, s1 )
              ...
              do while ( k >= kwtop )
                 if ( ( k >= kwtop+1 ) .and. a( k+1, k-1 ) /= zero ) then
                    ! move double pole block down and remove it
                    do k2 = k-1, kwbot-2
                       call stdlib_qlaqz2( .true., .true., k2, kwtop, kwtop+jw-1,kwbot, a, lda, b,&
                                  ldb, jw, kwtop, qc,ldqc, jw, kwtop, zc, ldzc )
                    end do
                    k = k-2
                 else
                    ...
                    do k2 = k, kwbot-2
                       ! move shift down
                       call stdlib_qlartg( b( k2+1, k2+1 ), b( k2+1, k2 ), c1, s1,temp )
                       b( k2+1, k2+1 ) = temp
                       b( k2+1, k2 ) = zero
                       call stdlib_qrot( k2+2-istartm+1, a( istartm, k2+1 ), 1_ilp,a( istartm, k2 ), &
                                 1_ilp, c1, s1 )
                       call stdlib_qrot( k2-istartm+1, b( istartm, k2+1 ), 1_ilp,b( istartm, k2 ), 1_ilp, &
                                 c1, s1 )
                       call stdlib_qrot( jw, zc( 1_ilp, k2+1-kwtop+1 ), 1_ilp, zc( 1_ilp,k2-kwtop+1 ), 1_ilp, c1, &
                                 s1 )
                       call stdlib_qlartg( a( k2+1, k2 ), a( k2+2, k2 ), c1, s1,temp )
                       a( k2+1, k2 ) = temp
                       a( k2+2, k2 ) = zero
                       call stdlib_qrot( istopm-k2, a( k2+1, k2+1 ), lda, a( k2+2,k2+1 ), lda, c1,&
                                  s1 )
                       call stdlib_qrot( istopm-k2, b( k2+1, k2+1 ), ldb, b( k2+2,k2+1 ), ldb, c1,&
                                  s1 )
                       call stdlib_qrot( jw, qc( 1_ilp, k2+1-kwtop+1 ), 1_ilp, qc( 1_ilp,k2+2-kwtop+1 ), 1_ilp, &
                                 c1, s1 )
                    end do
                    ! remove the shift
                    ...
                    b( kwbot, kwbot ) = temp
                    b( kwbot, kwbot-1 ) = zero
                    call stdlib_qrot( kwbot-istartm, b( istartm, kwbot ), 1_ilp,b( istartm, kwbot-1 ),&
                               1_ilp, c1, s1 )
                    call stdlib_qrot( kwbot-istartm+1, a( istartm, kwbot ), 1_ilp,a( istartm, kwbot-1 &
                              ), 1_ilp, c1, s1 )
                    call stdlib_qrot( jw, zc( 1_ilp, kwbot-kwtop+1 ), 1_ilp, zc( 1_ilp,kwbot-1-kwtop+1 ), 1_ilp, &
                              c1, s1 )
                    k = k-1
                 end if
              end do
           ...
           if ( istopm-ihi > 0_ilp ) then
              call stdlib_qgemm( 'T', 'N', jw, istopm-ihi, jw, one, qc, ldqc,a( kwtop, ihi+1 ), &
                        lda, zero, work, jw )
              call stdlib_qlacpy( 'ALL', jw, istopm-ihi, work, jw, a( kwtop,ihi+1 ), lda )
              call stdlib_qgemm( 'T', 'N', jw, istopm-ihi, jw, one, qc, ldqc,b( kwtop, ihi+1 ), &
                        ldb, zero, work, jw )
              call stdlib_qlacpy( 'ALL', jw, istopm-ihi, work, jw, b( kwtop,ihi+1 ), ldb )
           end if
           if ( ilq ) then
              call stdlib_qgemm( 'N', 'N', n, jw, jw, one, q( 1_ilp, kwtop ), ldq, qc,ldqc, zero, &
                        work, n )
              call stdlib_qlacpy( 'ALL', n, jw, work, n, q( 1_ilp, kwtop ), ldq )
           end if
           if ( kwtop-1-istartm+1 > 0_ilp ) then
              call stdlib_qgemm( 'N', 'N', kwtop-istartm, jw, jw, one, a( istartm,kwtop ), lda, &
                        zc, ldzc, zero, work,kwtop-istartm )
              call stdlib_qlacpy( 'ALL', kwtop-istartm, jw, work, kwtop-istartm,a( istartm, kwtop &
                        ), lda )
              call stdlib_qgemm( 'N', 'N', kwtop-istartm, jw, jw, one, b( istartm,kwtop ), ldb, &
                        zc, ldzc, zero, work,kwtop-istartm )
              call stdlib_qlacpy( 'ALL', kwtop-istartm, jw, work, kwtop-istartm,b( istartm, kwtop &
                        ), ldb )
           end if
           if ( ilz ) then
              call stdlib_qgemm( 'N', 'N', n, jw, jw, one, z( 1_ilp, kwtop ), ldz, zc,ldzc, zero, &
                        work, n )
              call stdlib_qlacpy( 'ALL', n, jw, work, n, z( 1_ilp, kwtop ), ldz )
           end if
     ...
     !! ZLAQZ3: Executes a single multishift QZ sweep
           ...
           ns = nshifts
           npos = max( nblock_qesired-ns, 1_ilp )
           ! the following block introduces the shifts and chases
           ! them down one by one just enough to make space for
           ! the other shifts. the near-the-diagonal block is
           ! of size (ns+1) x ns.
           call stdlib_wlaset( 'FULL', ns+1, ns+1, czero, cone, qc, ldqc )
           call stdlib_wlaset( 'FULL', ns, ns, czero, cone, zc, ldzc )
           do i = 1, ns
              ! introduce the shift
              scale = sqrt( abs( alpha( i ) ) ) * sqrt( abs( beta( i ) ) )
              if( scale >= safmin .and. scale <= safmax ) then
              ...
              call stdlib_wlartg( temp2, temp3, c, s, temp )
              call stdlib_wrot( ns, a( ilo, ilo ), lda, a( ilo+1, ilo ), lda, c,s )
              call stdlib_wrot( ns, b( ilo, ilo ), ldb, b( ilo+1, ilo ), ldb, c,s )
              call stdlib_wrot( ns+1, qc( 1_ilp, 1_ilp ), 1_ilp, qc( 1_ilp, 2_ilp ), 1_ilp, c,conjg( s ) )
              ! chase the shift down
              do j = 1, ns-i
                 call stdlib_wlaqz1( .true., .true., j, 1_ilp, ns, ihi-ilo+1, a( ilo,ilo ), lda, b( &
                           ilo, ilo ), ldb, ns+1, 1_ilp, qc,ldqc, ns, 1_ilp, zc, ldzc )
              end do
           end do
           ! update the rest of the pencil
           ...
           if ( ilq ) then
              call stdlib_wgemm( 'N', 'N', n, sheight, sheight, cone, q( 1_ilp, ilo ),ldq, qc, ldqc, &
                        czero, work, n )
              call stdlib_wlacpy( 'ALL', n, sheight, work, n, q( 1_ilp, ilo ), ldq )
           end if
           ! update a(istartm:ilo-1,ilo:ilo+ns-1) and b(istartm:ilo-1,ilo:ilo+ns-1)
           ! from the right with zc(1:ns,1:ns)
           sheight = ilo-1-istartm+1
           ...
           if ( ilz ) then
              call stdlib_wgemm( 'N', 'N', n, swidth, swidth, cone, z( 1_ilp, ilo ),ldz, zc, ldzc, &
                        czero, work, n )
              call stdlib_wlacpy( 'ALL', n, swidth, work, n, z( 1_ilp, ilo ), ldz )
           end if
           ! the following block chases the shifts down to the bottom
           ! right block. if possible, a shift is moved down npos
           ! positions at a time
           ...
           do while ( k < ihi-ns )
              ...
              ! istopb points to the last column we will be updating
              istopb = k+nblock-1
              call stdlib_wlaset( 'FULL', ns+np, ns+np, czero, cone, qc, ldqc )
              call stdlib_wlaset( 'FULL', ns+np, ns+np, czero, cone, zc, ldzc )
              ! near the diagonal shift chase
              do i = ns-1, 0, -1
                 do j = 0, np-1
                    ! move down the block with index k+i+j, updating
                    ! the (ns+np x ns+np) block:
                    ! (k:k+ns+np,k:k+ns+np-1)
                    call stdlib_wlaqz1( .true., .true., k+i+j, istartb, istopb, ihi,a, lda, b, &
                              ldb, nblock, k+1, qc, ldqc,nblock, k, zc, ldzc )
                 end do
              end do
              ! update rest of the pencil
              ...
              swidth = istopm-( k+ns+np )+1_ilp
              if ( swidth > 0_ilp ) then
                 call stdlib_wgemm( 'C', 'N', sheight, swidth, sheight, cone, qc,ldqc, a( k+1, k+&
                           ns+np ), lda, czero, work,sheight )
                 call stdlib_wlacpy( 'ALL', sheight, swidth, work, sheight, a( k+1,k+ns+np ), lda &
                           )
                 call stdlib_wgemm( 'C', 'N', sheight, swidth, sheight, cone, qc,ldqc, b( k+1, k+&
                           ns+np ), ldb, czero, work,sheight )
                 call stdlib_wlacpy( 'ALL', sheight, swidth, work, sheight, b( k+1,k+ns+np ), ldb &
                           )
              end if
              if ( ilq ) then
                 call stdlib_wgemm( 'N', 'N', n, nblock, nblock, cone, q( 1_ilp, k+1 ),ldq, qc, ldqc, &
                           czero, work, n )
                 call stdlib_wlacpy( 'ALL', n, nblock, work, n, q( 1_ilp, k+1 ), ldq )
              end if
              ! update a(istartm:k,k:k+ns+npos-1) and b(istartm:k,k:k+ns+npos-1)
              ! from the right with zc(1:ns+np,1:ns+np)
              sheight = k-istartm+1
              ...
              if ( ilz ) then
                 call stdlib_wgemm( 'N', 'N', n, nblock, nblock, cone, z( 1_ilp, k ),ldz, zc, ldzc, &
                           czero, work, n )
                 call stdlib_wlacpy( 'ALL', n, nblock, work, n, z( 1_ilp, k ), ldz )
              end if
              k = k+np
           end do
           ! the following block removes the shifts from the bottom right corner
           ! one by one. updates are initially applied to a(ihi-ns+1:ihi,ihi-ns:ihi).
           call stdlib_wlaset( 'FULL', ns, ns, czero, cone, qc, ldqc )
           call stdlib_wlaset( 'FULL', ns+1, ns+1, czero, cone, zc, ldzc )
           ! istartb points to the first row we will be updating
           istartb = ihi-ns+1
           ! istopb points to the last column we will be updating
           istopb = ihi
           do i = 1, ns
              ! chase the shift down to the bottom right corner
              do ishift = ihi-i, ihi-1
                 call stdlib_wlaqz1( .true., .true., ishift, istartb, istopb, ihi,a, lda, b, ldb, &
                           ns, ihi-ns+1, qc, ldqc, ns+1,ihi-ns, zc, ldzc )
              end do
           end do
           ! update rest of the pencil
           ...
           swidth = istopm-( ihi+1 )+1_ilp
           if ( swidth > 0_ilp ) then
              call stdlib_wgemm( 'C', 'N', sheight, swidth, sheight, cone, qc, ldqc,a( ihi-ns+1, &
                        ihi+1 ), lda, czero, work, sheight )
              call stdlib_wlacpy( 'ALL', sheight, swidth, work, sheight,a( ihi-ns+1, ihi+1 ), lda &
                        )
              call stdlib_wgemm( 'C', 'N', sheight, swidth, sheight, cone, qc, ldqc,b( ihi-ns+1, &
                        ihi+1 ), ldb, czero, work, sheight )
              call stdlib_wlacpy( 'ALL', sheight, swidth, work, sheight,b( ihi-ns+1, ihi+1 ), ldb &
                        )
           end if
           if ( ilq ) then
              call stdlib_wgemm( 'N', 'N', n, ns, ns, cone, q( 1_ilp, ihi-ns+1 ), ldq,qc, ldqc, czero,&
                         work, n )
              call stdlib_wlacpy( 'ALL', n, ns, work, n, q( 1_ilp, ihi-ns+1 ), ldq )
           end if
           ! update a(istartm:ihi-ns,ihi-ns:ihi)
           ! from the right with zc(1:ns+1,1:ns+1)
           sheight = ihi-ns-istartm+1
           swidth = ns+1
           if ( sheight > 0_ilp ) then
              call stdlib_wgemm( 'N', 'N', sheight, swidth, swidth, cone,a( istartm, ihi-ns ), &
                        lda, zc, ldzc, czero, work,sheight )
              call stdlib_wlacpy( 'ALL', sheight, swidth, work, sheight, a( istartm,ihi-ns ), lda &
                        )
              call stdlib_wgemm( 'N', 'N', sheight, swidth, swidth, cone,b( istartm, ihi-ns ), &
                        ldb, zc, ldzc, czero, work,sheight )
              call stdlib_wlacpy( 'ALL', sheight, swidth, work, sheight, b( istartm,ihi-ns ), ldb &
                        )
           end if
           if ( ilz ) then
              call stdlib_wgemm( 'N', 'N', n, ns+1, ns+1, cone, z( 1_ilp, ihi-ns ), ldz,zc, ldzc, &
                        czero, work, n )
              call stdlib_wlacpy( 'ALL', n, ns+1, work, n, z( 1_ilp, ihi-ns ), ldz )
           end if
     ...
     !! DLAQZ4: Executes a single multishift QZ sweep
           ...
           integer(ilp) :: i, j, ns, istartm, istopm, sheight, swidth, k, np, istartb, istopb, &
                     ishift, nblock, npos
           real(qp) :: temp, v(3_ilp), c1, s1, c2, s2, swap
           info = 0_ilp
           if ( nblock_qesired < nshifts+1 ) then
              info = -8_ilp
           end if
           ...
           ! the other shifts. the near-the-diagonal block is
           ! of size (ns+1) x ns.
           call stdlib_qlaset( 'FULL', ns+1, ns+1, zero, one, qc, ldqc )
           call stdlib_qlaset( 'FULL', ns, ns, zero, one, zc, ldzc )
           do i = 1, ns, 2
              ! introduce the shift
              call stdlib_qlaqz1( a( ilo, ilo ), lda, b( ilo, ilo ), ldb, sr( i ),sr( i+1 ), si( &
                        i ), ss( i ), ss( i+1 ), v )
              temp = v( 2_ilp )
              call stdlib_qlartg( temp, v( 3_ilp ), c1, s1, v( 2_ilp ) )
              call stdlib_qlartg( v( 1_ilp ), v( 2_ilp ), c2, s2, temp )
              call stdlib_qrot( ns, a( ilo+1, ilo ), lda, a( ilo+2, ilo ), lda, c1,s1 )
              call stdlib_qrot( ns, a( ilo, ilo ), lda, a( ilo+1, ilo ), lda, c2,s2 )
              call stdlib_qrot( ns, b( ilo+1, ilo ), ldb, b( ilo+2, ilo ), ldb, c1,s1 )
              call stdlib_qrot( ns, b( ilo, ilo ), ldb, b( ilo+1, ilo ), ldb, c2,s2 )
              call stdlib_qrot( ns+1, qc( 1_ilp, 2_ilp ), 1_ilp, qc( 1_ilp, 3_ilp ), 1_ilp, c1, s1 )
              call stdlib_qrot( ns+1, qc( 1_ilp, 1_ilp ), 1_ilp, qc( 1_ilp, 2_ilp ), 1_ilp, c2, s2 )
              ! chase the shift down
              do j = 1, ns-1-i
                 call stdlib_qlaqz2( .true., .true., j, 1_ilp, ns, ihi-ilo+1, a( ilo,ilo ), lda, b( &
                           ilo, ilo ), ldb, ns+1, 1_ilp, qc,ldqc, ns, 1_ilp, zc, ldzc )
              end do
           end do
           ! update the rest of the pencil
           ...
           if ( ilq ) then
              call stdlib_qgemm( 'N', 'N', n, sheight, sheight, one, q( 1_ilp, ilo ),ldq, qc, ldqc, &
                        zero, work, n )
              call stdlib_qlacpy( 'ALL', n, sheight, work, n, q( 1_ilp, ilo ), ldq )
           end if
           ! update a(istartm:ilo-1,ilo:ilo+ns-1) and b(istartm:ilo-1,ilo:ilo+ns-1)
           ! from the right with zc(1:ns,1:ns)
           sheight = ilo-1-istartm+1
           ...
           if ( ilz ) then
              call stdlib_qgemm( 'N', 'N', n, swidth, swidth, one, z( 1_ilp, ilo ), ldz,zc, ldzc, &
                        zero, work, n )
              call stdlib_qlacpy( 'ALL', n, swidth, work, n, z( 1_ilp, ilo ), ldz )
           end if
           ! the following block chases the shifts down to the bottom
           ! right block. if possible, a shift is moved down npos
           ! positions at a time
           ...
           do while ( k < ihi-ns )
              ...
              ! istopb points to the last column we will be updating
              istopb = k+nblock-1
              call stdlib_qlaset( 'FULL', ns+np, ns+np, zero, one, qc, ldqc )
              call stdlib_qlaset( 'FULL', ns+np, ns+np, zero, one, zc, ldzc )
              ! near the diagonal shift chase
              do i = ns-1, 0, -2
                 do j = 0, np-1
                    ! move down the block with index k+i+j-1, updating
                    ! the (ns+np x ns+np) block:
                    ! (k:k+ns+np,k:k+ns+np-1)
                    call stdlib_qlaqz2( .true., .true., k+i+j-1, istartb, istopb,ihi, a, lda, b, &
                              ldb, nblock, k+1, qc, ldqc,nblock, k, zc, ldzc )
                 end do
              end do
              ! update rest of the pencil
              ...
              swidth = istopm-( k+ns+np )+1_ilp
              if ( swidth > 0_ilp ) then
                 call stdlib_qgemm( 'T', 'N', sheight, swidth, sheight, one, qc,ldqc, a( k+1, k+&
                           ns+np ), lda, zero, work,sheight )
                 call stdlib_qlacpy( 'ALL', sheight, swidth, work, sheight, a( k+1,k+ns+np ), lda &
                           )
                 call stdlib_qgemm( 'T', 'N', sheight, swidth, sheight, one, qc,ldqc, b( k+1, k+&
                           ns+np ), ldb, zero, work,sheight )
                 call stdlib_qlacpy( 'ALL', sheight, swidth, work, sheight, b( k+1,k+ns+np ), ldb &
                           )
              end if
              if ( ilq ) then
                 call stdlib_qgemm( 'N', 'N', n, nblock, nblock, one, q( 1_ilp, k+1 ),ldq, qc, ldqc, &
                           zero, work, n )
                 call stdlib_qlacpy( 'ALL', n, nblock, work, n, q( 1_ilp, k+1 ), ldq )
              end if
              ! update a(istartm:k,k:k+ns+npos-1) and b(istartm:k,k:k+ns+npos-1)
              ! from the right with zc(1:ns+np,1:ns+np)
              sheight = k-istartm+1
              ...
              if ( ilz ) then
                 call stdlib_qgemm( 'N', 'N', n, nblock, nblock, one, z( 1_ilp, k ),ldz, zc, ldzc, &
                           zero, work, n )
                 call stdlib_qlacpy( 'ALL', n, nblock, work, n, z( 1_ilp, k ), ldz )
              end if
              k = k+np
           end do
           ! the following block removes the shifts from the bottom right corner
           ! one by one. updates are initially applied to a(ihi-ns+1:ihi,ihi-ns:ihi).
           call stdlib_qlaset( 'FULL', ns, ns, zero, one, qc, ldqc )
           call stdlib_qlaset( 'FULL', ns+1, ns+1, zero, one, zc, ldzc )
           ! istartb points to the first row we will be updating
           istartb = ihi-ns+1
           ! istopb points to the last column we will be updating
           istopb = ihi
           do i = 1, ns, 2
              ! chase the shift down to the bottom right corner
              do ishift = ihi-i-1, ihi-2
                 call stdlib_qlaqz2( .true., .true., ishift, istartb, istopb, ihi,a, lda, b, ldb, &
                           ns, ihi-ns+1, qc, ldqc, ns+1,ihi-ns, zc, ldzc )
              end do
           end do
           ! update rest of the pencil
           ...
           swidth = istopm-( ihi+1 )+1_ilp
           if ( swidth > 0_ilp ) then
              call stdlib_qgemm( 'T', 'N', sheight, swidth, sheight, one, qc, ldqc,a( ihi-ns+1, &
                        ihi+1 ), lda, zero, work, sheight )
              call stdlib_qlacpy( 'ALL', sheight, swidth, work, sheight,a( ihi-ns+1, ihi+1 ), lda &
                        )
              call stdlib_qgemm( 'T', 'N', sheight, swidth, sheight, one, qc, ldqc,b( ihi-ns+1, &
                        ihi+1 ), ldb, zero, work, sheight )
              call stdlib_qlacpy( 'ALL', sheight, swidth, work, sheight,b( ihi-ns+1, ihi+1 ), ldb &
                        )
           end if
           if ( ilq ) then
              call stdlib_qgemm( 'N', 'N', n, ns, ns, one, q( 1_ilp, ihi-ns+1 ), ldq,qc, ldqc, zero, &
                        work, n )
              call stdlib_qlacpy( 'ALL', n, ns, work, n, q( 1_ilp, ihi-ns+1 ), ldq )
           end if
           ! update a(istartm:ihi-ns,ihi-ns:ihi)
           ! from the right with zc(1:ns+1,1:ns+1)
           sheight = ihi-ns-istartm+1
           swidth = ns+1
           if ( sheight > 0_ilp ) then
              call stdlib_qgemm( 'N', 'N', sheight, swidth, swidth, one, a( istartm,ihi-ns ), lda,&
                         zc, ldzc, zero, work, sheight )
              call stdlib_qlacpy( 'ALL', sheight, swidth, work, sheight, a( istartm,ihi-ns ), lda &
                        )
              call stdlib_qgemm( 'N', 'N', sheight, swidth, swidth, one, b( istartm,ihi-ns ), ldb,&
                         zc, ldzc, zero, work, sheight )
              call stdlib_qlacpy( 'ALL', sheight, swidth, work, sheight, b( istartm,ihi-ns ), ldb &
                        )
           end if
           if ( ilz ) then
              call stdlib_qgemm( 'N', 'N', n, ns+1, ns+1, one, z( 1_ilp, ihi-ns ), ldz,zc, ldzc, zero,&
                         work, n )
              call stdlib_qlacpy( 'ALL', n, ns+1, work, n, z( 1_ilp, ihi-ns ), ldz )
           end if
