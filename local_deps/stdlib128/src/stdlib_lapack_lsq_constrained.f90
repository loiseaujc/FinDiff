  ...
  contains
     ...
     !! B = (0 R)*Q,   A = Z*T*Q.
        ...
        ! =====================================================================
           ...
           ! where t12 and r11 are upper triangular, and q and z are
           ! orthogonal.
           call stdlib_qggrqf( p, m, n, b, ldb, work, a, lda, work( p+1 ),work( p+mn+1 ), lwork-p-&
                     mn, info )
           lopt = work( p+mn+1 )
           ! update c = z**t *c = ( c1 ) n-p
                                ! ( c2 ) m+p-n
           call stdlib_qormqr( 'LEFT', 'TRANSPOSE', m, 1_ilp, mn, a, lda, work( p+1 ),c, max( 1_ilp, m ), &
                     work( p+mn+1 ), lwork-p-mn, info )
           lopt = max( lopt, int( work( p+mn+1 ),KIND=ilp) )
           ! solve t12*x2 = d for x2
           if( p>0_ilp ) then
              call stdlib_qtrtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', p, 1_ilp,b( 1_ilp, n-p+1 ), ldb, d,&
                         p, info )
              if( info>0_ilp ) then
                 info = 1_ilp
                 return
              ...
              ! put the solution in x
              call stdlib_qcopy( p, d, 1_ilp, x( n-p+1 ), 1_ilp )
              ! update c1
              call stdlib_qgemv( 'NO TRANSPOSE', n-p, p, -one, a( 1_ilp, n-p+1 ), lda,d, 1_ilp, one, c, 1_ilp &
                        )
           end if
           ! solve r11*x1 = c1 for x1
           if( n>p ) then
              call stdlib_qtrtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n-p, 1_ilp,a, lda, c, n-p, &
                        info )
              if( info>0_ilp ) then
                 info = 2_ilp
                 return
              ...
              ! put the solutions in x
              call stdlib_qcopy( n-p, c, 1_ilp, x, 1_ilp )
           end if
           ! compute the residual vector:
           if( m<n ) then
              nr = m + p - n
              if( nr>0_ilp )call stdlib_qgemv( 'NO TRANSPOSE', nr, n-m, -one, a( n-p+1, m+1 ),lda, d( &
                        nr+1 ), 1_ilp, one, c( n-p+1 ), 1_ilp )
           else
              nr = p
           end if
           if( nr>0_ilp ) then
              call stdlib_qtrmv( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', nr,a( n-p+1, n-p+1 ), lda, &
                        d, 1_ilp )
              call stdlib_qaxpy( nr, -one, d, 1_ilp, c( n-p+1 ), 1_ilp )
           end if
           ! backward transformation x = q**t*x
           call stdlib_qormrq( 'LEFT', 'TRANSPOSE', n, 1_ilp, p, b, ldb, work( 1_ilp ), x,n, work( p+mn+1 &
                     ), lwork-p-mn, info )
           work( 1_ilp ) = p + mn + max( lopt, int( work( p+mn+1 ),KIND=ilp) )
           return
     ...
     !! B = (0 R)*Q,   A = Z*T*Q.
        ...
        ! =====================================================================
           ...
           ! where t12 and r11 are upper triangular, and q and z are
           ! unitary.
           call stdlib_wggrqf( p, m, n, b, ldb, work, a, lda, work( p+1 ),work( p+mn+1 ), lwork-p-&
                     mn, info )
           lopt = real( work( p+mn+1 ),KIND=qp)
           ! update c = z**h *c = ( c1 ) n-p
                             ! ( c2 ) m+p-n
           call stdlib_wunmqr( 'LEFT', 'CONJUGATE TRANSPOSE', m, 1_ilp, mn, a, lda,work( p+1 ), c, &
                     max( 1_ilp, m ), work( p+mn+1 ),lwork-p-mn, info )
           lopt = max( lopt, int( work( p+mn+1 ),KIND=ilp) )
           ! solve t12*x2 = d for x2
           if( p>0_ilp ) then
              call stdlib_wtrtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', p, 1_ilp,b( 1_ilp, n-p+1 ), ldb, d,&
                         p, info )
              if( info>0_ilp ) then
                 info = 1_ilp
                 return
              ...
              ! put the solution in x
              call stdlib_wcopy( p, d, 1_ilp, x( n-p+1 ), 1_ilp )
              ! update c1
              call stdlib_wgemv( 'NO TRANSPOSE', n-p, p, -cone, a( 1_ilp, n-p+1 ), lda,d, 1_ilp, cone, c, &
                        1_ilp )
           end if
           ! solve r11*x1 = c1 for x1
           if( n>p ) then
              call stdlib_wtrtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n-p, 1_ilp,a, lda, c, n-p, &
                        info )
              if( info>0_ilp ) then
                 info = 2_ilp
                 return
              ...
              ! put the solutions in x
              call stdlib_wcopy( n-p, c, 1_ilp, x, 1_ilp )
           end if
           ! compute the residual vector:
           if( m<n ) then
              nr = m + p - n
              if( nr>0_ilp )call stdlib_wgemv( 'NO TRANSPOSE', nr, n-m, -cone, a( n-p+1, m+1 ),lda, d(&
                         nr+1 ), 1_ilp, cone, c( n-p+1 ), 1_ilp )
           else
              nr = p
           end if
           if( nr>0_ilp ) then
              call stdlib_wtrmv( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', nr,a( n-p+1, n-p+1 ), lda, &
                        d, 1_ilp )
              call stdlib_waxpy( nr, -cone, d, 1_ilp, c( n-p+1 ), 1_ilp )
           end if
           ! backward transformation x = q**h*x
           call stdlib_wunmrq( 'LEFT', 'CONJUGATE TRANSPOSE', n, 1_ilp, p, b, ldb,work( 1_ilp ), x, n, &
                     work( p+mn+1 ), lwork-p-mn, info )
           work( 1_ilp ) = p + mn + max( lopt, int( work( p+mn+1 ),KIND=ilp) )
           return
     ...
     !! where inv(B) denotes the inverse of B.
        ...
        ! ===================================================================
           ...
           ! where r11 and t22 are upper triangular, and q and z are
           ! orthogonal.
           call stdlib_qggqrf( n, m, p, a, lda, work, b, ldb, work( m+1 ),work( m+np+1 ), lwork-m-&
                     np, info )
           lopt = work( m+np+1 )
           ! update left-hand-side vector d = q**t*d = ( d1 ) m
                                                     ! ( d2 ) n-m
           call stdlib_qormqr( 'LEFT', 'TRANSPOSE', n, 1_ilp, m, a, lda, work, d,max( 1_ilp, n ), work( m+&
                     np+1 ), lwork-m-np, info )
           lopt = max( lopt, int( work( m+np+1 ),KIND=ilp) )
           ! solve t22*y2 = d2 for y2
           if( n>m ) then
              call stdlib_qtrtrs( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', n-m, 1_ilp,b( m+1, m+p-n+1 ), &
                        ldb, d( m+1 ), n-m, info )
              if( info>0_ilp ) then
                 info = 1_ilp
                 return
              end if
              call stdlib_qcopy( n-m, d( m+1 ), 1_ilp, y( m+p-n+1 ), 1_ilp )
           end if
           ! set y1 = 0
           do i = 1, m + p - n
              y( i ) = zero
           ...
           ! update d1 = d1 - t12*y2
           call stdlib_qgemv( 'NO TRANSPOSE', m, n-m, -one, b( 1_ilp, m+p-n+1 ), ldb,y( m+p-n+1 ), 1_ilp, &
                     one, d, 1_ilp )
           ! solve triangular system: r11*x = d1
           if( m>0_ilp ) then
              call stdlib_qtrtrs( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', m, 1_ilp, a, lda,d, m, info )
              ...
              ! copy d to x
              call stdlib_qcopy( m, d, 1_ilp, x, 1_ilp )
           end if
           ! backward transformation y = z**t *y
           call stdlib_qormrq( 'LEFT', 'TRANSPOSE', p, 1_ilp, np,b( max( 1_ilp, n-p+1 ), 1_ilp ), ldb, work( &
                     m+1 ), y,max( 1_ilp, p ), work( m+np+1 ), lwork-m-np, info )
           work( 1_ilp ) = m + np + max( lopt, int( work( m+np+1 ),KIND=ilp) )
           return
     ...
     !! where inv(B) denotes the inverse of B.
        ...
        ! ===================================================================
           ...
           ! where r11 and t22 are upper triangular, and q and z are
           ! unitary.
           call stdlib_wggqrf( n, m, p, a, lda, work, b, ldb, work( m+1 ),work( m+np+1 ), lwork-m-&
                     np, info )
           lopt = real( work( m+np+1 ),KIND=qp)
           ! update left-hand-side vector d = q**h*d = ( d1 ) m
                                                     ! ( d2 ) n-m
           call stdlib_wunmqr( 'LEFT', 'CONJUGATE TRANSPOSE', n, 1_ilp, m, a, lda, work,d, max( 1_ilp, n )&
                     , work( m+np+1 ), lwork-m-np, info )
           lopt = max( lopt, int( work( m+np+1 ),KIND=ilp) )
           ! solve t22*y2 = d2 for y2
           if( n>m ) then
              call stdlib_wtrtrs( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', n-m, 1_ilp,b( m+1, m+p-n+1 ), &
                        ldb, d( m+1 ), n-m, info )
              if( info>0_ilp ) then
                 info = 1_ilp
                 return
              end if
              call stdlib_wcopy( n-m, d( m+1 ), 1_ilp, y( m+p-n+1 ), 1_ilp )
           end if
           ! set y1 = 0
           do i = 1, m + p - n
              y( i ) = czero
           ...
           ! update d1 = d1 - t12*y2
           call stdlib_wgemv( 'NO TRANSPOSE', m, n-m, -cone, b( 1_ilp, m+p-n+1 ), ldb,y( m+p-n+1 ), 1_ilp,&
                      cone, d, 1_ilp )
           ! solve triangular system: r11*x = d1
           if( m>0_ilp ) then
              call stdlib_wtrtrs( 'UPPER', 'NO TRANSPOSE', 'NON UNIT', m, 1_ilp, a, lda,d, m, info )
              ...
              ! copy d to x
              call stdlib_wcopy( m, d, 1_ilp, x, 1_ilp )
           end if
           ! backward transformation y = z**h *y
           call stdlib_wunmrq( 'LEFT', 'CONJUGATE TRANSPOSE', p, 1_ilp, np,b( max( 1_ilp, n-p+1 ), 1_ilp ), &
                     ldb, work( m+1 ), y,max( 1_ilp, p ), work( m+np+1 ), lwork-m-np, info )
           work( 1_ilp ) = m + np + max( lopt, int( work( m+np+1 ),KIND=ilp) )
           return
