  ...
  contains
     ...
     !! real symmetric matrix A.
        ...
        ! =====================================================================
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_qlansy( 'M', uplo, n, a, lda, work )
           iscale = 0_ilp
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp )call stdlib_qlascl( uplo, 0_ilp, 0_ilp, one, sigma, n, n, a, lda, info )
           ! call stdlib_qsytrd to reduce symmetric matrix to tridiagonal form.
           inde = 1_ilp
           indtau = inde + n
           indwrk = indtau + n
           llwork = lwork - indwrk + 1_ilp
           call stdlib_qsytrd( uplo, n, a, lda, w, work( inde ), work( indtau ),work( indwrk ), &
                     llwork, iinfo )
           ! for eigenvalues only, call stdlib_qsterf.  for eigenvectors, first call
           ! stdlib_qorgtr to generate the orthogonal matrix, then call stdlib_qsteqr.
           if( .not.wantz ) then
     ...
     !! workspace than DSYEVX.
        ...
        ! =====================================================================
           ...
           call stdlib_qsytrd( uplo, n, a, lda, w, work( inde ), work( indtau ),work( indwrk ), &
                     llwork, iinfo )
           ! for eigenvalues only, call stdlib_qsterf.  for eigenvectors, first call
           ! stdlib_qstedc to generate the eigenvector matrix, work(indwrk), of the
           ! tridiagonal matrix, then call stdlib_qormtr to multiply it by the
           ! householder transformations stored in a.
           if( .not.wantz ) then
              call stdlib_qsterf( n, w, work( inde ), info )
           else
              call stdlib_qstedc( 'I', n, w, work( inde ), work( indwrk ), n,work( indwk2 ), &
                        llwrk2, iwork, liwork, info )
              call stdlib_qormtr( 'L', uplo, 'N', n, n, a, lda, work( indtau ),work( indwrk ), n, &
                        work( indwk2 ), llwrk2, iinfo )
              call stdlib_qlacpy( 'A', n, n, work( indwrk ), n, a, lda )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp )call stdlib_qscal( n, one / sigma, w, 1_ilp )
           work( 1_ilp ) = lopt
           iwork( 1_ilp ) = liopt
           return
     ...
     !! manner.
       ...
       ! =====================================================================
           ...
           if( iscale==1_ilp ) then
              if( lower ) then
                 do j = 1, n
                    call stdlib_qscal( n-j+1, sigma, a( j, j ), 1_ilp )
                 end do
              else
                 do j = 1, n
                    call stdlib_qscal( j, sigma, a( 1_ilp, j ), 1_ilp )
                 end do
              end if
              if( abstol>0_ilp )abstll = abstol*sigma
              if( valeig ) then
           ...
           ! initialize indices into workspaces.  note: the iwork indices are
           ! used only if stdlib_qsterf or stdlib_qstemr fail.
           ! work(indtau:indtau+n-1) stores the scalar factors of the
           ! elementary reflectors used in stdlib_qsytrd.
           indtau = 1_ilp
           ! work(indd:indd+n-1) stores the tridiagonal's diagonal entries.
           indd = indtau + n
           ! work(inde:inde+n-1) stores the off-diagonal entries of the
           ! tridiagonal matrix from stdlib_qsytrd.
           inde = indd + n
           ! work(inddd:inddd+n-1) is a copy of the diagonal entries over
           ! -written by stdlib_qstemr (the stdlib_qsterf path copies the diagonal to w).
           inddd = inde + n
           ! work(indee:indee+n-1) is a copy of the off-diagonal entries over
           ! -written while computing the eigenvalues in stdlib_qsterf and stdlib_qstemr.
           indee = inddd + n
           ! indwk is the starting offset of the left-over workspace, and
           ! llwork is the remaining workspace size.
           indwk = indee + n
           ...
           call stdlib_qsytrd( uplo, n, a, lda, work( indd ), work( inde ),work( indtau ), work( &
                     indwk ), llwork, iinfo )
           ! if all eigenvalues are desired
           ! then call stdlib_qsterf or stdlib_qstemr and stdlib_qormtr.
           if( ( alleig .or. ( indeig .and. il==1_ilp .and. iu==n ) ) .and.ieeeok==1_ilp ) then
              if( .not.wantz ) then
                 ...
                 call stdlib_qcopy( n-1, work( inde ), 1_ilp, work( indee ), 1_ilp )
                 call stdlib_qcopy( n, work( indd ), 1_ilp, work( inddd ), 1_ilp )
                 if (abstol <= two*n*eps) then
                    tryrac = .true.
                 else
                    tryrac = .false.
                 end if
                 call stdlib_qstemr( jobz, 'A', n, work( inddd ), work( indee ),vl, vu, il, iu, m,&
                            w, z, ldz, n, isuppz,tryrac, work( indwk ), lwork, iwork, liwork,info )
              ! apply orthogonal matrix used in reduction to tridiagonal
              ! form to eigenvectors returned by stdlib_qstemr.
                 if( wantz .and. info==0_ilp ) then
                    indwkn = inde
                    llwrkn = lwork - indwkn + 1_ilp
                    call stdlib_qormtr( 'L', uplo, 'N', n, m, a, lda,work( indtau ), z, ldz, work(&
                               indwkn ),llwrkn, iinfo )
                 end if
              end if
              if( info==0_ilp ) then
                 ! everything worked.  skip stdlib_qstebz/stdlib_qstein.  iwork(:) are
                 ! undefined.
                 m = n
                 go to 30
              end if
           ...
           ! also call stdlib_qstebz and stdlib_qstein if stdlib_qstemr fails.
           if( wantz ) then
              order = 'B'
           else
              order = 'E'
        ...
        30 continue
           ...
           ! if eigenvalues are not in order, then sort them, along with
           ! eigenvectors.  note: we do not sort the ifail portion of iwork.
           ! it may not be initialized (if stdlib_qstemr/stdlib_qstein succeeded), and we do
           ! not return this detailed information to the user.
           if( wantz ) then
              do j = 1, m - 1
                 i = 0_ilp
       ...
       ! =====================================================================
           ...
           if( ( alleig .or. test ) .and. ( abstol<=zero ) ) then
              ...
              if( .not.wantz ) then
                 ...
                 call stdlib_qlacpy( 'A', n, n, a, lda, z, ldz )
                 call stdlib_qorgtr( uplo, n, z, ldz, work( indtau ),work( indwrk ), llwork, &
                           iinfo )
                 call stdlib_qcopy( n-1, work( inde ), 1_ilp, work( indee ), 1_ilp )
     ...
     !! real symmetric matrix A in packed storage.
        ...
        ! =====================================================================
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_qlansp( 'M', uplo, n, ap, work )
           iscale = 0_ilp
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp ) then
              call stdlib_qscal( ( n*( n+1 ) ) / 2_ilp, sigma, ap, 1_ilp )
           end if
           ! call stdlib_qsptrd to reduce symmetric packed matrix to tridiagonal form.
           inde = 1_ilp
           indtau = inde + n
           call stdlib_qsptrd( uplo, n, ap, w, work( inde ), work( indtau ), iinfo )
           ! for eigenvalues only, call stdlib_qsterf.  for eigenvectors, first call
           ! stdlib_qopgtr to generate the orthogonal matrix, then call stdlib_qsteqr.
           if( .not.wantz ) then
              call stdlib_qsterf( n, w, work( inde ), info )
           else
              indwrk = indtau + n
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! for eigenvalues only, call stdlib_qsterf.  for eigenvectors, first call
           ! stdlib_qstedc to generate the eigenvector matrix, work(indwrk), of the
           ! tridiagonal matrix, then call stdlib_qopmtr to multiply it by the
           ! householder transformations represented in ap.
           if( .not.wantz ) then
              call stdlib_qsterf( n, w, work( inde ), info )
           else
              indwrk = indtau + n
              llwork = lwork - indwrk + 1_ilp
              call stdlib_qstedc( 'I', n, w, work( inde ), z, ldz, work( indwrk ),llwork, iwork, &
                        liwork, info )
              call stdlib_qopmtr( 'L', uplo, 'N', n, n, ap, work( indtau ), z, ldz,work( indwrk ),&
                         iinfo )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp )call stdlib_qscal( n, one / sigma, w, 1_ilp )
     ...
     !! indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           if( wantz ) then
              ...
              ! apply orthogonal matrix used in reduction to tridiagonal
              ! form to eigenvectors returned by stdlib_qstein.
              call stdlib_qopmtr( 'L', uplo, 'N', n, m, ap, work( indtau ), z, ldz,work( indwrk ),&
                         iinfo )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           20 continue
     ...
     !! a real symmetric band matrix A.
        ...
        ! =====================================================================
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_qlansb( 'M', uplo, n, kd, ab, ldab, work )
           iscale = 0_ilp
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp ) then
              if( lower ) then
                 call stdlib_qlascl( 'B', kd, kd, one, sigma, n, n, ab, ldab, info )
              else
                 call stdlib_qlascl( 'Q', kd, kd, one, sigma, n, n, ab, ldab, info )
              end if
           end if
           ! call stdlib_qsbtrd to reduce symmetric band matrix to tridiagonal form.
           inde = 1_ilp
           indwrk = inde + n
     ...
     !! complex Hermitian matrix A.
        ...
        ! =====================================================================
           ...
           if( n==1_ilp ) then
              w( 1_ilp ) = real( a( 1_ilp, 1_ilp ),KIND=qp)
              work( 1_ilp ) = 1_ilp
              if( wantz )a( 1_ilp, 1_ilp ) = cone
              return
           end if
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_wlanhe( 'M', uplo, n, a, lda, rwork )
           iscale = 0_ilp
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp )call stdlib_wlascl( uplo, 0_ilp, 0_ilp, one, sigma, n, n, a, lda, info )
           ! call stdlib_whetrd to reduce hermitian matrix to tridiagonal form.
           inde = 1_ilp
           indtau = 1_ilp
           indwrk = indtau + n
           llwork = lwork - indwrk + 1_ilp
           call stdlib_whetrd( uplo, n, a, lda, w, rwork( inde ), work( indtau ),work( indwrk ), &
                     llwork, iinfo )
           ! for eigenvalues only, call stdlib_qsterf.  for eigenvectors, first call
           ! stdlib_wungtr to generate the unitary matrix, then call stdlib_wsteqr.
           if( .not.wantz ) then
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           call stdlib_whetrd( uplo, n, a, lda, w, rwork( inde ), work( indtau ),work( indwrk ), &
                     llwork, iinfo )
           ! for eigenvalues only, call stdlib_qsterf.  for eigenvectors, first call
           ! stdlib_wstedc to generate the eigenvector matrix, work(indwrk), of the
           ! tridiagonal matrix, then call stdlib_wunmtr to multiply it to the
           ! householder transformations represented as householder vectors in
           ! a.
           if( .not.wantz ) then
              call stdlib_qsterf( n, w, rwork( inde ), info )
           else
              call stdlib_wstedc( 'I', n, w, rwork( inde ), work( indwrk ), n,work( indwk2 ), &
                        llwrk2, rwork( indrwk ), llrwk,iwork, liwork, info )
              call stdlib_wunmtr( 'L', uplo, 'N', n, n, a, lda, work( indtau ),work( indwrk ), n, &
                        work( indwk2 ), llwrk2, iinfo )
              call stdlib_wlacpy( 'A', n, n, work( indwrk ), n, a, lda )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp ) then
              if( info==0_ilp ) then
        ...
        ! =====================================================================
           ...
           if( n==1_ilp ) then
              ...
              if( alleig .or. indeig ) then
                 ...
                 if( vl<real( a( 1_ilp, 1_ilp ),KIND=qp) .and. vu>=real( a( 1_ilp, 1_ilp ),KIND=qp) )then
                    m = 1_ilp
                    w( 1_ilp ) = real( a( 1_ilp, 1_ilp ),KIND=qp)
                 end if
              end if
           ...
           anrm = stdlib_wlansy( 'M', uplo, n, a, lda, rwork )
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp
              sigma = rmin / anrm
           else if( anrm>rmax ) then
           ...
           if( iscale==1_ilp ) then
              if( lower ) then
                 do j = 1, n
                    call stdlib_wdscal( n-j+1, sigma, a( j, j ), 1_ilp )
                 end do
              else
                 do j = 1, n
                    call stdlib_wdscal( j, sigma, a( 1_ilp, j ), 1_ilp )
                 end do
              end if
              if( abstol>0_ilp )abstll = abstol*sigma
              if( valeig ) then
           ...
           ! initialize indices into workspaces.  note: the iwork indices are
           ! used only if stdlib_qsterf or stdlib_wstemr fail.
           ! work(indtau:indtau+n-1) stores the complex scalar factors of the
           ! elementary reflectors used in stdlib_whetrd.
           indtau = 1_ilp
           ! indwk is the starting offset of the remaining complex workspace,
           ! and llwork is the remaining complex workspace size.
           indwk = indtau + n
           ...
           indrd = 1_ilp
           ! rwork(indre:indre+n-1) stores the off-diagonal entries of the
           ! tridiagonal matrix from stdlib_whetrd.
           indre = indrd + n
           ! rwork(indrdd:indrdd+n-1) is a copy of the diagonal entries over
           ! -written by stdlib_wstemr (the stdlib_qsterf path copies the diagonal to w).
           indrdd = indre + n
           ! rwork(indree:indree+n-1) is a copy of the off-diagonal entries over
           ! -written while computing the eigenvalues in stdlib_qsterf and stdlib_wstemr.
           indree = indrdd + n
           ! indrwk is the starting offset of the left-over real workspace, and
           ! llrwork is the remaining workspace size.
           indrwk = indree + n
           ...
           indiwo = indifl + n
           ! call stdlib_whetrd to reduce hermitian matrix to tridiagonal form.
           call stdlib_whetrd( uplo, n, a, lda, rwork( indrd ), rwork( indre ),work( indtau ), &
                     work( indwk ), llwork, iinfo )
           ! if all eigenvalues are desired
           ! then call stdlib_qsterf or stdlib_wstemr and stdlib_wunmtr.
           test = .false.
           if( indeig ) then
              if( il==1_ilp .and. iu==n ) then
                 test = .true.
           ...
           if( ( alleig.or.test ) .and. ( ieeeok==1_ilp ) ) then
              if( .not.wantz ) then
                 call stdlib_qcopy( n, rwork( indrd ), 1_ilp, w, 1_ilp )
                 call stdlib_qcopy( n-1, rwork( indre ), 1_ilp, rwork( indree ), 1_ilp )
                 call stdlib_qsterf( n, w, rwork( indree ), info )
              else
                 call stdlib_qcopy( n-1, rwork( indre ), 1_ilp, rwork( indree ), 1_ilp )
                 call stdlib_qcopy( n, rwork( indrd ), 1_ilp, rwork( indrdd ), 1_ilp )
                 if (abstol <= two*n*eps) then
                    tryrac = .true.
                 else
                    tryrac = .false.
                 ...
                 if( wantz .and. info==0_ilp ) then
                    indwkn = indwk
                    llwrkn = lwork - indwkn + 1_ilp
                    call stdlib_wunmtr( 'L', uplo, 'N', n, m, a, lda,work( indtau ), z, ldz, work(&
                               indwkn ),llwrkn, iinfo )
                 end if
              end if
              if( info==0_ilp ) then
           ...
           ! otherwise, call stdlib_qstebz and, if eigenvectors are desired, stdlib_wstein.
           ! also call stdlib_qstebz and stdlib_wstein if stdlib_wstemr fails.
           if( wantz ) then
              order = 'B'
           else
              order = 'E'
           end if
           call stdlib_qstebz( range, order, n, vll, vuu, il, iu, abstll,rwork( indrd ), rwork( &
           indre ), m, nsplit, w,iwork( indibl ), iwork( indisp ), rwork( indrwk ),iwork( indiwo )&
                     , info )
           if( wantz ) then
              call stdlib_wstein( n, rwork( indrd ), rwork( indre ), m, w,iwork( indibl ), iwork( &
                        indisp ), z, ldz,rwork( indrwk ), iwork( indiwo ), iwork( indifl ),info )
              ! apply unitary matrix used in reduction to tridiagonal
     ...
     !! indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           indwrk = indtau + n
           llwork = lwork - indwrk + 1_ilp
           call stdlib_whetrd( uplo, n, a, lda, rwork( indd ), rwork( inde ),work( indtau ), work(&
                      indwrk ), llwork, iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal to
           ! zero, then call stdlib_qsterf or stdlib_wungtr and stdlib_wsteqr.  if this fails for
           ! some eigenvalue, then try stdlib_qstebz.
           test = .false.
           if( indeig ) then
              if( il==1_ilp .and. iu==n ) then
           ...
           if( ( alleig .or. test ) .and. ( abstol<=zero ) ) then
              ...
              if( .not.wantz ) then
                 ...
                 call stdlib_wlacpy( 'A', n, n, a, lda, z, ldz )
                 call stdlib_wungtr( uplo, n, z, ldz, work( indtau ),work( indwrk ), llwork, &
                           iinfo )
                 call stdlib_qcopy( n-1, rwork( inde ), 1_ilp, rwork( indee ), 1_ilp )
           ...
           indisp = indibl + n
           indiwk = indisp + n
           call stdlib_qstebz( range, order, n, vll, vuu, il, iu, abstll,rwork( indd ), rwork( &
           inde ), m, nsplit, w,iwork( indibl ), iwork( indisp ), rwork( indrwk ),iwork( indiwk ),&
                      info )
           if( wantz ) then
     ...
     !! complex Hermitian matrix in packed storage.
        ...
        ! =====================================================================
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_wlanhp( 'M', uplo, n, ap, rwork )
           iscale = 0_ilp
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp ) then
              call stdlib_wdscal( ( n*( n+1 ) ) / 2_ilp, sigma, ap, 1_ilp )
           end if
           ! call stdlib_whptrd to reduce hermitian packed matrix to tridiagonal form.
           inde = 1_ilp
           indtau = 1_ilp
           call stdlib_whptrd( uplo, n, ap, w, rwork( inde ), work( indtau ),iinfo )
           ! for eigenvalues only, call stdlib_qsterf.  for eigenvectors, first call
           ! stdlib_wupgtr to generate the orthogonal matrix, then call stdlib_wsteqr.
           if( .not.wantz ) then
              call stdlib_qsterf( n, w, rwork( inde ), info )
           else
              indwrk = indtau + n
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           call stdlib_whptrd( uplo, n, ap, w, rwork( inde ), work( indtau ),iinfo )
           ! for eigenvalues only, call stdlib_qsterf.  for eigenvectors, first call
           ! stdlib_wupgtr to generate the orthogonal matrix, then call stdlib_wstedc.
           if( .not.wantz ) then
              call stdlib_qsterf( n, w, rwork( inde ), info )
           else
              call stdlib_wstedc( 'I', n, w, rwork( inde ), z, ldz, work( indwrk ),llwrk, rwork( &
                        indrwk ), llrwk, iwork, liwork,info )
              call stdlib_wupmtr( 'L', uplo, 'N', n, n, ap, work( indtau ), z, ldz,work( indwrk ),&
                         iinfo )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp ) then
        ...
        ! =====================================================================
           ...
           if( n==1_ilp ) then
              if( alleig .or. indeig ) then
                 ...
                 if( vl<real( ap( 1_ilp ),KIND=qp) .and. vu>=real( ap( 1_ilp ),KIND=qp) ) then
                    m = 1_ilp
                    w( 1_ilp ) = real( ap( 1_ilp ),KIND=qp)
                 end if
              end if
           ...
           if( wantz ) then
              ...
              ! form to eigenvectors returned by stdlib_wstein.
              indwrk = indtau + n
              call stdlib_wupmtr( 'L', uplo, 'N', n, m, ap, work( indtau ), z, ldz,work( indwrk ),&
                         iinfo )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           20 continue
     ...
     !! a complex Hermitian band matrix A.
        ...
        ! =====================================================================
           ...
           if( n==1_ilp ) then
              if( lower ) then
                 w( 1_ilp ) = real( ab( 1_ilp, 1_ilp ),KIND=qp)
              else
                 w( 1_ilp ) = real( ab( kd+1, 1_ilp ),KIND=qp)
              end if
              if( wantz )z( 1_ilp, 1_ilp ) = one
              return
           end if
           ...
           rmax = sqrt( bignum )
           ! scale matrix to allowable range, if necessary.
           anrm = stdlib_wlanhb( 'M', uplo, n, kd, ab, ldab, rwork )
           iscale = 0_ilp
           if( anrm>zero .and. anrm<rmin ) then
              iscale = 1_ilp
              sigma = rmin / anrm
           ...
           if( iscale==1_ilp ) then
              if( lower ) then
                 call stdlib_wlascl( 'B', kd, kd, one, sigma, n, n, ab, ldab, info )
              else
                 call stdlib_wlascl( 'Q', kd, kd, one, sigma, n, n, ab, ldab, info )
              end if
           end if
     ...
     !! indices for the desired eigenvalues.
        ...
        ! =====================================================================
           ...
           indrwk = inde + n
           indwrk = 1_ilp
           call stdlib_whbtrd( jobz, uplo, n, kd, ab, ldab, rwork( indd ),rwork( inde ), q, ldq, &
                     work( indwrk ), iinfo )
           ! if all eigenvalues are desired and abstol is less than or equal
