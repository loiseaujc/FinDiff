  ...
  contains
     ...
     !! RCOND = 1 / ( norm(A) * norm(inv(A)) ).
        ...
        ! =====================================================================
           ...
           rcond = zero
           smlnum = stdlib_qlamch( 'SAFE MINIMUM' )*real( max( 1_ilp, n ),KIND=qp)
           ! compute the norm of the triangular matrix a.
           anorm = stdlib_qlantr( norm, uplo, diag, n, n, a, lda, work )
           ! continue only if anorm > 0.
           if( anorm>zero ) then
              ! estimate the norm of the inverse of a.
              ainvnm = zero
              ...
              if( kase/=0_ilp ) then
                 if( kase==kase1 ) then
                    ! multiply by inv(a).
                    call stdlib_qlatrs( uplo, 'NO TRANSPOSE', diag, normin, n, a,lda, work, scale,&
                               work( 2_ilp*n+1 ), info )
                 else
                    ! multiply by inv(a**t).
                    call stdlib_qlatrs( uplo, 'TRANSPOSE', diag, normin, n, a, lda,work, scale, &
                              work( 2_ilp*n+1 ), info )
                 end if
                 normin = 'Y'
                 ! multiply by 1/scale if doing so will not cause overflow.
     ...
     !! RCOND = 1 / ( norm(A) * norm(inv(A)) ).
        ...
        ! =====================================================================
           ...
           smlnum = stdlib_qlamch( 'SAFE MINIMUM' )*real( max( 1_ilp, n ),KIND=qp)
           ! compute the norm of the triangular matrix a.
           anorm = stdlib_wlantr( norm, uplo, diag, n, n, a, lda, rwork )
           ! continue only if anorm > 0.
           if( anorm>zero ) then
              ! estimate the norm of the inverse of a.
              ainvnm = zero
              ...
              if( kase/=0_ilp ) then
                 if( kase==kase1 ) then
                    ! multiply by inv(a).
                    call stdlib_wlatrs( uplo, 'NO TRANSPOSE', diag, normin, n, a,lda, work, scale,&
                               rwork, info )
                 else
                    ! multiply by inv(a**h).
                    call stdlib_wlatrs( uplo, 'CONJUGATE TRANSPOSE', diag, normin,n, a, lda, work,&
                               scale, rwork, info )
                 end if
                 normin = 'Y'
                 ! multiply by 1/scale if doing so will not cause overflow.
     ...
     !! non-trivial solution to A*x = 0 is returned.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           ! determine machine dependent parameters to control overflow.
           smlnum = stdlib_qlamch( 'SAFE MINIMUM' ) / stdlib_qlamch( 'PRECISION' )
           bignum = one / smlnum
           scale = one
           if( stdlib_lsame( normin, 'N' ) ) then
              ! compute the 1-norm of each column, not including the diagonal.
              if( upper ) then
                 ! a is upper triangular.
                 do j = 1, n
                    cnorm( j ) = stdlib_qasum( j-1, a( 1_ilp, j ), 1_ilp )
                 end do
              else
                 ! a is lower triangular.
                 do j = 1, n - 1
                    cnorm( j ) = stdlib_qasum( n-j, a( j+1, j ), 1_ilp )
                 end do
                 cnorm( n ) = zero
              end if
           end if
           ! scale the column norms by tscal if the maximum element in cnorm is
           ! greater than bignum.
           imax = stdlib_iqamax( n, cnorm, 1_ilp )
           tmax = cnorm( imax )
           if( tmax<=bignum ) then
              tscal = one
           else
              tscal = one / ( smlnum*tmax )
              call stdlib_qscal( n, tscal, cnorm, 1_ilp )
           end if
           ! compute a bound on the computed solution vector to see if the
           ! level 2 blas routine stdlib_qtrsv can be used.
           j = stdlib_iqamax( n, x, 1_ilp )
           xmax = abs( x( j ) )
           xbnd = xmax
           if( notran ) then
              ! compute the growth in a * x = b.
           ...
           if( ( grow*tscal )>smlnum ) then
              ! use the level 2 blas solve if the reciprocal of the bound on
              ! elements of x is not too small.
              call stdlib_qtrsv( uplo, trans, diag, n, a, lda, x, 1_ilp )
           else
              ! use a level 1 blas solve, scaling intermediate results.
              if( xmax>bignum ) then
                 ! scale x so that its components are less than or equal to
                 ! bignum in absolute value.
                 scale = bignum / xmax
                 call stdlib_qscal( n, scale, x, 1_ilp )
                 xmax = bignum
              end if
              if( notran ) then
                 ! solve a * x = b
                 loop_110: do j = jfirst, jlast, jinc
                    ...
                    if( xj>one ) then
                       ...
                       ! scale x by 1/2.
                       call stdlib_qscal( n, half, x, 1_ilp )
                       scale = scale*half
                    end if
                    if( upper ) then
                       if( j>1_ilp ) then
                          ! compute the update
                             ! x(1:j-1) := x(1:j-1) - x(j) * a(1:j-1,j)
                          call stdlib_qaxpy( j-1, -x( j )*tscal, a( 1_ilp, j ), 1_ilp, x,1_ilp )
                          i = stdlib_iqamax( j-1, x, 1_ilp )
                          xmax = abs( x( i ) )
                       end if
                    else
                       if( j<n ) then
                          ! compute the update
                             ! x(j+1:n) := x(j+1:n) - x(j) * a(j+1:n,j)
                          call stdlib_qaxpy( n-j, -x( j )*tscal, a( j+1, j ), 1_ilp,x( j+1 ), 1_ilp )
                                    
                          i = j + stdlib_iqamax( n-j, x( j+1 ), 1_ilp )
                          xmax = abs( x( i ) )
                       end if
                    end if
                 end do loop_110
                 ...
                 loop_160: do j = jfirst, jlast, jinc
                    ...
                    if( uscal==one ) then
                       ! if the scaling needed for a in the dot product is 1,
                       ! call stdlib_qdot to perform the dot product.
                       if( upper ) then
                          sumj = stdlib_qdot( j-1, a( 1_ilp, j ), 1_ilp, x, 1_ilp )
                       else if( j<n ) then
                          sumj = stdlib_qdot( n-j, a( j+1, j ), 1_ilp, x( j+1 ), 1_ilp )
                       end if
                    else
                       ! otherwise, use in-line code for the dot product.
                       if( upper ) then
           ...
           ! scale the column norms by 1/tscal for return.
           if( tscal/=one ) then
              call stdlib_qscal( n, one / tscal, cnorm, 1_ilp )
           end if
           return
     ...
     !! then s is set to 0 and a non-trivial solution to A*x = 0 is returned.
        ...
        ! =====================================================================
           ...
           smlnum = smlnum / stdlib_qlamch( 'PRECISION' )
           bignum = one / smlnum
           scale = one
           if( stdlib_lsame( normin, 'N' ) ) then
              ! compute the 1-norm of each column, not including the diagonal.
              if( upper ) then
                 ! a is upper triangular.
                 do j = 1, n
                    cnorm( j ) = stdlib_qzasum( j-1, a( 1_ilp, j ), 1_ilp )
                 end do
              else
                 ! a is lower triangular.
                 do j = 1, n - 1
                    cnorm( j ) = stdlib_qzasum( n-j, a( j+1, j ), 1_ilp )
                 end do
                 cnorm( n ) = zero
              end if
           end if
           ...
           ! compute a bound on the computed solution vector to see if the
           ! level 2 blas routine stdlib_wtrsv can be used.
           xmax = zero
           do j = 1, n
              xmax = max( xmax, cabs2( x( j ) ) )
           end do
           ...
           if( ( grow*tscal )>smlnum ) then
              ! use the level 2 blas solve if the reciprocal of the bound on
              ! elements of x is not too small.
              call stdlib_wtrsv( uplo, trans, diag, n, a, lda, x, 1_ilp )
           else
              ! use a level 1 blas solve, scaling intermediate results.
              if( xmax>bignum*half ) then
                 ! scale x so that its components are less than or equal to
                 ! bignum in absolute value.
                 scale = ( bignum*half ) / xmax
                 call stdlib_wdscal( n, scale, x, 1_ilp )
                 xmax = bignum
              else
                 xmax = xmax*two
              end if
              if( notran ) then
                 ...
                 loop_120: do j = jfirst, jlast, jinc
                    ...
                    if( tjj>smlnum ) then
                       ...
                       if( tjj<one ) then
                          if( xj>tjj*bignum ) then
                                ! scale x by 1/b(j).
                             rec = one / xj
                             call stdlib_wdscal( n, rec, x, 1_ilp )
                             scale = scale*rec
                             xmax = xmax*rec
                          end if
                       end if
                       x( j ) = stdlib_wladiv( x( j ), tjjs )
                       xj = cabs1( x( j ) )
                    else if( tjj>zero ) then
                          ! 0 < abs(a(j,j)) <= smlnum:
                       if( xj>tjj*bignum ) then
                    ...
                    if( xj>one ) then
                       ...
                       ! scale x by 1/2.
                       call stdlib_wdscal( n, half, x, 1_ilp )
                       scale = scale*half
                    end if
                    if( upper ) then
                       if( j>1_ilp ) then
                          ! compute the update
                             ! x(1:j-1) := x(1:j-1) - x(j) * a(1:j-1,j)
                          call stdlib_waxpy( j-1, -x( j )*tscal, a( 1_ilp, j ), 1_ilp, x,1_ilp )
                          i = stdlib_iwamax( j-1, x, 1_ilp )
                          xmax = cabs1( x( i ) )
                       end if
                    else
                       if( j<n ) then
                          ! compute the update
                             ! x(j+1:n) := x(j+1:n) - x(j) * a(j+1:n,j)
                          call stdlib_waxpy( n-j, -x( j )*tscal, a( j+1, j ), 1_ilp,x( j+1 ), 1_ilp )
                                    
                          i = j + stdlib_iwamax( n-j, x( j+1 ), 1_ilp )
                          xmax = cabs1( x( i ) )
                       end if
                    end if
                 end do loop_120
                 ...
                 loop_170: do j = jfirst, jlast, jinc
                    ...
                    if( cnorm( j )>( bignum-xj )*rec ) then
                       ...
                       if( tjj>one ) then
                             ! divide by a(j,j) when scaling x if a(j,j) > 1.
                          rec = min( one, rec*tjj )
                          uscal = stdlib_wladiv( uscal, tjjs )
                       end if
                       if( rec<one ) then
                          call stdlib_wdscal( n, rec, x, 1_ilp )
                          scale = scale*rec
                    ...
                    if( uscal==cmplx( one,KIND=qp) ) then
                       ! if the scaling needed for a in the dot product is 1,
                       ! call stdlib_wdotu to perform the dot product.
                       if( upper ) then
                          csumj = stdlib_wdotu( j-1, a( 1_ilp, j ), 1_ilp, x, 1_ilp )
                       else if( j<n ) then
                          csumj = stdlib_wdotu( n-j, a( j+1, j ), 1_ilp, x( j+1 ), 1_ilp )
                       end if
                    else
                       ! otherwise, use in-line code for the dot product.
                       if( upper ) then
                    ...
                    if( uscal==cmplx( tscal,KIND=qp) ) then
                       ...
                       ! compute x(j) := x(j) / a(j,j) - csumj if the dot
                       ! product has already been divided by 1/a(j,j).
                       x( j ) = stdlib_wladiv( x( j ), tjjs ) - csumj
                    end if
                    xmax = max( xmax, cabs1( x( j ) ) )
                 end do loop_170
              else
                 ...
                 loop_220: do j = jfirst, jlast, jinc
                    ...
                    if( uscal==cmplx( one,KIND=qp) ) then
                       ! if the scaling needed for a in the dot product is 1,
                       ! call stdlib_wdotc to perform the dot product.
                       if( upper ) then
                          csumj = stdlib_wdotc( j-1, a( 1_ilp, j ), 1_ilp, x, 1_ilp )
                       else if( j<n ) then
                          csumj = stdlib_wdotc( n-j, a( j+1, j ), 1_ilp, x( j+1 ), 1_ilp )
                       end if
                    else
                       ! otherwise, use in-line code for the dot product.
                       if( upper ) then
     ...
     !! This is the Level 3 BLAS version of the algorithm.
        ...
        ! =====================================================================
           ...
           if( nb<=1_ilp .or. nb>=n ) then
              ! use unblocked code
              call stdlib_qtrti2( uplo, diag, n, a, lda, info )
           else
              ! use blocked code
              if( upper ) then
                 ! compute inverse of upper triangular matrix
                 do j = 1, n, nb
                    jb = min( nb, n-j+1 )
                    ! compute rows 1:j-1 of current block column
                    call stdlib_qtrmm( 'LEFT', 'UPPER', 'NO TRANSPOSE', diag, j-1,jb, one, a, lda,&
                               a( 1_ilp, j ), lda )
                    call stdlib_qtrsm( 'RIGHT', 'UPPER', 'NO TRANSPOSE', diag, j-1,jb, -one, a( j,&
                               j ), lda, a( 1_ilp, j ), lda )
                    ! compute inverse of current diagonal block
                    call stdlib_qtrti2( 'UPPER', diag, jb, a( j, j ), lda, info )
                 end do
              else
                 ! compute inverse of lower triangular matrix
                 nn = ( ( n-1 ) / nb )*nb + 1_ilp
                 do j = nn, 1, -nb
                    ...
                    if( j+jb<=n ) then
                       ! compute rows j+jb:n of current block column
                       call stdlib_qtrmm( 'LEFT', 'LOWER', 'NO TRANSPOSE', diag,n-j-jb+1, jb, one,&
                                  a( j+jb, j+jb ), lda,a( j+jb, j ), lda )
                       call stdlib_qtrsm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', diag,n-j-jb+1, jb, -&
                                 one, a( j, j ), lda,a( j+jb, j ), lda )
                    end if
                    ! compute inverse of current diagonal block
                    call stdlib_qtrti2( 'LOWER', diag, jb, a( j, j ), lda, info )
                 end do
              end if
           end if
           return
     ...
     !! This is the Level 3 BLAS version of the algorithm.
        ...
        ! =====================================================================
           ...
           if( nb<=1_ilp .or. nb>=n ) then
              ! use unblocked code
              call stdlib_wtrti2( uplo, diag, n, a, lda, info )
           else
              ! use blocked code
              if( upper ) then
                 ! compute inverse of upper triangular matrix
                 do j = 1, n, nb
                    jb = min( nb, n-j+1 )
                    ! compute rows 1:j-1 of current block column
                    call stdlib_wtrmm( 'LEFT', 'UPPER', 'NO TRANSPOSE', diag, j-1,jb, cone, a, &
                              lda, a( 1_ilp, j ), lda )
                    call stdlib_wtrsm( 'RIGHT', 'UPPER', 'NO TRANSPOSE', diag, j-1,jb, -cone, a( &
                              j, j ), lda, a( 1_ilp, j ), lda )
                    ! compute inverse of current diagonal block
                    call stdlib_wtrti2( 'UPPER', diag, jb, a( j, j ), lda, info )
                 end do
              else
                 ! compute inverse of lower triangular matrix
                 nn = ( ( n-1 ) / nb )*nb + 1_ilp
                 do j = nn, 1, -nb
                    ...
                    if( j+jb<=n ) then
                       ! compute rows j+jb:n of current block column
                       call stdlib_wtrmm( 'LEFT', 'LOWER', 'NO TRANSPOSE', diag,n-j-jb+1, jb, &
                                 cone, a( j+jb, j+jb ), lda,a( j+jb, j ), lda )
                       call stdlib_wtrsm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', diag,n-j-jb+1, jb, -&
                                 cone, a( j, j ), lda,a( j+jb, j ), lda )
                    end if
                    ! compute inverse of current diagonal block
                    call stdlib_wtrti2( 'LOWER', diag, jb, a( j, j ), lda, info )
                 end do
              end if
           end if
           return
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do j = 1, n
                 ...
                 ! compute elements 1:j-1 of j-th column.
                 call stdlib_qtrmv( 'UPPER', 'NO TRANSPOSE', diag, j-1, a, lda,a( 1_ilp, j ), 1_ilp )
                           
                 call stdlib_qscal( j-1, ajj, a( 1_ilp, j ), 1_ilp )
              end do
           else
              ! compute inverse of lower triangular matrix.
              do j = n, 1, -1
                 ...
                 if( j<n ) then
                    ! compute elements j+1:n of j-th column.
                    call stdlib_qtrmv( 'LOWER', 'NO TRANSPOSE', diag, n-j,a( j+1, j+1 ), lda, a( &
                              j+1, j ), 1_ilp )
                    call stdlib_qscal( n-j, ajj, a( j+1, j ), 1_ilp )
                 end if
              end do
           end if
           return
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do j = 1, n
                 ...
                 ! compute elements 1:j-1 of j-th column.
                 call stdlib_wtrmv( 'UPPER', 'NO TRANSPOSE', diag, j-1, a, lda,a( 1_ilp, j ), 1_ilp )
                           
                 call stdlib_wscal( j-1, ajj, a( 1_ilp, j ), 1_ilp )
              end do
           else
              ! compute inverse of lower triangular matrix.
              do j = n, 1, -1
                 ...
                 if( j<n ) then
                    ! compute elements j+1:n of j-th column.
                    call stdlib_wtrmv( 'LOWER', 'NO TRANSPOSE', diag, n-j,a( j+1, j+1 ), lda, a( &
                              j+1, j ), 1_ilp )
                    call stdlib_wscal( n-j, ajj, a( j+1, j ), 1_ilp )
                 end if
              end do
           end if
           return
     ...
     !! refinement because doing so cannot improve the backward error.
        ...
        ! =====================================================================
           ...
           loop_250: do j = 1, nrhs
              ! compute residual r = b - op(a) * x,
              ! where op(a) = a or a**t, depending on trans.
              call stdlib_qcopy( n, x( 1_ilp, j ), 1_ilp, work( n+1 ), 1_ilp )
              call stdlib_qtrmv( uplo, trans, diag, n, a, lda, work( n+1 ), 1_ilp )
              call stdlib_qaxpy( n, -one, b( 1_ilp, j ), 1_ilp, work( n+1 ), 1_ilp )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(op(a))*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ! or vector z.  if the i-th component of the denominator is less
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**t).
                    call stdlib_qtrsv( uplo, transt, diag, n, a, lda, work( n+1 ),1_ilp )
                    do i = 1, n
                       work( n+i ) = work( i )*work( n+i )
                    end do
                 else
                    ...
                    call stdlib_qtrsv( uplo, trans, diag, n, a, lda, work( n+1 ),1_ilp )
                 end if
                 go to 210
              end if
              ! normalize error.
     ...
     !! refinement because doing so cannot improve the backward error.
        ...
        ! =====================================================================
           ...
           loop_250: do j = 1, nrhs
              ! compute residual r = b - op(a) * x,
              ! where op(a) = a, a**t, or a**h, depending on trans.
              call stdlib_wcopy( n, x( 1_ilp, j ), 1_ilp, work, 1_ilp )
              call stdlib_wtrmv( uplo, trans, diag, n, a, lda, work, 1_ilp )
              call stdlib_waxpy( n, -cone, b( 1_ilp, j ), 1_ilp, work, 1_ilp )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(op(a))*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ! or vector z.  if the i-th component of the denominator is less
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**h).
                    call stdlib_wtrsv( uplo, transt, diag, n, a, lda, work, 1_ilp )
                    do i = 1, n
                       work( i ) = rwork( i )*work( i )
                    end do
                 else
                    ...
                    call stdlib_wtrsv( uplo, transn, diag, n, a, lda, work, 1_ilp )
                 end if
                 go to 210
              end if
              ! normalize error.
     ...
     !! This is the blocked form of the algorithm, calling Level 3 BLAS.
        ...
        ! =====================================================================
           ...
           if( nb<=1_ilp .or. nb>=n ) then
              ! use unblocked code
              call stdlib_qlauu2( uplo, n, a, lda, info )
           else
              ! use blocked code
              if( upper ) then
                 ! compute the product u * u**t.
                 do i = 1, n, nb
                    ib = min( nb, n-i+1 )
                    call stdlib_qtrmm( 'RIGHT', 'UPPER', 'TRANSPOSE', 'NON-UNIT',i-1, ib, one, a( &
                              i, i ), lda, a( 1_ilp, i ),lda )
                    call stdlib_qlauu2( 'UPPER', ib, a( i, i ), lda, info )
                    if( i+ib<=n ) then
                       call stdlib_qgemm( 'NO TRANSPOSE', 'TRANSPOSE', i-1, ib,n-i-ib+1, one, a( &
                                 1_ilp, i+ib ), lda,a( i, i+ib ), lda, one, a( 1_ilp, i ), lda )
                       call stdlib_qsyrk( 'UPPER', 'NO TRANSPOSE', ib, n-i-ib+1,one, a( i, i+ib ),&
                                  lda, one, a( i, i ),lda )
                    end if
                 end do
              else
                 ...
                 do i = 1, n, nb
                    ib = min( nb, n-i+1 )
                    call stdlib_qtrmm( 'LEFT', 'LOWER', 'TRANSPOSE', 'NON-UNIT', ib,i-1, one, a( &
                              i, i ), lda, a( i, 1_ilp ), lda )
                    call stdlib_qlauu2( 'LOWER', ib, a( i, i ), lda, info )
                    if( i+ib<=n ) then
                       call stdlib_qgemm( 'TRANSPOSE', 'NO TRANSPOSE', ib, i-1,n-i-ib+1, one, a( &
                                 i+ib, i ), lda,a( i+ib, 1_ilp ), lda, one, a( i, 1_ilp ), lda )
                       call stdlib_qsyrk( 'LOWER', 'TRANSPOSE', ib, n-i-ib+1, one,a( i+ib, i ), &
                                 lda, one, a( i, i ), lda )
                    end if
                 end do
              end if
     ...
     !! This is the blocked form of the algorithm, calling Level 3 BLAS.
        ...
        ! =====================================================================
           ...
           if( nb<=1_ilp .or. nb>=n ) then
              ! use unblocked code
              call stdlib_wlauu2( uplo, n, a, lda, info )
           else
              ! use blocked code
              if( upper ) then
                 ! compute the product u * u**h.
                 do i = 1, n, nb
                    ib = min( nb, n-i+1 )
                    call stdlib_wtrmm( 'RIGHT', 'UPPER', 'CONJUGATE TRANSPOSE','NON-UNIT', i-1, &
                              ib, cone, a( i, i ), lda,a( 1_ilp, i ), lda )
                    call stdlib_wlauu2( 'UPPER', ib, a( i, i ), lda, info )
                    if( i+ib<=n ) then
                       call stdlib_wgemm( 'NO TRANSPOSE', 'CONJUGATE TRANSPOSE',i-1, ib, n-i-ib+1,&
                                  cone, a( 1_ilp, i+ib ),lda, a( i, i+ib ), lda, cone, a( 1_ilp, i ),lda )
                       call stdlib_wherk( 'UPPER', 'NO TRANSPOSE', ib, n-i-ib+1,one, a( i, i+ib ),&
                                  lda, one, a( i, i ),lda )
                    end if
                 end do
              else
                 ...
                 do i = 1, n, nb
                    ib = min( nb, n-i+1 )
                    call stdlib_wtrmm( 'LEFT', 'LOWER', 'CONJUGATE TRANSPOSE','NON-UNIT', ib, i-1,&
                               cone, a( i, i ), lda,a( i, 1_ilp ), lda )
                    call stdlib_wlauu2( 'LOWER', ib, a( i, i ), lda, info )
                    if( i+ib<=n ) then
                       call stdlib_wgemm( 'CONJUGATE TRANSPOSE', 'NO TRANSPOSE', ib,i-1, n-i-ib+1,&
                                  cone, a( i+ib, i ), lda,a( i+ib, 1_ilp ), lda, cone, a( i, 1_ilp ), lda )
                       call stdlib_wherk( 'LOWER', 'CONJUGATE TRANSPOSE', ib,n-i-ib+1, one, a( i+&
                                 ib, i ), lda, one,a( i, i ), lda )
                    end if
                 end do
              end if
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do i = 1, n
                 aii = a( i, i )
                 if( i<n ) then
                    a( i, i ) = stdlib_qdot( n-i+1, a( i, i ), lda, a( i, i ), lda )
                    call stdlib_qgemv( 'NO TRANSPOSE', i-1, n-i, one, a( 1_ilp, i+1 ),lda, a( i, i+1 )&
                              , lda, aii, a( 1_ilp, i ), 1_ilp )
                 else
                    call stdlib_qscal( i, aii, a( 1_ilp, i ), 1_ilp )
                 end if
              end do
           else
              ! compute the product l**t * l.
              do i = 1, n
                 aii = a( i, i )
                 if( i<n ) then
                    a( i, i ) = stdlib_qdot( n-i+1, a( i, i ), 1_ilp, a( i, i ), 1_ilp )
                    call stdlib_qgemv( 'TRANSPOSE', n-i, i-1, one, a( i+1, 1_ilp ), lda,a( i+1, i ), &
                              1_ilp, aii, a( i, 1_ilp ), lda )
                 else
                    call stdlib_qscal( i, aii, a( i, 1_ilp ), lda )
                 end if
              end do
           end if
           return
           ...
           if( upper ) then
              ...
              do i = 1, n
                 ...
                 if( i<n ) then
                    ...
                    call stdlib_wgemv( 'NO TRANSPOSE', i-1, n-i, cone, a( 1_ilp, i+1 ),lda, a( i, i+1 &
                              ), lda, cmplx( aii,KIND=qp),a( 1_ilp, i ), 1_ilp )
                    call stdlib_wlacgv( n-i, a( i, i+1 ), lda )
                 else
                    call stdlib_wdscal( i, aii, a( 1_ilp, i ), 1_ilp )
                 end if
              end do
           else
              ! compute the product l**h * l.
              do i = 1, n
                 aii = real( a( i, i ),KIND=qp)
                 if( i<n ) then
                    a( i, i ) = aii*aii + real( stdlib_wdotc( n-i, a( i+1, i ), 1_ilp,a( i+1, i ), 1_ilp )&
                              ,KIND=qp)
                    call stdlib_wlacgv( i-1, a( i, 1_ilp ), lda )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', n-i, i-1, cone,a( i+1, 1_ilp ), lda, a( &
                              i+1, i ), 1_ilp,cmplx( aii,KIND=qp), a( i, 1_ilp ), lda )
                    call stdlib_wlacgv( i-1, a( i, 1_ilp ), lda )
                 else
                    call stdlib_wdscal( i, aii, a( i, 1_ilp ), lda )
                 end if
              end do
           end if
           return
     ...
     !! RCOND = 1 / ( norm(A) * norm(inv(A)) ).
        ...
        ! =====================================================================
           ...
           smlnum = stdlib_qlamch( 'SAFE MINIMUM' )*real( max( 1_ilp, n ),KIND=qp)
           ! compute the norm of the triangular matrix a.
           anorm = stdlib_qlantp( norm, uplo, diag, n, ap, work )
           ! continue only if anorm > 0.
           if( anorm>zero ) then
              ! estimate the norm of the inverse of a.
              ainvnm = zero
              ...
              if( kase/=0_ilp ) then
                 if( kase==kase1 ) then
                    ! multiply by inv(a).
                    call stdlib_qlatps( uplo, 'NO TRANSPOSE', diag, normin, n, ap,work, scale, &
                              work( 2_ilp*n+1 ), info )
                 else
                    ! multiply by inv(a**t).
                    call stdlib_qlatps( uplo, 'TRANSPOSE', diag, normin, n, ap,work, scale, work( &
                              2_ilp*n+1 ), info )
                 end if
                 normin = 'Y'
                 ! multiply by 1/scale if doing so will not cause overflow.
     ...
     !! RCOND = 1 / ( norm(A) * norm(inv(A)) ).
        ...
        ! =====================================================================
           ...
           smlnum = stdlib_qlamch( 'SAFE MINIMUM' )*real( max( 1_ilp, n ),KIND=qp)
           ! compute the norm of the triangular matrix a.
           anorm = stdlib_wlantp( norm, uplo, diag, n, ap, rwork )
           ! continue only if anorm > 0.
           if( anorm>zero ) then
              ! estimate the norm of the inverse of a.
              ainvnm = zero
              ...
              if( kase/=0_ilp ) then
                 if( kase==kase1 ) then
                    ! multiply by inv(a).
                    call stdlib_wlatps( uplo, 'NO TRANSPOSE', diag, normin, n, ap,work, scale, &
                              rwork, info )
                 else
                    ! multiply by inv(a**h).
                    call stdlib_wlatps( uplo, 'CONJUGATE TRANSPOSE', diag, normin,n, ap, work, &
                              scale, rwork, info )
                 end if
                 normin = 'Y'
                 ! multiply by 1/scale if doing so will not cause overflow.
     ...
     !! nonsingular.
        ...
        ! =====================================================================
           ...
           ! solve a * x = b  or  a**t * x = b.
           do j = 1, nrhs
              call stdlib_qtpsv( uplo, trans, diag, n, ap, b( 1_ilp, j ), 1_ilp )
           end do
           return
     ...
     !! nonsingular.
        ...
        ! =====================================================================
           ...
           ! solve  a * x = b,  a**t * x = b,  or  a**h * x = b.
           do j = 1, nrhs
              call stdlib_wtpsv( uplo, trans, diag, n, ap, b( 1_ilp, j ), 1_ilp )
           end do
           return
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( normin, 'N' ) ) then
              ...
              if( upper ) then
                 ...
                 ip = 1_ilp
                 do j = 1, n
                    cnorm( j ) = stdlib_qasum( j-1, ap( ip ), 1_ilp )
                    ip = ip + j
                 end do
              else
                 ! a is lower triangular.
                 ip = 1_ilp
                 do j = 1, n - 1
                    cnorm( j ) = stdlib_qasum( n-j, ap( ip+1 ), 1_ilp )
                    ip = ip + n - j + 1_ilp
                 end do
                 cnorm( n ) = zero
              end if
           ...
           ! compute a bound on the computed solution vector to see if the
           ! level 2 blas routine stdlib_qtpsv can be used.
           j = stdlib_iqamax( n, x, 1_ilp )
           xmax = abs( x( j ) )
           xbnd = xmax
           if( notran ) then
           ...
           if( ( grow*tscal )>smlnum ) then
              ! use the level 2 blas solve if the reciprocal of the bound on
              ! elements of x is not too small.
              call stdlib_qtpsv( uplo, trans, diag, n, ap, x, 1_ilp )
           else
              ! use a level 1 blas solve, scaling intermediate results.
              if( xmax>bignum ) then
                 ! scale x so that its components are less than or equal to
              ...
              if( notran ) then
                 ...
                 loop_110: do j = jfirst, jlast, jinc
                    ...
                    if( upper ) then
                       if( j>1_ilp ) then
                          ! compute the update
                             ! x(1:j-1) := x(1:j-1) - x(j) * a(1:j-1,j)
                          call stdlib_qaxpy( j-1, -x( j )*tscal, ap( ip-j+1 ), 1_ilp, x,1_ilp )
                          i = stdlib_iqamax( j-1, x, 1_ilp )
                          xmax = abs( x( i ) )
                       end if
                       ip = ip - j
                       ...
                       if( j<n ) then
                          ! compute the update
                             ! x(j+1:n) := x(j+1:n) - x(j) * a(j+1:n,j)
                          call stdlib_qaxpy( n-j, -x( j )*tscal, ap( ip+1 ), 1_ilp,x( j+1 ), 1_ilp )
                 ...
                 loop_160: do j = jfirst, jlast, jinc
                    ...
                    if( uscal==one ) then
                       ...
                       ! call stdlib_qdot to perform the dot product.
                       if( upper ) then
                          sumj = stdlib_qdot( j-1, ap( ip-j+1 ), 1_ilp, x, 1_ilp )
                       else if( j<n ) then
                          sumj = stdlib_qdot( n-j, ap( ip+1 ), 1_ilp, x( j+1 ), 1_ilp )
                       end if
                    else
                       ! otherwise, use in-line code for the dot product.
                       if( upper ) then
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( normin, 'N' ) ) then
              ...
              if( upper ) then
                 ...
                 ip = 1_ilp
                 do j = 1, n
                    cnorm( j ) = stdlib_qzasum( j-1, ap( ip ), 1_ilp )
                    ip = ip + j
                 end do
              else
                 ! a is lower triangular.
                 ip = 1_ilp
                 do j = 1, n - 1
                    cnorm( j ) = stdlib_qzasum( n-j, ap( ip+1 ), 1_ilp )
                    ip = ip + n - j + 1_ilp
                 end do
                 cnorm( n ) = zero
              end if
           ...
           ! compute a bound on the computed solution vector to see if the
           ! level 2 blas routine stdlib_wtpsv can be used.
           xmax = zero
           do j = 1, n
              xmax = max( xmax, cabs2( x( j ) ) )
           end do
           ...
           if( ( grow*tscal )>smlnum ) then
              ! use the level 2 blas solve if the reciprocal of the bound on
              ! elements of x is not too small.
              call stdlib_wtpsv( uplo, trans, diag, n, ap, x, 1_ilp )
           else
              ! use a level 1 blas solve, scaling intermediate results.
              if( xmax>bignum*half ) then
                 ! scale x so that its components are less than or equal to
              ...
              if( notran ) then
                 ...
                 loop_120: do j = jfirst, jlast, jinc
                    ...
                    if( upper ) then
                       if( j>1_ilp ) then
                          ! compute the update
                             ! x(1:j-1) := x(1:j-1) - x(j) * a(1:j-1,j)
                          call stdlib_waxpy( j-1, -x( j )*tscal, ap( ip-j+1 ), 1_ilp, x,1_ilp )
                          i = stdlib_iwamax( j-1, x, 1_ilp )
                          xmax = cabs1( x( i ) )
                       end if
                       ip = ip - j
                       ...
                       if( j<n ) then
                          ! compute the update
                             ! x(j+1:n) := x(j+1:n) - x(j) * a(j+1:n,j)
                          call stdlib_waxpy( n-j, -x( j )*tscal, ap( ip+1 ), 1_ilp,x( j+1 ), 1_ilp )
                 ...
                 loop_170: do j = jfirst, jlast, jinc
                    ...
                    if( uscal==cmplx( one,KIND=qp) ) then
                       ...
                       ! call stdlib_wdotu to perform the dot product.
                       if( upper ) then
                          csumj = stdlib_wdotu( j-1, ap( ip-j+1 ), 1_ilp, x, 1_ilp )
                       else if( j<n ) then
                          csumj = stdlib_wdotu( n-j, ap( ip+1 ), 1_ilp, x( j+1 ), 1_ilp )
                       end if
                    else
                       ! otherwise, use in-line code for the dot product.
                       if( upper ) then
                 ...
                 loop_220: do j = jfirst, jlast, jinc
                    ...
                    if( uscal==cmplx( one,KIND=qp) ) then
                       ...
                       ! call stdlib_wdotc to perform the dot product.
                       if( upper ) then
                          csumj = stdlib_wdotc( j-1, ap( ip-j+1 ), 1_ilp, x, 1_ilp )
                       else if( j<n ) then
                          csumj = stdlib_wdotc( n-j, ap( ip+1 ), 1_ilp, x( j+1 ), 1_ilp )
                       end if
                    else
                       ! otherwise, use in-line code for the dot product.
                       if( upper ) then
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do j = 1, n
                 ...
                 ! compute elements 1:j-1 of j-th column.
                 call stdlib_qtpmv( 'UPPER', 'NO TRANSPOSE', diag, j-1, ap,ap( jc ), 1_ilp )
                 call stdlib_qscal( j-1, ajj, ap( jc ), 1_ilp )
                 jc = jc + j
              end do
           else
              ! compute inverse of lower triangular matrix.
              ...
              do j = n, 1, -1
                 ...
                 if( j<n ) then
                    ! compute elements j+1:n of j-th column.
                    call stdlib_qtpmv( 'LOWER', 'NO TRANSPOSE', diag, n-j,ap( jclast ), ap( jc+1 )&
                              , 1_ilp )
                    call stdlib_qscal( n-j, ajj, ap( jc+1 ), 1_ilp )
                 end if
                 jclast = jc
                 jc = jc - n + j - 2_ilp
              end do
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do j = 1, n
                 ...
                 ! compute elements 1:j-1 of j-th column.
                 call stdlib_wtpmv( 'UPPER', 'NO TRANSPOSE', diag, j-1, ap,ap( jc ), 1_ilp )
                 call stdlib_wscal( j-1, ajj, ap( jc ), 1_ilp )
                 jc = jc + j
              end do
           else
              ! compute inverse of lower triangular matrix.
              ...
              do j = n, 1, -1
                 ...
                 if( j<n ) then
                    ! compute elements j+1:n of j-th column.
                    call stdlib_wtpmv( 'LOWER', 'NO TRANSPOSE', diag, n-j,ap( jclast ), ap( jc+1 )&
                              , 1_ilp )
                    call stdlib_wscal( n-j, ajj, ap( jc+1 ), 1_ilp )
                 end if
                 jclast = jc
                 jc = jc - n + j - 2_ilp
              end do
     ...
     !! refinement because doing so cannot improve the backward error.
        ...
        ! =====================================================================
           ...
           loop_250: do j = 1, nrhs
              ...
              ! where op(a) = a or a**t, depending on trans.
              call stdlib_qcopy( n, x( 1_ilp, j ), 1_ilp, work( n+1 ), 1_ilp )
              call stdlib_qtpmv( uplo, trans, diag, n, ap, work( n+1 ), 1_ilp )
              call stdlib_qaxpy( n, -one, b( 1_ilp, j ), 1_ilp, work( n+1 ), 1_ilp )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(op(a))*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**t).
                    call stdlib_qtpsv( uplo, transt, diag, n, ap, work( n+1 ), 1_ilp )
                    do i = 1, n
                       work( n+i ) = work( i )*work( n+i )
                    end do
                 else
                    ...
                    call stdlib_qtpsv( uplo, trans, diag, n, ap, work( n+1 ), 1_ilp )
                 end if
                 go to 210
              end if
              ! normalize error.
     ...
     !! refinement because doing so cannot improve the backward error.
        ...
        ! =====================================================================
           ...
           loop_250: do j = 1, nrhs
              ...
              ! where op(a) = a, a**t, or a**h, depending on trans.
              call stdlib_wcopy( n, x( 1_ilp, j ), 1_ilp, work, 1_ilp )
              call stdlib_wtpmv( uplo, trans, diag, n, ap, work, 1_ilp )
              call stdlib_waxpy( n, -cone, b( 1_ilp, j ), 1_ilp, work, 1_ilp )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(op(a))*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**h).
                    call stdlib_wtpsv( uplo, transt, diag, n, ap, work, 1_ilp )
                    do i = 1, n
                       work( i ) = rwork( i )*work( i )
                    end do
                 else
                    ...
                    call stdlib_wtpsv( uplo, transn, diag, n, ap, work, 1_ilp )
                 end if
                 go to 210
              end if
              ! normalize error.
           ...
           if( nisodd ) then
              ...
              if( normaltransr ) then
                 ...
                 if( lower ) then
                   ...
                   ! t1 -> a(0,0), t2 -> a(0,1), s -> a(n1,0)
                   ! t1 -> a(0), t2 -> a(n), s -> a(n1)
                    call stdlib_qtrtri( 'L', diag, n1, a( 0_ilp ), n, info )
                    if( info>0 )return
                    call stdlib_qtrmm( 'R', 'L', 'N', diag, n2, n1, -one, a( 0_ilp ),n, a( n1 ), n )
                   ...
                   ! t1 -> a(n2), t2 -> a(n1), s -> a(0)
                    ...
                    if( info>0 )return
                    call stdlib_qtrmm( 'L', 'L', 'T', diag, n1, n2, -one, a( n2 ),n, a( 0_ilp ), n )
                              
                    call stdlib_qtrtri( 'U', diag, n2, a( n1 ), n, info )
                    if( info>0_ilp )info = info + n1
                    if( info>0 )return
                    call stdlib_qtrmm( 'R', 'U', 'N', diag, n1, n2, one, a( n1 ),n, a( 0_ilp ), n )
                 ...
                 if( lower ) then
                    ! srpa for lower, transpose and n is odd
                    ! t1 -> a(0), t2 -> a(1), s -> a(0+n1*n1)
                    call stdlib_qtrtri( 'U', diag, n1, a( 0_ilp ), n1, info )
                    if( info>0 )return
                    call stdlib_qtrmm( 'L', 'U', 'N', diag, n1, n2, -one, a( 0_ilp ),n1, a( n1*n1 ), &
                              n1 )
                    call stdlib_qtrtri( 'L', diag, n2, a( 1_ilp ), n1, info )
                    if( info>0_ilp )info = info + n1
                    if( info>0 )return
                    call stdlib_qtrmm( 'R', 'L', 'T', diag, n1, n2, one, a( 1_ilp ),n1, a( n1*n1 ), &
                              n1 )
                 else
                    ! srpa for upper, transpose and n is odd
                    ! t1 -> a(0+n2*n2), t2 -> a(0+n1*n2), s -> a(0)
                    call stdlib_qtrtri( 'U', diag, n1, a( n2*n2 ), n2, info )
                    if( info>0 )return
                    call stdlib_qtrmm( 'R', 'U', 'T', diag, n2, n1, -one,a( n2*n2 ), n2, a( 0_ilp ), &
                              n2 )
                    call stdlib_qtrtri( 'L', diag, n2, a( n1*n2 ), n2, info )
                    if( info>0_ilp )info = info + n1
                    if( info>0 )return
                    call stdlib_qtrmm( 'L', 'L', 'N', diag, n2, n1, one,a( n1*n2 ), n2, a( 0_ilp ), &
                              n2 )
                 end if
              end if
           else
              ...
              if( normaltransr ) then
                 ...
                 if( lower ) then
                    ...
                    ! t1 -> a(1,0), t2 -> a(0,0), s -> a(k+1,0)
                    ! t1 -> a(1), t2 -> a(0), s -> a(k+1)
                    call stdlib_qtrtri( 'L', diag, k, a( 1_ilp ), n+1, info )
                    if( info>0 )return
                    call stdlib_qtrmm( 'R', 'L', 'N', diag, k, k, -one, a( 1_ilp ),n+1, a( k+1 ), n+1 &
                              )
                    call stdlib_qtrtri( 'U', diag, k, a( 0_ilp ), n+1, info )
                    if( info>0_ilp )info = info + k
                    if( info>0 )return
                    call stdlib_qtrmm( 'L', 'U', 'T', diag, k, k, one, a( 0_ilp ), n+1,a( k+1 ), n+1 )
                              
                 else
                    ! srpa for upper, normal, and n is even ( a(0:n,0:k-1) )
                    ! t1 -> a(k+1,0) ,  t2 -> a(k,0),   s -> a(0,0)
                    ! t1 -> a(k+1), t2 -> a(k), s -> a(0)
                    call stdlib_qtrtri( 'L', diag, k, a( k+1 ), n+1, info )
                    if( info>0 )return
                    call stdlib_qtrmm( 'L', 'L', 'T', diag, k, k, -one, a( k+1 ),n+1, a( 0_ilp ), n+1 &
                              )
                    call stdlib_qtrtri( 'U', diag, k, a( k ), n+1, info )
                    if( info>0_ilp )info = info + k
                    if( info>0 )return
                    call stdlib_qtrmm( 'R', 'U', 'N', diag, k, k, one, a( k ), n+1,a( 0_ilp ), n+1 )
                 ...
                 if( lower ) then
                    ...
                    ! t1 -> b(0,1), t2 -> b(0,0), s -> b(0,k+1)
                    ! t1 -> a(0+k), t2 -> a(0+0), s -> a(0+k*(k+1)); lda=k
                    call stdlib_qtrtri( 'U', diag, k, a( k ), k, info )
                    if( info>0 )return
                    call stdlib_qtrmm( 'L', 'U', 'N', diag, k, k, -one, a( k ), k,a( k*( k+1 ) ), &
                              k )
                    call stdlib_qtrtri( 'L', diag, k, a( 0_ilp ), k, info )
                    if( info>0_ilp )info = info + k
                    if( info>0 )return
                    call stdlib_qtrmm( 'R', 'L', 'T', diag, k, k, one, a( 0_ilp ), k,a( k*( k+1 ) ), &
                              k )
                 else
                    ! srpa for upper, transpose and n is even (see paper)
                    ! t1 -> b(0,k+1),     t2 -> b(0,k),   s -> b(0,0)
                    ! t1 -> a(0+k*(k+1)), t2 -> a(0+k*k), s -> a(0+0)); lda=k
                    call stdlib_qtrtri( 'U', diag, k, a( k*( k+1 ) ), k, info )
                    if( info>0 )return
                    call stdlib_qtrmm( 'R', 'U', 'T', diag, k, k, -one,a( k*( k+1 ) ), k, a( 0_ilp ), &
                              k )
                    call stdlib_qtrtri( 'L', diag, k, a( k*k ), k, info )
                    if( info>0_ilp )info = info + k
                    if( info>0 )return
                    call stdlib_qtrmm( 'L', 'L', 'N', diag, k, k, one, a( k*k ), k,a( 0_ilp ), k )
           ...
           if( nisodd ) then
              ...
              if( normaltransr ) then
                 ...
                 if( lower ) then
                   ...
                   ! t1 -> a(0,0), t2 -> a(0,1), s -> a(n1,0)
                   ! t1 -> a(0), t2 -> a(n), s -> a(n1)
                    call stdlib_wtrtri( 'L', diag, n1, a( 0_ilp ), n, info )
                    if( info>0 )return
                    call stdlib_wtrmm( 'R', 'L', 'N', diag, n2, n1, -cone, a( 0_ilp ),n, a( n1 ), n )
                   ...
                   ! t1 -> a(n2), t2 -> a(n1), s -> a(0)
                    ...
                    if( info>0 )return
                    call stdlib_wtrmm( 'L', 'L', 'C', diag, n1, n2, -cone, a( n2 ),n, a( 0_ilp ), n )
                              
                    call stdlib_wtrtri( 'U', diag, n2, a( n1 ), n, info )
                    if( info>0_ilp )info = info + n1
                    if( info>0 )return
                    call stdlib_wtrmm( 'R', 'U', 'N', diag, n1, n2, cone, a( n1 ),n, a( 0_ilp ), n )
                 ...
                 if( lower ) then
                    ! srpa for lower, transpose and n is odd
                    ! t1 -> a(0), t2 -> a(1), s -> a(0+n1*n1)
                    call stdlib_wtrtri( 'U', diag, n1, a( 0_ilp ), n1, info )
                    if( info>0 )return
                    call stdlib_wtrmm( 'L', 'U', 'N', diag, n1, n2, -cone, a( 0_ilp ),n1, a( n1*n1 ), &
                              n1 )
                    call stdlib_wtrtri( 'L', diag, n2, a( 1_ilp ), n1, info )
                    if( info>0_ilp )info = info + n1
                    if( info>0 )return
                    call stdlib_wtrmm( 'R', 'L', 'C', diag, n1, n2, cone, a( 1_ilp ),n1, a( n1*n1 ), &
                              n1 )
                 else
                    ! srpa for upper, transpose and n is odd
                    ! t1 -> a(0+n2*n2), t2 -> a(0+n1*n2), s -> a(0)
                    call stdlib_wtrtri( 'U', diag, n1, a( n2*n2 ), n2, info )
                    if( info>0 )return
                    call stdlib_wtrmm( 'R', 'U', 'C', diag, n2, n1, -cone,a( n2*n2 ), n2, a( 0_ilp ), &
                              n2 )
                    call stdlib_wtrtri( 'L', diag, n2, a( n1*n2 ), n2, info )
                    if( info>0_ilp )info = info + n1
                    if( info>0 )return
                    call stdlib_wtrmm( 'L', 'L', 'N', diag, n2, n1, cone,a( n1*n2 ), n2, a( 0_ilp ), &
                              n2 )
                 end if
              end if
           else
              ...
              if( normaltransr ) then
                 ...
                 if( lower ) then
                    ...
                    ! t1 -> a(1,0), t2 -> a(0,0), s -> a(k+1,0)
                    ! t1 -> a(1), t2 -> a(0), s -> a(k+1)
                    call stdlib_wtrtri( 'L', diag, k, a( 1_ilp ), n+1, info )
                    if( info>0 )return
                    call stdlib_wtrmm( 'R', 'L', 'N', diag, k, k, -cone, a( 1_ilp ),n+1, a( k+1 ), n+&
                              1_ilp )
                    call stdlib_wtrtri( 'U', diag, k, a( 0_ilp ), n+1, info )
                    if( info>0_ilp )info = info + k
                    if( info>0 )return
                    call stdlib_wtrmm( 'L', 'U', 'C', diag, k, k, cone, a( 0_ilp ), n+1,a( k+1 ), n+1 &
                              )
                 else
                    ! srpa for upper, normal, and n is even ( a(0:n,0:k-1) )
                    ! t1 -> a(k+1,0) ,  t2 -> a(k,0),   s -> a(0,0)
                    ! t1 -> a(k+1), t2 -> a(k), s -> a(0)
                    call stdlib_wtrtri( 'L', diag, k, a( k+1 ), n+1, info )
                    if( info>0 )return
                    call stdlib_wtrmm( 'L', 'L', 'C', diag, k, k, -cone, a( k+1 ),n+1, a( 0_ilp ), n+&
                              1_ilp )
                    call stdlib_wtrtri( 'U', diag, k, a( k ), n+1, info )
                    if( info>0_ilp )info = info + k
                    if( info>0 )return
                    call stdlib_wtrmm( 'R', 'U', 'N', diag, k, k, cone, a( k ), n+1,a( 0_ilp ), n+1 )
                 ...
                 if( lower ) then
                    ...
                    ! t1 -> b(0,1), t2 -> b(0,0), s -> b(0,k+1)
                    ! t1 -> a(0+k), t2 -> a(0+0), s -> a(0+k*(k+1)); lda=k
                    call stdlib_wtrtri( 'U', diag, k, a( k ), k, info )
                    if( info>0 )return
                    call stdlib_wtrmm( 'L', 'U', 'N', diag, k, k, -cone, a( k ), k,a( k*( k+1 ) ),&
                               k )
                    call stdlib_wtrtri( 'L', diag, k, a( 0_ilp ), k, info )
                    if( info>0_ilp )info = info + k
                    if( info>0 )return
                    call stdlib_wtrmm( 'R', 'L', 'C', diag, k, k, cone, a( 0_ilp ), k,a( k*( k+1 ) ), &
                              k )
                 else
                    ! srpa for upper, transpose and n is even (see paper)
                    ! t1 -> b(0,k+1),     t2 -> b(0,k),   s -> b(0,0)
                    ! t1 -> a(0+k*(k+1)), t2 -> a(0+k*k), s -> a(0+0)); lda=k
                    call stdlib_wtrtri( 'U', diag, k, a( k*( k+1 ) ), k, info )
                    if( info>0 )return
                    call stdlib_wtrmm( 'R', 'U', 'C', diag, k, k, -cone,a( k*( k+1 ) ), k, a( 0_ilp ),&
                               k )
                    call stdlib_wtrtri( 'L', diag, k, a( k*k ), k, info )
                    if( info>0_ilp )info = info + k
                    if( info>0 )return
                    call stdlib_wtrmm( 'L', 'L', 'N', diag, k, k, cone, a( k*k ), k,a( 0_ilp ), k )
     ...
     !! RCOND = 1 / ( norm(A) * norm(inv(A)) ).
        ...
        ! =====================================================================
           ...
           smlnum = stdlib_qlamch( 'SAFE MINIMUM' )*real( max( 1_ilp, n ),KIND=qp)
           ! compute the norm of the triangular matrix a.
           anorm = stdlib_qlantb( norm, uplo, diag, n, kd, ab, ldab, work )
           ! continue only if anorm > 0.
           if( anorm>zero ) then
              ! estimate the norm of the inverse of a.
              ainvnm = zero
              ...
              if( kase/=0_ilp ) then
                 if( kase==kase1 ) then
                    ! multiply by inv(a).
                    call stdlib_qlatbs( uplo, 'NO TRANSPOSE', diag, normin, n, kd,ab, ldab, work, &
                              scale, work( 2_ilp*n+1 ), info )
                 else
                    ! multiply by inv(a**t).
                    call stdlib_qlatbs( uplo, 'TRANSPOSE', diag, normin, n, kd, ab,ldab, work, &
                              scale, work( 2_ilp*n+1 ), info )
                 end if
                 normin = 'Y'
                 ! multiply by 1/scale if doing so will not cause overflow.
     ...
     !! RCOND = 1 / ( norm(A) * norm(inv(A)) ).
        ...
        ! =====================================================================
           ...
           rcond = zero
           smlnum = stdlib_qlamch( 'SAFE MINIMUM' )*real( max( n, 1_ilp ),KIND=qp)
           ! compute the 1-norm of the triangular matrix a or a**h.
           anorm = stdlib_wlantb( norm, uplo, diag, n, kd, ab, ldab, rwork )
           ! continue only if anorm > 0.
           if( anorm>zero ) then
              ! estimate the 1-norm of the inverse of a.
              ainvnm = zero
              ...
              if( kase/=0_ilp ) then
                 if( kase==kase1 ) then
                    ! multiply by inv(a).
                    call stdlib_wlatbs( uplo, 'NO TRANSPOSE', diag, normin, n, kd,ab, ldab, work, &
                              scale, rwork, info )
                 else
                    ! multiply by inv(a**h).
                    call stdlib_wlatbs( uplo, 'CONJUGATE TRANSPOSE', diag, normin,n, kd, ab, ldab,&
                               work, scale, rwork, info )
                 end if
                 normin = 'Y'
                 ! multiply by 1/scale if doing so will not cause overflow.
     ...
     !! N-by NRHS matrix.  A check is made to verify that A is nonsingular.
        ...
        ! =====================================================================
           ...
           ! solve a * x = b  or  a**t * x = b.
           do j = 1, nrhs
              call stdlib_qtbsv( uplo, trans, diag, n, kd, ab, ldab, b( 1_ilp, j ), 1_ilp )
           end do
           return
     ...
     !! N-by-NRHS matrix.  A check is made to verify that A is nonsingular.
        ...
        ! =====================================================================
           ...
           ! solve a * x = b,  a**t * x = b,  or  a**h * x = b.
           do j = 1, nrhs
              call stdlib_wtbsv( uplo, trans, diag, n, kd, ab, ldab, b( 1_ilp, j ), 1_ilp )
           end do
           return
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( normin, 'N' ) ) then
              ...
              if( upper ) then
                 ...
                 do j = 1, n
                    jlen = min( kd, j-1 )
                    cnorm( j ) = stdlib_qasum( jlen, ab( kd+1-jlen, j ), 1_ilp )
                 end do
              else
                 ! a is lower triangular.
                 do j = 1, n
                    jlen = min( kd, n-j )
                    if( jlen>0_ilp ) then
                       cnorm( j ) = stdlib_qasum( jlen, ab( 2_ilp, j ), 1_ilp )
                    else
                       cnorm( j ) = zero
                    end if
                 end do
           ...
           ! compute a bound on the computed solution vector to see if the
           ! level 2 blas routine stdlib_qtbsv can be used.
           j = stdlib_iqamax( n, x, 1_ilp )
           xmax = abs( x( j ) )
           xbnd = xmax
           if( notran ) then
           ...
           if( ( grow*tscal )>smlnum ) then
              ! use the level 2 blas solve if the reciprocal of the bound on
              ! elements of x is not too small.
              call stdlib_qtbsv( uplo, trans, diag, n, kd, ab, ldab, x, 1_ilp )
           else
              ! use a level 1 blas solve, scaling intermediate results.
              if( xmax>bignum ) then
                 ! scale x so that its components are less than or equal to
              ...
              if( notran ) then
                    ...
                    if( upper ) then
                       if( j>1_ilp ) then
                          ! compute the update
                             ! x(max(1,j-kd):j-1) := x(max(1,j-kd):j-1) -
                                                   ! x(j)* a(max(1,j-kd):j-1,j)
                          jlen = min( kd, j-1 )
                          call stdlib_qaxpy( jlen, -x( j )*tscal,ab( kd+1-jlen, j ), 1_ilp, x( j-jlen &
                                    ), 1_ilp )
                          i = stdlib_iqamax( j-1, x, 1_ilp )
                          xmax = abs( x( i ) )
                       end if
                       ...
                       ! compute the update
                          ! x(j+1:min(j+kd,n)) := x(j+1:min(j+kd,n)) -
                                                ! x(j) * a(j+1:min(j+kd,n),j)
                       jlen = min( kd, n-j )
                       if( jlen>0_ilp )call stdlib_qaxpy( jlen, -x( j )*tscal, ab( 2_ilp, j ), 1_ilp,x( j+1 ),&
                                  1_ilp )
                       i = j + stdlib_iqamax( n-j, x( j+1 ), 1_ilp )
                       xmax = abs( x( i ) )
                    end if
                 ...
                 loop_160: do j = jfirst, jlast, jinc
                    ...
                    if( uscal==one ) then
                       ...
                       if( upper ) then
                          jlen = min( kd, j-1 )
                          sumj = stdlib_qdot( jlen, ab( kd+1-jlen, j ), 1_ilp,x( j-jlen ), 1_ilp )
                       else
                          jlen = min( kd, n-j )
                          if( jlen>0_ilp )sumj = stdlib_qdot( jlen, ab( 2_ilp, j ), 1_ilp, x( j+1 ), 1_ilp )
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( normin, 'N' ) ) then
              ...
              if( upper ) then
                 ...
                 do j = 1, n
                    jlen = min( kd, j-1 )
                    cnorm( j ) = stdlib_qzasum( jlen, ab( kd+1-jlen, j ), 1_ilp )
                 end do
              else
                 ! a is lower triangular.
                 do j = 1, n
                    jlen = min( kd, n-j )
                    if( jlen>0_ilp ) then
                       cnorm( j ) = stdlib_qzasum( jlen, ab( 2_ilp, j ), 1_ilp )
                    else
                       cnorm( j ) = zero
                    end if
                 end do
           ...
           ! compute a bound on the computed solution vector to see if the
           ! level 2 blas routine stdlib_wtbsv can be used.
           xmax = zero
           do j = 1, n
              xmax = max( xmax, cabs2( x( j ) ) )
           end do
           ...
           if( ( grow*tscal )>smlnum ) then
              ! use the level 2 blas solve if the reciprocal of the bound on
              ! elements of x is not too small.
              call stdlib_wtbsv( uplo, trans, diag, n, kd, ab, ldab, x, 1_ilp )
           else
              ! use a level 1 blas solve, scaling intermediate results.
              if( xmax>bignum*half ) then
                 ! scale x so that its components are less than or equal to
              ...
              if( notran ) then
                    ...
                    if( upper ) then
                       if( j>1_ilp ) then
                          ! compute the update
                             ! x(max(1,j-kd):j-1) := x(max(1,j-kd):j-1) -
                                                   ! x(j)* a(max(1,j-kd):j-1,j)
                          jlen = min( kd, j-1 )
                          call stdlib_waxpy( jlen, -x( j )*tscal,ab( kd+1-jlen, j ), 1_ilp, x( j-jlen &
                                    ), 1_ilp )
                          i = stdlib_iwamax( j-1, x, 1_ilp )
                          xmax = cabs1( x( i ) )
                       end if
                       ...
                       ! compute the update
                          ! x(j+1:min(j+kd,n)) := x(j+1:min(j+kd,n)) -
                                                ! x(j) * a(j+1:min(j+kd,n),j)
                       jlen = min( kd, n-j )
                       if( jlen>0_ilp )call stdlib_waxpy( jlen, -x( j )*tscal, ab( 2_ilp, j ), 1_ilp,x( j+1 ),&
                                  1_ilp )
                       i = j + stdlib_iwamax( n-j, x( j+1 ), 1_ilp )
                       xmax = cabs1( x( i ) )
                    end if
                 ...
                 loop_170: do j = jfirst, jlast, jinc
                    ...
                    if( uscal==cmplx( one,KIND=qp) ) then
                       ...
                       if( upper ) then
                          jlen = min( kd, j-1 )
                          csumj = stdlib_wdotu( jlen, ab( kd+1-jlen, j ), 1_ilp,x( j-jlen ), 1_ilp )
                                    
                       else
                          jlen = min( kd, n-j )
                          if( jlen>1_ilp )csumj = stdlib_wdotu( jlen, ab( 2_ilp, j ), 1_ilp, x( j+1 ),1_ilp )
                 ...
                 loop_220: do j = jfirst, jlast, jinc
                    ...
                    if( uscal==cmplx( one,KIND=qp) ) then
                       ...
                       if( upper ) then
                          jlen = min( kd, j-1 )
                          csumj = stdlib_wdotc( jlen, ab( kd+1-jlen, j ), 1_ilp,x( j-jlen ), 1_ilp )
                                    
                       else
                          jlen = min( kd, n-j )
                          if( jlen>1_ilp )csumj = stdlib_wdotc( jlen, ab( 2_ilp, j ), 1_ilp, x( j+1 ),1_ilp )
     ...
     !! refinement because doing so cannot improve the backward error.
        ...
        ! =====================================================================
           ...
           loop_250: do j = 1, nrhs
              ...
              ! where op(a) = a or a**t, depending on trans.
              call stdlib_qcopy( n, x( 1_ilp, j ), 1_ilp, work( n+1 ), 1_ilp )
              call stdlib_qtbmv( uplo, trans, diag, n, kd, ab, ldab, work( n+1 ),1_ilp )
              call stdlib_qaxpy( n, -one, b( 1_ilp, j ), 1_ilp, work( n+1 ), 1_ilp )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(op(a))*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**t).
                    call stdlib_qtbsv( uplo, transt, diag, n, kd, ab, ldab,work( n+1 ), 1_ilp )
                    ...
                    call stdlib_qtbsv( uplo, trans, diag, n, kd, ab, ldab,work( n+1 ), 1_ilp )
                 end if
                 go to 210
              end if
              ! normalize error.
     ...
     !! refinement because doing so cannot improve the backward error.
        ...
        ! =====================================================================
           ...
           loop_250: do j = 1, nrhs
              ...
              ! where op(a) = a, a**t, or a**h, depending on trans.
              call stdlib_wcopy( n, x( 1_ilp, j ), 1_ilp, work, 1_ilp )
              call stdlib_wtbmv( uplo, trans, diag, n, kd, ab, ldab, work, 1_ilp )
              call stdlib_waxpy( n, -cone, b( 1_ilp, j ), 1_ilp, work, 1_ilp )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(op(a))*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**h).
                    call stdlib_wtbsv( uplo, transt, diag, n, kd, ab, ldab, work,1_ilp )
                    do i = 1, n
                       work( i ) = rwork( i )*work( i )
                    end do
                 else
                    ...
                    call stdlib_wtbsv( uplo, transn, diag, n, kd, ab, ldab, work,1_ilp )
                 end if
                 go to 210
              end if
              ! normalize error.
