  ...
  contains
     ...
     !! real symmetric tridiagonal matrix A.
        ...
        ! =====================================================================
           ...
           if( iscale==1_ilp ) then
              call stdlib_qscal( n, sigma, d, 1_ilp )
              call stdlib_qscal( n-1, sigma, e( 1_ilp ), 1_ilp )
           end if
           ! for eigenvalues only, call stdlib_qsterf.  for eigenvalues and
           ! eigenvectors, call stdlib_qsteqr.
           if( .not.wantz ) then
              call stdlib_qsterf( n, d, e, info )
           else
              call stdlib_qsteqr( 'I', n, d, e, z, ldz, work, info )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp ) then
              if( info==0_ilp ) then
              ...
              call stdlib_qscal( imax, one / sigma, d, 1_ilp )
           end if
           return
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! for eigenvalues only, call stdlib_qsterf.  for eigenvalues and
           ! eigenvectors, call stdlib_qstedc.
           if( .not.wantz ) then
              call stdlib_qsterf( n, d, e, info )
           else
              call stdlib_qstedc( 'I', n, d, e, z, ldz, work, lwork, iwork, liwork,info )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           if( iscale==1_ilp )call stdlib_qscal( n, one / sigma, d, 1_ilp )
           work( 1_ilp ) = lwmin
           iwork( 1_ilp ) = liwmin
           return
     ...
     !! manner.
        ...
        ! =====================================================================
           ...
           ! initialize indices into workspaces.  note: these indices are used only
           ! if stdlib_qsterf or stdlib_qstemr fail.
           ! iwork(indibl:indibl+m-1) corresponds to iblock in stdlib_qstebz and
           ! stores the block indices of each of the m<=n eigenvalues.
           indibl = 1_ilp
           ! iwork(indisp:indisp+nsplit-1) corresponds to isplit in stdlib_qstebz and
           ! stores the starting and finishing indices of each block.
           indisp = indibl + n
           ! iwork(indifl:indifl+n-1) stores the indices of eigenvectors
           ! that corresponding to eigenvectors that fail to converge in
           ! stdlib_qstein.  this information is discarded; if any fail, the driver
           ! returns info > 0.
           indifl = indisp + n
           ! indiwo is the offset of the remaining integer workspace.
           indiwo = indisp + n
           ! if all eigenvalues are desired, then
           ! call stdlib_qsterf or stdlib_qstemr.  if this fails for some eigenvalue, then
           ! try stdlib_qstebz.
           test = .false.
           if( indeig ) then
              if( il==1_ilp .and. iu==n ) then
           ...
           if( ( alleig .or. test ) .and. ieeeok==1_ilp ) then
              call stdlib_qcopy( n-1, e( 1_ilp ), 1_ilp, work( 1_ilp ), 1_ilp )
              if( .not.wantz ) then
                 call stdlib_qcopy( n, d, 1_ilp, w, 1_ilp )
                 call stdlib_qsterf( n, w, work, info )
              else
                 call stdlib_qcopy( n, d, 1_ilp, work( n+1 ), 1_ilp )
                 if (abstol <= two*n*eps) then
                    tryrac = .true.
                 else
                    tryrac = .false.
                 end if
                 call stdlib_qstemr( jobz, 'A', n, work( n+1 ), work, vl, vu, il,iu, m, w, z, ldz,&
                            n, isuppz, tryrac,work( 2_ilp*n+1 ), lwork-2*n, iwork, liwork, info )
              end if
              if( info==0_ilp ) then
                 m = n
           ...
           ! otherwise, call stdlib_qstebz and, if eigenvectors are desired, stdlib_qstein.
           if( wantz ) then
              order = 'B'
           else
              order = 'E'
           end if
           call stdlib_qstebz( range, order, n, vll, vuu, il, iu, abstol, d, e, m,nsplit, w, &
                     iwork( indibl ), iwork( indisp ), work,iwork( indiwo ), info )
           if( wantz ) then
              call stdlib_qstein( n, d, e, m, w, iwork( indibl ), iwork( indisp ),z, ldz, work, &
                        iwork( indiwo ), iwork( indifl ),info )
           end if
           ! if matrix was scaled, then rescale eigenvalues appropriately.
           10 continue
           if( iscale==1_ilp ) then
              ...
              call stdlib_qscal( imax, one / sigma, w, 1_ilp )
           end if
           ! if eigenvalues are not in order, then sort them, along with
           ! eigenvectors.
           if( wantz ) then
        ...
        ! =====================================================================
           ...
           if( ( alleig .or. test ) .and. ( abstol<=zero ) ) then
              ...
              if( .not.wantz ) then
                 ...
                 call stdlib_qsteqr( 'I', n, w, work, z, ldz, work( indwrk ), info )
                 if( info==0_ilp ) then
                    do i = 1, n
                       ifail( i ) = 0_ilp
                    end do
     ...
     !! these eigenvalues range over many orders of magnitude.)
        ...
        ! =====================================================================
           ...
           ! Local Arrays 
           real(qp) :: c(1_ilp,1_ilp), vt(1_ilp,1_ilp)
           ! Local Scalars 
           integer(ilp) :: i, icompz, nru
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           if( icompz==2_ilp )call stdlib_qlaset( 'FULL', n, n, zero, one, z, ldz )
           ! call stdlib_qpttrf to factor the matrix.
           call stdlib_qpttrf( n, d, e, info )
           if( info/=0 )return
           do i = 1, n
              d( i ) = sqrt( d( i ) )
           end do
           ...
           ! call stdlib_qbdsqr to compute the singular values/vectors of the
           ! bidiagonal factor.
           if( icompz>0_ilp ) then
              nru = n
           else
           ...
           call stdlib_qbdsqr( 'LOWER', n, 0_ilp, nru, 0_ilp, d, e, vt, 1_ilp, z, ldz, c, 1_ilp,work, info )
     ...
     !! matrix, if these eigenvalues range over many orders of magnitude.)
        ...
        ! ====================================================================
           ...
           ! Local Arrays 
           complex(qp) :: c(1_ilp,1_ilp), vt(1_ilp,1_ilp)
           ! Local Scalars 
           integer(ilp) :: i, icompz, nru
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           if( icompz==2_ilp )call stdlib_wlaset( 'FULL', n, n, czero, cone, z, ldz )
           ! call stdlib_qpttrf to factor the matrix.
           call stdlib_qpttrf( n, d, e, info )
           if( info/=0 )return
           do i = 1, n
           ...
           ! call stdlib_wbdsqr to compute the singular values/vectors of the
           ! bidiagonal factor.
           if( icompz>0_ilp ) then
              nru = n
           else
           ...
           call stdlib_wbdsqr( 'LOWER', n, 0_ilp, nru, 0_ilp, d, e, vt, 1_ilp, z, ldz, c, 1_ilp,work, info )
     ...
     !! University, July 21, 1966.
        ...
        ! =====================================================================
           ...
           if( irange==3_ilp ) then
              ...
              iwork( 5_ilp ) = il - 1_ilp
              iwork( 6_ilp ) = iu
              call stdlib_qlaebz( 3_ilp, itmax, n, 2_ilp, 2_ilp, nb, atoli, rtoli, pivmin, d, e,work, iwork( &
                        5_ilp ), work( n+1 ), work( n+5 ), iout,iwork, w, iblock, iinfo )
              if( iwork( 6_ilp )==iu ) then
                 wl = work( n+1 )
                 wlu = work( n+3 )
           ...
           loop_70: do jb = 1, nsplit
              ...
              if( in==1_ilp ) then
                 ...
                 work( n+1 ) = gl
                 work( n+in+1 ) = gu
                 call stdlib_qlaebz( 1_ilp, 0_ilp, in, in, 1_ilp, nb, atoli, rtoli, pivmin,d( ibegin ), e( &
                 ibegin ), work( ibegin ),idumma, work( n+1 ), work( n+2*in+1 ), im,iwork, w( m+1 &
                           ), iblock( m+1 ), iinfo )
                 nwl = nwl + iwork( 1_ilp )
                 nwu = nwu + iwork( in+1 )
                 ...
                 itmax = int( ( log( gu-gl+pivmin )-log( pivmin ) ) /log( two ),KIND=ilp) + &
                           2_ilp
                 call stdlib_qlaebz( 2_ilp, itmax, in, in, 1_ilp, nb, atoli, rtoli, pivmin,d( ibegin ), e(&
                  ibegin ), work( ibegin ),idumma, work( n+1 ), work( n+2*in+1 ), iout,iwork, w( &
                            m+1 ), iblock( m+1 ), iinfo )
                 ! copy eigenvalues into w and iblock
                 ! use -jb for block number for unconverged eigenvalues.
     ...
     !! using the Pal-Walker-Kahan variant of the QL or QR algorithm.
        ...
        ! =====================================================================
           ...
           if( lend==l )go to 10
           ! scale submatrix in rows and columns l to lend
           anorm = stdlib_qlanst( 'M', lend-l+1, d( l ), e( l ) )
           iscale = 0_ilp
           if( anorm==zero )go to 10
           if( (anorm>ssfmax) ) then
              iscale = 1_ilp
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, anorm, ssfmax, lend-l+1, 1_ilp, d( l ), n,info )
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, anorm, ssfmax, lend-l, 1_ilp, e( l ), n,info )
           else if( anorm<ssfmin ) then
              iscale = 2_ilp
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, anorm, ssfmin, lend-l+1, 1_ilp, d( l ), n,info )
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, anorm, ssfmin, lend-l, 1_ilp, e( l ), n,info )
           end if
           do i = l, lend - 1
              e( i ) = e( i )**2_ilp
           end do
           ...
           if( lend>=l ) then
              ...
              p = d( l )
              if( m==l )go to 90
              ! if remaining matrix is 2 by 2, use stdlib_qlae2 to compute its
              ! eigenvalues.
              if( m==l+1 ) then
                 rte = sqrt( e( l ) )
                 call stdlib_qlae2( d( l ), rte, d( l+1 ), rt1, rt2 )
                 d( l ) = rt1
                 d( l+1 ) = rt2
                 e( l ) = zero
                 l = l + 2_ilp
           ...
           ! undo scaling if necessary
           150 continue
           if( iscale==1_ilp )call stdlib_qlascl( 'G', 0_ilp, 0_ilp, ssfmax, anorm, lendsv-lsv+1, 1_ilp,d( lsv ), &
                     n, info )
           if( iscale==2_ilp )call stdlib_qlascl( 'G', 0_ilp, 0_ilp, ssfmin, anorm, lendsv-lsv+1, 1_ilp,d( lsv ), &
                     n, info )
           ! check for no convergence to an eigenvalue after a total
           ! of n*maxit iterations.
           if( jtot<nmaxit )go to 10
     ...
     !! without guard digits, but we know of none.  See DLAED3 for details.
        ...
        ! =====================================================================
           ...
           ! eigenvalues, which requires (3n + 3n**2) real workspace and
           ! (2 + 5n + 2n lg(n)) integer workspace.
           ! since on many architectures stdlib_qsterf is much faster than any other
           ! algorithm for finding eigenvalues only, it is used here
           ! as the default. if the conditional clause is removed, then
           ! information on the size of workspace needs to be changed.
           ! if compz = 'n', use stdlib_qsterf to compute the eigenvalues.
           if( icompz==0_ilp ) then
              call stdlib_qsterf( n, d, e, info )
              go to 50
           end if
           ...
           ! solve the problem with another solver.
           if( n<=smlsiz ) then
              call stdlib_qsteqr( compz, n, d, e, z, ldz, work, info )
           else
              ! if compz = 'v', the z matrix must be stored elsewhere for later
              ! use.
              if( icompz==1_ilp ) then
              ...
              if( icompz==2_ilp ) then
                 call stdlib_qlaset( 'FULL', n, n, zero, one, z, ldz )
              end if
              ! scale.
              orgnrm = stdlib_qlanst( 'M', n, d, e )
              if( orgnrm==zero )go to 50
              ...
              if( start<=n ) then
                 ...
                 if( m>smlsiz ) then
                    ...
                    call stdlib_qlascl( 'G', 0_ilp, 0_ilp, orgnrm, one, m, 1_ilp, d( start ), m,info )
                    call stdlib_qlascl( 'G', 0_ilp, 0_ilp, orgnrm, one, m-1, 1_ilp, e( start ),m-1, info )
                    ...
                    call stdlib_qlaed0( icompz, n, m, d( start ), e( start ),z( strtrw, start ), &
                              ldz, work( 1_ilp ), n,work( storez ), iwork, info )
                    if( info/=0_ilp ) then
                       info = ( info / ( m+1 )+start-1 )*( n+1 ) +mod( info, ( m+1 ) ) + start - &
                                 1_ilp
                    ...
                    ! scale back.
                    call stdlib_qlascl( 'G', 0_ilp, 0_ilp, one, orgnrm, m, 1_ilp, d( start ), m,info )
                 else
                    if( icompz==1_ilp ) then
                       ! since qr won't update a z matrix which is larger than
                       ! the length of d, we must solve the sub-problem in a
                       ...
                       call stdlib_qsteqr( 'I', m, d( start ), e( start ), work, m,work( m*m+1 ), &
                                 info )
                       call stdlib_qlacpy( 'A', n, m, z( 1_ilp, start ), ldz,work( storez ), n )
                                 
                       call stdlib_qgemm( 'N', 'N', n, m, m, one,work( storez ), n, work, m, zero,&
                                 z( 1_ilp, start ), ldz )
                    else if( icompz==2_ilp ) then
                       call stdlib_qsteqr( 'I', m, d( start ), e( start ),z( start, start ), ldz, &
                                 work, info )
                    else
                       call stdlib_qsterf( m, d( start ), e( start ), info )
                    end if
                    if( info/=0_ilp ) then
                       info = start*( n+1 ) + finish
                       go to 50
              ...
              if( icompz==0_ilp ) then
                ...
                do ii = 2, n
                   ...
                   if( k/=i ) then
                      d( k ) = d( i )
                      d( i ) = p
                      call stdlib_qswap( n, z( 1_ilp, i ), 1_ilp, z( 1_ilp, k ), 1_ilp )
                   end if
                end do
              end if
           end if
     ...
     !! without guard digits, but we know of none.  See DLAED3 for details.
        ...
        ! =====================================================================
           ...
           ! solve the problem with another solver.
           if( n<=smlsiz ) then
              call stdlib_wsteqr( compz, n, d, e, z, ldz, rwork, info )
           else
              ! if compz = 'i', we simply call stdlib_qstedc instead.
              if( icompz==2_ilp ) then
                 call stdlib_qlaset( 'FULL', n, n, zero, one, rwork, n )
                 ll = n*n + 1_ilp
                 call stdlib_qstedc( 'I', n, d, e, rwork, n,rwork( ll ), lrwork-ll+1, iwork, &
                           liwork, info )
                 do j = 1, n
                    do i = 1, n
                       z( i, j ) = rwork( ( j-1 )*n+i )
              ...
              if( start<=n ) then
                 ...
                 if( m>smlsiz ) then
                    ...
                    call stdlib_wlaed0( n, m, d( start ), e( start ), z( 1_ilp, start ),ldz, work, n, &
                              rwork, iwork, info )
                    if( info>0_ilp ) then
                       info = ( info / ( m+1 )+start-1 )*( n+1 ) +mod( info, ( m+1 ) ) + start - &
                                 1_ilp
                    ...
                    call stdlib_qsteqr( 'I', m, d( start ), e( start ), rwork, m,rwork( m*m+1 ), &
                              info )
                    call stdlib_wlacrm( n, m, z( 1_ilp, start ), ldz, rwork, m, work, n,rwork( m*m+1 )&
                               )
                    call stdlib_wlacpy( 'A', n, m, work, n, z( 1_ilp, start ), ldz )
                    if( info>0_ilp ) then
                       info = start*( n+1 ) + finish
                       go to 70
                    end if
              ...
              do ii = 2, n
                ...
                if( k/=i ) then
                   d( k ) = d( i )
                   d( i ) = p
                   call stdlib_wswap( n, z( 1_ilp, i ), 1_ilp, z( 1_ilp, k ), 1_ilp )
                end if
              end do
           end if
           70 continue
     ...
     !! specified by an internal parameter MAXITS (currently set to 5).
        ...
        ! =====================================================================
           ...
           integer(ilp) :: b1, blksiz, bn, gpind, i, iinfo, indrv1, indrv2, indrv3, indrv4, &
                     indrv5, its, j, j1, jblk, jmax, nblk, nrmchk
           real(qp) :: dtpcrt, eps, eps1, nrm, onenrm, ortol, pertol, scl, sep, tol, xj, xjm, &
                     ztr
           ! Local Arrays 
           integer(ilp) :: iseed(4_ilp)
           ! Intrinsic Functions 
           ...
           ! get machine constants.
           eps = stdlib_qlamch( 'PRECISION' )
           ! initialize seed for random number generator stdlib_qlarnv.
           do i = 1, 4
              iseed( i ) = 1_ilp
           end do
           ! initialize pointers.
           ...
           loop_160: do nblk = 1, iblock( m )
              ...
              loop_150: do j = j1, m
                 ...
                 nrmchk = 0_ilp
                 ! get random starting vector.
                 call stdlib_qlarnv( 2_ilp, iseed, blksiz, work( indrv1+1 ) )
                 ! copy the matrix t so it won't be destroyed in factorization.
                 call stdlib_qcopy( blksiz, d( b1 ), 1_ilp, work( indrv4+1 ), 1_ilp )
                 call stdlib_qcopy( blksiz-1, e( b1 ), 1_ilp, work( indrv2+2 ), 1_ilp )
                 call stdlib_qcopy( blksiz-1, e( b1 ), 1_ilp, work( indrv3+1 ), 1_ilp )
                 ! compute lu factors with partial pivoting  ( pt = lu )
                 tol = zero
                 ...
                 if( its>maxits )go to 100
                 ! normalize and scale the righthand side vector pb.
                 jmax = stdlib_iqamax( blksiz, work( indrv1+1 ), 1_ilp )
                 scl = blksiz*onenrm*max( eps,abs( work( indrv4+blksiz ) ) ) /abs( work( indrv1+&
                           jmax ) )
                 call stdlib_qscal( blksiz, scl, work( indrv1+1 ), 1_ilp )
                 ! solve the system lu = pb.
                 call stdlib_qlagts( -1_ilp, blksiz, work( indrv4+1 ), work( indrv2+2 ),work( indrv3+&
                           1_ilp ), work( indrv5+1 ), iwork,work( indrv1+1 ), tol, iinfo )
                 ! reorthogonalize by modified gram-schmidt if eigenvalues are
                 ! close enough.
                 if( jblk==1 )go to 90
                 ...
                 if( gpind/=j ) then
                    do i = gpind, j - 1
                       ztr = -stdlib_qdot( blksiz, work( indrv1+1 ), 1_ilp, z( b1, i ),1_ilp )
                       call stdlib_qaxpy( blksiz, ztr, z( b1, i ), 1_ilp,work( indrv1+1 ), 1_ilp )
                    end do
                 end if
                 ! check the infinity norm of the iterate.
                 90 continue
                 ...
                 ! accept iterate as jth eigenvector.
                 110 continue
                 scl = one / stdlib_qnrm2( blksiz, work( indrv1+1 ), 1_ilp )
                 jmax = stdlib_iqamax( blksiz, work( indrv1+1 ), 1_ilp )
                 if( work( indrv1+jmax )<zero )scl = -scl
                 call stdlib_qscal( blksiz, scl, work( indrv1+1 ), 1_ilp )
                 120 continue
              ...
              loop_170: do j = j1, m
                 ...
                 if( gpind/=j ) then
                    do i = gpind, j - 1
                       ztr = zero
                       do jr = 1, blksiz
                          ztr = ztr + work( indrv1+jr )*real( z( b1-1+jr, i ),KIND=qp)
                       end do
                       do jr = 1, blksiz
                          work( indrv1+jr ) = work( indrv1+jr ) -ztr*real( z( b1-1+jr, i ),&
                                    KIND=qp)
     ...
     !! zero divisors.
        ...
        ! =====================================================================
           ...
           lquery = ( ( lwork==-1_ilp ).or.( liwork==-1_ilp ) )
           zquery = ( nzc==-1_ilp )
           ! stdlib_qstemr needs work of size 6*n, iwork of size 3*n.
           ! in addition, stdlib_qlarre needs work of size 6*n, iwork of size 5*n.
           ! furthermore, stdlib_qlarrv needs work of size 12*n, iwork of size 7*n.
           if( wantz ) then
              lwmin = 18_ilp*n
              liwmin = 10_ilp*n
           else
           ...
           if( valeig ) then
              ! we do not reference vl, vu in the cases range = 'i','a'
              ! the interval (wl, wu] contains all the wanted eigenvalues.
              ! it is either given by the user or computed in stdlib_qlarre.
              wl = vl
              wu = vu
           elseif( indeig ) then
              ! we do not reference il, iu in the cases range = 'v','a'
           ...
           if( n==2_ilp ) then
              if( .not.wantz ) then
                 call stdlib_qlae2( d(1_ilp), e(1_ilp), d(2_ilp), r1, r2 )
              else if( wantz.and.(.not.zquery) ) then
                 call stdlib_qlaev2( d(1_ilp), e(1_ilp), d(2_ilp), r1, r2, cs, sn )
              end if
              if( alleig.or.(valeig.and.(r2>wl).and.(r2<=wu)).or.(indeig.and.(iil==1_ilp)) ) &
                        then
                 m = m+1
           ...
           ! continue with general n
              ...
              ! scale matrix to allowable range, if necessary.
              ! the allowable range is related to the pivmin parameter; see the
              ! comments in stdlib_qlarrd.  the preference for scaling small values
              ! up is heuristic; we expect users' matrices not to be close to the
              ! rmax threshold.
              scale = one
              tnrm = stdlib_qlanst( 'M', n, d, e )
              ...
              if( scale/=one ) then
                 call stdlib_qscal( n, scale, d, 1_ilp )
                 call stdlib_qscal( n-1, scale, e, 1_ilp )
                 tnrm = tnrm*scale
                 if( valeig ) then
                    ! if eigenvalues in interval have to be found,
                    ! scale (wl, wu] accordingly
              ...
              ! into smaller subblocks if the corresponding off-diagonal elements
              ! are small
              ! thresh is the splitting parameter for stdlib_qlarre
              ! a negative thresh forces the old splitting criterion based on the
              ! size of the off-diagonal. a positive thresh switches to splitting
              ! which preserves relative accuracy.
              if( tryrac ) then
                 ! test whether the matrix warrants the more expensive relative approach.
                 call stdlib_qlarrr( n, d, e, iinfo )
              else
                 ! the user does not care about relative accurately eigenvalues
                 iinfo = -1_ilp
              endif
              ...
              if( tryrac ) then
                 ! copy original diagonal, needed to guarantee relative accuracy
                 call stdlib_qcopy(n,d,1_ilp,work(indd),1_ilp)
              endif
              ! store the squares of the offdiagonal values of t
              do j = 1, n-1
                 work( inde2+j-1 ) = e(j)**2_ilp
              ...
              ! set the tolerance parameters for bisection
              if( .not.wantz ) then
                 ! stdlib_qlarre computes the eigenvalues to full precision.
                 rtol1 = four * eps
                 rtol2 = four * eps
              else
                 ! stdlib_qlarre computes the eigenvalues to less than full precision.
                 ! stdlib_qlarrv will refine the eigenvalue approximations, and we can
                 ! need less accurate initial bisection in stdlib_qlarre.
                 ! note: these settings do only affect the subset case and stdlib_qlarre
                 rtol1 = sqrt(eps)
              ...
              ! note that if range /= 'v', stdlib_qlarre computes bounds on the desired
              ! part of the spectrum. all desired eigenvalues are contained in
              ! (wl,wu]
              if( wantz ) then
                 ! compute the desired eigenvectors corresponding to the computed
                 ! eigenvalues
                 call stdlib_qlarrv( n, wl, wu, d, e,pivmin, iwork( iinspl ), m,1_ilp, m, minrgp, &
                 rtol1, rtol2,w, work( inderr ), work( indgp ), iwork( iindbl ),iwork( iindw ), &
                           work( indgrs ), z, ldz,isuppz, work( indwrk ), iwork( iindwk ), iinfo )
                 if( iinfo/=0_ilp ) then
                    info = 20_ilp + abs( iinfo )
                 ...
                 ! stdlib_qlarre computes eigenvalues of the (shifted) root representation
                 ! stdlib_qlarrv returns the eigenvalues of the unshifted matrix.
                 ! however, if the eigenvectors are not desired by the user, we need
                 ! to apply the corresponding shifts from stdlib_qlarre to obtain the
                 ! eigenvalues of the original matrix.
                 do j = 1, m
                    itmp = iwork( iindbl+j-1 )
                    w( j ) = w( j ) + e( iwork( iinspl+itmp-1 ) )
              ...
              if ( tryrac ) then
                 ...
                 loop_39: do jblk = 1, iwork( iindbl+m-1 )
                    ...
                    ilast = iwork(iindw+wend-1)
                    rtol2 = four * eps
                    call stdlib_qlarrj( in,work(indd+ibegin-1), work(inde2+ibegin-1),ifirst, &
                    ilast, rtol2, offset, w(wbegin),work( inderr+wbegin-1 ),work( indwrk ), iwork(&
                               iindwk ), pivmin,tnrm, iinfo )
                    ibegin = iend + 1_ilp
                    wbegin = wend + 1_ilp
              ...
              ! if matrix was scaled, then rescale eigenvalues appropriately.
              if( scale/=one ) then
                 call stdlib_qscal( m, one / scale, w, 1_ilp )
              end if
           end if
           ! if eigenvalues are not in increasing order, then sort them,
           ! possibly along with eigenvectors.
           if( nsplit>1_ilp .or. n==2_ilp ) then
              if( .not. wantz ) then
                 call stdlib_qlasrt( 'I', m, w, iinfo )
                 if( iinfo/=0_ilp ) then
                    info = 3_ilp
                    return
                 end if
     ...
     !! with ZUNMTR or ZUPMTR.
        ...
        ! =====================================================================
           ...
           ! stdlib_qstemr needs work of size 6*n, iwork of size 3*n.
           ! in addition, stdlib_qlarre needs work of size 6*n, iwork of size 5*n.
           ! furthermore, stdlib_wlarrv needs work of size 12*n, iwork of size 7*n.
           if( wantz ) then
              lwmin = 18_ilp*n
              liwmin = 10_ilp*n
           else
           ...
           if( n==2_ilp ) then
              ...
              if( .not.wantz ) then
                 ...
                 ! stdlib_qlarre computes the eigenvalues to less than full precision.
                 ! stdlib_wlarrv will refine the eigenvalue approximations, and we only
                 ! need less accurate initial bisection in stdlib_qlarre.
                 ! note: these settings do only affect the subset case and stdlib_qlarre
                 rtol1 = sqrt(eps)
                 rtol2 = max( sqrt(eps)*5.0e-3_qp, four * eps )
              ...
              if( wantz ) then
                 ! compute the desired eigenvectors corresponding to the computed
                 ! eigenvalues
                 call stdlib_wlarrv( n, wl, wu, d, e,pivmin, iwork( iinspl ), m,1_ilp, m, minrgp, &
                 rtol1, rtol2,w, work( inderr ), work( indgp ), iwork( iindbl ),iwork( iindw ), &
                           work( indgrs ), z, ldz,isuppz, work( indwrk ), iwork( iindwk ), iinfo )
                 if( iinfo/=0_ilp ) then
                    info = 20_ilp + abs( iinfo )
                 ...
                 ! stdlib_qlarre computes eigenvalues of the (shifted) root representation
                 ! stdlib_wlarrv returns the eigenvalues of the unshifted matrix.
                 ! however, if the eigenvectors are not desired by the user, we need
                 ! to apply the corresponding shifts from stdlib_qlarre to obtain the
                 ! eigenvalues of the original matrix.
                 do j = 1, m
     ...
     !! tridiagonal form.
        ...
        ! =====================================================================
           ...
           if( lend>l ) then
              ...
              if( m==l+1 ) then
                 if( icompz>0_ilp ) then
                    call stdlib_qlaev2( d( l ), e( l ), d( l+1 ), rt1, rt2, c, s )
                    work( l ) = c
                    work( n-1+l ) = s
                    call stdlib_qlasr( 'R', 'V', 'B', n, 2_ilp, work( l ),work( n-1+l ), z( 1_ilp, l ), &
                              ldz )
                 else
                    call stdlib_qlae2( d( l ), e( l ), d( l+1 ), rt1, rt2 )
                 end if
                 d( l ) = rt1
                 d( l+1 ) = rt2
                 e( l ) = zero
              ...
              if( icompz>0_ilp ) then
                 mm = m - l + 1_ilp
                 call stdlib_qlasr( 'R', 'V', 'B', n, mm, work( l ), work( n-1+l ),z( 1_ilp, l ), ldz &
                           )
              end if
              d( l ) = d( l ) - p
              e( l ) = g
              ...
              if( m==l-1 ) then
                 if( icompz>0_ilp ) then
                    call stdlib_qlaev2( d( l-1 ), e( l-1 ), d( l ), rt1, rt2, c, s )
                    work( m ) = c
                    work( n-1+m ) = s
                    call stdlib_qlasr( 'R', 'V', 'F', n, 2_ilp, work( m ),work( n-1+m ), z( 1_ilp, l-1 ), &
                              ldz )
                 else
                    call stdlib_qlae2( d( l-1 ), e( l-1 ), d( l ), rt1, rt2 )
                 end if
                 d( l-1 ) = rt1
                 d( l ) = rt2
                 e( l-1 ) = zero
              ...
              if( icompz>0_ilp ) then
                 mm = l - m + 1_ilp
                 call stdlib_qlasr( 'R', 'V', 'F', n, mm, work( m ), work( n-1+m ),z( 1_ilp, m ), ldz &
                           )
              end if
              d( l ) = d( l ) - p
              e( lm1 ) = g
           ...
           140 continue
           if( iscale==1_ilp ) then
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, ssfmax, anorm, lendsv-lsv+1, 1_ilp,d( lsv ), n, info )
                        
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, ssfmax, anorm, lendsv-lsv, 1_ilp, e( lsv ),n, info )
                        
           else if( iscale==2_ilp ) then
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, ssfmin, anorm, lendsv-lsv+1, 1_ilp,d( lsv ), n, info )
                        
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, ssfmin, anorm, lendsv-lsv, 1_ilp, e( lsv ),n, info )
     ...
     !! matrix to tridiagonal form.
        ...
        ! =====================================================================
           ...
           if( lend==l )go to 10
           ! scale submatrix in rows and columns l to lend
           anorm = stdlib_qlanst( 'I', lend-l+1, d( l ), e( l ) )
           iscale = 0_ilp
           if( anorm==zero )go to 10
           if( anorm>ssfmax ) then
              iscale = 1_ilp
           ...
           if( lend>l ) then
              ...
              if( m==l+1 ) then
                 if( icompz>0_ilp ) then
                    ...
                    work( l ) = c
                    work( n-1+l ) = s
                    call stdlib_wlasr( 'R', 'V', 'B', n, 2_ilp, work( l ),work( n-1+l ), z( 1_ilp, l ), &
                              ldz )
                 else
                    call stdlib_qlae2( d( l ), e( l ), d( l+1 ), rt1, rt2 )
                 end if
              ...
              if( icompz>0_ilp ) then
                 mm = m - l + 1_ilp
                 call stdlib_wlasr( 'R', 'V', 'B', n, mm, work( l ), work( n-1+l ),z( 1_ilp, l ), ldz &
                           )
              end if
              d( l ) = d( l ) - p
              e( l ) = g
              ...
              if( m==l-1 ) then
                 if( icompz>0_ilp ) then
                    ...
                    work( m ) = c
                    work( n-1+m ) = s
                    call stdlib_wlasr( 'R', 'V', 'F', n, 2_ilp, work( m ),work( n-1+m ), z( 1_ilp, l-1 ), &
                              ldz )
                 else
                    call stdlib_qlae2( d( l-1 ), e( l-1 ), d( l ), rt1, rt2 )
                 end if
              ...
              if( icompz>0_ilp ) then
                 mm = l - m + 1_ilp
                 call stdlib_wlasr( 'R', 'V', 'F', n, mm, work( m ), work( n-1+m ),z( 1_ilp, m ), ldz &
                           )
              end if
              d( l ) = d( l ) - p
              e( lm1 ) = g
