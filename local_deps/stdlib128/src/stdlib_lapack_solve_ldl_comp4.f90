  ...
  contains
     ...
     !! for the solution.
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1, nrhs
              ...
              call stdlib_whpmv( uplo, n, -cone, ap, x( 1_ilp, j ), 1_ilp, cone, work, 1_ilp )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(a)*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ! or vector z.  if the i-th component of the denominator is less
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_whptrs( uplo, n, 1_ilp, afp, ipiv, work, n, info )
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call stdlib_whetrs_rook( uplo, n, 1_ilp, a, lda, ipiv, work, n, info )
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm/=zero )rcond = ( one / ainvnm ) / anorm
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( k<=n-nb ) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call stdlib_wlahef_rook( uplo, n-k+1, nb, kb, a( k, k ), lda,ipiv( k ), work, &
                           ldwork, iinfo )
              else
                 ! use unblocked code to factorize columns k:n of a
                 call stdlib_whetf2_rook( uplo, n-k+1, a( k, k ), lda, ipiv( k ),iinfo )
                 kb = n - k + 1_ilp
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp .and. iinfo>0_ilp )info = iinfo + k - 1_ilp
     ...
     !! A11 (if UPLO = 'U') or A22 (if UPLO = 'L').
        ...
        ! =====================================================================
           ...
           integer(ilp) :: imax, itemp, ii, j, jb, jj, jmax, jp1, jp2, k, kk, kkw, kp, kstep, kw, &
                     p
           real(qp) :: absakk, alpha, colmax, dtemp, r1, rowmax, t, sfmin
           ...
           if( stdlib_lsame( uplo, 'U' ) ) then
              ...
              p = k
              ! copy column k of a to column kw of w and update it
              if( k>1_ilp )call stdlib_wcopy( k-1, a( 1_ilp, k ), 1_ilp, w( 1_ilp, kw ), 1_ilp )
              w( k, kw ) = real( a( k, k ),KIND=qp)
              if( k<n ) then
                 call stdlib_wgemv( 'NO TRANSPOSE', k, n-k, -cone, a( 1_ilp, k+1 ), lda,w( k, kw+1 ), &
                           ldw, cone, w( 1_ilp, kw ), 1_ilp )
              ...
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( k>1_ilp )call stdlib_wcopy( k-1, w( 1_ilp, kw ), 1_ilp, a( 1_ilp, k ), 1_ilp )
              else
                 ! ============================================================
                 ! begin pivot search
                 ! case(1)
                 ...
                 if( .not.( absakk<alpha*colmax ) ) then
                    ...
                    12 continue
                       ! begin pivot search loop body
                       ! copy column imax to column kw-1 of w and update it
                       if( imax>1_ilp )call stdlib_wcopy( imax-1, a( 1_ilp, imax ), 1_ilp, w( 1_ilp, kw-1 ),1_ilp )
                                 
                       w( imax, kw-1 ) = real( a( imax, imax ),KIND=qp)
                       call stdlib_wcopy( k-imax, a( imax, imax+1 ), lda,w( imax+1, kw-1 ), 1_ilp )
                                 
                       call stdlib_wlacgv( k-imax, w( imax+1, kw-1 ), 1_ilp )
                       if( k<n ) then
                          call stdlib_wgemv( 'NO TRANSPOSE', k, n-k, -cone,a( 1_ilp, k+1 ), lda, w( &
                                    imax, kw+1 ), ldw,cone, w( 1_ilp, kw-1 ), 1_ilp )
                       ...
                       if( imax>1_ilp ) then
                          itemp = stdlib_iwamax( imax-1, w( 1_ilp, kw-1 ), 1_ilp )
                          dtemp = cabs1( w( itemp, kw-1 ) )
                          if( dtemp>rowmax ) then
                             rowmax = dtemp
                             jmax = itemp
                 ...
                 if( ( kstep==2_ilp ) .and. ( p/=k ) ) then
                    ...
                    call stdlib_wcopy( k-1-p, a( p+1, k ), 1_ilp, a( p, p+1 ),lda )
                    call stdlib_wlacgv( k-1-p, a( p, p+1 ), lda )
                    if( p>1_ilp )call stdlib_wcopy( p-1, a( 1_ilp, k ), 1_ilp, a( 1_ilp, p ), 1_ilp )
                    ! interchange rows k and p in the last k+1 to n columns of a
                    ! (columns k and k-1 of a for 2-by-2 pivot will be
                    ! later overwritten). interchange rows k and p
                    ! in last kkw to nb columns of w.
                    if( k<n )call stdlib_wswap( n-k, a( k, k+1 ), lda, a( p, k+1 ),lda )
                    call stdlib_wswap( n-kk+1, w( k, kkw ), ldw, w( p, kkw ),ldw )
                 end if
                 ! interchange rows and columns kp and kk.
                 ! updated column kp is already stored in column kkw of w.
                 if( kp/=kk ) then
                    ...
                    call stdlib_wlacgv( kk-1-kp, a( kp, kp+1 ), lda )
                 ...
                 if( kstep==1_ilp ) then
                    ...
                    if( k>1_ilp ) then
                       ...
                       if( abs( t )>=sfmin ) then
                          r1 = one / t
                          call stdlib_wdscal( k-1, r1, a( 1_ilp, k ), 1_ilp )
                       else
                          do ii = 1, k-1
                             a( ii, k ) = a( ii, k ) / t
                          end do
                       ...
                       ! (2) conjugate column w(kw)
                       call stdlib_wlacgv( k-1, w( 1_ilp, kw ), 1_ilp )
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns kw and kw-1 of w now hold
                    ! ( w(kw-1) w(kw) ) = ( u(k-1) u(k) )*d(k)
                    ...
                    ! (2) conjugate columns w(kw) and w(kw-1)
                    call stdlib_wlacgv( k-1, w( 1_ilp, kw ), 1_ilp )
                    call stdlib_wlacgv( k-2, w( 1_ilp, kw-1 ), 1_ilp )
                 end if
              end if
              ! store details of the interchanges in ipiv
              if( kstep==1_ilp ) then
              ...
              do j = ( ( k-1 ) / nb )*nb + 1, 1, -nb
                 ...
                 ! update the rectangular superdiagonal block
                 if( j>=2_ilp )call stdlib_wgemm( 'NO TRANSPOSE', 'TRANSPOSE', j-1, jb, n-k,-cone, a( &
                           1_ilp, k+1 ), lda, w( j, kw+1 ), ldw,cone, a( 1_ilp, j ), lda )
              end do
              ! put u12 in standard form by partially undoing the interchanges
              ! in of rows in columns k+1:n looping backwards from k+1 to n
              ...
              60 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j + 1_ilp
                 if( jp2/=jj .and. j<=n )call stdlib_wswap( n-j+1, a( jp2, j ), lda, a( jj, j ), &
                           lda )
                 jj = jj + 1_ilp
                 if( kstep==2_ilp .and. jp1/=jj .and. j<=n )call stdlib_wswap( n-j+1, a( jp1, j ), &
                           lda, a( jj, j ), lda )
              if( j<n )go to 60
              ! set kb to the number of columns factorized
              kb = n - k
              ...
              if( k<n )call stdlib_wcopy( n-k, a( k+1, k ), 1_ilp, w( k+1, k ), 1_ilp )
              if( k>1_ilp ) then
                 call stdlib_wgemv( 'NO TRANSPOSE', n-k+1, k-1, -cone, a( k, 1_ilp ),lda, w( k, 1_ilp ), &
                           ldw, cone, w( k, k ), 1_ilp )
              ...
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( k<n )call stdlib_wcopy( n-k, w( k+1, k ), 1_ilp, a( k+1, k ), 1_ilp )
              else
                 ! ============================================================
                 ! begin pivot search
                 ! case(1)
                 ...
                 if( .not.( absakk<alpha*colmax ) ) then
                    ...
                    72 continue
                       ...
                       call stdlib_wlacgv( imax-k, w( k, k+1 ), 1_ilp )
                       w( imax, k+1 ) = real( a( imax, imax ),KIND=qp)
                       if( imax<n )call stdlib_wcopy( n-imax, a( imax+1, imax ), 1_ilp,w( imax+1, k+1 &
                                 ), 1_ilp )
                       if( k>1_ilp ) then
                          call stdlib_wgemv( 'NO TRANSPOSE', n-k+1, k-1, -cone,a( k, 1_ilp ), lda, w( &
                                    imax, 1_ilp ), ldw,cone, w( k, k+1 ), 1_ilp )
                       ...
                       if( imax<n ) then
                          itemp = imax + stdlib_iwamax( n-imax, w( imax+1, k+1 ), 1_ilp)
                          dtemp = cabs1( w( itemp, k+1 ) )
                          if( dtemp>rowmax ) then
                             rowmax = dtemp
                             jmax = itemp
                 ...
                 if( ( kstep==2_ilp ) .and. ( p/=k ) ) then
                    ...
                    ! will be later overwritten.
                    a( p, p ) = real( a( k, k ),KIND=qp)
                    call stdlib_wcopy( p-k-1, a( k+1, k ), 1_ilp, a( p, k+1 ), lda )
                    call stdlib_wlacgv( p-k-1, a( p, k+1 ), lda )
                    if( p<n )call stdlib_wcopy( n-p, a( p+1, k ), 1_ilp, a( p+1, p ), 1_ilp )
                    ! interchange rows k and p in first k-1 columns of a
                    ! (columns k and k+1 of a for 2-by-2 pivot will be
                    ! later overwritten). interchange rows k and p
                    ! in first kk columns of w.
                    if( k>1_ilp )call stdlib_wswap( k-1, a( k, 1_ilp ), lda, a( p, 1_ilp ), lda )
                    call stdlib_wswap( kk, w( k, 1_ilp ), ldw, w( p, 1_ilp ), ldw )
                 end if
                 ! interchange rows and columns kp and kk.
                 ! updated column kp is already stored in column kk of w.
                 if( kp/=kk ) then
                 ...
                 if( kstep==1_ilp ) then
                    ...
                    if( k<n ) then
                       ...
                       if( abs( t )>=sfmin ) then
                          r1 = one / t
                          call stdlib_wdscal( n-k, r1, a( k+1, k ), 1_ilp )
                       else
                          do ii = k + 1, n
                             a( ii, k ) = a( ii, k ) / t
                          end do
                       ...
                       ! (2) conjugate column w(k)
                       call stdlib_wlacgv( n-k, w( k+1, k ), 1_ilp )
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns k and k+1 of w now hold
                    ! ( w(k) w(k+1) ) = ( l(k) l(k+1) )*d(k)
                    ...
                    ! (2) conjugate columns w(k) and w(k+1)
                    call stdlib_wlacgv( n-k, w( k+1, k ), 1_ilp )
                    call stdlib_wlacgv( n-k-1, w( k+2, k+1 ), 1_ilp )
                 end if
              end if
              ! store details of the interchanges in ipiv
              if( kstep==1_ilp ) then
              ...
              120 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j - 1_ilp
                 if( jp2/=jj .and. j>=1_ilp )call stdlib_wswap( j, a( jp2, 1_ilp ), lda, a( jj, 1_ilp ), lda )
                           
                 jj = jj -1_ilp
                 if( kstep==2_ilp .and. jp1/=jj .and. j>=1_ilp )call stdlib_wswap( j, a( jp1, 1_ilp ), lda, a(&
                            jj, 1_ilp ), lda )
              if( j>1 )go to 120
              ! set kb to the number of columns factorized
              kb = k - 1_ilp
              ...
              if( ( max( absakk, colmax )==zero ) ) then
                 ...
                 if( .not.( absakk<alpha*colmax ) ) then
                    ...
                    12 continue
                       ...
                       if( imax>1_ilp ) then
                          itemp = stdlib_iwamax( imax-1, a( 1_ilp, imax ), 1_ilp )
                          dtemp = cabs1( a( itemp, imax ) )
                          if( dtemp>rowmax ) then
                             rowmax = dtemp
                             jmax = itemp
                 ...
                 if( ( kstep==2_ilp ) .and. ( p/=k ) ) then
                    ! (1) swap columnar parts
                    if( p>1_ilp )call stdlib_wswap( p-1, a( 1_ilp, k ), 1_ilp, a( 1_ilp, p ), 1_ilp )
                    ! (2) swap and conjugate middle parts
                    do j = p + 1, k - 1
                       t = conjg( a( j, k ) )
                       a( j, k ) = conjg( a( p, j ) )
                 ...
                 if( kp/=kk ) then
                    ! (1) swap columnar parts
                    if( kp>1_ilp )call stdlib_wswap( kp-1, a( 1_ilp, kk ), 1_ilp, a( 1_ilp, kp ), 1_ilp )
                    ! (2) swap and conjugate middle parts
                    do j = kp + 1, kk - 1
                       t = conjg( a( j, kk ) )
                       a( j, kk ) = conjg( a( kp, j ) )
                    ...
                    ! (*) make sure that diagonal element of pivot is real
                    a( k, k ) = real( a( k, k ),KIND=qp)
                    if( kstep==2_ilp )a( k-1, k-1 ) = real( a( k-1, k-1 ),KIND=qp)
                 end if
                 ! update the leading submatrix
                 if( kstep==1_ilp ) then
                    ! 1-by-1 pivot block d(k): column k now holds
                    ...
                    if( k>1_ilp ) then
                       ...
                       if( abs( real( a( k, k ),KIND=qp) )>=sfmin ) then
                          ...
                          call stdlib_wher( uplo, k-1, -d11, a( 1_ilp, k ), 1_ilp, a, lda )
                          ! store u(k) in column k
                          call stdlib_wdscal( k-1, d11, a( 1_ilp, k ), 1_ilp )
                       else
                          ! store l(k) in column k
              ...
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( .not.( absakk<alpha*colmax ) ) then
                    ...
                    42 continue
                       ...
                       if( imax<n ) then
                          itemp = imax + stdlib_iwamax( n-imax, a( imax+1, imax ),1_ilp )
                          dtemp = cabs1( a( itemp, imax ) )
                          if( dtemp>rowmax ) then
                             rowmax = dtemp
                             jmax = itemp
                 ...
                 if( ( kstep==2_ilp ) .and. ( p/=k ) ) then
                    ! (1) swap columnar parts
                    if( p<n )call stdlib_wswap( n-p, a( p+1, k ), 1_ilp, a( p+1, p ), 1_ilp )
                    ! (2) swap and conjugate middle parts
                    do j = k + 1, p - 1
                       t = conjg( a( j, k ) )
                       a( j, k ) = conjg( a( p, j ) )
                    ...
                    if( k<n ) then
                       ...
                       if( abs( real( a( k, k ),KIND=qp) )>=sfmin ) then
                          ...
                          ! a := a - l(k)*d(k)*l(k)**t
                             ! = a - w(k)*(1/d(k))*w(k)**t
                          d11 = one / real( a( k, k ),KIND=qp)
                          call stdlib_wher( uplo, n-k, -d11, a( k+1, k ), 1_ilp,a( k+1, k+1 ), lda )
                                    
                          ! store l(k) in column k
                          call stdlib_wdscal( n-k, d11, a( k+1, k ), 1_ilp )
                       else
                          ! store l(k) in column k
                          d11 = real( a( k, k ),KIND=qp)
                          do ii = k + 1, n
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 call stdlib_wdscal( nrhs, s, b( k, 1_ilp ), ldb )
                 k = k - 1_ilp
              else
                 ! 2 x 2 diagonal block
                 ! interchange rows k and -ipiv(k), then k-1 and -ipiv(k-1)
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! stored in column k of a.
                 if( k>1_ilp ) then
                    call stdlib_wlacgv( nrhs, b( k, 1_ilp ), ldb )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', k-1, nrhs, -cone, b,ldb, a( 1_ilp, k ), &
                              1_ilp, cone, b( k, 1_ilp ), ldb )
                    call stdlib_wlacgv( nrhs, b( k, 1_ilp ), ldb )
                 end if
                 ! interchange rows k and ipiv(k).
                 ...
                 if( k>1_ilp ) then
                    ...
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', k-1, nrhs, -cone, b,ldb, a( 1_ilp, k ), &
                              1_ilp, cone, b( k, 1_ilp ), ldb )
                    call stdlib_wlacgv( nrhs, b( k, 1_ilp ), ldb )
                    call stdlib_wlacgv( nrhs, b( k+1, 1_ilp ), ldb )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', k-1, nrhs, -cone, b,ldb, a( 1_ilp, k+1 )&
                              , 1_ilp, cone, b( k+1, 1_ilp ), ldb )
                    call stdlib_wlacgv( nrhs, b( k+1, 1_ilp ), ldb )
                 end if
                 ! interchange rows k and -ipiv(k), then k+1 and -ipiv(k+1)
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 if( k<n ) then
                    call stdlib_wlacgv( nrhs, b( k, 1_ilp ), ldb )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', n-k, nrhs, -cone,b( k+1, 1_ilp ), ldb, &
                              a( k+1, k ), 1_ilp, cone,b( k, 1_ilp ), ldb )
                    call stdlib_wlacgv( nrhs, b( k, 1_ilp ), ldb )
                 end if
                 ! interchange rows k and ipiv(k).
           ...
           if( upper ) then
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 if( k>1_ilp ) then
                    ...
                    call stdlib_whemv( uplo, k-1, -cone, a, lda, work, 1_ilp, czero,a( 1_ilp, k ), 1_ilp )
                              
                    a( k, k ) = a( k, k ) - real( stdlib_wdotc( k-1, work, 1_ilp, a( 1_ilp,k ), 1_ilp ),&
                              KIND=qp)
                 end if
                 kstep = 1_ilp
              else
                 ...
                 if( k>1_ilp ) then
                    ...
                    a( k, k ) = a( k, k ) - real( stdlib_wdotc( k-1, work, 1_ilp, a( 1_ilp,k ), 1_ilp ),&
                              KIND=qp)
                    a( k, k+1 ) = a( k, k+1 ) -stdlib_wdotc( k-1, a( 1_ilp, k ), 1_ilp, a( 1_ilp, k+1 ), 1_ilp )
                              
                    call stdlib_wcopy( k-1, a( 1_ilp, k+1 ), 1_ilp, work, 1_ilp )
                    call stdlib_whemv( uplo, k-1, -cone, a, lda, work, 1_ilp, czero,a( 1_ilp, k+1 ), 1_ilp )
                              
                    a( k+1, k+1 ) = a( k+1, k+1 ) -real( stdlib_wdotc( k-1, work, 1_ilp, a( 1_ilp, k+1 ),&
                              1_ilp ),KIND=qp)
                 end if
                 kstep = 2_ilp
              end if
              if( kstep==1_ilp ) then
                 ...
                 kp = ipiv( k )
                 if( kp/=k ) then
                    if( kp>1_ilp )call stdlib_wswap( kp-1, a( 1_ilp, k ), 1_ilp, a( 1_ilp, kp ), 1_ilp )
                    do j = kp + 1, k - 1
                       temp = conjg( a( j, k ) )
                       a( j, k ) = conjg( a( kp, j ) )
                       a( kp, j ) = temp
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 if( k<n ) then
                    ...
                    call stdlib_whemv( uplo, n-k, -cone, a( k+1, k+1 ), lda, work,1_ilp, czero, a( k+&
                              1_ilp, k ), 1_ilp )
                    a( k, k ) = a( k, k ) - real( stdlib_wdotc( n-k, work, 1_ilp,a( k+1, k ), 1_ilp ),&
                              KIND=qp)
                 end if
                 kstep = 1_ilp
              else
                 ...
                 if( k<n ) then
                    ...
                    a( k, k ) = a( k, k ) - real( stdlib_wdotc( n-k, work, 1_ilp,a( k+1, k ), 1_ilp ),&
                              KIND=qp)
                    a( k, k-1 ) = a( k, k-1 ) -stdlib_wdotc( n-k, a( k+1, k ), 1_ilp, a( k+1, k-1 ),1_ilp &
                              )
                    call stdlib_wcopy( n-k, a( k+1, k-1 ), 1_ilp, work, 1_ilp )
                    call stdlib_whemv( uplo, n-k, -cone, a( k+1, k+1 ), lda, work,1_ilp, czero, a( k+&
                              1_ilp, k-1 ), 1_ilp )
                    a( k-1, k-1 ) = a( k-1, k-1 ) -real( stdlib_wdotc( n-k, work, 1_ilp, a( k+1, k-1 )&
                              ,1_ilp ),KIND=qp)
                 end if
                 kstep = 2_ilp
              end if
           ...
           if( upper ) then
              ...
              if( k<n ) then
                 do i = k, ( k - kb + 1 ), -1
                    ip = abs( ipiv( i ) )
                    if( ip/=i ) then
                       call stdlib_wswap( n-k, a( i, k+1 ), lda,a( ip, k+1 ), lda )
                    end if
                 end do
              end if
              ! decrease k and return to the start of the main loop
              ...
              if( k<=n-nb ) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call stdlib_wlahef_rk( uplo, n-k+1, nb, kb, a( k, k ), lda, e( k ),ipiv( k ), &
                           work, ldwork, iinfo )
              else
                 ! use unblocked code to factorize columns k:n of a
              ...
              if( k>1_ilp ) then
                 do i = k, ( k + kb - 1 ), 1
                    ip = abs( ipiv( i ) )
                    if( ip/=i ) then
                       call stdlib_wswap( k-1, a( i, 1_ilp ), lda,a( ip, 1_ilp ), lda )
                    end if
                 end do
              end if
              ! increase k and return to the start of the main loop
     ...
     !! This is the blocked version of the algorithm, calling Level 3 BLAS.
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              ! .....................................................
              ! copy first row a(1, 1:n) into h(1:n) (stored in work(1:n))
              call stdlib_wcopy( n, a( 1_ilp, 1_ilp ), lda, work( 1_ilp ), 1_ilp )
              ! j is the main loop index, increasing from 1 to n in steps of
              ...
              k1 = max(1_ilp, j)-j
              ! panel factorization
              call stdlib_wlahef_aa( uplo, 2_ilp-k1, n-j, jb,a( max(1_ilp, j), j+1 ), lda,ipiv( j+1 ), &
                        work, n, work( n*nb+1 ) )
              ! adjust ipiv and apply it back (j-th step picks (j+1)-th pivot)
              do j2 = j+2, min(n, j+jb+1)
                 ipiv( j2 ) = ipiv( j2 ) + j
                 if( (j2/=ipiv(j2)) .and. ((j1-k1)>2_ilp) ) then
                    call stdlib_wswap( j1-k1-2, a( 1_ilp, j2 ), 1_ilp,a( 1_ilp, ipiv(j2) ), 1_ilp )
                 end if
              end do
              j = j + jb
              ! trailing submatrix update, where
              ...
              if( j<n ) then
                ! if the first panel and jb=1 (nb=1), then nothing to do
                 if( j1>1_ilp .or. jb>1_ilp ) then
                    ...
                    alpha = conjg( a( j, j+1 ) )
                    a( j, j+1 ) = cone
                    call stdlib_wcopy( n-j, a( j-1, j+1 ), lda,work( (j+1-j1+1)+jb*n ), 1_ilp )
                              
                    call stdlib_wscal( n-j, alpha, work( (j+1-j1+1)+jb*n ), 1_ilp )
                    ! k1 identifies if the previous column of the panel has been
                     ! explicitly stored, e.g., k1=0 and k2=1 for the first panel,
                     ! and k1=1 and k2=0 for the rest
                    if( j1>1_ilp ) then
                    ...
                    do j2 = j+1, n, nb
                       nj = min( nb, n-j2+1 )
                       ! update (j2, j2) diagonal block with stdlib_wgemv
                       j3 = j2
                       do mj = nj-1, 1, -1
                          call stdlib_wgemm( 'CONJUGATE TRANSPOSE', 'TRANSPOSE',1_ilp, mj, jb+1,-cone,&
                           a( j1-k2, j3 ), lda,work( (j3-j1+1)+k1*n ), n,cone, a( j3, j3 ), lda )
                                     
                          j3 = j3 + 1_ilp
                       end do
                       ! update off-diagonal block of j2-th block row with stdlib_wgemm
                       call stdlib_wgemm( 'CONJUGATE TRANSPOSE', 'TRANSPOSE',nj, n-j3+1, jb+1,-&
                       cone, a( j1-k2, j2 ), lda,work( (j3-j1+1)+k1*n ), n,cone, a( j2, j3 ), lda &
                                 )
                    end do
                    ! recover t( j, j+1 )
                 ...
                 ! work(j+1, 1) stores h(j+1, 1)
                 call stdlib_wcopy( n-j, a( j+1, j+1 ), lda, work( 1_ilp ), 1_ilp )
              end if
              go to 10
           else
              ! .....................................................
              ...
              ! copy first column a(1:n, 1) into h(1:n, 1)
               ! (stored in work(1:n))
              call stdlib_wcopy( n, a( 1_ilp, 1_ilp ), 1_ilp, work( 1_ilp ), 1_ilp )
              ! j is the main loop index, increasing from 1 to n in steps of
              ! jb, where jb is the number of columns factorized by stdlib_wlahef;
              ! jb is either nb, or n-j+1 for the last block
              j = 0_ilp
              ...
              k1 = max(1_ilp, j)-j
              ! panel factorization
              call stdlib_wlahef_aa( uplo, 2_ilp-k1, n-j, jb,a( j+1, max(1_ilp, j) ), lda,ipiv( j+1 ), &
                        work, n, work( n*nb+1 ) )
              ! adjust ipiv and apply it back (j-th step picks (j+1)-th pivot)
              do j2 = j+2, min(n, j+jb+1)
                 ipiv( j2 ) = ipiv( j2 ) + j
                 if( (j2/=ipiv(j2)) .and. ((j1-k1)>2_ilp) ) then
                    call stdlib_wswap( j1-k1-2, a( j2, 1_ilp ), lda,a( ipiv(j2), 1_ilp ), lda )
                 end if
              end do
              j = j + jb
              ! trailing submatrix update, where
              ...
              if( j<n ) then
                ! if the first panel and jb=1 (nb=1), then nothing to do
                 if( j1>1_ilp .or. jb>1_ilp ) then
                    ...
                    alpha = conjg( a( j+1, j ) )
                    a( j+1, j ) = cone
                    call stdlib_wcopy( n-j, a( j+1, j-1 ), 1_ilp,work( (j+1-j1+1)+jb*n ), 1_ilp )
                    call stdlib_wscal( n-j, alpha, work( (j+1-j1+1)+jb*n ), 1_ilp )
                    ! k1 identifies if the previous column of the panel has been
                     ! explicitly stored, e.g., k1=0 and k2=1 for the first panel,
                     ! and k1=1 and k2=0 for the rest
                    ...
                    do j2 = j+1, n, nb
                       ...
                       j3 = j2
                       do mj = nj-1, 1, -1
                          call stdlib_wgemm( 'NO TRANSPOSE', 'CONJUGATE TRANSPOSE',mj, 1_ilp, jb+1,-&
                          cone, work( (j3-j1+1)+k1*n ), n,a( j3, j1-k2 ), lda,cone, a( j3, j3 ), &
                                    lda )
                          j3 = j3 + 1_ilp
                       end do
                       ! update off-diagonal block of j2-th block column with stdlib_wgemm
                       call stdlib_wgemm( 'NO TRANSPOSE', 'CONJUGATE TRANSPOSE',n-j3+1, nj, jb+1,-&
                       cone, work( (j3-j1+1)+k1*n ), n,a( j2, j1-k2 ), lda,cone, a( j3, j2 ), lda &
                                 )
                    end do
                    ! recover t( j+1, j )
                 ...
                 ! work(j+1, 1) stores h(j+1, 1)
                 call stdlib_wcopy( n-j, a( j+1, j+1 ), 1_ilp, work( 1_ilp ), 1_ilp )
              end if
              go to 11
           end if
           20 continue
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( uplo, 'U' ) ) then
              ...
              ! k is the column to be factorized
               ...
               ! > for the rest of the columns, k is j+1, skipping only the
                 ! first column
                 call stdlib_wlacgv( j-k1, a( 1_ilp, j ), 1_ilp )
                 call stdlib_wgemv( 'NO TRANSPOSE', mj, j-k1,-cone, h( j, k1 ), ldh,a( 1_ilp, j ), 1_ilp,&
                           cone, h( j, j ), 1_ilp )
                 call stdlib_wlacgv( j-k1, a( 1_ilp, j ), 1_ilp )
              end if
              ! copy h(i:n, i) into work
              call stdlib_wcopy( mj, h( j, j ), 1_ilp, work( 1_ilp ), 1_ilp )
              if( j>k1 ) then
                 ! compute work := work - l(j-1, j:n) * t(j-1,j),
                  ! where a(j-1, j) stores t(j-1, j) and a(j-2, j:n) stores u(j-1, j:n)
                 alpha = -conjg( a( k-1, j ) )
                 call stdlib_waxpy( mj, alpha, a( k-2, j ), lda, work( 1_ilp ), 1_ilp )
              end if
              ! set a(j, j) = t(j, j)
              a( k, j ) = real( work( 1_ilp ),KIND=qp)
              if( j<m ) then
                 ! compute work(2:n) = t(j, j) l(j, (j+1):n)
                  ! where a(j, j) stores t(j, j) and a(j-1, (j+1):n) stores u(j, (j+1):n)
                 if( k>1_ilp ) then
                    alpha = -a( k, j )
                    call stdlib_waxpy( m-j, alpha, a( k-1, j+1 ), lda,work( 2_ilp ), 1_ilp )
                 endif
                 ! find max(|work(2:n)|)
                 i2 = stdlib_iwamax( m-j, work( 2_ilp ), 1_ilp ) + 1_ilp
                 piv = work( i2 )
                 ! apply hermitian pivot
                 if( (i2/=2_ilp) .and. (piv/=0_ilp) ) then
                    ! swap work(i1) and work(i2)
                    ...
                    i1 = i1+j-1
                    i2 = i2+j-1
                    call stdlib_wswap( i2-i1-1, a( j1+i1-1, i1+1 ), lda,a( j1+i1, i2 ), 1_ilp )
                              
                    call stdlib_wlacgv( i2-i1, a( j1+i1-1, i1+1 ), lda )
                    call stdlib_wlacgv( i2-i1-1, a( j1+i1, i2 ), 1_ilp )
                    ! swap a(i1, i2+1:n) with a(i2, i2+1:n)
                    ...
                    a( j1+i2-1, i2 ) = piv
                    ! swap h(i1, 1:j1) with h(i2, 1:j1)
                    call stdlib_wswap( i1-1, h( i1, 1_ilp ), ldh, h( i2, 1_ilp ), ldh )
                    ipiv( i1 ) = i2
                    if( i1>(k1-1) ) then
                       ! swap l(1:i1-1, i1) with l(1:i1-1, i2),
                        ! skipping the first column
                       call stdlib_wswap( i1-k1+1, a( 1_ilp, i1 ), 1_ilp,a( 1_ilp, i2 ), 1_ilp )
                    end if
                 else
                    ipiv( j+1 ) = j+1
                 endif
                 ...
                 if( j<nb ) then
                    ! copy a(j+1:n, j+1) into h(j:n, j),
                    call stdlib_wcopy( m-j, a( k+1, j+1 ), lda,h( j+1, j+1 ), 1_ilp )
                 end if
                 ! compute l(j+2, j+1) = work( 3:n ) / t(j, j+1),
                  ! where a(j, j+1) = t(j, j+1) and a(j+2:n, j) = l(j+2:n, j+1)
                 if( j<(m-1) ) then
                    if( a( k, j+1 )/=czero ) then
                       alpha = cone / a( k, j+1 )
                       call stdlib_wcopy( m-j-1, work( 3_ilp ), 1_ilp, a( k, j+2 ), lda )
                       call stdlib_wscal( m-j-1, alpha, a( k, j+2 ), lda )
                    else
                       call stdlib_wlaset( 'FULL', 1_ilp, m-j-1, czero, czero,a( k, j+2 ), lda)
              ...
              ! k is the column to be factorized
               ...
               ! > for the rest of the columns, k is j+1, skipping only the
                 ! first column
                 call stdlib_wlacgv( j-k1, a( j, 1_ilp ), lda )
                 call stdlib_wgemv( 'NO TRANSPOSE', mj, j-k1,-cone, h( j, k1 ), ldh,a( j, 1_ilp ), &
                           lda,cone, h( j, j ), 1_ilp )
                 call stdlib_wlacgv( j-k1, a( j, 1_ilp ), lda )
              end if
              ! copy h(j:n, j) into work
              ...
              if( j>k1 ) then
                 ! compute work := work - l(j:n, j-1) * t(j-1,j),
                  ! where a(j-1, j) = t(j-1, j) and a(j, j-2) = l(j, j-1)
                 alpha = -conjg( a( j, k-1 ) )
                 call stdlib_waxpy( mj, alpha, a( j, k-2 ), 1_ilp, work( 1_ilp ), 1_ilp )
              end if
              ! set a(j, j) = t(j, j)
              a( j, k ) = real( work( 1_ilp ),KIND=qp)
              if( j<m ) then
                 ! compute work(2:n) = t(j, j) l((j+1):n, j)
                  ! where a(j, j) = t(j, j) and a((j+1):n, j-1) = l((j+1):n, j)
                 if( k>1_ilp ) then
                    alpha = -a( j, k )
                    call stdlib_waxpy( m-j, alpha, a( j+1, k-1 ), 1_ilp,work( 2_ilp ), 1_ilp )
                 endif
                 ! find max(|work(2:n)|)
                 i2 = stdlib_iwamax( m-j, work( 2_ilp ), 1_ilp ) + 1_ilp
                 piv = work( i2 )
                 ...
                 if( (i2/=2_ilp) .and. (piv/=0_ilp) ) then
                    ...
                    i1 = i1+j-1
                    i2 = i2+j-1
                    call stdlib_wswap( i2-i1-1, a( i1+1, j1+i1-1 ), 1_ilp,a( i2, j1+i1 ), lda )
                              
                    call stdlib_wlacgv( i2-i1, a( i1+1, j1+i1-1 ), 1_ilp )
                    call stdlib_wlacgv( i2-i1-1, a( i2, j1+i1 ), lda )
                    ! swap a(i2+1:n, i1) with a(i2+1:n, i2)
                    if( i2<m )call stdlib_wswap( m-i2, a( i2+1, j1+i1-1 ), 1_ilp,a( i2+1, j1+i2-1 ), &
                              1_ilp )
                    ! swap a(i1, i1) with a(i2, i2)
                    piv = a( i1, j1+i1-1 )
                    a( i1, j1+i1-1 ) = a( i2, j1+i2-1 )
                    ...
                    if( i1>(k1-1) ) then
                       ! swap l(1:i1-1, i1) with l(1:i1-1, i2),
                        ! skipping the first column
                       call stdlib_wswap( i1-k1+1, a( i1, 1_ilp ), lda,a( i2, 1_ilp ), lda )
                    end if
                 else
                    ipiv( j+1 ) = j+1
                 endif
                 ...
                 if( j<nb ) then
                    ! copy a(j+1:n, j+1) into h(j+1:n, j),
                    call stdlib_wcopy( m-j, a( j+1, k+1 ), 1_ilp,h( j+1, j+1 ), 1_ilp )
                 end if
                 ! compute l(j+2, j+1) = work( 3:n ) / t(j, j+1),
                  ! where a(j, j+1) = t(j, j+1) and a(j+2:n, j) = l(j+2:n, j+1)
                 if( j<(m-1) ) then
                    if( a( j+1, k )/=czero ) then
                       alpha = cone / a( j+1, k )
                       call stdlib_wcopy( m-j-1, work( 3_ilp ), 1_ilp, a( j+2, k ), 1_ilp )
                       call stdlib_wscal( m-j-1, alpha, a( j+2, k ), 1_ilp )
                    else
                       call stdlib_wlaset( 'FULL', m-j-1, 1_ilp, czero, czero,a( j+2, k ), lda )
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( n>1_ilp ) then
                 ...
                 ! compute u**h \ b -> b    [ (u**h \p**t * b) ]
                 call stdlib_wtrsm( 'L', 'U', 'C', 'U', n-1, nrhs, cone, a( 1_ilp, 2_ilp ),lda, b( 2_ilp, 1_ilp ),&
                            ldb )
              end if
              ! 2) solve with triangular matrix t
              ! compute t \ b -> b   [ t \ (u**h \p**t * b) ]
              call stdlib_wlacpy( 'F', 1_ilp, n, a(1_ilp, 1_ilp), lda+1, work(n), 1_ilp )
              if( n>1_ilp ) then
                  call stdlib_wlacpy( 'F', 1_ilp, n-1, a( 1_ilp, 2_ilp ), lda+1, work( 2_ilp*n ), 1_ilp)
                  call stdlib_wlacpy( 'F', 1_ilp, n-1, a( 1_ilp, 2_ilp ), lda+1, work( 1_ilp ), 1_ilp )
                  call stdlib_wlacgv( n-1, work( 1_ilp ), 1_ilp )
              end if
              call stdlib_wgtsv( n, nrhs, work(1_ilp), work(n), work(2_ilp*n), b, ldb,info )
              ! 3) backward substitution with u
              if( n>1_ilp ) then
                 ! compute u \ b -> b   [ u \ (t \ (u**h \p**t * b) ) ]
                 call stdlib_wtrsm( 'L', 'U', 'N', 'U', n-1, nrhs, cone, a( 1_ilp, 2_ilp ),lda, b(2_ilp, 1_ilp), &
                           ldb)
                 ! pivot, p * b  [ p * (u**h \ (t \ (u \p**t * b) )) ]
                 do k = n, 1, -1
                    kp = ipiv( k )
              ...
              if( n>1_ilp ) then
                 ...
                 ! compute l \ b -> b    [ (l \p**t * b) ]
                 call stdlib_wtrsm( 'L', 'L', 'N', 'U', n-1, nrhs, cone, a( 2_ilp, 1_ilp ),lda, b(2_ilp, 1_ilp), &
                           ldb)
              end if
              ! 2) solve with triangular matrix t
              ! compute t \ b -> b   [ t \ (l \p**t * b) ]
              call stdlib_wlacpy( 'F', 1_ilp, n, a(1_ilp, 1_ilp), lda+1, work(n), 1_ilp)
              if( n>1_ilp ) then
                  call stdlib_wlacpy( 'F', 1_ilp, n-1, a( 2_ilp, 1_ilp ), lda+1, work( 1_ilp ), 1_ilp)
                  call stdlib_wlacpy( 'F', 1_ilp, n-1, a( 2_ilp, 1_ilp ), lda+1, work( 2_ilp*n ), 1_ilp)
                  call stdlib_wlacgv( n-1, work( 2_ilp*n ), 1_ilp )
              end if
              call stdlib_wgtsv(n, nrhs, work(1_ilp), work(n), work(2_ilp*n), b, ldb,info)
              ! 3) backward substitution with l**h
              if( n>1_ilp ) then
                 ! compute l**h \ b -> b   [ l**h \ (t \ (l \p**t * b) ) ]
                 call stdlib_wtrsm( 'L', 'L', 'C', 'U', n-1, nrhs, cone, a( 2_ilp, 1_ilp ),lda, b( 2_ilp, 1_ilp ),&
                            ldb)
                 ! pivot, p * b  [ p * (l**h \ (t \ (l \p**t * b) )) ]
                 do k = n, 1, -1
                    kp = ipiv( k )
