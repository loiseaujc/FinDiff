  ...
  contains
     ...
     !! and optionally, the singular vectors in compact form.
        ...
        ! =====================================================================
           ...
           ! if the input matrix is too small, call stdlib_qlasdq to find the svd.
           if( n<=smlsiz ) then
              call stdlib_qlasdq( 'U', sqre, n, m, n, 0_ilp, d, e, vt, ldvt, u, ldu, u,ldu, work, &
                        info )
              return
           end if
           ! set up the computation tree.
           ...
           ! for the nodes on bottom level of the tree, solve
           ! their subproblems by stdlib_qlasdq.
           ndb1 = ( nd+1 ) / 2_ilp
           ncc = 0_ilp
           loop_30: do i = ndb1, nd
           ! ic : center row of each node
           ...
           ! nrf: starting row of the right  subproblem
              ...
              nrf = ic + 1_ilp
              sqrei = 1_ilp
              call stdlib_qlasdq( 'U', sqrei, nl, nlp1, nl, ncc, d( nlf ), e( nlf ),vt( nlf, nlf )&
                        , ldvt, u( nlf, nlf ), ldu,u( nlf, nlf ), ldu, work, info )
              if( info/=0_ilp ) then
                 return
              end if
              ...
              nrp1 = nr + sqrei
              call stdlib_qlasdq( 'U', sqrei, nr, nrp1, nr, ncc, d( nrf ), e( nrf ),vt( nrf, nrf )&
                        , ldvt, u( nrf, nrf ), ldu,u( nrf, nrf ), ldu, work, info )
              if( info/=0_ilp ) then
                 return
              end if
           ...
           loop_50: do lvl = nlvl, 1, -1
              ...
              do i = lf, ll
                 ...
                 alpha = d( ic )
                 beta = e( ic )
                 call stdlib_qlasd1( nl, nr, sqrei, d( nlf ), alpha, beta,u( nlf, nlf ), ldu, vt( &
                           nlf, nlf ), ldvt,iwork( idxqc ), iwork( iwk ), work, info )
              ! report the possible convergence failure.
                 if( info/=0_ilp ) then
                    return
     ...
     !! from the overall problem.
        ...
        ! =====================================================================
           ...
           ! the following values are for bookkeeping purposes only.  they are
           ! integer pointers which indicate the portion of the workspace
           ! used by a particular array in stdlib_qlasd2 and stdlib_qlasd3.
           ldu2 = n
           ldvt2 = m
           iz = 1_ilp
           isigma = iz + m
           ...
           beta = beta / orgnrm
           ! deflate singular values.
           call stdlib_qlasd2( nl, nr, sqre, k, d, work( iz ), alpha, beta, u, ldu,vt, ldvt, work(&
            isigma ), work( iu2 ), ldu2,work( ivt2 ), ldvt2, iwork( idxp ), iwork( idx ),iwork( &
                      idxc ), idxq, iwork( coltyp ), info )
           ! solve secular equation and update singular vectors.
           ldq = k
           call stdlib_qlasd3( nl, nr, sqre, k, d, work( iq ), ldq, work( isigma ),u, ldu, work( &
           iu2 ), ldu2, vt, ldvt, work( ivt2 ),ldvt2, iwork( idxc ), iwork( coltyp ), work( iz ),&
                     info )
           ! report the convergence failure.
           if( info/=0_ilp ) then
           ...
           n1 = k
           n2 = n - k
           call stdlib_qlamrg( n1, n2, d, 1_ilp, -1_ilp, idxq )
           return
     ...
     !! DLASD2 is called from DLASD1.
        ...
        ! =====================================================================
           ...
           call stdlib_qlamrg( nl, nr, dsigma( 2_ilp ), 1_ilp, 1_ilp, idx( 2_ilp ) )
           do i = 2, n
              idxi = 1_ilp + idx( i )
              d( i ) = dsigma( idxi )
              z( i ) = u2( idxi, 1_ilp )
           ...
           if( abs( z( j ) )<=tol ) then
              ...
              if( abs( d( j )-d( jprev ) )<=tol ) then
                 ...
                 call stdlib_qrot( n, u( 1_ilp, idxjp ), 1_ilp, u( 1_ilp, idxj ), 1_ilp, c, s )
                 call stdlib_qrot( m, vt( idxjp, 1_ilp ), ldvt, vt( idxj, 1_ilp ), ldvt, c,s )
                 if( coltyp( j )/=coltyp( jprev ) ) then
                    coltyp( j ) = 3_ilp
                 end if
                 coltyp( jprev ) = 4_ilp
           ...
           do j = 2, n
              ...
              call stdlib_qcopy( n, u( 1_ilp, idxj ), 1_ilp, u2( 1_ilp, j ), 1_ilp )
              call stdlib_qcopy( m, vt( idxj, 1_ilp ), ldvt, vt2( j, 1_ilp ), ldvt2 )
           end do
           ! determine dsigma(1), dsigma(2) and z(1)
           dsigma( 1_ilp ) = zero
           hlftol = tol / two
           ...
           ! move the rest of the updating row to z.
           call stdlib_qcopy( k-1, u2( 2_ilp, 1_ilp ), 1_ilp, z( 2_ilp ), 1_ilp )
           ! determine the first column of u2, the first row of vt2 and the
           ! last row of vt.
           call stdlib_qlaset( 'A', n, 1_ilp, zero, zero, u2, ldu2 )
           u2( nlp1, 1_ilp ) = one
           if( m>n ) then
              do i = 1, nlp1
                 vt( m, i ) = -s*vt( nlp1, i )
              ...
              call stdlib_qcopy( m, vt( nlp1, 1_ilp ), ldvt, vt2( 1_ilp, 1_ilp ), ldvt2 )
           end if
           if( m>n ) then
              call stdlib_qcopy( m, vt( m, 1_ilp ), ldvt, vt2( m, 1_ilp ), ldvt2 )
           end if
           ! the deflated singular values and their corresponding vectors go
           ! into the back of d, u, and v respectively.
           if( n>k ) then
              call stdlib_qcopy( n-k, dsigma( k+1 ), 1_ilp, d( k+1 ), 1_ilp )
              call stdlib_qlacpy( 'A', n, n-k, u2( 1_ilp, k+1 ), ldu2, u( 1_ilp, k+1 ),ldu )
              call stdlib_qlacpy( 'A', n-k, m, vt2( k+1, 1_ilp ), ldvt2, vt( k+1, 1_ilp ),ldvt )
           end if
           ! copy ctot into coltyp for referencing in stdlib_qlasd3.
           do j = 1, 4
              coltyp( j ) = ctot( j )
           end do
           return
     ...
     !! DLASD3 is called from DLASD1.
        ...
        ! =====================================================================
           ...
           if( k==1_ilp ) then
              d( 1_ilp ) = abs( z( 1_ilp ) )
              call stdlib_qcopy( m, vt2( 1_ilp, 1_ilp ), ldvt2, vt( 1_ilp, 1_ilp ), ldvt )
              if( z( 1_ilp )>zero ) then
                 call stdlib_qcopy( n, u2( 1_ilp, 1_ilp ), 1_ilp, u( 1_ilp, 1_ilp ), 1_ilp )
              else
                 do i = 1, n
                    u( i, 1_ilp ) = -u2( i, 1_ilp )
                 end do
           ...
           ! this code.
           do i = 1, k
              dsigma( i ) = stdlib_qlamc3( dsigma( i ), dsigma( i ) ) - dsigma( i )
           end do
           ! keep a copy of z.
           call stdlib_qcopy( k, z, 1_ilp, q, 1_ilp )
           ! normalize z.
           rho = stdlib_qnrm2( k, z, 1_ilp )
           call stdlib_qlascl( 'G', 0_ilp, 0_ilp, rho, one, k, 1_ilp, z, k, info )
           rho = rho*rho
           ! find the new singular values.
           do j = 1, k
              call stdlib_qlasd4( k, j, dsigma, z, u( 1_ilp, j ), rho, d( j ),vt( 1_ilp, j ), info )
           ...
           do i = 1, k
              ...
              temp = stdlib_qnrm2( k, u( 1_ilp, i ), 1_ilp )
              q( 1_ilp, i ) = u( 1_ilp, i ) / temp
              do j = 2, k
                 jc = idxc( j )
                 q( j, i ) = u( jc, i ) / temp
           ...
           ! update the left singular vector matrix.
           if( k==2_ilp ) then
              call stdlib_qgemm( 'N', 'N', n, k, k, one, u2, ldu2, q, ldq, zero, u,ldu )
              go to 100
           end if
           if( ctot( 1_ilp )>0_ilp ) then
              call stdlib_qgemm( 'N', 'N', nl, k, ctot( 1_ilp ), one, u2( 1_ilp, 2_ilp ), ldu2,q( 2_ilp, 1_ilp ), ldq,&
                         zero, u( 1_ilp, 1_ilp ), ldu )
              if( ctot( 3_ilp )>0_ilp ) then
                 ktemp = 2_ilp + ctot( 1_ilp ) + ctot( 2_ilp )
                 call stdlib_qgemm( 'N', 'N', nl, k, ctot( 3_ilp ), one, u2( 1_ilp, ktemp ),ldu2, q( &
                           ktemp, 1_ilp ), ldq, one, u( 1_ilp, 1_ilp ), ldu )
              end if
           else if( ctot( 3_ilp )>0_ilp ) then
              ktemp = 2_ilp + ctot( 1_ilp ) + ctot( 2_ilp )
              call stdlib_qgemm( 'N', 'N', nl, k, ctot( 3_ilp ), one, u2( 1_ilp, ktemp ),ldu2, q( ktemp, &
                        1_ilp ), ldq, zero, u( 1_ilp, 1_ilp ), ldu )
           else
              call stdlib_qlacpy( 'F', nl, k, u2, ldu2, u, ldu )
           end if
           call stdlib_qcopy( k, q( 1_ilp, 1_ilp ), ldq, u( nlp1, 1_ilp ), ldu )
           ktemp = 2_ilp + ctot( 1_ilp )
           ctemp = ctot( 2_ilp ) + ctot( 3_ilp )
           call stdlib_qgemm( 'N', 'N', nr, k, ctemp, one, u2( nlp2, ktemp ), ldu2,q( ktemp, 1_ilp ), &
                     ldq, zero, u( nlp2, 1_ilp ), ldu )
           ! generate the right singular vectors.
           100 continue
           do i = 1, k
              temp = stdlib_qnrm2( k, vt( 1_ilp, i ), 1_ilp )
              q( i, 1_ilp ) = vt( 1_ilp, i ) / temp
              do j = 2, k
                 jc = idxc( j )
                 q( i, j ) = vt( jc, i ) / temp
           ...
           ktemp = 1_ilp + ctot( 1_ilp )
           call stdlib_qgemm( 'N', 'N', k, nlp1, ktemp, one, q( 1_ilp, 1_ilp ), ldq,vt2( 1_ilp, 1_ilp ), ldvt2, &
                     zero, vt( 1_ilp, 1_ilp ), ldvt )
           ktemp = 2_ilp + ctot( 1_ilp ) + ctot( 2_ilp )
           if( ktemp<=ldvt2 )call stdlib_qgemm( 'N', 'N', k, nlp1, ctot( 3_ilp ), one, q( 1_ilp, ktemp ),&
                     ldq, vt2( ktemp, 1_ilp ), ldvt2, one, vt( 1_ilp, 1_ilp ),ldvt )
           ktemp = ctot( 1_ilp ) + 1_ilp
           nrp1 = nr + sqre
           if( ktemp>1_ilp ) then
           ...
           ctemp = 1_ilp + ctot( 2_ilp ) + ctot( 3_ilp )
           call stdlib_qgemm( 'N', 'N', k, nrp1, ctemp, one, q( 1_ilp, ktemp ), ldq,vt2( ktemp, nlp2 )&
                     , ldvt2, zero, vt( 1_ilp, nlp2 ), ldvt )
           return
     ...
     !! secular equation by simpler interpolating rational functions.
        ...
        ! =====================================================================
           ...
           if( n==2_ilp ) then
              call stdlib_qlasd5( i, d, z, delta, rho, sigma, work )
              return
           end if
           ! compute machine epsilon
           eps = stdlib_qlamch( 'EPSILON' )
          ...
          ! $          + abs( tau2 )*dw
              ...
              if( .not.swtch3 ) then
                 ...
                 dd( 2_ilp ) = delta( ii )*work( ii )
                 dd( 3_ilp ) = dtiip
                 call stdlib_qlaed6( niter, orgati, c, dd, zz, w, eta, info )
                 if( info/=0_ilp ) then
                    ! if info is not 0, i.e., stdlib_qlaed6 failed, switch back
                    ! to 2 pole interpolation.
                    swtch3 = .false.
                    info = 0_ilp
                    dtipsq = work( ip1 )*delta( ip1 )
          ...
          ! $          + abs( tau2 )*dw
           ...
           ! $          .or. (sgub-sglb)<=eight*abs(sgub+sglb) ) then
                 ...
                 if( .not.swtch3 ) then
                    ...
                    call stdlib_qlaed6( niter, orgati, c, dd, zz, w, eta, info )
                    if( info/=0_ilp ) then
                       ! if info is not 0, i.e., stdlib_qlaed6 failed, switch
                       ! back to two pole interpolation
                       swtch3 = .false.
                       info = 0_ilp
                       dtipsq = work( ip1 )*delta( ip1 )
     ...
     !! LAPACK Working Note #3, for a detailed description of the algorithm.
        ...
        ! =====================================================================
           ...
           if( ( iuplo==1_ilp ) .and. ( sqre1==1_ilp ) ) then
              ...
              call stdlib_qlartg( d( n ), e( n ), cs, sn, r )
              d( n ) = r
              e( n ) = zero
              if( rotate ) then
                 work( n ) = cs
              ...
              sqre1 = 0_ilp
              ! update singular vectors if desired.
              if( ncvt>0_ilp )call stdlib_qlasr( 'L', 'V', 'F', np1, ncvt, work( 1_ilp ),work( np1 ), vt, &
                        ldvt )
           end if
           ! if matrix lower bidiagonal, rotate to be upper bidiagonal
           ! by applying givens rotations on the left.
           if( iuplo==2_ilp ) then
              ...
              if( nru>0_ilp ) then
                 if( sqre1==0_ilp ) then
                    call stdlib_qlasr( 'R', 'V', 'F', nru, n, work( 1_ilp ),work( np1 ), u, ldu )
                              
                 else
                    call stdlib_qlasr( 'R', 'V', 'F', nru, np1, work( 1_ilp ),work( np1 ), u, ldu )
                              
                 end if
              end if
              if( ncc>0_ilp ) then
                 if( sqre1==0_ilp ) then
                    call stdlib_qlasr( 'L', 'V', 'F', n, ncc, work( 1_ilp ),work( np1 ), c, ldc )
                              
                 else
                    call stdlib_qlasr( 'L', 'V', 'F', np1, ncc, work( 1_ilp ),work( np1 ), c, ldc )
           ...
           do i = 1, n
              ...
              if( isub/=i ) then
                 ...
                 d( isub ) = d( i )
                 d( i ) = smin
                 if( ncvt>0_ilp )call stdlib_qswap( ncvt, vt( isub, 1_ilp ), ldvt, vt( i, 1_ilp ), ldvt )
                           
                 if( nru>0_ilp )call stdlib_qswap( nru, u( 1_ilp, isub ), 1_ilp, u( 1_ilp, i ), 1_ilp )
                 if( ncc>0_ilp )call stdlib_qswap( ncc, c( isub, 1_ilp ), ldc, c( i, 1_ilp ), ldc )
              end if
           end do
           return
     ...
     !! the singular vectors in explicit form.
        ...
        ! =====================================================================
           ...
           if( n<=smlsiz ) then
              if( icompq==0_ilp ) then
                 call stdlib_qlasdq( 'U', sqre, n, 0_ilp, 0_ilp, 0_ilp, d, e, vt, ldu, u, ldu,u, ldu, work, &
                           info )
              else
                 call stdlib_qlasdq( 'U', sqre, n, m, n, 0_ilp, d, e, vt, ldu, u, ldu,u, ldu, work, &
                           info )
              end if
              return
           end if
           ...
           loop_30: do i = ndb1, nd
              ...
              if( icompq==0_ilp ) then
                 ...
                 itemp = nwork1 + nl*smlszp
                 call stdlib_qcopy( nlp1, work( nwork1 ), 1_ilp, work( vfi ), 1_ilp )
                 call stdlib_qcopy( nlp1, work( itemp ), 1_ilp, work( vli ), 1_ilp )
              else
                 call stdlib_qlaset( 'A', nl, nl, zero, one, u( nlf, 1_ilp ), ldu )
                 call stdlib_qlaset( 'A', nlp1, nlp1, zero, one, vt( nlf, 1_ilp ), ldu )
                 call stdlib_qlasdq( 'U', sqrei, nl, nlp1, nl, ncc, d( nlf ),e( nlf ), vt( nlf, 1_ilp &
                           ), ldu, u( nlf, 1_ilp ), ldu,u( nlf, 1_ilp ), ldu, work( nwork1 ), info )
                 call stdlib_qcopy( nlp1, vt( nlf, 1_ilp ), 1_ilp, work( vfi ), 1_ilp )
                 call stdlib_qcopy( nlp1, vt( nlf, nlp1 ), 1_ilp, work( vli ), 1_ilp )
              end if
              if( info/=0_ilp ) then
                 return
              end if
              ...
              if( icompq==0_ilp ) then
                 ...
                 itemp = nwork1 + ( nrp1-1 )*smlszp
                 call stdlib_qcopy( nrp1, work( nwork1 ), 1_ilp, work( vfi ), 1_ilp )
                 call stdlib_qcopy( nrp1, work( itemp ), 1_ilp, work( vli ), 1_ilp )
              else
                 call stdlib_qlaset( 'A', nr, nr, zero, one, u( nrf, 1_ilp ), ldu )
                 call stdlib_qlaset( 'A', nrp1, nrp1, zero, one, vt( nrf, 1_ilp ), ldu )
                 call stdlib_qlasdq( 'U', sqrei, nr, nrp1, nr, ncc, d( nrf ),e( nrf ), vt( nrf, 1_ilp &
                           ), ldu, u( nrf, 1_ilp ), ldu,u( nrf, 1_ilp ), ldu, work( nwork1 ), info )
                 call stdlib_qcopy( nrp1, vt( nrf, 1_ilp ), 1_ilp, work( vfi ), 1_ilp )
                 call stdlib_qcopy( nrp1, vt( nrf, nrp1 ), 1_ilp, work( vli ), 1_ilp )
              end if
              if( info/=0_ilp ) then
                 return
              end if
           ...
           loop_50: do lvl = nlvl, 1, -1
              ...
              loop_40: do i = lf, ll
                 ...
                 beta = e( ic )
                 if( icompq==0_ilp ) then
                    call stdlib_qlasd6( icompq, nl, nr, sqrei, d( nlf ),work( vfi ), work( vli ), &
                    alpha, beta,iwork( idxqi ), perm, givptr( 1_ilp ), givcol,ldgcol, givnum, ldu, &
                    poles, difl, difr, z,k( 1_ilp ), c( 1_ilp ), s( 1_ilp ), work( nwork1 ),iwork( iwk ), &
                              info )
                 else
     ...
     givptr, givcol, ldgcol, givnum,ldgnum, poles, difl, difr, z, k, c, s, work,iwork, info )
        ...
        ! =====================================================================
           ...
           ! the following values are for bookkeeping purposes only.  they are
           ! integer pointers which indicate the portion of the workspace
           ! used by a particular array in stdlib_qlasd7 and stdlib_qlasd8.
           isigma = 1_ilp
           iw = isigma + n
           ivfw = iw + m
           ivlw = ivfw + m
           ...
           beta = beta / orgnrm
           ! sort and deflate singular values.
           call stdlib_qlasd7( icompq, nl, nr, sqre, k, d, z, work( iw ), vf,work( ivfw ), vl, &
           work( ivlw ), alpha, beta,work( isigma ), iwork( idx ), iwork( idxp ), idxq,perm, &
                     givptr, givcol, ldgcol, givnum, ldgnum, c, s,info )
           ! solve secular equation, compute difl, difr, and update vf, vl.
           call stdlib_qlasd8( icompq, k, d, z, vf, vl, difl, difr, ldgnum,work( isigma ), work( &
                     iw ), info )
           ! report the possible convergence failure.
           if( info/=0_ilp ) then
              return
           ...
           ! save the poles if icompq = 1.
           if( icompq==1_ilp ) then
              call stdlib_qcopy( k, d, 1_ilp, poles( 1_ilp, 1_ilp ), 1_ilp )
              call stdlib_qcopy( k, work( isigma ), 1_ilp, poles( 1_ilp, 2_ilp ), 1_ilp )
           end if
           ! unscale.
           call stdlib_qlascl( 'G', 0_ilp, 0_ilp, one, orgnrm, n, 1_ilp, d, n, info )
           ! prepare the idxq sorting permutation.
        ...
        ! =====================================================================
           ...
           if( abs( z( j ) )<=tol ) then
              ...
              if( abs( d( j )-d( jprev ) )<=tol ) then
                 ...
                 call stdlib_qrot( 1_ilp, vf( jprev ), 1_ilp, vf( j ), 1_ilp, c, s )
                 call stdlib_qrot( 1_ilp, vl( jprev ), 1_ilp, vl( j ), 1_ilp, c, s )
                 k2 = k2 - 1_ilp
                 idxp( k2 ) = jprev
                 jprev = j
              else
           ...
           if( m>n ) then
              ...
              call stdlib_qrot( 1_ilp, vf( m ), 1_ilp, vf( 1_ilp ), 1_ilp, c, s )
              call stdlib_qrot( 1_ilp, vl( m ), 1_ilp, vl( 1_ilp ), 1_ilp, c, s )
           else
              if( abs( z1 )<=tol ) then
                 z( 1_ilp ) = tol
              else
           ...
           ! restore z, vf, and vl.
           call stdlib_qcopy( k-1, zw( 2_ilp ), 1_ilp, z( 2_ilp ), 1_ilp )
           call stdlib_qcopy( n-1, vfw( 2_ilp ), 1_ilp, vf( 2_ilp ), 1_ilp )
           call stdlib_qcopy( n-1, vlw( 2_ilp ), 1_ilp, vl( 2_ilp ), 1_ilp )
           return
     ...
     !! DLASD8 is called from DLASD6.
        ...
        ! =====================================================================
           ...
           rho = rho*rho
           ! initialize work(iwk3).
           call stdlib_qlaset( 'A', k, 1_ilp, one, one, work( iwk3 ), k )
           ! compute the updated singular values, the arrays difl, difr,
           ! and the updated z.
           do j = 1, k
           ...
           do j = 1, k
              ...
              work( j ) = -z( j ) / diflj / ( dsigma( j )+dj )
              do i = 1, j - 1
                 work( i ) = z( i ) / ( stdlib_qlamc3( dsigma( i ), dsigj )-diflj )/ ( dsigma( i )&
                           +dj )
              end do
              do i = j + 1, k
                 work( i ) = z( i ) / ( stdlib_qlamc3( dsigma( i ), dsigjp )+difrj )/ ( dsigma( i &
                           )+dj )
              end do
              temp = stdlib_qnrm2( k, work, 1_ilp )
              work( iwk2i+j ) = stdlib_qdot( k, work, 1_ilp, vf, 1_ilp ) / temp
              work( iwk3i+j ) = stdlib_qdot( k, work, 1_ilp, vl, 1_ilp ) / temp
              if( icompq==1_ilp ) then
                 difr( j, 2_ilp ) = temp
              end if
           end do
           call stdlib_qcopy( k, work( iwk2 ), 1_ilp, vf, 1_ilp )
           call stdlib_qcopy( k, work( iwk3 ), 1_ilp, vl, 1_ilp )
           return
