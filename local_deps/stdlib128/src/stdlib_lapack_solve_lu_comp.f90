  ...
  contains
     ...
     !! RCOND = 1 / ( norm(A) * norm(inv(A)) ).
        ...
        ! =====================================================================
           ...
           smlnum = stdlib_qlamch( 'SAFE MINIMUM' )
           ! estimate the norm of inv(a).
           ainvnm = zero
           normin = 'N'
           if( onenrm ) then
           ...
           if( kase/=0_ilp ) then
              if( kase==kase1 ) then
                 ! multiply by inv(l).
                 call stdlib_qlatrs( 'LOWER', 'NO TRANSPOSE', 'UNIT', normin, n, a,lda, work, sl, &
                           work( 2_ilp*n+1 ), info )
                 ! multiply by inv(u).
                 call stdlib_qlatrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', normin, n,a, lda, work, &
                           su, work( 3_ilp*n+1 ), info )
              else
                 ! multiply by inv(u**t).
                 call stdlib_qlatrs( 'UPPER', 'TRANSPOSE', 'NON-UNIT', normin, n, a,lda, work, su,&
                            work( 3_ilp*n+1 ), info )
                 ! multiply by inv(l**t).
                 call stdlib_qlatrs( 'LOWER', 'TRANSPOSE', 'UNIT', normin, n, a,lda, work, sl, &
                           work( 2_ilp*n+1 ), info )
              end if
              ! divide x by 1/(sl*su) if doing so will not cause overflow.
              scale = sl*su
              normin = 'Y'
              if( scale/=one ) then
                 ix = stdlib_iqamax( n, work, 1_ilp )
                 if( scale<abs( work( ix ) )*smlnum .or. scale==zero )go to 20
                 call stdlib_qrscl( n, scale, work, 1_ilp )
              end if
              go to 10
           end if
           ! compute the estimate of the reciprocal condition number.
     ...
     !! RCOND = 1 / ( norm(A) * norm(inv(A)) ).
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==kase1 ) then
                 ! multiply by inv(l).
                 call stdlib_wlatrs( 'LOWER', 'NO TRANSPOSE', 'UNIT', normin, n, a,lda, work, sl, &
                           rwork, info )
                 ! multiply by inv(u).
                 call stdlib_wlatrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', normin, n,a, lda, work, &
                           su, rwork( n+1 ), info )
              else
                 ! multiply by inv(u**h).
                 call stdlib_wlatrs( 'UPPER', 'CONJUGATE TRANSPOSE', 'NON-UNIT',normin, n, a, lda,&
                            work, su, rwork( n+1 ),info )
                 ! multiply by inv(l**h).
                 call stdlib_wlatrs( 'LOWER', 'CONJUGATE TRANSPOSE', 'UNIT', normin,n, a, lda, &
                           work, sl, rwork, info )
              end if
              ! divide x by 1/(sl*su) if doing so will not cause overflow.
              scale = sl*su
     ...
     !! This is the right-looking Level 3 BLAS version of the algorithm.
        ...
        ! =====================================================================
           ...
           if( nb<=1_ilp .or. nb>=min( m, n ) ) then
              ! use unblocked code.
              call stdlib_qgetrf2( m, n, a, lda, ipiv, info )
           else
              ! use blocked code.
              do j = 1, min( m, n ), nb
                 jb = min( min( m, n )-j+1, nb )
                 ! factor diagonal and subdiagonal blocks and test for exact
                 ! singularity.
                 call stdlib_qgetrf2( m-j+1, jb, a( j, j ), lda, ipiv( j ), iinfo )
                 ! adjust info and the pivot indices.
                 if( info==0_ilp .and. iinfo>0_ilp )info = iinfo + j - 1_ilp
                 do i = j, min( m, j+jb-1 )
                    ipiv( i ) = j - 1_ilp + ipiv( i )
                 ...
                 ! apply interchanges to columns 1:j-1.
                 call stdlib_qlaswp( j-1, a, lda, j, j+jb-1, ipiv, 1_ilp )
                 if( j+jb<=n ) then
                    ! apply interchanges to columns j+jb:n.
                    call stdlib_qlaswp( n-j-jb+1, a( 1_ilp, j+jb ), lda, j, j+jb-1,ipiv, 1_ilp )
                    ! compute block row of u.
     ...
     !! This is the right-looking Level 3 BLAS version of the algorithm.
        ...
        ! =====================================================================
           ...
           if( nb<=1_ilp .or. nb>=min( m, n ) ) then
              ! use unblocked code.
              call stdlib_wgetrf2( m, n, a, lda, ipiv, info )
           else
              ! use blocked code.
              do j = 1, min( m, n ), nb
                 jb = min( min( m, n )-j+1, nb )
                 ! factor diagonal and subdiagonal blocks and test for exact
                 ! singularity.
                 call stdlib_wgetrf2( m-j+1, jb, a( j, j ), lda, ipiv( j ), iinfo )
                 ! adjust info and the pivot indices.
                 if( info==0_ilp .and. iinfo>0_ilp )info = iinfo + j - 1_ilp
                 do i = j, min( m, j+jb-1 )
                    ipiv( i ) = j - 1_ilp + ipiv( i )
                 ...
                 ! apply interchanges to columns 1:j-1.
                 call stdlib_wlaswp( j-1, a, lda, j, j+jb-1, ipiv, 1_ilp )
                 if( j+jb<=n ) then
                    ! apply interchanges to columns j+jb:n.
                    call stdlib_wlaswp( n-j-jb+1, a( 1_ilp, j+jb ), lda, j, j+jb-1,ipiv, 1_ilp )
                    ! compute block row of u.
     ...
     !! then calls itself to factor A22 and do the swaps on A21.
        ...
        ! =====================================================================
           ...
           if ( m==1_ilp ) then
              ...
              ! find pivot and test for singularity
              i = stdlib_iqamax( m, a( 1_ilp, 1_ilp ), 1_ilp )
              ipiv( 1_ilp ) = i
              if( a( i, 1_ilp )/=zero ) then
                 ! apply the interchange
                 if( i/=1_ilp ) then
                 ...
                 ! compute elements 2:m of the column
                 if( abs(a( 1_ilp, 1_ilp )) >= sfmin ) then
                    call stdlib_qscal( m-1, one / a( 1_ilp, 1_ilp ), a( 2_ilp, 1_ilp ), 1_ilp )
                 else
                    do i = 1, m-1
                       a( 1_ilp+i, 1_ilp ) = a( 1_ilp+i, 1_ilp ) / a( 1_ilp, 1_ilp )
                    end do
              ...
              ! factor [ --- ]
                     ! [ a21 ]
              call stdlib_qgetrf2( m, n1, a, lda, ipiv, iinfo )
              if ( info==0_ilp .and. iinfo>0_ilp )info = iinfo
                                    ! [ a12 ]
              ! apply interchanges to [ --- ]
                                    ! [ a22 ]
              call stdlib_qlaswp( n2, a( 1_ilp, n1+1 ), lda, 1_ilp, n1, ipiv, 1_ilp )
              ! solve a12
              call stdlib_qtrsm( 'L', 'L', 'N', 'U', n1, n2, one, a, lda,a( 1_ilp, n1+1 ), lda )
                        
              ! update a22
              call stdlib_qgemm( 'N', 'N', m-n1, n2, n1, -one, a( n1+1, 1_ilp ), lda,a( 1_ilp, n1+1 ), &
                        lda, one, a( n1+1, n1+1 ), lda )
              ! factor a22
              call stdlib_qgetrf2( m-n1, n2, a( n1+1, n1+1 ), lda, ipiv( n1+1 ),iinfo )
              ! adjust info and the pivot indices
              if ( info==0_ilp .and. iinfo>0_ilp )info = iinfo + n1
              do i = n1+1, min( m, n )
                 ipiv( i ) = ipiv( i ) + n1
              ...
              ! apply interchanges to a21
              call stdlib_qlaswp( n1, a( 1_ilp, 1_ilp ), lda, n1+1, min( m, n), ipiv, 1_ilp )
           end if
           return
     ...
     !! then calls itself to factor A22 and do the swaps on A21.
        ...
        ! =====================================================================
           ...
           if ( m==1_ilp ) then
              ...
              sfmin = stdlib_qlamch('S')
              ! find pivot and test for singularity
              i = stdlib_iwamax( m, a( 1_ilp, 1_ilp ), 1_ilp )
              ipiv( 1_ilp ) = i
              if( a( i, 1_ilp )/=czero ) then
                 ! apply the interchange
                 if( i/=1_ilp ) then
                 ...
                 ! compute elements 2:m of the column
                 if( abs(a( 1_ilp, 1_ilp )) >= sfmin ) then
                    call stdlib_wscal( m-1, cone / a( 1_ilp, 1_ilp ), a( 2_ilp, 1_ilp ), 1_ilp )
                 else
                    do i = 1, m-1
                       a( 1_ilp+i, 1_ilp ) = a( 1_ilp+i, 1_ilp ) / a( 1_ilp, 1_ilp )
                    end do
              ...
              ! factor [ --- ]
                     ! [ a21 ]
              call stdlib_wgetrf2( m, n1, a, lda, ipiv, iinfo )
              if ( info==0_ilp .and. iinfo>0_ilp )info = iinfo
                                    ! [ a12 ]
              ! apply interchanges to [ --- ]
                                    ! [ a22 ]
              call stdlib_wlaswp( n2, a( 1_ilp, n1+1 ), lda, 1_ilp, n1, ipiv, 1_ilp )
              ! solve a12
              call stdlib_wtrsm( 'L', 'L', 'N', 'U', n1, n2, cone, a, lda,a( 1_ilp, n1+1 ), lda )
                        
              ! update a22
              call stdlib_wgemm( 'N', 'N', m-n1, n2, n1, -cone, a( n1+1, 1_ilp ), lda,a( 1_ilp, n1+1 ), &
                        lda, cone, a( n1+1, n1+1 ), lda )
              ! factor a22
              call stdlib_wgetrf2( m-n1, n2, a( n1+1, n1+1 ), lda, ipiv( n1+1 ),iinfo )
              ! adjust info and the pivot indices
              if ( info==0_ilp .and. iinfo>0_ilp )info = iinfo + n1
              do i = n1+1, min( m, n )
                 ipiv( i ) = ipiv( i ) + n1
              ...
              ! apply interchanges to a21
              call stdlib_wlaswp( n1, a( 1_ilp, 1_ilp ), lda, n1+1, min( m, n), ipiv, 1_ilp )
           end if
           return
     ...
     !! This is the right-looking Level 2 BLAS version of the algorithm.
        ...
        ! =====================================================================
           ...
           do j = 1, min( m, n )
              ! find pivot and test for singularity.
              jp = j - 1_ilp + stdlib_iqamax( m-j+1, a( j, j ), 1_ilp )
              ipiv( j ) = jp
              if( a( jp, j )/=zero ) then
                 ! apply the interchange to columns 1:n.
                 if( jp/=j )call stdlib_qswap( n, a( j, 1_ilp ), lda, a( jp, 1_ilp ), lda )
                 ! compute elements j+1:m of j-th column.
                 if( j<m ) then
                    if( abs(a( j, j )) >= sfmin ) then
                       call stdlib_qscal( m-j, one / a( j, j ), a( j+1, j ), 1_ilp )
                    else
                      do i = 1, m-j
                         a( j+i, j ) = a( j+i, j ) / a( j, j )
                      end do
              ...
              if( j<min( m, n ) ) then
                 ! update trailing submatrix.
                 call stdlib_qger( m-j, n-j, -one, a( j+1, j ), 1_ilp, a( j, j+1 ), lda,a( j+1, j+1 ),&
                            lda )
              end if
           end do
           return
     ...
     !! This is the right-looking Level 2 BLAS version of the algorithm.
        ...
        ! =====================================================================
           ...
           do j = 1, min( m, n )
              ! find pivot and test for singularity.
              jp = j - 1_ilp + stdlib_iwamax( m-j+1, a( j, j ), 1_ilp )
              ipiv( j ) = jp
              if( a( jp, j )/=czero ) then
                 ! apply the interchange to columns 1:n.
                 if( jp/=j )call stdlib_wswap( n, a( j, 1_ilp ), lda, a( jp, 1_ilp ), lda )
                 ! compute elements j+1:m of j-th column.
                 if( j<m ) then
                    if( abs(a( j, j )) >= sfmin ) then
                       call stdlib_wscal( m-j, cone / a( j, j ), a( j+1, j ), 1_ilp )
                    else
                       do i = 1, m-j
                          a( j+i, j ) = a( j+i, j ) / a( j, j )
                       end do
              ...
              if( j<min( m, n ) ) then
                 ! update trailing submatrix.
                 call stdlib_wgeru( m-j, n-j, -cone, a( j+1, j ), 1_ilp, a( j, j+1 ),lda, a( j+1, j+1 &
                           ), lda )
              end if
           end do
           return
     ...
     !! by DGETRF.
        ...
        ! =====================================================================
           ...
           if( notran ) then
              ! solve a * x = b.
              ! apply row interchanges to the right hand sides.
              call stdlib_qlaswp( nrhs, b, ldb, 1_ilp, n, ipiv, 1_ilp )
              ! solve l*x = b, overwriting b with x.
              call stdlib_qtrsm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'UNIT', n, nrhs,one, a, lda, b, &
                        ldb )
              ! solve u*x = b, overwriting b with x.
              call stdlib_qtrsm( 'LEFT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n,nrhs, one, a, lda,&
                         b, ldb )
           else
              ! solve a**t * x = b.
              ! solve u**t *x = b, overwriting b with x.
              call stdlib_qtrsm( 'LEFT', 'UPPER', 'TRANSPOSE', 'NON-UNIT', n, nrhs,one, a, lda, b,&
                         ldb )
              ! solve l**t *x = b, overwriting b with x.
              call stdlib_qtrsm( 'LEFT', 'LOWER', 'TRANSPOSE', 'UNIT', n, nrhs, one,a, lda, b, &
                        ldb )
              ! apply row interchanges to the solution vectors.
              call stdlib_qlaswp( nrhs, b, ldb, 1_ilp, n, ipiv, -1_ilp )
           end if
     ...
     !! by ZGETRF.
        ...
        ! =====================================================================
           ...
           if( notran ) then
              ! solve a * x = b.
              ! apply row interchanges to the right hand sides.
              call stdlib_wlaswp( nrhs, b, ldb, 1_ilp, n, ipiv, 1_ilp )
              ! solve l*x = b, overwriting b with x.
              call stdlib_wtrsm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'UNIT', n, nrhs,cone, a, lda, b,&
                         ldb )
              ! solve u*x = b, overwriting b with x.
              call stdlib_wtrsm( 'LEFT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n,nrhs, cone, a, &
                        lda, b, ldb )
           else
              ! solve a**t * x = b  or a**h * x = b.
              ! solve u**t *x = b or u**h *x = b, overwriting b with x.
              call stdlib_wtrsm( 'LEFT', 'UPPER', trans, 'NON-UNIT', n, nrhs, cone,a, lda, b, ldb &
                        )
              ! solve l**t *x = b, or l**h *x = b overwriting b with x.
     ...
     !! inv(A)*L = inv(U) for inv(A).
        ...
        ! =====================================================================
           ...
           ! quick return if possible
           if( n==0 )return
           ! form inv(u).  if info > 0 from stdlib_qtrtri, then u is singular,
           ! and the inverse is not computed.
           call stdlib_qtrtri( 'UPPER', 'NON-UNIT', n, a, lda, info )
           if( info>0 )return
           nbmin = 2_ilp
           ldwork = n
           if( nb>1_ilp .and. nb<n ) then
           ...
           if( nb<nbmin .or. nb>=n ) then
              ...
              do j = n, 1, -1
                 ...
                 ! compute current column of inv(a).
                 if( j<n )call stdlib_qgemv( 'NO TRANSPOSE', n, n-j, -one, a( 1_ilp, j+1 ),lda, work( &
                           j+1 ), 1_ilp, one, a( 1_ilp, j ), 1_ilp )
              end do
           else
              ! use blocked code.
              ...
              do j = nn, 1, -nb
                 ...
                 ! compute current block column of inv(a).
                 if( j+jb<=n )call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', n, jb,n-j-jb+1, -&
                           one, a( 1_ilp, j+jb ), lda,work( j+jb ), ldwork, one, a( 1_ilp, j ), lda )
                 call stdlib_qtrsm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', 'UNIT', n, jb,one, work( j )&
                           , ldwork, a( 1_ilp, j ), lda )
              end do
           end if
           ! apply column interchanges.
           do j = n - 1, 1, -1
              jp = ipiv( j )
              if( jp/=j )call stdlib_qswap( n, a( 1_ilp, j ), 1_ilp, a( 1_ilp, jp ), 1_ilp )
           end do
           work( 1_ilp ) = iws
           return
     ...
     !! inv(A)*L = inv(U) for inv(A).
        ...
        ! =====================================================================
           ...
           ! quick return if possible
           if( n==0 )return
           ! form inv(u).  if info > 0 from stdlib_wtrtri, then u is singular,
           ! and the inverse is not computed.
           call stdlib_wtrtri( 'UPPER', 'NON-UNIT', n, a, lda, info )
           if( info>0 )return
           nbmin = 2_ilp
           ldwork = n
           if( nb>1_ilp .and. nb<n ) then
           ...
           if( nb<nbmin .or. nb>=n ) then
              ...
              do j = n, 1, -1
                 ...
                 ! compute current column of inv(a).
                 if( j<n )call stdlib_wgemv( 'NO TRANSPOSE', n, n-j, -cone, a( 1_ilp, j+1 ),lda, work(&
                            j+1 ), 1_ilp, cone, a( 1_ilp, j ), 1_ilp )
              end do
           else
              ! use blocked code.
              ...
              do j = nn, 1, -nb
                 ...
                 ! compute current block column of inv(a).
                 if( j+jb<=n )call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', n, jb,n-j-jb+1, -&
                           cone, a( 1_ilp, j+jb ), lda,work( j+jb ), ldwork, cone, a( 1_ilp, j ), lda )
                 call stdlib_wtrsm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', 'UNIT', n, jb,cone, work( j &
                           ), ldwork, a( 1_ilp, j ), lda )
              end do
           end if
           ! apply column interchanges.
           do j = n - 1, 1, -1
              jp = ipiv( j )
              if( jp/=j )call stdlib_wswap( n, a( 1_ilp, j ), 1_ilp, a( 1_ilp, jp ), 1_ilp )
           end do
           work( 1_ilp ) = iws
           return
     ...
     !! the solution.
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1, nrhs
              ...
              ! compute residual r = b - op(a) * x,
              ! where op(a) = a, a**t, or a**h, depending on trans.
              call stdlib_qcopy( n, b( 1_ilp, j ), 1_ilp, work( n+1 ), 1_ilp )
              call stdlib_qgemv( trans, n, n, -one, a, lda, x( 1_ilp, j ), 1_ilp, one,work( n+1 ), 1_ilp )
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_qgetrs( trans, n, 1_ilp, af, ldaf, ipiv, work( n+1 ), n,info )
                 call stdlib_qaxpy( n, one, work( n+1 ), 1_ilp, x( 1_ilp, j ), 1_ilp )
                 lstres = berr( j )
                 count = count + 1_ilp
                 go to 20
              end if
              ...
              ! is incremented by safe1 if the i-th component of
              ! abs(op(a))*abs(x) + abs(b) is less than safe2.
              ! use stdlib_qlacn2 to estimate the infinity-norm of the matrix
                 ! inv(op(a)) * diag(w),
              ! where w = abs(r) + nz*eps*( abs(op(a))*abs(x)+abs(b) )))
              do i = 1, n
                 if( work( i )>safe2 ) then
              ...
              kase = 0_ilp
              100 continue
              call stdlib_qlacn2( n, work( 2_ilp*n+1 ), work( n+1 ), iwork, ferr( j ),kase, isave )
                        
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**t).
                    call stdlib_qgetrs( transt, n, 1_ilp, af, ldaf, ipiv, work( n+1 ),n, info )
     ...
     !! the solution.
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1, nrhs
              ...
              ! compute residual r = b - op(a) * x,
              ! where op(a) = a, a**t, or a**h, depending on trans.
              call stdlib_wcopy( n, b( 1_ilp, j ), 1_ilp, work, 1_ilp )
              call stdlib_wgemv( trans, n, n, -cone, a, lda, x( 1_ilp, j ), 1_ilp, cone, work,1_ilp )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(op(a))*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ! or vector z.  if the i-th component of the denominator is less
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_wgetrs( trans, n, 1_ilp, af, ldaf, ipiv, work, n, info )
                 call stdlib_waxpy( n, cone, work, 1_ilp, x( 1_ilp, j ), 1_ilp )
                 lstres = berr( j )
                 count = count + 1_ilp
                 go to 20
              end if
              ...
              ! is incremented by safe1 if the i-th component of
              ! abs(op(a))*abs(x) + abs(b) is less than safe2.
              ! use stdlib_wlacn2 to estimate the infinity-norm of the matrix
                 ! inv(op(a)) * diag(w),
              ! where w = abs(r) + nz*eps*( abs(op(a))*abs(x)+abs(b) )))
              do i = 1, n
                 if( rwork( i )>safe2 ) then
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**h).
                    call stdlib_wgetrs( transt, n, 1_ilp, af, ldaf, ipiv, work, n,info )
                    do i = 1, n
                       work( i ) = rwork( i )*work( i )
                    end do
                 else
                    ...
                    call stdlib_wgetrs( transn, n, 1_ilp, af, ldaf, ipiv, work, n,info )
                 end if
                 go to 100
              end if
              ! normalize error.
     ...
     !! column scaling factors in the vectors R and C.
        ...
        ! =====================================================================
           ...
           ! initialize large and small.
           small = stdlib_qlamch( 'SAFE MINIMUM' ) / stdlib_qlamch( 'PRECISION' )
           large = one / small
           if( rowcnd>=thresh .and. amax>=small .and. amax<=large )then
              ! no row scaling
              if( colcnd>=thresh ) then
     ...
     !! This is the Level 2 BLAS algorithm.
        ...
        ! =====================================================================
           ...
           loop_40: do i = 1, n - 1
              ...
              if( i==1_ilp )smin = max( eps*xmax, smlnum )
              ! swap rows
              if( ipv/=i )call stdlib_qswap( n, a( ipv, 1_ilp ), lda, a( i, 1_ilp ), lda )
              ipiv( i ) = ipv
              ! swap columns
              if( jpv/=i )call stdlib_qswap( n, a( 1_ilp, jpv ), 1_ilp, a( 1_ilp, i ), 1_ilp )
              jpiv( i ) = jpv
              ! check for singularity
              if( abs( a( i, i ) )<smin ) then
                 info = i
              ...
              call stdlib_qger( n-i, n-i, -one, a( i+1, i ), 1_ilp, a( i, i+1 ), lda,a( i+1, i+1 ), &
                        lda )
           end do loop_40
           if( abs( a( n, n ) )<smin ) then
              info = n
     ...
     !! This is a level 1 BLAS version of the algorithm.
        ...
        ! =====================================================================
           ...
           if( n==1_ilp ) then
              ...
              if( abs( a( 1_ilp, 1_ilp ) )<smlnum ) then
                 info = 1_ilp
                 a( 1_ilp, 1_ilp ) = cmplx( smlnum, zero,KIND=qp)
              end if
              return
           end if
           ! factorize a using complete pivoting.
           ...
           loop_40: do i = 1, n - 1
              ...
              if( i==1_ilp )smin = max( eps*xmax, smlnum )
              ! swap rows
              if( ipv/=i )call stdlib_wswap( n, a( ipv, 1_ilp ), lda, a( i, 1_ilp ), lda )
              ipiv( i ) = ipv
              ! swap columns
              if( jpv/=i )call stdlib_wswap( n, a( 1_ilp, jpv ), 1_ilp, a( 1_ilp, i ), 1_ilp )
              jpiv( i ) = jpv
              ! check for singularity
              if( abs( a( i, i ) )<smin ) then
                 info = i
              ...
              call stdlib_wgeru( n-i, n-i, -cmplx( one,KIND=qp), a( i+1, i ), 1_ilp,a( i, i+1 ), lda, &
                        a( i+1, i+1 ), lda )
           end do loop_40
           if( abs( a( n, n ) )<smin ) then
              info = n
     ...
     !! complete pivoting computed by DGETC2.
        ...
        ! =====================================================================
           ...
           call stdlib_qlabad( smlnum, bignum )
           ! apply permutations ipiv to rhs
           call stdlib_qlaswp( 1_ilp, rhs, lda, 1_ilp, n-1, ipiv, 1_ilp )
           ! solve for l part
           do i = 1, n - 1
              do j = i + 1, n
                 rhs( j ) = rhs( j ) - a( j, i )*rhs( i )
           ...
           scale = one
           ! check for scaling
           i = stdlib_iqamax( n, rhs, 1_ilp )
           if( two*smlnum*abs( rhs( i ) )>abs( a( n, n ) ) ) then
              temp = ( one / two ) / abs( rhs( i ) )
              call stdlib_qscal( n, temp, rhs( 1_ilp ), 1_ilp )
              scale = scale*temp
           end if
           do i = n, 1, -1
              temp = one / a( i, i )
           ...
           ! apply permutations jpiv to the solution (rhs)
           call stdlib_qlaswp( 1_ilp, rhs, lda, 1_ilp, n-1, jpiv, -1_ilp )
           return
     ...
     !! complete pivoting computed by ZGETC2.
        ...
        ! =====================================================================
           ...
           call stdlib_qlabad( smlnum, bignum )
           ! apply permutations ipiv to rhs
           call stdlib_wlaswp( 1_ilp, rhs, lda, 1_ilp, n-1, ipiv, 1_ilp )
           ! solve for l part
           do i = 1, n - 1
              do j = i + 1, n
                 rhs( j ) = rhs( j ) - a( j, i )*rhs( i )
           ...
           scale = one
           ! check for scaling
           i = stdlib_iwamax( n, rhs, 1_ilp )
           if( two*smlnum*abs( rhs( i ) )>abs( a( n, n ) ) ) then
              temp = cmplx( one / two, zero,KIND=qp) / abs( rhs( i ) )
              call stdlib_wscal( n, temp, rhs( 1_ilp ), 1_ilp )
           ...
           ! apply permutations jpiv to the solution (rhs)
           call stdlib_wlaswp( 1_ilp, rhs, lda, 1_ilp, n-1, jpiv, -1_ilp )
           return
     ...
     !! unit diagonal elements and U is upper triangular.
        ...
        ! =====================================================================
           ...
           ! Local Arrays 
           integer(ilp) :: iwork(maxdim)
           real(qp) :: work(4_ilp*maxdim), xm(maxdim), xp(maxdim)
           ! Intrinsic Functions 
           ! Executable Statements 
           if( ijob/=2_ilp ) then
              ! apply permutations ipiv to rhs
              call stdlib_qlaswp( 1_ilp, rhs, ldz, 1_ilp, n-1, ipiv, 1_ilp )
              ! solve for l-part choosing rhs either to +1 or -1.
              pmone = -one
              loop_10: do j = 1, n - 1
                 bp = rhs( j ) + one
                 ...
                 ! look-ahead for l-part rhs(1:n-1) = + or -1, splus and
                 ! smin computed more efficiently than in bsolve [1].
                 splus = splus + stdlib_qdot( n-j, z( j+1, j ), 1_ilp, z( j+1, j ), 1_ilp )
                 sminu = stdlib_qdot( n-j, z( j+1, j ), 1_ilp, rhs( j+1 ), 1_ilp )
                 splus = splus*rhs( j )
                 if( splus>sminu ) then
                    rhs( j ) = bp
                 else if( sminu>splus ) then
                 ...
                 ! compute the remaining r.h.s.
                 temp = -rhs( j )
                 call stdlib_qaxpy( n-j, temp, z( j+1, j ), 1_ilp, rhs( j+1 ), 1_ilp )
              end do loop_10
              ! solve for u-part, look-ahead for rhs(n) = +-1. this is not done
              ! in bsolve and will hopefully give us a better estimate because
              ! any ill-conditioning of the original matrix is transferred to u
              ! and not to l. u(n, n) is an approximation to sigma_min(lu).
              call stdlib_qcopy( n-1, rhs, 1_ilp, xp, 1_ilp )
              xp( n ) = rhs( n ) + one
              rhs( n ) = rhs( n ) - one
              splus = zero
              sminu = zero
              ...
              if( splus>sminu )call stdlib_qcopy( n, xp, 1_ilp, rhs, 1_ilp )
              ! apply the permutations jpiv to the computed solution (rhs)
              call stdlib_qlaswp( 1_ilp, rhs, ldz, 1_ilp, n-1, jpiv, -1_ilp )
              ! compute the sum of squares
              call stdlib_qlassq( n, rhs, 1_ilp, rdscal, rdsum )
           else
              ! ijob = 2, compute approximate nullvector xm of z
              call stdlib_qgecon( 'I', n, z, ldz, one, temp, work, iwork, info )
              call stdlib_qcopy( n, work( n+1 ), 1_ilp, xm, 1_ilp )
              ! compute rhs
              call stdlib_qlaswp( 1_ilp, xm, ldz, 1_ilp, n-1, ipiv, -1_ilp )
              temp = one / sqrt( stdlib_qdot( n, xm, 1_ilp, xm, 1_ilp ) )
              call stdlib_qscal( n, temp, xm, 1_ilp )
              call stdlib_qcopy( n, xm, 1_ilp, xp, 1_ilp )
              call stdlib_qaxpy( n, one, rhs, 1_ilp, xp, 1_ilp )
              call stdlib_qaxpy( n, -one, xm, 1_ilp, rhs, 1_ilp )
              call stdlib_qgesc2( n, z, ldz, rhs, ipiv, jpiv, temp )
              call stdlib_qgesc2( n, z, ldz, xp, ipiv, jpiv, temp )
              if( stdlib_qasum( n, xp, 1_ilp )>stdlib_qasum( n, rhs, 1_ilp ) )call stdlib_qcopy( n, xp, 1_ilp,&
                         rhs, 1_ilp )
              ! compute the sum of squares
              call stdlib_qlassq( n, rhs, 1_ilp, rdscal, rdsum )
           end if
     ...
     !! triangular with unit diagonal elements and U is upper triangular.
        ...
        ! =====================================================================
           ...
           complex(qp) :: work(4_ilp*maxdim), xm(maxdim), xp(maxdim)
           ! Intrinsic Functions 
           ! Executable Statements 
           if( ijob/=2_ilp ) then
              ! apply permutations ipiv to rhs
              call stdlib_wlaswp( 1_ilp, rhs, ldz, 1_ilp, n-1, ipiv, 1_ilp )
              ! solve for l-part choosing rhs either to +1 or -1.
              pmone = -cone
              loop_10: do j = 1, n - 1
                 bp = rhs( j ) + cone
                 ...
                 ! lockahead for l- part rhs(1:n-1) = +-1
                 ! splus and smin computed more efficiently than in bsolve[1].
                 splus = splus + real( stdlib_wdotc( n-j, z( j+1, j ), 1_ilp, z( j+1,j ), 1_ilp ),KIND=qp)
                           
                 sminu = real( stdlib_wdotc( n-j, z( j+1, j ), 1_ilp, rhs( j+1 ), 1_ilp ),KIND=qp)
                 ...
                 ! compute the remaining r.h.s.
                 temp = -rhs( j )
                 call stdlib_waxpy( n-j, temp, z( j+1, j ), 1_ilp, rhs( j+1 ), 1_ilp )
              end do loop_10
              ! solve for u- part, lockahead for rhs(n) = +-1. this is not done
              ! in bsolve and will hopefully give us a better estimate because
              ! any ill-conditioning of the original matrix is transferred to u
              ! and not to l. u(n, n) is an approximation to sigma_min(lu).
              call stdlib_wcopy( n-1, rhs, 1_ilp, work, 1_ilp )
              work( n ) = rhs( n ) + cone
              rhs( n ) = rhs( n ) - cone
              splus = zero
              sminu = zero
              ...
              if( splus>sminu )call stdlib_wcopy( n, work, 1_ilp, rhs, 1_ilp )
              ! apply the permutations jpiv to the computed solution (rhs)
              call stdlib_wlaswp( 1_ilp, rhs, ldz, 1_ilp, n-1, jpiv, -1_ilp )
              ! compute the sum of squares
              call stdlib_wlassq( n, rhs, 1_ilp, rdscal, rdsum )
              return
           end if
           ! entry ijob = 2
           ! compute approximate nullvector xm of z
           call stdlib_wgecon( 'I', n, z, ldz, one, rtemp, work, rwork, info )
           call stdlib_wcopy( n, work( n+1 ), 1_ilp, xm, 1_ilp )
           ! compute rhs
           call stdlib_wlaswp( 1_ilp, xm, ldz, 1_ilp, n-1, ipiv, -1_ilp )
           temp = cone / sqrt( stdlib_wdotc( n, xm, 1_ilp, xm, 1_ilp ) )
           call stdlib_wscal( n, temp, xm, 1_ilp )
           call stdlib_wcopy( n, xm, 1_ilp, xp, 1_ilp )
           call stdlib_waxpy( n, cone, rhs, 1_ilp, xp, 1_ilp )
           call stdlib_waxpy( n, -cone, xm, 1_ilp, rhs, 1_ilp )
           call stdlib_wgesc2( n, z, ldz, rhs, ipiv, jpiv, scale )
           call stdlib_wgesc2( n, z, ldz, xp, ipiv, jpiv, scale )
           if( stdlib_qzasum( n, xp, 1_ilp )>stdlib_qzasum( n, rhs, 1_ilp ) )call stdlib_wcopy( n, xp, 1_ilp, &
                     rhs, 1_ilp )
           ! compute the sum of squares
           call stdlib_wlassq( n, rhs, 1_ilp, rdscal, rdsum )
           return
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==2_ilp ) then
                 ...
                 if (notrans) then
                    call stdlib_qgetrs( 'NO TRANSPOSE', n, 1_ilp, af, ldaf, ipiv,work, n, info )
                              
                 else
                    call stdlib_qgetrs( 'TRANSPOSE', n, 1_ilp, af, ldaf, ipiv,work, n, info )
                 end if
                 ! multiply by inv(c).
                 if ( cmode == 1_ilp ) then
                    do i = 1, n
              ...
              if( kase==kase1 ) then
                 ...
                 if( lnoti ) then
                    do j = 1, n - 1
                       ...
                       call stdlib_qaxpy( lm, -t, ab( kd+1, j ), 1_ilp, work( j+1 ), 1_ilp )
                    end do
                 end if
                 ! multiply by inv(u).
                 call stdlib_qlatbs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', normin, n,kl+ku, ab, &
                           ldab, work, scale, work( 2_ilp*n+1 ),info )
              else
                 ! multiply by inv(u**t).
                 call stdlib_qlatbs( 'UPPER', 'TRANSPOSE', 'NON-UNIT', normin, n,kl+ku, ab, ldab, &
                           work, scale, work( 2_ilp*n+1 ),info )
                 ! multiply by inv(l**t).
                 if( lnoti ) then
                    do j = n - 1, 1, -1
                       lm = min( kl, n-j )
                       work( j ) = work( j ) - stdlib_qdot( lm, ab( kd+1, j ), 1_ilp,work( j+1 ), 1_ilp )
              ...
              if( kase==kase1 ) then
                 ...
                 if( lnoti ) then
                    do j = 1, n - 1
                       ...
                       call stdlib_waxpy( lm, -t, ab( kd+1, j ), 1_ilp, work( j+1 ), 1_ilp )
                    end do
                 end if
                 ! multiply by inv(u).
                 call stdlib_wlatbs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', normin, n,kl+ku, ab, &
                           ldab, work, scale, rwork, info )
              else
                 ! multiply by inv(u**h).
                 call stdlib_wlatbs( 'UPPER', 'CONJUGATE TRANSPOSE', 'NON-UNIT',normin, n, kl+ku, &
                           ab, ldab, work, scale, rwork,info )
                 ! multiply by inv(l**h).
                 if( lnoti ) then
                    do j = n - 1, 1, -1
                       lm = min( kl, n-j )
                       work( j ) = work( j ) - stdlib_wdotc( lm, ab( kd+1, j ), 1_ilp,work( j+1 ), 1_ilp )
     ...
     !! This is the blocked version of the algorithm, calling Level 3 BLAS.
        ...
        ! =====================================================================
           ...
           if( nb<=1_ilp .or. nb>kl ) then
              ! use unblocked code
              call stdlib_qgbtf2( m, n, kl, ku, ab, ldab, ipiv, info )
           else
              ! use blocked code
              ! zero the superdiagonal elements of the work array work13
              do j = 1, nb
              ...
              loop_180: do j = 1, min( m, n ), nb
                 ...
                 loop_80: do jj = j, j + jb - 1
                    ...
                    ! subdiagonal elements in the current column.
                    km = min( kl, m-jj )
                    jp = stdlib_iqamax( km+1, ab( kv+1, jj ), 1_ilp )
                    ipiv( jj ) = jp + jj - j
                    if( ab( kv+jp, jj )/=zero ) then
                       ju = max( ju, min( jj+ku+jp-1, n ) )
                       if( jp/=1_ilp ) then
                          ! apply interchange to columns j to j+jb-1
                          if( jp+jj-1<j+kl ) then
                             call stdlib_qswap( jb, ab( kv+1+jj-j, j ), ldab-1,ab( kv+jp+jj-j, j )&
                                       , ldab-1 )
                          else
                             ! the interchange affects columns j to jj-1 of a31
                             ! which are stored in the work array work31
                             call stdlib_qswap( jj-j, ab( kv+1+jj-j, j ), ldab-1,work31( jp+jj-j-&
                                       kl, 1_ilp ), ldwork )
                             call stdlib_qswap( j+jb-jj, ab( kv+1, jj ), ldab-1,ab( kv+jp, jj ), &
                                       ldab-1 )
                          end if
                       end if
                       ! compute multipliers
                       call stdlib_qscal( km, one / ab( kv+1, jj ), ab( kv+2, jj ),1_ilp )
                       ! update trailing submatrix within the band and within
                       ! the current block. jm is the index of the last column
                       ! which needs to be updated.
                       jm = min( ju, j+jb-1 )
                       if( jm>jj )call stdlib_qger( km, jm-jj, -one, ab( kv+2, jj ), 1_ilp,ab( kv, jj+&
                                 1_ilp ), ldab-1,ab( kv+1, jj+1 ), ldab-1 )
                    else
                       ! if pivot is zero, set info to the index of the pivot
                       ! unless a zero pivot has already been found.
                    ...
                    ! copy current column of a31 into the work array work31
                    nw = min( jj-j+1, i3 )
                    if( nw>0_ilp )call stdlib_qcopy( nw, ab( kv+kl+1-jj+j, jj ), 1_ilp,work31( 1_ilp, jj-j+1 )&
                              , 1_ilp )
                 end do loop_80
                 if( j+jb<=n ) then
                    ! apply the row interchanges to the other blocks.
                    ...
                    ! a32.
                    call stdlib_qlaswp( j2, ab( kv+1-jb, j+jb ), ldab-1, 1_ilp, jb,ipiv( j ), 1_ilp )
                    ...
                    if( j2>0_ilp ) then
                       ! update a12
                       call stdlib_qtrsm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'UNIT',jb, j2, one, ab(&
                                  kv+1, j ), ldab-1,ab( kv+1-jb, j+jb ), ldab-1 )
                       if( i2>0_ilp ) then
                          ! update a22
                          call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', i2, j2,jb, -one, ab( &
                          kv+1+jb, j ), ldab-1,ab( kv+1-jb, j+jb ), ldab-1, one,ab( kv+1, j+jb ), &
                                    ldab-1 )
                       end if
                       if( i3>0_ilp ) then
                          ! update a32
                          call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', i3, j2,jb, -one, &
                          work31, ldwork,ab( kv+1-jb, j+jb ), ldab-1, one,ab( kv+kl+1-jb, j+jb ), &
                                    ldab-1 )
                       end if
                    end if
                    if( j3>0_ilp ) then
                       ...
                       ! update a13 in the work array
                       call stdlib_qtrsm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'UNIT',jb, j3, one, ab(&
                                  kv+1, j ), ldab-1,work13, ldwork )
                       if( i2>0_ilp ) then
                          ! update a23
                       ...
                       if( i3>0_ilp ) then
                          ! update a33
                          call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', i3, j3,jb, -one, &
                                    work31, ldwork, work13,ldwork, one, ab( 1_ilp+kl, j+kv ), ldab-1 )
                       end if
                       ! copy the lower triangle of a13 back into place
                       do jj = 1, j3
                 ...
                 do jj = j + jb - 1, j, -1
                    ...
                    if( jp/=1_ilp ) then
                       ...
                       if( jp+jj-1<j+kl ) then
                          ! the interchange does not affect a31
                          call stdlib_qswap( jj-j, ab( kv+1+jj-j, j ), ldab-1,ab( kv+jp+jj-j, j ),&
                                     ldab-1 )
                       else
                          ! the interchange does affect a31
                          call stdlib_qswap( jj-j, ab( kv+1+jj-j, j ), ldab-1,work31( jp+jj-j-kl, &
                                    1_ilp ), ldwork )
                       end if
                    end if
                    ! copy the current column of a31 back into place
                    nw = min( i3, jj-j+1 )
                    if( nw>0_ilp )call stdlib_qcopy( nw, work31( 1_ilp, jj-j+1 ), 1_ilp,ab( kv+kl+1-jj+j, jj )&
                              , 1_ilp )
                 end do
              end do loop_180
           end if
     ...
     !! This is the blocked version of the algorithm, calling Level 3 BLAS.
        ...
        ! =====================================================================
           ...
           if( nb<=1_ilp .or. nb>kl ) then
              ! use unblocked code
              call stdlib_wgbtf2( m, n, kl, ku, ab, ldab, ipiv, info )
           else
              ! use blocked code
              ! czero the superdiagonal elements of the work array work13
              do j = 1, nb
              ...
              loop_180: do j = 1, min( m, n ), nb
                 ...
                 loop_80: do jj = j, j + jb - 1
                    ...
                    ! subdiagonal elements in the current column.
                    km = min( kl, m-jj )
                    jp = stdlib_iwamax( km+1, ab( kv+1, jj ), 1_ilp )
                    ipiv( jj ) = jp + jj - j
                    if( ab( kv+jp, jj )/=czero ) then
                       ju = max( ju, min( jj+ku+jp-1, n ) )
                       if( jp/=1_ilp ) then
                          ! apply interchange to columns j to j+jb-1
                          if( jp+jj-1<j+kl ) then
                             call stdlib_wswap( jb, ab( kv+1+jj-j, j ), ldab-1,ab( kv+jp+jj-j, j )&
                                       , ldab-1 )
                          else
                             ! the interchange affects columns j to jj-1 of a31
                             ! which are stored in the work array work31
                             call stdlib_wswap( jj-j, ab( kv+1+jj-j, j ), ldab-1,work31( jp+jj-j-&
                                       kl, 1_ilp ), ldwork )
                             call stdlib_wswap( j+jb-jj, ab( kv+1, jj ), ldab-1,ab( kv+jp, jj ), &
                                       ldab-1 )
                          end if
                       end if
                       ! compute multipliers
                       call stdlib_wscal( km, cone / ab( kv+1, jj ), ab( kv+2, jj ),1_ilp )
                       ! update trailing submatrix within the band and within
                       ! the current block. jm is the index of the last column
                       ! which needs to be updated.
                       jm = min( ju, j+jb-1 )
                       if( jm>jj )call stdlib_wgeru( km, jm-jj, -cone, ab( kv+2, jj ), 1_ilp,ab( kv, &
                                 jj+1 ), ldab-1,ab( kv+1, jj+1 ), ldab-1 )
                    else
                       ! if pivot is czero, set info to the index of the pivot
                       ! unless a czero pivot has already been found.
                    ...
                    ! copy current column of a31 into the work array work31
                    nw = min( jj-j+1, i3 )
                    if( nw>0_ilp )call stdlib_wcopy( nw, ab( kv+kl+1-jj+j, jj ), 1_ilp,work31( 1_ilp, jj-j+1 )&
                              , 1_ilp )
                 end do loop_80
                 if( j+jb<=n ) then
                    ! apply the row interchanges to the other blocks.
                    ...
                    ! a32.
                    call stdlib_wlaswp( j2, ab( kv+1-jb, j+jb ), ldab-1, 1_ilp, jb,ipiv( j ), 1_ilp )
                    ...
                    if( j2>0_ilp ) then
                       ! update a12
                       call stdlib_wtrsm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'UNIT',jb, j2, cone, &
                                 ab( kv+1, j ), ldab-1,ab( kv+1-jb, j+jb ), ldab-1 )
                       if( i2>0_ilp ) then
                          ! update a22
                          call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', i2, j2,jb, -cone, ab(&
                           kv+1+jb, j ), ldab-1,ab( kv+1-jb, j+jb ), ldab-1, cone,ab( kv+1, j+jb )&
                                     , ldab-1 )
                       end if
                       if( i3>0_ilp ) then
                          ! update a32
                          call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', i3, j2,jb, -cone, &
                          work31, ldwork,ab( kv+1-jb, j+jb ), ldab-1, cone,ab( kv+kl+1-jb, j+jb ),&
                                     ldab-1 )
                       end if
                    end if
                    if( j3>0_ilp ) then
                       ...
                       ! update a13 in the work array
                       call stdlib_wtrsm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'UNIT',jb, j3, cone, &
                                 ab( kv+1, j ), ldab-1,work13, ldwork )
                       if( i2>0_ilp ) then
                          ! update a23
                       ...
                       if( i3>0_ilp ) then
                          ! update a33
                          call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', i3, j3,jb, -cone, &
                                    work31, ldwork, work13,ldwork, cone, ab( 1_ilp+kl, j+kv ), ldab-1 )
                       end if
                       ! copy the lower triangle of a13 back into place
                       do jj = 1, j3
                 ...
                 do jj = j + jb - 1, j, -1
                    ...
                    if( jp/=1_ilp ) then
                       ...
                       if( jp+jj-1<j+kl ) then
                          ! the interchange does not affect a31
                          call stdlib_wswap( jj-j, ab( kv+1+jj-j, j ), ldab-1,ab( kv+jp+jj-j, j ),&
                                     ldab-1 )
                       else
                          ! the interchange does affect a31
                          call stdlib_wswap( jj-j, ab( kv+1+jj-j, j ), ldab-1,work31( jp+jj-j-kl, &
                                    1_ilp ), ldwork )
                       end if
                    end if
                    ! copy the current column of a31 back into place
                    nw = min( i3, jj-j+1 )
                    if( nw>0_ilp )call stdlib_wcopy( nw, work31( 1_ilp, jj-j+1 ), 1_ilp,ab( kv+kl+1-jj+j, jj )&
                              , 1_ilp )
                 end do
              end do loop_180
           end if
     ...
     !! This is the unblocked version of the algorithm, calling Level 2 BLAS.
        ...
        ! =====================================================================
           ...
           loop_40: do j = 1, min( m, n )
              ...
              ! subdiagonal elements in the current column.
              km = min( kl, m-j )
              jp = stdlib_iqamax( km+1, ab( kv+1, j ), 1_ilp )
              ipiv( j ) = jp + j - 1_ilp
              if( ab( kv+jp, j )/=zero ) then
                 ju = max( ju, min( j+ku+jp-1, n ) )
                 ! apply interchange to columns j to ju.
                 if( jp/=1_ilp )call stdlib_qswap( ju-j+1, ab( kv+jp, j ), ldab-1,ab( kv+1, j ), ldab-&
                           1_ilp )
                 if( km>0_ilp ) then
                    ! compute multipliers.
                    call stdlib_qscal( km, one / ab( kv+1, j ), ab( kv+2, j ), 1_ilp )
                    ! update trailing submatrix within the band.
                    if( ju>j )call stdlib_qger( km, ju-j, -one, ab( kv+2, j ), 1_ilp,ab( kv, j+1 ), &
                              ldab-1, ab( kv+1, j+1 ),ldab-1 )
                 end if
              else
                 ! if pivot is zero, set info to the index of the pivot
     ...
     !! This is the unblocked version of the algorithm, calling Level 2 BLAS.
        ...
        ! =====================================================================
           ...
           loop_40: do j = 1, min( m, n )
              ...
              ! subdiagonal elements in the current column.
              km = min( kl, m-j )
              jp = stdlib_iwamax( km+1, ab( kv+1, j ), 1_ilp )
              ipiv( j ) = jp + j - 1_ilp
              if( ab( kv+jp, j )/=czero ) then
                 ju = max( ju, min( j+ku+jp-1, n ) )
                 ! apply interchange to columns j to ju.
                 if( jp/=1_ilp )call stdlib_wswap( ju-j+1, ab( kv+jp, j ), ldab-1,ab( kv+1, j ), ldab-&
                           1_ilp )
                 if( km>0_ilp ) then
                    ! compute multipliers.
                    call stdlib_wscal( km, cone / ab( kv+1, j ), ab( kv+2, j ), 1_ilp )
                    ! update trailing submatrix within the band.
                    if( ju>j )call stdlib_wgeru( km, ju-j, -cone, ab( kv+2, j ), 1_ilp,ab( kv, j+1 ), &
                              ldab-1, ab( kv+1, j+1 ),ldab-1 )
                 end if
              else
                 ! if pivot is czero, set info to the index of the pivot
           ...
           if( notran ) then
              ...
              if( lnoti ) then
                 do j = 1, n - 1
                    lm = min( kl, n-j )
                    l = ipiv( j )
                    if( l/=j )call stdlib_qswap( nrhs, b( l, 1_ilp ), ldb, b( j, 1_ilp ), ldb )
                    call stdlib_qger( lm, nrhs, -one, ab( kd+1, j ), 1_ilp, b( j, 1_ilp ),ldb, b( j+1, 1_ilp )&
                              , ldb )
                 end do
              end if
              do i = 1, nrhs
                 ! solve u*x = b, overwriting b with x.
                 call stdlib_qtbsv( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n, kl+ku,ab, ldab, b( 1_ilp, &
                           i ), 1_ilp )
              end do
           else
              ! solve a**t*x = b.
              do i = 1, nrhs
                 ! solve u**t*x = b, overwriting b with x.
                 call stdlib_qtbsv( 'UPPER', 'TRANSPOSE', 'NON-UNIT', n, kl+ku, ab,ldab, b( 1_ilp, i )&
                           , 1_ilp )
              end do
              ! solve l**t*x = b, overwriting b with x.
              if( lnoti ) then
                 do j = n - 1, 1, -1
                    lm = min( kl, n-j )
                    call stdlib_qgemv( 'TRANSPOSE', lm, nrhs, -one, b( j+1, 1_ilp ),ldb, ab( kd+1, j )&
                              , 1_ilp, one, b( j, 1_ilp ), ldb )
                    l = ipiv( j )
                    if( l/=j )call stdlib_qswap( nrhs, b( l, 1_ilp ), ldb, b( j, 1_ilp ), ldb )
                 end do
           ...
           if( notran ) then
              ...
              if( lnoti ) then
                 do j = 1, n - 1
                    lm = min( kl, n-j )
                    l = ipiv( j )
                    if( l/=j )call stdlib_wswap( nrhs, b( l, 1_ilp ), ldb, b( j, 1_ilp ), ldb )
                    call stdlib_wgeru( lm, nrhs, -cone, ab( kd+1, j ), 1_ilp, b( j, 1_ilp ),ldb, b( j+1, &
                              1_ilp ), ldb )
                 end do
              end if
              do i = 1, nrhs
                 ! solve u*x = b, overwriting b with x.
                 call stdlib_wtbsv( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n, kl+ku,ab, ldab, b( 1_ilp, &
                           i ), 1_ilp )
              end do
           else if( stdlib_lsame( trans, 'T' ) ) then
              ! solve a**t * x = b.
              do i = 1, nrhs
                 ! solve u**t * x = b, overwriting b with x.
                 call stdlib_wtbsv( 'UPPER', 'TRANSPOSE', 'NON-UNIT', n, kl+ku, ab,ldab, b( 1_ilp, i )&
                           , 1_ilp )
              end do
              ! solve l**t * x = b, overwriting b with x.
              if( lnoti ) then
                 do j = n - 1, 1, -1
                    lm = min( kl, n-j )
                    call stdlib_wgemv( 'TRANSPOSE', lm, nrhs, -cone, b( j+1, 1_ilp ),ldb, ab( kd+1, j &
                              ), 1_ilp, cone, b( j, 1_ilp ), ldb )
                    l = ipiv( j )
                    if( l/=j )call stdlib_wswap( nrhs, b( l, 1_ilp ), ldb, b( j, 1_ilp ), ldb )
                 end do
              ...
              do i = 1, nrhs
                 ! solve u**h * x = b, overwriting b with x.
                 call stdlib_wtbsv( 'UPPER', 'CONJUGATE TRANSPOSE', 'NON-UNIT', n,kl+ku, ab, ldab,&
                            b( 1_ilp, i ), 1_ilp )
              end do
              ! solve l**h * x = b, overwriting b with x.
              if( lnoti ) then
                 do j = n - 1, 1, -1
                    lm = min( kl, n-j )
                    call stdlib_wlacgv( nrhs, b( j, 1_ilp ), ldb )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', lm, nrhs, -cone,b( j+1, 1_ilp ), ldb, &
                              ab( kd+1, j ), 1_ilp, cone,b( j, 1_ilp ), ldb )
                    call stdlib_wlacgv( nrhs, b( j, 1_ilp ), ldb )
                    l = ipiv( j )
                    if( l/=j )call stdlib_wswap( nrhs, b( l, 1_ilp ), ldb, b( j, 1_ilp ), ldb )
     ...
     !! error bounds and backward error estimates for the solution.
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1, nrhs
              ...
              ! where op(a) = a, a**t, or a**h, depending on trans.
              call stdlib_qcopy( n, b( 1_ilp, j ), 1_ilp, work( n+1 ), 1_ilp )
              call stdlib_qgbmv( trans, n, n, kl, ku, -one, ab, ldab, x( 1_ilp, j ), 1_ilp,one, work( n+1 &
                        ), 1_ilp )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(op(a))*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_qgbtrs( trans, n, kl, ku, 1_ilp, afb, ldafb, ipiv,work( n+1 ), n, info )
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**t).
                    call stdlib_qgbtrs( transt, n, kl, ku, 1_ilp, afb, ldafb, ipiv,work( n+1 ), n, &
                              info )
                    do i = 1, n
                       work( n+i ) = work( n+i )*work( i )
                    end do
                    ...
                    call stdlib_qgbtrs( trans, n, kl, ku, 1_ilp, afb, ldafb, ipiv,work( n+1 ), n, &
                              info )
                 end if
                 go to 100
              end if
     ...
     !! error bounds and backward error estimates for the solution.
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1, nrhs
              ...
              ! where op(a) = a, a**t, or a**h, depending on trans.
              call stdlib_wcopy( n, b( 1_ilp, j ), 1_ilp, work, 1_ilp )
              call stdlib_wgbmv( trans, n, n, kl, ku, -cone, ab, ldab, x( 1_ilp, j ), 1_ilp,cone, work, 1_ilp &
                        )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(op(a))*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_wgbtrs( trans, n, kl, ku, 1_ilp, afb, ldafb, ipiv, work, n,info )
                 call stdlib_waxpy( n, cone, work, 1_ilp, x( 1_ilp, j ), 1_ilp )
                 lstres = berr( j )
                 count = count + 1_ilp
                 go to 20
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**h).
                    call stdlib_wgbtrs( transt, n, kl, ku, 1_ilp, afb, ldafb, ipiv,work, n, info )
                    ...
                    call stdlib_wgbtrs( transn, n, kl, ku, 1_ilp, afb, ldafb, ipiv,work, n, info )
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==2_ilp ) then
                 ...
                 if ( notrans ) then
                    call stdlib_qgbtrs( 'NO TRANSPOSE', n, kl, ku, 1_ilp, afb, ldafb,ipiv, work, n, &
                              info )
                 else
                    call stdlib_qgbtrs( 'TRANSPOSE', n, kl, ku, 1_ilp, afb, ldafb, ipiv,work, n, info &
                              )
                 end if
                 ! multiply by inv(c).
                 if ( cmode == 1_ilp ) then
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==kase1 ) then
                 ! multiply by inv(u)*inv(l).
                 call stdlib_qgttrs( 'NO TRANSPOSE', n, 1_ilp, dl, d, du, du2, ipiv,work, n, info )
                           
              else
                 ! multiply by inv(l**t)*inv(u**t).
                 call stdlib_qgttrs( 'TRANSPOSE', n, 1_ilp, dl, d, du, du2, ipiv, work,n, info )
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==kase1 ) then
                 ! multiply by inv(u)*inv(l).
                 call stdlib_wgttrs( 'NO TRANSPOSE', n, 1_ilp, dl, d, du, du2, ipiv,work, n, info )
                           
              else
                 ! multiply by inv(l**h)*inv(u**h).
                 call stdlib_wgttrs( 'CONJUGATE TRANSPOSE', n, 1_ilp, dl, d, du, du2,ipiv, work, n, &
                           info )
              end if
              go to 20
           end if
     ...
     !! by DGTTRF.
        ...
        ! =====================================================================
           ...
           if( nb>=nrhs ) then
              call stdlib_qgtts2( itrans, n, nrhs, dl, d, du, du2, ipiv, b, ldb )
           else
              do j = 1, nrhs, nb
                 jb = min( nrhs-j+1, nb )
                 call stdlib_qgtts2( itrans, n, jb, dl, d, du, du2, ipiv, b( 1_ilp, j ),ldb )
              end do
           end if
     ...
     !! by ZGTTRF.
        ...
        ! =====================================================================
           ...
           if( nb>=nrhs ) then
              call stdlib_wgtts2( itrans, n, nrhs, dl, d, du, du2, ipiv, b, ldb )
           else
              do j = 1, nrhs, nb
                 jb = min( nrhs-j+1, nb )
                 call stdlib_wgtts2( itrans, n, jb, dl, d, du, du2, ipiv, b( 1_ilp, j ),ldb )
              end do
           end if
     ...
     !! error bounds and backward error estimates for the solution.
        ...
        ! =====================================================================
           ...
           loop_110: do j = 1, nrhs
              ...
              ! where op(a) = a, a**t, or a**h, depending on trans.
              call stdlib_qcopy( n, b( 1_ilp, j ), 1_ilp, work( n+1 ), 1_ilp )
              call stdlib_qlagtm( trans, n, 1_ilp, -one, dl, d, du, x( 1_ilp, j ), ldx, one,work( n+1 ), &
                        n )
              ! compute abs(op(a))*abs(x) + abs(b) for use in the backward
              ! error bound.
              if( notran ) then
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_qgttrs( trans, n, 1_ilp, dlf, df, duf, du2, ipiv,work( n+1 ), n, info )
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**t).
                    call stdlib_qgttrs( transt, n, 1_ilp, dlf, df, duf, du2, ipiv,work( n+1 ), n, &
                              info )
                    do i = 1, n
                       work( n+i ) = work( i )*work( n+i )
                    end do
                    ...
                    call stdlib_qgttrs( transn, n, 1_ilp, dlf, df, duf, du2, ipiv,work( n+1 ), n, &
                              info )
                 end if
                 go to 70
              end if
     ...
     !! error bounds and backward error estimates for the solution.
        ...
        ! =====================================================================
           ...
           loop_110: do j = 1, nrhs
              ...
              ! where op(a) = a, a**t, or a**h, depending on trans.
              call stdlib_wcopy( n, b( 1_ilp, j ), 1_ilp, work, 1_ilp )
              call stdlib_wlagtm( trans, n, 1_ilp, -one, dl, d, du, x( 1_ilp, j ), ldx, one,work, n )
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_wgttrs( trans, n, 1_ilp, dlf, df, duf, du2, ipiv, work, n,info )
                 call stdlib_waxpy( n, cmplx( one,KIND=qp), work, 1_ilp, x( 1_ilp, j ), 1_ilp )
                 lstres = berr( j )
                 count = count + 1_ilp
                 go to 20
              end if
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! multiply by diag(w)*inv(op(a)**h).
                    call stdlib_wgttrs( transt, n, 1_ilp, dlf, df, duf, du2, ipiv, work,n, info )
                    ...
                    call stdlib_wgttrs( transn, n, 1_ilp, dlf, df, duf, du2, ipiv, work,n, info )
