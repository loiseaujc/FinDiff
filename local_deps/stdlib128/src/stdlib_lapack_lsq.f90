  ...
  contains
     ...
     !! value.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              ...
              if( minmn>0_ilp ) then
                 ...
                 if( m>=n .and. m>=mnthr ) then
                    ! path 1a - overdetermined, with many more rows than
                              ! columns
                    ! compute space needed for stdlib_qgeqrf
                    call stdlib_qgeqrf( m, n, a, lda, dum(1_ilp), dum(1_ilp), -1_ilp, info )
                    lwork_qgeqrf=dum(1_ilp)
                    ! compute space needed for stdlib_qormqr
                    call stdlib_qormqr( 'L', 'T', m, nrhs, n, a, lda, dum(1_ilp), b,ldb, dum(1_ilp), -1_ilp, &
                              info )
                    lwork_qormqr=dum(1_ilp)
                    mm = n
                 ...
                 if( m>=n ) then
                    ! path 1 - overdetermined or exactly determined
                    ! compute workspace needed for stdlib_qbdsqr
                    bdspac = max( 1_ilp, 5_ilp*n )
                    ! compute space needed for stdlib_qgebrd
                    call stdlib_qgebrd( mm, n, a, lda, s, dum(1_ilp), dum(1_ilp),dum(1_ilp), dum(1_ilp), -1_ilp, info &
                              )
                    lwork_qgebrd=dum(1_ilp)
                    ! compute space needed for stdlib_qormbr
                    call stdlib_qormbr( 'Q', 'L', 'T', mm, nrhs, n, a, lda, dum(1_ilp),b, ldb, dum(1_ilp),&
                               -1_ilp, info )
                    lwork_qormbr=dum(1_ilp)
                    ! compute space needed for stdlib_qorgbr
                    call stdlib_qorgbr( 'P', n, n, n, a, lda, dum(1_ilp),dum(1_ilp), -1_ilp, info )
                    lwork_qorgbr=dum(1_ilp)
                    ! compute total workspace needed
                    maxwrk = max( maxwrk, 3_ilp*n + lwork_qgebrd )
                    maxwrk = max( maxwrk, 3_ilp*n + lwork_qormbr )
                    maxwrk = max( maxwrk, 3_ilp*n + lwork_qorgbr )
                    maxwrk = max( maxwrk, bdspac )
                    maxwrk = max( maxwrk, n*nrhs )
                    minwrk = max( 3_ilp*n + mm, 3_ilp*n + nrhs, bdspac )
                    maxwrk = max( minwrk, maxwrk )
                 ...
                 if( n>m ) then
                    ...
                    if( n>=mnthr ) then
                       ! path 2a - underdetermined, with many more columns
                       ! than rows
                       ! compute space needed for stdlib_qgelqf
                       call stdlib_qgelqf( m, n, a, lda, dum(1_ilp), dum(1_ilp),-1_ilp, info )
                       lwork_qgelqf=dum(1_ilp)
                       ! compute space needed for stdlib_qgebrd
                       call stdlib_qgebrd( m, m, a, lda, s, dum(1_ilp), dum(1_ilp),dum(1_ilp), dum(1_ilp), -1_ilp, &
                                 info )
                       lwork_qgebrd=dum(1_ilp)
                       ! compute space needed for stdlib_qormbr
                       call stdlib_qormbr( 'Q', 'L', 'T', m, nrhs, n, a, lda,dum(1_ilp), b, ldb, dum(&
                                 1_ilp), -1_ilp, info )
                       lwork_qormbr=dum(1_ilp)
                       ! compute space needed for stdlib_qorgbr
                       call stdlib_qorgbr( 'P', m, m, m, a, lda, dum(1_ilp),dum(1_ilp), -1_ilp, info )
                       lwork_qorgbr=dum(1_ilp)
                       ! compute space needed for stdlib_qormlq
                       call stdlib_qormlq( 'L', 'T', n, nrhs, m, a, lda, dum(1_ilp),b, ldb, dum(1_ilp), -&
                                 1_ilp, info )
                       lwork_qormlq=dum(1_ilp)
                       ! compute total workspace needed
                       maxwrk = m + lwork_qgelqf
                       maxwrk = max( maxwrk, m*m + 4_ilp*m + lwork_qgebrd )
                       maxwrk = max( maxwrk, m*m + 4_ilp*m + lwork_qormbr )
                       maxwrk = max( maxwrk, m*m + 4_ilp*m + lwork_qorgbr )
                       maxwrk = max( maxwrk, m*m + m + bdspac )
                       if( nrhs>1_ilp ) then
                          maxwrk = max( maxwrk, m*m + m + m*nrhs )
                       else
                       ...
                       ! path 2 - underdetermined
                       ! compute space needed for stdlib_qgebrd
                       call stdlib_qgebrd( m, n, a, lda, s, dum(1_ilp), dum(1_ilp),dum(1_ilp), dum(1_ilp), -1_ilp, &
                                 info )
                       lwork_qgebrd=dum(1_ilp)
                       ! compute space needed for stdlib_qormbr
                       call stdlib_qormbr( 'Q', 'L', 'T', m, nrhs, m, a, lda,dum(1_ilp), b, ldb, dum(&
                                 1_ilp), -1_ilp, info )
                       lwork_qormbr=dum(1_ilp)
                       ! compute space needed for stdlib_qorgbr
                       call stdlib_qorgbr( 'P', m, n, m, a, lda, dum(1_ilp),dum(1_ilp), -1_ilp, info )
                       lwork_qorgbr=dum(1_ilp)
                       maxwrk = 3_ilp*m + lwork_qgebrd
                       maxwrk = max( maxwrk, 3_ilp*m + lwork_qormbr )
                       maxwrk = max( maxwrk, 3_ilp*m + lwork_qorgbr )
                       maxwrk = max( maxwrk, bdspac )
                       maxwrk = max( maxwrk, n*nrhs )
                    end if
                 end if
           ...
           if( anrm>zero .and. anrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, anrm, smlnum, m, n, a, lda, info )
              iascl = 1_ilp
           else if( anrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, anrm, bignum, m, n, a, lda, info )
              iascl = 2_ilp
           else if( anrm==zero ) then
              ! matrix all zero. return zero solution.
              call stdlib_qlaset( 'F', max( m, n ), nrhs, zero, zero, b, ldb )
              call stdlib_qlaset( 'F', minmn, 1_ilp, zero, zero, s, minmn )
              rank = 0_ilp
              go to 70
           end if
           ! scale b if max element outside range [smlnum,bignum]
           bnrm = stdlib_qlange( 'M', m, nrhs, b, ldb, work )
           ibscl = 0_ilp
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, bnrm, smlnum, m, nrhs, b, ldb, info )
              ibscl = 1_ilp
           else if( bnrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, bnrm, bignum, m, nrhs, b, ldb, info )
              ibscl = 2_ilp
           end if
           ! overdetermined case
           if( m>=n ) then
              ...
              if( m>=mnthr ) then
                 ...
                 ! compute a=q*r
                 ! (workspace: need 2*n, prefer n+n*nb)
                 call stdlib_qgeqrf( m, n, a, lda, work( itau ), work( iwork ),lwork-iwork+1, &
                           info )
                 ! multiply b by transpose(q)
                 ! (workspace: need n+nrhs, prefer n+nrhs*nb)
                 call stdlib_qormqr( 'L', 'T', m, nrhs, n, a, lda, work( itau ), b,ldb, work( &
                           iwork ), lwork-iwork+1, info )
                 ! zero out below r
                 if( n>1_ilp )call stdlib_qlaset( 'L', n-1, n-1, zero, zero, a( 2_ilp, 1_ilp ), lda )
              end if
              ie = 1_ilp
              itauq = ie + n
              itaup = itauq + n
              ...
              ! bidiagonalize r in a
              ! (workspace: need 3*n+mm, prefer 3*n+(mm+n)*nb)
              call stdlib_qgebrd( mm, n, a, lda, s, work( ie ), work( itauq ),work( itaup ), work(&
                         iwork ), lwork-iwork+1,info )
              ! multiply b by transpose of left bidiagonalizing vectors of r
              ! (workspace: need 3*n+nrhs, prefer 3*n+nrhs*nb)
              call stdlib_qormbr( 'Q', 'L', 'T', mm, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        iwork ), lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors of r in a
              ! (workspace: need 4*n-1, prefer 3*n+(n-1)*nb)
              call stdlib_qorgbr( 'P', n, n, n, a, lda, work( itaup ),work( iwork ), lwork-iwork+&
                        1_ilp, info )
              iwork = ie + n
              ! perform bidiagonal qr iteration
                ! multiply b by transpose of left singular vectors
                ! compute right singular vectors in a
              ! (workspace: need bdspac)
              call stdlib_qbdsqr( 'U', n, n, 0_ilp, nrhs, s, work( ie ), a, lda, dum,1_ilp, b, ldb, work( &
                        iwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp ), sfmin )
              ...
              do i = 1, n
                 if( s( i )>thr ) then
                    call stdlib_qrscl( nrhs, s( i ), b( i, 1_ilp ), ldb )
                    rank = rank + 1_ilp
                 else
                    call stdlib_qlaset( 'F', 1_ilp, nrhs, zero, zero, b( i, 1_ilp ), ldb )
                 end if
              end do
              ! multiply b by right singular vectors
              ! (workspace: need n, prefer n*nrhs)
              if( lwork>=ldb*nrhs .and. nrhs>1_ilp ) then
                 ...
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_qgemm( 'T', 'N', n, bl, n, one, a, lda, b( 1_ilp, i ),ldb, zero, work,&
                               n )
                    call stdlib_qlacpy( 'G', n, bl, work, n, b( 1_ilp, i ), ldb )
                 end do
              else
                 call stdlib_qgemv( 'T', n, n, one, a, lda, b, 1_ilp, zero, work, 1_ilp )
                 call stdlib_qcopy( n, work, 1_ilp, b, 1_ilp )
              end if
           else if( n>=mnthr .and. lwork>=4_ilp*m+m*m+max( m, 2_ilp*m-4, nrhs, n-3*m ) ) then
              ! path 2a - underdetermined, with many more columns than rows
              ! and sufficient workspace for an efficient algorithm
              ...
              call stdlib_qlaset( 'U', m-1, m-1, zero, zero, work( il+ldwork ),ldwork )
              ie = il + ldwork*m
              itauq = ie + m
              itaup = itauq + m
              iwork = itaup + m
              ! bidiagonalize l in work(il)
              ! (workspace: need m*m+5*m, prefer m*m+4*m+2*m*nb)
              call stdlib_qgebrd( m, m, work( il ), ldwork, s, work( ie ),work( itauq ), work( &
                        itaup ), work( iwork ),lwork-iwork+1, info )
              ! multiply b by transpose of left bidiagonalizing vectors of l
              ! (workspace: need m*m+4*m+nrhs, prefer m*m+4*m+nrhs*nb)
              call stdlib_qormbr( 'Q', 'L', 'T', m, nrhs, m, work( il ), ldwork,work( itauq ), b, &
                        ldb, work( iwork ),lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors of r in work(il)
              ! (workspace: need m*m+5*m-1, prefer m*m+4*m+(m-1)*nb)
              call stdlib_qorgbr( 'P', m, m, m, work( il ), ldwork, work( itaup ),work( iwork ), &
                        lwork-iwork+1, info )
              iwork = ie + m
              ! perform bidiagonal qr iteration,
                 ! computing right singular vectors of l in work(il) and
                 ! multiplying b by transpose of left singular vectors
              ! (workspace: need m*m+m+bdspac)
              call stdlib_qbdsqr( 'U', m, m, 0_ilp, nrhs, s, work( ie ), work( il ),ldwork, a, lda, b,&
                         ldb, work( iwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp ), sfmin )
              ...
              ! (workspace: need m*m+2*m, prefer m*m+m+m*nrhs)
              if( lwork>=ldb*nrhs+iwork-1 .and. nrhs>1_ilp ) then
                 call stdlib_qgemm( 'T', 'N', m, nrhs, m, one, work( il ), ldwork,b, ldb, zero, &
                           work( iwork ), ldb )
                 call stdlib_qlacpy( 'G', m, nrhs, work( iwork ), ldb, b, ldb )
              else if( nrhs>1_ilp ) then
                 chunk = ( lwork-iwork+1 ) / m
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_qgemm( 'T', 'N', m, bl, m, one, work( il ), ldwork,b( 1_ilp, i ), ldb,&
                               zero, work( iwork ), m )
                    call stdlib_qlacpy( 'G', m, bl, work( iwork ), m, b( 1_ilp, i ),ldb )
                 end do
              else
                 call stdlib_qgemv( 'T', m, m, one, work( il ), ldwork, b( 1_ilp, 1_ilp ),1_ilp, zero, work( &
                           iwork ), 1_ilp )
                 call stdlib_qcopy( m, work( iwork ), 1_ilp, b( 1_ilp, 1_ilp ), 1_ilp )
              end if
              ! zero out below first m rows of b
              call stdlib_qlaset( 'F', n-m, nrhs, zero, zero, b( m+1, 1_ilp ), ldb )
              iwork = itau + m
              ! multiply transpose(q) by b
              ! (workspace: need m+nrhs, prefer m+nrhs*nb)
              call stdlib_qormlq( 'L', 'T', n, nrhs, m, a, lda, work( itau ), b,ldb, work( iwork )&
                        , lwork-iwork+1, info )
           else
              ! path 2 - remaining underdetermined cases
              ie = 1_ilp
              ...
              ! bidiagonalize a
              ! (workspace: need 3*m+n, prefer 3*m+(m+n)*nb)
              call stdlib_qgebrd( m, n, a, lda, s, work( ie ), work( itauq ),work( itaup ), work( &
                        iwork ), lwork-iwork+1,info )
              ! multiply b by transpose of left bidiagonalizing vectors
              ! (workspace: need 3*m+nrhs, prefer 3*m+nrhs*nb)
              call stdlib_qormbr( 'Q', 'L', 'T', m, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        iwork ), lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors in a
              ! (workspace: need 4*m, prefer 3*m+m*nb)
              call stdlib_qorgbr( 'P', m, n, m, a, lda, work( itaup ),work( iwork ), lwork-iwork+&
                        1_ilp, info )
              iwork = ie + m
              ! perform bidiagonal qr iteration,
                 ! computing right singular vectors of a in a and
                 ! multiplying b by transpose of left singular vectors
              ! (workspace: need bdspac)
              call stdlib_qbdsqr( 'L', m, n, 0_ilp, nrhs, s, work( ie ), a, lda, dum,1_ilp, b, ldb, work( &
                        iwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp ), sfmin )
              ...
              if( lwork>=ldb*nrhs .and. nrhs>1_ilp ) then
                 ...
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_qgemm( 'T', 'N', n, bl, m, one, a, lda, b( 1_ilp, i ),ldb, zero, work,&
                               n )
                    call stdlib_qlacpy( 'F', n, bl, work, n, b( 1_ilp, i ), ldb )
                 end do
              else
                 call stdlib_qgemv( 'T', m, n, one, a, lda, b, 1_ilp, zero, work, 1_ilp )
                 call stdlib_qcopy( n, work, 1_ilp, b, 1_ilp )
              end if
           end if
           ! undo scaling
           if( iascl==1_ilp ) then
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, anrm, smlnum, n, nrhs, b, ldb, info )
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, smlnum, anrm, minmn, 1_ilp, s, minmn,info )
           else if( iascl==2_ilp ) then
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, anrm, bignum, n, nrhs, b, ldb, info )
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, bignum, anrm, minmn, 1_ilp, s, minmn,info )
           end if
           if( ibscl==1_ilp ) then
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, smlnum, bnrm, n, nrhs, b, ldb, info )
           else if( ibscl==2_ilp ) then
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, bignum, bnrm, n, nrhs, b, ldb, info )
           end if
           70 continue
           work( 1_ilp ) = maxwrk
           return
     ...
     !! value.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              ...
              if( minmn>0_ilp ) then
                 ...
                 if( m>=n .and. m>=mnthr ) then
                    ! path 1a - overdetermined, with many more rows than
                              ! columns
                    ! compute space needed for stdlib_wgeqrf
                    call stdlib_wgeqrf( m, n, a, lda, dum(1_ilp), dum(1_ilp), -1_ilp, info )
                    lwork_wgeqrf = real( dum(1_ilp),KIND=qp)
                    ! compute space needed for stdlib_wunmqr
                    call stdlib_wunmqr( 'L', 'C', m, nrhs, n, a, lda, dum(1_ilp), b,ldb, dum(1_ilp), -1_ilp, &
                              info )
                    lwork_wunmqr = real( dum(1_ilp),KIND=qp)
                    mm = n
                    maxwrk = max( maxwrk, n + n*stdlib_ilaenv( 1_ilp, 'ZGEQRF', ' ', m,n, -1_ilp, -1_ilp ) )
                 ...
                 if( m>=n ) then
                    ...
                    call stdlib_wgebrd( mm, n, a, lda, s, s, dum(1_ilp), dum(1_ilp), dum(1_ilp),-1_ilp, info )
                              
                    lwork_wgebrd = real( dum(1_ilp),KIND=qp)
                    ! compute space needed for stdlib_wunmbr
                    call stdlib_wunmbr( 'Q', 'L', 'C', mm, nrhs, n, a, lda, dum(1_ilp),b, ldb, dum(1_ilp),&
                               -1_ilp, info )
                    lwork_wunmbr = real( dum(1_ilp),KIND=qp)
                    ! compute space needed for stdlib_wungbr
                    call stdlib_wungbr( 'P', n, n, n, a, lda, dum(1_ilp),dum(1_ilp), -1_ilp, info )
                    lwork_wungbr = real( dum(1_ilp),KIND=qp)
                    ! compute total workspace needed
                    maxwrk = max( maxwrk, 2_ilp*n + lwork_wgebrd )
                    maxwrk = max( maxwrk, 2_ilp*n + lwork_wunmbr )
                    maxwrk = max( maxwrk, 2_ilp*n + lwork_wungbr )
                    maxwrk = max( maxwrk, n*nrhs )
                    minwrk = 2_ilp*n + max( nrhs, m )
                 end if
                 if( n>m ) then
                    ...
                    if( n>=mnthr ) then
                       ! path 2a - underdetermined, with many more columns
                       ! than rows
                       ! compute space needed for stdlib_wgelqf
                       call stdlib_wgelqf( m, n, a, lda, dum(1_ilp), dum(1_ilp),-1_ilp, info )
                       lwork_wgelqf = real( dum(1_ilp),KIND=qp)
                       ! compute space needed for stdlib_wgebrd
                       call stdlib_wgebrd( m, m, a, lda, s, s, dum(1_ilp), dum(1_ilp),dum(1_ilp), -1_ilp, info )
                                 
                       lwork_wgebrd = real( dum(1_ilp),KIND=qp)
                       ! compute space needed for stdlib_wunmbr
                       call stdlib_wunmbr( 'Q', 'L', 'C', m, nrhs, n, a, lda,dum(1_ilp), b, ldb, dum(&
                                 1_ilp), -1_ilp, info )
                       lwork_wunmbr = real( dum(1_ilp),KIND=qp)
                       ! compute space needed for stdlib_wungbr
                       call stdlib_wungbr( 'P', m, m, m, a, lda, dum(1_ilp),dum(1_ilp), -1_ilp, info )
                       lwork_wungbr = real( dum(1_ilp),KIND=qp)
                       ! compute space needed for stdlib_wunmlq
                       call stdlib_wunmlq( 'L', 'C', n, nrhs, m, a, lda, dum(1_ilp),b, ldb, dum(1_ilp), -&
                                 1_ilp, info )
                       lwork_wunmlq = real( dum(1_ilp),KIND=qp)
                       ! compute total workspace needed
                       maxwrk = m + lwork_wgelqf
                       maxwrk = max( maxwrk, 3_ilp*m + m*m + lwork_wgebrd )
                       maxwrk = max( maxwrk, 3_ilp*m + m*m + lwork_wunmbr )
                       maxwrk = max( maxwrk, 3_ilp*m + m*m + lwork_wungbr )
                       if( nrhs>1_ilp ) then
                          maxwrk = max( maxwrk, m*m + m + m*nrhs )
                       else
                          maxwrk = max( maxwrk, m*m + 2_ilp*m )
                       ...
                       ! path 2 - underdetermined
                       ! compute space needed for stdlib_wgebrd
                       call stdlib_wgebrd( m, n, a, lda, s, s, dum(1_ilp), dum(1_ilp),dum(1_ilp), -1_ilp, info )
                                 
                       lwork_wgebrd = real( dum(1_ilp),KIND=qp)
                       ! compute space needed for stdlib_wunmbr
                       call stdlib_wunmbr( 'Q', 'L', 'C', m, nrhs, m, a, lda,dum(1_ilp), b, ldb, dum(&
                                 1_ilp), -1_ilp, info )
                       lwork_wunmbr = real( dum(1_ilp),KIND=qp)
                       ! compute space needed for stdlib_wungbr
                       call stdlib_wungbr( 'P', m, n, m, a, lda, dum(1_ilp),dum(1_ilp), -1_ilp, info )
                       lwork_wungbr = real( dum(1_ilp),KIND=qp)
                       maxwrk = 2_ilp*m + lwork_wgebrd
                       maxwrk = max( maxwrk, 2_ilp*m + lwork_wunmbr )
                       maxwrk = max( maxwrk, 2_ilp*m + lwork_wungbr )
                       maxwrk = max( maxwrk, n*nrhs )
                    end if
                 end if
                 maxwrk = max( minwrk, maxwrk )
           ...
           call stdlib_qlabad( smlnum, bignum )
           ! scale a if max element outside range [smlnum,bignum]
           anrm = stdlib_wlange( 'M', m, n, a, lda, rwork )
           iascl = 0_ilp
           if( anrm>zero .and. anrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, anrm, smlnum, m, n, a, lda, info )
              iascl = 1_ilp
           else if( anrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, anrm, bignum, m, n, a, lda, info )
              iascl = 2_ilp
           else if( anrm==zero ) then
              ! matrix all zero. return zero solution.
              call stdlib_wlaset( 'F', max( m, n ), nrhs, czero, czero, b, ldb )
              call stdlib_qlaset( 'F', minmn, 1_ilp, zero, zero, s, minmn )
              rank = 0_ilp
              go to 70
           end if
           ! scale b if max element outside range [smlnum,bignum]
           bnrm = stdlib_wlange( 'M', m, nrhs, b, ldb, rwork )
           ibscl = 0_ilp
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, bnrm, smlnum, m, nrhs, b, ldb, info )
              ibscl = 1_ilp
           else if( bnrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, bnrm, bignum, m, nrhs, b, ldb, info )
              ibscl = 2_ilp
           end if
           ! overdetermined case
           if( m>=n ) then
              ...
              if( m>=mnthr ) then
                 ...
                 ! (cworkspace: need 2*n, prefer n+n*nb)
                 ! (rworkspace: none)
                 call stdlib_wgeqrf( m, n, a, lda, work( itau ), work( iwork ),lwork-iwork+1, &
                           info )
                 ! multiply b by transpose(q)
                 ! (cworkspace: need n+nrhs, prefer n+nrhs*nb)
                 ! (rworkspace: none)
                 call stdlib_wunmqr( 'L', 'C', m, nrhs, n, a, lda, work( itau ), b,ldb, work( &
                           iwork ), lwork-iwork+1, info )
                 ! zero out below r
                 if( n>1_ilp )call stdlib_wlaset( 'L', n-1, n-1, czero, czero, a( 2_ilp, 1_ilp ),lda )
              end if
              ie = 1_ilp
              itauq = 1_ilp
              itaup = itauq + n
              ...
              ! (cworkspace: need 2*n+mm, prefer 2*n+(mm+n)*nb)
              ! (rworkspace: need n)
              call stdlib_wgebrd( mm, n, a, lda, s, rwork( ie ), work( itauq ),work( itaup ), &
                        work( iwork ), lwork-iwork+1,info )
              ! multiply b by transpose of left bidiagonalizing vectors of r
              ! (cworkspace: need 2*n+nrhs, prefer 2*n+nrhs*nb)
              ! (rworkspace: none)
              call stdlib_wunmbr( 'Q', 'L', 'C', mm, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        iwork ), lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors of r in a
              ! (cworkspace: need 3*n-1, prefer 2*n+(n-1)*nb)
              ! (rworkspace: none)
              call stdlib_wungbr( 'P', n, n, n, a, lda, work( itaup ),work( iwork ), lwork-iwork+&
                        1_ilp, info )
              irwork = ie + n
              ! perform bidiagonal qr iteration
                ! multiply b by transpose of left singular vectors
              ...
              ! (cworkspace: none)
              ! (rworkspace: need bdspac)
              call stdlib_wbdsqr( 'U', n, n, 0_ilp, nrhs, s, rwork( ie ), a, lda, dum,1_ilp, b, ldb, &
                        rwork( irwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp ), sfmin )
              ...
              do i = 1, n
                 if( s( i )>thr ) then
                    call stdlib_wdrscl( nrhs, s( i ), b( i, 1_ilp ), ldb )
                    rank = rank + 1_ilp
                 else
                    call stdlib_wlaset( 'F', 1_ilp, nrhs, czero, czero, b( i, 1_ilp ), ldb )
                 end if
              end do
              ! multiply b by right singular vectors
              ! (cworkspace: need n, prefer n*nrhs)
              ...
              if( lwork>=ldb*nrhs .and. nrhs>1_ilp ) then
                 ...
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_wgemm( 'C', 'N', n, bl, n, cone, a, lda, b( 1_ilp, i ),ldb, czero, &
                              work, n )
                    call stdlib_wlacpy( 'G', n, bl, work, n, b( 1_ilp, i ), ldb )
                 end do
              else
                 call stdlib_wgemv( 'C', n, n, cone, a, lda, b, 1_ilp, czero, work, 1_ilp )
                 call stdlib_wcopy( n, work, 1_ilp, b, 1_ilp )
              end if
           else if( n>=mnthr .and. lwork>=3_ilp*m+m*m+max( m, nrhs, n-2*m ) )then
              ! underdetermined case, m much less than n
              ! path 2a - underdetermined, with many more columns than rows
              ...
              call stdlib_wlaset( 'U', m-1, m-1, czero, czero, work( il+ldwork ),ldwork )
              ie = 1_ilp
              itauq = il + ldwork*m
              itaup = itauq + m
              iwork = itaup + m
              ...
              ! (cworkspace: need m*m+4*m, prefer m*m+3*m+2*m*nb)
              ! (rworkspace: need m)
              call stdlib_wgebrd( m, m, work( il ), ldwork, s, rwork( ie ),work( itauq ), work( &
                        itaup ), work( iwork ),lwork-iwork+1, info )
              ! multiply b by transpose of left bidiagonalizing vectors of l
              ! (cworkspace: need m*m+3*m+nrhs, prefer m*m+3*m+nrhs*nb)
              ! (rworkspace: none)
              call stdlib_wunmbr( 'Q', 'L', 'C', m, nrhs, m, work( il ), ldwork,work( itauq ), b, &
                        ldb, work( iwork ),lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors of r in work(il)
              ! (cworkspace: need m*m+4*m-1, prefer m*m+3*m+(m-1)*nb)
              ! (rworkspace: none)
              call stdlib_wungbr( 'P', m, m, m, work( il ), ldwork, work( itaup ),work( iwork ), &
                        lwork-iwork+1, info )
              irwork = ie + m
              ! perform bidiagonal qr iteration, computing right singular
              ! vectors of l in work(il) and multiplying b by transpose of
              ...
              ! (cworkspace: need m*m)
              ! (rworkspace: need bdspac)
              call stdlib_wbdsqr( 'U', m, m, 0_ilp, nrhs, s, rwork( ie ), work( il ),ldwork, a, lda, &
                        b, ldb, rwork( irwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp ), sfmin )
              ...
              ! (rworkspace: none)
              if( lwork>=ldb*nrhs+iwork-1 .and. nrhs>1_ilp ) then
                 call stdlib_wgemm( 'C', 'N', m, nrhs, m, cone, work( il ), ldwork,b, ldb, czero, &
                           work( iwork ), ldb )
                 call stdlib_wlacpy( 'G', m, nrhs, work( iwork ), ldb, b, ldb )
              else if( nrhs>1_ilp ) then
                 chunk = ( lwork-iwork+1 ) / m
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_wgemm( 'C', 'N', m, bl, m, cone, work( il ), ldwork,b( 1_ilp, i ), &
                              ldb, czero, work( iwork ), m )
                    call stdlib_wlacpy( 'G', m, bl, work( iwork ), m, b( 1_ilp, i ),ldb )
                 end do
              else
                 call stdlib_wgemv( 'C', m, m, cone, work( il ), ldwork, b( 1_ilp, 1_ilp ),1_ilp, czero, work(&
                            iwork ), 1_ilp )
                 call stdlib_wcopy( m, work( iwork ), 1_ilp, b( 1_ilp, 1_ilp ), 1_ilp )
              end if
              ! zero out below first m rows of b
              call stdlib_wlaset( 'F', n-m, nrhs, czero, czero, b( m+1, 1_ilp ), ldb )
              iwork = itau + m
              ! multiply transpose(q) by b
              ! (cworkspace: need m+nrhs, prefer m+nhrs*nb)
              ! (rworkspace: none)
              call stdlib_wunmlq( 'L', 'C', n, nrhs, m, a, lda, work( itau ), b,ldb, work( iwork )&
                        , lwork-iwork+1, info )
           else
              ! path 2 - remaining underdetermined cases
              ie = 1_ilp
              ...
              ! (cworkspace: need 3*m, prefer 2*m+(m+n)*nb)
              ! (rworkspace: need n)
              call stdlib_wgebrd( m, n, a, lda, s, rwork( ie ), work( itauq ),work( itaup ), work(&
                         iwork ), lwork-iwork+1,info )
              ! multiply b by transpose of left bidiagonalizing vectors
              ! (cworkspace: need 2*m+nrhs, prefer 2*m+nrhs*nb)
              ! (rworkspace: none)
              call stdlib_wunmbr( 'Q', 'L', 'C', m, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        iwork ), lwork-iwork+1, info )
              ! generate right bidiagonalizing vectors in a
              ! (cworkspace: need 3*m, prefer 2*m+m*nb)
              ! (rworkspace: none)
              call stdlib_wungbr( 'P', m, n, m, a, lda, work( itaup ),work( iwork ), lwork-iwork+&
                        1_ilp, info )
              irwork = ie + m
              ! perform bidiagonal qr iteration,
                 ! computing right singular vectors of a in a and
              ...
              ! (cworkspace: none)
              ! (rworkspace: need bdspac)
              call stdlib_wbdsqr( 'L', m, n, 0_ilp, nrhs, s, rwork( ie ), a, lda, dum,1_ilp, b, ldb, &
                        rwork( irwork ), info )
              if( info/=0 )go to 70
              ! multiply b by reciprocals of singular values
              thr = max( rcond*s( 1_ilp ), sfmin )
              ...
              if( lwork>=ldb*nrhs .and. nrhs>1_ilp ) then
                 ...
                 do i = 1, nrhs, chunk
                    bl = min( nrhs-i+1, chunk )
                    call stdlib_wgemm( 'C', 'N', n, bl, m, cone, a, lda, b( 1_ilp, i ),ldb, czero, &
                              work, n )
                    call stdlib_wlacpy( 'F', n, bl, work, n, b( 1_ilp, i ), ldb )
                 end do
              else
                 call stdlib_wgemv( 'C', m, n, cone, a, lda, b, 1_ilp, czero, work, 1_ilp )
                 call stdlib_wcopy( n, work, 1_ilp, b, 1_ilp )
              end if
           end if
           ! undo scaling
           if( iascl==1_ilp ) then
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, anrm, smlnum, n, nrhs, b, ldb, info )
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, smlnum, anrm, minmn, 1_ilp, s, minmn,info )
           else if( iascl==2_ilp ) then
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, anrm, bignum, n, nrhs, b, ldb, info )
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, bignum, anrm, minmn, 1_ilp, s, minmn,info )
           end if
           if( ibscl==1_ilp ) then
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, smlnum, bnrm, n, nrhs, b, ldb, info )
           else if( ibscl==2_ilp ) then
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, bignum, bnrm, n, nrhs, b, ldb, info )
           end if
           70 continue
           work( 1_ilp ) = maxwrk
           return
     ...
     !! more simple.
        ...
        ! =====================================================================
           ...
           ! get machine parameters
           smlnum = stdlib_qlamch( 'S' ) / stdlib_qlamch( 'P' )
           bignum = one / smlnum
           call stdlib_qlabad( smlnum, bignum )
           ! scale a, b if max entries outside range [smlnum,bignum]
           anrm = stdlib_qlange( 'M', m, n, a, lda, work )
           ...
           ! compute qr factorization with column pivoting of a:
              ! a * p = q * r
           call stdlib_qgeqp3( m, n, a, lda, jpvt, work( 1_ilp ), work( mn+1 ),lwork-mn, info )
           ...
           if( rank<mn ) then
              i = rank + 1_ilp
              call stdlib_qlaic1( imin, rank, work( ismin ), smin, a( 1_ilp, i ),a( i, i ), sminpr, &
                        s1, c1 )
              call stdlib_qlaic1( imax, rank, work( ismax ), smax, a( 1_ilp, i ),a( i, i ), smaxpr, &
                        s2, c2 )
              if( smaxpr*rcond<=sminpr ) then
                 do i = 1, rank
                    work( ismin+i-1 ) = s1*work( ismin+i-1 )
           ...
           ! where r11 = r(1:rank,1:rank)
           ! [r11,r12] = [ t11, 0 ] * y
           if( rank<n )call stdlib_qtzrzf( rank, n, a, lda, work( mn+1 ), work( 2_ilp*mn+1 ),lwork-&
                     2_ilp*mn, info )
           ! workspace: 2*mn.
           ! details of householder rotations stored in work(mn+1:2*mn)
           ! b(1:m,1:nrhs) := q**t * b(1:m,1:nrhs)
           call stdlib_qormqr( 'LEFT', 'TRANSPOSE', m, nrhs, mn, a, lda, work( 1_ilp ),b, ldb, work( &
                     2_ilp*mn+1 ), lwork-2*mn, info )
           wsize = max( wsize, 2_ilp*mn+work( 2_ilp*mn+1 ) )
           ! workspace: 2*mn+nb*nrhs.
           ! b(1:rank,1:nrhs) := inv(t11) * b(1:rank,1:nrhs)
           call stdlib_qtrsm( 'LEFT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', rank,nrhs, one, a, lda,&
                      b, ldb )
           do j = 1, nrhs
              do i = rank + 1, n
                 b( i, j ) = zero
           ...
           ! b(1:n,1:nrhs) := y**t * b(1:n,1:nrhs)
           if( rank<n ) then
              call stdlib_qormrz( 'LEFT', 'TRANSPOSE', n, nrhs, rank, n-rank, a,lda, work( mn+1 ),&
                         b, ldb, work( 2_ilp*mn+1 ),lwork-2*mn, info )
           end if
           ! workspace: 2*mn+nrhs.
           ! b(1:n,1:nrhs) := p * b(1:n,1:nrhs)
           do j = 1, nrhs
              ...
              call stdlib_qcopy( n, work( 1_ilp ), 1_ilp, b( 1_ilp, j ), 1_ilp )
           end do
           ! workspace: n.
           ! undo scaling
           if( iascl==1_ilp ) then
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, anrm, smlnum, n, nrhs, b, ldb, info )
              call stdlib_qlascl( 'U', 0_ilp, 0_ilp, smlnum, anrm, rank, rank, a, lda,info )
           else if( iascl==2_ilp ) then
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, anrm, bignum, n, nrhs, b, ldb, info )
              call stdlib_qlascl( 'U', 0_ilp, 0_ilp, bignum, anrm, rank, rank, a, lda,info )
           end if
           if( ibscl==1_ilp ) then
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, smlnum, bnrm, n, nrhs, b, ldb, info )
           else if( ibscl==2_ilp ) then
     ...
     !! o Matrix B (the right hand side) is updated with Blas-3.
        ...
        ! =====================================================================
           ...
           nb = max( nb1, nb2, nb3, nb4 )
           lwkopt = max( 1_ilp, mn+2*n+nb*( n+1 ), 2_ilp*mn+nb*nrhs )
           work( 1_ilp ) = cmplx( lwkopt,KIND=qp)
           lquery = ( lwork==-1_ilp )
           if( m<0_ilp ) then
              info = -1_ilp
           else if( n<0_ilp ) then
           ...
           ! compute qr factorization with column pivoting of a:
              ! a * p = q * r
           call stdlib_wgeqp3( m, n, a, lda, jpvt, work( 1_ilp ), work( mn+1 ),lwork-mn, rwork, info )
           ...
           if( rank<mn ) then
              i = rank + 1_ilp
              call stdlib_wlaic1( imin, rank, work( ismin ), smin, a( 1_ilp, i ),a( i, i ), sminpr, &
                        s1, c1 )
              call stdlib_wlaic1( imax, rank, work( ismax ), smax, a( 1_ilp, i ),a( i, i ), smaxpr, &
                        s2, c2 )
              if( smaxpr*rcond<=sminpr ) then
                 do i = 1, rank
                    work( ismin+i-1 ) = s1*work( ismin+i-1 )
           ...
           ! where r11 = r(1:rank,1:rank)
           ! [r11,r12] = [ t11, 0 ] * y
           if( rank<n )call stdlib_wtzrzf( rank, n, a, lda, work( mn+1 ), work( 2_ilp*mn+1 ),lwork-&
                     2_ilp*mn, info )
           ! complex workspace: 2*mn.
           ! details of householder rotations stored in work(mn+1:2*mn)
           ! b(1:m,1:nrhs) := q**h * b(1:m,1:nrhs)
           call stdlib_wunmqr( 'LEFT', 'CONJUGATE TRANSPOSE', m, nrhs, mn, a, lda,work( 1_ilp ), b, &
                     ldb, work( 2_ilp*mn+1 ), lwork-2*mn, info )
           wsize = max( wsize, 2_ilp*mn+real( work( 2_ilp*mn+1 ),KIND=qp) )
           ! complex workspace: 2*mn+nb*nrhs.
           ! b(1:rank,1:nrhs) := inv(t11) * b(1:rank,1:nrhs)
           call stdlib_wtrsm( 'LEFT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', rank,nrhs, cone, a, &
                     lda, b, ldb )
           do j = 1, nrhs
              do i = rank + 1, n
                 b( i, j ) = czero
           ...
           ! b(1:n,1:nrhs) := y**h * b(1:n,1:nrhs)
           if( rank<n ) then
              call stdlib_wunmrz( 'LEFT', 'CONJUGATE TRANSPOSE', n, nrhs, rank,n-rank, a, lda, &
                        work( mn+1 ), b, ldb,work( 2_ilp*mn+1 ), lwork-2*mn, info )
           end if
           ! complex workspace: 2*mn+nrhs.
           ! b(1:n,1:nrhs) := p * b(1:n,1:nrhs)
           do j = 1, nrhs
              ...
              call stdlib_wcopy( n, work( 1_ilp ), 1_ilp, b( 1_ilp, j ), 1_ilp )
           end do
           ! complex workspace: n.
           ! undo scaling
           if( iascl==1_ilp ) then
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, anrm, smlnum, n, nrhs, b, ldb, info )
              call stdlib_wlascl( 'U', 0_ilp, 0_ilp, smlnum, anrm, rank, rank, a, lda,info )
           else if( iascl==2_ilp ) then
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, anrm, bignum, n, nrhs, b, ldb, info )
              call stdlib_wlascl( 'U', 0_ilp, 0_ilp, bignum, anrm, rank, rank, a, lda,info )
           end if
           if( ibscl==1_ilp ) then
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, smlnum, bnrm, n, nrhs, b, ldb, info )
           else if( ibscl==2_ilp ) then
     ...
     !! matrix X.
        ...
        ! =====================================================================
           ...
           ! quick return if possible
           if( min( m, n, nrhs )==0_ilp ) then
              call stdlib_qlaset( 'FULL', max( m, n ), nrhs, zero, zero, b, ldb )
              return
           end if
           ! get machine parameters
           smlnum = stdlib_qlamch( 'S' ) / stdlib_qlamch( 'P' )
           ...
           call stdlib_qlabad( smlnum, bignum )
           ! scale a, b if max element outside range [smlnum,bignum]
           anrm = stdlib_qlange( 'M', m, n, a, lda, rwork )
           iascl = 0_ilp
           if( anrm>zero .and. anrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, anrm, smlnum, m, n, a, lda, info )
           ...
           brow = m
           if( tpsd )brow = n
           bnrm = stdlib_qlange( 'M', brow, nrhs, b, ldb, rwork )
           ibscl = 0_ilp
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, bnrm, smlnum, brow, nrhs, b, ldb,info )
              ibscl = 1_ilp
           else if( bnrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, bnrm, bignum, brow, nrhs, b, ldb,info )
              ibscl = 2_ilp
           end if
           if( m>=n ) then
              ! compute qr factorization of a
              call stdlib_qgeqrf( m, n, a, lda, work( 1_ilp ), work( mn+1 ), lwork-mn,info )
              ! workspace at least n, optimally n*nb
              if( .not.tpsd ) then
                 ! least-squares problem min || a * x - b ||
                 ! b(1:m,1:nrhs) := q**t * b(1:m,1:nrhs)
                 call stdlib_qormqr( 'LEFT', 'TRANSPOSE', m, nrhs, n, a, lda,work( 1_ilp ), b, ldb, &
                           work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:n,1:nrhs) := inv(r) * b(1:n,1:nrhs)
                 call stdlib_qtrtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp ) then
                    return
                 end if
                 ...
                 ! underdetermined system of equations a**t * x = b
                 ! b(1:n,1:nrhs) := inv(r**t) * b(1:n,1:nrhs)
                 call stdlib_qtrtrs( 'UPPER', 'TRANSPOSE', 'NON-UNIT', n, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp ) then
                    return
                 end if
                 ...
                 ! b(1:m,1:nrhs) := q(1:n,:) * b(1:n,1:nrhs)
                 call stdlib_qormqr( 'LEFT', 'NO TRANSPOSE', m, nrhs, n, a, lda,work( 1_ilp ), b, ldb,&
                            work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = m
              end if
              ...
              ! compute lq factorization of a
              call stdlib_qgelqf( m, n, a, lda, work( 1_ilp ), work( mn+1 ), lwork-mn,info )
              ! workspace at least m, optimally m*nb.
              if( .not.tpsd ) then
                 ! underdetermined system of equations a * x = b
                 ! b(1:m,1:nrhs) := inv(l) * b(1:m,1:nrhs)
                 call stdlib_qtrtrs( 'LOWER', 'NO TRANSPOSE', 'NON-UNIT', m, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp ) then
                    return
                 end if
                 ...
                 ! b(1:n,1:nrhs) := q(1:n,:)**t * b(1:m,1:nrhs)
                 call stdlib_qormlq( 'LEFT', 'TRANSPOSE', n, nrhs, m, a, lda,work( 1_ilp ), b, ldb, &
                           work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = n
              else
                 ! overdetermined system min || a**t * x - b ||
                 ! b(1:n,1:nrhs) := q * b(1:n,1:nrhs)
                 call stdlib_qormlq( 'LEFT', 'NO TRANSPOSE', n, nrhs, m, a, lda,work( 1_ilp ), b, ldb,&
                            work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:m,1:nrhs) := inv(l**t) * b(1:m,1:nrhs)
                 call stdlib_qtrtrs( 'LOWER', 'TRANSPOSE', 'NON-UNIT', m, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp ) then
                    return
                 end if
           ...
           ! undo scaling
           if( iascl==1_ilp ) then
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, anrm, smlnum, scllen, nrhs, b, ldb,info )
           else if( iascl==2_ilp ) then
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, anrm, bignum, scllen, nrhs, b, ldb,info )
           end if
           if( ibscl==1_ilp ) then
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, smlnum, bnrm, scllen, nrhs, b, ldb,info )
           else if( ibscl==2_ilp ) then
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, bignum, bnrm, scllen, nrhs, b, ldb,info )
           end if
           50 continue
           work( 1_ilp ) = real( wsize,KIND=qp)
           return
     ...
     !! matrix X.
        ...
        ! =====================================================================
           ...
           ! quick return if possible
           if( min( m, n, nrhs )==0_ilp ) then
              call stdlib_wlaset( 'FULL', max( m, n ), nrhs, czero, czero, b, ldb )
              return
           end if
           ! get machine parameters
           smlnum = stdlib_qlamch( 'S' ) / stdlib_qlamch( 'P' )
           ...
           brow = m
           if( tpsd )brow = n
           bnrm = stdlib_wlange( 'M', brow, nrhs, b, ldb, rwork )
           ibscl = 0_ilp
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, bnrm, smlnum, brow, nrhs, b, ldb,info )
              ibscl = 1_ilp
           else if( bnrm>bignum ) then
              ! scale matrix norm down to bignum
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, bnrm, bignum, brow, nrhs, b, ldb,info )
              ibscl = 2_ilp
           end if
           if( m>=n ) then
              ! compute qr factorization of a
              call stdlib_wgeqrf( m, n, a, lda, work( 1_ilp ), work( mn+1 ), lwork-mn,info )
              ! workspace at least n, optimally n*nb
              if( .not.tpsd ) then
                 ! least-squares problem min || a * x - b ||
                 ! b(1:m,1:nrhs) := q**h * b(1:m,1:nrhs)
                 call stdlib_wunmqr( 'LEFT', 'CONJUGATE TRANSPOSE', m, nrhs, n, a,lda, work( 1_ilp ), &
                           b, ldb, work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:n,1:nrhs) := inv(r) * b(1:n,1:nrhs)
                 call stdlib_wtrtrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', n, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp ) then
                    return
                 end if
                 ...
                 ! underdetermined system of equations a**t * x = b
                 ! b(1:n,1:nrhs) := inv(r**h) * b(1:n,1:nrhs)
                 call stdlib_wtrtrs( 'UPPER', 'CONJUGATE TRANSPOSE','NON-UNIT',n, nrhs, a, lda, b,&
                            ldb, info )
                 if( info>0_ilp ) then
                    return
                 end if
                 ...
                 ! b(1:m,1:nrhs) := q(1:n,:) * b(1:n,1:nrhs)
                 call stdlib_wunmqr( 'LEFT', 'NO TRANSPOSE', m, nrhs, n, a, lda,work( 1_ilp ), b, ldb,&
                            work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = m
              end if
              ...
              ! compute lq factorization of a
              call stdlib_wgelqf( m, n, a, lda, work( 1_ilp ), work( mn+1 ), lwork-mn,info )
              ! workspace at least m, optimally m*nb.
              if( .not.tpsd ) then
                 ! underdetermined system of equations a * x = b
                 ! b(1:m,1:nrhs) := inv(l) * b(1:m,1:nrhs)
                 call stdlib_wtrtrs( 'LOWER', 'NO TRANSPOSE', 'NON-UNIT', m, nrhs,a, lda, b, ldb, &
                           info )
                 if( info>0_ilp ) then
                    return
                 end if
                 ...
                 ! b(1:n,1:nrhs) := q(1:n,:)**h * b(1:m,1:nrhs)
                 call stdlib_wunmlq( 'LEFT', 'CONJUGATE TRANSPOSE', n, nrhs, m, a,lda, work( 1_ilp ), &
                           b, ldb, work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = n
              else
                 ! overdetermined system min || a**h * x - b ||
                 ! b(1:n,1:nrhs) := q * b(1:n,1:nrhs)
                 call stdlib_wunmlq( 'LEFT', 'NO TRANSPOSE', n, nrhs, m, a, lda,work( 1_ilp ), b, ldb,&
                            work( mn+1 ), lwork-mn,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:m,1:nrhs) := inv(l**h) * b(1:m,1:nrhs)
                 call stdlib_wtrtrs( 'LOWER', 'CONJUGATE TRANSPOSE', 'NON-UNIT',m, nrhs, a, lda, &
                           b, ldb, info )
                 if( info>0_ilp ) then
                    return
                 end if
           ...
           ! undo scaling
           if( iascl==1_ilp ) then
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, anrm, smlnum, scllen, nrhs, b, ldb,info )
           else if( iascl==2_ilp ) then
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, anrm, bignum, scllen, nrhs, b, ldb,info )
           end if
           if( ibscl==1_ilp ) then
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, smlnum, bnrm, scllen, nrhs, b, ldb,info )
           else if( ibscl==2_ilp ) then
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, bignum, bnrm, scllen, nrhs, b, ldb,info )
           end if
           50 continue
           work( 1_ilp ) = real( wsize,KIND=qp)
           return
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           if( anrm>zero .and. anrm<smlnum ) then
              ...
              ! matrix all zero. return zero solution.
              call stdlib_qlaset( 'F', max( m, n ), nrhs, zero, zero, b, ldb )
              call stdlib_qlaset( 'F', minmn, 1_ilp, zero, zero, s, 1_ilp )
              rank = 0_ilp
              go to 10
           end if
           ! scale b if max entry outside range [smlnum,bignum].
           ...
           ! if m < n make sure certain entries of b are zero.
           if( m<n )call stdlib_qlaset( 'F', n-m, nrhs, zero, zero, b( m+1, 1_ilp ), ldb )
           ! overdetermined case.
           if( m>=n ) then
              ! path 1 - overdetermined or exactly determined.
              mm = m
              if( m>=mnthr ) then
                 ...
                 ! compute a=q*r.
                 ! (workspace: need 2*n, prefer n+n*nb)
                 call stdlib_qgeqrf( m, n, a, lda, work( itau ), work( nwork ),lwork-nwork+1, &
                           info )
                 ! multiply b by transpose(q).
                 ! (workspace: need n+nrhs, prefer n+nrhs*nb)
                 call stdlib_qormqr( 'L', 'T', m, nrhs, n, a, lda, work( itau ), b,ldb, work( &
                 ...
                 ! zero out below r.
                 if( n>1_ilp ) then
                    call stdlib_qlaset( 'L', n-1, n-1, zero, zero, a( 2_ilp, 1_ilp ), lda )
                 end if
              end if
              ie = 1_ilp
              itauq = ie + n
              ...
              call stdlib_qormbr( 'Q', 'L', 'T', mm, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_qlalsd( 'U', smlsiz, n, nrhs, s, work( ie ), b, ldb,rcond, rank, work( &
                        nwork ), iwork, info )
              if( info/=0_ilp ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of r.
              call stdlib_qormbr( 'P', 'L', 'N', n, nrhs, n, a, lda, work( itaup ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
           else if( n>=mnthr .and. lwork>=4_ilp*m+m*m+max( m, 2_ilp*m-4, nrhs, n-3*m, wlalsd ) ) &
                     then
              ! path 2a - underdetermined, with many more columns than rows
              ...
              call stdlib_qormbr( 'Q', 'L', 'T', m, nrhs, m, work( il ), ldwork,work( itauq ), b, &
                        ldb, work( nwork ),lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_qlalsd( 'U', smlsiz, m, nrhs, s, work( ie ), b, ldb,rcond, rank, work( &
                        nwork ), iwork, info )
              if( info/=0_ilp ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of l.
              call stdlib_qormbr( 'P', 'L', 'N', m, nrhs, m, work( il ), ldwork,work( itaup ), b, &
                        ldb, work( nwork ),lwork-nwork+1, info )
              ! zero out below first m rows of b.
              call stdlib_qlaset( 'F', n-m, nrhs, zero, zero, b( m+1, 1_ilp ), ldb )
              nwork = itau + m
              ! multiply transpose(q) by b.
              ! (workspace: need m+nrhs, prefer m+nrhs*nb)
              call stdlib_qormlq( 'L', 'T', n, nrhs, m, a, lda, work( itau ), b,ldb, work( nwork )&
                        , lwork-nwork+1, info )
           else
              ! path 2 - remaining underdetermined cases.
              ie = 1_ilp
              ...
              call stdlib_qormbr( 'Q', 'L', 'T', m, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_qlalsd( 'L', smlsiz, m, nrhs, s, work( ie ), b, ldb,rcond, rank, work( &
                        nwork ), iwork, info )
              if( info/=0_ilp ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of a.
              call stdlib_qormbr( 'P', 'L', 'N', n, nrhs, m, a, lda, work( itaup ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
           end if
           ! undo scaling.
           if( iascl==1_ilp ) then
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              ...
              if( minmn>0_ilp ) then
                 smlsiz = stdlib_ilaenv( 9_ilp, 'ZGELSD', ' ', 0_ilp, 0_ilp, 0_ilp, 0_ilp )
                 mnthr = stdlib_ilaenv( 6_ilp, 'ZGELSD', ' ', m, n, nrhs, -1_ilp )
                 nlvl = max( int( log( real( minmn,KIND=qp) / real( smlsiz + 1_ilp,KIND=qp) ) /log( &
                           two ),KIND=ilp) + 1_ilp, 0_ilp )
                 liwork = 3_ilp*minmn*nlvl + 11_ilp*minmn
                 mm = m
                 if( m>=n .and. m>=mnthr ) then
           ...
           ! if m < n make sure b(m+1:n,:) = 0
           if( m<n )call stdlib_wlaset( 'F', n-m, nrhs, czero, czero, b( m+1, 1_ilp ), ldb )
           ! overdetermined case.
           if( m>=n ) then
              ! path 1 - overdetermined or exactly determined.
              mm = m
              if( m>=mnthr ) then
                 ...
                 ! zero out below r.
                 if( n>1_ilp ) then
                    call stdlib_wlaset( 'L', n-1, n-1, czero, czero, a( 2_ilp, 1_ilp ),lda )
                 end if
              end if
              itauq = 1_ilp
              itaup = itauq + n
              ...
              call stdlib_wunmbr( 'Q', 'L', 'C', mm, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_wlalsd( 'U', smlsiz, n, nrhs, s, rwork( ie ), b, ldb,rcond, rank, work( &
                        nwork ), rwork( nrwork ),iwork, info )
              if( info/=0_ilp ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of r.
              call stdlib_wunmbr( 'P', 'L', 'N', n, nrhs, n, a, lda, work( itaup ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
           else if( n>=mnthr .and. lwork>=4_ilp*m+m*m+max( m, 2_ilp*m-4, nrhs, n-3*m ) ) then
              ! path 2a - underdetermined, with many more columns than rows
              ! and sufficient workspace for an efficient algorithm.
              ...
              call stdlib_wunmbr( 'Q', 'L', 'C', m, nrhs, m, work( il ), ldwork,work( itauq ), b, &
                        ldb, work( nwork ),lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_wlalsd( 'U', smlsiz, m, nrhs, s, rwork( ie ), b, ldb,rcond, rank, work( &
                        nwork ), rwork( nrwork ),iwork, info )
              if( info/=0_ilp ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of l.
              call stdlib_wunmbr( 'P', 'L', 'N', m, nrhs, m, work( il ), ldwork,work( itaup ), b, &
                        ldb, work( nwork ),lwork-nwork+1, info )
              ! zero out below first m rows of b.
              call stdlib_wlaset( 'F', n-m, nrhs, czero, czero, b( m+1, 1_ilp ), ldb )
              nwork = itau + m
              ! multiply transpose(q) by b.
              ! (cworkspace: need nrhs, prefer nrhs*nb)
              call stdlib_wunmlq( 'L', 'C', n, nrhs, m, a, lda, work( itau ), b,ldb, work( nwork )&
                        , lwork-nwork+1, info )
           else
              ! path 2 - remaining underdetermined cases.
              itauq = 1_ilp
              ...
              call stdlib_wunmbr( 'Q', 'L', 'C', m, nrhs, n, a, lda, work( itauq ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
              ! solve the bidiagonal least squares problem.
              call stdlib_wlalsd( 'L', smlsiz, m, nrhs, s, rwork( ie ), b, ldb,rcond, rank, work( &
                        nwork ), rwork( nrwork ),iwork, info )
              if( info/=0_ilp ) then
                 go to 10
              end if
              ! multiply b by right bidiagonalizing vectors of a.
              call stdlib_wunmbr( 'P', 'L', 'N', n, nrhs, m, a, lda, work( itaup ),b, ldb, work( &
                        nwork ), lwork-nwork+1, info )
           end if
           ! undo scaling.
           if( iascl==1_ilp ) then
     ...
     !! matrix X.
        ...
        ! =====================================================================
           ...
           integer(ilp) :: i, iascl, ibscl, j, maxmn, brow, scllen, tszo, tszm, lwo, lwm, lw1, &
                     lw2, wsizeo, wsizem, info2
           real(qp) :: anrm, bignum, bnrm, smlnum, tq(5_ilp), workq(1_ilp)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments.
           info = 0_ilp
           ...
           ! determine the optimum and minimum lwork
            if( m>=n ) then
              call stdlib_qgeqr( m, n, a, lda, tq, -1_ilp, workq, -1_ilp, info2 )
              tszo = int( tq( 1_ilp ),KIND=ilp)
              lwo  = int( workq( 1_ilp ),KIND=ilp)
              call stdlib_qgemqr( 'L', trans, m, nrhs, n, a, lda, tq,tszo, b, ldb, workq, -1_ilp, &
                        info2 )
              lwo  = max( lwo, int( workq( 1_ilp ),KIND=ilp) )
              call stdlib_qgeqr( m, n, a, lda, tq, -2_ilp, workq, -2_ilp, info2 )
              tszm = int( tq( 1_ilp ),KIND=ilp)
              lwm  = int( workq( 1_ilp ),KIND=ilp)
              call stdlib_qgemqr( 'L', trans, m, nrhs, n, a, lda, tq,tszm, b, ldb, workq, -1_ilp, &
                        info2 )
              lwm = max( lwm, int( workq( 1_ilp ),KIND=ilp) )
              wsizeo = tszo + lwo
              wsizem = tszm + lwm
            else
              call stdlib_qgelq( m, n, a, lda, tq, -1_ilp, workq, -1_ilp, info2 )
              tszo = int( tq( 1_ilp ),KIND=ilp)
              lwo  = int( workq( 1_ilp ),KIND=ilp)
              call stdlib_qgemlq( 'L', trans, n, nrhs, m, a, lda, tq,tszo, b, ldb, workq, -1_ilp, &
                        info2 )
              lwo  = max( lwo, int( workq( 1_ilp ),KIND=ilp) )
              call stdlib_qgelq( m, n, a, lda, tq, -2_ilp, workq, -2_ilp, info2 )
              tszm = int( tq( 1_ilp ),KIND=ilp)
              lwm  = int( workq( 1_ilp ),KIND=ilp)
              call stdlib_qgemlq( 'L', trans, n, nrhs, m, a, lda, tq,tszm, b, ldb, workq, -1_ilp, &
                        info2 )
              lwm  = max( lwm, int( workq( 1_ilp ),KIND=ilp) )
              wsizeo = tszo + lwo
              wsizem = tszm + lwm
            ...
            work( 1_ilp ) = real( wsizeo,KIND=qp)
           end if
           if( info/=0_ilp ) then
             call stdlib_xerbla( 'DGETSLS', -info )
             return
           ...
           if( lquery ) then
             if( lwork==-2_ilp ) work( 1_ilp ) = real( wsizem,KIND=qp)
             return
           end if
           if( lwork<wsizeo ) then
             lw1 = tszm
           ...
           if( anrm>zero .and. anrm<smlnum ) then
              ...
              ! matrix all zero. return zero solution.
              call stdlib_qlaset( 'F', maxmn, nrhs, zero, zero, b, ldb )
              go to 50
           end if
           brow = m
           if ( tran ) then
           ...
           bnrm = stdlib_qlange( 'M', brow, nrhs, b, ldb, work )
           ibscl = 0_ilp
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, bnrm, smlnum, brow, nrhs, b, ldb,info )
           ...
           if ( m>=n ) then
              ! compute qr factorization of a
             call stdlib_qgeqr( m, n, a, lda, work( lw2+1 ), lw1,work( 1_ilp ), lw2, info )
             if ( .not.tran ) then
                 ! least-squares problem min || a * x - b ||
                 ! b(1:m,1:nrhs) := q**t * b(1:m,1:nrhs)
               call stdlib_qgemqr( 'L' , 'T', m, nrhs, n, a, lda,work( lw2+1 ), lw1, b, ldb, work(&
                          1_ilp ), lw2,info )
                 ! b(1:n,1:nrhs) := inv(r) * b(1:n,1:nrhs)
               call stdlib_qtrtrs( 'U', 'N', 'N', n, nrhs,a, lda, b, ldb, info )
               if( info>0_ilp ) then
                 return
               end if
               scllen = n
                 ...
                 ! overdetermined system of equations a**t * x = b
                 ! b(1:n,1:nrhs) := inv(r**t) * b(1:n,1:nrhs)
                 call stdlib_qtrtrs( 'U', 'T', 'N', n, nrhs,a, lda, b, ldb, info )
                 if( info>0_ilp ) then
                    return
                 end if
                 ! b(n+1:m,1:nrhs) = zero
                 ...
                 ! b(1:m,1:nrhs) := q(1:n,:) * b(1:n,1:nrhs)
                 call stdlib_qgemqr( 'L', 'N', m, nrhs, n, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp ), lw2,info )
                 scllen = m
              end if
           else
              ! compute lq factorization of a
              call stdlib_qgelq( m, n, a, lda, work( lw2+1 ), lw1,work( 1_ilp ), lw2, info )
              ! workspace at least m, optimally m*nb.
              if( .not.tran ) then
                 ! underdetermined system of equations a * x = b
                 ! b(1:m,1:nrhs) := inv(l) * b(1:m,1:nrhs)
                 call stdlib_qtrtrs( 'L', 'N', 'N', m, nrhs,a, lda, b, ldb, info )
                 if( info>0_ilp ) then
                    return
                 end if
                 ! b(m+1:n,1:nrhs) = 0
                 ...
                 ! b(1:n,1:nrhs) := q(1:n,:)**t * b(1:m,1:nrhs)
                 call stdlib_qgemlq( 'L', 'T', n, nrhs, m, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp ), lw2,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = n
              else
                 ! overdetermined system min || a**t * x - b ||
                 ! b(1:n,1:nrhs) := q * b(1:n,1:nrhs)
                 call stdlib_qgemlq( 'L', 'N', n, nrhs, m, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp ), lw2,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:m,1:nrhs) := inv(l**t) * b(1:m,1:nrhs)
                 call stdlib_qtrtrs( 'LOWER', 'TRANSPOSE', 'NON-UNIT', m, nrhs,a, lda, b, ldb, &
           ...
           50 continue
           work( 1_ilp ) = real( tszo + lwo,KIND=qp)
           return
     ...
     !! matrix X.
        ...
        ! =====================================================================
           ...
           integer(ilp) :: i, iascl, ibscl, j, maxmn, brow, scllen, tszo, tszm, lwo, lwm, lw1, &
                     lw2, wsizeo, wsizem, info2
           real(qp) :: anrm, bignum, bnrm, smlnum, dum(1_ilp)
           complex(qp) :: tq(5_ilp), workq(1_ilp)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! test the input arguments.
           info = 0_ilp
           ...
           ! determine the optimum and minimum lwork
            if( m>=n ) then
              call stdlib_wgeqr( m, n, a, lda, tq, -1_ilp, workq, -1_ilp, info2 )
              tszo = int( tq( 1_ilp ),KIND=ilp)
              lwo  = int( workq( 1_ilp ),KIND=ilp)
              call stdlib_wgemqr( 'L', trans, m, nrhs, n, a, lda, tq,tszo, b, ldb, workq, -1_ilp, &
                        info2 )
              lwo  = max( lwo, int( workq( 1_ilp ),KIND=ilp) )
              call stdlib_wgeqr( m, n, a, lda, tq, -2_ilp, workq, -2_ilp, info2 )
              tszm = int( tq( 1_ilp ),KIND=ilp)
              lwm  = int( workq( 1_ilp ),KIND=ilp)
              call stdlib_wgemqr( 'L', trans, m, nrhs, n, a, lda, tq,tszm, b, ldb, workq, -1_ilp, &
                        info2 )
              lwm = max( lwm, int( workq( 1_ilp ),KIND=ilp) )
              wsizeo = tszo + lwo
              wsizem = tszm + lwm
            else
              call stdlib_wgelq( m, n, a, lda, tq, -1_ilp, workq, -1_ilp, info2 )
              tszo = int( tq( 1_ilp ),KIND=ilp)
              lwo  = int( workq( 1_ilp ),KIND=ilp)
              call stdlib_wgemlq( 'L', trans, n, nrhs, m, a, lda, tq,tszo, b, ldb, workq, -1_ilp, &
                        info2 )
              lwo  = max( lwo, int( workq( 1_ilp ),KIND=ilp) )
              call stdlib_wgelq( m, n, a, lda, tq, -2_ilp, workq, -2_ilp, info2 )
              tszm = int( tq( 1_ilp ),KIND=ilp)
              lwm  = int( workq( 1_ilp ),KIND=ilp)
              call stdlib_wgemlq( 'L', trans, n, nrhs, m, a, lda, tq,tszm, b, ldb, workq, -1_ilp, &
                        info2 )
              lwm  = max( lwm, int( workq( 1_ilp ),KIND=ilp) )
              wsizeo = tszo + lwo
              wsizem = tszm + lwm
           ...
           ! get machine parameters
            ...
            call stdlib_qlabad( smlnum, bignum )
           ! scale a, b if max element outside range [smlnum,bignum]
           anrm = stdlib_wlange( 'M', m, n, a, lda, dum )
           iascl = 0_ilp
           if( anrm>zero .and. anrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, anrm, smlnum, m, n, a, lda, info )
              ...
              ! matrix all zero. return zero solution.
              call stdlib_wlaset( 'F', maxmn, nrhs, czero, czero, b, ldb )
              go to 50
           end if
           brow = m
           if ( tran ) then
           ...
           bnrm = stdlib_wlange( 'M', brow, nrhs, b, ldb, dum )
           ibscl = 0_ilp
           if( bnrm>zero .and. bnrm<smlnum ) then
              ! scale matrix norm up to smlnum
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, bnrm, smlnum, brow, nrhs, b, ldb,info )
           ...
           if ( m>=n ) then
              ! compute qr factorization of a
             call stdlib_wgeqr( m, n, a, lda, work( lw2+1 ), lw1,work( 1_ilp ), lw2, info )
             if ( .not.tran ) then
                 ! least-squares problem min || a * x - b ||
                 ! b(1:m,1:nrhs) := q**t * b(1:m,1:nrhs)
               call stdlib_wgemqr( 'L' , 'C', m, nrhs, n, a, lda,work( lw2+1 ), lw1, b, ldb, work(&
                          1_ilp ), lw2,info )
                 ! b(1:n,1:nrhs) := inv(r) * b(1:n,1:nrhs)
               call stdlib_wtrtrs( 'U', 'N', 'N', n, nrhs,a, lda, b, ldb, info )
               if( info>0_ilp ) then
                 return
               end if
               scllen = n
                 ...
                 ! overdetermined system of equations a**t * x = b
                 ! b(1:n,1:nrhs) := inv(r**t) * b(1:n,1:nrhs)
                 call stdlib_wtrtrs( 'U', 'C', 'N', n, nrhs,a, lda, b, ldb, info )
                 if( info>0_ilp ) then
                    return
                 end if
                 ! b(n+1:m,1:nrhs) = czero
                 ...
                 ! b(1:m,1:nrhs) := q(1:n,:) * b(1:n,1:nrhs)
                 call stdlib_wgemqr( 'L', 'N', m, nrhs, n, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp ), lw2,info )
                 scllen = m
              end if
           else
              ! compute lq factorization of a
              call stdlib_wgelq( m, n, a, lda, work( lw2+1 ), lw1,work( 1_ilp ), lw2, info )
              ! workspace at least m, optimally m*nb.
              if( .not.tran ) then
                 ! underdetermined system of equations a * x = b
                 ! b(1:m,1:nrhs) := inv(l) * b(1:m,1:nrhs)
                 call stdlib_wtrtrs( 'L', 'N', 'N', m, nrhs,a, lda, b, ldb, info )
                 if( info>0_ilp ) then
                    return
                 end if
                 ! b(m+1:n,1:nrhs) = 0
                 ...
                 ! b(1:n,1:nrhs) := q(1:n,:)**t * b(1:m,1:nrhs)
                 call stdlib_wgemlq( 'L', 'C', n, nrhs, m, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp ), lw2,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 scllen = n
              else
                 ! overdetermined system min || a**t * x - b ||
                 ! b(1:n,1:nrhs) := q * b(1:n,1:nrhs)
                 call stdlib_wgemlq( 'L', 'N', n, nrhs, m, a, lda,work( lw2+1 ), lw1, b, ldb, &
                           work( 1_ilp ), lw2,info )
                 ! workspace at least nrhs, optimally nrhs*nb
                 ! b(1:m,1:nrhs) := inv(l**t) * b(1:m,1:nrhs)
                 call stdlib_wtrtrs( 'L', 'C', 'N', m, nrhs,a, lda, b, ldb, info )
                 if( info>0_ilp ) then
                    return
                 end if
                 scllen = m
