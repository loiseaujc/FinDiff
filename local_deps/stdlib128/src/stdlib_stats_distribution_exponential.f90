module stdlib_stats_distribution_exponential
    ...
    !!
        ...
        module procedure pdf_exp_lambda_rqp
        module procedure pdf_exp_lambda_csp
        module procedure pdf_exp_lambda_cdp
        module procedure pdf_exp_lambda_cqp
    ...
    !!
        ...
        module procedure cdf_exp_lambda_rqp
        module procedure cdf_exp_lambda_csp
        module procedure cdf_exp_lambda_cdp
        module procedure cdf_exp_lambda_cqp
...
contains
    ...
    impure function rvs_exp_0_rqp( ) result(res)
    !
    ! Standard exponential random variate (lambda=1; scale=1/lambda=1)
    !
        real(qp) :: res, x
    ...
    impure elemental function rvs_exp_rqp(loc, scale) result(res)
    !
    ! Exponential distributed random variate
    !
    ...
    impure elemental function rvs_exp_cqp(loc, scale) result(res)
        complex(qp), intent(in) :: loc, scale
        complex(qp) :: res
        real(qp) :: tr, ti

        tr = rvs_exp_rqp(loc%re, scale%re)
        ti = rvs_exp_rqp(loc%im, scale%im)
    ...
    impure function rvs_exp_array_rqp(loc, scale, array_size) result(res)
        real(qp), intent(in) :: loc, scale
        integer, intent(in) :: array_size
    ...
    impure function rvs_exp_array_cqp(loc, scale, array_size) result(res)
        complex(qp), intent(in) :: loc, scale
        integer, intent(in) :: array_size
    ...
    elemental function pdf_exp_rqp(x, loc, scale) result(res)
    !
    ! Exponential Distribution Probability Density Function
    !
    ...
    elemental function pdf_exp_cqp(x, loc, scale) result(res)
        complex(qp), intent(in) :: x, loc, scale
        real(qp) :: res

        res = pdf_exp_rqp(x%re, loc%re, scale%re)
        res = res * pdf_exp_rqp(x%im, loc%im, scale%im)
    ...
    elemental function cdf_exp_rqp(x, loc, scale) result(res)
    !
    ! Exponential Distribution Cumulative Distribution Function
    !
        real(qp), intent(in) :: x, loc, scale
    ...
    elemental function cdf_exp_cqp(x, loc, scale) result(res)
        complex(qp), intent(in) :: x, loc, scale
        real(qp) :: res

        res = cdf_exp_rqp(x%re, loc%re, scale%re)
        res = res * cdf_exp_rqp(x%im, loc%im, scale%im)
    ...
    impure elemental function rvs_exp_lambda_rqp(lambda) result(res)
    !
    ! Exponential distributed random variate
    !
        ...
        res = rvs_exp_rqp(0._qp, 1.0_qp/lambda)
    end function rvs_exp_lambda_rqp
    ...
    impure elemental function rvs_exp_lambda_cqp(lambda) result(res)
        complex(qp), intent(in) :: lambda
        complex(qp) :: res
        real(qp) :: tr, ti

        tr = rvs_exp_rqp(0._qp, 1.0_qp/lambda%re)
        ti = rvs_exp_rqp(0._qp, 1.0_qp/lambda%im)
        res = cmplx(tr, ti, kind=qp)
    end function rvs_exp_lambda_cqp
    ...
    impure function rvs_exp_array_lambda_rqp(lambda, array_size) result(res)
        real(qp), intent(in) :: lambda
        integer, intent(in) :: array_size
        real(qp) :: res(array_size)

        res = rvs_exp_array_rqp(0._qp, 1.0_qp/lambda, array_size)
    end function rvs_exp_array_lambda_rqp
    ...
    impure function rvs_exp_array_lambda_cqp(lambda, array_size) result(res)
        complex(qp), intent(in) :: lambda
        integer, intent(in) :: array_size
        complex(qp) :: res(array_size)
        integer :: i
    ...
    end function rvs_exp_array_lambda_cqp
    ...
    elemental function pdf_exp_lambda_rqp(x, lambda) result(res)
    !
    ! Exponential Distribution Probability Density Function
    !
        ...
        res = pdf_exp_rqp(x, 0._qp, 1.0_qp/lambda)
    end function pdf_exp_lambda_rqp
    ...
    elemental function pdf_exp_lambda_cqp(x, lambda) result(res)
        complex(qp), intent(in) :: x, lambda
        real(qp) :: res

        res = pdf_exp_rqp(x%re, 0._qp, 1.0_qp/lambda%re)
        res = res * pdf_exp_rqp(x%im, 0._qp, 1.0_qp/lambda%im)
    end function pdf_exp_lambda_cqp
    ...
    elemental function cdf_exp_lambda_rqp(x, lambda) result(res)
    !
    ! Exponential Distribution Cumulative Distribution Function
    !
        real(qp), intent(in) :: x, lambda
        ...
        res = cdf_exp_rqp(x, 0._qp, 1.0_qp/lambda)
    end function cdf_exp_lambda_rqp
    ...
    elemental function cdf_exp_lambda_cqp(x, lambda) result(res)
        complex(qp), intent(in) :: x, lambda
        real(qp) :: res

        res = cdf_exp_rqp(x%re, 0._qp, 1.0_qp/lambda%re)
        res = res * cdf_exp_rqp(x%im, 0._qp, 1.0_qp/lambda%im)
    end function cdf_exp_lambda_cqp
