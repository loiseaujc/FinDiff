  ...
  contains
     ...
     !! This is an auxiliary routine called by DGEBRD
        ...
        ! =====================================================================
           ...
           if( m>=n ) then
              ...
              loop_10: do i = 1, nb
                 ! update a(i:m,i)
                 call stdlib_qgemv( 'NO TRANSPOSE', m-i+1, i-1, -one, a( i, 1_ilp ),lda, y( i, 1_ilp ), &
                           ldy, one, a( i, i ), 1_ilp )
                 call stdlib_qgemv( 'NO TRANSPOSE', m-i+1, i-1, -one, x( i, 1_ilp ),ldx, a( 1_ilp, i ), 1_ilp,&
                            one, a( i, i ), 1_ilp )
                 ! generate reflection q(i) to annihilate a(i+1:m,i)
                 ...
                 if( i<n ) then
                    a( i, i ) = one
                    ! compute y(i+1:n,i)
                    call stdlib_qgemv( 'TRANSPOSE', m-i+1, n-i, one, a( i, i+1 ),lda, a( i, i ), &
                              1_ilp, zero, y( i+1, i ), 1_ilp )
                    call stdlib_qgemv( 'TRANSPOSE', m-i+1, i-1, one, a( i, 1_ilp ), lda,a( i, i ), 1_ilp, &
                              zero, y( 1_ilp, i ), 1_ilp )
                    call stdlib_qgemv( 'NO TRANSPOSE', n-i, i-1, -one, y( i+1, 1_ilp ),ldy, y( 1_ilp, i ),&
                               1_ilp, one, y( i+1, i ), 1_ilp )
                    call stdlib_qgemv( 'TRANSPOSE', m-i+1, i-1, one, x( i, 1_ilp ), ldx,a( i, i ), 1_ilp, &
                              zero, y( 1_ilp, i ), 1_ilp )
                    call stdlib_qgemv( 'TRANSPOSE', i-1, n-i, -one, a( 1_ilp, i+1 ),lda, y( 1_ilp, i ), 1_ilp,&
                               one, y( i+1, i ), 1_ilp )
                    call stdlib_qscal( n-i, tauq( i ), y( i+1, i ), 1_ilp )
                    ! update a(i,i+1:n)
                    call stdlib_qgemv( 'NO TRANSPOSE', n-i, i, -one, y( i+1, 1_ilp ),ldy, a( i, 1_ilp ), &
                              lda, one, a( i, i+1 ), lda )
                    call stdlib_qgemv( 'TRANSPOSE', i-1, n-i, -one, a( 1_ilp, i+1 ),lda, x( i, 1_ilp ), &
                              ldx, one, a( i, i+1 ), lda )
                    ! generate reflection p(i) to annihilate a(i,i+2:n)
                    ...
                    a( i, i+1 ) = one
                    ! compute x(i+1:m,i)
                    call stdlib_qgemv( 'NO TRANSPOSE', m-i, n-i, one, a( i+1, i+1 ),lda, a( i, i+&
                              1_ilp ), lda, zero, x( i+1, i ), 1_ilp )
                    call stdlib_qgemv( 'TRANSPOSE', n-i, i, one, y( i+1, 1_ilp ), ldy,a( i, i+1 ), &
                              lda, zero, x( 1_ilp, i ), 1_ilp )
                    call stdlib_qgemv( 'NO TRANSPOSE', m-i, i, -one, a( i+1, 1_ilp ),lda, x( 1_ilp, i ), &
                              1_ilp, one, x( i+1, i ), 1_ilp )
                    call stdlib_qgemv( 'NO TRANSPOSE', i-1, n-i, one, a( 1_ilp, i+1 ),lda, a( i, i+1 )&
                              , lda, zero, x( 1_ilp, i ), 1_ilp )
                    call stdlib_qgemv( 'NO TRANSPOSE', m-i, i-1, -one, x( i+1, 1_ilp ),ldx, x( 1_ilp, i ),&
                               1_ilp, one, x( i+1, i ), 1_ilp )
                    call stdlib_qscal( m-i, taup( i ), x( i+1, i ), 1_ilp )
                 end if
              end do loop_10
           else
              ! reduce to lower bidiagonal form
              loop_20: do i = 1, nb
                 ! update a(i,i:n)
                 call stdlib_qgemv( 'NO TRANSPOSE', n-i+1, i-1, -one, y( i, 1_ilp ),ldy, a( i, 1_ilp ), &
                           lda, one, a( i, i ), lda )
                 call stdlib_qgemv( 'TRANSPOSE', i-1, n-i+1, -one, a( 1_ilp, i ), lda,x( i, 1_ilp ), ldx, &
                           one, a( i, i ), lda )
                 ! generate reflection p(i) to annihilate a(i,i+1:n)
                 ...
                 if( i<m ) then
                    a( i, i ) = one
                    ! compute x(i+1:m,i)
                    call stdlib_qgemv( 'NO TRANSPOSE', m-i, n-i+1, one, a( i+1, i ),lda, a( i, i )&
                              , lda, zero, x( i+1, i ), 1_ilp )
                    call stdlib_qgemv( 'TRANSPOSE', n-i+1, i-1, one, y( i, 1_ilp ), ldy,a( i, i ), &
                              lda, zero, x( 1_ilp, i ), 1_ilp )
                    call stdlib_qgemv( 'NO TRANSPOSE', m-i, i-1, -one, a( i+1, 1_ilp ),lda, x( 1_ilp, i ),&
                               1_ilp, one, x( i+1, i ), 1_ilp )
                    call stdlib_qgemv( 'NO TRANSPOSE', i-1, n-i+1, one, a( 1_ilp, i ),lda, a( i, i ), &
                              lda, zero, x( 1_ilp, i ), 1_ilp )
                    call stdlib_qgemv( 'NO TRANSPOSE', m-i, i-1, -one, x( i+1, 1_ilp ),ldx, x( 1_ilp, i ),&
                               1_ilp, one, x( i+1, i ), 1_ilp )
                    call stdlib_qscal( m-i, taup( i ), x( i+1, i ), 1_ilp )
                    ! update a(i+1:m,i)
                    call stdlib_qgemv( 'NO TRANSPOSE', m-i, i-1, -one, a( i+1, 1_ilp ),lda, y( i, 1_ilp ),&
                               ldy, one, a( i+1, i ), 1_ilp )
                    call stdlib_qgemv( 'NO TRANSPOSE', m-i, i, -one, x( i+1, 1_ilp ),ldx, a( 1_ilp, i ), &
                              1_ilp, one, a( i+1, i ), 1_ilp )
                    ! generate reflection q(i) to annihilate a(i+2:m,i)
                    ...
                    a( i+1, i ) = one
                    ! compute y(i+1:n,i)
                    call stdlib_qgemv( 'TRANSPOSE', m-i, n-i, one, a( i+1, i+1 ),lda, a( i+1, i ),&
                               1_ilp, zero, y( i+1, i ), 1_ilp )
                    call stdlib_qgemv( 'TRANSPOSE', m-i, i-1, one, a( i+1, 1_ilp ), lda,a( i+1, i ), &
                              1_ilp, zero, y( 1_ilp, i ), 1_ilp )
                    call stdlib_qgemv( 'NO TRANSPOSE', n-i, i-1, -one, y( i+1, 1_ilp ),ldy, y( 1_ilp, i ),&
                               1_ilp, one, y( i+1, i ), 1_ilp )
                    call stdlib_qgemv( 'TRANSPOSE', m-i, i, one, x( i+1, 1_ilp ), ldx,a( i+1, i ), 1_ilp, &
                              zero, y( 1_ilp, i ), 1_ilp )
                    call stdlib_qgemv( 'TRANSPOSE', i, n-i, -one, a( 1_ilp, i+1 ), lda,y( 1_ilp, i ), 1_ilp, &
                              one, y( i+1, i ), 1_ilp )
                    call stdlib_qscal( n-i, tauq( i ), y( i+1, i ), 1_ilp )
                 end if
              end do loop_20
     ...
     !! This is an auxiliary routine called by ZGEBRD
        ...
        ! =====================================================================
           ...
           if( m>=n ) then
              ...
              loop_10: do i = 1, nb
                 ! update a(i:m,i)
                 call stdlib_wlacgv( i-1, y( i, 1_ilp ), ldy )
                 call stdlib_wgemv( 'NO TRANSPOSE', m-i+1, i-1, -cone, a( i, 1_ilp ),lda, y( i, 1_ilp ), &
                           ldy, cone, a( i, i ), 1_ilp )
                 call stdlib_wlacgv( i-1, y( i, 1_ilp ), ldy )
                 call stdlib_wgemv( 'NO TRANSPOSE', m-i+1, i-1, -cone, x( i, 1_ilp ),ldx, a( 1_ilp, i ), &
                           1_ilp, cone, a( i, i ), 1_ilp )
                 ! generate reflection q(i) to annihilate a(i+1:m,i)
                 alpha = a( i, i )
                 ...
                 if( i<n ) then
                    a( i, i ) = cone
                    ! compute y(i+1:n,i)
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', m-i+1, n-i, cone,a( i, i+1 ), lda, &
                              a( i, i ), 1_ilp, czero,y( i+1, i ), 1_ilp )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', m-i+1, i-1, cone,a( i, 1_ilp ), lda, a( &
                              i, i ), 1_ilp, czero,y( 1_ilp, i ), 1_ilp )
                    call stdlib_wgemv( 'NO TRANSPOSE', n-i, i-1, -cone, y( i+1, 1_ilp ),ldy, y( 1_ilp, i )&
                              , 1_ilp, cone, y( i+1, i ), 1_ilp )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', m-i+1, i-1, cone,x( i, 1_ilp ), ldx, a( &
                              i, i ), 1_ilp, czero,y( 1_ilp, i ), 1_ilp )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', i-1, n-i, -cone,a( 1_ilp, i+1 ), lda, y(&
                               1_ilp, i ), 1_ilp, cone,y( i+1, i ), 1_ilp )
                    call stdlib_wscal( n-i, tauq( i ), y( i+1, i ), 1_ilp )
                    ! update a(i,i+1:n)
                    call stdlib_wlacgv( n-i, a( i, i+1 ), lda )
                    call stdlib_wlacgv( i, a( i, 1_ilp ), lda )
                    call stdlib_wgemv( 'NO TRANSPOSE', n-i, i, -cone, y( i+1, 1_ilp ),ldy, a( i, 1_ilp ), &
                              lda, cone, a( i, i+1 ), lda )
                    call stdlib_wlacgv( i, a( i, 1_ilp ), lda )
                    call stdlib_wlacgv( i-1, x( i, 1_ilp ), ldx )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', i-1, n-i, -cone,a( 1_ilp, i+1 ), lda, x(&
                               i, 1_ilp ), ldx, cone,a( i, i+1 ), lda )
                    call stdlib_wlacgv( i-1, x( i, 1_ilp ), ldx )
                    ! generate reflection p(i) to annihilate a(i,i+2:n)
                    alpha = a( i, i+1 )
                    ...
                    a( i, i+1 ) = cone
                    ! compute x(i+1:m,i)
                    call stdlib_wgemv( 'NO TRANSPOSE', m-i, n-i, cone, a( i+1, i+1 ),lda, a( i, i+&
                              1_ilp ), lda, czero, x( i+1, i ), 1_ilp )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', n-i, i, cone,y( i+1, 1_ilp ), ldy, a( i,&
                               i+1 ), lda, czero,x( 1_ilp, i ), 1_ilp )
                    call stdlib_wgemv( 'NO TRANSPOSE', m-i, i, -cone, a( i+1, 1_ilp ),lda, x( 1_ilp, i ), &
                              1_ilp, cone, x( i+1, i ), 1_ilp )
                    call stdlib_wgemv( 'NO TRANSPOSE', i-1, n-i, cone, a( 1_ilp, i+1 ),lda, a( i, i+1 &
                              ), lda, czero, x( 1_ilp, i ), 1_ilp )
                    call stdlib_wgemv( 'NO TRANSPOSE', m-i, i-1, -cone, x( i+1, 1_ilp ),ldx, x( 1_ilp, i )&
                              , 1_ilp, cone, x( i+1, i ), 1_ilp )
                    call stdlib_wscal( m-i, taup( i ), x( i+1, i ), 1_ilp )
                    call stdlib_wlacgv( n-i, a( i, i+1 ), lda )
                 end if
              end do loop_10
           else
              ...
              loop_20: do i = 1, nb
                 ...
                 call stdlib_wgemv( 'NO TRANSPOSE', n-i+1, i-1, -cone, y( i, 1_ilp ),ldy, a( i, 1_ilp ), &
                           lda, cone, a( i, i ), lda )
                 call stdlib_wlacgv( i-1, a( i, 1_ilp ), lda )
                 call stdlib_wlacgv( i-1, x( i, 1_ilp ), ldx )
                 call stdlib_wgemv( 'CONJUGATE TRANSPOSE', i-1, n-i+1, -cone,a( 1_ilp, i ), lda, x( i,&
                            1_ilp ), ldx, cone, a( i, i ),lda )
                 call stdlib_wlacgv( i-1, x( i, 1_ilp ), ldx )
                 ! generate reflection p(i) to annihilate a(i,i+1:n)
                 alpha = a( i, i )
                 ...
                 if( i<m ) then
                    a( i, i ) = cone
                    ! compute x(i+1:m,i)
                    call stdlib_wgemv( 'NO TRANSPOSE', m-i, n-i+1, cone, a( i+1, i ),lda, a( i, i &
                              ), lda, czero, x( i+1, i ), 1_ilp )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', n-i+1, i-1, cone,y( i, 1_ilp ), ldy, a( &
                              i, i ), lda, czero,x( 1_ilp, i ), 1_ilp )
                    call stdlib_wgemv( 'NO TRANSPOSE', m-i, i-1, -cone, a( i+1, 1_ilp ),lda, x( 1_ilp, i )&
                              , 1_ilp, cone, x( i+1, i ), 1_ilp )
                    call stdlib_wgemv( 'NO TRANSPOSE', i-1, n-i+1, cone, a( 1_ilp, i ),lda, a( i, i ),&
                               lda, czero, x( 1_ilp, i ), 1_ilp )
                    call stdlib_wgemv( 'NO TRANSPOSE', m-i, i-1, -cone, x( i+1, 1_ilp ),ldx, x( 1_ilp, i )&
                              , 1_ilp, cone, x( i+1, i ), 1_ilp )
                    call stdlib_wscal( m-i, taup( i ), x( i+1, i ), 1_ilp )
                    ...
                    ! update a(i+1:m,i)
                    call stdlib_wlacgv( i-1, y( i, 1_ilp ), ldy )
                    call stdlib_wgemv( 'NO TRANSPOSE', m-i, i-1, -cone, a( i+1, 1_ilp ),lda, y( i, 1_ilp )&
                              , ldy, cone, a( i+1, i ), 1_ilp )
                    call stdlib_wlacgv( i-1, y( i, 1_ilp ), ldy )
                    call stdlib_wgemv( 'NO TRANSPOSE', m-i, i, -cone, x( i+1, 1_ilp ),ldx, a( 1_ilp, i ), &
                              1_ilp, cone, a( i+1, i ), 1_ilp )
                    ! generate reflection q(i) to annihilate a(i+2:m,i)
                    alpha = a( i+1, i )
                    ...
                    a( i+1, i ) = cone
                    ! compute y(i+1:n,i)
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', m-i, n-i, cone,a( i+1, i+1 ), lda, &
                              a( i+1, i ), 1_ilp, czero,y( i+1, i ), 1_ilp )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', m-i, i-1, cone,a( i+1, 1_ilp ), lda, a( &
                              i+1, i ), 1_ilp, czero,y( 1_ilp, i ), 1_ilp )
                    call stdlib_wgemv( 'NO TRANSPOSE', n-i, i-1, -cone, y( i+1, 1_ilp ),ldy, y( 1_ilp, i )&
                              , 1_ilp, cone, y( i+1, i ), 1_ilp )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', m-i, i, cone,x( i+1, 1_ilp ), ldx, a( i+&
                              1_ilp, i ), 1_ilp, czero,y( 1_ilp, i ), 1_ilp )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', i, n-i, -cone,a( 1_ilp, i+1 ), lda, y( &
                              1_ilp, i ), 1_ilp, cone,y( i+1, i ), 1_ilp )
                    call stdlib_wscal( n-i, tauq( i ), y( i+1, i ), 1_ilp )
                 else
                    call stdlib_wlacgv( n-i+1, a( i, i ), lda )
     ...
     !! [-SNL  CSL ] [  0   H  ] [ SNR  CSR ]     [  0    SSMIN ].
       ...
       ! =====================================================================
           ...
           if( ga==zero ) then
              ...
              if( ga>fa ) then
                 pmax = 2_ilp
                 if( ( fa / ga )<stdlib_qlamch( 'EPS' ) ) then
                    ! case of very large ga
                    gasmal = .false.
                    ssmax = ga
                    if( ha>one ) then
     ...
     !! rotation is by PI/2.
        ...
        ! ===================================================================
           ...
           ! generate the rotation.
           ! call stdlib_qlartgp( z, w, cs, sn, r ) might seem more natural;
           ! reordering the arguments ensures that if z = 0 then the rotation
           ! is by pi/2.
           call stdlib_qlartgp( w, z, sn, cs, r )
           return
           ! end stdlib_qlartgs
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              ! the svd of real 2-by-2 triangular c
               ! ( csl -snl )*( a b )*(  csr  snr ) = ( r 0 )
               ! ( snl  csl ) ( 0 d ) ( -snr  csr )   ( 0 t )
              call stdlib_qlasv2( a, b, d, s1, s2, snr, csr, snl, csl )
              if( abs( csl )>=abs( snl ) .or. abs( csr )>=abs( snr ) )then
                 ! compute the (1,1) and (1,2) elements of u**t *a and v**t *b,
                 ! and (1,2) element of |u|**t *|a| and |v|**t *|b|.
                 ua11r = csl*a1
                 ...
                 if( ( abs( ua11r )+abs( ua12 ) )/=zero ) then
                    if( aua12 / ( abs( ua11r )+abs( ua12 ) )<=avb12 /( abs( vb11r )+abs( vb12 ) ) &
                       ...
                       call stdlib_qlartg( -ua11r, ua12, csq, snq, r )
                    else
                       call stdlib_qlartg( -vb11r, vb12, csq, snq, r )
                    end if
                 else
                    call stdlib_qlartg( -vb11r, vb12, csq, snq, r )
                 end if
                 ...
                 if( ( abs( ua21 )+abs( ua22 ) )/=zero ) then
                    if( aua22 / ( abs( ua21 )+abs( ua22 ) )<=avb22 /( abs( vb21 )+abs( vb22 ) ) ) &
                              then
                       call stdlib_qlartg( -ua21, ua22, csq, snq, r )
                    else
                       call stdlib_qlartg( -vb21, vb22, csq, snq, r )
                    end if
                 else
                    call stdlib_qlartg( -vb21, vb22, csq, snq, r )
                 end if
              ...
              ! the svd of real 2-by-2 triangular c
               ! ( csl -snl )*( a 0 )*(  csr  snr ) = ( r 0 )
               ! ( snl  csl ) ( c d ) ( -snr  csr )   ( 0 t )
              call stdlib_qlasv2( a, c, d, s1, s2, snr, csr, snl, csl )
              if( abs( csr )>=abs( snr ) .or. abs( csl )>=abs( snl ) )then
                 ! compute the (2,1) and (2,2) elements of u**t *a and v**t *b,
                 ! and (2,1) element of |u|**t *|a| and |v|**t *|b|.
                 ua21 = -snr*a1 + csr*a2
                 ...
                 if( ( abs( ua21 )+abs( ua22r ) )/=zero ) then
                    if( aua21 / ( abs( ua21 )+abs( ua22r ) )<=avb21 /( abs( vb21 )+abs( vb22r ) ) &
                       ...
                       call stdlib_qlartg( ua22r, ua21, csq, snq, r )
                    else
                       call stdlib_qlartg( vb22r, vb21, csq, snq, r )
                    end if
                 else
                    call stdlib_qlartg( vb22r, vb21, csq, snq, r )
                 end if
                 ...
                 if( ( abs( ua11 )+abs( ua12 ) )/=zero ) then
                    if( aua11 / ( abs( ua11 )+abs( ua12 ) )<=avb11 /( abs( vb11 )+abs( vb12 ) ) ) &
                              then
                       call stdlib_qlartg( ua12, ua11, csq, snq, r )
                    else
                       call stdlib_qlartg( vb12, vb11, csq, snq, r )
                    end if
                 else
                    call stdlib_qlartg( vb12, vb11, csq, snq, r )
                 end if
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              ! the svd of real 2 by 2 triangular c
               ! ( csl -snl )*( a b )*(  csr  snr ) = ( r 0 )
               ! ( snl  csl ) ( 0 d ) ( -snr  csr )   ( 0 t )
              call stdlib_qlasv2( a, fb, d, s1, s2, snr, csr, snl, csl )
              if( abs( csl )>=abs( snl ) .or. abs( csr )>=abs( snr ) )then
                 ! compute the (1,1) and (1,2) elements of u**h *a and v**h *b,
                 ! and (1,2) element of |u|**h *|a| and |v|**h *|b|.
                 ua11r = csl*a1
                 ...
                 ! zero (2,2) elements of u**h *a and v**h *b, and then swap.
                 if( ( abs1( ua21 )+abs1( ua22 ) )==zero ) then
                    call stdlib_wlartg( -conjg( vb21 ), conjg( vb22 ), csq, snq,r )
                 else if( ( abs1( vb21 )+abs( vb22 ) )==zero ) then
                    call stdlib_wlartg( -conjg( ua21 ), conjg( ua22 ), csq, snq,r )
                 else if( aua22 / ( abs1( ua21 )+abs1( ua22 ) )<=avb22 /( abs1( vb21 )+abs1( vb22 &
                           ) ) ) then
                    call stdlib_wlartg( -conjg( ua21 ), conjg( ua22 ), csq, snq,r )
                 else
              ...
              ! the svd of real 2 by 2 triangular c
               ! ( csl -snl )*( a 0 )*(  csr  snr ) = ( r 0 )
               ! ( snl  csl ) ( c d ) ( -snr  csr )   ( 0 t )
              call stdlib_qlasv2( a, fc, d, s1, s2, snr, csr, snl, csl )
              if( abs( csr )>=abs( snr ) .or. abs( csl )>=abs( snl ) )then
                 ! compute the (2,1) and (2,2) elements of u**h *a and v**h *b,
                 ! and (2,1) element of |u|**h *|a| and |v|**h *|b|.
                 ua21 = -d1*snr*a1 + csr*a2
                 ...
                 ! zero (2,1) elements of u**h *a and v**h *b.
                 if( ( abs1( ua21 )+abs( ua22r ) )==zero ) then
                    call stdlib_wlartg( cmplx( vb22r,KIND=qp), vb21, csq, snq, r )
                 else if( ( abs1( vb21 )+abs( vb22r ) )==zero ) then
                    call stdlib_wlartg( cmplx( ua22r,KIND=qp), ua21, csq, snq, r )
                 else if( aua21 / ( abs1( ua21 )+abs( ua22r ) )<=avb21 /( abs1( vb21 )+abs( vb22r &
                           ) ) ) then
                    call stdlib_wlartg( cmplx( ua22r,KIND=qp), ua21, csq, snq, r )
                 else
                 ...
                 ! zero (1,1) elements of u**h *a and v**h *b, and then swap.
                 if( ( abs1( ua11 )+abs1( ua12 ) )==zero ) then
                    call stdlib_wlartg( vb12, vb11, csq, snq, r )
                 else if( ( abs1( vb11 )+abs1( vb12 ) )==zero ) then
                    call stdlib_wlartg( ua12, ua11, csq, snq, r )
                 else if( aua11 / ( abs1( ua11 )+abs1( ua12 ) )<=avb11 /( abs1( vb11 )+abs1( vb12 &
                           ) ) ) then
                    call stdlib_wlartg( ua12, ua11, csq, snq, r )
                 else
     ...
     !! as the measurement of the linear dependency of the vectors X and Y.
        ...
        ! =====================================================================
           ...
           ! compute the qr factorization of the n-by-2 matrix ( x y )
           call stdlib_qlarfg( n, x( 1_ilp ), x( 1_ilp+incx ), incx, tau )
           a11 = x( 1_ilp )
           x( 1_ilp ) = one
           c = -tau*stdlib_qdot( n, x, incx, y, incy )
           call stdlib_qaxpy( n, c, x, incx, y, incy )
           call stdlib_qlarfg( n-1, y( 1_ilp+incy ), y( 1_ilp+2*incy ), incy, tau )
           a12 = y( 1_ilp )
           a22 = y( 1_ilp+incy )
           ! compute the svd of 2-by-2 upper triangular matrix.
     ...
     !! as the measurement of the linear dependency of the vectors X and Y.
        ...
        ! =====================================================================
           ...
           ! compute the qr factorization of the n-by-2 matrix ( x y )
           call stdlib_wlarfg( n, x( 1_ilp ), x( 1_ilp+incx ), incx, tau )
           a11 = x( 1_ilp )
           x( 1_ilp ) = cone
           c = -conjg( tau )*stdlib_wdotc( n, x, incx, y, incy )
           call stdlib_waxpy( n, c, x, incx, y, incy )
           call stdlib_wlarfg( n-1, y( 1_ilp+incy ), y( 1_ilp+2*incy ), incy, tau )
           a12 = y( 1_ilp )
           a22 = y( 1_ilp+incy )
           ! compute the svd of 2-by-2 upper triangular matrix.
