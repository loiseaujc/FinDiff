...
!! Compute the (Moore-Penrose) pseudo-inverse of a matrix.    
     ...
     module subroutine stdlib_linalg_pseudoinvert_q(a,pinva,rtol,err)
         ...
         type(linalg_state_type) :: err0
         integer(ilp) :: m,n,k,i,j
         real(qp), parameter :: alpha = 1.0_qp, beta = 0.0_qp         
         character, parameter :: H =  'T' 
         ...
         !> Discard singular values
         cutoff = tolerance*maxval(s)
         s = merge(1.0_qp/s,0.0_qp,s>cutoff)
     ...
     module function stdlib_linalg_pinv_q_operator(a) result(pinva)
         ...
         call stdlib_linalg_pseudoinvert_q(ap,pinva,err=err)

         if (err%error()) then 
         pinva = ieee_value(1.0_qp,ieee_quiet_nan)
         endif
     ...
     module subroutine stdlib_linalg_pseudoinvert_w(a,pinva,rtol,err)
         ...
         type(linalg_state_type) :: err0
         integer(ilp) :: m,n,k,i,j
         complex(qp), parameter :: alpha = 1.0_qp, beta = 0.0_qp         
         character, parameter :: H =  'C' 
     ...
     module function stdlib_linalg_pinv_w_operator(a) result(pinva)
         ...
         call stdlib_linalg_pseudoinvert_w(ap,pinva,err=err)

         if (err%error()) then 
         pinva = cmplx(ieee_value(1.0_qp,ieee_quiet_nan), &
