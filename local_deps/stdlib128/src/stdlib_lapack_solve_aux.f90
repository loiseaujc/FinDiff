  ...
  contains
     ...
     !! Reverse communication is used for evaluating matrix-vector products.
        ...
        ! =====================================================================
           ...
           ! first iteration.  x has been overwritten by transpose(a)*x.
           40 continue
           isave( 2_ilp ) = stdlib_iqamax( n, x, 1_ilp )
           isave( 3_ilp ) = 2_ilp
           ! main loop - iterations 2,3,...,itmax.
           50 continue
           do i = 1, n
           ...
           ! x has been overwritten by a*x.
           70 continue
           call stdlib_qcopy( n, x, 1_ilp, v, 1_ilp )
           estold = est
           est = stdlib_qasum( n, v, 1_ilp )
           do i = 1, n
              if( x(i)>=zero ) then
                 xs = one
              else
           ...
           ! x has been overwritten by a*x.
           140 continue
           temp = two*( stdlib_qasum( n, x, 1_ilp ) / real( 3_ilp*n,KIND=qp) )
           if( temp>est ) then
              call stdlib_qcopy( n, x, 1_ilp, v, 1_ilp )
              est = temp
           end if
     ...
     !! Reverse communication is used for evaluating matrix-vector products.
        ...
        ! =====================================================================
           ...
           ! first iteration.  x has been overwritten by ctrans(a)*x.
           40 continue
           isave( 2_ilp ) = stdlib_iwmax1( n, x, 1_ilp )
           isave( 3_ilp ) = 2_ilp
           ! main loop - iterations 2,3,...,itmax.
           50 continue
           do i = 1, n
           ...
           ! x has been overwritten by a*x.
           70 continue
           call stdlib_wcopy( n, x, 1_ilp, v, 1_ilp )
           estold = est
           est = stdlib_qzsum1( n, v, 1_ilp )
           ! test for cycling.
           if( est<=estold )go to 100
           do i = 1, n
              absxi = abs( x( i ) )
           ...
           ! x has been overwritten by a*x.
           120 continue
           temp = two*( stdlib_qzsum1( n, x, 1_ilp ) / real( 3_ilp*n,KIND=qp) )
           if( temp>est ) then
              call stdlib_wcopy( n, x, 1_ilp, v, 1_ilp )
              est = temp
           end if
