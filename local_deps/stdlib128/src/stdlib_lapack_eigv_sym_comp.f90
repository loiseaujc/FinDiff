  ...
  contains
     ...
     !! B must have been previously factorized as U**T*U or L*L**T by DPOTRF.
        ...
        ! =====================================================================
           ...
           if( nb<=1_ilp .or. nb>=n ) then
              ! use unblocked code
              call stdlib_qsygs2( itype, uplo, n, a, lda, b, ldb, info )
           else
              ! use blocked code
              if( itype==1_ilp ) then
                 if( upper ) then
                    ...
                    do k = 1, n, nb
                       ...
                       if( k+kb<=n ) then
                          call stdlib_qtrsm( 'LEFT', uplo, 'TRANSPOSE', 'NON-UNIT',kb, n-k-kb+1, &
                                    one, b( k, k ), ldb,a( k, k+kb ), lda )
                          call stdlib_qsymm( 'LEFT', uplo, kb, n-k-kb+1, -half,a( k, k ), lda, b( &
                                    k, k+kb ), ldb, one,a( k, k+kb ), lda )
                          call stdlib_qsyr2k( uplo, 'TRANSPOSE', n-k-kb+1, kb, -one,a( k, k+kb ), &
                                    lda, b( k, k+kb ), ldb,one, a( k+kb, k+kb ), lda )
                          call stdlib_qsymm( 'LEFT', uplo, kb, n-k-kb+1, -half,a( k, k ), lda, b( &
                                    k, k+kb ), ldb, one,a( k, k+kb ), lda )
                          call stdlib_qtrsm( 'RIGHT', uplo, 'NO TRANSPOSE','NON-UNIT', kb, n-k-kb+&
                                    1_ilp, one,b( k+kb, k+kb ), ldb, a( k, k+kb ),lda )
                       end if
                    end do
                 else
                    ...
                    do k = 1, n, nb
                       ...
                       if( k+kb<=n ) then
                          call stdlib_qtrsm( 'RIGHT', uplo, 'TRANSPOSE', 'NON-UNIT',n-k-kb+1, kb, &
                                    one, b( k, k ), ldb,a( k+kb, k ), lda )
                          call stdlib_qsymm( 'RIGHT', uplo, n-k-kb+1, kb, -half,a( k, k ), lda, b(&
                                     k+kb, k ), ldb, one,a( k+kb, k ), lda )
                          call stdlib_qsyr2k( uplo, 'NO TRANSPOSE', n-k-kb+1, kb,-one, a( k+kb, k &
                                    ), lda, b( k+kb, k ),ldb, one, a( k+kb, k+kb ), lda )
                          call stdlib_qsymm( 'RIGHT', uplo, n-k-kb+1, kb, -half,a( k, k ), lda, b(&
                                     k+kb, k ), ldb, one,a( k+kb, k ), lda )
                          call stdlib_qtrsm( 'LEFT', uplo, 'NO TRANSPOSE','NON-UNIT', n-k-kb+1, &
                                    kb, one,b( k+kb, k+kb ), ldb, a( k+kb, k ),lda )
                       end if
                    end do
                 end if
                 ...
                 if( upper ) then
                    ...
                    do k = 1, n, nb
                       kb = min( n-k+1, nb )
                       ! update the upper triangle of a(1:k+kb-1,1:k+kb-1)
                       call stdlib_qtrmm( 'LEFT', uplo, 'NO TRANSPOSE', 'NON-UNIT',k-1, kb, one, &
                                 b, ldb, a( 1_ilp, k ), lda )
                       call stdlib_qsymm( 'RIGHT', uplo, k-1, kb, half, a( k, k ),lda, b( 1_ilp, k ), &
                                 ldb, one, a( 1_ilp, k ), lda )
                       call stdlib_qsyr2k( uplo, 'NO TRANSPOSE', k-1, kb, one,a( 1_ilp, k ), lda, b( &
                                 1_ilp, k ), ldb, one, a,lda )
                       call stdlib_qsymm( 'RIGHT', uplo, k-1, kb, half, a( k, k ),lda, b( 1_ilp, k ), &
                                 ldb, one, a( 1_ilp, k ), lda )
                    ...
                    do k = 1, n, nb
                       kb = min( n-k+1, nb )
                       ! update the lower triangle of a(1:k+kb-1,1:k+kb-1)
                       call stdlib_qtrmm( 'RIGHT', uplo, 'NO TRANSPOSE', 'NON-UNIT',kb, k-1, one, &
                                 b, ldb, a( k, 1_ilp ), lda )
                       call stdlib_qsymm( 'LEFT', uplo, kb, k-1, half, a( k, k ),lda, b( k, 1_ilp ), &
                                 ldb, one, a( k, 1_ilp ), lda )
                       call stdlib_qsyr2k( uplo, 'TRANSPOSE', k-1, kb, one,a( k, 1_ilp ), lda, b( k, &
                                 1_ilp ), ldb, one, a,lda )
                       call stdlib_qsymm( 'LEFT', uplo, kb, k-1, half, a( k, k ),lda, b( k, 1_ilp ), &
                                 ldb, one, a( k, 1_ilp ), lda )
           ...
           if( itype==1_ilp ) then
              if( upper ) then
                 ...
                 do k = 1, n
                    ...
                    a( k, k ) = akk
                    if( k<n ) then
                       call stdlib_qscal( n-k, one / bkk, a( k, k+1 ), lda )
                       ct = -half*akk
                       call stdlib_qaxpy( n-k, ct, b( k, k+1 ), ldb, a( k, k+1 ),lda )
                       call stdlib_qsyr2( uplo, n-k, -one, a( k, k+1 ), lda,b( k, k+1 ), ldb, a( &
                                 k+1, k+1 ), lda )
                       call stdlib_qaxpy( n-k, ct, b( k, k+1 ), ldb, a( k, k+1 ),lda )
                       call stdlib_qtrsv( uplo, 'TRANSPOSE', 'NON-UNIT', n-k,b( k+1, k+1 ), ldb, &
                                 a( k, k+1 ), lda )
                    end if
                 end do
              else
                 ...
                 do k = 1, n
                    ...
                    a( k, k ) = akk
                    if( k<n ) then
                       call stdlib_qscal( n-k, one / bkk, a( k+1, k ), 1_ilp )
                       ct = -half*akk
                       call stdlib_qaxpy( n-k, ct, b( k+1, k ), 1_ilp, a( k+1, k ), 1_ilp )
                       call stdlib_qsyr2( uplo, n-k, -one, a( k+1, k ), 1_ilp,b( k+1, k ), 1_ilp, a( k+1, &
                                 k+1 ), lda )
                       call stdlib_qaxpy( n-k, ct, b( k+1, k ), 1_ilp, a( k+1, k ), 1_ilp )
                       call stdlib_qtrsv( uplo, 'NO TRANSPOSE', 'NON-UNIT', n-k,b( k+1, k+1 ), &
                                 ldb, a( k+1, k ), 1_ilp )
                    end if
                 end do
              end if
              ...
              if( upper ) then
                 ...
                 do k = 1, n
                    ...
                    akk = a( k, k )
                    bkk = b( k, k )
                    call stdlib_qtrmv( uplo, 'NO TRANSPOSE', 'NON-UNIT', k-1, b,ldb, a( 1_ilp, k ), 1_ilp &
                              )
                    ct = half*akk
                    call stdlib_qaxpy( k-1, ct, b( 1_ilp, k ), 1_ilp, a( 1_ilp, k ), 1_ilp )
                    call stdlib_qsyr2( uplo, k-1, one, a( 1_ilp, k ), 1_ilp, b( 1_ilp, k ), 1_ilp,a, lda )
                    call stdlib_qaxpy( k-1, ct, b( 1_ilp, k ), 1_ilp, a( 1_ilp, k ), 1_ilp )
                    call stdlib_qscal( k-1, bkk, a( 1_ilp, k ), 1_ilp )
                    a( k, k ) = akk*bkk**2_ilp
                 end do
              else
                 ! compute l**t *a*l
                 do k = 1, n
                    ...
                    akk = a( k, k )
                    bkk = b( k, k )
                    call stdlib_qtrmv( uplo, 'TRANSPOSE', 'NON-UNIT', k-1, b, ldb,a( k, 1_ilp ), lda )
                              
                    ct = half*akk
                    call stdlib_qaxpy( k-1, ct, b( k, 1_ilp ), ldb, a( k, 1_ilp ), lda )
                    call stdlib_qsyr2( uplo, k-1, one, a( k, 1_ilp ), lda, b( k, 1_ilp ),ldb, a, lda )
                              
                    call stdlib_qaxpy( k-1, ct, b( k, 1_ilp ), ldb, a( k, 1_ilp ), lda )
                    call stdlib_qscal( k-1, bkk, a( k, 1_ilp ), lda )
                    a( k, k ) = akk*bkk**2_ilp
                 end do
              end if
           end if
           ...
           if( itype==1_ilp ) then
              if( upper ) then
                 ...
                 do j = 1, n
                    ...
                    ! compute the j-th column of the upper triangle of a
                    bjj = bp( jj )
                    call stdlib_qtpsv( uplo, 'TRANSPOSE', 'NONUNIT', j, bp,ap( j1 ), 1_ilp )
                    call stdlib_qspmv( uplo, j-1, -one, ap, bp( j1 ), 1_ilp, one,ap( j1 ), 1_ilp )
                    call stdlib_qscal( j-1, one / bjj, ap( j1 ), 1_ilp )
                    ap( jj ) = ( ap( jj )-stdlib_qdot( j-1, ap( j1 ), 1_ilp, bp( j1 ),1_ilp ) ) / &
                              bjj
                 end do
              else
                 ! compute inv(l)*a*inv(l**t)
                 ...
                 do k = 1, n
                    ...
                    ap( kk ) = akk
                    if( k<n ) then
                       call stdlib_qscal( n-k, one / bkk, ap( kk+1 ), 1_ilp )
                       ct = -half*akk
                       call stdlib_qaxpy( n-k, ct, bp( kk+1 ), 1_ilp, ap( kk+1 ), 1_ilp )
                       call stdlib_qspr2( uplo, n-k, -one, ap( kk+1 ), 1_ilp,bp( kk+1 ), 1_ilp, ap( k1k1 )&
                                  )
                       call stdlib_qaxpy( n-k, ct, bp( kk+1 ), 1_ilp, ap( kk+1 ), 1_ilp )
                       call stdlib_qtpsv( uplo, 'NO TRANSPOSE', 'NON-UNIT', n-k,bp( k1k1 ), ap( &
                                 kk+1 ), 1_ilp )
                    end if
                    kk = k1k1
                 end do
              ...
              if( upper ) then
                 ...
                 do k = 1, n
                    ...
                    akk = ap( kk )
                    bkk = bp( kk )
                    call stdlib_qtpmv( uplo, 'NO TRANSPOSE', 'NON-UNIT', k-1, bp,ap( k1 ), 1_ilp )
                              
                    ct = half*akk
                    call stdlib_qaxpy( k-1, ct, bp( k1 ), 1_ilp, ap( k1 ), 1_ilp )
                    call stdlib_qspr2( uplo, k-1, one, ap( k1 ), 1_ilp, bp( k1 ), 1_ilp,ap )
                    call stdlib_qaxpy( k-1, ct, bp( k1 ), 1_ilp, ap( k1 ), 1_ilp )
                    call stdlib_qscal( k-1, bkk, ap( k1 ), 1_ilp )
                    ap( kk ) = akk*bkk**2_ilp
                 end do
              else
                 ! compute l**t *a*l
                 ...
                 do j = 1, n
                    ...
                    ajj = ap( jj )
                    bjj = bp( jj )
                    ap( jj ) = ajj*bjj + stdlib_qdot( n-j, ap( jj+1 ), 1_ilp,bp( jj+1 ), 1_ilp )
                    call stdlib_qscal( n-j, bjj, ap( jj+1 ), 1_ilp )
                    call stdlib_qspmv( uplo, n-j, one, ap( j1j1 ), bp( jj+1 ), 1_ilp,one, ap( jj+1 ), &
                              1_ilp )
                    call stdlib_qtpmv( uplo, 'TRANSPOSE', 'NON-UNIT', n-j+1,bp( jj ), ap( jj ), 1_ilp &
                              )
                    jj = j1j1
                 end do
              end if
     ...
     !! bandwidth of A.
        ...
        ! =====================================================================
           ...
           inca = ldab*ka1
           ! initialize x to the unit matrix, if needed
           if( wantx )call stdlib_qlaset( 'FULL', n, n, zero, one, x, ldx )
           ! set m to the splitting point m. it must be the same value as is
           ! used in stdlib_qpbstf. the chosen value allows the arrays work and rwork
           ! to be of dimension (n).
           m = ( n+kb ) / 2_ilp
           ! the routine works in two phases, corresponding to the two halves
           ! of the split cholesky factorization of b as s**t*s where
           ...
           if( upper ) then
              ...
              if( update ) then
                 ...
                 if( wantx ) then
                    ! post-multiply x by inv(s(i))
                    call stdlib_qscal( n-m, one / bii, x( m+1, i ), 1_ilp )
                    if( kbt>0_ilp )call stdlib_qger( n-m, kbt, -one, x( m+1, i ), 1_ilp,bb( kb1-kbt, i ), &
                              1_ilp, x( m+1, i-kbt ), ldx )
                 end if
                 ! store a(i,i1) in ra1 for use in next loop over k
                 ra1 = ab( i-i1+ka1, i1 )
              ...
              loop_130: do k = 1, kb - 1
                 if( update ) then
                    ...
                    if( i-k+ka<n .and. i-k>1_ilp ) then
                       ! generate rotation to annihilate a(i,i-k+ka+1)
                       call stdlib_qlartg( ab( k+1, i-k+ka ), ra1,work( n+i-k+ka-m ), work( i-k+&
                                 ka-m ),ra )
                       ! create nonzero element a(i-k,i-k+ka+1) outside the
                       ! band and store it in work(i-k)
                       t = -bb( kb1-k, i )*ra1
                 ...
                 ! generate rotations in 1st set to annihilate elements which
                 ! have been created outside the band
                 if( nrt>0_ilp )call stdlib_qlargv( nrt, ab( 1_ilp, j2t ), inca, work( j2t-m ), ka1,work( &
                           n+j2t-m ), ka1 )
                 if( nr>0_ilp ) then
                    ! apply rotations in 1st set from the right
                    do l = 1, ka - 1
                    ...
                    ! apply rotations in 1st set from both sides to diagonal
                    ! blocks
                    call stdlib_qlar2v( nr, ab( ka1, j2 ), ab( ka1, j2+1 ),ab( ka, j2+1 ), inca, &
                              work( n+j2-m ),work( j2-m ), ka1 )
                 end if
                 ! start applying rotations in 1st set from the left
                 do l = ka - 1, kb - k + 1, -1
                    nrt = ( n-j2+l ) / ka1
                    if( nrt>0_ilp )call stdlib_qlartv( nrt, ab( l, j2+ka1-l ), inca,ab( l+1, j2+ka1-l &
                              ), inca,work( n+j2-m ), work( j2-m ), ka1 )
                 end do
                 if( wantx ) then
                    ! post-multiply x by product of rotations in 1st set
                    do j = j2, j1, ka1
                       call stdlib_qrot( n-m, x( m+1, j ), 1_ilp, x( m+1, j+1 ), 1_ilp,work( n+j-m ), &
                                 work( j-m ) )
                    end do
                 end if
              end do loop_130
              ...
              loop_170: do k = kb, 1, -1
                 ...
                 do l = kb - k, 1, -1
                    nrt = ( n-j2+ka+l ) / ka1
                    if( nrt>0_ilp )call stdlib_qlartv( nrt, ab( l, j2-l+1 ), inca,ab( l+1, j2-l+1 ), &
                              inca, work( n+j2-ka ),work( j2-ka ), ka1 )
                 end do
                 nr = ( n-j2+ka ) / ka1
                 j1 = j2 + ( nr-1 )*ka1
              ...
              loop_210: do k = kb, 1, -1
                 ...
                 if( nr>0_ilp ) then
                    ! generate rotations in 2nd set to annihilate elements
                    ! which have been created outside the band
                    call stdlib_qlargv( nr, ab( 1_ilp, j2 ), inca, work( j2 ), ka1,work( n+j2 ), ka1 )
                 ...
                 if( wantx ) then
                    ! post-multiply x by product of rotations in 2nd set
                    do j = j2, j1, ka1
                       call stdlib_qrot( n-m, x( m+1, j ), 1_ilp, x( m+1, j+1 ), 1_ilp,work( n+j ), work( &
                                 j ) )
                    end do
                 end if
              end do loop_210
              ...
              if( update ) then
                 ...
                 if( wantx ) then
                    ! post-multiply x by inv(s(i))
                    call stdlib_qscal( n-m, one / bii, x( m+1, i ), 1_ilp )
                    if( kbt>0_ilp )call stdlib_qger( n-m, kbt, -one, x( m+1, i ), 1_ilp,bb( kbt+1, i-kbt )&
                              , ldbb-1,x( m+1, i-kbt ), ldx )
                 end if
                 ! store a(i1,i) in ra1 for use in next loop over k
                 ra1 = ab( i1-i+1, i )
              ...
              loop_360: do k = 1, kb - 1
                 if( update ) then
                    ...
                    if( i-k+ka<n .and. i-k>1_ilp ) then
                       ! generate rotation to annihilate a(i-k+ka+1,i)
                       call stdlib_qlartg( ab( ka1-k, i ), ra1, work( n+i-k+ka-m ),work( i-k+ka-m &
                                 ), ra )
                       ! create nonzero element a(i-k+ka+1,i-k) outside the
                       ! band and store it in work(i-k)
                       t = -bb( k+1, i-k )*ra1
                 ...
                 ! generate rotations in 1st set to annihilate elements which
                 ! have been created outside the band
                 if( nrt>0_ilp )call stdlib_qlargv( nrt, ab( ka1, j2t-ka ), inca, work( j2t-m ),ka1, &
                           work( n+j2t-m ), ka1 )
                 if( nr>0_ilp ) then
                    ! apply rotations in 1st set from the left
                    do l = 1, ka - 1
                       call stdlib_qlartv( nr, ab( l+1, j2-l ), inca,ab( l+2, j2-l ), inca, work( &
                                 n+j2-m ),work( j2-m ), ka1 )
                    end do
                    ! apply rotations in 1st set from both sides to diagonal
                    ! blocks
                    call stdlib_qlar2v( nr, ab( 1_ilp, j2 ), ab( 1_ilp, j2+1 ), ab( 2_ilp, j2 ),inca, work( n+&
                              j2-m ), work( j2-m ), ka1 )
                 end if
                 ! start applying rotations in 1st set from the right
                 do l = ka - 1, kb - k + 1, -1
                    nrt = ( n-j2+l ) / ka1
                    if( nrt>0_ilp )call stdlib_qlartv( nrt, ab( ka1-l+1, j2 ), inca,ab( ka1-l, j2+1 ),&
                               inca, work( n+j2-m ),work( j2-m ), ka1 )
                 end do
                 if( wantx ) then
                    ! post-multiply x by product of rotations in 1st set
              ...
              loop_400: do k = kb, 1, -1
                 ...
                 do l = kb - k, 1, -1
                    nrt = ( n-j2+ka+l ) / ka1
                    if( nrt>0_ilp )call stdlib_qlartv( nrt, ab( ka1-l+1, j2-ka ), inca,ab( ka1-l, j2-&
                              ka+1 ), inca,work( n+j2-ka ), work( j2-ka ), ka1 )
                 end do
                 nr = ( n-j2+ka ) / ka1
                 j1 = j2 + ( nr-1 )*ka1
              ...
              loop_440: do k = kb, 1, -1
                 ...
                 if( nr>0_ilp ) then
                    ! generate rotations in 2nd set to annihilate elements
                    ! which have been created outside the band
                    call stdlib_qlargv( nr, ab( ka1, j2-ka ), inca, work( j2 ), ka1,work( n+j2 ), &
                              ka1 )
                    ! apply rotations in 2nd set from the left
                    do l = 1, ka - 1
                       call stdlib_qlartv( nr, ab( l+1, j2-l ), inca,ab( l+2, j2-l ), inca, work( &
           ...
           if( upper ) then
              ...
              if( update ) then
                 ...
                 if( wantx ) then
                    ! post-multiply x by inv(s(i))
                    call stdlib_qscal( nx, one / bii, x( 1_ilp, i ), 1_ilp )
                    if( kbt>0_ilp )call stdlib_qger( nx, kbt, -one, x( 1_ilp, i ), 1_ilp, bb( kb, i+1 ),ldbb-&
                              1_ilp, x( 1_ilp, i+1 ), ldx )
                 end if
                 ! store a(i1,i) in ra1 for use in next loop over k
                 ra1 = ab( i1-i+ka1, i )
              ...
              loop_610: do k = 1, kb - 1
                 if( update ) then
                    ...
                    if( i+k-ka1>0_ilp .and. i+k<m ) then
                       ! generate rotation to annihilate a(i+k-ka-1,i)
                       call stdlib_qlartg( ab( k+1, i ), ra1, work( n+i+k-ka ),work( i+k-ka ), ra &
                                 )
                       ! create nonzero element a(i+k-ka-1,i+k) outside the
                       ! band and store it in work(m-kb+i+k)
                       t = -bb( kb1-k, i+k )*ra1
                 ...
                 ! generate rotations in 1st set to annihilate elements which
                 ! have been created outside the band
                 if( nrt>0_ilp )call stdlib_qlargv( nrt, ab( 1_ilp, j1+ka ), inca, work( j1 ), ka1,work( &
                           n+j1 ), ka1 )
                 if( nr>0_ilp ) then
                    ! apply rotations in 1st set from the left
                    do l = 1, ka - 1
                       call stdlib_qlartv( nr, ab( ka1-l, j1+l ), inca,ab( ka-l, j1+l ), inca, &
                                 work( n+j1 ),work( j1 ), ka1 )
                    end do
                    ! apply rotations in 1st set from both sides to diagonal
                    ! blocks
                    call stdlib_qlar2v( nr, ab( ka1, j1 ), ab( ka1, j1-1 ),ab( ka, j1 ), inca, &
                              work( n+j1 ),work( j1 ), ka1 )
                 end if
                 ! start applying rotations in 1st set from the right
                 do l = ka - 1, kb - k + 1, -1
                    nrt = ( j2+l-1 ) / ka1
                    j1t = j2 - ( nrt-1 )*ka1
                    if( nrt>0_ilp )call stdlib_qlartv( nrt, ab( l, j1t ), inca,ab( l+1, j1t-1 ), inca,&
                               work( n+j1t ),work( j1t ), ka1 )
                 end do
                 if( wantx ) then
                    ! post-multiply x by product of rotations in 1st set
                    do j = j1, j2, ka1
                       call stdlib_qrot( nx, x( 1_ilp, j ), 1_ilp, x( 1_ilp, j-1 ), 1_ilp,work( n+j ), work( j ) )
              ...
              loop_650: do k = kb, 1, -1
                 ...
                 do l = kb - k, 1, -1
                    nrt = ( j2+ka+l-1 ) / ka1
                    j1t = j2 - ( nrt-1 )*ka1
                    if( nrt>0_ilp )call stdlib_qlartv( nrt, ab( l, j1t+ka ), inca,ab( l+1, j1t+ka-1 ),&
                               inca,work( n+m-kb+j1t+ka ),work( m-kb+j1t+ka ), ka1 )
                 end do
                 nr = ( j2+ka-1 ) / ka1
                 j1 = j2 - ( nr-1 )*ka1
              ...
              loop_690: do k = kb, 1, -1
                 ...
                 if( nr>0_ilp ) then
                    ! generate rotations in 2nd set to annihilate elements
                    ! which have been created outside the band
                    call stdlib_qlargv( nr, ab( 1_ilp, j1+ka ), inca, work( m-kb+j1 ),ka1, work( n+m-&
                              kb+j1 ), ka1 )
                    ! apply rotations in 2nd set from the left
                    do l = 1, ka - 1
                       call stdlib_qlartv( nr, ab( ka1-l, j1+l ), inca,ab( ka-l, j1+l ), inca,&
                 ...
                 if( wantx ) then
                    ! post-multiply x by product of rotations in 2nd set
                    do j = j1, j2, ka1
                       call stdlib_qrot( nx, x( 1_ilp, j ), 1_ilp, x( 1_ilp, j-1 ), 1_ilp,work( n+m-kb+j ), work( &
                                 m-kb+j ) )
                    end do
                 end if
              end do loop_690
              ...
              if( update ) then
                 ...
                 if( wantx ) then
                    ! post-multiply x by inv(s(i))
                    call stdlib_qscal( nx, one / bii, x( 1_ilp, i ), 1_ilp )
                    if( kbt>0_ilp )call stdlib_qger( nx, kbt, -one, x( 1_ilp, i ), 1_ilp, bb( 2_ilp, i ), 1_ilp,x( 1_ilp, &
                              i+1 ), ldx )
                 end if
                 ! store a(i,i1) in ra1 for use in next loop over k
                 ra1 = ab( i-i1+1, i1 )
              ...
              loop_840: do k = 1, kb - 1
                 if( update ) then
                    ...
                    if( i+k-ka1>0_ilp .and. i+k<m ) then
                       ! generate rotation to annihilate a(i,i+k-ka-1)
                       call stdlib_qlartg( ab( ka1-k, i+k-ka ), ra1,work( n+i+k-ka ), work( i+k-&
                                 ka ), ra )
                       ! create nonzero element a(i+k,i+k-ka-1) outside the
                       ! band and store it in work(m-kb+i+k)
                       t = -bb( k+1, i )*ra1
                 ...
                 ! generate rotations in 1st set to annihilate elements which
                 ! have been created outside the band
                 if( nrt>0_ilp )call stdlib_qlargv( nrt, ab( ka1, j1 ), inca, work( j1 ), ka1,work( n+&
                           j1 ), ka1 )
                 if( nr>0_ilp ) then
                    ! apply rotations in 1st set from the right
                    do l = 1, ka - 1
                       call stdlib_qlartv( nr, ab( l+1, j1 ), inca, ab( l+2, j1-1 ),inca, work( n+&
                                 j1 ), work( j1 ), ka1 )
                    end do
                    ! apply rotations in 1st set from both sides to diagonal
                    ! blocks
                    call stdlib_qlar2v( nr, ab( 1_ilp, j1 ), ab( 1_ilp, j1-1 ),ab( 2_ilp, j1-1 ), inca, work( &
                              n+j1 ),work( j1 ), ka1 )
                 end if
                 ! start applying rotations in 1st set from the left
                 do l = ka - 1, kb - k + 1, -1
                    nrt = ( j2+l-1 ) / ka1
                    j1t = j2 - ( nrt-1 )*ka1
                    if( nrt>0_ilp )call stdlib_qlartv( nrt, ab( ka1-l+1, j1t-ka1+l ), inca,ab( ka1-l, &
                              j1t-ka1+l ), inca,work( n+j1t ), work( j1t ), ka1 )
                 end do
                 if( wantx ) then
                    ! post-multiply x by product of rotations in 1st set
              ...
              loop_880: do k = kb, 1, -1
                 ...
                 do l = kb - k, 1, -1
                    nrt = ( j2+ka+l-1 ) / ka1
                    j1t = j2 - ( nrt-1 )*ka1
                    if( nrt>0_ilp )call stdlib_qlartv( nrt, ab( ka1-l+1, j1t+l-1 ), inca,ab( ka1-l, &
                              j1t+l-1 ), inca,work( n+m-kb+j1t+ka ),work( m-kb+j1t+ka ), ka1 )
                 end do
                 nr = ( j2+ka-1 ) / ka1
                 j1 = j2 - ( nr-1 )*ka1
              ...
              loop_920: do k = kb, 1, -1
                 ...
                 if( nr>0_ilp ) then
                    ! generate rotations in 2nd set to annihilate elements
                    ! which have been created outside the band
                    call stdlib_qlargv( nr, ab( ka1, j1 ), inca, work( m-kb+j1 ),ka1, work( n+m-&
                              kb+j1 ), ka1 )
                    ! apply rotations in 2nd set from the right
                    do l = 1, ka - 1
                       call stdlib_qlartv( nr, ab( l+1, j1 ), inca, ab( l+2, j1-1 ),inca, work( n+&
     ...
     !! B must have been previously factorized as U**H*U or L*L**H by ZPOTRF.
        ...
        ! =====================================================================
           ...
           if( nb<=1_ilp .or. nb>=n ) then
              ! use unblocked code
              call stdlib_whegs2( itype, uplo, n, a, lda, b, ldb, info )
           else
              ! use blocked code
              if( itype==1_ilp ) then
                 if( upper ) then
                    ...
                    do k = 1, n, nb
                       ...
                       if( k+kb<=n ) then
                          call stdlib_wtrsm( 'LEFT', uplo, 'CONJUGATE TRANSPOSE','NON-UNIT', kb, &
                                    n-k-kb+1, cone,b( k, k ), ldb, a( k, k+kb ), lda )
                          call stdlib_whemm( 'LEFT', uplo, kb, n-k-kb+1, -chalf,a( k, k ), lda, b(&
                                     k, k+kb ), ldb,cone, a( k, k+kb ), lda )
                          call stdlib_wher2k( uplo, 'CONJUGATE TRANSPOSE', n-k-kb+1,kb, -cone, a( &
                                    k, k+kb ), lda,b( k, k+kb ), ldb, one,a( k+kb, k+kb ), lda )
                          call stdlib_whemm( 'LEFT', uplo, kb, n-k-kb+1, -chalf,a( k, k ), lda, b(&
                                     k, k+kb ), ldb,cone, a( k, k+kb ), lda )
                          call stdlib_wtrsm( 'RIGHT', uplo, 'NO TRANSPOSE','NON-UNIT', kb, n-k-kb+&
                                    1_ilp, cone,b( k+kb, k+kb ), ldb, a( k, k+kb ),lda )
                       end if
                    end do
                 else
                    ...
                    do k = 1, n, nb
                       ...
                       if( k+kb<=n ) then
                          call stdlib_wtrsm( 'RIGHT', uplo, 'CONJUGATE TRANSPOSE','NON-UNIT', n-k-&
                                    kb+1, kb, cone,b( k, k ), ldb, a( k+kb, k ), lda )
                          call stdlib_whemm( 'RIGHT', uplo, n-k-kb+1, kb, -chalf,a( k, k ), lda, &
                                    b( k+kb, k ), ldb,cone, a( k+kb, k ), lda )
                          call stdlib_wher2k( uplo, 'NO TRANSPOSE', n-k-kb+1, kb,-cone, a( k+kb, &
                                    k ), lda,b( k+kb, k ), ldb, one,a( k+kb, k+kb ), lda )
                          call stdlib_whemm( 'RIGHT', uplo, n-k-kb+1, kb, -chalf,a( k, k ), lda, &
                                    b( k+kb, k ), ldb,cone, a( k+kb, k ), lda )
                          call stdlib_wtrsm( 'LEFT', uplo, 'NO TRANSPOSE','NON-UNIT', n-k-kb+1, &
                                    kb, cone,b( k+kb, k+kb ), ldb, a( k+kb, k ),lda )
                       end if
                    end do
                 end if
                 ...
                 if( upper ) then
                    ...
                    do k = 1, n, nb
                       kb = min( n-k+1, nb )
                       ! update the upper triangle of a(1:k+kb-1,1:k+kb-1)
                       call stdlib_wtrmm( 'LEFT', uplo, 'NO TRANSPOSE', 'NON-UNIT',k-1, kb, cone, &
                                 b, ldb, a( 1_ilp, k ), lda )
                       call stdlib_whemm( 'RIGHT', uplo, k-1, kb, chalf, a( k, k ),lda, b( 1_ilp, k ),&
                                  ldb, cone, a( 1_ilp, k ),lda )
                       call stdlib_wher2k( uplo, 'NO TRANSPOSE', k-1, kb, cone,a( 1_ilp, k ), lda, b( &
                                 1_ilp, k ), ldb, one, a,lda )
                       call stdlib_whemm( 'RIGHT', uplo, k-1, kb, chalf, a( k, k ),lda, b( 1_ilp, k ),&
                                  ldb, cone, a( 1_ilp, k ),lda )
                    ...
                    do k = 1, n, nb
                       kb = min( n-k+1, nb )
                       ! update the lower triangle of a(1:k+kb-1,1:k+kb-1)
                       call stdlib_wtrmm( 'RIGHT', uplo, 'NO TRANSPOSE', 'NON-UNIT',kb, k-1, cone,&
                                  b, ldb, a( k, 1_ilp ), lda )
                       call stdlib_whemm( 'LEFT', uplo, kb, k-1, chalf, a( k, k ),lda, b( k, 1_ilp ), &
                                 ldb, cone, a( k, 1_ilp ),lda )
                       call stdlib_wher2k( uplo, 'CONJUGATE TRANSPOSE', k-1, kb,cone, a( k, 1_ilp ), &
                                 lda, b( k, 1_ilp ), ldb,one, a, lda )
                       call stdlib_whemm( 'LEFT', uplo, kb, k-1, chalf, a( k, k ),lda, b( k, 1_ilp ), &
                                 ldb, cone, a( k, 1_ilp ),lda )
           ...
           if( itype==1_ilp ) then
              if( upper ) then
                 ...
                 do k = 1, n
                    ...
                    a( k, k ) = akk
                    if( k<n ) then
                       call stdlib_wdscal( n-k, one / bkk, a( k, k+1 ), lda )
                       ct = -half*akk
                       call stdlib_wlacgv( n-k, a( k, k+1 ), lda )
                       call stdlib_wlacgv( n-k, b( k, k+1 ), ldb )
                       call stdlib_waxpy( n-k, ct, b( k, k+1 ), ldb, a( k, k+1 ),lda )
                       call stdlib_wher2( uplo, n-k, -cone, a( k, k+1 ), lda,b( k, k+1 ), ldb, a( &
                                 k+1, k+1 ), lda )
                       call stdlib_waxpy( n-k, ct, b( k, k+1 ), ldb, a( k, k+1 ),lda )
                       call stdlib_wlacgv( n-k, b( k, k+1 ), ldb )
                       call stdlib_wtrsv( uplo, 'CONJUGATE TRANSPOSE', 'NON-UNIT',n-k, b( k+1, k+&
                                 1_ilp ), ldb, a( k, k+1 ),lda )
                       call stdlib_wlacgv( n-k, a( k, k+1 ), lda )
                    end if
                 end do
                 ...
                 do k = 1, n
                    ...
                    a( k, k ) = akk
                    if( k<n ) then
                       call stdlib_wdscal( n-k, one / bkk, a( k+1, k ), 1_ilp )
                       ct = -half*akk
                       call stdlib_waxpy( n-k, ct, b( k+1, k ), 1_ilp, a( k+1, k ), 1_ilp )
                       call stdlib_wher2( uplo, n-k, -cone, a( k+1, k ), 1_ilp,b( k+1, k ), 1_ilp, a( k+1,&
                                  k+1 ), lda )
                       call stdlib_waxpy( n-k, ct, b( k+1, k ), 1_ilp, a( k+1, k ), 1_ilp )
                       call stdlib_wtrsv( uplo, 'NO TRANSPOSE', 'NON-UNIT', n-k,b( k+1, k+1 ), &
                                 ldb, a( k+1, k ), 1_ilp )
                    end if
                 end do
              end if
              ...
              if( upper ) then
                 ...
                 do k = 1, n
                    ...
                    akk = real( a( k, k ),KIND=qp)
                    bkk = real( b( k, k ),KIND=qp)
                    call stdlib_wtrmv( uplo, 'NO TRANSPOSE', 'NON-UNIT', k-1, b,ldb, a( 1_ilp, k ), 1_ilp &
                              )
                    ct = half*akk
                    call stdlib_waxpy( k-1, ct, b( 1_ilp, k ), 1_ilp, a( 1_ilp, k ), 1_ilp )
                    call stdlib_wher2( uplo, k-1, cone, a( 1_ilp, k ), 1_ilp, b( 1_ilp, k ), 1_ilp,a, lda )
                              
                    call stdlib_waxpy( k-1, ct, b( 1_ilp, k ), 1_ilp, a( 1_ilp, k ), 1_ilp )
                    call stdlib_wdscal( k-1, bkk, a( 1_ilp, k ), 1_ilp )
                    a( k, k ) = akk*bkk**2_ilp
                 end do
              else
                 ! compute l**h *a*l
                 do k = 1, n
                    ...
                    akk = real( a( k, k ),KIND=qp)
                    bkk = real( b( k, k ),KIND=qp)
                    call stdlib_wlacgv( k-1, a( k, 1_ilp ), lda )
                    call stdlib_wtrmv( uplo, 'CONJUGATE TRANSPOSE', 'NON-UNIT', k-1,b, ldb, a( k, &
                              1_ilp ), lda )
                    ct = half*akk
                    call stdlib_wlacgv( k-1, b( k, 1_ilp ), ldb )
                    call stdlib_waxpy( k-1, ct, b( k, 1_ilp ), ldb, a( k, 1_ilp ), lda )
                    call stdlib_wher2( uplo, k-1, cone, a( k, 1_ilp ), lda, b( k, 1_ilp ),ldb, a, lda )
                              
                    call stdlib_waxpy( k-1, ct, b( k, 1_ilp ), ldb, a( k, 1_ilp ), lda )
                    call stdlib_wlacgv( k-1, b( k, 1_ilp ), ldb )
                    call stdlib_wdscal( k-1, bkk, a( k, 1_ilp ), lda )
                    call stdlib_wlacgv( k-1, a( k, 1_ilp ), lda )
                    a( k, k ) = akk*bkk**2_ilp
                 end do
              end if
           ...
           if( itype==1_ilp ) then
              if( upper ) then
                 ...
                 do j = 1, n
                    ...
                    call stdlib_wtpsv( uplo, 'CONJUGATE TRANSPOSE', 'NON-UNIT', j,bp, ap( j1 ), 1_ilp &
                              )
                    call stdlib_whpmv( uplo, j-1, -cone, ap, bp( j1 ), 1_ilp, cone,ap( j1 ), 1_ilp )
                              
                    call stdlib_wdscal( j-1, one / bjj, ap( j1 ), 1_ilp )
                    ap( jj ) = ( ap( jj )-stdlib_wdotc( j-1, ap( j1 ), 1_ilp, bp( j1 ),1_ilp ) ) / &
                              bjj
                 end do
              else
                 ! compute inv(l)*a*inv(l**h)
                 ...
                 do k = 1, n
                    ...
                    ap( kk ) = akk
                    if( k<n ) then
                       call stdlib_wdscal( n-k, one / bkk, ap( kk+1 ), 1_ilp )
                       ct = -half*akk
                       call stdlib_waxpy( n-k, ct, bp( kk+1 ), 1_ilp, ap( kk+1 ), 1_ilp )
                       call stdlib_whpr2( uplo, n-k, -cone, ap( kk+1 ), 1_ilp,bp( kk+1 ), 1_ilp, ap( k1k1 &
                                 ) )
                       call stdlib_waxpy( n-k, ct, bp( kk+1 ), 1_ilp, ap( kk+1 ), 1_ilp )
                       call stdlib_wtpsv( uplo, 'NO TRANSPOSE', 'NON-UNIT', n-k,bp( k1k1 ), ap( &
                                 kk+1 ), 1_ilp )
                    end if
                    kk = k1k1
                 end do
              ...
              if( upper ) then
                 ...
                 do k = 1, n
                    ...
                    akk = real( ap( kk ),KIND=qp)
                    bkk = real( bp( kk ),KIND=qp)
                    call stdlib_wtpmv( uplo, 'NO TRANSPOSE', 'NON-UNIT', k-1, bp,ap( k1 ), 1_ilp )
                              
                    ct = half*akk
                    call stdlib_waxpy( k-1, ct, bp( k1 ), 1_ilp, ap( k1 ), 1_ilp )
                    call stdlib_whpr2( uplo, k-1, cone, ap( k1 ), 1_ilp, bp( k1 ), 1_ilp,ap )
                    call stdlib_waxpy( k-1, ct, bp( k1 ), 1_ilp, ap( k1 ), 1_ilp )
                    call stdlib_wdscal( k-1, bkk, ap( k1 ), 1_ilp )
                    ap( kk ) = akk*bkk**2_ilp
                 end do
              else
                 ! compute l**h *a*l
                 ...
                 do j = 1, n
                    ...
                    bjj = real( bp( jj ),KIND=qp)
                    ap( jj ) = ajj*bjj + stdlib_wdotc( n-j, ap( jj+1 ), 1_ilp,bp( jj+1 ), 1_ilp )
                    call stdlib_wdscal( n-j, bjj, ap( jj+1 ), 1_ilp )
                    call stdlib_whpmv( uplo, n-j, cone, ap( j1j1 ), bp( jj+1 ), 1_ilp,cone, ap( jj+1 )&
                              , 1_ilp )
                    call stdlib_wtpmv( uplo, 'CONJUGATE TRANSPOSE', 'NON-UNIT',n-j+1, bp( jj ), &
                              ap( jj ), 1_ilp )
                    jj = j1j1
                 end do
              end if
     ...
     !! bandwidth of A.
        ...
        ! =====================================================================
           ...
           inca = ldab*ka1
           ! initialize x to the unit matrix, if needed
           if( wantx )call stdlib_wlaset( 'FULL', n, n, czero, cone, x, ldx )
           ! set m to the splitting point m. it must be the same value as is
           ! used in stdlib_wpbstf. the chosen value allows the arrays work and rwork
           ! to be of dimension (n).
           m = ( n+kb ) / 2_ilp
           ! the routine works in two phases, corresponding to the two halves
           ! of the split cholesky factorization of b as s**h*s where
           ...
           if( upper ) then
              ...
              if( update ) then
                 ...
                 if( wantx ) then
                    ! post-multiply x by inv(s(i))
                    call stdlib_wdscal( n-m, one / bii, x( m+1, i ), 1_ilp )
                    if( kbt>0_ilp )call stdlib_wgerc( n-m, kbt, -cone, x( m+1, i ), 1_ilp,bb( kb1-kbt, i )&
                              , 1_ilp, x( m+1, i-kbt ),ldx )
                 end if
                 ! store a(i,i1) in ra1 for use in next loop over k
                 ra1 = ab( i-i1+ka1, i1 )
              ...
              loop_130: do k = 1, kb - 1
                 if( update ) then
                    ...
                    if( i-k+ka<n .and. i-k>1_ilp ) then
                       ! generate rotation to annihilate a(i,i-k+ka+1)
                       call stdlib_wlartg( ab( k+1, i-k+ka ), ra1,rwork( i-k+ka-m ), work( i-k+ka-&
                                 m ), ra )
                       ! create nonzero element a(i-k,i-k+ka+1) outside the
                       ! band and store it in work(i-k)
                       t = -bb( kb1-k, i )*ra1
                 ...
                 ! generate rotations in 1st set to annihilate elements which
                 ! have been created outside the band
                 if( nrt>0_ilp )call stdlib_wlargv( nrt, ab( 1_ilp, j2t ), inca, work( j2t-m ), ka1,rwork(&
                            j2t-m ), ka1 )
                 if( nr>0_ilp ) then
                    ! apply rotations in 1st set from the right
                    do l = 1, ka - 1
                    ...
                    ! apply rotations in 1st set from both sides to diagonal
                    ! blocks
                    call stdlib_wlar2v( nr, ab( ka1, j2 ), ab( ka1, j2+1 ),ab( ka, j2+1 ), inca, &
                              rwork( j2-m ),work( j2-m ), ka1 )
                    call stdlib_wlacgv( nr, work( j2-m ), ka1 )
                 end if
                 ! start applying rotations in 1st set from the left
                 do l = ka - 1, kb - k + 1, -1
                    nrt = ( n-j2+l ) / ka1
                    if( nrt>0_ilp )call stdlib_wlartv( nrt, ab( l, j2+ka1-l ), inca,ab( l+1, j2+ka1-l &
                              ), inca, rwork( j2-m ),work( j2-m ), ka1 )
                 end do
                 if( wantx ) then
                    ! post-multiply x by product of rotations in 1st set
                    do j = j2, j1, ka1
                       call stdlib_wrot( n-m, x( m+1, j ), 1_ilp, x( m+1, j+1 ), 1_ilp,rwork( j-m ), &
                                 conjg( work( j-m ) ) )
                    end do
                 end if
              end do loop_130
              ...
              loop_170: do k = kb, 1, -1
                 ...
                 do l = kb - k, 1, -1
                    nrt = ( n-j2+ka+l ) / ka1
                    if( nrt>0_ilp )call stdlib_wlartv( nrt, ab( l, j2-l+1 ), inca,ab( l+1, j2-l+1 ), &
                              inca, rwork( j2-ka ),work( j2-ka ), ka1 )
                 end do
                 nr = ( n-j2+ka ) / ka1
                 j1 = j2 + ( nr-1 )*ka1
              ...
              loop_210: do k = kb, 1, -1
                 ...
                 if( nr>0_ilp ) then
                    ! generate rotations in 2nd set to annihilate elements
                    ! which have been created outside the band
                    call stdlib_wlargv( nr, ab( 1_ilp, j2 ), inca, work( j2 ), ka1,rwork( j2 ), ka1 )
                    ...
                    call stdlib_wlar2v( nr, ab( ka1, j2 ), ab( ka1, j2+1 ),ab( ka, j2+1 ), inca, &
                              rwork( j2 ),work( j2 ), ka1 )
                    call stdlib_wlacgv( nr, work( j2 ), ka1 )
                 end if
                 ! start applying rotations in 2nd set from the left
                 do l = ka - 1, kb - k + 1, -1
                    nrt = ( n-j2+l ) / ka1
                 ...
                 if( wantx ) then
                    ! post-multiply x by product of rotations in 2nd set
                    do j = j2, j1, ka1
                       call stdlib_wrot( n-m, x( m+1, j ), 1_ilp, x( m+1, j+1 ), 1_ilp,rwork( j ), conjg( &
                                 work( j ) ) )
                    end do
                 end if
              end do loop_210
              ...
              if( update ) then
                 ! form  inv(s(i))**h * a * inv(s(i))
                 bii = real( bb( 1_ilp, i ),KIND=qp)
                 ab( 1_ilp, i ) = ( real( ab( 1_ilp, i ),KIND=qp) / bii ) / bii
                 do j = i + 1, i1
                    ab( j-i+1, i ) = ab( j-i+1, i ) / bii
                 end do
                 do j = max( 1, i-ka ), i - 1
                 ...
                 do k = i - kbt, i - 1
                    do j = i - kbt, k
                       ab( k-j+1, j ) = ab( k-j+1, j ) -bb( i-j+1, j )*conjg( ab( i-k+1,k ) ) - &
                       conjg( bb( i-k+1, k ) )*ab( i-j+1, j ) + real( ab( 1_ilp, i ),KIND=qp)*bb( i-j+&
                                 1_ilp, j )*conjg( bb( i-k+1,k ) )
                    end do
                    do j = max( 1, i-ka ), i - kbt - 1
                       ab( k-j+1, j ) = ab( k-j+1, j ) -conjg( bb( i-k+1, k ) )*ab( i-j+1, j )
                 ...
                 if( wantx ) then
                    ! post-multiply x by inv(s(i))
                    call stdlib_wdscal( n-m, one / bii, x( m+1, i ), 1_ilp )
                    if( kbt>0_ilp )call stdlib_wgeru( n-m, kbt, -cone, x( m+1, i ), 1_ilp,bb( kbt+1, i-&
                              kbt ), ldbb-1,x( m+1, i-kbt ), ldx )
                 end if
                 ! store a(i1,i) in ra1 for use in next loop over k
                 ra1 = ab( i1-i+1, i )
              ...
              loop_360: do k = 1, kb - 1
                 if( update ) then
                    ...
                    if( i-k+ka<n .and. i-k>1_ilp ) then
                       ! generate rotation to annihilate a(i-k+ka+1,i)
                       call stdlib_wlartg( ab( ka1-k, i ), ra1, rwork( i-k+ka-m ),work( i-k+ka-m )&
                                 , ra )
                       ! create nonzero element a(i-k+ka+1,i-k) outside the
                       ! band and store it in work(i-k)
                       t = -bb( k+1, i-k )*ra1
                 ...
                 ! generate rotations in 1st set to annihilate elements which
                 ! have been created outside the band
                 if( nrt>0_ilp )call stdlib_wlargv( nrt, ab( ka1, j2t-ka ), inca, work( j2t-m ),ka1, &
                           rwork( j2t-m ), ka1 )
                 if( nr>0_ilp ) then
                    ! apply rotations in 1st set from the left
                    do l = 1, ka - 1
                       call stdlib_wlartv( nr, ab( l+1, j2-l ), inca,ab( l+2, j2-l ), inca, rwork(&
                                  j2-m ),work( j2-m ), ka1 )
                    end do
                    ! apply rotations in 1st set from both sides to diagonal
                    ! blocks
                    call stdlib_wlar2v( nr, ab( 1_ilp, j2 ), ab( 1_ilp, j2+1 ), ab( 2_ilp, j2 ),inca, rwork( &
                              j2-m ), work( j2-m ), ka1 )
                    call stdlib_wlacgv( nr, work( j2-m ), ka1 )
                 end if
                 ! start applying rotations in 1st set from the right
                 do l = ka - 1, kb - k + 1, -1
                    nrt = ( n-j2+l ) / ka1
                    if( nrt>0_ilp )call stdlib_wlartv( nrt, ab( ka1-l+1, j2 ), inca,ab( ka1-l, j2+1 ),&
                               inca, rwork( j2-m ),work( j2-m ), ka1 )
                 end do
                 if( wantx ) then
                    ! post-multiply x by product of rotations in 1st set
                    do j = j2, j1, ka1
                       call stdlib_wrot( n-m, x( m+1, j ), 1_ilp, x( m+1, j+1 ), 1_ilp,rwork( j-m ), work(&
                                  j-m ) )
                    end do
                 end if
              end do loop_360
              ...
              loop_400: do k = kb, 1, -1
                 ...
                 do l = kb - k, 1, -1
                    nrt = ( n-j2+ka+l ) / ka1
                    if( nrt>0_ilp )call stdlib_wlartv( nrt, ab( ka1-l+1, j2-ka ), inca,ab( ka1-l, j2-&
                              ka+1 ), inca,rwork( j2-ka ), work( j2-ka ), ka1 )
                 end do
                 nr = ( n-j2+ka ) / ka1
                 j1 = j2 + ( nr-1 )*ka1
              ...
              loop_440: do k = kb, 1, -1
                 ...
                 if( nr>0_ilp ) then
                    ! generate rotations in 2nd set to annihilate elements
                    ! which have been created outside the band
                    call stdlib_wlargv( nr, ab( ka1, j2-ka ), inca, work( j2 ), ka1,rwork( j2 ), &
                              ka1 )
                    ! apply rotations in 2nd set from the left
                    do l = 1, ka - 1
                       call stdlib_wlartv( nr, ab( l+1, j2-l ), inca,ab( l+2, j2-l ), inca, rwork(&
                 ...
                 if( wantx ) then
                    ! post-multiply x by product of rotations in 2nd set
                    do j = j2, j1, ka1
                       call stdlib_wrot( n-m, x( m+1, j ), 1_ilp, x( m+1, j+1 ), 1_ilp,rwork( j ), work( &
                                 j ) )
                    end do
                 end if
              end do loop_440
           ...
           if( upper ) then
              ...
              if( update ) then
                 ...
                 if( wantx ) then
                    ! post-multiply x by inv(s(i))
                    call stdlib_wdscal( nx, one / bii, x( 1_ilp, i ), 1_ilp )
                    if( kbt>0_ilp )call stdlib_wgeru( nx, kbt, -cone, x( 1_ilp, i ), 1_ilp,bb( kb, i+1 ), &
                              ldbb-1, x( 1_ilp, i+1 ), ldx )
                 end if
                 ! store a(i1,i) in ra1 for use in next loop over k
                 ra1 = ab( i1-i+ka1, i )
              ...
              loop_610: do k = 1, kb - 1
                 ...
                 ! generate rotations in 1st set to annihilate elements which
                 ! have been created outside the band
                 if( nrt>0_ilp )call stdlib_wlargv( nrt, ab( 1_ilp, j1+ka ), inca, work( j1 ), ka1,rwork( &
                           j1 ), ka1 )
                 if( nr>0_ilp ) then
                    ! apply rotations in 1st set from the left
                    do l = 1, ka - 1
                       call stdlib_wlartv( nr, ab( ka1-l, j1+l ), inca,ab( ka-l, j1+l ), inca, &
                                 rwork( j1 ),work( j1 ), ka1 )
                    end do
                    ! apply rotations in 1st set from both sides to diagonal
                    ! blocks
                    call stdlib_wlar2v( nr, ab( ka1, j1 ), ab( ka1, j1-1 ),ab( ka, j1 ), inca, &
                              rwork( j1 ), work( j1 ),ka1 )
                    call stdlib_wlacgv( nr, work( j1 ), ka1 )
                 end if
                 ! start applying rotations in 1st set from the right
                 do l = ka - 1, kb - k + 1, -1
                    nrt = ( j2+l-1 ) / ka1
                    j1t = j2 - ( nrt-1 )*ka1
                    if( nrt>0_ilp )call stdlib_wlartv( nrt, ab( l, j1t ), inca,ab( l+1, j1t-1 ), inca,&
                               rwork( j1t ),work( j1t ), ka1 )
                 end do
                 if( wantx ) then
                    ! post-multiply x by product of rotations in 1st set
                    do j = j1, j2, ka1
                       call stdlib_wrot( nx, x( 1_ilp, j ), 1_ilp, x( 1_ilp, j-1 ), 1_ilp,rwork( j ), work( j ) )
              ...
              loop_650: do k = kb, 1, -1
                 ...
                 do l = kb - k, 1, -1
                    nrt = ( j2+ka+l-1 ) / ka1
                    j1t = j2 - ( nrt-1 )*ka1
                    if( nrt>0_ilp )call stdlib_wlartv( nrt, ab( l, j1t+ka ), inca,ab( l+1, j1t+ka-1 ),&
                               inca,rwork( m-kb+j1t+ka ),work( m-kb+j1t+ka ), ka1 )
                 end do
                 nr = ( j2+ka-1 ) / ka1
                 j1 = j2 - ( nr-1 )*ka1
              ...
              loop_690: do k = kb, 1, -1
                 ...
                 if( nr>0_ilp ) then
                    ! generate rotations in 2nd set to annihilate elements
                    ! which have been created outside the band
                    call stdlib_wlargv( nr, ab( 1_ilp, j1+ka ), inca, work( m-kb+j1 ),ka1, rwork( m-&
                              kb+j1 ), ka1 )
                    ! apply rotations in 2nd set from the left
                    do l = 1, ka - 1
                       call stdlib_wlartv( nr, ab( ka1-l, j1+l ), inca,ab( ka-l, j1+l ), inca, &
                    ...
                    call stdlib_wlar2v( nr, ab( ka1, j1 ), ab( ka1, j1-1 ),ab( ka, j1 ), inca, &
                              rwork( m-kb+j1 ),work( m-kb+j1 ), ka1 )
                    call stdlib_wlacgv( nr, work( m-kb+j1 ), ka1 )
                 end if
                 ! start applying rotations in 2nd set from the right
                 do l = ka - 1, kb - k + 1, -1
                    nrt = ( j2+l-1 ) / ka1
                 ...
                 if( wantx ) then
                    ! post-multiply x by product of rotations in 2nd set
                    do j = j1, j2, ka1
                       call stdlib_wrot( nx, x( 1_ilp, j ), 1_ilp, x( 1_ilp, j-1 ), 1_ilp,rwork( m-kb+j ), work( &
                                 m-kb+j ) )
                    end do
                 end if
              end do loop_690
              ...
              if( update ) then
                 ...
                 do k = i + 1, i + kbt
                    do j = k, i + kbt
                       ab( j-k+1, k ) = ab( j-k+1, k ) -bb( j-i+1, i )*conjg( ab( k-i+1,i ) ) - &
                       conjg( bb( k-i+1, i ) )*ab( j-i+1, i ) + real( ab( 1_ilp, i ),KIND=qp)*bb( j-i+&
                                 1_ilp, i )*conjg( bb( k-i+1,i ) )
                    end do
                    do j = i + kbt + 1, min( n, i+ka )
                       ab( j-k+1, k ) = ab( j-k+1, k ) -conjg( bb( k-i+1, i ) )*ab( j-i+1, i )
                 ...
                 if( wantx ) then
                    ! post-multiply x by inv(s(i))
                    call stdlib_wdscal( nx, one / bii, x( 1_ilp, i ), 1_ilp )
                    if( kbt>0_ilp )call stdlib_wgerc( nx, kbt, -cone, x( 1_ilp, i ), 1_ilp, bb( 2_ilp, i ),1_ilp, x( &
                              1_ilp, i+1 ), ldx )
                 end if
                 ! store a(i,i1) in ra1 for use in next loop over k
                 ra1 = ab( i-i1+1, i1 )
              ...
              loop_840: do k = 1, kb - 1
                 if( update ) then
                    ...
                    if( i+k-ka1>0_ilp .and. i+k<m ) then
                       ! generate rotation to annihilate a(i,i+k-ka-1)
                       call stdlib_wlartg( ab( ka1-k, i+k-ka ), ra1,rwork( i+k-ka ), work( i+k-ka &
                                 ), ra )
                       ! create nonzero element a(i+k,i+k-ka-1) outside the
                       ! band and store it in work(m-kb+i+k)
                       t = -bb( k+1, i )*ra1
                 ...
                 ! generate rotations in 1st set to annihilate elements which
                 ! have been created outside the band
                 if( nrt>0_ilp )call stdlib_wlargv( nrt, ab( ka1, j1 ), inca, work( j1 ), ka1,rwork( &
                           j1 ), ka1 )
                 if( nr>0_ilp ) then
                    ! apply rotations in 1st set from the right
                    do l = 1, ka - 1
                       call stdlib_wlartv( nr, ab( l+1, j1 ), inca, ab( l+2, j1-1 ),inca, rwork( &
                                 j1 ), work( j1 ), ka1 )
                    end do
                    ! apply rotations in 1st set from both sides to diagonal
                    ! blocks
                    call stdlib_wlar2v( nr, ab( 1_ilp, j1 ), ab( 1_ilp, j1-1 ),ab( 2_ilp, j1-1 ), inca, rwork(&
                               j1 ),work( j1 ), ka1 )
                    call stdlib_wlacgv( nr, work( j1 ), ka1 )
                 end if
                 ! start applying rotations in 1st set from the left
                 do l = ka - 1, kb - k + 1, -1
                    nrt = ( j2+l-1 ) / ka1
                    j1t = j2 - ( nrt-1 )*ka1
                    if( nrt>0_ilp )call stdlib_wlartv( nrt, ab( ka1-l+1, j1t-ka1+l ), inca,ab( ka1-l, &
                              j1t-ka1+l ), inca,rwork( j1t ), work( j1t ), ka1 )
                 end do
                 if( wantx ) then
                    ! post-multiply x by product of rotations in 1st set
                    do j = j1, j2, ka1
                       call stdlib_wrot( nx, x( 1_ilp, j ), 1_ilp, x( 1_ilp, j-1 ), 1_ilp,rwork( j ), conjg( work(&
                                  j ) ) )
                    end do
                 end if
              end do loop_840
              ...
              loop_880: do k = kb, 1, -1
                 ...
                 do l = kb - k, 1, -1
                    nrt = ( j2+ka+l-1 ) / ka1
                    j1t = j2 - ( nrt-1 )*ka1
                    if( nrt>0_ilp )call stdlib_wlartv( nrt, ab( ka1-l+1, j1t+l-1 ), inca,ab( ka1-l, &
                              j1t+l-1 ), inca,rwork( m-kb+j1t+ka ),work( m-kb+j1t+ka ), ka1 )
                 end do
                 nr = ( j2+ka-1 ) / ka1
                 j1 = j2 - ( nr-1 )*ka1
              ...
              loop_920: do k = kb, 1, -1
                 ...
                 if( nr>0_ilp ) then
                    ! generate rotations in 2nd set to annihilate elements
                    ! which have been created outside the band
                    call stdlib_wlargv( nr, ab( ka1, j1 ), inca, work( m-kb+j1 ),ka1, rwork( m-kb+&
                              j1 ), ka1 )
                    ! apply rotations in 2nd set from the right
                    do l = 1, ka - 1
                       call stdlib_wlartv( nr, ab( l+1, j1 ), inca, ab( l+2, j1-1 ),inca, rwork( &
                 ...
                 if( wantx ) then
                    ! post-multiply x by product of rotations in 2nd set
                    do j = j1, j2, ka1
                       call stdlib_wrot( nx, x( 1_ilp, j ), 1_ilp, x( 1_ilp, j-1 ), 1_ilp,rwork( m-kb+j ), conjg( &
                                 work( m-kb+j ) ) )
                    end do
                 end if
              end do loop_920
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do j = n, m + 1, -1
                 ...
                 ! compute elements j-km:j-1 of the j-th column and update the
                 ! the leading submatrix within the band.
                 call stdlib_qscal( km, one / ajj, ab( kd+1-km, j ), 1_ilp )
                 call stdlib_qsyr( 'UPPER', km, -one, ab( kd+1-km, j ), 1_ilp,ab( kd+1, j-km ), kld )
              ...
              do j = n, m + 1, -1
                 ...
                 call stdlib_qsyr( 'LOWER', km, -one, ab( km+1, j-km ), kld,ab( 1_ilp, j-km ), kld )
              ...
              do j = 1, m
                 ...
                 ! trailing submatrix within the band.
                 if( km>0_ilp ) then
                    call stdlib_qscal( km, one / ajj, ab( 2_ilp, j ), 1_ilp )
                    call stdlib_qsyr( 'LOWER', km, -one, ab( 2_ilp, j ), 1_ilp,ab( 1_ilp, j+1 ), kld )
                 end if
              end do
           end if
           return
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do j = n, m + 1, -1
                 ...
                 ! compute elements j-km:j-1 of the j-th column and update the
                 ! the leading submatrix within the band.
                 call stdlib_wdscal( km, one / ajj, ab( kd+1-km, j ), 1_ilp )
                 call stdlib_wher( 'UPPER', km, -one, ab( kd+1-km, j ), 1_ilp,ab( kd+1, j-km ), kld )
              ...
              do j = n, m + 1, -1
                 ...
                 call stdlib_wher( 'LOWER', km, -one, ab( km+1, j-km ), kld,ab( 1_ilp, j-km ), kld )
              ...
              do j = 1, m
                 ...
                 ! trailing submatrix within the band.
                 if( km>0_ilp ) then
                    call stdlib_wdscal( km, one / ajj, ab( 2_ilp, j ), 1_ilp )
                    call stdlib_wher( 'LOWER', km, -one, ab( 2_ilp, j ), 1_ilp,ab( 1_ilp, j+1 ), kld )
                 end if
              end do
           end if
           return
           ...
           if( left ) then
              if( notran ) then
                 do i = 1, n, nb
                    ...
                    ldwork = m
                    ! multiply bottom part of c by q12.
                    call stdlib_qlacpy( 'ALL', n1, len, c( n2+1, i ), ldc, work,ldwork )
                    call stdlib_qtrmm( 'LEFT', 'LOWER', 'NO TRANSPOSE', 'NON-UNIT',n1, len, one, &
                              q( 1_ilp, n2+1 ), ldq, work,ldwork )
                    ! multiply top part of c by q11.
                    call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', n1, len, n2,one, q, ldq, c(&
                               1_ilp, i ), ldc, one, work,ldwork )
                    ! multiply top part of c by q21.
                    call stdlib_qlacpy( 'ALL', n2, len, c( 1_ilp, i ), ldc,work( n1+1 ), ldwork )
                              
                    call stdlib_qtrmm( 'LEFT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT',n2, len, one, &
                              q( n1+1, 1_ilp ), ldq,work( n1+1 ), ldwork )
                    ! multiply bottom part of c by q22.
                    call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', n2, len, n1,one, q( n1+1, &
                              n2+1 ), ldq, c( n2+1, i ), ldc,one, work( n1+1 ), ldwork )
                    ! copy everything back.
                    call stdlib_qlacpy( 'ALL', m, len, work, ldwork, c( 1_ilp, i ),ldc )
                 end do
              else
                 do i = 1, n, nb
                    len = min( nb, n-i+1 )
                    ldwork = m
                    ! multiply bottom part of c by q21**t.
                    call stdlib_qlacpy( 'ALL', n2, len, c( n1+1, i ), ldc, work,ldwork )
                    call stdlib_qtrmm( 'LEFT', 'UPPER', 'TRANSPOSE', 'NON-UNIT',n2, len, one, q( &
                              n1+1, 1_ilp ), ldq, work,ldwork )
                    ! multiply top part of c by q11**t.
                    call stdlib_qgemm( 'TRANSPOSE', 'NO TRANSPOSE', n2, len, n1,one, q, ldq, c( 1_ilp,&
                               i ), ldc, one, work,ldwork )
                    ! multiply top part of c by q12**t.
                    call stdlib_qlacpy( 'ALL', n1, len, c( 1_ilp, i ), ldc,work( n2+1 ), ldwork )
                              
                    call stdlib_qtrmm( 'LEFT', 'LOWER', 'TRANSPOSE', 'NON-UNIT',n1, len, one, q( &
                              1_ilp, n2+1 ), ldq,work( n2+1 ), ldwork )
                    ! multiply bottom part of c by q22**t.
                    call stdlib_qgemm( 'TRANSPOSE', 'NO TRANSPOSE', n1, len, n2,one, q( n1+1, n2+&
                              1_ilp ), ldq, c( n1+1, i ), ldc,one, work( n2+1 ), ldwork )
                    ! copy everything back.
                    call stdlib_qlacpy( 'ALL', m, len, work, ldwork, c( 1_ilp, i ),ldc )
                 end do
              ...
              if( notran ) then
                 do i = 1, m, nb
                    ...
                    ldwork = len
                    ! multiply right part of c by q21.
                    call stdlib_qlacpy( 'ALL', len, n2, c( i, n1+1 ), ldc, work,ldwork )
                    call stdlib_qtrmm( 'RIGHT', 'UPPER', 'NO TRANSPOSE', 'NON-UNIT',len, n2, one, &
                              q( n1+1, 1_ilp ), ldq, work,ldwork )
                    ! multiply left part of c by q11.
                    call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', len, n2, n1,one, c( i, 1_ilp ),&
                               ldc, q, ldq, one, work,ldwork )
                    ! multiply left part of c by q12.
                    call stdlib_qlacpy( 'ALL', len, n1, c( i, 1_ilp ), ldc,work( 1_ilp + n2*ldwork ), &
                              ldwork )
                    call stdlib_qtrmm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', 'NON-UNIT',len, n1, one, &
                              q( 1_ilp, n2+1 ), ldq,work( 1_ilp + n2*ldwork ), ldwork )
                    ! multiply right part of c by q22.
                    call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', len, n1, n2,one, c( i, n1+&
                              1_ilp ), ldc, q( n1+1, n2+1 ), ldq,one, work( 1_ilp + n2*ldwork ), ldwork )
                    ! copy everything back.
                    call stdlib_qlacpy( 'ALL', len, n, work, ldwork, c( i, 1_ilp ),ldc )
                 end do
              else
                 do i = 1, m, nb
                    len = min( nb, m-i+1 )
                    ldwork = len
                    ! multiply right part of c by q12**t.
                    call stdlib_qlacpy( 'ALL', len, n1, c( i, n2+1 ), ldc, work,ldwork )
                    call stdlib_qtrmm( 'RIGHT', 'LOWER', 'TRANSPOSE', 'NON-UNIT',len, n1, one, q( &
                              1_ilp, n2+1 ), ldq, work,ldwork )
                    ! multiply left part of c by q11**t.
                    call stdlib_qgemm( 'NO TRANSPOSE', 'TRANSPOSE', len, n1, n2,one, c( i, 1_ilp ), &
                              ldc, q, ldq, one, work,ldwork )
                    ! multiply left part of c by q21**t.
                    call stdlib_qlacpy( 'ALL', len, n2, c( i, 1_ilp ), ldc,work( 1_ilp + n1*ldwork ), &
                              ldwork )
                    call stdlib_qtrmm( 'RIGHT', 'UPPER', 'TRANSPOSE', 'NON-UNIT',len, n2, one, q( &
                              n1+1, 1_ilp ), ldq,work( 1_ilp + n1*ldwork ), ldwork )
                    ! multiply right part of c by q22**t.
                    call stdlib_qgemm( 'NO TRANSPOSE', 'TRANSPOSE', len, n2, n1,one, c( i, n2+1 ),&
                               ldc, q( n1+1, n2+1 ), ldq,one, work( 1_ilp + n1*ldwork ), ldwork )
                    ! copy everything back.
                    call stdlib_qlacpy( 'ALL', len, n, work, ldwork, c( i, 1_ilp ),ldc )
                 end do
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( uplo, 'U' ) ) then
              ...
              loop_10: do i = n, n - nb + 1, -1
                 ...
                 if( i<n ) then
                    ! update a(1:i,i)
                    call stdlib_qgemv( 'NO TRANSPOSE', i, n-i, -one, a( 1_ilp, i+1 ),lda, w( i, iw+1 )&
                              , ldw, one, a( 1_ilp, i ), 1_ilp )
                    call stdlib_qgemv( 'NO TRANSPOSE', i, n-i, -one, w( 1_ilp, iw+1 ),ldw, a( i, i+1 )&
                              , lda, one, a( 1_ilp, i ), 1_ilp )
                 end if
                 if( i>1_ilp ) then
                    ! generate elementary reflector h(i) to annihilate
                    ! a(1:i-2,i)
                    call stdlib_qlarfg( i-1, a( i-1, i ), a( 1_ilp, i ), 1_ilp, tau( i-1 ) )
                    e( i-1 ) = a( i-1, i )
                    a( i-1, i ) = one
                    ! compute w(1:i-1,i)
                    call stdlib_qsymv( 'UPPER', i-1, one, a, lda, a( 1_ilp, i ), 1_ilp,zero, w( 1_ilp, iw ), &
                              1_ilp )
                    if( i<n ) then
                       call stdlib_qgemv( 'TRANSPOSE', i-1, n-i, one, w( 1_ilp, iw+1 ),ldw, a( 1_ilp, i ),&
                                  1_ilp, zero, w( i+1, iw ), 1_ilp )
                       call stdlib_qgemv( 'NO TRANSPOSE', i-1, n-i, -one,a( 1_ilp, i+1 ), lda, w( i+1,&
                                  iw ), 1_ilp, one,w( 1_ilp, iw ), 1_ilp )
                       call stdlib_qgemv( 'TRANSPOSE', i-1, n-i, one, a( 1_ilp, i+1 ),lda, a( 1_ilp, i ), &
                                 1_ilp, zero, w( i+1, iw ), 1_ilp )
                       call stdlib_qgemv( 'NO TRANSPOSE', i-1, n-i, -one,w( 1_ilp, iw+1 ), ldw, w( i+&
                                 1_ilp, iw ), 1_ilp, one,w( 1_ilp, iw ), 1_ilp )
                    end if
                    call stdlib_qscal( i-1, tau( i-1 ), w( 1_ilp, iw ), 1_ilp )
                    alpha = -half*tau( i-1 )*stdlib_qdot( i-1, w( 1_ilp, iw ), 1_ilp,a( 1_ilp, i ), 1_ilp )
                              
                    call stdlib_qaxpy( i-1, alpha, a( 1_ilp, i ), 1_ilp, w( 1_ilp, iw ), 1_ilp )
                 end if
              end do loop_10
           else
              ! reduce first nb columns of lower triangle
              do i = 1, nb
                 ! update a(i:n,i)
                 call stdlib_qgemv( 'NO TRANSPOSE', n-i+1, i-1, -one, a( i, 1_ilp ),lda, w( i, 1_ilp ), &
                           ldw, one, a( i, i ), 1_ilp )
                 call stdlib_qgemv( 'NO TRANSPOSE', n-i+1, i-1, -one, w( i, 1_ilp ),ldw, a( i, 1_ilp ), &
                           lda, one, a( i, i ), 1_ilp )
                 if( i<n ) then
                    ! generate elementary reflector h(i) to annihilate
                    ! a(i+2:n,i)
                    call stdlib_qlarfg( n-i, a( i+1, i ), a( min( i+2, n ), i ), 1_ilp,tau( i ) )
                              
                    e( i ) = a( i+1, i )
                    a( i+1, i ) = one
                    ! compute w(i+1:n,i)
                    call stdlib_qsymv( 'LOWER', n-i, one, a( i+1, i+1 ), lda,a( i+1, i ), 1_ilp, zero,&
                               w( i+1, i ), 1_ilp )
                    call stdlib_qgemv( 'TRANSPOSE', n-i, i-1, one, w( i+1, 1_ilp ), ldw,a( i+1, i ), &
                              1_ilp, zero, w( 1_ilp, i ), 1_ilp )
                    call stdlib_qgemv( 'NO TRANSPOSE', n-i, i-1, -one, a( i+1, 1_ilp ),lda, w( 1_ilp, i ),&
                               1_ilp, one, w( i+1, i ), 1_ilp )
                    call stdlib_qgemv( 'TRANSPOSE', n-i, i-1, one, a( i+1, 1_ilp ), lda,a( i+1, i ), &
                              1_ilp, zero, w( 1_ilp, i ), 1_ilp )
                    call stdlib_qgemv( 'NO TRANSPOSE', n-i, i-1, -one, w( i+1, 1_ilp ),ldw, w( 1_ilp, i ),&
                               1_ilp, one, w( i+1, i ), 1_ilp )
                    call stdlib_qscal( n-i, tau( i ), w( i+1, i ), 1_ilp )
                    alpha = -half*tau( i )*stdlib_qdot( n-i, w( i+1, i ), 1_ilp,a( i+1, i ), 1_ilp )
                              
                    call stdlib_qaxpy( n-i, alpha, a( i+1, i ), 1_ilp, w( i+1, i ), 1_ilp )
                 end if
              end do
           end if
           return
           ...
           if( stdlib_lsame( uplo, 'U' ) ) then
              ...
              loop_10: do i = n, n - nb + 1, -1
                 ...
                 if( i<n ) then
                    ...
                    call stdlib_wlacgv( n-i, w( i, iw+1 ), ldw )
                    call stdlib_wgemv( 'NO TRANSPOSE', i, n-i, -cone, a( 1_ilp, i+1 ),lda, w( i, iw+1 &
                              ), ldw, cone, a( 1_ilp, i ), 1_ilp )
                    call stdlib_wlacgv( n-i, w( i, iw+1 ), ldw )
                    call stdlib_wlacgv( n-i, a( i, i+1 ), lda )
                    call stdlib_wgemv( 'NO TRANSPOSE', i, n-i, -cone, w( 1_ilp, iw+1 ),ldw, a( i, i+1 &
                              ), lda, cone, a( 1_ilp, i ), 1_ilp )
                    call stdlib_wlacgv( n-i, a( i, i+1 ), lda )
                    a( i, i ) = real( a( i, i ),KIND=qp)
                 end if
                 if( i>1_ilp ) then
                    ...
                    ! a(1:i-2,i)
                    alpha = a( i-1, i )
                    call stdlib_wlarfg( i-1, alpha, a( 1_ilp, i ), 1_ilp, tau( i-1 ) )
                    e( i-1 ) = real( alpha,KIND=qp)
                    a( i-1, i ) = cone
                    ! compute w(1:i-1,i)
                    call stdlib_whemv( 'UPPER', i-1, cone, a, lda, a( 1_ilp, i ), 1_ilp,czero, w( 1_ilp, iw ),&
                               1_ilp )
                    if( i<n ) then
                       call stdlib_wgemv( 'CONJUGATE TRANSPOSE', i-1, n-i, cone,w( 1_ilp, iw+1 ), ldw,&
                                  a( 1_ilp, i ), 1_ilp, czero,w( i+1, iw ), 1_ilp )
                       call stdlib_wgemv( 'NO TRANSPOSE', i-1, n-i, -cone,a( 1_ilp, i+1 ), lda, w( i+&
                                 1_ilp, iw ), 1_ilp, cone,w( 1_ilp, iw ), 1_ilp )
                       call stdlib_wgemv( 'CONJUGATE TRANSPOSE', i-1, n-i, cone,a( 1_ilp, i+1 ), lda, &
                                 a( 1_ilp, i ), 1_ilp, czero,w( i+1, iw ), 1_ilp )
                       call stdlib_wgemv( 'NO TRANSPOSE', i-1, n-i, -cone,w( 1_ilp, iw+1 ), ldw, w( i+&
                                 1_ilp, iw ), 1_ilp, cone,w( 1_ilp, iw ), 1_ilp )
                    end if
                    call stdlib_wscal( i-1, tau( i-1 ), w( 1_ilp, iw ), 1_ilp )
                    alpha = -chalf*tau( i-1 )*stdlib_wdotc( i-1, w( 1_ilp, iw ), 1_ilp,a( 1_ilp, i ), 1_ilp )
                              
                    call stdlib_waxpy( i-1, alpha, a( 1_ilp, i ), 1_ilp, w( 1_ilp, iw ), 1_ilp )
                 end if
              end do loop_10
           else
              ! reduce first nb columns of lower triangle
              loop_20: do i = 1, nb
                 ! update a(i:n,i)
                 a( i, i ) = real( a( i, i ),KIND=qp)
                 call stdlib_wlacgv( i-1, w( i, 1_ilp ), ldw )
                 call stdlib_wgemv( 'NO TRANSPOSE', n-i+1, i-1, -cone, a( i, 1_ilp ),lda, w( i, 1_ilp ), &
                           ldw, cone, a( i, i ), 1_ilp )
                 call stdlib_wlacgv( i-1, w( i, 1_ilp ), ldw )
                 call stdlib_wlacgv( i-1, a( i, 1_ilp ), lda )
                 call stdlib_wgemv( 'NO TRANSPOSE', n-i+1, i-1, -cone, w( i, 1_ilp ),ldw, a( i, 1_ilp ), &
                           lda, cone, a( i, i ), 1_ilp )
                 call stdlib_wlacgv( i-1, a( i, 1_ilp ), lda )
                 a( i, i ) = real( a( i, i ),KIND=qp)
                 if( i<n ) then
                    ...
                    ! a(i+2:n,i)
                    alpha = a( i+1, i )
                    call stdlib_wlarfg( n-i, alpha, a( min( i+2, n ), i ), 1_ilp,tau( i ) )
                    e( i ) = real( alpha,KIND=qp)
                    a( i+1, i ) = cone
                    ! compute w(i+1:n,i)
                    call stdlib_whemv( 'LOWER', n-i, cone, a( i+1, i+1 ), lda,a( i+1, i ), 1_ilp, &
                              czero, w( i+1, i ), 1_ilp )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', n-i, i-1, cone,w( i+1, 1_ilp ), ldw, a( &
                              i+1, i ), 1_ilp, czero,w( 1_ilp, i ), 1_ilp )
                    call stdlib_wgemv( 'NO TRANSPOSE', n-i, i-1, -cone, a( i+1, 1_ilp ),lda, w( 1_ilp, i )&
                              , 1_ilp, cone, w( i+1, i ), 1_ilp )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', n-i, i-1, cone,a( i+1, 1_ilp ), lda, a( &
                              i+1, i ), 1_ilp, czero,w( 1_ilp, i ), 1_ilp )
                    call stdlib_wgemv( 'NO TRANSPOSE', n-i, i-1, -cone, w( i+1, 1_ilp ),ldw, w( 1_ilp, i )&
                              , 1_ilp, cone, w( i+1, i ), 1_ilp )
                    call stdlib_wscal( n-i, tau( i ), w( i+1, i ), 1_ilp )
                    alpha = -chalf*tau( i )*stdlib_wdotc( n-i, w( i+1, i ), 1_ilp,a( i+1, i ), 1_ilp )
                              
                    call stdlib_waxpy( n-i, alpha, a( i+1, i ), 1_ilp, w( i+1, i ), 1_ilp )
                 end if
              end do loop_20
           end if
           return
     ...
     !! being intended for use in applications such as inverse iteration.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           eps = stdlib_qlamch( 'EPSILON' )
           sfmin = stdlib_qlamch( 'SAFE MINIMUM' )
           bignum = one / sfmin
           if( job<0_ilp ) then
              if( tol<=zero ) then
                 tol = abs( a( 1_ilp ) )
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do i = n - 1, 1, -1
                 ! generate elementary reflector h(i) = i - tau * v * v**t
                 ! to annihilate a(1:i-1,i+1)
                 call stdlib_qlarfg( i, ap( i1+i-1 ), ap( i1 ), 1_ilp, taui )
                 e( i ) = ap( i1+i-1 )
                 if( taui/=zero ) then
                    ! apply h(i) from both sides to a(1:i,1:i)
                    ap( i1+i-1 ) = one
                    ! compute  y := tau * a * v  storing y in tau(1:i)
                    call stdlib_qspmv( uplo, i, taui, ap, ap( i1 ), 1_ilp, zero, tau,1_ilp )
                    ! compute  w := y - 1/2 * tau * (y**t *v) * v
                    alpha = -half*taui*stdlib_qdot( i, tau, 1_ilp, ap( i1 ), 1_ilp )
                    call stdlib_qaxpy( i, alpha, ap( i1 ), 1_ilp, tau, 1_ilp )
                    ! apply the transformation as a rank-2 update:
                       ! a := a - v * w**t - w * v**t
                    call stdlib_qspr2( uplo, i, -one, ap( i1 ), 1_ilp, tau, 1_ilp, ap )
                    ap( i1+i-1 ) = e( i )
                 end if
                 d( i+1 ) = ap( i1+i )
                 tau( i ) = taui
              ...
              do i = 1, n - 1
                 ...
                 ! generate elementary reflector h(i) = i - tau * v * v**t
                 ! to annihilate a(i+2:n,i)
                 call stdlib_qlarfg( n-i, ap( ii+1 ), ap( ii+2 ), 1_ilp, taui )
                 e( i ) = ap( ii+1 )
                 if( taui/=zero ) then
                    ! apply h(i) from both sides to a(i+1:n,i+1:n)
                    ap( ii+1 ) = one
                    ! compute  y := tau * a * v  storing y in tau(i:n-1)
                    call stdlib_qspmv( uplo, n-i, taui, ap( i1i1 ), ap( ii+1 ), 1_ilp,zero, tau( i ), &
                              1_ilp )
                    ! compute  w := y - 1/2 * tau * (y**t *v) * v
                    alpha = -half*taui*stdlib_qdot( n-i, tau( i ), 1_ilp, ap( ii+1 ),1_ilp )
                    call stdlib_qaxpy( n-i, alpha, ap( ii+1 ), 1_ilp, tau( i ), 1_ilp )
                    ! apply the transformation as a rank-2 update:
                       ! a := a - v * w**t - w * v**t
                    call stdlib_qspr2( uplo, n-i, -one, ap( ii+1 ), 1_ilp, tau( i ), 1_ilp,ap( i1i1 ) )
     ...
     !! if UPLO = 'L', Q = H(1) H(2) . . . H(n-1).
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           if( upper ) then
              ! q was determined by a call to stdlib_qsptrd with uplo = 'u'
              ! unpack the vectors which define the elementary reflectors and
              ! set the last row and column of q equal to those of the unit
              ! matrix
              ij = 2_ilp
              ...
              q( n, n ) = one
              ! generate q(1:n-1,1:n-1)
              call stdlib_qorg2l( n-1, n-1, n-1, q, ldq, tau, work, iinfo )
           else
              ! q was determined by a call to stdlib_qsptrd with uplo = 'l'.
              ! unpack the vectors which define the elementary reflectors and
              ! set the first row and column of q equal to those of the unit
              ! matrix
              q( 1_ilp, 1_ilp ) = one
              ...
              if( n>1_ilp ) then
                 ! generate q(2:n,2:n)
                 call stdlib_qorg2r( n-1, n-1, n-1, q( 2_ilp, 2_ilp ), ldq, tau, work,iinfo )
              end if
           end if
           return
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do i = i1, i2, i3
                 ...
                 aii = ap( ii )
                 ap( ii ) = one
                 call stdlib_qlarf( side, mi, ni, ap( ii-i+1 ), 1_ilp, tau( i ), c, ldc,work )
                 ap( ii ) = aii
                 if( forwrd ) then
                    ii = ii + i + 2_ilp
                 else
              ...
              do i = i1, i2, i3
                 ...
                 ! apply h(i)
                 call stdlib_qlarf( side, mi, ni, ap( ii ), 1_ilp, tau( i ),c( ic, jc ), ldc, work )
     ...
     !! Q**T * A * Q = T.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           ! initialize q to the unit matrix, if needed
           if( initq )call stdlib_qlaset( 'FULL', n, n, zero, one, q, ldq )
           ! wherever possible, plane rotations are generated and applied in
           ! vector operations of length nr over the index set j1:j2:kd1.
           ! the cosines and sines of the plane rotations are stored in the
           ! arrays d and work.
                 ...
                 loop_90: do i = 1, n - 2
                    ...
                    loop_80: do k = kdn + 1, 2, -1
                       ...
                       if( nr>0_ilp ) then
                          ! generate plane rotations to annihilate nonzero
                          ! elements which have been created outside the band
                          call stdlib_qlargv( nr, ab( 1_ilp, j1-1 ), inca, work( j1 ),kd1, d( j1 ), &
                                    kd1 )
                          ! apply rotations from the right
                          ! dependent on the the number of diagonals either
                          ! stdlib_qlartv or stdlib_qrot is used
                          if( nr>=2_ilp*kd-1 ) then
                             do l = 1, kd - 1
                                call stdlib_qlartv( nr, ab( l+1, j1-1 ), inca,ab( l, j1 ), inca, &
                                          d( j1 ),work( j1 ), kd1 )
                             end do
                          else
                             jend = j1 + ( nr-1 )*kd1
                             do jinc = j1, jend, kd1
                                call stdlib_qrot( kdm1, ab( 2_ilp, jinc-1 ), 1_ilp,ab( 1_ilp, jinc ), 1_ilp, d( &
                                          jinc ),work( jinc ) )
                             end do
                          end if
                       end if
                       if( k>2_ilp ) then
                          if( k<=n-i+1 ) then
                             ! generate plane rotation to annihilate a(i,i+k-1)
                             ! within the band
                             call stdlib_qlartg( ab( kd-k+3, i+k-2 ),ab( kd-k+2, i+k-1 ), d( i+k-&
                                       1_ilp ),work( i+k-1 ), temp )
                             ab( kd-k+3, i+k-2 ) = temp
                             ! apply rotation from the right
                             call stdlib_qrot( k-3, ab( kd-k+4, i+k-2 ), 1_ilp,ab( kd-k+3, i+k-1 ), 1_ilp,&
                                        d( i+k-1 ),work( i+k-1 ) )
                          end if
                          nr = nr + 1_ilp
                          j1 = j1 - kdn - 1_ilp
                       ...
                       ! apply plane rotations from both sides to diagonal
                       ! blocks
                       if( nr>0_ilp )call stdlib_qlar2v( nr, ab( kd1, j1-1 ), ab( kd1, j1 ),ab( kd, &
                                 j1 ), inca, d( j1 ),work( j1 ), kd1 )
                       ! apply plane rotations from the left
                       if( nr>0_ilp ) then
                          if( 2_ilp*kd-1<nr ) then
                          ...
                          ! stdlib_qlartv or stdlib_qrot is used
                             do l = 1, kd - 1
                                ...
                                if( nrt>0_ilp )call stdlib_qlartv( nrt, ab( kd-l, j1+l ), inca,ab( kd-&
                                          l+1, j1+l ), inca,d( j1 ), work( j1 ), kd1 )
                             end do
                          else
                             j1end = j1 + kd1*( nr-2 )
                             if( j1end>=j1 ) then
                                do jin = j1, j1end, kd1
                                   call stdlib_qrot( kd-1, ab( kd-1, jin+1 ), incx,ab( kd, jin+1 )&
                                             , incx,d( jin ), work( jin ) )
                                end do
                             end if
                             lend = min( kdm1, n-j2 )
                             last = j1end + kd1
                             if( lend>0_ilp )call stdlib_qrot( lend, ab( kd-1, last+1 ), incx,ab( kd, &
                                       last+1 ), incx, d( last ),work( last ) )
                          end if
                       end if
                       if( wantq ) then
                       ...
                       ! initially the identity matrix
                             ...
                             do j = j1, j2, kd1
                                ...
                                nq = 1_ilp + iqaend - iqb
                                iqaend = min( iqaend+kd, iqend )
                                call stdlib_qrot( nq, q( iqb, j-1 ), 1_ilp, q( iqb, j ),1_ilp, d( j ), &
                                          work( j ) )
                             end do
                          else
                             do j = j1, j2, kd1
                                call stdlib_qrot( n, q( 1_ilp, j-1 ), 1_ilp, q( 1_ilp, j ), 1_ilp,d( j ), work( j &
                                          ) )
                             end do
                          end if
                       end if
                 ...
                 loop_210: do i = 1, n - 2
                    ...
                    loop_200: do k = kdn + 1, 2, -1
                       ...
                       if( nr>0_ilp ) then
                          ! generate plane rotations to annihilate nonzero
                          ! elements which have been created outside the band
                          call stdlib_qlargv( nr, ab( kd1, j1-kd1 ), inca,work( j1 ), kd1, d( j1 )&
                                    , kd1 )
                          ! apply plane rotations from one side
                          ! dependent on the the number of diagonals either
                          ! stdlib_qlartv or stdlib_qrot is used
                          if( nr>2_ilp*kd-1 ) then
                             do l = 1, kd - 1
                                call stdlib_qlartv( nr, ab( kd1-l, j1-kd1+l ), inca,ab( kd1-l+1, &
                                          j1-kd1+l ), inca,d( j1 ), work( j1 ), kd1 )
                             end do
                          else
                             jend = j1 + kd1*( nr-1 )
                             do jinc = j1, jend, kd1
                                call stdlib_qrot( kdm1, ab( kd, jinc-kd ), incx,ab( kd1, jinc-kd )&
                                          , incx,d( jinc ), work( jinc ) )
                             end do
                          end if
                       end if
                       if( k>2_ilp ) then
                          if( k<=n-i+1 ) then
                             ! generate plane rotation to annihilate a(i+k-1,i)
                             ! within the band
                             call stdlib_qlartg( ab( k-1, i ), ab( k, i ),d( i+k-1 ), work( i+k-1 &
                                       ), temp )
                             ab( k-1, i ) = temp
                             ! apply rotation from the left
                             call stdlib_qrot( k-3, ab( k-2, i+1 ), ldab-1,ab( k-1, i+1 ), ldab-1,&
                                        d( i+k-1 ),work( i+k-1 ) )
                          end if
                          nr = nr + 1_ilp
                          j1 = j1 - kdn - 1_ilp
                       ...
                       ! apply plane rotations from both sides to diagonal
                       ! blocks
                       if( nr>0_ilp )call stdlib_qlar2v( nr, ab( 1_ilp, j1-1 ), ab( 1_ilp, j1 ),ab( 2_ilp, j1-1 ),&
                                  inca, d( j1 ),work( j1 ), kd1 )
                       ! apply plane rotations from the right
                          ! dependent on the the number of diagonals either
                          ! stdlib_qlartv or stdlib_qrot is used
                       if( nr>0_ilp ) then
                          if( nr>2_ilp*kd-1 ) then
                             do l = 1, kd - 1
                                ...
                                if( nrt>0_ilp )call stdlib_qlartv( nrt, ab( l+2, j1-1 ), inca,ab( l+1,&
                                           j1 ), inca, d( j1 ),work( j1 ), kd1 )
                             end do
                          else
                             j1end = j1 + kd1*( nr-2 )
                             if( j1end>=j1 ) then
                                do j1inc = j1, j1end, kd1
                                   call stdlib_qrot( kdm1, ab( 3_ilp, j1inc-1 ), 1_ilp,ab( 2_ilp, j1inc ), 1_ilp, &
                                             d( j1inc ),work( j1inc ) )
                                end do
                             end if
                             lend = min( kdm1, n-j2 )
                             last = j1end + kd1
                             if( lend>0_ilp )call stdlib_qrot( lend, ab( 3_ilp, last-1 ), 1_ilp,ab( 2_ilp, last ),&
                                        1_ilp, d( last ),work( last ) )
                          end if
                       end if
                       if( wantq ) then
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do i = n - 1, 1, -1
                 ...
                 ! to annihilate a(1:i-1,i+1)
                 alpha = ap( i1+i-1 )
                 call stdlib_wlarfg( i, alpha, ap( i1 ), 1_ilp, taui )
                 e( i ) = real( alpha,KIND=qp)
                 if( taui/=czero ) then
                    ! apply h(i) from both sides to a(1:i,1:i)
                    ap( i1+i-1 ) = cone
                    ! compute  y := tau * a * v  storing y in tau(1:i)
                    call stdlib_whpmv( uplo, i, taui, ap, ap( i1 ), 1_ilp, czero, tau,1_ilp )
                    ! compute  w := y - 1/2 * tau * (y**h *v) * v
                    alpha = -chalf*taui*stdlib_wdotc( i, tau, 1_ilp, ap( i1 ), 1_ilp )
                    call stdlib_waxpy( i, alpha, ap( i1 ), 1_ilp, tau, 1_ilp )
                    ! apply the transformation as a rank-2 update:
                       ! a := a - v * w**h - w * v**h
                    call stdlib_whpr2( uplo, i, -cone, ap( i1 ), 1_ilp, tau, 1_ilp, ap )
                 end if
                 ap( i1+i-1 ) = e( i )
              ...
              d( 1_ilp ) = real( ap( 1_ilp ),KIND=qp)
           else
              ! reduce the lower triangle of a. ii is the index in ap of
              ! a(i,i) and i1i1 is the index of a(i+1,i+1).
              ii = 1_ilp
              ap( 1_ilp ) = real( ap( 1_ilp ),KIND=qp)
              do i = 1, n - 1
                 i1i1 = ii + n - i + 1_ilp
                 ! generate elementary reflector h(i) = i - tau * v * v**h
                 ! to annihilate a(i+2:n,i)
                 alpha = ap( ii+1 )
                 call stdlib_wlarfg( n-i, alpha, ap( ii+2 ), 1_ilp, taui )
                 e( i ) = real( alpha,KIND=qp)
                 if( taui/=czero ) then
                    ! apply h(i) from both sides to a(i+1:n,i+1:n)
                    ap( ii+1 ) = cone
                    ! compute  y := tau * a * v  storing y in tau(i:n-1)
                    call stdlib_whpmv( uplo, n-i, taui, ap( i1i1 ), ap( ii+1 ), 1_ilp,czero, tau( i ),&
                               1_ilp )
                    ! compute  w := y - 1/2 * tau * (y**h *v) * v
                    alpha = -chalf*taui*stdlib_wdotc( n-i, tau( i ), 1_ilp, ap( ii+1 ),1_ilp )
                    call stdlib_waxpy( n-i, alpha, ap( ii+1 ), 1_ilp, tau( i ), 1_ilp )
                    ! apply the transformation as a rank-2 update:
                       ! a := a - v * w**h - w * v**h
                    call stdlib_whpr2( uplo, n-i, -cone, ap( ii+1 ), 1_ilp, tau( i ), 1_ilp,ap( i1i1 ) )
     ...
     !! if UPLO = 'L', Q = H(1) H(2) . . . H(n-1).
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           if( upper ) then
              ! q was determined by a call to stdlib_whptrd with uplo = 'u'
              ! unpack the vectors which define the elementary reflectors and
              ! set the last row and column of q equal to those of the unit
              ! matrix
              ij = 2_ilp
              ...
              q( n, n ) = cone
              ! generate q(1:n-1,1:n-1)
              call stdlib_wung2l( n-1, n-1, n-1, q, ldq, tau, work, iinfo )
           else
              ! q was determined by a call to stdlib_whptrd with uplo = 'l'.
              ! unpack the vectors which define the elementary reflectors and
              ! set the first row and column of q equal to those of the unit
              ! matrix
              q( 1_ilp, 1_ilp ) = cone
              ...
              if( n>1_ilp ) then
                 ! generate q(2:n,2:n)
                 call stdlib_wung2r( n-1, n-1, n-1, q( 2_ilp, 2_ilp ), ldq, tau, work,iinfo )
              end if
           end if
           return
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do i = i1, i2, i3
                 ...
                 aii = ap( ii )
                 ap( ii ) = cone
                 call stdlib_wlarf( side, mi, ni, ap( ii-i+1 ), 1_ilp, taui, c, ldc,work )
                 ap( ii ) = aii
                 if( forwrd ) then
                    ii = ii + i + 2_ilp
                 else
              ...
              loop_20: do i = i1, i2, i3
                 ...
                 call stdlib_wlarf( side, mi, ni, ap( ii ), 1_ilp, taui, c( ic, jc ),ldc, work )
     ...
     !! Q**H * A * Q = T.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           ! initialize q to the unit matrix, if needed
           if( initq )call stdlib_wlaset( 'FULL', n, n, czero, cone, q, ldq )
           ! wherever possible, plane rotations are generated and applied in
           ! vector operations of length nr over the index set j1:j2:kd1.
           ! the real cosines and complex sines of the plane rotations are
           ! stored in the arrays d and work.
           ...
           if( upper ) then
              if( kd>1_ilp ) then
                 ...
                 j1 = kdn + 2_ilp
                 j2 = 1_ilp
                 ab( kd1, 1_ilp ) = real( ab( kd1, 1_ilp ),KIND=qp)
                 loop_90: do i = 1, n - 2
                    ! reduce i-th row of matrix to tridiagonal form
                    loop_80: do k = kdn + 1, 2, -1
                       j1 = j1 + kdn
                       ...
                       if( nr>0_ilp ) then
                          ! generate plane rotations to annihilate nonzero
                          ! elements which have been created outside the band
                          call stdlib_wlargv( nr, ab( 1_ilp, j1-1 ), inca, work( j1 ),kd1, d( j1 ), &
                                    kd1 )
                          ! apply rotations from the right
                          ! dependent on the the number of diagonals either
                          ! stdlib_wlartv or stdlib_wrot is used
                          if( nr>=2_ilp*kd-1 ) then
                             do l = 1, kd - 1
                                call stdlib_wlartv( nr, ab( l+1, j1-1 ), inca,ab( l, j1 ), inca, &
                                          d( j1 ),work( j1 ), kd1 )
                             end do
                          else
                             jend = j1 + ( nr-1 )*kd1
                             do jinc = j1, jend, kd1
                                call stdlib_wrot( kdm1, ab( 2_ilp, jinc-1 ), 1_ilp,ab( 1_ilp, jinc ), 1_ilp, d( &
                                          jinc ),work( jinc ) )
                             end do
                          end if
                       end if
                       if( k>2_ilp ) then
                          if( k<=n-i+1 ) then
                             ! generate plane rotation to annihilate a(i,i+k-1)
                             ! within the band
                             call stdlib_wlartg( ab( kd-k+3, i+k-2 ),ab( kd-k+2, i+k-1 ), d( i+k-&
                                       1_ilp ),work( i+k-1 ), temp )
                             ab( kd-k+3, i+k-2 ) = temp
                             ! apply rotation from the right
                             call stdlib_wrot( k-3, ab( kd-k+4, i+k-2 ), 1_ilp,ab( kd-k+3, i+k-1 ), 1_ilp,&
                                        d( i+k-1 ),work( i+k-1 ) )
                          end if
                          nr = nr + 1_ilp
                          j1 = j1 - kdn - 1_ilp
                       ...
                       ! apply plane rotations from both sides to diagonal
                       ! blocks
                       if( nr>0_ilp )call stdlib_wlar2v( nr, ab( kd1, j1-1 ), ab( kd1, j1 ),ab( kd, &
                                 j1 ), inca, d( j1 ),work( j1 ), kd1 )
                       ! apply plane rotations from the left
                       if( nr>0_ilp ) then
                          call stdlib_wlacgv( nr, work( j1 ), kd1 )
                          if( 2_ilp*kd-1<nr ) then
                          ! dependent on the the number of diagonals either
                          ! stdlib_wlartv or stdlib_wrot is used
                             do l = 1, kd - 1
                                ...
                                if( nrt>0_ilp )call stdlib_wlartv( nrt, ab( kd-l, j1+l ), inca,ab( kd-&
                                          l+1, j1+l ), inca,d( j1 ), work( j1 ), kd1 )
                             end do
                          else
                             j1end = j1 + kd1*( nr-2 )
                             if( j1end>=j1 ) then
                                do jin = j1, j1end, kd1
                                   call stdlib_wrot( kd-1, ab( kd-1, jin+1 ), incx,ab( kd, jin+1 )&
                                             , incx,d( jin ), work( jin ) )
                                end do
                             end if
                             lend = min( kdm1, n-j2 )
                             last = j1end + kd1
                             if( lend>0_ilp )call stdlib_wrot( lend, ab( kd-1, last+1 ), incx,ab( kd, &
                                       last+1 ), incx, d( last ),work( last ) )
                          end if
                       end if
                       if( wantq ) then
                       ...
                       ! initially the identity matrix
                             ...
                             do j = j1, j2, kd1
                                ...
                                nq = 1_ilp + iqaend - iqb
                                iqaend = min( iqaend+kd, iqend )
                                call stdlib_wrot( nq, q( iqb, j-1 ), 1_ilp, q( iqb, j ),1_ilp, d( j ), &
                                          conjg( work( j ) ) )
                             end do
                          else
                             do j = j1, j2, kd1
                                call stdlib_wrot( n, q( 1_ilp, j-1 ), 1_ilp, q( 1_ilp, j ), 1_ilp,d( j ), conjg( &
                                          work( j ) ) )
                             end do
                          end if
                       end if
              ...
              if( kd>0_ilp ) then
                 ...
                 do i = 1, n - 1
                    ...
                    if( i<n-1 )ab( kd, i+2 ) = ab( kd, i+2 )*t
                    if( wantq ) then
                       call stdlib_wscal( n, conjg( t ), q( 1_ilp, i+1 ), 1_ilp )
                    end if
                 end do
              else
                 ! set e to zero if original matrix was diagonal
              ...
              if( kd>1_ilp ) then
                 ...
                 j1 = kdn + 2_ilp
                 j2 = 1_ilp
                 ab( 1_ilp, 1_ilp ) = real( ab( 1_ilp, 1_ilp ),KIND=qp)
                 loop_210: do i = 1, n - 2
                    ! reduce i-th column of matrix to tridiagonal form
                    loop_200: do k = kdn + 1, 2, -1
                       j1 = j1 + kdn
                       ...
                       if( nr>0_ilp ) then
                          ! generate plane rotations to annihilate nonzero
                          ! elements which have been created outside the band
                          call stdlib_wlargv( nr, ab( kd1, j1-kd1 ), inca,work( j1 ), kd1, d( j1 )&
                                    , kd1 )
                          ! apply plane rotations from one side
                          ! dependent on the the number of diagonals either
                          ! stdlib_wlartv or stdlib_wrot is used
                          if( nr>2_ilp*kd-1 ) then
                             do l = 1, kd - 1
                                call stdlib_wlartv( nr, ab( kd1-l, j1-kd1+l ), inca,ab( kd1-l+1, &
                                          j1-kd1+l ), inca,d( j1 ), work( j1 ), kd1 )
                             end do
                          else
                             jend = j1 + kd1*( nr-1 )
                             do jinc = j1, jend, kd1
                                call stdlib_wrot( kdm1, ab( kd, jinc-kd ), incx,ab( kd1, jinc-kd )&
                                          , incx,d( jinc ), work( jinc ) )
                             end do
                          end if
                       end if
                       if( k>2_ilp ) then
                          if( k<=n-i+1 ) then
                             ! generate plane rotation to annihilate a(i+k-1,i)
                             ! within the band
                             call stdlib_wlartg( ab( k-1, i ), ab( k, i ),d( i+k-1 ), work( i+k-1 &
                                       ), temp )
                             ab( k-1, i ) = temp
                             ! apply rotation from the left
                             call stdlib_wrot( k-3, ab( k-2, i+1 ), ldab-1,ab( k-1, i+1 ), ldab-1,&
                                        d( i+k-1 ),work( i+k-1 ) )
                          end if
                          nr = nr + 1_ilp
                          j1 = j1 - kdn - 1_ilp
                       ...
                       ! apply plane rotations from both sides to diagonal
                       ! blocks
                       if( nr>0_ilp )call stdlib_wlar2v( nr, ab( 1_ilp, j1-1 ), ab( 1_ilp, j1 ),ab( 2_ilp, j1-1 ),&
                                  inca, d( j1 ),work( j1 ), kd1 )
                       ! apply plane rotations from the right
                          ! dependent on the the number of diagonals either
                          ! stdlib_wlartv or stdlib_wrot is used
                       if( nr>0_ilp ) then
                          ...
                          if( nr>2_ilp*kd-1 ) then
                             do l = 1, kd - 1
                                ...
                                if( nrt>0_ilp )call stdlib_wlartv( nrt, ab( l+2, j1-1 ), inca,ab( l+1,&
                                           j1 ), inca, d( j1 ),work( j1 ), kd1 )
                             end do
                          else
                             j1end = j1 + kd1*( nr-2 )
                             if( j1end>=j1 ) then
                                do j1inc = j1, j1end, kd1
                                   call stdlib_wrot( kdm1, ab( 3_ilp, j1inc-1 ), 1_ilp,ab( 2_ilp, j1inc ), 1_ilp, &
                                             d( j1inc ),work( j1inc ) )
                                end do
                             end if
                             lend = min( kdm1, n-j2 )
                             last = j1end + kd1
                             if( lend>0_ilp )call stdlib_wrot( lend, ab( 3_ilp, last-1 ), 1_ilp,ab( 2_ilp, last ),&
                                        1_ilp, d( last ),work( last ) )
                          end if
                       end if
                       if( wantq ) then
                       ...
                       ! initially the identity matrix
                             ...
                             do j = j1, j2, kd1
                                call stdlib_wrot( n, q( 1_ilp, j-1 ), 1_ilp, q( 1_ilp, j ), 1_ilp,d( j ), work( j &
                                          ) )
                             end do
                          end if
                       end if
              ...
              if( kd>0_ilp ) then
                 ...
                 do i = 1, n - 1
                    ...
                    if( i<n-1 )ab( 2_ilp, i+1 ) = ab( 2_ilp, i+1 )*t
                    if( wantq ) then
                       call stdlib_wscal( n, t, q( 1_ilp, i+1 ), 1_ilp )
                    end if
                 end do
              else
                 ! set e to zero if original matrix was diagonal
