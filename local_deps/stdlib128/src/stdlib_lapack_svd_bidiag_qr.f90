  ...
  contains
     ...
     !! the dqds Algorithm (Positive Case)", LAPACK Working Note.
        ...
        ! =====================================================================
           ...
           if( n<0_ilp ) then
              ...
              call stdlib_qlas2( d( 1_ilp ), e( 1_ilp ), d( 2_ilp ), sigmn, sigmx )
              d( 1_ilp ) = sigmx
              d( 2_ilp ) = sigmn
              return
           end if
           ...
           ! early return if sigmx is zero (matrix is already diagonal).
           if( sigmx==zero ) then
              call stdlib_qlasrt( 'D', n, d, iinfo )
              return
           end if
           do i = 1, n
              sigmx = max( sigmx, d( i ) )
           ...
           scale = sqrt( eps / safmin )
           call stdlib_qcopy( n, d, 1_ilp, work( 1_ilp ), 2_ilp )
           call stdlib_qcopy( n-1, e, 1_ilp, work( 2_ilp ), 2_ilp )
           call stdlib_qlascl( 'G', 0_ilp, 0_ilp, sigmx, scale, 2_ilp*n-1, 1_ilp, work, 2_ilp*n-1,iinfo )
           ! compute the q's and e's.
           do i = 1, 2*n - 1
              work( i ) = work( i )**2_ilp
           end do
           work( 2_ilp*n ) = zero
           call stdlib_qlasq2( n, work, info )
           if( info==0_ilp ) then
              do i = 1, n
                 d( i ) = sqrt( work( i ) )
              end do
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, scale, sigmx, n, 1_ilp, d, n, iinfo )
           else if( info==2_ilp ) then
           ! maximum number of iterations exceeded.  move data from work
           ! into d and e so the calling subroutine can try to finish
              do i = 1, n
              ...
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, scale, sigmx, n, 1_ilp, d, n, iinfo )
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, scale, sigmx, n, 1_ilp, e, n, iinfo )
           end if
           return
     ...
     !! is passed to DLASQ3.
        ...
        ! =====================================================================
           ...
           ! Executable Statements 
           ! test the input arguments.
           ! (in case stdlib_qlasq2 is not called by stdlib_qlasq1)
           info = 0_ilp
           eps = stdlib_qlamch( 'PRECISION' )
           safmin = stdlib_qlamch( 'SAFE MINIMUM' )
           tol = eps*hundrd
           ...
           if( e==zero ) then
              ...
              call stdlib_qlasrt( 'D', n, z, iinfo )
              z( 2_ilp*n-1 ) = d
              return
           end if
           trace = d + e
           ...
           ! initialise variables to pass to stdlib_qlasq3.
           ttype = 0_ilp
           dmin1 = zero
           dmin2 = zero
           dn    = zero
           ...
           loop_160: do iwhila = 1, n + 1
              ...
              ! pp = 0 for ping, pp = 1 for pong.
              ! pp = 2 indicates that flipping was applied to the z array and
                     ! and that the tests for deflation upon entry in stdlib_qlasq3
                     ! should not be performed.
              nbig = 100_ilp*( n0-i0+1 )
              loop_140: do iwhilb = 1, nbig
                 if( i0>n0 )go to 150
                 ! while submatrix unfinished take a good dqds step.
                 call stdlib_qlasq3( i0, n0, z, pp, dmin, sigma, desig, qmax, nfail,iter, ndiv, &
                           ieee, ttype, dmin1, dmin2, dn, dn1,dn2, g, tau )
                 pp = 1_ilp - pp
                 ! when emin is very small check for splits.
                 if( pp==0_ilp .and. n0-i0>=3_ilp ) then
           ...
           z( 2_ilp*n+1 ) = trace
           z( 2_ilp*n+2 ) = e
           z( 2_ilp*n+3 ) = real( iter,KIND=qp)
           z( 2_ilp*n+4 ) = real( ndiv,KIND=qp) / real( n**2_ilp,KIND=qp)
           z( 2_ilp*n+5 ) = hundrd*nfail / real( iter,KIND=qp)
           return
     ...
     !! is positive.
        ...
        ! =====================================================================
           ...
           if( dmin>=zero .and. dmin1>=zero ) then
              ...
              go to 70
           else if( stdlib_qisnan( dmin ) ) then
              ! nan.
              if( tau==zero ) then
                 go to 80
              else
