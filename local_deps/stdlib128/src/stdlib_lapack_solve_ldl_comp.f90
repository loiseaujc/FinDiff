  ...
  contains
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              ! multiply by inv(l*d*l**t) or inv(u*d*u**t).
              call stdlib_qsytrs( uplo, n, 1_ilp, a, lda, ipiv, work, n, info )
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm/=zero )rcond = ( one / ainvnm ) / anorm
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              ! multiply by inv(l*d*l**t) or inv(u*d*u**t).
              call stdlib_wsytrs( uplo, n, 1_ilp, a, lda, ipiv, work, n, info )
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm/=zero )rcond = ( one / ainvnm ) / anorm
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( k>nb ) then
                 ! factorize columns k-kb+1:k of a and use blocked code to
                 ! update columns 1:k-kb
                 call stdlib_qlasyf( uplo, k, nb, kb, a, lda, ipiv, work, ldwork,iinfo )
              else
                 ! use unblocked code to factorize columns 1:k of a
                 call stdlib_qsytf2( uplo, k, a, lda, ipiv, iinfo )
                 kb = k
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp .and. iinfo>0_ilp )info = iinfo
              ...
              if( k<=n-nb ) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call stdlib_qlasyf( uplo, n-k+1, nb, kb, a( k, k ), lda, ipiv( k ),work, ldwork, &
                           iinfo )
              else
                 ! use unblocked code to factorize columns k:n of a
                 call stdlib_qsytf2( uplo, n-k+1, a( k, k ), lda, ipiv( k ), iinfo )
                 kb = n - k + 1_ilp
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp .and. iinfo>0_ilp )info = iinfo + k - 1_ilp
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( k>nb ) then
                 ! factorize columns k-kb+1:k of a and use blocked code to
                 ! update columns 1:k-kb
                 call stdlib_wlasyf( uplo, k, nb, kb, a, lda, ipiv, work, n, iinfo )
              else
                 ! use unblocked code to factorize columns 1:k of a
                 call stdlib_wsytf2( uplo, k, a, lda, ipiv, iinfo )
                 kb = k
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp .and. iinfo>0_ilp )info = iinfo
              ...
              if( k<=n-nb ) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call stdlib_wlasyf( uplo, n-k+1, nb, kb, a( k, k ), lda, ipiv( k ),work, n, &
                           iinfo )
              else
                 ! use unblocked code to factorize columns k:n of a
                 call stdlib_wsytf2( uplo, n-k+1, a( k, k ), lda, ipiv( k ), iinfo )
                 kb = n - k + 1_ilp
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp .and. iinfo>0_ilp )info = iinfo + k - 1_ilp
     ...
     !! A22 (if UPLO = 'L').
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( uplo, 'U' ) ) then
              ...
              if( ( k<=n-nb+1 .and. nb<n ) .or. k<1 )go to 30
              ! copy column k of a to column kw of w and update it
              call stdlib_qcopy( k, a( 1_ilp, k ), 1_ilp, w( 1_ilp, kw ), 1_ilp )
              if( k<n )call stdlib_qgemv( 'NO TRANSPOSE', k, n-k, -one, a( 1_ilp, k+1 ), lda,w( k, kw+&
                        1_ilp ), ldw, one, w( 1_ilp, kw ), 1_ilp )
              kstep = 1_ilp
              ! determine rows and columns to be interchanged and whether
              ! a 1-by-1 or 2-by-2 pivot block will be used
              ...
              ! determine both colmax and imax.
              if( k>1_ilp ) then
                 imax = stdlib_iqamax( k-1, w( 1_ilp, kw ), 1_ilp )
                 colmax = abs( w( imax, kw ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! copy column imax to column kw-1 of w and update it
                    call stdlib_qcopy( imax, a( 1_ilp, imax ), 1_ilp, w( 1_ilp, kw-1 ), 1_ilp )
                    call stdlib_qcopy( k-imax, a( imax, imax+1 ), lda,w( imax+1, kw-1 ), 1_ilp )
                              
                    if( k<n )call stdlib_qgemv( 'NO TRANSPOSE', k, n-k, -one, a( 1_ilp, k+1 ),lda, w( &
                              imax, kw+1 ), ldw, one,w( 1_ilp, kw-1 ), 1_ilp )
                    ! jmax is the column-index of the largest off-diagonal
                    ! element in row imax, and rowmax is its absolute value
                    jmax = imax + stdlib_iqamax( k-imax, w( imax+1, kw-1 ), 1_ilp )
                    rowmax = abs( w( jmax, kw-1 ) )
                    if( imax>1_ilp ) then
                       jmax = stdlib_iqamax( imax-1, w( 1_ilp, kw-1 ), 1_ilp )
                       rowmax = max( rowmax, abs( w( jmax, kw-1 ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                       ...
                       kp = imax
                       ! copy column kw-1 of w to column kw of w
                       call stdlib_qcopy( k, w( 1_ilp, kw-1 ), 1_ilp, w( 1_ilp, kw ), 1_ilp )
                    else
                       ! interchange rows and columns k-1 and imax, use 2-by-2
                       ! pivot block
                       kp = imax
                 ...
                 if( kp/=kk ) then
                    ...
                    ! will be later overwritten.
                    a( kp, kp ) = a( kk, kk )
                    call stdlib_qcopy( kk-1-kp, a( kp+1, kk ), 1_ilp, a( kp, kp+1 ),lda )
                    if( kp>1_ilp )call stdlib_qcopy( kp-1, a( 1_ilp, kk ), 1_ilp, a( 1_ilp, kp ), 1_ilp )
                    ! interchange rows kk and kp in last k+1 to n columns of a
                    ! (columns k (or k and k-1 for 2-by-2 pivot) of a will be
                    ! later overwritten). interchange rows kk and kp
                    ! in last kkw to nb columns of w.
                    if( k<n )call stdlib_qswap( n-k, a( kk, k+1 ), lda, a( kp, k+1 ),lda )
                    call stdlib_qswap( n-kk+1, w( kk, kkw ), ldw, w( kp, kkw ),ldw )
                 end if
                 if( kstep==1_ilp ) then
                    ! 1-by-1 pivot block d(k): column kw of w now holds
                    ! w(kw) = u(k)*d(k),
                    ...
                    ! and not stored.
                       ! a(k,k) := d(k,k) = w(k,kw)
                       ! a(1:k-1,k) := u(1:k-1,k) = w(1:k-1,kw)/d(k,k)
                    call stdlib_qcopy( k, w( 1_ilp, kw ), 1_ilp, a( 1_ilp, k ), 1_ilp )
                    r1 = one / a( k, k )
                    call stdlib_qscal( k-1, r1, a( 1_ilp, k ), 1_ilp )
                 else
                    ! 2-by-2 pivot block d(k): columns kw and kw-1 of w now hold
                    ! ( w(kw-1) w(kw) ) = ( u(k-1) u(k) )*d(k)
                    ! where u(k) and u(k-1) are the k-th and (k-1)-th columns
              ...
              do j = ( ( k-1 ) / nb )*nb + 1, 1, -nb
                 ...
                 ! update the upper triangle of the diagonal block
                 do jj = j, j + jb - 1
                    call stdlib_qgemv( 'NO TRANSPOSE', jj-j+1, n-k, -one,a( j, k+1 ), lda, w( jj, &
                              kw+1 ), ldw, one,a( j, jj ), 1_ilp )
                 end do
                 ! update the rectangular superdiagonal block
                 call stdlib_qgemm( 'NO TRANSPOSE', 'TRANSPOSE', j-1, jb, n-k, -one,a( 1_ilp, k+1 ), &
                           lda, w( j, kw+1 ), ldw, one,a( 1_ilp, j ), lda )
              end do
              ! put u12 in standard form by partially undoing the interchanges
              ! in columns k+1:n looping backwards from k+1 to n
              ...
              60 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j + 1_ilp
                 if( jp/=jj .and. j<=n )call stdlib_qswap( n-j+1, a( jp, j ), lda, a( jj, j ), &
                           lda )
              if( j<n )go to 60
              ! set kb to the number of columns factorized
              kb = n - k
              ...
              if( ( k>=nb .and. nb<n ) .or. k>n )go to 90
              ! copy column k of a to column k of w and update it
              call stdlib_qcopy( n-k+1, a( k, k ), 1_ilp, w( k, k ), 1_ilp )
              call stdlib_qgemv( 'NO TRANSPOSE', n-k+1, k-1, -one, a( k, 1_ilp ), lda,w( k, 1_ilp ), ldw, &
                        one, w( k, k ), 1_ilp )
              kstep = 1_ilp
              ! determine rows and columns to be interchanged and whether
              ! a 1-by-1 or 2-by-2 pivot block will be used
              ...
              ! determine both colmax and imax.
              if( k<n ) then
                 imax = k + stdlib_iqamax( n-k, w( k+1, k ), 1_ilp )
                 colmax = abs( w( imax, k ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! copy column imax to column k+1 of w and update it
                    call stdlib_qcopy( imax-k, a( imax, k ), lda, w( k, k+1 ), 1_ilp )
                    call stdlib_qcopy( n-imax+1, a( imax, imax ), 1_ilp, w( imax, k+1 ),1_ilp )
                    call stdlib_qgemv( 'NO TRANSPOSE', n-k+1, k-1, -one, a( k, 1_ilp ),lda, w( imax, &
                              1_ilp ), ldw, one, w( k, k+1 ), 1_ilp )
                    ! jmax is the column-index of the largest off-diagonal
                    ! element in row imax, and rowmax is its absolute value
                    jmax = k - 1_ilp + stdlib_iqamax( imax-k, w( k, k+1 ), 1_ilp )
                    rowmax = abs( w( jmax, k+1 ) )
                    if( imax<n ) then
                       jmax = imax + stdlib_iqamax( n-imax, w( imax+1, k+1 ), 1_ilp )
                       rowmax = max( rowmax, abs( w( jmax, k+1 ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                       ...
                       kp = imax
                       ! copy column k+1 of w to column k of w
                       call stdlib_qcopy( n-k+1, w( k, k+1 ), 1_ilp, w( k, k ), 1_ilp )
                    else
                       ! interchange rows and columns k+1 and imax, use 2-by-2
                       ! pivot block
                       kp = imax
                 ...
                 if( kp/=kk ) then
                    ...
                    ! will be later overwritten.
                    a( kp, kp ) = a( kk, kk )
                    call stdlib_qcopy( kp-kk-1, a( kk+1, kk ), 1_ilp, a( kp, kk+1 ),lda )
                    if( kp<n )call stdlib_qcopy( n-kp, a( kp+1, kk ), 1_ilp, a( kp+1, kp ), 1_ilp )
                              
                    ! interchange rows kk and kp in first k-1 columns of a
                    ! (columns k (or k and k+1 for 2-by-2 pivot) of a will be
                    ! later overwritten). interchange rows kk and kp
                    ! in first kk columns of w.
                    if( k>1_ilp )call stdlib_qswap( k-1, a( kk, 1_ilp ), lda, a( kp, 1_ilp ), lda )
                    call stdlib_qswap( kk, w( kk, 1_ilp ), ldw, w( kp, 1_ilp ), ldw )
                 end if
                 if( kstep==1_ilp ) then
                    ! 1-by-1 pivot block d(k): column k of w now holds
                    ! w(k) = l(k)*d(k),
                    ...
                    ! and not stored)
                       ! a(k,k) := d(k,k) = w(k,k)
                       ! a(k+1:n,k) := l(k+1:n,k) = w(k+1:n,k)/d(k,k)
                    call stdlib_qcopy( n-k+1, w( k, k ), 1_ilp, a( k, k ), 1_ilp )
                    if( k<n ) then
                       r1 = one / a( k, k )
                       call stdlib_qscal( n-k, r1, a( k+1, k ), 1_ilp )
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns k and k+1 of w now hold
                    ! ( w(k) w(k+1) ) = ( l(k) l(k+1) )*d(k)
              ...
              do j = k, n, nb
                 ...
                 ! update the lower triangle of the diagonal block
                 do jj = j, j + jb - 1
                    call stdlib_qgemv( 'NO TRANSPOSE', j+jb-jj, k-1, -one,a( jj, 1_ilp ), lda, w( jj, &
                              1_ilp ), ldw, one,a( jj, jj ), 1_ilp )
                 end do
                 ! update the rectangular subdiagonal block
                 if( j+jb<=n )call stdlib_qgemm( 'NO TRANSPOSE', 'TRANSPOSE', n-j-jb+1, jb,k-1, -&
                           one, a( j+jb, 1_ilp ), lda, w( j, 1_ilp ), ldw,one, a( j+jb, j ), lda )
              end do
              ! put l21 in standard form by partially undoing the interchanges
              ! of rows in columns 1:k-1 looping backwards from k-1 to 1
              ...
              120 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j - 1_ilp
                 if( jp/=jj .and. j>=1_ilp )call stdlib_qswap( j, a( jp, 1_ilp ), lda, a( jj, 1_ilp ), lda )
     ...
     !! A22 (if UPLO = 'L').
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( uplo, 'U' ) ) then
              ...
              if( ( k<=n-nb+1 .and. nb<n ) .or. k<1 )go to 30
              ! copy column k of a to column kw of w and update it
              call stdlib_wcopy( k, a( 1_ilp, k ), 1_ilp, w( 1_ilp, kw ), 1_ilp )
              if( k<n )call stdlib_wgemv( 'NO TRANSPOSE', k, n-k, -cone, a( 1_ilp, k+1 ), lda,w( k, &
                        kw+1 ), ldw, cone, w( 1_ilp, kw ), 1_ilp )
              kstep = 1_ilp
              ! determine rows and columns to be interchanged and whether
              ! a 1-by-1 or 2-by-2 pivot block will be used
              ...
              ! imax is the row-index of the largest off-diagonal element in
              if( k>1_ilp ) then
                 imax = stdlib_iwamax( k-1, w( 1_ilp, kw ), 1_ilp )
                 colmax = cabs1( w( imax, kw ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! copy column imax to column kw-1 of w and update it
                    call stdlib_wcopy( imax, a( 1_ilp, imax ), 1_ilp, w( 1_ilp, kw-1 ), 1_ilp )
                    call stdlib_wcopy( k-imax, a( imax, imax+1 ), lda,w( imax+1, kw-1 ), 1_ilp )
                              
                    if( k<n )call stdlib_wgemv( 'NO TRANSPOSE', k, n-k, -cone,a( 1_ilp, k+1 ), lda, w(&
                               imax, kw+1 ), ldw,cone, w( 1_ilp, kw-1 ), 1_ilp )
                    ! jmax is the column-index of the largest off-diagonal
                    ! element in row imax, and rowmax is its absolute value
                    jmax = imax + stdlib_iwamax( k-imax, w( imax+1, kw-1 ), 1_ilp )
                    rowmax = cabs1( w( jmax, kw-1 ) )
                    if( imax>1_ilp ) then
                       jmax = stdlib_iwamax( imax-1, w( 1_ilp, kw-1 ), 1_ilp )
                       rowmax = max( rowmax, cabs1( w( jmax, kw-1 ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                       ...
                       kp = imax
                       ! copy column kw-1 of w to column kw of w
                       call stdlib_wcopy( k, w( 1_ilp, kw-1 ), 1_ilp, w( 1_ilp, kw ), 1_ilp )
                    else
                       ! interchange rows and columns k-1 and imax, use 2-by-2
                       ! pivot block
                       kp = imax
                 ...
                 if( kp/=kk ) then
                    ...
                    ! will be later overwritten.
                    a( kp, kp ) = a( kk, kk )
                    call stdlib_wcopy( kk-1-kp, a( kp+1, kk ), 1_ilp, a( kp, kp+1 ),lda )
                    if( kp>1_ilp )call stdlib_wcopy( kp-1, a( 1_ilp, kk ), 1_ilp, a( 1_ilp, kp ), 1_ilp )
                    ! interchange rows kk and kp in last k+1 to n columns of a
                    ! (columns k (or k and k-1 for 2-by-2 pivot) of a will be
                    ! later overwritten). interchange rows kk and kp
                    ! in last kkw to nb columns of w.
                    if( k<n )call stdlib_wswap( n-k, a( kk, k+1 ), lda, a( kp, k+1 ),lda )
                    call stdlib_wswap( n-kk+1, w( kk, kkw ), ldw, w( kp, kkw ),ldw )
                 end if
                 if( kstep==1_ilp ) then
                    ! 1-by-1 pivot block d(k): column kw of w now holds
                    ! w(kw) = u(k)*d(k),
                    ...
                    ! and not stored.
                       ! a(k,k) := d(k,k) = w(k,kw)
                       ! a(1:k-1,k) := u(1:k-1,k) = w(1:k-1,kw)/d(k,k)
                    call stdlib_wcopy( k, w( 1_ilp, kw ), 1_ilp, a( 1_ilp, k ), 1_ilp )
                    r1 = cone / a( k, k )
                    call stdlib_wscal( k-1, r1, a( 1_ilp, k ), 1_ilp )
                 else
                    ! 2-by-2 pivot block d(k): columns kw and kw-1 of w now hold
                    ! ( w(kw-1) w(kw) ) = ( u(k-1) u(k) )*d(k)
                    ! where u(k) and u(k-1) are the k-th and (k-1)-th columns
              ...
              do j = ( ( k-1 ) / nb )*nb + 1, 1, -nb
                 ...
                 ! update the upper triangle of the diagonal block
                 do jj = j, j + jb - 1
                    call stdlib_wgemv( 'NO TRANSPOSE', jj-j+1, n-k, -cone,a( j, k+1 ), lda, w( jj,&
                               kw+1 ), ldw, cone,a( j, jj ), 1_ilp )
                 end do
                 ! update the rectangular superdiagonal block
                 call stdlib_wgemm( 'NO TRANSPOSE', 'TRANSPOSE', j-1, jb, n-k,-cone, a( 1_ilp, k+1 ), &
                           lda, w( j, kw+1 ), ldw,cone, a( 1_ilp, j ), lda )
              end do
              ! put u12 in standard form by partially undoing the interchanges
              ! in columns k+1:n looping backwards from k+1 to n
              ...
              60 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j + 1_ilp
                 if( jp/=jj .and. j<=n )call stdlib_wswap( n-j+1, a( jp, j ), lda, a( jj, j ), &
                           lda )
              if( j<n )go to 60
              ! set kb to the number of columns factorized
              kb = n - k
              ...
              if( ( k>=nb .and. nb<n ) .or. k>n )go to 90
              ! copy column k of a to column k of w and update it
              call stdlib_wcopy( n-k+1, a( k, k ), 1_ilp, w( k, k ), 1_ilp )
              call stdlib_wgemv( 'NO TRANSPOSE', n-k+1, k-1, -cone, a( k, 1_ilp ), lda,w( k, 1_ilp ), ldw,&
                         cone, w( k, k ), 1_ilp )
              kstep = 1_ilp
              ! determine rows and columns to be interchanged and whether
              ! a 1-by-1 or 2-by-2 pivot block will be used
              ...
              ! imax is the row-index of the largest off-diagonal element in
              if( k<n ) then
                 imax = k + stdlib_iwamax( n-k, w( k+1, k ), 1_ilp )
                 colmax = cabs1( w( imax, k ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! copy column imax to column k+1 of w and update it
                    call stdlib_wcopy( imax-k, a( imax, k ), lda, w( k, k+1 ), 1_ilp )
                    call stdlib_wcopy( n-imax+1, a( imax, imax ), 1_ilp, w( imax, k+1 ),1_ilp )
                    call stdlib_wgemv( 'NO TRANSPOSE', n-k+1, k-1, -cone, a( k, 1_ilp ),lda, w( imax, &
                              1_ilp ), ldw, cone, w( k, k+1 ),1_ilp )
                    ! jmax is the column-index of the largest off-diagonal
                    ! element in row imax, and rowmax is its absolute value
                    jmax = k - 1_ilp + stdlib_iwamax( imax-k, w( k, k+1 ), 1_ilp )
                    rowmax = cabs1( w( jmax, k+1 ) )
                    if( imax<n ) then
                       jmax = imax + stdlib_iwamax( n-imax, w( imax+1, k+1 ), 1_ilp )
                       rowmax = max( rowmax, cabs1( w( jmax, k+1 ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                       ...
                       kp = imax
                       ! copy column k+1 of w to column k of w
                       call stdlib_wcopy( n-k+1, w( k, k+1 ), 1_ilp, w( k, k ), 1_ilp )
                    else
                       ! interchange rows and columns k+1 and imax, use 2-by-2
                       ! pivot block
                       kp = imax
                 ...
                 if( kp/=kk ) then
                    ...
                    ! will be later overwritten.
                    a( kp, kp ) = a( kk, kk )
                    call stdlib_wcopy( kp-kk-1, a( kk+1, kk ), 1_ilp, a( kp, kk+1 ),lda )
                    if( kp<n )call stdlib_wcopy( n-kp, a( kp+1, kk ), 1_ilp, a( kp+1, kp ), 1_ilp )
                              
                    ! interchange rows kk and kp in first k-1 columns of a
                    ! (columns k (or k and k+1 for 2-by-2 pivot) of a will be
                    ! later overwritten). interchange rows kk and kp
                    ! in first kk columns of w.
                    if( k>1_ilp )call stdlib_wswap( k-1, a( kk, 1_ilp ), lda, a( kp, 1_ilp ), lda )
                    call stdlib_wswap( kk, w( kk, 1_ilp ), ldw, w( kp, 1_ilp ), ldw )
                 end if
                 if( kstep==1_ilp ) then
                    ! 1-by-1 pivot block d(k): column k of w now holds
                    ! w(k) = l(k)*d(k),
                    ...
                    ! and not stored)
                       ! a(k,k) := d(k,k) = w(k,k)
                       ! a(k+1:n,k) := l(k+1:n,k) = w(k+1:n,k)/d(k,k)
                    call stdlib_wcopy( n-k+1, w( k, k ), 1_ilp, a( k, k ), 1_ilp )
                    if( k<n ) then
                       r1 = cone / a( k, k )
                       call stdlib_wscal( n-k, r1, a( k+1, k ), 1_ilp )
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns k and k+1 of w now hold
                    ! ( w(k) w(k+1) ) = ( l(k) l(k+1) )*d(k)
              ...
              do j = k, n, nb
                 ...
                 ! update the lower triangle of the diagonal block
                 do jj = j, j + jb - 1
                    call stdlib_wgemv( 'NO TRANSPOSE', j+jb-jj, k-1, -cone,a( jj, 1_ilp ), lda, w( jj,&
                               1_ilp ), ldw, cone,a( jj, jj ), 1_ilp )
                 end do
                 ! update the rectangular subdiagonal block
                 if( j+jb<=n )call stdlib_wgemm( 'NO TRANSPOSE', 'TRANSPOSE', n-j-jb+1, jb,k-1, -&
                           cone, a( j+jb, 1_ilp ), lda, w( j, 1_ilp ),ldw, cone, a( j+jb, j ), lda )
              end do
              ! put l21 in standard form by partially undoing the interchanges
              ! of rows in columns 1:k-1 looping backwards from k-1 to 1
              ...
              120 continue
                 ...
                 ! of the rows to swap back doesn't include diagonal element)
                 j = j - 1_ilp
                 if( jp/=jj .and. j>=1_ilp )call stdlib_wswap( j, a( jp, 1_ilp ), lda, a( jj, 1_ilp ), lda )
     ...
     !! This is the unblocked version of the algorithm, calling Level 2 BLAS.
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              ! determine both colmax and imax.
              if( k>1_ilp ) then
                 imax = stdlib_iqamax( k-1, a( 1_ilp, k ), 1_ilp )
                 colmax = abs( a( imax, k ) )
              else
                 colmax = zero
              end if
              if( (max( absakk, colmax )==zero) .or. stdlib_qisnan(absakk) ) then
                 ! column k is zero or underflow, or contains a nan:
                 ! set info and continue
                 if( info==0_ilp )info = k
                 kp = k
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! jmax is the column-index of the largest off-diagonal
                    ! element in row imax, and rowmax is its absolute value
                    jmax = imax + stdlib_iqamax( k-imax, a( imax, imax+1 ), lda )
                    rowmax = abs( a( imax, jmax ) )
                    if( imax>1_ilp ) then
                       jmax = stdlib_iqamax( imax-1, a( 1_ilp, imax ), 1_ilp )
                       rowmax = max( rowmax, abs( a( jmax, imax ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                 ...
                 if( kp/=kk ) then
                    ! interchange rows and columns kk and kp in the leading
                    ! submatrix a(1:k,1:k)
                    call stdlib_qswap( kp-1, a( 1_ilp, kk ), 1_ilp, a( 1_ilp, kp ), 1_ilp )
                    call stdlib_qswap( kk-kp-1, a( kp+1, kk ), 1_ilp, a( kp, kp+1 ),lda )
                    t = a( kk, kk )
                    a( kk, kk ) = a( kp, kp )
                    a( kp, kp ) = t
                    if( kstep==2_ilp ) then
                 ...
                 if( kstep==1_ilp ) then
                    ...
                    ! a := a - u(k)*d(k)*u(k)**t = a - w(k)*1/d(k)*w(k)**t
                    r1 = one / a( k, k )
                    call stdlib_qsyr( uplo, k-1, -r1, a( 1_ilp, k ), 1_ilp, a, lda )
                    ! store u(k) in column k
                    call stdlib_qscal( k-1, r1, a( 1_ilp, k ), 1_ilp )
                 else
                    ! 2-by-2 pivot block d(k): columns k and k-1 now hold
              ...
              ! determine both colmax and imax.
              if( k<n ) then
                 imax = k + stdlib_iqamax( n-k, a( k+1, k ), 1_ilp )
                 colmax = abs( a( imax, k ) )
              else
                 colmax = zero
              end if
              if( (max( absakk, colmax )==zero) .or. stdlib_qisnan(absakk) ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! jmax is the column-index of the largest off-diagonal
                    ! element in row imax, and rowmax is its absolute value
                    jmax = k - 1_ilp + stdlib_iqamax( imax-k, a( imax, k ), lda )
                    rowmax = abs( a( imax, jmax ) )
                    if( imax<n ) then
                       jmax = imax + stdlib_iqamax( n-imax, a( imax+1, imax ), 1_ilp )
                       rowmax = max( rowmax, abs( a( jmax, imax ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                 ...
                 if( kp/=kk ) then
                    ! interchange rows and columns kk and kp in the trailing
                    ! submatrix a(k:n,k:n)
                    if( kp<n )call stdlib_qswap( n-kp, a( kp+1, kk ), 1_ilp, a( kp+1, kp ), 1_ilp )
                              
                    call stdlib_qswap( kp-kk-1, a( kk+1, kk ), 1_ilp, a( kp, kk+1 ),lda )
                    t = a( kk, kk )
                    a( kk, kk ) = a( kp, kp )
                    a( kp, kp ) = t
                    if( kstep==2_ilp ) then
                 ...
                 if( kstep==1_ilp ) then
                    ...
                    if( k<n ) then
                       ...
                       ! a := a - l(k)*d(k)*l(k)**t = a - w(k)*(1/d(k))*w(k)**t
                       d11 = one / a( k, k )
                       call stdlib_qsyr( uplo, n-k, -d11, a( k+1, k ), 1_ilp,a( k+1, k+1 ), lda )
                                 
                       ! store l(k) in column k
                       call stdlib_qscal( n-k, d11, a( k+1, k ), 1_ilp )
                    end if
                 else
                    ! 2-by-2 pivot block d(k)
                    if( k<n-1 ) then
     ...
     !! This is the unblocked version of the algorithm, calling Level 2 BLAS.
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              ! determine both colmax and imax.
              if( k>1_ilp ) then
                 imax = stdlib_iwamax( k-1, a( 1_ilp, k ), 1_ilp )
                 colmax = cabs1( a( imax, k ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero .or. stdlib_qisnan(absakk) ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! jmax is the column-index of the largest off-diagonal
                    ! element in row imax, and rowmax is its absolute value
                    jmax = imax + stdlib_iwamax( k-imax, a( imax, imax+1 ), lda )
                    rowmax = cabs1( a( imax, jmax ) )
                    if( imax>1_ilp ) then
                       jmax = stdlib_iwamax( imax-1, a( 1_ilp, imax ), 1_ilp )
                       rowmax = max( rowmax, cabs1( a( jmax, imax ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                 ...
                 if( kp/=kk ) then
                    ! interchange rows and columns kk and kp in the leading
                    ! submatrix a(1:k,1:k)
                    call stdlib_wswap( kp-1, a( 1_ilp, kk ), 1_ilp, a( 1_ilp, kp ), 1_ilp )
                    call stdlib_wswap( kk-kp-1, a( kp+1, kk ), 1_ilp, a( kp, kp+1 ),lda )
                    t = a( kk, kk )
                    a( kk, kk ) = a( kp, kp )
                    a( kp, kp ) = t
                    if( kstep==2_ilp ) then
                 ...
                 if( kstep==1_ilp ) then
                    ...
                    ! a := a - u(k)*d(k)*u(k)**t = a - w(k)*1/d(k)*w(k)**t
                    r1 = cone / a( k, k )
                    call stdlib_wsyr( uplo, k-1, -r1, a( 1_ilp, k ), 1_ilp, a, lda )
                    ! store u(k) in column k
                    call stdlib_wscal( k-1, r1, a( 1_ilp, k ), 1_ilp )
                 else
                    ! 2-by-2 pivot block d(k): columns k and k-1 now hold
              ...
              ! determine both colmax and imax.
              if( k<n ) then
                 imax = k + stdlib_iwamax( n-k, a( k+1, k ), 1_ilp )
                 colmax = cabs1( a( imax, k ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero .or. stdlib_qisnan(absakk) ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! jmax is the column-index of the largest off-diagonal
                    ! element in row imax, and rowmax is its absolute value
                    jmax = k - 1_ilp + stdlib_iwamax( imax-k, a( imax, k ), lda )
                    rowmax = cabs1( a( imax, jmax ) )
                    if( imax<n ) then
                       jmax = imax + stdlib_iwamax( n-imax, a( imax+1, imax ), 1_ilp )
                       rowmax = max( rowmax, cabs1( a( jmax, imax ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                 ...
                 if( kp/=kk ) then
                    ! interchange rows and columns kk and kp in the trailing
                    ! submatrix a(k:n,k:n)
                    if( kp<n )call stdlib_wswap( n-kp, a( kp+1, kk ), 1_ilp, a( kp+1, kp ), 1_ilp )
                              
                    call stdlib_wswap( kp-kk-1, a( kk+1, kk ), 1_ilp, a( kp, kk+1 ),lda )
                    t = a( kk, kk )
                    a( kk, kk ) = a( kp, kp )
                    a( kp, kp ) = t
                    if( kstep==2_ilp ) then
                 ...
                 if( kstep==1_ilp ) then
                    ...
                    if( k<n ) then
                       ...
                       ! a := a - l(k)*d(k)*l(k)**t = a - w(k)*(1/d(k))*w(k)**t
                       r1 = cone / a( k, k )
                       call stdlib_wsyr( uplo, n-k, -r1, a( k+1, k ), 1_ilp,a( k+1, k+1 ), lda )
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! interchange rows k and ipiv(k).
                 kp = ipiv( k )
                 if( kp/=k )call stdlib_qswap( nrhs, b( k, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in column k of a.
                 call stdlib_qger( k-1, nrhs, -one, a( 1_ilp, k ), 1_ilp, b( k, 1_ilp ), ldb,b( 1_ilp, 1_ilp ), ldb )
                           
                 ! multiply by the inverse of the diagonal block.
                 call stdlib_qscal( nrhs, one / a( k, k ), b( k, 1_ilp ), ldb )
                 k = k - 1_ilp
              else
                 ! 2 x 2 diagonal block
                 ! interchange rows k-1 and -ipiv(k).
                 kp = -ipiv( k )
                 if( kp/=k-1 )call stdlib_qswap( nrhs, b( k-1, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in columns k-1 and k of a.
                 call stdlib_qger( k-2, nrhs, -one, a( 1_ilp, k ), 1_ilp, b( k, 1_ilp ), ldb,b( 1_ilp, 1_ilp ), ldb )
                           
                 call stdlib_qger( k-2, nrhs, -one, a( 1_ilp, k-1 ), 1_ilp, b( k-1, 1_ilp ),ldb, b( 1_ilp, 1_ilp ), &
                           ldb )
                 ! multiply by the inverse of the diagonal block.
                 akm1k = a( k-1, k )
                 akm1 = a( k-1, k-1 ) / akm1k
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! multiply by inv(u**t(k)), where u(k) is the transformation
                 ! stored in column k of a.
                 call stdlib_qgemv( 'TRANSPOSE', k-1, nrhs, -one, b, ldb, a( 1_ilp, k ),1_ilp, one, b( k, &
                           1_ilp ), ldb )
                 ! interchange rows k and ipiv(k).
                 kp = ipiv( k )
                 if( kp/=k )call stdlib_qswap( nrhs, b( k, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
                 ...
                 call stdlib_qgemv( 'TRANSPOSE', k-1, nrhs, -one, b, ldb, a( 1_ilp, k ),1_ilp, one, b( k, &
                           1_ilp ), ldb )
                 call stdlib_qgemv( 'TRANSPOSE', k-1, nrhs, -one, b, ldb,a( 1_ilp, k+1 ), 1_ilp, one, b( &
                           k+1, 1_ilp ), ldb )
                 ! interchange rows k and -ipiv(k).
                 kp = -ipiv( k )
                 if( kp/=k )call stdlib_qswap( nrhs, b( k, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! multiply by inv(l(k)), where l(k) is the transformation
                 ! stored in column k of a.
                 if( k<n )call stdlib_qger( n-k, nrhs, -one, a( k+1, k ), 1_ilp, b( k, 1_ilp ),ldb, b( k+&
                           1_ilp, 1_ilp ), ldb )
                 ! multiply by the inverse of the diagonal block.
                 call stdlib_qscal( nrhs, one / a( k, k ), b( k, 1_ilp ), ldb )
                 k = k + 1_ilp
                 ...
                 ! stored in columns k and k+1 of a.
                 if( k<n-1 ) then
                    call stdlib_qger( n-k-1, nrhs, -one, a( k+2, k ), 1_ilp, b( k, 1_ilp ),ldb, b( k+2, 1_ilp &
                              ), ldb )
                    call stdlib_qger( n-k-1, nrhs, -one, a( k+2, k+1 ), 1_ilp,b( k+1, 1_ilp ), ldb, b( k+&
                              2_ilp, 1_ilp ), ldb )
                 end if
                 ! multiply by the inverse of the diagonal block.
                 akm1k = a( k+1, k )
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! multiply by inv(l**t(k)), where l(k) is the transformation
                 ! stored in column k of a.
                 if( k<n )call stdlib_qgemv( 'TRANSPOSE', n-k, nrhs, -one, b( k+1, 1_ilp ),ldb, a( k+&
                           1_ilp, k ), 1_ilp, one, b( k, 1_ilp ), ldb )
                 ! interchange rows k and ipiv(k).
                 kp = ipiv( k )
                 if( kp/=k )call stdlib_qswap( nrhs, b( k, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
                 ...
                 ! stored in columns k-1 and k of a.
                 if( k<n ) then
                    call stdlib_qgemv( 'TRANSPOSE', n-k, nrhs, -one, b( k+1, 1_ilp ),ldb, a( k+1, k ),&
                               1_ilp, one, b( k, 1_ilp ), ldb )
                    call stdlib_qgemv( 'TRANSPOSE', n-k, nrhs, -one, b( k+1, 1_ilp ),ldb, a( k+1, k-1 &
                              ), 1_ilp, one, b( k-1, 1_ilp ),ldb )
                 end if
                 ! interchange rows k and -ipiv(k).
                 kp = -ipiv( k )
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! interchange rows k and ipiv(k).
                 kp = ipiv( k )
                 if( kp/=k )call stdlib_wswap( nrhs, b( k, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in column k of a.
                 call stdlib_wgeru( k-1, nrhs, -cone, a( 1_ilp, k ), 1_ilp, b( k, 1_ilp ), ldb,b( 1_ilp, 1_ilp ), ldb &
                           )
                 ! multiply by the inverse of the diagonal block.
                 call stdlib_wscal( nrhs, cone / a( k, k ), b( k, 1_ilp ), ldb )
                 k = k - 1_ilp
              else
                 ! 2 x 2 diagonal block
                 ! interchange rows k-1 and -ipiv(k).
                 kp = -ipiv( k )
                 if( kp/=k-1 )call stdlib_wswap( nrhs, b( k-1, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in columns k-1 and k of a.
                 call stdlib_wgeru( k-2, nrhs, -cone, a( 1_ilp, k ), 1_ilp, b( k, 1_ilp ), ldb,b( 1_ilp, 1_ilp ), ldb &
                           )
                 call stdlib_wgeru( k-2, nrhs, -cone, a( 1_ilp, k-1 ), 1_ilp, b( k-1, 1_ilp ),ldb, b( 1_ilp, 1_ilp ), &
                           ldb )
                 ! multiply by the inverse of the diagonal block.
                 akm1k = a( k-1, k )
                 akm1 = a( k-1, k-1 ) / akm1k
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! multiply by inv(u**t(k)), where u(k) is the transformation
                 ! stored in column k of a.
                 call stdlib_wgemv( 'TRANSPOSE', k-1, nrhs, -cone, b, ldb, a( 1_ilp, k ),1_ilp, cone, b( &
                           k, 1_ilp ), ldb )
                 ! interchange rows k and ipiv(k).
                 kp = ipiv( k )
                 if( kp/=k )call stdlib_wswap( nrhs, b( k, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
                 ...
                 call stdlib_wgemv( 'TRANSPOSE', k-1, nrhs, -cone, b, ldb, a( 1_ilp, k ),1_ilp, cone, b( &
                           k, 1_ilp ), ldb )
                 call stdlib_wgemv( 'TRANSPOSE', k-1, nrhs, -cone, b, ldb,a( 1_ilp, k+1 ), 1_ilp, cone, b(&
                            k+1, 1_ilp ), ldb )
                 ! interchange rows k and -ipiv(k).
                 kp = -ipiv( k )
                 if( kp/=k )call stdlib_wswap( nrhs, b( k, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! multiply by inv(l(k)), where l(k) is the transformation
                 ! stored in column k of a.
                 if( k<n )call stdlib_wgeru( n-k, nrhs, -cone, a( k+1, k ), 1_ilp, b( k, 1_ilp ),ldb, b( &
                           k+1, 1_ilp ), ldb )
                 ! multiply by the inverse of the diagonal block.
                 call stdlib_wscal( nrhs, cone / a( k, k ), b( k, 1_ilp ), ldb )
                 k = k + 1_ilp
                 ...
                 ! stored in columns k and k+1 of a.
                 if( k<n-1 ) then
                    call stdlib_wgeru( n-k-1, nrhs, -cone, a( k+2, k ), 1_ilp, b( k, 1_ilp ),ldb, b( k+2, &
                              1_ilp ), ldb )
                    call stdlib_wgeru( n-k-1, nrhs, -cone, a( k+2, k+1 ), 1_ilp,b( k+1, 1_ilp ), ldb, b( &
                              k+2, 1_ilp ), ldb )
                 end if
                 ! multiply by the inverse of the diagonal block.
                 akm1k = a( k+1, k )
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! multiply by inv(l**t(k)), where l(k) is the transformation
                 ! stored in column k of a.
                 if( k<n )call stdlib_wgemv( 'TRANSPOSE', n-k, nrhs, -cone, b( k+1, 1_ilp ),ldb, a( k+&
                           1_ilp, k ), 1_ilp, cone, b( k, 1_ilp ), ldb )
                 ! interchange rows k and ipiv(k).
                 kp = ipiv( k )
                 if( kp/=k )call stdlib_wswap( nrhs, b( k, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
                 ...
                 ! stored in columns k-1 and k of a.
                 if( k<n ) then
                    call stdlib_wgemv( 'TRANSPOSE', n-k, nrhs, -cone, b( k+1, 1_ilp ),ldb, a( k+1, k )&
                              , 1_ilp, cone, b( k, 1_ilp ), ldb )
                    call stdlib_wgemv( 'TRANSPOSE', n-k, nrhs, -cone, b( k+1, 1_ilp ),ldb, a( k+1, k-&
                              1_ilp ), 1_ilp, cone, b( k-1, 1_ilp ),ldb )
                 end if
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! compute column k of the inverse.
                 if( k>1_ilp ) then
                    call stdlib_qcopy( k-1, a( 1_ilp, k ), 1_ilp, work, 1_ilp )
                    call stdlib_qsymv( uplo, k-1, -one, a, lda, work, 1_ilp, zero,a( 1_ilp, k ), 1_ilp )
                              
                    a( k, k ) = a( k, k ) - stdlib_qdot( k-1, work, 1_ilp, a( 1_ilp, k ),1_ilp )
                 end if
                 kstep = 1_ilp
              else
                 ! 2 x 2 diagonal block
                 ...
                 if( k>1_ilp ) then
                    ...
                    a( k, k ) = a( k, k ) - stdlib_qdot( k-1, work, 1_ilp, a( 1_ilp, k ),1_ilp )
                    a( k, k+1 ) = a( k, k+1 ) -stdlib_qdot( k-1, a( 1_ilp, k ), 1_ilp, a( 1_ilp, k+1 ), 1_ilp )
                              
                    call stdlib_qcopy( k-1, a( 1_ilp, k+1 ), 1_ilp, work, 1_ilp )
                    call stdlib_qsymv( uplo, k-1, -one, a, lda, work, 1_ilp, zero,a( 1_ilp, k+1 ), 1_ilp )
                              
                    a( k+1, k+1 ) = a( k+1, k+1 ) -stdlib_qdot( k-1, work, 1_ilp, a( 1_ilp, k+1 ), 1_ilp )
              ...
              if( kp/=k ) then
                 ! interchange rows and columns k and kp in the leading
                 ! submatrix a(1:k+1,1:k+1)
                 call stdlib_qswap( kp-1, a( 1_ilp, k ), 1_ilp, a( 1_ilp, kp ), 1_ilp )
                 call stdlib_qswap( k-kp-1, a( kp+1, k ), 1_ilp, a( kp, kp+1 ), lda )
                 temp = a( k, k )
                 a( k, k ) = a( kp, kp )
                 a( kp, kp ) = temp
                 if( kstep==2_ilp ) then
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! compute column k of the inverse.
                 if( k<n ) then
                    call stdlib_qcopy( n-k, a( k+1, k ), 1_ilp, work, 1_ilp )
                    call stdlib_qsymv( uplo, n-k, -one, a( k+1, k+1 ), lda, work, 1_ilp,zero, a( k+1, &
                              k ), 1_ilp )
                    a( k, k ) = a( k, k ) - stdlib_qdot( n-k, work, 1_ilp, a( k+1, k ),1_ilp )
                 end if
                 kstep = 1_ilp
              else
                 ! 2 x 2 diagonal block
                 ...
                 if( k<n ) then
                    ...
                    call stdlib_qsymv( uplo, n-k, -one, a( k+1, k+1 ), lda, work, 1_ilp,zero, a( k+1, &
                              k ), 1_ilp )
                    a( k, k ) = a( k, k ) - stdlib_qdot( n-k, work, 1_ilp, a( k+1, k ),1_ilp )
                    a( k, k-1 ) = a( k, k-1 ) -stdlib_qdot( n-k, a( k+1, k ), 1_ilp, a( k+1, k-1 ),1_ilp )
                              
                    call stdlib_qcopy( n-k, a( k+1, k-1 ), 1_ilp, work, 1_ilp )
                    call stdlib_qsymv( uplo, n-k, -one, a( k+1, k+1 ), lda, work, 1_ilp,zero, a( k+1, &
                              k-1 ), 1_ilp )
                    a( k-1, k-1 ) = a( k-1, k-1 ) -stdlib_qdot( n-k, work, 1_ilp, a( k+1, k-1 ), 1_ilp )
              ...
              if( kp/=k ) then
                 ! interchange rows and columns k and kp in the trailing
                 ! submatrix a(k-1:n,k-1:n)
                 if( kp<n )call stdlib_qswap( n-kp, a( kp+1, k ), 1_ilp, a( kp+1, kp ), 1_ilp )
                 call stdlib_qswap( kp-k-1, a( k+1, k ), 1_ilp, a( kp, k+1 ), lda )
                 temp = a( k, k )
                 a( k, k ) = a( kp, kp )
                 a( kp, kp ) = temp
                 if( kstep==2_ilp ) then
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! compute column k of the inverse.
                 if( k>1_ilp ) then
                    call stdlib_wcopy( k-1, a( 1_ilp, k ), 1_ilp, work, 1_ilp )
                    call stdlib_wsymv( uplo, k-1, -cone, a, lda, work, 1_ilp, czero,a( 1_ilp, k ), 1_ilp )
                              
                    a( k, k ) = a( k, k ) - stdlib_wdotu( k-1, work, 1_ilp, a( 1_ilp, k ),1_ilp )
                 end if
                 kstep = 1_ilp
              else
                 ! 2 x 2 diagonal block
                 ...
                 if( k>1_ilp ) then
                    ...
                    a( k, k ) = a( k, k ) - stdlib_wdotu( k-1, work, 1_ilp, a( 1_ilp, k ),1_ilp )
                    a( k, k+1 ) = a( k, k+1 ) -stdlib_wdotu( k-1, a( 1_ilp, k ), 1_ilp, a( 1_ilp, k+1 ), 1_ilp )
                              
                    call stdlib_wcopy( k-1, a( 1_ilp, k+1 ), 1_ilp, work, 1_ilp )
                    call stdlib_wsymv( uplo, k-1, -cone, a, lda, work, 1_ilp, czero,a( 1_ilp, k+1 ), 1_ilp )
                              
                    a( k+1, k+1 ) = a( k+1, k+1 ) -stdlib_wdotu( k-1, work, 1_ilp, a( 1_ilp, k+1 ), 1_ilp )
              ...
              if( kp/=k ) then
                 ! interchange rows and columns k and kp in the leading
                 ! submatrix a(1:k+1,1:k+1)
                 call stdlib_wswap( kp-1, a( 1_ilp, k ), 1_ilp, a( 1_ilp, kp ), 1_ilp )
                 call stdlib_wswap( k-kp-1, a( kp+1, k ), 1_ilp, a( kp, kp+1 ), lda )
                 temp = a( k, k )
                 a( k, k ) = a( kp, kp )
                 a( kp, kp ) = temp
                 if( kstep==2_ilp ) then
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! compute column k of the inverse.
                 if( k<n ) then
                    call stdlib_wcopy( n-k, a( k+1, k ), 1_ilp, work, 1_ilp )
                    call stdlib_wsymv( uplo, n-k, -cone, a( k+1, k+1 ), lda, work, 1_ilp,czero, a( k+&
                              1_ilp, k ), 1_ilp )
                    a( k, k ) = a( k, k ) - stdlib_wdotu( n-k, work, 1_ilp, a( k+1, k ),1_ilp )
                 end if
                 kstep = 1_ilp
              else
                 ! 2 x 2 diagonal block
                 ...
                 if( k<n ) then
                    ...
                    call stdlib_wsymv( uplo, n-k, -cone, a( k+1, k+1 ), lda, work, 1_ilp,czero, a( k+&
                              1_ilp, k ), 1_ilp )
                    a( k, k ) = a( k, k ) - stdlib_wdotu( n-k, work, 1_ilp, a( k+1, k ),1_ilp )
                    a( k, k-1 ) = a( k, k-1 ) -stdlib_wdotu( n-k, a( k+1, k ), 1_ilp, a( k+1, k-1 ),1_ilp &
                              )
                    call stdlib_wcopy( n-k, a( k+1, k-1 ), 1_ilp, work, 1_ilp )
                    call stdlib_wsymv( uplo, n-k, -cone, a( k+1, k+1 ), lda, work, 1_ilp,czero, a( k+&
                              1_ilp, k-1 ), 1_ilp )
                    a( k-1, k-1 ) = a( k-1, k-1 ) -stdlib_wdotu( n-k, work, 1_ilp, a( k+1, k-1 ), 1_ilp )
              ...
              if( kp/=k ) then
                 ! interchange rows and columns k and kp in the trailing
                 ! submatrix a(k-1:n,k-1:n)
                 if( kp<n )call stdlib_wswap( n-kp, a( kp+1, k ), 1_ilp, a( kp+1, kp ), 1_ilp )
                 call stdlib_wswap( kp-k-1, a( k+1, k ), 1_ilp, a( kp, k+1 ), lda )
                 temp = a( k, k )
                 a( k, k ) = a( kp, kp )
                 a( kp, kp ) = temp
                 if( kstep==2_ilp ) then
     ...
     !! provides error bounds and backward error estimates for the solution.
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1, nrhs
              ...
              call stdlib_qsymv( uplo, n, -one, a, lda, x( 1_ilp, j ), 1_ilp, one,work( n+1 ), 1_ilp )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(a)*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ! or vector z.  if the i-th component of the denominator is less
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_qsytrs( uplo, n, 1_ilp, af, ldaf, ipiv, work( n+1 ), n,info )
     ...
     !! provides error bounds and backward error estimates for the solution.
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1, nrhs
              ...
              call stdlib_wsymv( uplo, n, -cone, a, lda, x( 1_ilp, j ), 1_ilp, cone, work, 1_ilp )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(a)*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ! or vector z.  if the i-th component of the denominator is less
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_wsytrs( uplo, n, 1_ilp, af, ldaf, ipiv, work, n, info )
     ...
     !! scalings.
        ...
        ! =====================================================================
           ...
           do iter = 1, max_iter
              ...
              call stdlib_qlassq( n, work( n+1 ), 1_ilp, scale, sumsq )
              std = scale * sqrt( sumsq / n )
              if ( std < tol * avg ) goto 999
              do i = 1, n
                 t = abs( a( i, i ) )
     ...
     !! scalings.
        ...
        ! =====================================================================
           ...
           do iter = 1, max_iter
              ...
              call stdlib_wlassq( n, work( n+1 ), 1_ilp, scale, sumsq )
              std = scale * sqrt( sumsq / n )
              if ( std < tol * avg ) goto 999
              do i = 1, n
                 t = cabs1( a( i, i ) )
                 ...
                 c0 = -(t*si)*si + 2_ilp * real( work( i ),KIND=qp) * si - n*avg
                 d = c1*c1 - 4_ilp*c0*c2
                 if ( d <= 0_ilp ) then
                    info = -1_ilp
                    return
     ...
     !! A = L*D*L**T computed by DSYTRF and converted by DSYCONV.
        ...
        ! =====================================================================
           ...
           if( n==0 .or. nrhs==0 )return
           ! convert a
           call stdlib_qsyconv( uplo, 'C', n, a, lda, ipiv, work, iinfo )
           if( upper ) then
              ! solve a*x = b, where a = u*d*u**t.
             ! p**t * b
             k=n
             do while ( k >= 1 )
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! interchange rows k-1 and -ipiv(k).
                 kp = -ipiv( k )
                 if( kp==-ipiv( k-1 ) )call stdlib_qswap( nrhs, b( k-1, 1_ilp ), ldb, b( kp, 1_ilp ), ldb &
                           )
                 k=k-2
              end if
             end do
        ! compute (u \p**t * b) -> b    [ (u \p**t * b) ]
             call stdlib_qtrsm('L','U','N','U',n,nrhs,one,a,lda,b,ldb)
        ! compute d \ b -> b   [ d \ (u \p**t * b) ]
              i=n
              do while ( i >= 1 )
                 if( ipiv(i) > 0_ilp ) then
                   call stdlib_qscal( nrhs, one / a( i, i ), b( i, 1_ilp ), ldb )
                 elseif ( i > 1_ilp) then
                    if ( ipiv(i-1) == ipiv(i) ) then
                       akm1k = work(i)
                       akm1 = a( i-1, i-1 ) / akm1k
            ...
            ! compute (u**t \ b) -> b   [ u**t \ (d \ (u \p**t * b) ) ]
              call stdlib_qtrsm('L','U','T','U',n,nrhs,one,a,lda,b,ldb)
             ! p * b  [ p * (u**t \ (d \ (u \p**t * b) )) ]
             k=1_ilp
             do while ( k <= n )
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! interchange rows k-1 and -ipiv(k).
                 kp = -ipiv( k )
                 if( k < n .and. kp==-ipiv( k+1 ) )call stdlib_qswap( nrhs, b( k, 1_ilp ), ldb, b( kp,&
                            1_ilp ), ldb )
                 k=k+2
              endif
             end do
             ...
             do while ( k <= n )
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! interchange rows k and -ipiv(k+1).
                 kp = -ipiv( k+1 )
                 if( kp==-ipiv( k ) )call stdlib_qswap( nrhs, b( k+1, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
                           
                 k=k+2
              endif
             end do
        ! compute (l \p**t * b) -> b    [ (l \p**t * b) ]
             call stdlib_qtrsm('L','L','N','U',n,nrhs,one,a,lda,b,ldb)
        ! compute d \ b -> b   [ d \ (l \p**t * b) ]
              i=1_ilp
              do while ( i <= n )
                 if( ipiv(i) > 0_ilp ) then
        ...
        ! compute (l**t \ b) -> b   [ l**t \ (d \ (l \p**t * b) ) ]
             call stdlib_qtrsm('L','L','T','U',n,nrhs,one,a,lda,b,ldb)
             ! p * b  [ p * (l**t \ (d \ (l \p**t * b) )) ]
             k=n
             do while ( k >= 1 )
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! interchange rows k-1 and -ipiv(k).
                 kp = -ipiv( k )
                 if( k>1_ilp .and. kp==-ipiv( k-1 ) )call stdlib_qswap( nrhs, b( k, 1_ilp ), ldb, b( kp, &
                           1_ilp ), ldb )
                 k=k-2
              endif
             end do
     ...
     !! A = L*D*L**T computed by ZSYTRF and converted by ZSYCONV.
        ...
        ! =====================================================================
           ...
           if( n==0 .or. nrhs==0 )return
           ! convert a
           call stdlib_wsyconv( uplo, 'C', n, a, lda, ipiv, work, iinfo )
           if( upper ) then
              ! solve a*x = b, where a = u*d*u**t.
             ! p**t * b
             k=n
             do while ( k >= 1 )
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! interchange rows k-1 and -ipiv(k).
                 kp = -ipiv( k )
                 if( kp==-ipiv( k-1 ) )call stdlib_wswap( nrhs, b( k-1, 1_ilp ), ldb, b( kp, 1_ilp ), ldb &
                           )
                 k=k-2
              end if
             end do
        ! compute (u \p**t * b) -> b    [ (u \p**t * b) ]
             call stdlib_wtrsm('L','U','N','U',n,nrhs,cone,a,lda,b,ldb)
        ! compute d \ b -> b   [ d \ (u \p**t * b) ]
              i=n
              do while ( i >= 1 )
                 if( ipiv(i) > 0_ilp ) then
                   call stdlib_wscal( nrhs, cone / a( i, i ), b( i, 1_ilp ), ldb )
                 elseif ( i > 1_ilp) then
                    if ( ipiv(i-1) == ipiv(i) ) then
                       akm1k = work(i)
                       akm1 = a( i-1, i-1 ) / akm1k
            ...
            ! compute (u**t \ b) -> b   [ u**t \ (d \ (u \p**t * b) ) ]
              call stdlib_wtrsm('L','U','T','U',n,nrhs,cone,a,lda,b,ldb)
             ! p * b  [ p * (u**t \ (d \ (u \p**t * b) )) ]
             k=1_ilp
             do while ( k <= n )
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! interchange rows k-1 and -ipiv(k).
                 kp = -ipiv( k )
                 if( k < n .and. kp==-ipiv( k+1 ) )call stdlib_wswap( nrhs, b( k, 1_ilp ), ldb, b( kp,&
                            1_ilp ), ldb )
                 k=k+2
              endif
             end do
             ...
             do while ( k <= n )
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! interchange rows k and -ipiv(k+1).
                 kp = -ipiv( k+1 )
                 if( kp==-ipiv( k ) )call stdlib_wswap( nrhs, b( k+1, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
                           
                 k=k+2
              endif
             end do
        ! compute (l \p**t * b) -> b    [ (l \p**t * b) ]
             call stdlib_wtrsm('L','L','N','U',n,nrhs,cone,a,lda,b,ldb)
        ! compute d \ b -> b   [ d \ (l \p**t * b) ]
              i=1_ilp
              do while ( i <= n )
                 if( ipiv(i) > 0_ilp ) then
        ...
        ! compute (l**t \ b) -> b   [ l**t \ (d \ (l \p**t * b) ) ]
             call stdlib_wtrsm('L','L','T','U',n,nrhs,cone,a,lda,b,ldb)
             ! p * b  [ p * (l**t \ (d \ (l \p**t * b) )) ]
             k=n
             do while ( k >= 1 )
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! interchange rows k-1 and -ipiv(k).
                 kp = -ipiv( k )
                 if( k>1_ilp .and. kp==-ipiv( k-1 ) )call stdlib_wswap( nrhs, b( k, 1_ilp ), ldb, b( kp, &
                           1_ilp ), ldb )
                 k=k-2
              endif
             end do
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do k = n, 1, -1
                 kp = abs( ipiv( k ) )
                 if( kp/=k ) then
                    call stdlib_qswap( nrhs, b( k, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
                 end if
              end do
              ! compute (u \p**t * b) -> b    [ (u \p**t * b) ]
              call stdlib_qtrsm( 'L', 'U', 'N', 'U', n, nrhs, one, a, lda, b, ldb )
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              do k = n, 1, -1
                 kp = abs( ipiv( k ) )
                 if( kp/=k ) then
                    call stdlib_wswap( nrhs, b( k, 1_ilp ), ldb, b( kp, 1_ilp ), ldb )
                 end if
              end do
              ! compute (u \p**t * b) -> b    [ (u \p**t * b) ]
              call stdlib_wtrsm( 'L', 'U', 'N', 'U', n, nrhs, cone, a, lda, b, ldb )
     ...
     !! a symmetric matrix.
        ...
        ! =====================================================================
           ...
           if (upper) then
               ...
               ! first swap
                ! - swap column i1 and i2 from i1 to i1-1
              call stdlib_qswap( i1-1, a(1_ilp,i1), 1_ilp, a(1_ilp,i2), 1_ilp )
                ! second swap :
                ! - swap a(i1,i1) and a(i2,i2)
                ! - swap row i1 from i1+1 to i2-1 with col i2 from i1+1 to i2-1
              tmp=a(i1,i1)
              ...
              do i=i2+1,n
               ...
               ! first swap
                ! - swap row i1 and i2 from i1 to i1-1
              call stdlib_qswap( i1-1, a(i1,1_ilp), lda, a(i2,1_ilp), lda )
               ! second swap :
                ! - swap a(i1,i1) and a(i2,i2)
                ! - swap col i1 from i1+1 to i2-1 with row i2 from i1+1 to i2-1
               tmp=a(i1,i1)
     ...
     !! a symmetric matrix.
        ...
        ! =====================================================================
           ...
           if (upper) then
               ...
               ! first swap
                ! - swap column i1 and i2 from i1 to i1-1
              call stdlib_wswap( i1-1, a(1_ilp,i1), 1_ilp, a(1_ilp,i2), 1_ilp )
                ! second swap :
                ! - swap a(i1,i1) and a(i2,i2)
                ! - swap row i1 from i1+1 to i2-1 with col i2 from i1+1 to i2-1
              tmp=a(i1,i1)
              ...
              do i=i2+1,n
               ...
               ! first swap
                ! - swap row i1 and i2 from i1 to i1-1
              call stdlib_wswap( i1-1, a(i1,1_ilp), lda, a(i2,1_ilp), lda )
               ! second swap :
                ! - swap a(i1,i1) and a(i2,i2)
                ! - swap col i1 from i1+1 to i2-1 with row i2 from i1+1 to i2-1
               tmp=a(i1,i1)
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              ! multiply by inv(l*d*l**t) or inv(u*d*u**t).
              call stdlib_qsptrs( uplo, n, 1_ilp, ap, ipiv, work, n, info )
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm/=zero )rcond = ( one / ainvnm ) / anorm
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              ! multiply by inv(l*d*l**t) or inv(u*d*u**t).
              call stdlib_wsptrs( uplo, n, 1_ilp, ap, ipiv, work, n, info )
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm/=zero )rcond = ( one / ainvnm ) / anorm
     ...
     !! 1-by-1 and 2-by-2 diagonal blocks.
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              ! column k, and colmax is its absolute value
              if( k>1_ilp ) then
                 imax = stdlib_iqamax( k-1, ap( kc ), 1_ilp )
                 colmax = abs( ap( kc+imax-1 ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    kpc = ( imax-1 )*imax / 2_ilp + 1_ilp
                    if( imax>1_ilp ) then
                       jmax = stdlib_iqamax( imax-1, ap( kpc ), 1_ilp )
                       rowmax = max( rowmax, abs( ap( kpc+jmax-1 ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                 ...
                 if( kp/=kk ) then
                    ! interchange rows and columns kk and kp in the leading
                    ! submatrix a(1:k,1:k)
                    call stdlib_qswap( kp-1, ap( knc ), 1_ilp, ap( kpc ), 1_ilp )
                    kx = kpc + kp - 1_ilp
                    do j = kp + 1, kk - 1
                       kx = kx + j - 1_ilp
                       t = ap( knc+j-1 )
                 ...
                 if( kstep==1_ilp ) then
                    ...
                    ! a := a - u(k)*d(k)*u(k)**t = a - w(k)*1/d(k)*w(k)**t
                    r1 = one / ap( kc+k-1 )
                    call stdlib_qspr( uplo, k-1, -r1, ap( kc ), 1_ilp, ap )
                    ! store u(k) in column k
                    call stdlib_qscal( k-1, r1, ap( kc ), 1_ilp )
                 else
                    ! 2-by-2 pivot block d(k): columns k and k-1 now hold
                    ! ( w(k-1) w(k) ) = ( u(k-1) u(k) )*d(k)
                    ! where u(k) and u(k-1) are the k-th and (k-1)-th columns
              ...
              ! column k, and colmax is its absolute value
              if( k<n ) then
                 imax = k + stdlib_iqamax( n-k, ap( kc+1 ), 1_ilp )
                 colmax = abs( ap( kc+imax-k ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    kpc = npp - ( n-imax+1 )*( n-imax+2 ) / 2_ilp + 1_ilp
                    if( imax<n ) then
                       jmax = imax + stdlib_iqamax( n-imax, ap( kpc+1 ), 1_ilp )
                       rowmax = max( rowmax, abs( ap( kpc+jmax-imax ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                 ...
                 if( kp/=kk ) then
                    ! interchange rows and columns kk and kp in the trailing
                    ! submatrix a(k:n,k:n)
                    if( kp<n )call stdlib_qswap( n-kp, ap( knc+kp-kk+1 ), 1_ilp, ap( kpc+1 ),1_ilp )
                 ...
                 if( kstep==1_ilp ) then
                    ...
                    if( k<n ) then
                       ...
                       ! a := a - l(k)*d(k)*l(k)**t = a - w(k)*(1/d(k))*w(k)**t
                       r1 = one / ap( kc )
                       call stdlib_qspr( uplo, n-k, -r1, ap( kc+1 ), 1_ilp,ap( kc+n-k+1 ) )
                       ! store l(k) in column k
                       call stdlib_qscal( n-k, r1, ap( kc+1 ), 1_ilp )
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns k and k+1 now hold
                    ! ( w(k) w(k+1) ) = ( l(k) l(k+1) )*d(k)
     ...
     !! 1-by-1 and 2-by-2 diagonal blocks.
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              ! column k, and colmax is its absolute value
              if( k>1_ilp ) then
                 imax = stdlib_iwamax( k-1, ap( kc ), 1_ilp )
                 colmax = cabs1( ap( kc+imax-1 ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    kpc = ( imax-1 )*imax / 2_ilp + 1_ilp
                    if( imax>1_ilp ) then
                       jmax = stdlib_iwamax( imax-1, ap( kpc ), 1_ilp )
                       rowmax = max( rowmax, cabs1( ap( kpc+jmax-1 ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                 ...
                 if( kp/=kk ) then
                    ! interchange rows and columns kk and kp in the leading
                    ! submatrix a(1:k,1:k)
                    call stdlib_wswap( kp-1, ap( knc ), 1_ilp, ap( kpc ), 1_ilp )
                    kx = kpc + kp - 1_ilp
                    do j = kp + 1, kk - 1
                       kx = kx + j - 1_ilp
                       t = ap( knc+j-1 )
                 ...
                 if( kstep==1_ilp ) then
                    ...
                    ! a := a - u(k)*d(k)*u(k)**t = a - w(k)*1/d(k)*w(k)**t
                    r1 = cone / ap( kc+k-1 )
                    call stdlib_wspr( uplo, k-1, -r1, ap( kc ), 1_ilp, ap )
                    ! store u(k) in column k
                    call stdlib_wscal( k-1, r1, ap( kc ), 1_ilp )
                 else
                    ! 2-by-2 pivot block d(k): columns k and k-1 now hold
                    ! ( w(k-1) w(k) ) = ( u(k-1) u(k) )*d(k)
                    ! where u(k) and u(k-1) are the k-th and (k-1)-th columns
              ...
              ! column k, and colmax is its absolute value
              if( k<n ) then
                 imax = k + stdlib_iwamax( n-k, ap( kc+1 ), 1_ilp )
                 colmax = cabs1( ap( kc+imax-k ) )
              else
                 colmax = zero
              end if
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    kpc = npp - ( n-imax+1 )*( n-imax+2 ) / 2_ilp + 1_ilp
                    if( imax<n ) then
                       jmax = imax + stdlib_iwamax( n-imax, ap( kpc+1 ), 1_ilp )
                       rowmax = max( rowmax, cabs1( ap( kpc+jmax-imax ) ) )
                    end if
                    if( absakk>=alpha*colmax*( colmax / rowmax ) ) then
                       ! no interchange, use 1-by-1 pivot block
                 ...
                 if( kp/=kk ) then
                    ! interchange rows and columns kk and kp in the trailing
                    ! submatrix a(k:n,k:n)
                    if( kp<n )call stdlib_wswap( n-kp, ap( knc+kp-kk+1 ), 1_ilp, ap( kpc+1 ),1_ilp )
                 ...
                 if( kstep==1_ilp ) then
                    ...
                    if( k<n ) then
                       ...
                       ! a := a - l(k)*d(k)*l(k)**t = a - w(k)*(1/d(k))*w(k)**t
                       r1 = cone / ap( kc )
                       call stdlib_wspr( uplo, n-k, -r1, ap( kc+1 ), 1_ilp,ap( kc+n-k+1 ) )
                       ! store l(k) in column k
                       call stdlib_wscal( n-k, r1, ap( kc+1 ), 1_ilp )
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns k and k+1 now hold
                    ! ( w(k) w(k+1) ) = ( l(k) l(k+1) )*d(k)
