  ...
  contains
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              if( n==0_ilp ) then
                 ...
                 call stdlib_qsytrf( uplo, n, a, lda, ipiv, work, -1_ilp, info )
                 lwkopt = work(1_ilp)
              end if
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_qsytrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_qsytrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_qsytrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              if( n==0_ilp ) then
                 ...
                 call stdlib_wsytrf( uplo, n, a, lda, ipiv, work, -1_ilp, info )
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_wsytrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_wsytrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_wsytrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp ) = lwkopt
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_qlacpy( uplo, n, n, a, lda, af, ldaf )
              call stdlib_qsytrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_qlansy( 'I', uplo, n, a, lda, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_qsycon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, iwork,info )
           ! compute the solution vectors x.
           call stdlib_qlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_qsytrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_qsyrfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, iwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_qlamch( 'EPSILON' ) )info = n + 1_ilp
           work( 1_ilp ) = lwkopt
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_wlacpy( uplo, n, n, a, lda, af, ldaf )
              call stdlib_wsytrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_wlansy( 'I', uplo, n, a, lda, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_wsycon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_wsytrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_wsyrfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_qlamch( 'EPSILON' ) )info = n + 1_ilp
           work( 1_ilp ) = lwkopt
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              if( n==0_ilp ) then
                 ...
                 call stdlib_qsytrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp, info )
                 lwkopt = work(1_ilp)
              end if
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = p*u*d*(u**t)*(p**t) or
           ! a = p*u*d*(u**t)*(p**t).
           call stdlib_qsytrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_qsytrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              if( n==0_ilp ) then
                 ...
                 call stdlib_wsytrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp, info )
                 lwkopt = real( work(1_ilp),KIND=qp)
              end if
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = p*u*d*(u**t)*(p**t) or
           ! a = p*u*d*(u**t)*(p**t).
           call stdlib_wsytrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_wsytrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              if( n==0_ilp ) then
                 ...
                 call stdlib_qsytrf_rook( uplo, n, a, lda, ipiv, work, -1_ilp, info )
                 lwkopt = work(1_ilp)
              end if
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_qsytrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs_rook ( use level 2 blas)
              call stdlib_qsytrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              if( n==0_ilp ) then
                 ...
                 call stdlib_wsytrf_rook( uplo, n, a, lda, ipiv, work, -1_ilp, info )
                 lwkopt = real( work(1_ilp),KIND=qp)
              end if
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_wsytrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs_rook ( use level 2 blas)
              call stdlib_wsytrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp ) = lwkopt
           return
     ...
     !! used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_whetrf( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              if ( lwork<n ) then
              ! solve with trs ( use level blas 2)
                 call stdlib_whetrs( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
              else
              ! solve with trs2 ( use level blas 3)
                 call stdlib_whetrs2( uplo,n,nrhs,a,lda,ipiv,b,ldb,work,info )
              end if
           end if
           work( 1_ilp ) = lwkopt
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**h or a = l*d*l**h.
              call stdlib_wlacpy( uplo, n, n, a, lda, af, ldaf )
              call stdlib_whetrf( uplo, n, af, ldaf, ipiv, work, lwork, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_wlanhe( 'I', uplo, n, a, lda, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_whecon( uplo, n, af, ldaf, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_whetrs( uplo, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_wherfs( uplo, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_qlamch( 'EPSILON' ) )info = n + 1_ilp
           work( 1_ilp ) = lwkopt
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              if( n==0_ilp ) then
                 ...
                 call stdlib_whetrf_rk( uplo, n, a, lda, e, ipiv, work, -1_ilp, info )
                 lwkopt = real( work(1_ilp),KIND=qp)
              end if
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = p*u*d*(u**h)*(p**t) or
           ! a = p*u*d*(u**h)*(p**t).
           call stdlib_whetrf_rk( uplo, n, a, lda, e, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b with blas3 solver, overwriting b with x.
              call stdlib_whetrs_3( uplo, n, nrhs, a, lda, e, ipiv, b, ldb, info )
           end if
           work( 1_ilp ) = lwkopt
           return
     ...
     !! of equations A * X = B by calling ZHETRS_ROOK (uses BLAS 2).
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_whetrf_rook( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
              ! solve with trs ( use level blas 2)
              call stdlib_whetrs_rook( uplo, n, nrhs, a, lda, ipiv, b, ldb, info )
           end if
           work( 1_ilp ) = lwkopt
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_qsptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**t or a = l*d*l**t.
           call stdlib_wsptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_qcopy( n*( n+1 ) / 2_ilp, ap, 1_ilp, afp, 1_ilp )
              call stdlib_qsptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_qlansp( 'I', uplo, n, ap, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_qspcon( uplo, n, afp, ipiv, anorm, rcond, work, iwork, info )
           ! compute the solution vectors x.
           call stdlib_qlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_qsptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_qsprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     iwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_qlamch( 'EPSILON' ) )info = n + 1_ilp
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**t or a = l*d*l**t.
              call stdlib_wcopy( n*( n+1 ) / 2_ilp, ap, 1_ilp, afp, 1_ilp )
              call stdlib_wsptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_wlansp( 'I', uplo, n, ap, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_wspcon( uplo, n, afp, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_wsptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_wsprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_qlamch( 'EPSILON' ) )info = n + 1_ilp
           return
     ...
     !! solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the factorization a = u*d*u**h or a = l*d*l**h.
           call stdlib_whptrf( uplo, n, ap, ipiv, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the factorization a = u*d*u**h or a = l*d*l**h.
              call stdlib_wcopy( n*( n+1 ) / 2_ilp, ap, 1_ilp, afp, 1_ilp )
              call stdlib_whptrf( uplo, n, afp, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_wlanhp( 'I', uplo, n, ap, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_whpcon( uplo, n, afp, ipiv, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_whptrs( uplo, n, nrhs, afp, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_whprfs( uplo, n, nrhs, ap, afp, ipiv, b, ldb, x, ldx, ferr,berr, work, &
                     rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_qlamch( 'EPSILON' ) )info = n + 1_ilp
           return
     ...
     !! form of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              call stdlib_qsytrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp, info )
              lwkopt_sytrf = int( work(1_ilp),KIND=ilp)
              call stdlib_qsytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp, info )
              lwkopt_sytrs = int( work(1_ilp),KIND=ilp)
              lwkopt = max( lwkopt_sytrf, lwkopt_sytrs )
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = u**t*t*u or a = l*t*l**t.
           call stdlib_qsytrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
     ...
     !! form of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              call stdlib_wsytrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp, info )
              lwkopt_sytrf = int( work(1_ilp),KIND=ilp)
              call stdlib_wsytrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp, info )
              lwkopt_sytrs = int( work(1_ilp),KIND=ilp)
              lwkopt = max( lwkopt_sytrf, lwkopt_sytrs )
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = u**t*t*u or a = l*t*l**t.
           call stdlib_wsytrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
     ...
     !! of A is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              call stdlib_whetrf_aa( uplo, n, a, lda, ipiv, work, -1_ilp, info )
              lwkopt_hetrf = int( work(1_ilp),KIND=ilp)
              call stdlib_whetrs_aa( uplo, n, nrhs, a, lda, ipiv, b, ldb, work,-1_ilp, info )
              lwkopt_hetrs = int( work(1_ilp),KIND=ilp)
              lwkopt = max( lwkopt_hetrf, lwkopt_hetrs )
              work( 1_ilp ) = lwkopt
           end if
           ...
           ! compute the factorization a = u**h*t*u or a = l*t*l**h.
           call stdlib_whetrf_aa( uplo, n, a, lda, ipiv, work, lwork, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
