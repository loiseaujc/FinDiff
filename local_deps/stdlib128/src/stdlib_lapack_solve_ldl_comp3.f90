  ...
  contains
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call stdlib_whetrs( uplo, n, 1_ilp, a, lda, ipiv, work, n, info )
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm/=zero )rcond = ( one / ainvnm ) / anorm
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( k>nb ) then
                 ! factorize columns k-kb+1:k of a and use blocked code to
                 ! update columns 1:k-kb
                 call stdlib_wlahef( uplo, k, nb, kb, a, lda, ipiv, work, n, iinfo )
              else
                 ! use unblocked code to factorize columns 1:k of a
                 call stdlib_whetf2( uplo, k, a, lda, ipiv, iinfo )
                 kb = k
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp .and. iinfo>0_ilp )info = iinfo
              ...
              if( k<=n-nb ) then
                 ! factorize columns k:k+kb-1 of a and use blocked code to
                 ! update columns k+kb:n
                 call stdlib_wlahef( uplo, n-k+1, nb, kb, a( k, k ), lda, ipiv( k ),work, n, &
                           iinfo )
              else
                 ! use unblocked code to factorize columns k:n of a
                 call stdlib_whetf2( uplo, n-k+1, a( k, k ), lda, ipiv( k ), iinfo )
                 kb = n - k + 1_ilp
              end if
              ! set info on the first occurrence of a zero pivot
              if( info==0_ilp .and. iinfo>0_ilp )info = iinfo + k - 1_ilp
     ...
     !! A22 (if UPLO = 'L').
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( uplo, 'U' ) ) then
              ...
              kstep = 1_ilp
              ! copy column k of a to column kw of w and update it
              call stdlib_wcopy( k-1, a( 1_ilp, k ), 1_ilp, w( 1_ilp, kw ), 1_ilp )
              ...
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( absakk>=alpha*colmax ) then
                    ...
                    ! begin pivot search along imax row
                    ! copy column imax to column kw-1 of w and update it
                    call stdlib_wcopy( imax-1, a( 1_ilp, imax ), 1_ilp, w( 1_ilp, kw-1 ), 1_ilp )
                    ...
                    if( k<n ) then
                       call stdlib_wgemv( 'NO TRANSPOSE', k, n-k, -cone,a( 1_ilp, k+1 ), lda, w( imax,&
                                  kw+1 ), ldw,cone, w( 1_ilp, kw-1 ), 1_ilp )
           ...
           if( upper ) then
              ...
              if( (max( absakk, colmax )==zero) .or. stdlib_qisnan(absakk) ) then
                 ...
                 if( kstep==1_ilp ) then
                    ...
                    ! a := a - u(k)*d(k)*u(k)**h = a - w(k)*1/d(k)*w(k)**h
                    r1 = one / real( a( k, k ),KIND=qp)
                    call stdlib_wher( uplo, k-1, -r1, a( 1_ilp, k ), 1_ilp, a, lda )
                    ! store u(k) in column k
                    call stdlib_wdscal( k-1, r1, a( 1_ilp, k ), 1_ilp )
                 else
                    ! 2-by-2 pivot block d(k): columns k and k-1 now hold
              ...
              if( (max( absakk, colmax )==zero) .or. stdlib_qisnan(absakk) ) then
                 ...
                 if( kstep==1_ilp ) then
                    ...
                    if( k<n ) then
                       ...
                       ! a := a - l(k)*d(k)*l(k)**h = a - w(k)*(1/d(k))*w(k)**h
                       r1 = one / real( a( k, k ),KIND=qp)
                       call stdlib_wher( uplo, n-k, -r1, a( k+1, k ), 1_ilp,a( k+1, k+1 ), lda )
     ...
     !! provides error bounds and backward error estimates for the solution.
        ...
        ! =====================================================================
           ...
           loop_140: do j = 1, nrhs
              ...
              call stdlib_whemv( uplo, n, -cone, a, lda, x( 1_ilp, j ), 1_ilp, cone, work, 1_ilp )
              ! compute componentwise relative backward error from formula
              ! max(i) ( abs(r(i)) / ( abs(a)*abs(x) + abs(b) )(i) )
              ! where abs(z) is the componentwise absolute value of the matrix
              ! or vector z.  if the i-th component of the denominator is less
              ...
              if( berr( j )>eps .and. two*berr( j )<=lstres .and.count<=itmax ) then
                 ! update solution and try again.
                 call stdlib_whetrs( uplo, n, 1_ilp, af, ldaf, ipiv, work, n, info )
     ...
     !! A = L*D*L**H computed by ZHETRF and converted by ZSYCONV.
        ...
        ! compute d \ b -> b   [ d \ (u \p**t * b) ]
              ...
              do while ( i >= 1 )
                 if( ipiv(i) > 0_ilp ) then
                   ...
                   call stdlib_wdscal( nrhs, s, b( i, 1_ilp ), ldb )
                 elseif ( i > 1_ilp) then
                    if ( ipiv(i-1) == ipiv(i) ) then
                       akm1k = work(i)
                       akm1 = a( i-1, i-1 ) / akm1k
            ...
            ! compute (u**h \ b) -> b   [ u**h \ (d \ (u \p**t * b) ) ]
              call stdlib_wtrsm('L','U','C','U',n,nrhs,cone,a,lda,b,ldb)
             ! p * b  [ p * (u**h \ (d \ (u \p**t * b) )) ]
             k=1_ilp
             do while ( k <= n )
              if( ipiv( k )>0_ilp ) then
        ...
        ! compute (l**h \ b) -> b   [ l**h \ (d \ (l \p**t * b) ) ]
             call stdlib_wtrsm('L','L','C','U',n,nrhs,cone,a,lda,b,ldb)
             ! p * b  [ p * (l**h \ (d \ (l \p**t * b) )) ]
             k=n
             do while ( k >= 1 )
              if( ipiv( k )>0_ilp ) then
     ...
     !! condition number is computed as RCOND = 1 / (ANORM * norm(inv(A))).
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              ! multiply by inv(l*d*l**h) or inv(u*d*u**h).
              call stdlib_whptrs( uplo, n, 1_ilp, ap, ipiv, work, n, info )
              go to 30
           end if
           ! compute the estimate of the reciprocal condition number.
           if( ainvnm/=zero )rcond = ( one / ainvnm ) / anorm
           ...
           if( upper ) then
              ...
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( kp/=kk ) then
                    ...
                    ap( kc+k-1 ) = real( ap( kc+k-1 ),KIND=qp)
                    if( kstep==2_ilp )ap( kc-1 ) = real( ap( kc-1 ),KIND=qp)
                 end if
                 ! update the leading submatrix
                 if( kstep==1_ilp ) then
                    ! 1-by-1 pivot block d(k): column k now holds
                    ...
                    call stdlib_whpr( uplo, k-1, -r1, ap( kc ), 1_ilp, ap )
                    ! store u(k) in column k
                    call stdlib_wdscal( k-1, r1, ap( kc ), 1_ilp )
                 else
                    ! 2-by-2 pivot block d(k): columns k and k-1 now hold
                    ! ( w(k-1) w(k) ) = ( u(k-1) u(k) )*d(k)
                    ! where u(k) and u(k-1) are the k-th and (k-1)-th columns
                    ...
                    ! a := a - ( u(k-1) u(k) )*d(k)*( u(k-1) u(k) )**h
                       ! = a - ( w(k-1) w(k) )*inv(d(k))*( w(k-1) w(k) )**h
                    if( k>2_ilp ) then
                       d = stdlib_qlapy2( real( ap( k-1+( k-1 )*k / 2_ilp ),KIND=qp),aimag( ap( k-1+( &
                                 k-1 )*k / 2_ilp ) ) )
                       d22 = real( ap( k-1+( k-2 )*( k-1 ) / 2_ilp ),KIND=qp) / d
                       d11 = real( ap( k+( k-1 )*k / 2_ilp ),KIND=qp) / d
                       tt = one / ( d11*d22-one )
                       d12 = ap( k-1+( k-1 )*k / 2_ilp ) / d
                       d = tt / d
                       do j = k - 2, 1, -1
                          ...
                          ap( j+( k-1 )*k / 2_ilp ) = wk
                          ap( j+( k-2 )*( k-1 ) / 2_ilp ) = wkm1
                          ap( j+( j-1 )*j / 2_ilp ) = cmplx( real( ap( j+( j-1 )*j / 2_ilp ),KIND=qp), &
                                    zero,KIND=qp)
                       end do
                    end if
                 end if
              ...
              if( max( absakk, colmax )==zero ) then
                 ...
                 if( kp/=kk ) then
                    ...
                    ap( kc ) = real( ap( kc ),KIND=qp)
                    if( kstep==2_ilp )ap( knc ) = real( ap( knc ),KIND=qp)
                 end if
                 ! update the trailing submatrix
                 if( kstep==1_ilp ) then
                    ! 1-by-1 pivot block d(k): column k now holds
                    ...
                    if( k<n ) then
                       ...
                       call stdlib_whpr( uplo, n-k, -r1, ap( kc+1 ), 1_ilp,ap( kc+n-k+1 ) )
                       ! store l(k) in column k
                       call stdlib_wdscal( n-k, r1, ap( kc+1 ), 1_ilp )
                    end if
                 else
                    ! 2-by-2 pivot block d(k): columns k and k+1 now hold
                    ! ( w(k) w(k+1) ) = ( l(k) l(k+1) )*d(k)
                    ...
                    if( k<n-1 ) then
                       ...
                       ! where l(k) and l(k+1) are the k-th and (k+1)-th
                       ! columns of l
                       d = stdlib_qlapy2( real( ap( k+1+( k-1 )*( 2_ilp*n-k ) / 2_ilp ),KIND=qp),aimag( &
                                 ap( k+1+( k-1 )*( 2_ilp*n-k ) / 2_ilp ) ) )
                       d11 = real( ap( k+1+k*( 2_ilp*n-k-1 ) / 2_ilp ),KIND=qp) / d
                       d22 = real( ap( k+( k-1 )*( 2_ilp*n-k ) / 2_ilp ),KIND=qp) / d
                       tt = one / ( d11*d22-one )
                       d21 = ap( k+1+( k-1 )*( 2_ilp*n-k ) / 2_ilp ) / d
                       d = tt / d
                       do j = k + 2, n
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in column k of a.
                 call stdlib_wgeru( k-1, nrhs, -cone, ap( kc ), 1_ilp, b( k, 1_ilp ), ldb,b( 1_ilp, 1_ilp ), ldb )
                 ...
                 ! multiply by inv(u(k)), where u(k) is the transformation
                 ! stored in columns k-1 and k of a.
                 call stdlib_wgeru( k-2, nrhs, -cone, ap( kc ), 1_ilp, b( k, 1_ilp ), ldb,b( 1_ilp, 1_ilp ), ldb )
                           
                 call stdlib_wgeru( k-2, nrhs, -cone, ap( kc-( k-1 ) ), 1_ilp,b( k-1, 1_ilp ), ldb, b( 1_ilp, &
                           1_ilp ), ldb )
                 ! multiply by the inverse of the diagonal block.
                 akm1k = ap( kc+k-2 )
                 akm1 = ap( kc-1 ) / akm1k
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 if( k>1_ilp ) then
                    call stdlib_wlacgv( nrhs, b( k, 1_ilp ), ldb )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', k-1, nrhs, -cone, b,ldb, ap( kc ), &
                              1_ilp, cone, b( k, 1_ilp ), ldb )
                    call stdlib_wlacgv( nrhs, b( k, 1_ilp ), ldb )
                 end if
                 ! interchange rows k and ipiv(k).
                 ...
                 if( k>1_ilp ) then
                    ...
                    call stdlib_wlacgv( nrhs, b( k, 1_ilp ), ldb )
                    call stdlib_wlacgv( nrhs, b( k+1, 1_ilp ), ldb )
                    call stdlib_wgemv( 'CONJUGATE TRANSPOSE', k-1, nrhs, -cone, b,ldb, ap( kc+k ),&
                               1_ilp, cone, b( k+1, 1_ilp ), ldb )
                    call stdlib_wlacgv( nrhs, b( k+1, 1_ilp ), ldb )
                 end if
                 ! interchange rows k and -ipiv(k).
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! multiply by inv(l(k)), where l(k) is the transformation
                 ! stored in column k of a.
                 if( k<n )call stdlib_wgeru( n-k, nrhs, -cone, ap( kc+1 ), 1_ilp, b( k, 1_ilp ),ldb, b( k+&
                           1_ilp, 1_ilp ), ldb )
                 ! multiply by the inverse of the diagonal block.
                 ...
                 ! stored in columns k and k+1 of a.
                 if( k<n-1 ) then
                    call stdlib_wgeru( n-k-1, nrhs, -cone, ap( kc+2 ), 1_ilp, b( k, 1_ilp ),ldb, b( k+2, &
                              1_ilp ), ldb )
                    call stdlib_wgeru( n-k-1, nrhs, -cone, ap( kc+n-k+2 ), 1_ilp,b( k+1, 1_ilp ), ldb, b( &
                              k+2, 1_ilp ), ldb )
                 end if
                 ! multiply by the inverse of the diagonal block.
                 akm1k = ap( kc+1 )
        ...
        ! =====================================================================
           ...
           if( upper ) then
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! compute column k of the inverse.
                 if( k>1_ilp ) then
                    call stdlib_wcopy( k-1, ap( kc ), 1_ilp, work, 1_ilp )
                    call stdlib_whpmv( uplo, k-1, -cone, ap, work, 1_ilp, czero,ap( kc ), 1_ilp )
                    ap( kc+k-1 ) = ap( kc+k-1 ) -real( stdlib_wdotc( k-1, work, 1_ilp, ap( kc ), 1_ilp ),&
                              KIND=qp)
                 end if
                 kstep = 1_ilp
              else
                 ...
                 if( k>1_ilp ) then
                    ...
                    ap( kc+k-1 ) = ap( kc+k-1 ) -real( stdlib_wdotc( k-1, work, 1_ilp, ap( kc ), 1_ilp ),&
                              KIND=qp)
                    ap( kcnext+k-1 ) = ap( kcnext+k-1 ) -stdlib_wdotc( k-1, ap( kc ), 1_ilp, ap( &
                              kcnext ),1_ilp )
                    call stdlib_wcopy( k-1, ap( kcnext ), 1_ilp, work, 1_ilp )
                    call stdlib_whpmv( uplo, k-1, -cone, ap, work, 1_ilp, czero,ap( kcnext ), 1_ilp )
                              
                    ap( kcnext+k ) = ap( kcnext+k ) -real( stdlib_wdotc( k-1, work, 1_ilp, ap( kcnext &
                              ),1_ilp ),KIND=qp)
                 end if
                 kstep = 2_ilp
                 kcnext = kcnext + k + 1_ilp
              ...
              if( kp/=k ) then
                 ...
                 ! submatrix a(1:k+1,1:k+1)
                 kpc = ( kp-1 )*kp / 2_ilp + 1_ilp
                 call stdlib_wswap( kp-1, ap( kc ), 1_ilp, ap( kpc ), 1_ilp )
                 kx = kpc + kp - 1_ilp
                 do j = kp + 1, k - 1
                    kx = kx + j - 1_ilp
                    temp = conjg( ap( kc+j-1 ) )
              ...
              if( ipiv( k )>0_ilp ) then
                 ...
                 ! compute column k of the inverse.
                 if( k<n ) then
                    call stdlib_wcopy( n-k, ap( kc+1 ), 1_ilp, work, 1_ilp )
                    call stdlib_whpmv( uplo, n-k, -cone, ap( kc+n-k+1 ), work, 1_ilp,czero, ap( kc+1 )&
                              , 1_ilp )
                    ap( kc ) = ap( kc ) - real( stdlib_wdotc( n-k, work, 1_ilp,ap( kc+1 ), 1_ilp ),&
                              KIND=qp)
                 end if
                 kstep = 1_ilp
              else
                 ...
                 if( k<n ) then
                    call stdlib_wcopy( n-k, ap( kc+1 ), 1_ilp, work, 1_ilp )
                    call stdlib_whpmv( uplo, n-k, -cone, ap( kc+( n-k+1 ) ), work,1_ilp, czero, ap( &
                              kc+1 ), 1_ilp )
                    ap( kc ) = ap( kc ) - real( stdlib_wdotc( n-k, work, 1_ilp,ap( kc+1 ), 1_ilp ),&
                              KIND=qp)
                    ap( kcnext+1 ) = ap( kcnext+1 ) -stdlib_wdotc( n-k, ap( kc+1 ), 1_ilp,ap( kcnext+&
                              2_ilp ), 1_ilp )
                    call stdlib_wcopy( n-k, ap( kcnext+2 ), 1_ilp, work, 1_ilp )
                    call stdlib_whpmv( uplo, n-k, -cone, ap( kc+( n-k+1 ) ), work,1_ilp, czero, ap( &
                              kcnext+2 ), 1_ilp )
                    ap( kcnext ) = ap( kcnext ) -real( stdlib_wdotc( n-k, work, 1_ilp, ap( kcnext+2 ),&
                              1_ilp ),KIND=qp)
                 end if
                 kstep = 2_ilp
                 kcnext = kcnext - ( n-k+3 )
              ...
              if( kp/=k ) then
                 ...
                 ! submatrix a(k-1:n,k-1:n)
                 kpc = npp - ( n-kp+1 )*( n-kp+2 ) / 2_ilp + 1_ilp
                 if( kp<n )call stdlib_wswap( n-kp, ap( kc+kp-k+1 ), 1_ilp, ap( kpc+1 ), 1_ilp )
                 kx = kc + kp - k
                 do j = k + 1, kp - 1
                    kx = kx + n - j + 1_ilp
                    temp = conjg( ap( kc+j-k ) )
