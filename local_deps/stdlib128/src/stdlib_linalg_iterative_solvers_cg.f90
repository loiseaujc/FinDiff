...
contains
    ...
    module subroutine stdlib_solve_cg_kernel_qp(A,b,x,rtol,atol,maxiter,workspace)
        ...
        !-------------------------
        integer :: iter
        real(qp) :: norm_sq, norm_sq_old, norm_sq0
        real(qp) :: alpha, beta, tolsq
        !-------------------------
        iter = 0
        associate(  P  => workspace%tmp(:,1), &
                    R  => workspace%tmp(:,2), &
            ...
            if(associated(workspace%callback)) call workspace%callback(x, norm_sq, iter)
            do while( norm_sq >= tolsq .and. iter < maxiter)
                call A%matvec(P,Ap, alpha= one_qp, beta=zero_qp, op='N') ! Ap = A*P
    ...
    module subroutine stdlib_solve_cg_dense_qp(A,b,x,di,rtol,atol,maxiter,restart,workspace)
        ...
        if(.not.allocated(workspace_%tmp)) allocate( workspace_%tmp(n,stdlib_size_wksp_cg), source = zero_qp )
        !-------------------------
        ! main call to the solver
