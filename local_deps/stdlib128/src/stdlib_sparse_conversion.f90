...
module stdlib_sparse_conversion
    ...
    interface diag
        ...
        module procedure :: dense2diagonal_qp
        module procedure :: coo2diagonal_qp
        module procedure :: csr2diagonal_qp
        module procedure :: csc2diagonal_qp
        module procedure :: ell2diagonal_qp
        ...
        module procedure :: dense2diagonal_cqp
        module procedure :: coo2diagonal_cqp
        module procedure :: csr2diagonal_cqp
        module procedure :: csc2diagonal_cqp
        module procedure :: ell2diagonal_cqp
    end interface
    public :: diag
    ...
    interface from_ijv
        ...
        module procedure :: coo_from_ijv_qp
        module procedure :: csr_from_ijv_qp
        module procedure :: ell_from_ijv_qp
        module procedure :: sellc_from_ijv_qp
        ...
        module procedure :: coo_from_ijv_cqp
        module procedure :: csr_from_ijv_cqp
        module procedure :: ell_from_ijv_cqp
        module procedure :: sellc_from_ijv_cqp
    end interface
    public :: from_ijv
...
contains
    ...
    subroutine dense2coo_qp(dense,COO)
        real(qp), intent(in) :: dense(:,:)
        type(COO_qp_type), intent(out) :: COO
        integer(ilp) :: num_rows, num_cols, nnz
        integer(ilp) :: i, j, idx

        num_rows = size(dense,dim=1)
        num_cols = size(dense,dim=2)
        nnz      = count( abs(dense) > tiny(1._qp) )
        ...
        do i = 1, num_rows
            do j = 1, num_cols
                if(abs(dense(i,j)) < tiny(1._qp)) cycle
                COO%index(1,idx) = i
                COO%index(2,idx) = j
                COO%data(idx) = dense(i,j)
                idx = idx + 1
    ...
    subroutine dense2coo_cqp(dense,COO)
        complex(qp), intent(in) :: dense(:,:)
        type(COO_cqp_type), intent(out) :: COO
        integer(ilp) :: num_rows, num_cols, nnz
        integer(ilp) :: i, j, idx
    ...
    subroutine coo2dense_qp(COO,dense)
        type(COO_qp_type), intent(in) :: COO
        real(qp), allocatable, intent(out) :: dense(:,:)
        integer(ilp) :: idx

        if(.not.allocated(dense)) allocate(dense(COO%nrows,COO%nrows),source=zero_qp)
        do concurrent(idx = 1:COO%nnz)
            dense( COO%index(1,idx) , COO%index(2,idx) ) = COO%data(idx)
        end do
    end subroutine
    ...
    subroutine coo2dense_cqp(COO,dense)
        type(COO_cqp_type), intent(in) :: COO
        complex(qp), allocatable, intent(out) :: dense(:,:)
        integer(ilp) :: idx

        if(.not.allocated(dense)) allocate(dense(COO%nrows,COO%nrows),source=zero_cqp)
        do concurrent(idx = 1:COO%nnz)
            dense( COO%index(1,idx) , COO%index(2,idx) ) = COO%data(idx)
        end do
    end subroutine
    ...
    subroutine coo2csr_qp(COO,CSR)
        type(COO_qp_type), intent(in)  :: COO
        type(CSR_qp_type), intent(out) :: CSR
        integer(ilp) :: i
    ...
    subroutine coo2csr_cqp(COO,CSR)
        type(COO_cqp_type), intent(in)  :: COO
        type(CSR_cqp_type), intent(out) :: CSR
        integer(ilp) :: i
    ...
    subroutine coo2csc_qp(COO,CSC)
        type(COO_qp_type), intent(in)  :: COO
        type(CSC_qp_type), intent(out) :: CSC
        ...
        allocate(data, source = COO%data )
        nnz = COO%nnz
        call sort_coo_unique_qp( temp, data, nnz, COO%nrows, COO%ncols )
    ...
    subroutine coo2csc_cqp(COO,CSC)
        type(COO_cqp_type), intent(in)  :: COO
        type(CSC_cqp_type), intent(out) :: CSC
        ...
        allocate(data, source = COO%data )
        nnz = COO%nnz
        call sort_coo_unique_cqp( temp, data, nnz, COO%nrows, COO%ncols )
    ...
    subroutine csr2dense_qp(CSR,dense)
        type(CSR_qp_type), intent(in) :: CSR
        real(qp), allocatable, intent(out) :: dense(:,:)
        integer(ilp) :: i, j

        if(.not.allocated(dense)) allocate(dense(CSR%nrows,CSR%nrows),source=zero_qp)
        if( CSR%storage == sparse_full) then
            do i = 1, CSR%nrows
                do j = CSR%rowptr(i), CSR%rowptr(i+1)-1
                    dense(i,CSR%col(j)) = CSR%data(j)
    ...
    subroutine csr2dense_cqp(CSR,dense)
        type(CSR_cqp_type), intent(in) :: CSR
        complex(qp), allocatable, intent(out) :: dense(:,:)
        integer(ilp) :: i, j

        if(.not.allocated(dense)) allocate(dense(CSR%nrows,CSR%nrows),source=zero_cqp)
        if( CSR%storage == sparse_full) then
            do i = 1, CSR%nrows
                do j = CSR%rowptr(i), CSR%rowptr(i+1)-1
                    dense(i,CSR%col(j)) = CSR%data(j)
    ...
    subroutine csr2coo_cqp(CSR,COO)
        type(CSR_cqp_type), intent(in)  :: CSR
        type(COO_cqp_type), intent(out) :: COO
        integer(ilp) :: i, j
    ...
    subroutine csc2coo_qp(CSC,COO)
        type(CSC_qp_type), intent(in)  :: CSC
        type(COO_qp_type), intent(out) :: COO
        integer(ilp) :: i, j
        ...
        call sort_coo_unique_qp( COO%index, COO%data, COO%nnz, COO%nrows, COO%ncols )
    end subroutine
    ...
    subroutine csc2coo_cqp(CSC,COO)
        type(CSC_cqp_type), intent(in)  :: CSC
        type(COO_cqp_type), intent(out) :: COO
        integer(ilp) :: i, j
        ...
        call sort_coo_unique_cqp( COO%index, COO%data, COO%nnz, COO%nrows, COO%ncols )
    end subroutine
    ...
    subroutine csr2ell_qp(CSR,ELL,num_nz_rows)
        type(CSR_qp_type), intent(in)  :: CSR
        type(ELL_qp_type), intent(out) :: ELL
        integer, intent(in), optional :: num_nz_rows !! number of non zeros per row
    ...
    subroutine csr2ell_cqp(CSR,ELL,num_nz_rows)
        type(CSR_cqp_type), intent(in)  :: CSR
        type(ELL_cqp_type), intent(out) :: ELL
        integer, intent(in), optional :: num_nz_rows !! number of non zeros per row
    ...
    subroutine csr2sellc_qp(CSR,SELLC,chunk)
        !! csr2sellc: This function enables transfering data from a CSR matrix to a SELL-C matrix
        !! This algorithm was gracefully provided by Ivan Privec and adapted by Jose Alves
        type(CSR_qp_type), intent(in)    :: CSR
        type(SELLC_qp_type), intent(out) :: SELLC
        integer, intent(in), optional :: chunk
    ...
    subroutine csr2sellc_cqp(CSR,SELLC,chunk)
        !! csr2sellc: This function enables transfering data from a CSR matrix to a SELL-C matrix
        !! This algorithm was gracefully provided by Ivan Privec and adapted by Jose Alves
        type(CSR_cqp_type), intent(in)    :: CSR
        type(SELLC_cqp_type), intent(out) :: SELLC
        integer, intent(in), optional :: chunk
        complex(qp), parameter :: zero = zero_cqp
        integer(ilp) :: i, j, num_chunks
    ...
    recursive subroutine quicksort_i_qp(a, b, first, last)
        integer(ilp), intent(inout) :: a(*) !! reference table to sort
        real(qp), intent(inout)  :: b(*) !! secondary real data to sort w.r.t. a
        integer(ilp), intent(in)     :: first, last
        integer(ilp)  :: i, j, x, t
        real(qp) :: d
        ...
        if (first < i-1) call quicksort_i_qp(a, b, first, i-1)
        if (j+1 < last)  call quicksort_i_qp(a, b, j+1, last)
    end subroutine 
    ...
    recursive subroutine quicksort_i_cqp(a, b, first, last)
        integer(ilp), intent(inout) :: a(*) !! reference table to sort
        complex(qp), intent(inout)  :: b(*) !! secondary real data to sort w.r.t. a
        integer(ilp), intent(in)     :: first, last
        integer(ilp)  :: i, j, x, t
        complex(qp) :: d
        ...
        if (first < i-1) call quicksort_i_cqp(a, b, first, i-1)
        if (j+1 < last)  call quicksort_i_cqp(a, b, j+1, last)
    end subroutine 
    ...
    subroutine sort_coo_unique_qp( a, data, n, num_rows, num_cols )
        !! Sort a 2d array in increasing order first by index 1 and then by index 2
        real(qp), intent(inout) :: data(*)
        integer(ilp), intent(inout) :: a(2,*)
        integer(ilp), intent(inout) :: n
        integer(ilp), intent(in) :: num_rows
        integer(ilp), intent(in) :: num_cols
        ...
        do n_i = 1, num_rows
            ...
            adr1 = count_i_aux(n_i)
            dd = adr1-adr0+1
            if(dd>0) call quicksort_i_qp(cols_(adr0),temp(adr0),1,dd)
        end do
        !---------------------------------------------------------
        ! Remove duplicates
        do ed = 1,n
    ...
    subroutine sort_coo_unique_cqp( a, data, n, num_rows, num_cols )
        !! Sort a 2d array in increasing order first by index 1 and then by index 2
        complex(qp), intent(inout) :: data(*)
        integer(ilp), intent(inout) :: a(2,*)
        integer(ilp), intent(inout) :: n
        integer(ilp), intent(in) :: num_rows
        integer(ilp), intent(in) :: num_cols
        ...
        do n_i = 1, num_rows
            ...
            adr1 = count_i_aux(n_i)
            dd = adr1-adr0+1
            if(dd>0) call quicksort_i_cqp(cols_(adr0),temp(adr0),1,dd)
        end do
        !---------------------------------------------------------
        ! Remove duplicates
        do ed = 1,n
    ...
    subroutine coo_from_ijv_qp(COO,row,col,data,nrows,ncols)
        type(COO_qp_type), intent(inout) :: COO
        integer(ilp), intent(in) :: row(:)
        integer(ilp), intent(in) :: col(:)
        real(qp), intent(in), optional :: data(:)
        integer(ilp), intent(in), optional :: nrows
        integer(ilp), intent(in), optional :: ncols
    ...
    subroutine coo_from_ijv_cqp(COO,row,col,data,nrows,ncols)
        type(COO_cqp_type), intent(inout) :: COO
        integer(ilp), intent(in) :: row(:)
        integer(ilp), intent(in) :: col(:)
        complex(qp), intent(in), optional :: data(:)
        integer(ilp), intent(in), optional :: nrows
        integer(ilp), intent(in), optional :: ncols
    ...
    subroutine csr_from_ijv_qp(CSR,row,col,data,nrows,ncols)
        type(CSR_qp_type), intent(inout) :: CSR
        integer(ilp), intent(in) :: row(:)
        integer(ilp), intent(in) :: col(:)
        real(qp), intent(in), optional :: data(:)
        integer(ilp), intent(in), optional :: nrows
    ...
    subroutine csr_from_ijv_cqp(CSR,row,col,data,nrows,ncols)
        type(CSR_cqp_type), intent(inout) :: CSR
        integer(ilp), intent(in) :: row(:)
        integer(ilp), intent(in) :: col(:)
        complex(qp), intent(in), optional :: data(:)
        integer(ilp), intent(in), optional :: nrows
    ...
    subroutine ell_from_ijv_qp(ELL,row,col,data,nrows,ncols,num_nz_rows)
        type(ELL_qp_type), intent(inout) :: ELL
        integer(ilp), intent(in) :: row(:)
        integer(ilp), intent(in) :: col(:)
        real(qp), intent(in), optional :: data(:)
        integer(ilp), intent(in), optional :: nrows
    ...
    subroutine ell_from_ijv_cqp(ELL,row,col,data,nrows,ncols,num_nz_rows)
        type(ELL_cqp_type), intent(inout) :: ELL
        integer(ilp), intent(in) :: row(:)
        integer(ilp), intent(in) :: col(:)
        complex(qp), intent(in), optional :: data(:)
        integer(ilp), intent(in), optional :: nrows
    ...
    subroutine sellc_from_ijv_qp(SELLC,row,col,data,nrows,ncols,chunk)
        type(SELLC_qp_type), intent(inout) :: SELLC
        integer(ilp), intent(in) :: row(:)
        integer(ilp), intent(in) :: col(:)
        real(qp), intent(in), optional :: data(:)
        integer(ilp), intent(in), optional :: nrows
    ...
    subroutine sellc_from_ijv_cqp(SELLC,row,col,data,nrows,ncols,chunk)
        type(SELLC_cqp_type), intent(inout) :: SELLC
        integer(ilp), intent(in) :: row(:)
        integer(ilp), intent(in) :: col(:)
        complex(qp), intent(in), optional :: data(:)
        integer(ilp), intent(in), optional :: nrows
    ...
    subroutine dense2diagonal_qp(dense,diagonal)
        real(qp), intent(in) :: dense(:,:)
        real(qp), intent(inout), allocatable :: diagonal(:)
        integer :: num_rows
        integer :: i
    ...
    subroutine dense2diagonal_cqp(dense,diagonal)
        complex(qp), intent(in) :: dense(:,:)
        complex(qp), intent(inout), allocatable :: diagonal(:)
        integer :: num_rows
        integer :: i
    ...
    subroutine coo2diagonal_qp(COO,diagonal)
        type(COO_qp_type), intent(in) :: COO
        real(qp), intent(inout), allocatable :: diagonal(:)
        integer :: idx
    ...
    subroutine coo2diagonal_cqp(COO,diagonal)
        type(COO_cqp_type), intent(in) :: COO
        complex(qp), intent(inout), allocatable :: diagonal(:)
        integer :: idx
    ...
    subroutine csr2diagonal_qp(CSR,diagonal)
        type(CSR_qp_type), intent(in) :: CSR
        real(qp), intent(inout), allocatable :: diagonal(:)
        integer :: i, j
    ...
    subroutine csr2diagonal_cqp(CSR,diagonal)
        type(CSR_cqp_type), intent(in) :: CSR
        complex(qp), intent(inout), allocatable :: diagonal(:)
        integer :: i, j
    ...
    subroutine csc2diagonal_qp(CSC,diagonal)
        type(CSC_qp_type), intent(in) :: CSC
        real(qp), intent(inout), allocatable :: diagonal(:)
        integer :: i, j
    ...
    subroutine csc2diagonal_cqp(CSC,diagonal)
        type(CSC_cqp_type), intent(in) :: CSC
        complex(qp), intent(inout), allocatable :: diagonal(:)
        integer :: i, j
    ...
    subroutine ell2diagonal_qp(ELL,diagonal)
        type(ELL_qp_type), intent(in) :: ELL
        real(qp), intent(inout), allocatable :: diagonal(:)
        integer :: i, k
    ...
    subroutine ell2diagonal_cqp(ELL,diagonal)
        type(ELL_cqp_type), intent(in) :: ELL
        complex(qp), intent(inout), allocatable :: diagonal(:)
        integer :: i, k
