  ...
  contains
     ...
     !! where  alpha =  x**T*w.
        ...
        ! =====================================================================
           ...
           alpha = stdlib_qdot( j, x, 1_ilp, w, 1_ilp )
           absalp = abs( alpha )
           absgam = abs( gamma )
           absest = abs( sest )
           if( job==1_ilp ) then
     ...
     !! where  alpha =  x**H * w.
        ...
        ! =====================================================================
           ...
           ! Executable Statements 
           eps = stdlib_qlamch( 'EPSILON' )
           alpha = stdlib_wdotc( j, x, 1_ilp, w, 1_ilp )
           absalp = abs( alpha )
           absgam = abs( gamma )
           absest = abs( sest )
           if( job==1_ilp ) then
     ...
     !! (4R) The inverse transformation of (1L).
        ...
        ! =====================================================================
           ...
           if( icompq==0_ilp ) then
              ...
              ! step (1l): apply back the givens rotations performed.
              do i = 1, givptr
                 call stdlib_qrot( nrhs, b( givcol( i, 2_ilp ), 1_ilp ), ldb,b( givcol( i, 1_ilp ), 1_ilp ), ldb, &
                           givnum( i, 2_ilp ),givnum( i, 1_ilp ) )
              end do
              ! step (2l): permute rows of b.
              call stdlib_qcopy( nrhs, b( nlp1, 1_ilp ), ldb, bx( 1_ilp, 1_ilp ), ldbx )
              do i = 2, n
                 call stdlib_qcopy( nrhs, b( perm( i ), 1_ilp ), ldb, bx( i, 1_ilp ), ldbx )
              end do
              ! step (3l): apply the inverse of the left singular vector
              ! matrix to bx.
              if( k==1_ilp ) then
                 call stdlib_qcopy( nrhs, bx, ldbx, b, ldb )
                 if( z( 1_ilp )<zero ) then
                    call stdlib_qscal( nrhs, negone, b, ldb )
                 end if
              else
                 loop_50: do j = 1, k
                    diflj = difl( j )
                    ...
                    do i = 1, j - 1
                       if( ( z( i )==zero ) .or.( poles( i, 2_ilp )==zero ) ) then
                          ...
                          work( i ) = poles( i, 2_ilp )*z( i ) /( stdlib_qlamc3( poles( i, 2_ilp ), dsigj &
                                    )-diflj ) / ( poles( i, 2_ilp )+dj )
                       end if
                    end do
                    do i = j + 1, k
                       if( ( z( i )==zero ) .or.( poles( i, 2_ilp )==zero ) ) then
                          ...
                          work( i ) = poles( i, 2_ilp )*z( i ) /( stdlib_qlamc3( poles( i, 2_ilp ), &
                                    dsigjp )+difrj ) / ( poles( i, 2_ilp )+dj )
                       end if
                    end do
                    work( 1_ilp ) = negone
                    temp = stdlib_qnrm2( k, work, 1_ilp )
                    call stdlib_qgemv( 'T', k, nrhs, one, bx, ldbx, work, 1_ilp, zero,b( j, 1_ilp ), ldb )
                              
                    call stdlib_qlascl( 'G', 0_ilp, 0_ilp, temp, one, 1_ilp, nrhs, b( j, 1_ilp ),ldb, info )
                              
                 end do loop_50
              end if
              ! move the deflated rows of bx to b also.
              if( k<max( m, n ) )call stdlib_qlacpy( 'A', n-k, nrhs, bx( k+1, 1_ilp ), ldbx,b( k+1, 1_ilp &
                        ), ldb )
           else
              ! apply back the right orthogonal transformations.
              ! step (1r): apply back the new right singular vector matrix
              ! to b.
              if( k==1_ilp ) then
                 call stdlib_qcopy( nrhs, b, ldb, bx, ldbx )
              else
                 do j = 1, k
                    dsigj = poles( j, 2_ilp )
                    if( z( j )==zero ) then
                    ...
                    do i = 1, j - 1
                       if( z( j )==zero ) then
                          ...
                          work( i ) = z( j ) / ( stdlib_qlamc3( dsigj, -poles( i+1,2_ilp ) )-difr( i, &
                                    1_ilp ) ) /( dsigj+poles( i, 1_ilp ) ) / difr( i, 2_ilp )
                       end if
                    end do
                    do i = j + 1, k
                       if( z( j )==zero ) then
                          ...
                          work( i ) = z( j ) / ( stdlib_qlamc3( dsigj, -poles( i,2_ilp ) )-difl( i ) )&
                                     /( dsigj+poles( i, 1_ilp ) ) / difr( i, 2_ilp )
                       end if
                    end do
                    call stdlib_qgemv( 'T', k, nrhs, one, b, ldb, work, 1_ilp, zero,bx( j, 1_ilp ), ldbx )
              ...
              ! related to the right null space of the subproblem.
              if( sqre==1_ilp ) then
                 call stdlib_qcopy( nrhs, b( m, 1_ilp ), ldb, bx( m, 1_ilp ), ldbx )
                 call stdlib_qrot( nrhs, bx( 1_ilp, 1_ilp ), ldbx, bx( m, 1_ilp ), ldbx, c, s )
              end if
              if( k<max( m, n ) )call stdlib_qlacpy( 'A', n-k, nrhs, b( k+1, 1_ilp ), ldb, bx( k+1, 1_ilp &
                        ),ldbx )
              ! step (3r): permute rows of b.
              call stdlib_qcopy( nrhs, bx( 1_ilp, 1_ilp ), ldbx, b( nlp1, 1_ilp ), ldb )
              if( sqre==1_ilp ) then
                 call stdlib_qcopy( nrhs, bx( m, 1_ilp ), ldbx, b( m, 1_ilp ), ldb )
              end if
              do i = 2, n
                 call stdlib_qcopy( nrhs, bx( i, 1_ilp ), ldbx, b( perm( i ), 1_ilp ), ldb )
              end do
              ! step (4r): apply back the givens rotations performed.
              do i = givptr, 1, -1
                 call stdlib_qrot( nrhs, b( givcol( i, 2_ilp ), 1_ilp ), ldb,b( givcol( i, 1_ilp ), 1_ilp ), ldb, &
     ...
     !! (4R) The inverse transformation of (1L).
        ...
        ! =====================================================================
           ...
           if( icompq==0_ilp ) then
              ...
              ! step (1l): apply back the givens rotations performed.
              do i = 1, givptr
                 call stdlib_wdrot( nrhs, b( givcol( i, 2_ilp ), 1_ilp ), ldb,b( givcol( i, 1_ilp ), 1_ilp ), ldb,&
                            givnum( i, 2_ilp ),givnum( i, 1_ilp ) )
              end do
              ! step (2l): permute rows of b.
              call stdlib_wcopy( nrhs, b( nlp1, 1_ilp ), ldb, bx( 1_ilp, 1_ilp ), ldbx )
              do i = 2, n
                 call stdlib_wcopy( nrhs, b( perm( i ), 1_ilp ), ldb, bx( i, 1_ilp ), ldbx )
              end do
              ! step (3l): apply the inverse of the left singular vector
              ! matrix to bx.
              if( k==1_ilp ) then
                 call stdlib_wcopy( nrhs, bx, ldbx, b, ldb )
                 if( z( 1_ilp )<zero ) then
                    call stdlib_wdscal( nrhs, negone, b, ldb )
                 end if
              else
                 loop_100: do j = 1, k
                    diflj = difl( j )
                    ...
                    do i = 1, j - 1
                       if( ( z( i )==zero ) .or.( poles( i, 2_ilp )==zero ) ) then
                          ...
                          rwork( i ) = poles( i, 2_ilp )*z( i ) /( stdlib_qlamc3( poles( i, 2_ilp ), &
                                    dsigj )-diflj ) / ( poles( i, 2_ilp )+dj )
                       end if
                    end do
                    do i = j + 1, k
                    ...
                    rwork( 1_ilp ) = negone
                    temp = stdlib_qnrm2( k, rwork, 1_ilp )
                    ! since b and bx are complex, the following call to stdlib_qgemv
                    ! is performed in two steps (real and imaginary parts).
                    ! call stdlib_qgemv( 't', k, nrhs, one, bx, ldbx, work, 1, zero,
          ! $                     b( j, 1 ), ldb )
                    i = k + nrhs*2_ilp
                    do jcol = 1, nrhs
                       do jrow = 1, k
                    ...
                    call stdlib_qgemv( 'T', k, nrhs, one, rwork( 1_ilp+k+nrhs*2_ilp ), k,rwork( 1_ilp ), 1_ilp, &
                              zero, rwork( 1_ilp+k ), 1_ilp )
                    i = k + nrhs*2_ilp
                    do jcol = 1, nrhs
                       do jrow = 1, k
                    ...
                    call stdlib_wlascl( 'G', 0_ilp, 0_ilp, temp, one, 1_ilp, nrhs, b( j, 1_ilp ),ldb, info )
                              
                 end do loop_100
              end if
              ! move the deflated rows of bx to b also.
              if( k<max( m, n ) )call stdlib_wlacpy( 'A', n-k, nrhs, bx( k+1, 1_ilp ), ldbx,b( k+1, 1_ilp &
                        ), ldb )
           else
              ! apply back the right orthogonal transformations.
              ! step (1r): apply back the new right singular vector matrix
              ! to b.
              if( k==1_ilp ) then
                 call stdlib_wcopy( nrhs, b, ldb, bx, ldbx )
              else
                 loop_180: do j = 1, k
                    dsigj = poles( j, 2_ilp )
                    if( z( j )==zero ) then
                    ...
                    do i = 1, j - 1
                       if( z( j )==zero ) then
                          ...
                          rwork( i ) = z( j ) / ( stdlib_qlamc3( dsigj, -poles( i+1,2_ilp ) )-difr( i,&
                                     1_ilp ) ) /( dsigj+poles( i, 1_ilp ) ) / difr( i, 2_ilp )
                       end if
                    end do
                    do i = j + 1, k
                       if( z( j )==zero ) then
                          ...
                          rwork( i ) = z( j ) / ( stdlib_qlamc3( dsigj, -poles( i,2_ilp ) )-difl( i ) &
                                    ) /( dsigj+poles( i, 1_ilp ) ) / difr( i, 2_ilp )
                       end if
                    end do
                    ! since b and bx are complex, the following call to stdlib_qgemv
                    ! is performed in two steps (real and imaginary parts).
                    ! call stdlib_qgemv( 't', k, nrhs, one, b, ldb, work, 1, zero,
          ! $                     bx( j, 1 ), ldbx )
                    i = k + nrhs*2_ilp
                    do jcol = 1, nrhs
                       do jrow = 1, k
              ...
              ! related to the right null space of the subproblem.
              if( sqre==1_ilp ) then
                 call stdlib_wcopy( nrhs, b( m, 1_ilp ), ldb, bx( m, 1_ilp ), ldbx )
                 call stdlib_wdrot( nrhs, bx( 1_ilp, 1_ilp ), ldbx, bx( m, 1_ilp ), ldbx, c, s )
              end if
              if( k<max( m, n ) )call stdlib_wlacpy( 'A', n-k, nrhs, b( k+1, 1_ilp ), ldb, bx( k+1, 1_ilp &
                        ),ldbx )
              ! step (3r): permute rows of b.
              call stdlib_wcopy( nrhs, bx( 1_ilp, 1_ilp ), ldbx, b( nlp1, 1_ilp ), ldb )
              if( sqre==1_ilp ) then
                 call stdlib_wcopy( nrhs, bx( m, 1_ilp ), ldbx, b( m, 1_ilp ), ldb )
              end if
              do i = 2, n
                 call stdlib_wcopy( nrhs, bx( i, 1_ilp ), ldbx, b( perm( i ), 1_ilp ), ldb )
              end do
              ! step (4r): apply back the givens rotations performed.
              do i = givptr, 1, -1
                 call stdlib_wdrot( nrhs, b( givcol( i, 2_ilp ), 1_ilp ), ldb,b( givcol( i, 1_ilp ), 1_ilp ), ldb,&
     ...
     !! compact form by DLALSA.
        ...
        ! =====================================================================
           ...
           ndiml = inode + n
           ndimr = ndiml + n
           call stdlib_qlasdt( n, nlvl, nd, iwork( inode ), iwork( ndiml ),iwork( ndimr ), smlsiz &
                     )
           ! the following code applies back the left singular vector factors.
           ! for applying back the right singular vector factors, go to 50.
           if( icompq==1_ilp ) then
           ...
           ! the nodes on the bottom level of the tree were solved
           ! by stdlib_qlasdq. the corresponding left and right singular vector
           ! matrices are in explicit form. first apply back the left
           ! singular vector matrices.
           ndb1 = ( nd+1 ) / 2_ilp
           do i = ndb1, nd
              ...
              nlf = ic - nl
              nrf = ic + 1_ilp
              call stdlib_qgemm( 'T', 'N', nl, nrhs, nl, one, u( nlf, 1_ilp ), ldu,b( nlf, 1_ilp ), ldb, &
                        zero, bx( nlf, 1_ilp ), ldbx )
              call stdlib_qgemm( 'T', 'N', nr, nrhs, nr, one, u( nrf, 1_ilp ), ldu,b( nrf, 1_ilp ), ldb, &
                        zero, bx( nrf, 1_ilp ), ldbx )
           end do
           ! next copy the rows of b that correspond to unchanged rows
           ! in the bidiagonal matrix to bx.
           do i = 1, nd
              ic = iwork( inode+i-1 )
              call stdlib_qcopy( nrhs, b( ic, 1_ilp ), ldb, bx( ic, 1_ilp ), ldbx )
           end do
           ! finally go through the left singular vector matrices of all
           ! the other subproblems bottom-up on the tree.
           j = 2_ilp**nlvl
           ...
           do lvl = nlvl, 1, -1
              ...
              do i = lf, ll
                 ...
                 nrf = ic + 1_ilp
                 j = j - 1_ilp
                 call stdlib_qlals0( icompq, nl, nr, sqre, nrhs, bx( nlf, 1_ilp ), ldbx,b( nlf, 1_ilp ), &
                 ldb, perm( nlf, lvl ),givptr( j ), givcol( nlf, lvl2 ), ldgcol,givnum( nlf, lvl2 &
                 ), ldu, poles( nlf, lvl2 ),difl( nlf, lvl ), difr( nlf, lvl2 ),z( nlf, lvl ), k( &
                           j ), c( j ), s( j ), work,info )
              end do
           ...
           loop_70: do lvl = 1, nlvl
              ...
              do i = ll, lf, -1
                 ...
                 j = j + 1_ilp
                 call stdlib_qlals0( icompq, nl, nr, sqre, nrhs, b( nlf, 1_ilp ), ldb,bx( nlf, 1_ilp ), &
                 ldbx, perm( nlf, lvl ),givptr( j ), givcol( nlf, lvl2 ), ldgcol,givnum( nlf, &
                 lvl2 ), ldu, poles( nlf, lvl2 ),difl( nlf, lvl ), difr( nlf, lvl2 ),z( nlf, lvl )&
                           , k( j ), c( j ), s( j ), work,info )
              end do
           ...
           ! the nodes on the bottom level of the tree were solved
           ! by stdlib_qlasdq. the corresponding right singular vector
           ! matrices are in explicit form. apply them back.
           ndb1 = ( nd+1 ) / 2_ilp
           do i = ndb1, nd
              i1 = i - 1_ilp
              ...
              nlf = ic - nl
              nrf = ic + 1_ilp
              call stdlib_qgemm( 'T', 'N', nlp1, nrhs, nlp1, one, vt( nlf, 1_ilp ), ldu,b( nlf, 1_ilp ), &
                        ldb, zero, bx( nlf, 1_ilp ), ldbx )
              call stdlib_qgemm( 'T', 'N', nrp1, nrhs, nrp1, one, vt( nrf, 1_ilp ), ldu,b( nrf, 1_ilp ), &
                        ldb, zero, bx( nrf, 1_ilp ), ldbx )
           end do
           90 continue
           return
     ...
     !! compact form by ZLALSA.
        ...
        ! =====================================================================
           ...
           loop_130: do i = ndb1, nd
              ...
              nlf = ic - nl
              nrf = ic + 1_ilp
              ! since b and bx are complex, the following call to stdlib_qgemm
              ! is performed in two steps (real and imaginary parts).
              ! call stdlib_qgemm( 't', 'n', nl, nrhs, nl, one, u( nlf, 1 ), ldu,
           ! $               b( nlf, 1 ), ldb, zero, bx( nlf, 1 ), ldbx )
              j = nl*nrhs*2_ilp
              do jcol = 1, nrhs
                 do jrow = nlf, nlf + nl - 1
              ...
              call stdlib_qgemm( 'T', 'N', nl, nrhs, nl, one, u( nlf, 1_ilp ), ldu,rwork( 1_ilp+nl*nrhs*2_ilp &
                        ), nl, zero, rwork( 1_ilp ), nl )
              j = nl*nrhs*2_ilp
              do jcol = 1, nrhs
                 do jrow = nlf, nlf + nl - 1
              ...
              ! since b and bx are complex, the following call to stdlib_qgemm
              ! is performed in two steps (real and imaginary parts).
              ! call stdlib_qgemm( 't', 'n', nr, nrhs, nr, one, u( nrf, 1 ), ldu,
          ! $               b( nrf, 1 ), ldb, zero, bx( nrf, 1 ), ldbx )
              j = nr*nrhs*2_ilp
              do jcol = 1, nrhs
                 do jrow = nrf, nrf + nr - 1
              ...
              call stdlib_qgemm( 'T', 'N', nr, nrhs, nr, one, u( nrf, 1_ilp ), ldu,rwork( 1_ilp+nr*nrhs*2_ilp &
                        ), nr, zero, rwork( 1_ilp ), nr )
              j = nr*nrhs*2_ilp
              do jcol = 1, nrhs
                 do jrow = nrf, nrf + nr - 1
           ...
           do i = 1, nd
              ic = iwork( inode+i-1 )
              call stdlib_wcopy( nrhs, b( ic, 1_ilp ), ldb, bx( ic, 1_ilp ), ldbx )
           end do
           ! finally go through the left singular vector matrices of all
           ! the other subproblems bottom-up on the tree.
           j = 2_ilp**nlvl
           ...
           do lvl = nlvl, 1, -1
              ...
              do i = lf, ll
                 ...
                 nrf = ic + 1_ilp
                 j = j - 1_ilp
                 call stdlib_wlals0( icompq, nl, nr, sqre, nrhs, bx( nlf, 1_ilp ), ldbx,b( nlf, 1_ilp ), &
                 ldb, perm( nlf, lvl ),givptr( j ), givcol( nlf, lvl2 ), ldgcol,givnum( nlf, lvl2 &
                 ), ldu, poles( nlf, lvl2 ),difl( nlf, lvl ), difr( nlf, lvl2 ),z( nlf, lvl ), k( &
                           j ), c( j ), s( j ), rwork,info )
              end do
           ...
           loop_190: do lvl = 1, nlvl
              ...
              do i = ll, lf, -1
                 ...
                 j = j + 1_ilp
                 call stdlib_wlals0( icompq, nl, nr, sqre, nrhs, b( nlf, 1_ilp ), ldb,bx( nlf, 1_ilp ), &
                 ldbx, perm( nlf, lvl ),givptr( j ), givcol( nlf, lvl2 ), ldgcol,givnum( nlf, &
                 lvl2 ), ldu, poles( nlf, lvl2 ),difl( nlf, lvl ), difr( nlf, lvl2 ),z( nlf, lvl )&
                           , k( j ), c( j ), s( j ), rwork,info )
              end do
           ...
           loop_320: do i = ndb1, nd
              ...
              nlf = ic - nl
              nrf = ic + 1_ilp
              ! since b and bx are complex, the following call to stdlib_qgemm is
              ! performed in two steps (real and imaginary parts).
              ! call stdlib_qgemm( 't', 'n', nlp1, nrhs, nlp1, one, vt( nlf, 1 ), ldu,
          ! $               b( nlf, 1 ), ldb, zero, bx( nlf, 1 ), ldbx )
              j = nlp1*nrhs*2_ilp
              do jcol = 1, nrhs
                 do jrow = nlf, nlf + nlp1 - 1
              ...
              call stdlib_qgemm( 'T', 'N', nlp1, nrhs, nlp1, one, vt( nlf, 1_ilp ), ldu,rwork( 1_ilp+&
                        nlp1*nrhs*2_ilp ), nlp1, zero, rwork( 1_ilp ),nlp1 )
              j = nlp1*nrhs*2_ilp
              do jcol = 1, nrhs
                 do jrow = nlf, nlf + nlp1 - 1
              ...
              ! since b and bx are complex, the following call to stdlib_qgemm is
              ! performed in two steps (real and imaginary parts).
              ! call stdlib_qgemm( 't', 'n', nrp1, nrhs, nrp1, one, vt( nrf, 1 ), ldu,
          ! $               b( nrf, 1 ), ldb, zero, bx( nrf, 1 ), ldbx )
              j = nrp1*nrhs*2_ilp
              do jcol = 1, nrhs
                 do jrow = nrf, nrf + nrp1 - 1
              ...
              call stdlib_qgemm( 'T', 'N', nrp1, nrhs, nrp1, one, vt( nrf, 1_ilp ), ldu,rwork( 1_ilp+&
                        nrp1*nrhs*2_ilp ), nrp1, zero, rwork( 1_ilp ),nrp1 )
              j = nrp1*nrhs*2_ilp
              do jcol = 1, nrhs
                 do jrow = nrf, nrf + nrp1 - 1
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           if( n==0_ilp ) then
              ...
              if( d( 1_ilp )==zero ) then
                 call stdlib_qlaset( 'A', 1_ilp, nrhs, zero, zero, b, ldb )
              else
                 rank = 1_ilp
                 call stdlib_qlascl( 'G', 0_ilp, 0_ilp, d( 1_ilp ), one, 1_ilp, nrhs, b, ldb, info )
                 d( 1_ilp ) = abs( d( 1_ilp ) )
              end if
              return
           end if
           ...
           if( uplo=='L' ) then
              do i = 1, n - 1
                 call stdlib_qlartg( d( i ), e( i ), cs, sn, r )
                 d( i ) = r
                 e( i ) = sn*d( i+1 )
                 d( i+1 ) = cs*d( i+1 )
                 if( nrhs==1_ilp ) then
                    call stdlib_qrot( 1_ilp, b( i, 1_ilp ), 1_ilp, b( i+1, 1_ilp ), 1_ilp, cs, sn )
                 else
                    work( i*2_ilp-1 ) = cs
                    work( i*2_ilp ) = sn
                 end if
              ...
              if( nrhs>1_ilp ) then
                 do i = 1, nrhs
                    do j = 1, n - 1
                       cs = work( j*2_ilp-1 )
                       sn = work( j*2_ilp )
                       call stdlib_qrot( 1_ilp, b( j, i ), 1_ilp, b( j+1, i ), 1_ilp, cs, sn )
                    end do
                 end do
              end if
           end if
           ...
           if( orgnrm==zero ) then
              call stdlib_qlaset( 'A', n, nrhs, zero, zero, b, ldb )
              return
           end if
           call stdlib_qlascl( 'G', 0_ilp, 0_ilp, orgnrm, one, n, 1_ilp, d, n, info )
           call stdlib_qlascl( 'G', 0_ilp, 0_ilp, orgnrm, one, nm1, 1_ilp, e, nm1, info )
           ! if n is smaller than the minimum divide size smlsiz, then solve
           ! the problem with another solver.
           if( n<=smlsiz ) then
              nwork = 1_ilp + n*n
              call stdlib_qlaset( 'A', n, n, zero, one, work, n )
              call stdlib_qlasdq( 'U', 0_ilp, n, n, 0_ilp, nrhs, d, e, work, n, work, n, b,ldb, work( &
                        nwork ), info )
              if( info/=0_ilp ) then
                 return
              end if
              tol = rcnd*abs( d( stdlib_iqamax( n, d, 1_ilp ) ) )
              do i = 1, n
                 if( d( i )<=tol ) then
                    call stdlib_qlaset( 'A', 1_ilp, nrhs, zero, zero, b( i, 1_ilp ), ldb )
                 else
                    call stdlib_qlascl( 'G', 0_ilp, 0_ilp, d( i ), one, 1_ilp, nrhs, b( i, 1_ilp ),ldb, info )
                              
                    rank = rank + 1_ilp
                 end if
              end do
              call stdlib_qgemm( 'T', 'N', n, nrhs, n, one, work, n, b, ldb, zero,work( nwork ), &
                        n )
              call stdlib_qlacpy( 'A', n, nrhs, work( nwork ), n, b, ldb )
              ! unscale.
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, one, orgnrm, n, 1_ilp, d, n, info )
              call stdlib_qlasrt( 'D', n, d, info )
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, orgnrm, one, n, nrhs, b, ldb, info )
              return
           end if
           ! book-keeping and setting up some constants.
           ...
           loop_60: do i = 1, nm1
              if( ( abs( e( i ) )<eps ) .or. ( i==nm1 ) ) then
                 ...
                 if( i<nm1 ) then
                    ...
                    iwork( nsub ) = n
                    iwork( sizei+nsub-1 ) = 1_ilp
                    call stdlib_qcopy( nrhs, b( n, 1_ilp ), ldb, work( bx+nm1 ), n )
                 end if
                 st1 = st - 1_ilp
                 if( nsize==1_ilp ) then
                    ! this is a 1-by-1 subproblem and is not solved
                    ! explicitly.
                    call stdlib_qcopy( nrhs, b( st, 1_ilp ), ldb, work( bx+st1 ), n )
                 else if( nsize<=smlsiz ) then
                    ! this is a small subproblem and is solved by stdlib_qlasdq.
                    call stdlib_qlaset( 'A', nsize, nsize, zero, one,work( vt+st1 ), n )
                    call stdlib_qlasdq( 'U', 0_ilp, nsize, nsize, 0_ilp, nrhs, d( st ),e( st ), work( vt+&
                              st1 ), n, work( nwork ),n, b( st, 1_ilp ), ldb, work( nwork ), info )
                    if( info/=0_ilp ) then
                       return
                    end if
                    call stdlib_qlacpy( 'A', nsize, nrhs, b( st, 1_ilp ), ldb,work( bx+st1 ), n )
                    ...
                    bxst = bx + st1
                    call stdlib_qlalsa( icmpq2, smlsiz, nsize, nrhs, b( st, 1_ilp ),ldb, work( bxst ),&
                     n, work( u+st1 ), n,work( vt+st1 ), iwork( k+st1 ),work( difl+st1 ), work( &
                     difr+st1 ),work( z+st1 ), work( poles+st1 ),iwork( givptr+st1 ), iwork( &
                     givcol+st1 ), n,iwork( perm+st1 ), work( givnum+st1 ),work( c+st1 ), work( s+&
                               st1 ), work( nwork ),iwork( iwk ), info )
           ...
           do i = 1, n
              ...
              ! subproblems were not solved explicitly.
              if( abs( d( i ) )<=tol ) then
                 call stdlib_qlaset( 'A', 1_ilp, nrhs, zero, zero, work( bx+i-1 ), n )
              else
                 rank = rank + 1_ilp
                 call stdlib_qlascl( 'G', 0_ilp, 0_ilp, d( i ), one, 1_ilp, nrhs,work( bx+i-1 ), n, info )
           ...
           do i = 1, nsub
              ...
              bxst = bx + st1
              if( nsize==1_ilp ) then
                 call stdlib_qcopy( nrhs, work( bxst ), n, b( st, 1_ilp ), ldb )
              else if( nsize<=smlsiz ) then
                 call stdlib_qgemm( 'T', 'N', nsize, nrhs, nsize, one,work( vt+st1 ), n, work( &
                           bxst ), n, zero,b( st, 1_ilp ), ldb )
              else
                 call stdlib_qlalsa( icmpq2, smlsiz, nsize, nrhs, work( bxst ), n,b( st, 1_ilp ), ldb,&
                  work( u+st1 ), n,work( vt+st1 ), iwork( k+st1 ),work( difl+st1 ), work( difr+&
                  st1 ),work( z+st1 ), work( poles+st1 ),iwork( givptr+st1 ), iwork( givcol+st1 ),&
                   n,iwork( perm+st1 ), work( givnum+st1 ),work( c+st1 ), work( s+st1 ), work( &
                             nwork ),iwork( iwk ), info )
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           if( n==0_ilp ) then
              ...
              if( d( 1_ilp )==zero ) then
                 call stdlib_wlaset( 'A', 1_ilp, nrhs, czero, czero, b, ldb )
              else
                 rank = 1_ilp
                 call stdlib_wlascl( 'G', 0_ilp, 0_ilp, d( 1_ilp ), one, 1_ilp, nrhs, b, ldb, info )
                 d( 1_ilp ) = abs( d( 1_ilp ) )
              end if
              return
           end if
           ...
           if( uplo=='L' ) then
              do i = 1, n - 1
                 ...
                 d( i+1 ) = cs*d( i+1 )
                 if( nrhs==1_ilp ) then
                    call stdlib_wdrot( 1_ilp, b( i, 1_ilp ), 1_ilp, b( i+1, 1_ilp ), 1_ilp, cs, sn )
                 else
                    rwork( i*2_ilp-1 ) = cs
                    rwork( i*2_ilp ) = sn
                 end if
              ...
              if( nrhs>1_ilp ) then
                 do i = 1, nrhs
                    do j = 1, n - 1
                       cs = rwork( j*2_ilp-1 )
                       sn = rwork( j*2_ilp )
                       call stdlib_wdrot( 1_ilp, b( j, i ), 1_ilp, b( j+1, i ), 1_ilp, cs, sn )
                    end do
                 end do
              end if
           end if
           ...
           orgnrm = stdlib_qlanst( 'M', n, d, e )
           if( orgnrm==zero ) then
              call stdlib_wlaset( 'A', n, nrhs, czero, czero, b, ldb )
              return
           end if
           call stdlib_qlascl( 'G', 0_ilp, 0_ilp, orgnrm, one, n, 1_ilp, d, n, info )
           call stdlib_qlascl( 'G', 0_ilp, 0_ilp, orgnrm, one, nm1, 1_ilp, e, nm1, info )
           ...
           if( n<=smlsiz ) then
              ...
              irwib = irwrb + n*nrhs
              irwb = irwib + n*nrhs
              call stdlib_qlaset( 'A', n, n, zero, one, rwork( irwu ), n )
              call stdlib_qlaset( 'A', n, n, zero, one, rwork( irwvt ), n )
              call stdlib_qlasdq( 'U', 0_ilp, n, n, n, 0_ilp, d, e, rwork( irwvt ), n,rwork( irwu ), n, &
                        rwork( irwwrk ), 1_ilp,rwork( irwwrk ), info )
              if( info/=0_ilp ) then
                 return
              end if
              ! in the real version, b is passed to stdlib_qlasdq and multiplied
              ! internally by q**h. here b is complex and that product is
              ! computed below in two steps (real and imaginary parts).
              j = irwb - 1_ilp
              do jcol = 1, nrhs
              ...
              call stdlib_qgemm( 'T', 'N', n, nrhs, n, one, rwork( irwu ), n,rwork( irwb ), n, &
                        zero, rwork( irwrb ), n )
              j = irwb - 1_ilp
              do jcol = 1, nrhs
                 do jrow = 1, n
              ...
              do i = 1, n
                 if( d( i )<=tol ) then
                    call stdlib_wlaset( 'A', 1_ilp, nrhs, czero, czero, b( i, 1_ilp ), ldb )
                 else
                    call stdlib_wlascl( 'G', 0_ilp, 0_ilp, d( i ), one, 1_ilp, nrhs, b( i, 1_ilp ),ldb, info )
                              
                    rank = rank + 1_ilp
                 end if
              end do
              ! since b is complex, the following call to stdlib_qgemm is performed
              ! in two steps (real and imaginary parts). that is for v * b
              ! (in the real version of the code v**h is stored in work).
              ! call stdlib_qgemm( 't', 'n', n, nrhs, n, one, work, n, b, ldb, zero,
          ! $               work( nwork ), n )
              j = irwb - 1_ilp
              do jcol = 1, nrhs
                 do jrow = 1, n
              ...
              call stdlib_qgemm( 'T', 'N', n, nrhs, n, one, rwork( irwvt ), n,rwork( irwb ), n, &
                        zero, rwork( irwrb ), n )
              j = irwb - 1_ilp
              do jcol = 1, nrhs
                 do jrow = 1, n
              ...
              call stdlib_qlascl( 'G', 0_ilp, 0_ilp, one, orgnrm, n, 1_ilp, d, n, info )
              call stdlib_qlasrt( 'D', n, d, info )
              call stdlib_wlascl( 'G', 0_ilp, 0_ilp, orgnrm, one, n, nrhs, b, ldb, info )
              return
           end if
           ! book-keeping and setting up some constants.
           nlvl = int( log( real( n,KIND=qp) / real( smlsiz+1,KIND=qp) ) / log( two ),KIND=ilp) + &
           ...
           loop_240: do i = 1, nm1
              if( ( abs( e( i ) )<eps ) .or. ( i==nm1 ) ) then
                 ...
                 if( i<nm1 ) then
                    ...
                    iwork( nsub ) = n
                    iwork( sizei+nsub-1 ) = 1_ilp
                    call stdlib_wcopy( nrhs, b( n, 1_ilp ), ldb, work( bx+nm1 ), n )
                 end if
                 st1 = st - 1_ilp
                 if( nsize==1_ilp ) then
                    ! this is a 1-by-1 subproblem and is not solved
                    ! explicitly.
                    call stdlib_wcopy( nrhs, b( st, 1_ilp ), ldb, work( bx+st1 ), n )
                 else if( nsize<=smlsiz ) then
                    ! this is a small subproblem and is solved by stdlib_qlasdq.
                    call stdlib_qlaset( 'A', nsize, nsize, zero, one,rwork( vt+st1 ), n )
                    call stdlib_qlaset( 'A', nsize, nsize, zero, one,rwork( u+st1 ), n )
                    call stdlib_qlasdq( 'U', 0_ilp, nsize, nsize, nsize, 0_ilp, d( st ),e( st ), rwork( &
                    vt+st1 ), n, rwork( u+st1 ),n, rwork( nrwork ), 1_ilp, rwork( nrwork ),info )
                    ...
                    call stdlib_qgemm( 'T', 'N', nsize, nrhs, nsize, one,rwork( u+st1 ), n, rwork(&
                               irwb ), nsize,zero, rwork( irwrb ), nsize )
                    j = irwb - 1_ilp
                    do jcol = 1, nrhs
                       do jrow = st, st + nsize - 1
                    ...
                    call stdlib_wlacpy( 'A', nsize, nrhs, b( st, 1_ilp ), ldb,work( bx+st1 ), n )
                    ...
                    bxst = bx + st1
                    call stdlib_wlalsa( icmpq2, smlsiz, nsize, nrhs, b( st, 1_ilp ),ldb, work( bxst ),&
                     n, rwork( u+st1 ), n,rwork( vt+st1 ), iwork( k+st1 ),rwork( difl+st1 ), &
                     rwork( difr+st1 ),rwork( z+st1 ), rwork( poles+st1 ),iwork( givptr+st1 ), &
                     iwork( givcol+st1 ), n,iwork( perm+st1 ), rwork( givnum+st1 ),rwork( c+st1 ),&
                                rwork( s+st1 ),rwork( nrwork ), iwork( iwk ), info )
           ...
           do i = 1, n
              ...
              ! subproblems were not solved explicitly.
              if( abs( d( i ) )<=tol ) then
                 call stdlib_wlaset( 'A', 1_ilp, nrhs, czero, czero, work( bx+i-1 ), n )
              else
                 rank = rank + 1_ilp
                 call stdlib_wlascl( 'G', 0_ilp, 0_ilp, d( i ), one, 1_ilp, nrhs,work( bx+i-1 ), n, info )
           ...
           loop_320: do i = 1, nsub
              ...
              bxst = bx + st1
              if( nsize==1_ilp ) then
                 call stdlib_wcopy( nrhs, work( bxst ), n, b( st, 1_ilp ), ldb )
              else if( nsize<=smlsiz ) then
                 ! since b and bx are complex, the following call to stdlib_qgemm
                 ! is performed in two steps (real and imaginary parts).
                 ! call stdlib_qgemm( 't', 'n', nsize, nrhs, nsize, one,
          ! $                  rwork( vt+st1 ), n, rwork( bxst ), n, zero,
          ! $                  b( st, 1 ), ldb )
                 j = bxst - n - 1_ilp
                 jreal = irwb - 1_ilp
                 ...
                 call stdlib_qgemm( 'T', 'N', nsize, nrhs, nsize, one,rwork( vt+st1 ), n, rwork( &
                           irwb ), nsize, zero,rwork( irwrb ), nsize )
                 j = bxst - n - 1_ilp
                 jimag = irwb - 1_ilp
                 do jcol = 1, nrhs
                 ...
                 call stdlib_wlalsa( icmpq2, smlsiz, nsize, nrhs, work( bxst ), n,b( st, 1_ilp ), ldb,&
                  rwork( u+st1 ), n,rwork( vt+st1 ), iwork( k+st1 ),rwork( difl+st1 ), rwork( &
                  difr+st1 ),rwork( z+st1 ), rwork( poles+st1 ),iwork( givptr+st1 ), iwork( &
                  givcol+st1 ), n,iwork( perm+st1 ), rwork( givnum+st1 ),rwork( c+st1 ), rwork( s+&
                            st1 ),rwork( nrwork ), iwork( iwk ), info )
