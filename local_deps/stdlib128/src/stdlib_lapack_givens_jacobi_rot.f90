submodule(stdlib_lapack_base) stdlib_lapack_givens_jacobi_rot
  ...
  contains
     ...
     !! Below, wp=>dp stands for quad precision from LA_CONSTANTS module.
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ! february 2021
           use stdlib_blas_constants_qp, only: zero, half, one, czero, safmax, safmin, rtmin, rtmax
        ! Scalar Arguments 
     ...
     !! Below, wp=>dp stands for quad precision from LA_CONSTANTS module.
        ...
        real(qp) :: abssq
        ! Statement Function Definitions 
        abssq( t ) = real( t,KIND=qp)**2_ilp + aimag( t )**2_ilp
        ! Executable Statements 
        if( g == czero ) then
           c = one
           s = czero
     ...
     !! If x(i)=0, then c(i)=0 and s(i) is chosen so that r(i) is real.
        ...
        ! =====================================================================
           ...
           abssq( ff ) = real( ff,KIND=qp)**2_ilp + aimag( ff )**2_ilp
           ! Executable Statements 
           ! if( first ) then
              ! first = .false.
              safmin = stdlib_qlamch( 'S' )
           ...
           loop_60: do i = 1, n
              f = x( ix )
              g = y( iy )
              ! use identical algorithm as in stdlib_wlartg
              scale = max( abs1( f ), abs1( g ) )
              fs = f
              gs = g
              count = 0_ilp
              ...
              if( f2<=max( g2, one )*safmin ) then
                 ...
                 if( f==czero ) then
                    cs = zero
                    r = stdlib_qlapy2( real( g,KIND=qp), aimag( g ) )
                    ! do complex/real division explicitly with two real
                    ! divisions
                    d = stdlib_qlapy2( real( gs,KIND=qp), aimag( gs ) )
                    sn = cmplx( real( gs,KIND=qp) / d, -aimag( gs ) / d,KIND=qp)
                    go to 50
                 end if
                 f2s = stdlib_qlapy2( real( fs,KIND=qp), aimag( fs ) )
                 ! g2 and g2s are accurate
                 ! g2 is at least safmin, and g2s is at least safmn2
                 g2s = sqrt( g2 )
                 ! error in cs from underflow in f2s is at most
