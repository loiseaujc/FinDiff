  ...
  contains
     ...
     !! symmetric tridiagonal matrix using the divide and conquer method.
        ...
        ! =====================================================================
           ...
           loop_70: do i = 0, spm1
              ...
              if( icompq==2_ilp ) then
                 call stdlib_qsteqr( 'I', matsiz, d( submat ), e( submat ),q( submat, submat ), &
                           ldq, work, info )
                 if( info/=0 )go to 130
              else
                 call stdlib_qsteqr( 'I', matsiz, d( submat ), e( submat ),work( iq-1+iwork( &
                           iqptr+curr ) ), matsiz, work,info )
                 if( info/=0 )go to 130
                 if( icompq==1_ilp ) then
                    call stdlib_qgemm( 'N', 'N', qsiz, matsiz, matsiz, one,q( 1_ilp, submat ), ldq, &
                    work( iq-1+iwork( iqptr+curr ) ), matsiz, zero, qstore( 1_ilp, submat ),ldqs )
           ...
           ! merge lower order eigensystems (of size msd2 and matsiz - msd2)
           ! into an eigensystem of size matsiz.
           ! stdlib_qlaed1 is used only for the full eigensystem of a tridiagonal
           ! matrix.
           ! stdlib_qlaed7 handles the cases in which eigenvalues only or eigenvalues
           ! and eigenvectors of a full symmetric matrix (which was reduced to
           ! tridiagonal form) are desired.
                 if( icompq==2_ilp ) then
           ...
           if( icompq==1_ilp ) then
              do i = 1, n
                 j = iwork( indxq+i )
                 work( i ) = d( j )
                 call stdlib_qcopy( qsiz, qstore( 1_ilp, j ), 1_ilp, q( 1_ilp, i ), 1_ilp )
              end do
              call stdlib_qcopy( n, work, 1_ilp, d, 1_ilp )
           else if( icompq==2_ilp ) then
              do i = 1, n
                 j = iwork( indxq+i )
                 work( i ) = d( j )
                 call stdlib_qcopy( n, q( 1_ilp, j ), 1_ilp, work( n*i+1 ), 1_ilp )
              end do
              call stdlib_qcopy( n, work, 1_ilp, d, 1_ilp )
              call stdlib_qlacpy( 'A', n, n, work( n+1 ), n, q, ldq )
           else
              do i = 1, n
                 j = iwork( indxq+i )
                 work( i ) = d( j )
        ...
        ! warning:      n could be as big as qsiz!
          ...
          ! $        then
           ...
           do i = 0, spm1
              ...
              ll = iq - 1_ilp + iwork( iqptr+curr )
              call stdlib_qsteqr( 'I', matsiz, d( submat ), e( submat ),rwork( ll ), matsiz, &
                        rwork, info )
              call stdlib_wlacrm( qsiz, matsiz, q( 1_ilp, submat ), ldq, rwork( ll ),matsiz, qstore( &
                        1_ilp, submat ), ldqs,rwork( iwrem ) )
              iwork( iqptr+curr+1 ) = iwork( iqptr+curr ) + matsiz**2_ilp
              curr = curr + 1_ilp
              if( info>0_ilp ) then
           ...
           ! merge lower order eigensystems (of size msd2 and matsiz - msd2)
           ! into an eigensystem of size matsiz.  stdlib_wlaed7 handles the case
           ! when the eigenvectors of a full or band hermitian matrix (which
           ! was reduced to tridiagonal form) are desired.
           ! i am free to use q as a valuable working space until loop 150.
                 call stdlib_wlaed7( matsiz, msd2, qsiz, tlvls, curlvl, curprb,d( submat ), &
                 qstore( 1_ilp, submat ), ldqs,e( submat+msd2-1 ), iwork( indxq+submat ),rwork( iq ), &
                 iwork( iqptr ), iwork( iprmpt ),iwork( iperm ), iwork( igivpt ),iwork( igivcl ), &
                           rwork( igivnm ),q( 1_ilp, submat ), rwork( iwrem ),iwork( subpbs+1 ), info )
                 if( info>0_ilp ) then
           ...
           do i = 1, n
              j = iwork( indxq+i )
              rwork( i ) = d( j )
              call stdlib_wcopy( qsiz, qstore( 1_ilp, j ), 1_ilp, q( 1_ilp, i ), 1_ilp )
           end do
           call stdlib_qcopy( n, rwork, 1_ilp, d, 1_ilp )
           return
     ...
     !! the overall problem.
        ...
        ! =====================================================================
           ...
           ! the following values are integer pointers which indicate
           ! the portion of the workspace
           ! used by a particular array in stdlib_qlaed2 and stdlib_qlaed3.
           iz = 1_ilp
           idlmda = iz + n
           iw = idlmda + n
           iq2 = iw + n
           ...
           ! form the z-vector which consists of the last row of q_1 and the
           ! first row of q_2.
           call stdlib_qcopy( cutpnt, q( cutpnt, 1_ilp ), ldq, work( iz ), 1_ilp )
           zpp1 = cutpnt + 1_ilp
           call stdlib_qcopy( n-cutpnt, q( zpp1, zpp1 ), ldq, work( iz+cutpnt ), 1_ilp )
           ! deflate eigenvalues.
           call stdlib_qlaed2( k, n, cutpnt, d, q, ldq, indxq, rho, work( iz ),work( idlmda ), &
           work( iw ), work( iq2 ),iwork( indx ), iwork( indxc ), iwork( indxp ),iwork( coltyp ), &
                     info )
           if( info/=0 )go to 20
           ! solve secular equation.
           if( k/=0_ilp ) then
              is = ( iwork( coltyp )+iwork( coltyp+1 ) )*cutpnt +( iwork( coltyp+1 )+iwork( &
                        coltyp+2 ) )*( n-cutpnt ) + iq2
              call stdlib_qlaed3( k, n, cutpnt, d, q, ldq, rho, work( idlmda ),work( iq2 ), iwork(&
                         indxc ), iwork( coltyp ),work( iw ), work( is ), info )
              if( info/=0 )go to 20
           ! prepare the indxq sorting permutation.
              n1 = k
              n2 = n - k
              call stdlib_qlamrg( n1, n2, d, 1_ilp, -1_ilp, indxq )
           else
              do i = 1, n
                 indxq( i ) = i
              end do
     ...
     !! equation problem is reduced by one.
        ...
        ! =====================================================================
           ...
           n1p1 = n1 + 1_ilp
           if( rho<zero ) then
              call stdlib_qscal( n2, mone, z( n1p1 ), 1_ilp )
           end if
           ! normalize z so that norm(z) = 1.  since z is the concatenation of
           ! two normalized vectors, norm2(z) = sqrt(2).
           t = one / sqrt( two )
           call stdlib_qscal( n, t, z, 1_ilp )
           ! rho = abs( norm(z)**2 * rho )
           rho = abs( two*rho )
           ! sort the eigenvalues into increasing order
           do i = n1p1, n
           ...
           call stdlib_qlamrg( n1, n2, dlamda, 1_ilp, 1_ilp, indxc )
           do i = 1, n
              indx( i ) = indxq( indxc( i ) )
           end do
           ! calculate the allowable deflation tolerance
           imax = stdlib_iqamax( n, z, 1_ilp )
           jmax = stdlib_iqamax( n, d, 1_ilp )
           ...
           if( rho*abs( z( imax ) )<=tol ) then
              ...
              do j = 1, n
                 i = indx( j )
                 call stdlib_qcopy( n, q( 1_ilp, i ), 1_ilp, q2( iq2 ), 1_ilp )
                 dlamda( j ) = d( i )
                 iq2 = iq2 + n
              end do
              call stdlib_qlacpy( 'A', n, n, q2, n, q, ldq )
              call stdlib_qcopy( n, dlamda, 1_ilp, d, 1_ilp )
              go to 190
           end if
           ! if there are multiple eigenvalues then the problem deflates.  here
           ! the number of equal eigenvalues are found.  as each equal
           ...
           if( rho*abs( z( nj ) )<=tol ) then
              ...
              if( abs( t*c*s )<=tol ) then
                 ...
                 if( coltyp( nj )/=coltyp( pj ) )coltyp( nj ) = 2_ilp
                 coltyp( pj ) = 4_ilp
                 call stdlib_qrot( n, q( 1_ilp, pj ), 1_ilp, q( 1_ilp, nj ), 1_ilp, c, s )
                 t = d( pj )*c**2_ilp + d( nj )*s**2_ilp
                 d( nj ) = d( pj )*s**2_ilp + d( nj )*c**2_ilp
                 d( pj ) = t
                 k2 = k2 - 1_ilp
           ...
           do j = 1, ctot( 1 )
              js = indx( i )
              call stdlib_qcopy( n1, q( 1_ilp, js ), 1_ilp, q2( iq1 ), 1_ilp )
              z( i ) = d( js )
              i = i + 1_ilp
              iq1 = iq1 + n1
           end do
           do j = 1, ctot( 2 )
              js = indx( i )
              call stdlib_qcopy( n1, q( 1_ilp, js ), 1_ilp, q2( iq1 ), 1_ilp )
              call stdlib_qcopy( n2, q( n1+1, js ), 1_ilp, q2( iq2 ), 1_ilp )
              z( i ) = d( js )
              i = i + 1_ilp
              iq1 = iq1 + n1
              iq2 = iq2 + n2
           ...
           do j = 1, ctot( 4 )
              js = indx( i )
              call stdlib_qcopy( n, q( 1_ilp, js ), 1_ilp, q2( iq2 ), 1_ilp )
              iq2 = iq2 + n
              z( i ) = d( js )
              i = i + 1_ilp
           end do
           ...
           ! into the last n - k slots of d and q respectively.
           if( k<n ) then
              call stdlib_qlacpy( 'A', n, ctot( 4_ilp ), q2( iq1 ), n,q( 1_ilp, k+1 ), ldq )
              call stdlib_qcopy( n-k, z( k+1 ), 1_ilp, d( k+1 ), 1_ilp )
           end if
           ! copy ctot into coltyp for referencing in stdlib_qlaed3.
           do j = 1, 4
              coltyp( j ) = ctot( j )
           end do
           190 continue
     ...
     !! without guard digits, but we know of none.
        ...
        ! =====================================================================
           ...
           ! this code.
           do i = 1, k
              dlamda( i ) = stdlib_qlamc3( dlamda( i ), dlamda( i ) ) - dlamda( i )
           end do
           do j = 1, k
              call stdlib_qlaed4( k, j, dlamda, w, q( 1_ilp, j ), rho, d( j ), info )
              ! if the zero finder fails, the computation is terminated.
              if( info/=0 )go to 120
           end do
           if( k==1 )go to 110
           ...
           ! compute updated w.
           call stdlib_qcopy( k, w, 1_ilp, s, 1_ilp )
           ! initialize w(i) = q(i,i)
           call stdlib_qcopy( k, q, ldq+1, w, 1_ilp )
           do j = 1, k
              do i = 1, j - 1
                 w( i ) = w( i )*( q( i, j ) / ( dlamda( i )-dlamda( j ) ) )
              end do
           ...
           n12 = ctot( 1_ilp ) + ctot( 2_ilp )
           n23 = ctot( 2_ilp ) + ctot( 3_ilp )
           call stdlib_qlacpy( 'A', n23, k, q( ctot( 1_ilp )+1_ilp, 1_ilp ), ldq, s, n23 )
           iq2 = n1*n12 + 1_ilp
           if( n23/=0_ilp ) then
              call stdlib_qgemm( 'N', 'N', n2, k, n23, one, q2( iq2 ), n2, s, n23,zero, q( n1+1, &
                        1_ilp ), ldq )
           else
              call stdlib_qlaset( 'A', n2, k, zero, zero, q( n1+1, 1_ilp ), ldq )
           end if
           call stdlib_qlacpy( 'A', n12, k, q, ldq, s, n12 )
           if( n12/=0_ilp ) then
              call stdlib_qgemm( 'N', 'N', n1, k, n12, one, q2, n1, s, n12, zero, q,ldq )
           else
              call stdlib_qlaset( 'A', n1, k, zero, zero, q( 1_ilp, 1_ilp ), ldq )
           end if
           120 continue
           return
     ...
     !! secular equation by simpler interpolating rational functions.
        ...
        ! =====================================================================
           ...
           if( n==2_ilp ) then
              call stdlib_qlaed5( i, d, z, delta, rho, dlam )
              return
           end if
           ! compute machine epsilon
           eps = stdlib_qlamch( 'EPSILON' )
           ...
           if( i==n ) then
              ...
              if( .not.swtch3 ) then
                 ...
                 zz( 2_ilp ) = z( ii )*z( ii )
                 call stdlib_qlaed6( niter, orgati, c, delta( iim1 ), zz, w, eta,info )
                 if( info/=0 )go to 250
              end if
              ! note, eta should be positive if w is negative, and
              ! eta should be negative otherwise. however,
     ...
     !! in some extremely rare situations.
        ...
        ! =====================================================================
           ...
           ! Local Arrays 
           real(qp) :: dscale(3_ilp), zscale(3_ilp)
           ! Local Scalars 
           logical(lk) :: scale
           integer(ilp) :: i, iter, niter
     ...
     !! the overall problem.
        ...
        ! =====================================================================
           ...
           ! the following values are for bookkeeping purposes only.  they are
           ! integer pointers which indicate the portion of the workspace
           ! used by a particular array in stdlib_qlaed8 and stdlib_qlaed9.
           if( icompq==1_ilp ) then
              ldq2 = qsiz
           else
              ldq2 = n
           ...
           curr = ptr + curpbm
           call stdlib_qlaeda( n, tlvls, curlvl, curpbm, prmptr, perm, givptr,givcol, givnum, &
                     qstore, qptr, work( iz ),work( iz+n ), info )
           ! when solving the final problem, we no longer need the stored data,
           ! so we will overwrite the data from this level onto the previously
           ! used storage space.
           ...
           ! sort and deflate eigenvalues.
           call stdlib_qlaed8( icompq, k, n, qsiz, d, q, ldq, indxq, rho, cutpnt,work( iz ), work(&
            idlmda ), work( iq2 ), ldq2,work( iw ), perm( prmptr( curr ) ), givptr( curr+1 ),&
            givcol( 1_ilp, givptr( curr ) ),givnum( 1_ilp, givptr( curr ) ), iwork( indxp ),iwork( indx ),&
                       info )
           prmptr( curr+1 ) = prmptr( curr ) + n
           ...
           ! solve secular equation.
           if( k/=0_ilp ) then
              call stdlib_qlaed9( k, 1_ilp, k, n, d, work( is ), k, rho, work( idlmda ),work( iw ), &
                        qstore( qptr( curr ) ), k, info )
              if( info/=0 )go to 30
              if( icompq==1_ilp ) then
                 call stdlib_qgemm( 'N', 'N', qsiz, k, k, one, work( iq2 ), ldq2,qstore( qptr( &
                           curr ) ), k, zero, q, ldq )
              end if
              qptr( curr+1 ) = qptr( curr ) + k**2_ilp
           ! prepare the indxq sorting permutation.
     ...
     !! the overall problem.
        ...
        ! =====================================================================
           ...
           ! solve secular equation.
           if( k/=0_ilp ) then
              call stdlib_qlaed9( k, 1_ilp, k, n, d, rwork( iq ), k, rho,rwork( idlmda ), rwork( iw ),&
                        qstore( qptr( curr ) ), k, info )
              call stdlib_wlacrm( qsiz, k, work, qsiz, qstore( qptr( curr ) ), k, q,ldq, rwork( &
                        iq ) )
              qptr( curr+1 ) = qptr( curr ) + k**2_ilp
              if( info/=0_ilp ) then
                 return
     ...
     !! equation problem is reduced by one.
        ...
        ! =====================================================================
           ...
           i = 1_ilp
           j = cutpnt + 1_ilp
           call stdlib_qlamrg( n1, n2, dlamda, 1_ilp, 1_ilp, indx )
           do i = 1, n
              d( i ) = dlamda( indx( i ) )
              z( i ) = w( indx( i ) )
           end do
           ...
           if( rho*abs( z( imax ) )<=tol ) then
              ...
              if( icompq==0_ilp ) then
                 ...
                 do j = 1, n
                    perm( j ) = indxq( indx( j ) )
                    call stdlib_qcopy( qsiz, q( 1_ilp, perm( j ) ), 1_ilp, q2( 1_ilp, j ), 1_ilp )
                 end do
                 call stdlib_qlacpy( 'A', qsiz, n, q2( 1_ilp, 1_ilp ), ldq2, q( 1_ilp, 1_ilp ),ldq )
              end if
              return
           end if
           ! if there are multiple eigenvalues then the problem deflates.  here
           ...
           if( rho*abs( z( j ) )<=tol ) then
              ...
              if( abs( t*c*s )<=tol ) then
                 ...
                 givnum( 2_ilp, givptr ) = s
                 if( icompq==1_ilp ) then
                    call stdlib_qrot( qsiz, q( 1_ilp, indxq( indx( jlam ) ) ), 1_ilp,q( 1_ilp, indxq( indx( j &
                              ) ) ), 1_ilp, c, s )
                 end if
                 t = d( jlam )*c*c + d( j )*s*s
                 d( j ) = d( jlam )*s*s + d( j )*c*c
           ...
           if( k<n ) then
              if( icompq==0_ilp ) then
                 call stdlib_qcopy( n-k, dlamda( k+1 ), 1_ilp, d( k+1 ), 1_ilp )
              else
                 call stdlib_qcopy( n-k, dlamda( k+1 ), 1_ilp, d( k+1 ), 1_ilp )
                 call stdlib_qlacpy( 'A', qsiz, n-k, q2( 1_ilp, k+1 ), ldq2,q( 1_ilp, k+1 ), ldq )
              end if
           end if
           return
     ...
     !! equation problem is reduced by one.
        ...
        ! =====================================================================
           ...
           if( rho*abs( z( imax ) )<=tol ) then
              ...
              do j = 1, n
                 perm( j ) = indxq( indx( j ) )
                 call stdlib_wcopy( qsiz, q( 1_ilp, perm( j ) ), 1_ilp, q2( 1_ilp, j ), 1_ilp )
              end do
              call stdlib_wlacpy( 'A', qsiz, n, q2( 1_ilp, 1_ilp ), ldq2, q( 1_ilp, 1_ilp ), ldq )
              return
           end if
           ! if there are multiple eigenvalues then the problem deflates.  here
           ! the number of equal eigenvalues are found.  as each equal
           ...
           if( rho*abs( z( j ) )<=tol ) then
              ...
              if( abs( t*c*s )<=tol ) then
                 ...
                 givnum( 1_ilp, givptr ) = c
                 givnum( 2_ilp, givptr ) = s
                 call stdlib_wdrot( qsiz, q( 1_ilp, indxq( indx( jlam ) ) ), 1_ilp,q( 1_ilp, indxq( indx( j ) &
                           ) ), 1_ilp, c, s )
                 t = d( jlam )*c*c + d( j )*s*s
                 d( j ) = d( jlam )*s*s + d( j )*c*c
                 d( jlam ) = t
           ...
           if( k<n ) then
              call stdlib_qcopy( n-k, dlamda( k+1 ), 1_ilp, d( k+1 ), 1_ilp )
              call stdlib_wlacpy( 'A', qsiz, n-k, q2( 1_ilp, k+1 ), ldq2, q( 1_ilp, k+1 ),ldq )
           end if
           return
     ...
     !! eigenvectors for use in calculating the next level of Z vectors.
        ...
        ! =====================================================================
           ...
           do j = 1, k
              ...
              temp = stdlib_qnrm2( k, q( 1_ilp, j ), 1_ilp )
              do i = 1, k
                 s( i, j ) = q( i, j ) / temp
              end do
           end do
