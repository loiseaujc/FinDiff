  ...
  contains
     ...
     !! problem.
        ...
        ! =====================================================================
           ...
           call stdlib_qcopy( bsiz1, q( qptr( curr )+bsiz1-1 ), bsiz1,z( mid-bsiz1 ), 1_ilp )
           call stdlib_qcopy( bsiz2, q( qptr( curr+1 ) ), bsiz2, z( mid ), 1_ilp )
           do k = mid + bsiz2, n
              z( k ) = zero
           end do
           ! loop through remaining levels 1 -> curlvl applying the givens
           ...
           loop_70: do k = 1, curlvl - 1
             ...
             ! apply givens at curr and curr+1
              do i = givptr( curr ), givptr( curr+1 ) - 1
                 call stdlib_qrot( 1_ilp, z( zptr1+givcol( 1_ilp, i )-1_ilp ), 1_ilp,z( zptr1+givcol( 2_ilp, i )-1_ilp ), &
                           1_ilp, givnum( 1_ilp, i ),givnum( 2_ilp, i ) )
              end do
              do i = givptr( curr+1 ), givptr( curr+2 ) - 1
                 call stdlib_qrot( 1_ilp, z( mid-1+givcol( 1_ilp, i ) ), 1_ilp,z( mid-1+givcol( 2_ilp, i ) ), 1_ilp, &
                           givnum( 1_ilp, i ),givnum( 2_ilp, i ) )
              end do
              psiz1 = prmptr( curr+1 ) - prmptr( curr )
              psiz2 = prmptr( curr+2 ) - prmptr( curr+1 )
              ...
              if( bsiz1>0_ilp ) then
                 call stdlib_qgemv( 'T', bsiz1, bsiz1, one, q( qptr( curr ) ),bsiz1, ztemp( 1_ilp ), &
                           1_ilp, zero, z( zptr1 ), 1_ilp )
              end if
              call stdlib_qcopy( psiz1-bsiz1, ztemp( bsiz1+1 ), 1_ilp, z( zptr1+bsiz1 ),1_ilp )
              if( bsiz2>0_ilp ) then
                 call stdlib_qgemv( 'T', bsiz2, bsiz2, one, q( qptr( curr+1 ) ),bsiz2, ztemp( &
                           psiz1+1 ), 1_ilp, zero, z( mid ), 1_ilp )
              end if
              call stdlib_qcopy( psiz2-bsiz2, ztemp( psiz1+bsiz2+1 ), 1_ilp,z( mid+bsiz2 ), 1_ilp )
     ...
     !! semi-widths in the arrays W and WERR respectively.
        ...
        ! =====================================================================
           ...
           loop_100: do ip = 1, olnint
              ...
              prev = i
              ! perform one bisection step
              negcnt = stdlib_qlaneg( n, d, lld, mid, pivmin, r )
              if( negcnt<=i-1 ) then
                 work( k-1 ) = mid
              else
                 work( k ) = mid
     ...
     !! University, July 21, 1966.
        ...
        ! =====================================================================
           ...
           ! [jan/28/2009] remove the line below since spdiam variable not use
           ! spdiam = gu - gl
           ! input arguments for stdlib_qlaebz:
           ! the relative tolerance.  an interval (a,b] lies within
           ! "relative tolerance" if  b-a < reltol*max(|a|,|b|),
           rtoli = reltol
           ! set the absolute tolerance for interval convergence to zero to force
           ...
           if( irange==indrng ) then
              ...
              iwork( 5_ilp ) = il - 1_ilp
              iwork( 6_ilp ) = iu
              call stdlib_qlaebz( 3_ilp, itmax, n, 2_ilp, 2_ilp, nb, atoli, rtoli, pivmin,d, e, e2, iwork( 5_ilp )&
                        , work( n+1 ), work( n+5 ), iout,iwork, w, iblock, iinfo )
              if( iinfo /= 0_ilp ) then
                 info = iinfo
                 return
      ...
      ! *              the uncertainty of eigenvalues of a 2x2 matrix is very small
                 ...
                 ! copy eigenvalues into w and iblock
                 ! use -jblk for block number for unconverged eigenvalues.
                 ! loop over the number of output intervals from stdlib_qlaebz
                 do j = 1, iout
                    ! eigenvalue approximation is middle point of interval
                    tmp1 = half*( work( j+n )+work( j+in+n ) )
                    ! semi length of error interval
     ...
     !! Gerschgorin intervals for the matrices L_i D_i L_i^T.
        ...
        ! =====================================================================
           ...
           spdiam = gu - gl
           ! compute splitting points
           call stdlib_qlarra( n, d, e, e2, spltol, spdiam,nsplit, isplit, iinfo )
           ! can force use of bisection instead of faster dqds.
           ! option left in the code for future multisection work.
           forceb = .false.
           ! initialize usedqd, dqds should be used for allrng unless someone
           ...
           if( (irange==allrng) .and. (.not. forceb) ) then
              ...
              ! we call stdlib_qlarrd to find crude approximations to the eigenvalues
              ! in the desired range. in case irange = indrng, we also obtain the
              ! interval (vl,vu] that contains all the wanted eigenvalues.
              ! an interval [left,right] has converged if
              ! right-left<rtol*max(abs(left),abs(right))
              ! stdlib_qlarrd needs a work of size 4*n, iwork of size 3*n
              call stdlib_qlarrd( range, 'B', n, vl, vu, il, iu, gers,bsrtol, d, e, e2, pivmin, &
                        nsplit, isplit,mm, w, werr, vl, vu, iblock, indexw,work, iwork, iinfo )
              if( iinfo/=0_ilp ) then
                 info = -1_ilp
                 return
      ...
      ! **
           ...
           loop_170: do jblk = 1, nsplit
              ...
              if(( (irange==allrng) .and. (.not. forceb) ).or.usedqd) then
                 ! case of dqds
                 ! find approximations to the extremal eigenvalues of the block
                 call stdlib_qlarrk( in, 1_ilp, gl, gu, d(ibegin),e2(ibegin), pivmin, rtl, tmp, tmp1, &
                           iinfo )
                 if( iinfo/=0_ilp ) then
                    info = -1_ilp
                    return
                 ...
                 isleft = max(gl, tmp - tmp1- hndrd * eps* abs(tmp - tmp1))
                 call stdlib_qlarrk( in, in, gl, gu, d(ibegin),e2(ibegin), pivmin, rtl, tmp, tmp1,&
                            iinfo )
                 if( iinfo/=0_ilp ) then
                    info = -1_ilp
                    return
              ...
              if( ( irange==allrng ) .and. (.not.forceb) ) then
                 ...
                 ! stdlib_qlarrd has computed iblock and indexw for each eigenvalue
                 ! approximation.
                 ! choose sigma
                 if( usedqd ) then
                    s1 = isleft + fourth * spdiam
              ...
              ! compute the negcount at the 1/4 and 3/4 points
              if(mb>1_ilp) then
                 call stdlib_qlarrc( 'T', in, s1, s2, d(ibegin),e(ibegin), pivmin, cnt, cnt1, &
                           cnt2, iinfo)
              endif
              if(mb==1_ilp) then
                 sigma = gl
              ...
              e( iend ) = sigma
              ! store d and l.
              call stdlib_qcopy( in, work, 1_ilp, d( ibegin ), 1_ilp )
              call stdlib_qcopy( in-1, work( in+1 ), 1_ilp, e( ibegin ), 1_ilp )
              if(mb>1_ilp ) then
                 ! perturb each entry of the base representation by a small
                 ! (but random) relative amount to overcome difficulties with
                 ! glued matrices.
                 ...
                 call stdlib_qlarnv(2_ilp, iseed, 2_ilp*in-1, work(1_ilp))
                 do i = 1,in-1
                    d(ibegin+i-1) = d(ibegin+i-1)*(one+eps*pert*work(i))
                    e(ibegin+i-1) = e(ibegin+i-1)*(one+eps*pert*work(in+i))
                 end do
              ...
              ! don't update the gerschgorin intervals because keeping track
              ! of the updates would be too much work in stdlib_qlarrv.
              ! we update w instead and use it to locate the proper gerschgorin
              ! intervals.
              ! compute the required eigenvalues of l d l' by bisection or dqds
              if ( .not.usedqd ) then
                 ! if stdlib_qlarrd has been used, shift the eigenvalue approximations
                 ! according to their representation. this is necessary for
                 ! a uniform stdlib_qlarrv since dqds computes eigenvalues of the
                 ! shifted representation. in stdlib_qlarrv, w will always hold the
                 ! unshifted eigenvalue approximation.
                 do j=wbegin,wend
                    w(j) = w(j) - sigma
                    werr(j) = werr(j) + abs(w(j)) * eps
                 end do
                 ! call stdlib_qlarrb to reduce eigenvalue error of the approximations
                 ! from stdlib_qlarrd
                 do i = ibegin, iend-1
                    work( i ) = d( i ) * e( i )**2_ilp
                 end do
                 ! use bisection to find ev from indl to indu
                 call stdlib_qlarrb(in, d(ibegin), work(ibegin),indl, indu, rtol1, rtol2, indl-1,&
                 w(wbegin), wgap(wbegin), werr(wbegin),work( 2_ilp*n+1 ), iwork, pivmin, spdiam,in, &
                           iinfo )
                 if( iinfo /= 0_ilp ) then
                    info = -4_ilp
                 ...
                 ! stdlib_qlarrb computes all gaps correctly except for the last one
                 ! record distance to vu/gu
                 wgap( wend ) = max( zero,( vu-sigma ) - ( w( wend ) + werr( wend ) ) )
                 do i = indl, indu
                    m = m + 1_ilp
                 ...
                 ! the eigenvalues of t. however, t is not guaranteed to define its
                 ! eigenvalues to high relative accuracy anyway.
                 ! set rtol to the order of the tolerance used in stdlib_qlasq2
                 ! this is an estimated error, the worst case bound is 4*n*eps
                 ! which is usually too large and requires unnecessary work to be
                 ! done by bisection when computing the eigenvectors
                 ...
                 work( 2_ilp*in-1 ) = abs( d( iend ) )
                 work( 2_ilp*in ) = zero
                 call stdlib_qlasq2( in, work, iinfo )
                 if( iinfo /= 0_ilp ) then
                    ! if iinfo = -5 then an index is part of a tight cluster
                    ! and should be changed. the index is in iwork(1) and the
                    ! gap is in work(n+1)
                 ...
                 do i = m - mb + 1, m
                    ! the value of rtol below should be the tolerance in stdlib_qlasq2
                    werr( i ) = rtol * abs( w(i) )
                 end do
                 do i = m - mb + 1, m - 1
                    ! compute the right gap between the intervals
     ...
     !! eigenvalues of L(+) D(+) L(+)^T is relatively isolated.
        ...
        ! =====================================================================
           ...
           fact = real(2_ilp**ktrymax,KIND=qp)
           ...
           sawnan1 = sawnan1 .or.  stdlib_qisnan( max1 )
           if( forcer .or.(max1<=growthbound .and. .not.sawnan1 ) ) then
              sigma = lsigma
              shift = sleft
              goto 100
           ...
           sawnan2 = sawnan2 .or.  stdlib_qisnan( max2 )
           if( forcer .or.(max2<=growthbound .and. .not.sawnan2 ) ) then
              sigma = rsigma
              shift = sright
              goto 100
           ...
           ! refined test for rrr. this test supposes that no nan occurred.
           ! moreover, we use the refined rrr test only for isolated clusters.
           if((clwdth<mingap/real(128_ilp,KIND=qp)) .and.(min(max1,max2)<fail2).and.(.not.sawnan1)&
                     .and.(.not.sawnan2)) then
              dorrr1 = .true.
           else
              dorrr1 = .false.
           ...
           elseif (shift==sright) then
              ! store new l and d back into dplus, lplus
              call stdlib_qcopy( n, work, 1_ilp, dplus, 1_ilp )
              call stdlib_qcopy( n-1, work(n+1), 1_ilp, lplus, 1_ilp )
           endif
           return
     ...
     !! The input eigenvalues should have been computed by DLARRE.
        ...
        ! =====================================================================
           ...
           ! the width of the part of z that is used
           zusedw = zusedu - zusedl + 1_ilp
           call stdlib_qlaset( 'FULL', n, zusedw, zero, zero,z(1_ilp,zusedl), ldz )
           eps = stdlib_qlamch( 'PRECISION' )
           rqtol = two * eps
           ! set expert flags for standard code.
           tryrqc = .true.
           ...
           loop_170: do jblk = 1, iblock( m )
              ...
              ! high relative accuracy is required for the computation of the
              ! corresponding eigenvectors.
              call stdlib_qcopy( im, w( wbegin ), 1_ilp,work( wbegin ), 1_ilp )
              ! we store in w the eigenvalue approximations w.r.t. the original
              ! matrix t.
              do i=1,im
                 w(wbegin+i-1) = w(wbegin+i-1)+sigma
              ...
              if( idone<im ) then
                 ...
                 loop_150: do i = 1, oldncl
                    ...
                    if( ndepth>0_ilp ) then
                       ...
                       call stdlib_qcopy( in, z( ibegin, j ), 1_ilp, d( ibegin ), 1_ilp )
                       call stdlib_qcopy( in-1, z( ibegin, j+1 ), 1_ilp, l( ibegin ),1_ilp )
                       sigma = z( iend, j+1 )
                       ! set the corresponding entries in z to zero
                       call stdlib_qlaset( 'FULL', in, 2_ilp, zero, zero,z( ibegin, j), ldz )
                    end if
                    ! compute dl and dll of current rrr
                    do j = ibegin, iend-1
                       tmp = d( j )*l( j )
                    ...
                    if( ndepth>0_ilp ) then
                       ...
                       ! perform limited bisection (if necessary) to get approximate
                       ! eigenvalues to the precision needed.
                       call stdlib_qlarrb( in, d( ibegin ),work(indlld+ibegin-1),p, q, rtol1, &
                       rtol2, offset,work(wbegin),wgap(wbegin),werr(wbegin),work( indwrk ), iwork(&
                                  iindwk ),pivmin, spdiam, in, iinfo )
                       if( iinfo/=0_ilp ) then
                          info = -1_ilp
                       ...
                       ! of wgap, the entries of work might stem from rrrs with
                       ! different shifts. the gaps from wbegin-1+oldfst to
                       ! wbegin-1+oldlst are correctly computed in stdlib_qlarrb.
                       ! however, we only allow the gaps to become greater since
                       ! this is what should happen when we decrease werr
                       if( oldfst>1_ilp) then
                          wgap( wbegin+oldfst-2 ) =max(wgap(wbegin+oldfst-2),w(wbegin+oldfst-1)-&
                    ...
                    loop_140: do j = oldfst, oldlst
                       ...
                       if( newsiz>1_ilp) then
                          ...
                          do k =1,2
                             ...
                             offset = indexw( wbegin ) - 1_ilp
                             call stdlib_qlarrb( in, d(ibegin),work( indlld+ibegin-1 ),p,p,rqtol, &
                             rqtol, offset,work(wbegin),wgap(wbegin),werr(wbegin),work( indwrk ),&
                                       iwork( iindwk ), pivmin, spdiam,in, iinfo )
                          end do
                          if((wbegin+newlst-1<dol).or.(wbegin+newfst-1>dou)) then
                          ...
                          ! compute rrr of child cluster.
                          ! note that the new rrr is stored in z
                          ! stdlib_qlarrf needs lwork = 2*n
                          call stdlib_qlarrf( in, d( ibegin ), l( ibegin ),work(indld+ibegin-1),&
                          newfst, newlst, work(wbegin),wgap(wbegin), werr(wbegin),spdiam, lgap, &
                          rgap, pivmin, tau,z(ibegin, newftt),z(ibegin, newftt+1),work( indwrk ), &
                                    iinfo )
                          if( iinfo==0_ilp ) then
                             ! a new rrr for the cluster was found by stdlib_qlarrf
                             ! update shift and store it
                             ssigma = sigma + tau
                             z( iend, newftt+1 ) = ssigma
                             ! work() are the midpoints and werr() the semi-width
                          ...
                          if(needbs) then
                             ...
                             itmp1 = iwork( iindr+windex )
                             offset = indexw( wbegin ) - 1_ilp
                             call stdlib_qlarrb( in, d(ibegin),work(indlld+ibegin-1),indeig,&
                             indeig,zero, two*eps, offset,work(wbegin),wgap(wbegin),werr(wbegin),&
                                       work( indwrk ),iwork( iindwk ), pivmin, spdiam,itmp1, iinfo )
                             if( iinfo/=0_ilp ) then
                                info = -3_ilp
                          ...
                          ! given lambda, compute the eigenvector.
                          call stdlib_qlar1v( in, 1_ilp, in, lambda, d( ibegin ),l( ibegin ), work(&
                          indld+ibegin-1),work(indlld+ibegin-1),pivmin, gaptol, z( ibegin, windex &
                          ),.not.usedbs, negcnt, ztz, mingma,iwork( iindr+windex ), isuppz( &
                                    2_ilp*windex-1 ),nrminv, resid, rqcorr, work( indwrk ) )
                          if(iter == 0_ilp) then
             ...
             if(usedrq .and. usedbs .and.bstres<=resid) then
                             ...
                             if (stp2ii) then
                                ! improve error angle by second step
                                call stdlib_qlar1v( in, 1_ilp, in, lambda,d( ibegin ), l( ibegin ),&
                                work(indld+ibegin-1),work(indlld+ibegin-1),pivmin, gaptol, z( &
                                ibegin, windex ),.not.usedbs, negcnt, ztz, mingma,iwork( iindr+&
                                windex ),isuppz( 2_ilp*windex-1 ),nrminv, resid, rqcorr, work( indwrk &
                                          ) )
                          ...
                          call stdlib_qscal( zto-zfrom+1, nrminv,z( zfrom, windex ), 1_ilp )
                          125 continue
                          ! update w
                          w( windex ) = lambda+sigma
                          ! recompute the gaps on the left and right
     ...
     !! The input eigenvalues should have been computed by DLARRE.
        ...
        ! =====================================================================
           ...
           ! the width of the part of z that is used
           zusedw = zusedu - zusedl + 1_ilp
           call stdlib_wlaset( 'FULL', n, zusedw, czero, czero,z(1_ilp,zusedl), ldz )
           eps = stdlib_qlamch( 'PRECISION' )
           rqtol = two * eps
           ! set expert flags for standard code.
           tryrqc = .true.
           ...
           loop_170: do jblk = 1, iblock( m )
              ...
              if( idone<im ) then
                 ...
                 loop_150: do i = 1, oldncl
                    ...
                    if( ndepth>0_ilp ) then
                       ...
                       ! set the corresponding entries in z to zero
                       call stdlib_wlaset( 'FULL', in, 2_ilp, czero, czero,z( ibegin, j), ldz )
                    ...
                    loop_140: do j = oldfst, oldlst
                       ...
                       if( newsiz>1_ilp) then
                          ...
                          ! given lambda, compute the eigenvector.
                          call stdlib_wlar1v( in, 1_ilp, in, lambda, d( ibegin ),l( ibegin ), work(&
                          indld+ibegin-1),work(indlld+ibegin-1),pivmin, gaptol, z( ibegin, windex &
                          ),.not.usedbs, negcnt, ztz, mingma,iwork( iindr+windex ), isuppz( &
                                    2_ilp*windex-1 ),nrminv, resid, rqcorr, work( indwrk ) )
                          if(iter == 0_ilp) then
             ...
             if(usedrq .and. usedbs .and.bstres<=resid) then
                             ...
                             if (stp2ii) then
                                ! improve error angle by second step
                                call stdlib_wlar1v( in, 1_ilp, in, lambda,d( ibegin ), l( ibegin ),&
                                work(indld+ibegin-1),work(indlld+ibegin-1),pivmin, gaptol, z( &
                                ibegin, windex ),.not.usedbs, negcnt, ztz, mingma,iwork( iindr+&
                                windex ),isuppz( 2_ilp*windex-1 ),nrminv, resid, rqcorr, work( indwrk &
                                          ) )
                          ...
                          call stdlib_wdscal( zto-zfrom+1, nrminv,z( zfrom, windex ), 1_ilp )
                          125 continue
                          ! update w
                          w( windex ) = lambda+sigma
                          ! recompute the gaps on the left and right
