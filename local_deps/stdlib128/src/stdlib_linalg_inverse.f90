...
!! Compute inverse of a square matrix    
     ...
     module subroutine stdlib_linalg_invert_inplace_q(a,pivot,err)
         ...
         if (info==0) then
            ...
            lwork = max(1,min(huge(0_ilp),ceiling(1.05_qp*real(n,qp)*nb,kind=ilp)))
     ...
     module subroutine stdlib_linalg_invert_split_q(a,inva,pivot,err)
         ...
         if (any(sa/=sinva)) then
             ...
             !> Compute matrix inverse
             call stdlib_linalg_invert_inplace_q(inva,pivot=pivot,err=err0)      
     ...
     module function stdlib_linalg_inverse_q(a,err) result(inva)
         ...
         !> Compute matrix inverse
         call stdlib_linalg_invert_inplace_q(inva,err=err)
     ...
     module function stdlib_linalg_inverse_q_operator(a) result(inva)
         ...
         ! Provide an error handler to return NaNs on issues
         inva = stdlib_linalg_inverse_q(a,err=err)
         ...
         if (err%error()) then 
            ...
            inva = ieee_value(1.0_qp,ieee_quiet_nan)
         endif
     ...
     module subroutine stdlib_linalg_invert_split_w(a,inva,pivot,err)
         ...
         if (any(sa/=sinva)) then
             ...
             !> Compute matrix inverse
             call stdlib_linalg_invert_inplace_w(inva,pivot=pivot,err=err0)      
     ...
     module function stdlib_linalg_inverse_w(a,err) result(inva)
         ...
         !> Compute matrix inverse
         call stdlib_linalg_invert_inplace_w(inva,err=err)
     ...
     module function stdlib_linalg_inverse_w_operator(a) result(inva)
         ...
         ! Provide an error handler to return NaNs on issues
         inva = stdlib_linalg_inverse_w(a,err=err)
         ...
         if (err%error()) then 
            ...
            inva = cmplx(ieee_value(1.0_qp,ieee_quiet_nan), &
                         ieee_value(1.0_qp,ieee_quiet_nan), kind=qp)
         endif
