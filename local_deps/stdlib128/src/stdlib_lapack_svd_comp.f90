  ...
  contains
     ...
     !! If m >= n, B is upper bidiagonal; if m < n, B is lower bidiagonal.
        ...
        ! =====================================================================
           ...
           do i = 1, minmn - nx, nb
              ...
              ! the matrices x and y which are needed to update the unreduced
              ! part of the matrix
              call stdlib_qlabrd( m-i+1, n-i+1, nb, a( i, i ), lda, d( i ), e( i ),tauq( i ), &
                        taup( i ), work, ldwrkx,work( ldwrkx*nb+1 ), ldwrky )
              ! update the trailing submatrix a(i+nb:m,i+nb:n), using an update
              ! of the form  a := a - v*y**t - x*u**t
              call stdlib_qgemm( 'NO TRANSPOSE', 'TRANSPOSE', m-i-nb+1, n-i-nb+1,nb, -one, a( i+&
                        nb, i ), lda,work( ldwrkx*nb+nb+1 ), ldwrky, one,a( i+nb, i+nb ), lda )
              call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', m-i-nb+1, n-i-nb+1,nb, -one, &
                        work( nb+1 ), ldwrkx, a( i, i+nb ), lda,one, a( i+nb, i+nb ), lda )
              ! copy diagonal and off-diagonal elements of b back into a
              if( m>=n ) then
                 do j = i, i + nb - 1
           ...
           ! use unblocked code to reduce the remainder of the matrix
           call stdlib_qgebd2( m-i+1, n-i+1, a( i, i ), lda, d( i ), e( i ),tauq( i ), taup( i ), &
                     work, iinfo )
           work( 1_ilp ) = ws
           return
     ...
     !! If m >= n, B is upper bidiagonal; if m < n, B is lower bidiagonal.
        ...
        ! =====================================================================
           ...
           do i = 1, minmn - nx, nb
              ...
              ! the matrices x and y which are needed to update the unreduced
              ! part of the matrix
              call stdlib_wlabrd( m-i+1, n-i+1, nb, a( i, i ), lda, d( i ), e( i ),tauq( i ), &
                        taup( i ), work, ldwrkx,work( ldwrkx*nb+1 ), ldwrky )
              ! update the trailing submatrix a(i+ib:m,i+ib:n), using
              ! an update of the form  a := a - v*y**h - x*u**h
           ...
           ! use unblocked code to reduce the remainder of the matrix
           call stdlib_wgebd2( m-i+1, n-i+1, a( i, i ), lda, d( i ), e( i ),tauq( i ), taup( i ), &
                     work, iinfo )
           work( 1_ilp ) = ws
           return
     ...
     !! If m >= n, B is upper bidiagonal; if m < n, B is lower bidiagonal.
        ...
        ! =====================================================================
           ...
           if( m>=n ) then
              ...
              do i = 1, n
                 ! generate elementary reflector h(i) to annihilate a(i+1:m,i)
                 call stdlib_qlarfg( m-i+1, a( i, i ), a( min( i+1, m ), i ), 1_ilp,tauq( i ) )
                           
                 d( i ) = a( i, i )
                 a( i, i ) = one
                 ! apply h(i) to a(i:m,i+1:n) from the left
                 if( i<n )call stdlib_qlarf( 'LEFT', m-i+1, n-i, a( i, i ), 1_ilp, tauq( i ),a( i, i+&
                           1_ilp ), lda, work )
                 a( i, i ) = d( i )
                 if( i<n ) then
                    ! generate elementary reflector g(i) to annihilate
                    ...
                    a( i, i+1 ) = one
                    ! apply g(i) to a(i+1:m,i+1:n) from the right
                    call stdlib_qlarf( 'RIGHT', m-i, n-i, a( i, i+1 ), lda,taup( i ), a( i+1, i+1 &
                              ), lda, work )
                    a( i, i+1 ) = e( i )
                 else
                    taup( i ) = zero
              ...
              do i = 1, m
                 ...
                 a( i, i ) = one
                 ! apply g(i) to a(i+1:m,i:n) from the right
                 if( i<m )call stdlib_qlarf( 'RIGHT', m-i, n-i+1, a( i, i ), lda,taup( i ), a( i+&
                           1_ilp, i ), lda, work )
                 a( i, i ) = d( i )
                 if( i<m ) then
                    ! generate elementary reflector h(i) to annihilate
                    ! a(i+2:m,i)
                    call stdlib_qlarfg( m-i, a( i+1, i ), a( min( i+2, m ), i ), 1_ilp,tauq( i ) )
                              
                    e( i ) = a( i+1, i )
                    a( i+1, i ) = one
                    ! apply h(i) to a(i+1:m,i+1:n) from the left
                    call stdlib_qlarf( 'LEFT', m-i, n-i, a( i+1, i ), 1_ilp, tauq( i ),a( i+1, i+1 ), &
                              lda, work )
                    a( i+1, i ) = e( i )
                 else
                    tauq( i ) = zero
        ...
        ! =====================================================================
           ...
           if( m>=n ) then
              ...
              do i = 1, n
                 ! generate elementary reflector h(i) to annihilate a(i+1:m,i)
                 alpha = a( i, i )
                 call stdlib_wlarfg( m-i+1, alpha, a( min( i+1, m ), i ), 1_ilp,tauq( i ) )
                 d( i ) = real( alpha,KIND=qp)
                 a( i, i ) = cone
                 ! apply h(i)**h to a(i:m,i+1:n) from the left
                 if( i<n )call stdlib_wlarf( 'LEFT', m-i+1, n-i, a( i, i ), 1_ilp,conjg( tauq( i ) ), &
                           a( i, i+1 ), lda, work )
                 a( i, i ) = d( i )
                 if( i<n ) then
                    ! generate elementary reflector g(i) to annihilate
                    ...
                    alpha = a( i, i+1 )
                    call stdlib_wlarfg( n-i, alpha, a( i, min( i+2, n ) ), lda,taup( i ) )
                    e( i ) = real( alpha,KIND=qp)
                    a( i, i+1 ) = cone
                    ! apply g(i) to a(i+1:m,i+1:n) from the right
                    call stdlib_wlarf( 'RIGHT', m-i, n-i, a( i, i+1 ), lda,taup( i ), a( i+1, i+1 &
                              ), lda, work )
                    call stdlib_wlacgv( n-i, a( i, i+1 ), lda )
                    a( i, i+1 ) = e( i )
                 else
              ...
              do i = 1, m
                 ...
                 alpha = a( i, i )
                 call stdlib_wlarfg( n-i+1, alpha, a( i, min( i+1, n ) ), lda,taup( i ) )
                 d( i ) = real( alpha,KIND=qp)
                 a( i, i ) = cone
                 ! apply g(i) to a(i+1:m,i:n) from the right
                 if( i<m )call stdlib_wlarf( 'RIGHT', m-i, n-i+1, a( i, i ), lda,taup( i ), a( i+&
                           1_ilp, i ), lda, work )
                 call stdlib_wlacgv( n-i+1, a( i, i ), lda )
                 a( i, i ) = d( i )
                 if( i<m ) then
                    ...
                    ! a(i+2:m,i)
                    alpha = a( i+1, i )
                    call stdlib_wlarfg( m-i, alpha, a( min( i+2, m ), i ), 1_ilp,tauq( i ) )
                    e( i ) = real( alpha,KIND=qp)
                    a( i+1, i ) = cone
                    ! apply h(i)**h to a(i+1:m,i+1:n) from the left
                    call stdlib_wlarf( 'LEFT', m-i, n-i, a( i+1, i ), 1_ilp,conjg( tauq( i ) ), a( i+&
                              1_ilp, i+1 ), lda,work )
                    a( i+1, i ) = e( i )
                 else
                    tauq( i ) = czero
     ...
     !! Q**T*C for a given matrix C.
        ...
        ! =====================================================================
           ...
           ! initialize q and p**t to the unit matrix, if needed
           if( wantq )call stdlib_qlaset( 'FULL', m, m, zero, one, q, ldq )
           if( wantpt )call stdlib_qlaset( 'FULL', n, n, zero, one, pt, ldpt )
           ! quick return if possible.
           if( m==0 .or. n==0 )return
           minmn = min( m, n )
           if( kl+ku>1_ilp ) then
              ...
              loop_90: do i = 1, minmn
                 ...
                 loop_80: do kk = 1, kb
                    ...
                    ! generate plane rotations to annihilate nonzero elements
                    ! which have been created below the band
                    if( nr>0_ilp )call stdlib_qlargv( nr, ab( klu1, j1-klm-1 ), inca,work( j1 ), kb1, &
                              work( mn+j1 ), kb1 )
                    ! apply plane rotations from the left
                    do l = 1, kb
                       if( j2-klm+l-1>n ) then
                       ...
                       if( nrt>0_ilp )call stdlib_qlartv( nrt, ab( klu1-l, j1-klm+l-1 ), inca,ab( &
                                 klu1-l+1, j1-klm+l-1 ), inca,work( mn+j1 ), work( j1 ), kb1 )
                    end do
                    if( ml>ml0 ) then
                       if( ml<=m-i+1 ) then
                          ! generate plane rotation to annihilate a(i+ml-1,i)
                          ! within the band, and apply rotation from the left
                          call stdlib_qlartg( ab( ku+ml-1, i ), ab( ku+ml, i ),work( mn+i+ml-1 ), &
                                    work( i+ml-1 ),ra )
                          ab( ku+ml-1, i ) = ra
                          if( i<n )call stdlib_qrot( min( ku+ml-2, n-i ),ab( ku+ml-2, i+1 ), ldab-&
                                    1_ilp,ab( ku+ml-1, i+1 ), ldab-1,work( mn+i+ml-1 ), work( i+ml-1 ) )
                       end if
                       nr = nr + 1_ilp
                       j1 = j1 - kb1
                    ...
                    if( wantq ) then
                       ! accumulate product of plane rotations in q
                       do j = j1, j2, kb1
                          call stdlib_qrot( m, q( 1_ilp, j-1 ), 1_ilp, q( 1_ilp, j ), 1_ilp,work( mn+j ), work( j &
                                    ) )
                       end do
                    end if
                    if( wantc ) then
                       ! apply plane rotations to c
                       do j = j1, j2, kb1
                          call stdlib_qrot( ncc, c( j-1, 1_ilp ), ldc, c( j, 1_ilp ), ldc,work( mn+j ), &
                                    work( j ) )
                       end do
                    end if
                    if( j2+kun>n ) then
                    ...
                    ! generate plane rotations to annihilate nonzero elements
                    ! which have been generated above the band
                    if( nr>0_ilp )call stdlib_qlargv( nr, ab( 1_ilp, j1+kun-1 ), inca,work( j1+kun ), kb1,&
                               work( mn+j1+kun ),kb1 )
                    ! apply plane rotations from the right
                    do l = 1, kb
                       if( j2+l-1>m ) then
                       ...
                       if( nrt>0_ilp )call stdlib_qlartv( nrt, ab( l+1, j1+kun-1 ), inca,ab( l, j1+&
                                 kun ), inca,work( mn+j1+kun ), work( j1+kun ),kb1 )
                    end do
                    if( ml==ml0 .and. mu>mu0 ) then
                       if( mu<=n-i+1 ) then
                          ! generate plane rotation to annihilate a(i,i+mu-1)
                          ! within the band, and apply rotation from the right
                          call stdlib_qlartg( ab( ku-mu+3, i+mu-2 ),ab( ku-mu+2, i+mu-1 ),work( &
                                    mn+i+mu-1 ), work( i+mu-1 ),ra )
                          ab( ku-mu+3, i+mu-2 ) = ra
                          call stdlib_qrot( min( kl+mu-2, m-i ),ab( ku-mu+4, i+mu-2 ), 1_ilp,ab( ku-&
                                    mu+3, i+mu-1 ), 1_ilp,work( mn+i+mu-1 ), work( i+mu-1 ) )
                       end if
                       nr = nr + 1_ilp
                       j1 = j1 - kb1
                    ...
                    if( wantpt ) then
                       ! accumulate product of plane rotations in p**t
                       do j = j1, j2, kb1
                          call stdlib_qrot( n, pt( j+kun-1, 1_ilp ), ldpt,pt( j+kun, 1_ilp ), ldpt, work( &
                                    mn+j+kun ),work( j+kun ) )
                       end do
                    end if
                    if( j2+kb>m ) then
           ...
           if( ku==0_ilp .and. kl>0_ilp ) then
              ...
              ! and off-diagonal elements in e
              do i = 1, min( m-1, n )
                 call stdlib_qlartg( ab( 1_ilp, i ), ab( 2_ilp, i ), rc, rs, ra )
                 d( i ) = ra
                 if( i<n ) then
                    e( i ) = rs*ab( 1_ilp, i+1 )
                    ab( 1_ilp, i+1 ) = rc*ab( 1_ilp, i+1 )
                 end if
                 if( wantq )call stdlib_qrot( m, q( 1_ilp, i ), 1_ilp, q( 1_ilp, i+1 ), 1_ilp, rc, rs )
                 if( wantc )call stdlib_qrot( ncc, c( i, 1_ilp ), ldc, c( i+1, 1_ilp ), ldc, rc,rs )
              ...
              if( m<n ) then
                 ...
                 rb = ab( ku, m+1 )
                 do i = m, 1, -1
                    call stdlib_qlartg( ab( ku+1, i ), rb, rc, rs, ra )
                    d( i ) = ra
                    if( i>1_ilp ) then
                       rb = -rs*ab( ku, i )
                       e( i-1 ) = rc*ab( ku, i )
                    end if
                    if( wantpt )call stdlib_qrot( n, pt( i, 1_ilp ), ldpt, pt( m+1, 1_ilp ), ldpt,rc, rs )
     ...
     !! Q**H*C for a given matrix C.
        ...
        ! =====================================================================
           ...
           ! initialize q and p**h to the unit matrix, if needed
           if( wantq )call stdlib_wlaset( 'FULL', m, m, czero, cone, q, ldq )
           if( wantpt )call stdlib_wlaset( 'FULL', n, n, czero, cone, pt, ldpt )
           ! quick return if possible.
           if( m==0 .or. n==0 )return
           minmn = min( m, n )
           if( kl+ku>1_ilp ) then
              ...
              loop_90: do i = 1, minmn
                 ...
                 loop_80: do kk = 1, kb
                    ...
                    ! generate plane rotations to annihilate nonzero elements
                    ! which have been created below the band
                    if( nr>0_ilp )call stdlib_wlargv( nr, ab( klu1, j1-klm-1 ), inca,work( j1 ), kb1, &
                              rwork( j1 ), kb1 )
                    ! apply plane rotations from the left
                    do l = 1, kb
                       if( j2-klm+l-1>n ) then
                       ...
                       if( nrt>0_ilp )call stdlib_wlartv( nrt, ab( klu1-l, j1-klm+l-1 ), inca,ab( &
                                 klu1-l+1, j1-klm+l-1 ), inca,rwork( j1 ), work( j1 ), kb1 )
                    end do
                    if( ml>ml0 ) then
                       if( ml<=m-i+1 ) then
                          ! generate plane rotation to annihilate a(i+ml-1,i)
                          ! within the band, and apply rotation from the left
                          call stdlib_wlartg( ab( ku+ml-1, i ), ab( ku+ml, i ),rwork( i+ml-1 ), &
                                    work( i+ml-1 ), ra )
                          ab( ku+ml-1, i ) = ra
                          if( i<n )call stdlib_wrot( min( ku+ml-2, n-i ),ab( ku+ml-2, i+1 ), ldab-&
                                    1_ilp,ab( ku+ml-1, i+1 ), ldab-1,rwork( i+ml-1 ), work( i+ml-1 ) )
                       end if
                       nr = nr + 1_ilp
                       j1 = j1 - kb1
                    ...
                    if( wantq ) then
                       ! accumulate product of plane rotations in q
                       do j = j1, j2, kb1
                          call stdlib_wrot( m, q( 1_ilp, j-1 ), 1_ilp, q( 1_ilp, j ), 1_ilp,rwork( j ), conjg( &
                                    work( j ) ) )
                       end do
                    end if
                    if( wantc ) then
                       ! apply plane rotations to c
                       do j = j1, j2, kb1
                          call stdlib_wrot( ncc, c( j-1, 1_ilp ), ldc, c( j, 1_ilp ), ldc,rwork( j ), &
                                    work( j ) )
                       end do
                    end if
                    if( j2+kun>n ) then
                    ...
                    ! generate plane rotations to annihilate nonzero elements
                    ! which have been generated above the band
                    if( nr>0_ilp )call stdlib_wlargv( nr, ab( 1_ilp, j1+kun-1 ), inca,work( j1+kun ), kb1,&
                               rwork( j1+kun ),kb1 )
                    ! apply plane rotations from the right
                    do l = 1, kb
                       if( j2+l-1>m ) then
                       ...
                       if( nrt>0_ilp )call stdlib_wlartv( nrt, ab( l+1, j1+kun-1 ), inca,ab( l, j1+&
                                 kun ), inca,rwork( j1+kun ), work( j1+kun ), kb1 )
                    end do
                    if( ml==ml0 .and. mu>mu0 ) then
                       if( mu<=n-i+1 ) then
                          ! generate plane rotation to annihilate a(i,i+mu-1)
                          ! within the band, and apply rotation from the right
                          call stdlib_wlartg( ab( ku-mu+3, i+mu-2 ),ab( ku-mu+2, i+mu-1 ),rwork( &
                                    i+mu-1 ), work( i+mu-1 ), ra )
                          ab( ku-mu+3, i+mu-2 ) = ra
                          call stdlib_wrot( min( kl+mu-2, m-i ),ab( ku-mu+4, i+mu-2 ), 1_ilp,ab( ku-&
                                    mu+3, i+mu-1 ), 1_ilp,rwork( i+mu-1 ), work( i+mu-1 ) )
                       end if
                       nr = nr + 1_ilp
                       j1 = j1 - kb1
                    ...
                    if( wantpt ) then
                       ! accumulate product of plane rotations in p**h
                       do j = j1, j2, kb1
                          call stdlib_wrot( n, pt( j+kun-1, 1_ilp ), ldpt,pt( j+kun, 1_ilp ), ldpt, rwork(&
                                     j+kun ),conjg( work( j+kun ) ) )
                       end do
                    end if
                    if( j2+kb>m ) then
           ...
           if( ku==0_ilp .and. kl>0_ilp ) then
              ...
              ! elements on subdiagonal elements
              do i = 1, min( m-1, n )
                 call stdlib_wlartg( ab( 1_ilp, i ), ab( 2_ilp, i ), rc, rs, ra )
                 ab( 1_ilp, i ) = ra
                 if( i<n ) then
                    ab( 2_ilp, i ) = rs*ab( 1_ilp, i+1 )
                    ab( 1_ilp, i+1 ) = rc*ab( 1_ilp, i+1 )
                 end if
                 if( wantq )call stdlib_wrot( m, q( 1_ilp, i ), 1_ilp, q( 1_ilp, i+1 ), 1_ilp, rc,conjg( rs ) )
                           
                 if( wantc )call stdlib_wrot( ncc, c( i, 1_ilp ), ldc, c( i+1, 1_ilp ), ldc, rc,rs )
              ...
              if( ku>0_ilp .and. m<n ) then
                 ...
                 rb = ab( ku, m+1 )
                 do i = m, 1, -1
                    call stdlib_wlartg( ab( ku+1, i ), rb, rc, rs, ra )
                    ab( ku+1, i ) = ra
                    if( i>1_ilp ) then
                       rb = -conjg( rs )*ab( ku, i )
                       ab( ku, i ) = rc*ab( ku, i )
                    end if
                    if( wantpt )call stdlib_wrot( n, pt( i, 1_ilp ), ldpt, pt( m+1, 1_ilp ), ldpt,rc, &
                              conjg( rs ) )
                 end do
              end if
           end if
           ...
           loop_120: do i = 1, minmn
              ...
              if( wantq )call stdlib_wscal( m, t, q( 1_ilp, i ), 1_ilp )
              if( wantc )call stdlib_wscal( ncc, conjg( t ), c( i, 1_ilp ), ldc )
              if( i<minmn ) then
                 if( ku==0_ilp .and. kl==0_ilp ) then
                    e( i ) = zero
                    t = ab( 1_ilp, i+1 )
                    ...
                    if( wantpt )call stdlib_wscal( n, t, pt( i+1, 1_ilp ), ldpt )
                    t = ab( ku+1, i+1 )*conjg( t )
                 end if
              end if
           end do loop_120
     ...
     !! parameters (marked by [TP]) are available for the implementer.
           ...
           ! .. go go go ...
              ...
              loop_2000: do ibr = 1, nbl
                 ...
                 loop_1002: do ir1 = 0, min( lkahead, nbl-ibr )
                    ...
                    loop_2001: do p = igl, min( igl+kbl-1, n-1 )
           ! .. de rijk's pivoting
                       q = stdlib_iqamax( n-p+1, sva( p ), 1_ilp ) + p - 1_ilp
                       if( p/=q ) then
                          call stdlib_qswap( m, a( 1_ilp, p ), 1_ilp, a( 1_ilp, q ), 1_ilp )
                          if( rsvec )call stdlib_qswap( mvl, v( 1_ilp, p ), 1_ilp,v( 1_ilp, q ), 1_ilp )
                          temp1 = sva( p )
                          sva( p ) = sva( q )
                          sva( q ) = temp1
                          temp1 = d( p )
              ...
              ! norm computation.
              ! caveat:
              ! some blas implementations compute stdlib_qnrm2(m,a(1,p),1)
              ! as sqrt(stdlib_qdot(m,a(1,p),1,a(1,p),1)), which may result in
              ! overflow for ||a(:,p)||_2 > sqrt(overflow_threshold), and
              ! underflow for ||a(:,p)||_2 < sqrt(underflow_threshold).
              ! hence, stdlib_qnrm2 cannot be trusted, not even in the case when
              ! the true norm is far from the under(over)flow boundaries.
              ! if properly implemented stdlib_qnrm2 is available, the if-then-else
              ! below should read "aapp = stdlib_qnrm2( m, a(1,p), 1 ) * d(p)".
                          if( ( sva( p )<rootbig ) .and.( sva( p )>rootsfmin ) ) then
                             sva( p ) = stdlib_qnrm2( m, a( 1_ilp, p ), 1_ilp )*d( p )
                          else
                             temp1 = zero
                             aapp = one
                             call stdlib_qlassq( m, a( 1_ilp, p ), 1_ilp, temp1, aapp )
                             sva( p ) = temp1*sqrt( aapp )*d( p )
                          end if
                          aapp = sva( p )
                       else
                          ...
                          loop_2002: do q = p + 1, min( igl+kbl-1, n )
                             ...
                             if( aaqq>zero ) then
                                ...
                                if( aaqq>=one ) then
                                   rotok = ( small*aapp )<=aaqq
                                   if( aapp<( big / aaqq ) ) then
                                      aapq = ( stdlib_qdot( m, a( 1_ilp, p ), 1_ilp, a( 1_ilp,q ), 1_ilp )*d( p )&
                                                *d( q ) / aaqq )/ aapp
                                   else
                                      call stdlib_qcopy( m, a( 1_ilp, p ), 1_ilp, work, 1_ilp )
                                      call stdlib_qlascl( 'G', 0_ilp, 0_ilp, aapp, d( p ),m, 1_ilp, work, lda,&
                                                 ierr )
                                      aapq = stdlib_qdot( m, work, 1_ilp, a( 1_ilp, q ),1_ilp )*d( q ) / &
                                                aaqq
                                   end if
                                else
                                   rotok = aapp<=( aaqq / small )
                                   if( aapp>( small / aaqq ) ) then
                                      ...
                                      call stdlib_qcopy( m, a( 1_ilp, q ), 1_ilp, work, 1_ilp )
                                      call stdlib_qlascl( 'G', 0_ilp, 0_ilp, aaqq, d( q ),m, 1_ilp, work, lda,&
                                                 ierr )
                                      aapq = stdlib_qdot( m, work, 1_ilp, a( 1_ilp, p ),1_ilp )*d( p ) / &
                                                aapp
                                   end if
                                end if
                                mxaapq = max( mxaapq, abs( aapq ) )
              ! to rotate or not to rotate, that is the question ...
                 ...
                 ! rotated = rotated + one
                                   ...
                                   if( rotok ) then
                                      ...
                                      if( abs( theta )>bigtheta ) then
                                         ...
                                         fastr( 3_ilp ) = t*d( p ) / d( q )
                                         fastr( 4_ilp ) = -t*d( q ) / d( p )
                                         call stdlib_qrotm( m, a( 1_ilp, p ), 1_ilp,a( 1_ilp, q ), 1_ilp, fastr )
                                                   
                                         if( rsvec )call stdlib_qrotm( mvl,v( 1_ilp, p ), 1_ilp,v( 1_ilp, q ),&
                                                    1_ilp,fastr )
                                         sva( q ) = aaqq*sqrt( max( zero,one+t*apoaq*aapq ) )
                       ...
                       ! Choose Correct Signum For Theta And Rotate
                                         ...
                                         if( d( p )>=one ) then
                                            if( d( q )>=one ) then
                                               ...
                                               d( p ) = d( p )*cs
                                               d( q ) = d( q )*cs
                                               call stdlib_qrotm( m, a( 1_ilp, p ), 1_ilp,a( 1_ilp, q ), 1_ilp,&
                                                         fastr )
                                               if( rsvec )call stdlib_qrotm( mvl,v( 1_ilp, p ), 1_ilp, v( &
                                                         1_ilp, q ),1_ilp, fastr )
                                            else
                                               call stdlib_qaxpy( m, -t*aqoap,a( 1_ilp, q ), 1_ilp,a( 1_ilp, &
                                                         p ), 1_ilp )
                                               call stdlib_qaxpy( m, cs*sn*apoaq,a( 1_ilp, p ), 1_ilp,a( &
                                                         1_ilp, q ), 1_ilp )
                                               d( p ) = d( p )*cs
                                               d( q ) = d( q ) / cs
                                               if( rsvec ) then
                                                  call stdlib_qaxpy( mvl, -t*aqoap,v( 1_ilp, q ), 1_ilp,v(&
                                                             1_ilp, p ), 1_ilp )
                                                  call stdlib_qaxpy( mvl,cs*sn*apoaq,v( 1_ilp, p ), 1_ilp,&
                                                            v( 1_ilp, q ), 1_ilp )
                                               end if
                                            end if
                                         else
                                            if( d( q )>=one ) then
                                               call stdlib_qaxpy( m, t*apoaq,a( 1_ilp, p ), 1_ilp,a( 1_ilp, q &
                                                         ), 1_ilp )
                                               call stdlib_qaxpy( m, -cs*sn*aqoap,a( 1_ilp, q ), 1_ilp,a( &
                                                         1_ilp, p ), 1_ilp )
                                               d( p ) = d( p ) / cs
                                               d( q ) = d( q )*cs
                                               if( rsvec ) then
                                                  call stdlib_qaxpy( mvl, t*apoaq,v( 1_ilp, p ), 1_ilp,v( &
                                                            1_ilp, q ), 1_ilp )
                                                  call stdlib_qaxpy( mvl,-cs*sn*aqoap,v( 1_ilp, q ), &
                                                            1_ilp,v( 1_ilp, p ), 1_ilp )
                                               end if
                                            else
                                               if( d( p )>=d( q ) ) then
                                                  call stdlib_qaxpy( m, -t*aqoap,a( 1_ilp, q ), 1_ilp,a( &
                                                            1_ilp, p ), 1_ilp )
                                                  call stdlib_qaxpy( m, cs*sn*apoaq,a( 1_ilp, p ), 1_ilp,&
                                                            a( 1_ilp, q ), 1_ilp )
                                                  d( p ) = d( p )*cs
                                                  d( q ) = d( q ) / cs
                                                  if( rsvec ) then
                                                     call stdlib_qaxpy( mvl,-t*aqoap,v( 1_ilp, q ), 1_ilp,&
                                                               v( 1_ilp, p ), 1_ilp )
                                                     call stdlib_qaxpy( mvl,cs*sn*apoaq,v( 1_ilp, p ),&
                                                                1_ilp,v( 1_ilp, q ), 1_ilp )
                                                  end if
                                               else
                                                  call stdlib_qaxpy( m, t*apoaq,a( 1_ilp, p ), 1_ilp,a( 1_ilp,&
                                                             q ), 1_ilp )
                                                  call stdlib_qaxpy( m,-cs*sn*aqoap,a( 1_ilp, q ), 1_ilp,&
                                                            a( 1_ilp, p ), 1_ilp )
                                                  d( p ) = d( p ) / cs
                                                  d( q ) = d( q )*cs
                                                  if( rsvec ) then
                                                     call stdlib_qaxpy( mvl,t*apoaq, v( 1_ilp, p ),1_ilp, &
                                                               v( 1_ilp, q ), 1_ilp )
                                                     call stdlib_qaxpy( mvl,-cs*sn*aqoap,v( 1_ilp, q )&
                                                               , 1_ilp,v( 1_ilp, p ), 1_ilp )
                                                  end if
                    ...
                    ! .. have to use modified gram-schmidt like transformation
                                      call stdlib_qcopy( m, a( 1_ilp, p ), 1_ilp, work, 1_ilp )
                                      call stdlib_qlascl( 'G', 0_ilp, 0_ilp, aapp, one, m,1_ilp, work, lda, &
                                                ierr )
                                      call stdlib_qlascl( 'G', 0_ilp, 0_ilp, aaqq, one, m,1_ilp, a( 1_ilp, q ), &
                                                lda, ierr )
                                      temp1 = -aapq*d( p ) / d( q )
                                      call stdlib_qaxpy( m, temp1, work, 1_ilp,a( 1_ilp, q ), 1_ilp )
                                      call stdlib_qlascl( 'G', 0_ilp, 0_ilp, one, aaqq, m,1_ilp, a( 1_ilp, q ), &
                                                lda, ierr )
                                      sva( q ) = aaqq*sqrt( max( zero,one-aapq*aapq ) )
                                      mxsinj = max( mxsinj, sfmin )
                                   end if
                 ...
                 ! recompute sva(q), sva(p).
                                   if( ( sva( q ) / aaqq )**2_ilp<=rooteps )then
                                      if( ( aaqq<rootbig ) .and.( aaqq>rootsfmin ) ) then
                                         sva( q ) = stdlib_qnrm2( m, a( 1_ilp, q ), 1_ilp )*d( q )
                                      else
                                         t = zero
                                         aaqq = one
                                         call stdlib_qlassq( m, a( 1_ilp, q ), 1_ilp, t,aaqq )
                                         sva( q ) = t*sqrt( aaqq )*d( q )
                                      end if
                                   end if
                                   if( ( aapp / aapp0 )<=rooteps ) then
                                      if( ( aapp<rootbig ) .and.( aapp>rootsfmin ) ) then
                                         aapp = stdlib_qnrm2( m, a( 1_ilp, p ), 1_ilp )*d( p )
                                      else
                                         t = zero
                                         aapp = one
                                         call stdlib_qlassq( m, a( 1_ilp, p ), 1_ilp, t,aapp )
                                         aapp = t*sqrt( aapp )*d( p )
                                      end if
                                      sva( p ) = aapp
                                   end if
                 ...
                 ! rotated  = rotated + 1
                       ...
                       ! Choose Correct Signum For Theta And Rotate
                                      ...
                                      if( aapp>aaqq ) then
                                         ...
                                         call stdlib_qcopy( m, a( 1_ilp, q ), 1_ilp, work,1_ilp )
                                         call stdlib_qlascl( 'G', 0_ilp, 0_ilp, aaqq, one,m, 1_ilp, work, lda,&
                                                    ierr )
                                         call stdlib_qlascl( 'G', 0_ilp, 0_ilp, aapp, one,m, 1_ilp, a( 1_ilp, p ),&
                                                    lda,ierr )
                                         temp1 = -aapq*d( q ) / d( p )
                                         call stdlib_qaxpy( m, temp1, work, 1_ilp,a( 1_ilp, p ), 1_ilp )
                                                   
                                         call stdlib_qlascl( 'G', 0_ilp, 0_ilp, one, aapp,m, 1_ilp, a( 1_ilp, p ),&
                                                    lda,ierr )
                                         sva( p ) = aapp*sqrt( max( zero,one-aapq*aapq ) )
                                         mxsinj = max( mxsinj, sfmin )
                                      end if
      ...
      ! 2000 :: end of the ibr-loop
           ! .. update sva(n)
              if( ( sva( n )<rootbig ) .and. ( sva( n )>rootsfmin ) )then
                 sva( n ) = stdlib_qnrm2( m, a( 1_ilp, n ), 1_ilp )*d( n )
              else
                 t = zero
                 aapp = one
                 call stdlib_qlassq( m, a( 1_ilp, n ), 1_ilp, t, aapp )
                 sva( n ) = t*sqrt( aapp )*d( n )
              end if
           ! additional steering devices
              if( ( i<swband ) .and. ( ( mxaapq<=roottol ) .or.( iswrot<=n ) ) )swband = i
     ...
     !! parameters (marked by [TP]) are available for the implementer.
        ...
        ! =====================================================================
           ...
           swband = 0_ilp
      ! [tp] swband is a tuning parameter [tp]. it is meaningful and effective
           ! if stdlib_wgesvj is used as a computational routine in the preconditioned
           ! jacobi svd algorithm stdlib_wgejsv. for sweeps i=1:swband the procedure
           ! works on pivots inside a band-like region around the diagonal.
           ! the boundaries are determined dynamically, based on the number of
           ! pivots above a threshold.
           kbl = min( 8_ilp, n )
      ...
      ! [tp] lkahead is a tuning parameter.
           ...
           ! .. de rijk's pivoting
                       q = stdlib_iqamax( n-p+1, sva( p ), 1_ilp ) + p - 1_ilp
                       if( p/=q ) then
                          call stdlib_wswap( m, a( 1_ilp, p ), 1_ilp, a( 1_ilp, q ), 1_ilp )
                          if( rsvec )call stdlib_wswap( mvl, v( 1_ilp, p ), 1_ilp,v( 1_ilp, q ), 1_ilp )
                          temp1 = sva( p )
                          sva( p ) = sva( q )
                          sva( q ) = temp1
                          aapq = d(p)
              ...
              ! caveat:
              ! unfortunately, some blas implementations compute sncrm2(m,a(1,p),1)
              ! as sqrt(s=stdlib_wdotc(m,a(1,p),1,a(1,p),1)), which may cause the result to
              ! overflow for ||a(:,p)||_2 > sqrt(overflow_threshold), and to
              ! underflow for ||a(:,p)||_2 < sqrt(underflow_threshold).
              ! hence, stdlib_qznrm2 cannot be trusted, not even in the case when
              ! the true norm is far from the under(over)flow boundaries.
              ! if properly implemented stdlib_qznrm2 is available, the if-then-else-end if
              ! below should be replaced with "aapp = stdlib_qznrm2( m, a(1,p), 1 )".
                          if( ( sva( p )<rootbig ) .and.( sva( p )>rootsfmin ) ) then
                             sva( p ) = stdlib_qznrm2( m, a( 1_ilp, p ), 1_ilp )
                          else
                             temp1 = zero
                             aapp = one
                             call stdlib_wlassq( m, a( 1_ilp, p ), 1_ilp, temp1, aapp )
                             sva( p ) = temp1*sqrt( aapp )
                          end if
                          aapp = sva( p )
                       else
                          ...
                          loop_2002: do q = p + 1, min( igl+kbl-1, n )
                             ...
                             if( aaqq>zero ) then
                                ...
                                if( aaqq>=one ) then
                                   rotok = ( small*aapp )<=aaqq
                                   if( aapp<( big / aaqq ) ) then
                                      aapq = ( stdlib_wdotc( m, a( 1_ilp, p ), 1_ilp,a( 1_ilp, q ), 1_ilp ) / &
                                                aaqq ) / aapp
                                   else
                                      call stdlib_wcopy( m, a( 1_ilp, p ), 1_ilp,work, 1_ilp )
                                      call stdlib_wlascl( 'G', 0_ilp, 0_ilp, aapp, one,m, 1_ilp, work, lda, &
                                                ierr )
                                      aapq = stdlib_wdotc( m, work, 1_ilp,a( 1_ilp, q ), 1_ilp ) / &
                                                aaqq
                                   end if
                                else
                                   rotok = aapp<=( aaqq / small )
                                   if( aapp>( small / aaqq ) ) then
                                      ...
                                      call stdlib_wcopy( m, a( 1_ilp, q ), 1_ilp,work, 1_ilp )
                                      call stdlib_wlascl( 'G', 0_ilp, 0_ilp, aaqq,one, m, 1_ilp,work, lda, &
                                                ierr )
                                      aapq = stdlib_wdotc( m, a( 1_ilp, p ), 1_ilp,work, 1_ilp ) / &
                                                aapp
                                   end if
                                end if
                                 ! aapq = aapq * conjg( cwork(p) ) * cwork(q)
      ...
      ! [rtd]      rotated = rotated + one
                                   ...
                                   if( rotok ) then
                                      ...
                                      if( abs( theta )>bigtheta ) then
                                         t  = half / theta
                                         cs = one
                                         call stdlib_wrot( m, a(1_ilp,p), 1_ilp, a(1_ilp,q), 1_ilp,cs, conjg(ompq)&
                                                   *t )
                                         if ( rsvec ) then
                                             call stdlib_wrot( mvl, v(1_ilp,p), 1_ilp,v(1_ilp,q), 1_ilp, cs, &
                                                       conjg(ompq)*t )
                                         end if
                                         sva( q ) = aaqq*sqrt( max( zero,one+t*apoaq*aapq1 ) )
                    ...
                    ! .. have to use modified gram-schmidt like transformation
                                      ...
                                      call stdlib_wlascl( 'G', 0_ilp, 0_ilp, aapp, one, m,1_ilp, work, lda,&
                                                ierr )
                                      call stdlib_wlascl( 'G', 0_ilp, 0_ilp, aaqq, one, m,1_ilp, a( 1_ilp, q ), &
                                                lda, ierr )
                                      call stdlib_waxpy( m, -aapq, work, 1_ilp,a( 1_ilp, q ), 1_ilp )
                                      call stdlib_wlascl( 'G', 0_ilp, 0_ilp, one, aaqq, m,1_ilp, a( 1_ilp, q ), &
                                                lda, ierr )
                                      sva( q ) = aaqq*sqrt( max( zero,one-aapq1*aapq1 ) )
                                      mxsinj = max( mxsinj, sfmin )
                                   end if
                 ...
                 ! recompute sva(q), sva(p).
                                   if( ( sva( q ) / aaqq )**2_ilp<=rooteps )then
                                      if( ( aaqq<rootbig ) .and.( aaqq>rootsfmin ) ) then
                                         sva( q ) = stdlib_qznrm2( m, a( 1_ilp, q ), 1_ilp )
                                      else
                                         t = zero
                                         aaqq = one
                                         call stdlib_wlassq( m, a( 1_ilp, q ), 1_ilp, t,aaqq )
                                         sva( q ) = t*sqrt( aaqq )
                                      end if
                                   end if
                                   if( ( aapp / aapp0 )<=rooteps ) then
                                      if( ( aapp<rootbig ) .and.( aapp>rootsfmin ) ) then
                                         aapp = stdlib_qznrm2( m, a( 1_ilp, p ), 1_ilp )
                                      else
                                         t = zero
                                         aapp = one
                                         call stdlib_wlassq( m, a( 1_ilp, p ), 1_ilp, t,aapp )
                                         aapp = t*sqrt( aapp )
                                      end if
                                      sva( p ) = aapp
                                   end if
           ...
           ! M X 2 Jacobi Svd 
              ! safe gram matrix computation
                                if( aaqq>=one ) then
                                   ...
                                   if( aapp>( small / aaqq ) ) then
                                      aapq = ( stdlib_wdotc( m, a( 1_ilp, p ), 1_ilp,a( 1_ilp, q ), 1_ilp ) / max(&
                                                aaqq,aapp) )/ min(aaqq,aapp)
                                   else
                                      call stdlib_wcopy( m, a( 1_ilp, q ), 1_ilp,work, 1_ilp )
                                      call stdlib_wlascl( 'G', 0_ilp, 0_ilp, aaqq,one, m, 1_ilp,work, lda, &
                    ...
                    ! .. have to use modified gram-schmidt like transformation
                                    if( aapp>aaqq ) then
                                        ...
                                        call stdlib_wcopy( m, a( 1_ilp, q ), 1_ilp,work, 1_ilp )
                                         call stdlib_wlascl( 'G', 0_ilp, 0_ilp, aaqq, one,m, 1_ilp, work,lda,&
                                                   ierr )
                                         call stdlib_wlascl( 'G', 0_ilp, 0_ilp, aapp, one,m, 1_ilp, a( 1_ilp, p ),&
                                                    lda,ierr )
                                         call stdlib_waxpy( m, -conjg(aapq),work, 1_ilp, a( 1_ilp, p ), 1_ilp &
                                                   )
                                         call stdlib_wlascl( 'G', 0_ilp, 0_ilp, one, aapp,m, 1_ilp, a( 1_ilp, p ),&
                                                    lda,ierr )
                                         sva( p ) = aapp*sqrt( max( zero,one-aapq1*aapq1 ) )
      ...
      ! 2000 :: end of the ibr-loop
           ! .. update sva(n)
              if( ( sva( n )<rootbig ) .and. ( sva( n )>rootsfmin ) )then
                 sva( n ) = stdlib_qznrm2( m, a( 1_ilp, n ), 1_ilp )
              else
                 t = zero
                 aapp = one
                 call stdlib_wlassq( m, a( 1_ilp, n ), 1_ilp, t, aapp )
                 sva( n ) = t*sqrt( aapp )
              end if
           ! additional steering devices
              if( ( i<swband ) .and. ( ( mxaapq<=roottol ) .or.( iswrot<=n ) ) )swband = i
     ...
     !! is given in TOL.
      ...
      ! [tp] swband is a tuning parameter. it is meaningful and effective
           ! if stdlib_wgesvj is used as a computational routine in the preconditioned
           ! jacobi svd algorithm stdlib_wgejsv.
           ! | *   *   * [x] [x] [x]|
           ! | *   *   * [x] [x] [x]|    row-cycling in the nblr-by-nblc [x] blocks.
           ! | *   *   * [x] [x] [x]|    row-cyclic pivoting inside each [x] block.
           ! |[x] [x] [x] *   *   * |
     ...
     !! may be postmultiplied into input matrices U1, V1, or Q1.
        ...
        ! =====================================================================
           ...
           ! initialize u, v and q, if necessary
           if( initu )call stdlib_qlaset( 'FULL', m, m, zero, one, u, ldu )
           if( initv )call stdlib_qlaset( 'FULL', p, p, zero, one, v, ldv )
           ...
           loop_40: do kcycle = 1, maxit
              ...
              loop_20: do i = 1, l - 1
                 loop_10: do j = i + 1, l
                    ...
                    call stdlib_qlags2( upper, a1, a2, a3, b1, b2, b3, csu, snu,csv, snv, csq, &
                              snq )
                    ! update (k+i)-th and (k+j)-th rows of matrix a: u**t *a
                    if( k+j<=m )call stdlib_qrot( l, a( k+j, n-l+1 ), lda, a( k+i, n-l+1 ),lda, &
                              csu, snu )
                    ! update i-th and j-th rows of matrix b: v**t *b
                    call stdlib_qrot( l, b( j, n-l+1 ), ldb, b( i, n-l+1 ), ldb,csv, snv )
                    ! update (n-l+i)-th and (n-l+j)-th columns of matrices
                    ! a and b: a*q and b*q
                    call stdlib_qrot( min( k+l, m ), a( 1_ilp, n-l+j ), 1_ilp,a( 1_ilp, n-l+i ), 1_ilp, csq, snq )
                              
                    call stdlib_qrot( l, b( 1_ilp, n-l+j ), 1_ilp, b( 1_ilp, n-l+i ), 1_ilp, csq,snq )
                    if( upper ) then
                       if( k+i<=m )a( k+i, n-l+j ) = zero
                       b( i, n-l+j ) = zero
                    else
                    ...
                    ! update orthogonal matrices u, v, q, if desired.
                    if( wantu .and. k+j<=m )call stdlib_qrot( m, u( 1_ilp, k+j ), 1_ilp, u( 1_ilp, k+i ), 1_ilp, &
                              csu,snu )
                    if( wantv )call stdlib_qrot( p, v( 1_ilp, j ), 1_ilp, v( 1_ilp, i ), 1_ilp, csv, snv )
                    if( wantq )call stdlib_qrot( n, q( 1_ilp, n-l+j ), 1_ilp, q( 1_ilp, n-l+i ), 1_ilp, csq,snq )
              ...
              if( .not.upper ) then
                 ...
                 error = zero
                 do i = 1, min( l, m-k )
                    call stdlib_qcopy( l-i+1, a( k+i, n-l+i ), lda, work, 1_ilp )
                    call stdlib_qcopy( l-i+1, b( i, n-l+i ), ldb, work( l+1 ), 1_ilp )
                    call stdlib_qlapll( l-i+1, work, 1_ilp, work( l+1 ), 1_ilp, ssmin )
                    error = max( error, ssmin )
                 end do
                 if( abs( error )<=min( tola, tolb ) )go to 50
              end if
           ...
           do i = 1, min( l, m-k )
              ...
              if( (gamma<=hugenum).and.(gamma>=-hugenum) ) then
                 ! change sign if necessary
                 if( gamma<zero ) then
                    call stdlib_qscal( l-i+1, -one, b( i, n-l+i ), ldb )
                    if( wantv )call stdlib_qscal( p, -one, v( 1_ilp, i ), 1_ilp )
                 end if
                 call stdlib_qlartg( abs( gamma ), one, beta( k+i ), alpha( k+i ),rwk )
                 if( alpha( k+i )>=beta( k+i ) ) then
                    call stdlib_qscal( l-i+1, one / alpha( k+i ), a( k+i, n-l+i ),lda )
                 else
                    call stdlib_qscal( l-i+1, one / beta( k+i ), b( i, n-l+i ),ldb )
                    call stdlib_qcopy( l-i+1, b( i, n-l+i ), ldb, a( k+i, n-l+i ),lda )
                 end if
              else
                 alpha( k+i ) = zero
                 beta( k+i ) = one
     ...
     !! may be postmultiplied into input matrices U1, V1, or Q1.
        ...
        ! =====================================================================
           ...
           ! initialize u, v and q, if necessary
           if( initu )call stdlib_wlaset( 'FULL', m, m, czero, cone, u, ldu )
           if( initv )call stdlib_wlaset( 'FULL', p, p, czero, cone, v, ldv )
           ...
           loop_40: do kcycle = 1, maxit
              ...
              loop_20: do i = 1, l - 1
                 loop_10: do j = i + 1, l
                    ...
                    call stdlib_wlags2( upper, a1, a2, a3, b1, b2, b3, csu, snu,csv, snv, csq, &
                              snq )
                    ! update (k+i)-th and (k+j)-th rows of matrix a: u**h *a
                    if( k+j<=m )call stdlib_wrot( l, a( k+j, n-l+1 ), lda, a( k+i, n-l+1 ),lda, &
                              csu, conjg( snu ) )
                    ! update i-th and j-th rows of matrix b: v**h *b
                    call stdlib_wrot( l, b( j, n-l+1 ), ldb, b( i, n-l+1 ), ldb,csv, conjg( snv ) &
                              )
                    ! update (n-l+i)-th and (n-l+j)-th columns of matrices
                    ! a and b: a*q and b*q
                    call stdlib_wrot( min( k+l, m ), a( 1_ilp, n-l+j ), 1_ilp,a( 1_ilp, n-l+i ), 1_ilp, csq, snq )
                              
                    call stdlib_wrot( l, b( 1_ilp, n-l+j ), 1_ilp, b( 1_ilp, n-l+i ), 1_ilp, csq,snq )
                    if( upper ) then
                       if( k+i<=m )a( k+i, n-l+j ) = czero
                       b( i, n-l+j ) = czero
                    else
                    ...
                    ! update unitary matrices u, v, q, if desired.
                    if( wantu .and. k+j<=m )call stdlib_wrot( m, u( 1_ilp, k+j ), 1_ilp, u( 1_ilp, k+i ), 1_ilp, &
                              csu,snu )
                    if( wantv )call stdlib_wrot( p, v( 1_ilp, j ), 1_ilp, v( 1_ilp, i ), 1_ilp, csv, snv )
                    if( wantq )call stdlib_wrot( n, q( 1_ilp, n-l+j ), 1_ilp, q( 1_ilp, n-l+i ), 1_ilp, csq,snq )
              ...
              if( .not.upper ) then
                 ...
                 error = zero
                 do i = 1, min( l, m-k )
                    call stdlib_wcopy( l-i+1, a( k+i, n-l+i ), lda, work, 1_ilp )
                    call stdlib_wcopy( l-i+1, b( i, n-l+i ), ldb, work( l+1 ), 1_ilp )
                    call stdlib_wlapll( l-i+1, work, 1_ilp, work( l+1 ), 1_ilp, ssmin )
                    error = max( error, ssmin )
                 end do
                 if( abs( error )<=min( tola, tolb ) )go to 50
              end if
           ...
           do i = 1, min( l, m-k )
              ...
              if( (gamma<=hugenum).and.(gamma>=-hugenum) ) then
                 if( gamma<zero ) then
                    call stdlib_wdscal( l-i+1, -one, b( i, n-l+i ), ldb )
                    if( wantv )call stdlib_wdscal( p, -one, v( 1_ilp, i ), 1_ilp )
                 end if
                 call stdlib_qlartg( abs( gamma ), one, beta( k+i ), alpha( k+i ),rwk )
                 if( alpha( k+i )>=beta( k+i ) ) then
                    call stdlib_wdscal( l-i+1, one / alpha( k+i ), a( k+i, n-l+i ),lda )
                 else
                    call stdlib_wdscal( l-i+1, one / beta( k+i ), b( i, n-l+i ),ldb )
                    call stdlib_wcopy( l-i+1, b( i, n-l+i ), ldb, a( k+i, n-l+i ),lda )
                 end if
              else
                 alpha( k+i ) = zero
                 beta( k+i ) = one
     ...
     !! an N-by-N matrix.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              ...
              if( wantq ) then
                 if( m>=k ) then
                    call stdlib_wungqr( m, n, k, a, lda, tau, work, -1_ilp, iinfo )
                 else
                    if( m>1_ilp ) then
                       call stdlib_wungqr( m-1, m-1, m-1, a, lda, tau, work, -1_ilp,iinfo )
                    end if
                 end if
              else
                 if( k<n ) then
                    call stdlib_wunglq( m, n, k, a, lda, tau, work, -1_ilp, iinfo )
                 else
                    if( n>1_ilp ) then
                       call stdlib_wunglq( n-1, n-1, n-1, a, lda, tau, work, -1_ilp,iinfo )
                    end if
                 end if
              end if
              lwkopt = real( work( 1_ilp ),KIND=qp)
              lwkopt = max (lwkopt, mn)
           end if
           if( info/=0_ilp ) then
              call stdlib_xerbla( 'ZUNGBR', -info )
           ...
           if( wantq ) then
              ! form q, determined by a call to stdlib_wgebrd to reduce an m-by-k
              ! matrix
              if( m>=k ) then
                 ! if m >= k, assume m >= n >= k
                 call stdlib_wungqr( m, n, k, a, lda, tau, work, lwork, iinfo )
              else
                 ! if m < k, assume m = n
                 ! shift the vectors which define the elementary reflectors cone
                 ! column to the right, and set the first row and column of q
                 ...
                 if( m>1_ilp ) then
                    ! form q(2:m,2:m)
                    call stdlib_wungqr( m-1, m-1, m-1, a( 2_ilp, 2_ilp ), lda, tau, work,lwork, iinfo )
                              
                 end if
              end if
           else
              ! form p**h, determined by a call to stdlib_wgebrd to reduce a k-by-n
              ! matrix
              if( k<n ) then
                 ! if k < n, assume k <= m <= n
                 call stdlib_wunglq( m, n, k, a, lda, tau, work, lwork, iinfo )
              else
                 ! if k >= n, assume m = n
                 ! shift the vectors which define the elementary reflectors cone
                 ! row downward, and set the first row and column of p**h to
                 ...
                 if( n>1_ilp ) then
                    ! form p**h(2:n,2:n)
                    call stdlib_wunglq( n-1, n-1, n-1, a( 2_ilp, 2_ilp ), lda, tau, work,lwork, iinfo )
     ...
     !! an N-by-N matrix.
        ...
        ! =====================================================================
           ...
           if( info==0_ilp ) then
              ...
              if( wantq ) then
                 if( m>=k ) then
                    call stdlib_qorgqr( m, n, k, a, lda, tau, work, -1_ilp, iinfo )
                 else
                    if( m>1_ilp ) then
                       call stdlib_qorgqr( m-1, m-1, m-1, a, lda, tau, work, -1_ilp,iinfo )
                    end if
                 end if
              else
                 if( k<n ) then
                    call stdlib_qorglq( m, n, k, a, lda, tau, work, -1_ilp, iinfo )
                 else
                    if( n>1_ilp ) then
                       call stdlib_qorglq( n-1, n-1, n-1, a, lda, tau, work, -1_ilp,iinfo )
                    end if
                 end if
              end if
              lwkopt = work( 1_ilp )
           ...
           if( wantq ) then
              ! form q, determined by a call to stdlib_qgebrd to reduce an m-by-k
              ! matrix
              if( m>=k ) then
                 ! if m >= k, assume m >= n >= k
                 call stdlib_qorgqr( m, n, k, a, lda, tau, work, lwork, iinfo )
              else
                 ! if m < k, assume m = n
                 ! shift the vectors which define the elementary reflectors one
                 ! column to the right, and set the first row and column of q
                 ...
                 if( m>1_ilp ) then
                    ! form q(2:m,2:m)
                    call stdlib_qorgqr( m-1, m-1, m-1, a( 2_ilp, 2_ilp ), lda, tau, work,lwork, iinfo )
                              
                 end if
              end if
           else
              ! form p**t, determined by a call to stdlib_qgebrd to reduce a k-by-n
              ! matrix
              if( k<n ) then
                 ! if k < n, assume k <= m <= n
                 call stdlib_qorglq( m, n, k, a, lda, tau, work, lwork, iinfo )
              else
                 ! if k >= n, assume m = n
                 ! shift the vectors which define the elementary reflectors one
                 ! row downward, and set the first row and column of p**t to
                 ...
                 if( n>1_ilp ) then
                    ! form p**t(2:n,2:n)
                    call stdlib_qorglq( n-1, n-1, n-1, a( 2_ilp, 2_ilp ), lda, tau, work,lwork, iinfo )
     ...
     !! if k >= nq, P = G(1) G(2) . . . G(nq-1).
        ...
        ! =====================================================================
           ...
           if( applyq ) then
              ! apply q
              if( nq>=k ) then
                 ! q was determined by a call to stdlib_wgebrd with nq >= k
                 call stdlib_wunmqr( side, trans, m, n, k, a, lda, tau, c, ldc,work, lwork, iinfo &
                           )
              else if( nq>1_ilp ) then
                 ! q was determined by a call to stdlib_wgebrd with nq < k
                 if( left ) then
                 ...
                 call stdlib_wunmqr( side, trans, mi, ni, nq-1, a( 2_ilp, 1_ilp ), lda, tau,c( i1, i2 ), &
                           ldc, work, lwork, iinfo )
              end if
           else
              ! apply p
              ...
              if( nq>k ) then
                 ! p was determined by a call to stdlib_wgebrd with nq > k
                 call stdlib_wunmlq( side, transt, m, n, k, a, lda, tau, c, ldc,work, lwork, &
                           iinfo )
              else if( nq>1_ilp ) then
                 ! p was determined by a call to stdlib_wgebrd with nq <= k
                 if( left ) then
                 ...
                 call stdlib_wunmlq( side, transt, mi, ni, nq-1, a( 1_ilp, 2_ilp ), lda,tau, c( i1, i2 ), &
                           ldc, work, lwork, iinfo )
              end if
           end if
           work( 1_ilp ) = lwkopt
     ...
     !! if k >= nq, P = G(1) G(2) . . . G(nq-1).
        ...
        ! =====================================================================
           ...
           if( applyq ) then
              ! apply q
              if( nq>=k ) then
                 ! q was determined by a call to stdlib_qgebrd with nq >= k
                 call stdlib_qormqr( side, trans, m, n, k, a, lda, tau, c, ldc,work, lwork, iinfo &
                           )
              else if( nq>1_ilp ) then
                 ! q was determined by a call to stdlib_qgebrd with nq < k
                 if( left ) then
                 ...
                 call stdlib_qormqr( side, trans, mi, ni, nq-1, a( 2_ilp, 1_ilp ), lda, tau,c( i1, i2 ), &
                           ldc, work, lwork, iinfo )
              end if
           else
              ! apply p
              ...
              if( nq>k ) then
                 ! p was determined by a call to stdlib_qgebrd with nq > k
                 call stdlib_qormlq( side, transt, m, n, k, a, lda, tau, c, ldc,work, lwork, &
                           iinfo )
              else if( nq>1_ilp ) then
                 ! p was determined by a call to stdlib_qgebrd with nq <= k
                 if( left ) then
                 ...
                 call stdlib_qormlq( side, transt, mi, ni, nq-1, a( 1_ilp, 2_ilp ), lda,tau, c( i1, i2 ), &
                           ldc, work, lwork, iinfo )
              end if
           end if
           work( 1_ilp ) = lwkopt
