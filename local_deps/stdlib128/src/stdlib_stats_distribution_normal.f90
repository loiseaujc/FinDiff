...
contains
    ...
    impure subroutine zigset
        ...
        impure function rvs_norm_0_rqp () result(res)
            !
            ! Standard normal random variate (0,1)
            !
            real(qp) :: res
        ...
        impure elemental function rvs_norm_cqp (loc, scale) result(res)
            !
            ! Normally distributed complex. The real part and imaginary part are       &
            ! independent of each other.
            !
        ...
        impure function rvs_norm_array_rqp (loc, scale, array_size) result(res)
            real(qp), intent(in) :: loc, scale
            integer, intent(in) :: array_size
        ...
        impure function rvs_norm_array_cqp (loc, scale, array_size) result(res)
            complex(qp), intent(in) :: loc, scale
            integer, intent(in) :: array_size
            integer :: i
        ...
        elemental function pdf_norm_rqp (x, loc, scale) result(res)
            !
            ! Normal distribution probability density function
            !
        ...
        elemental function pdf_norm_cqp (x, loc, scale) result(res)
        ...
        elemental function cdf_norm_rqp (x, loc, scale) result(res)
            !
            ! Normal distribution cumulative distribution function
            !
            real(qp), intent(in) :: x, loc, scale
        ...
        elemental function cdf_norm_cqp (x, loc, scale) result(res)
            complex(qp), intent(in) :: x, loc, scale
            real(qp) :: res
