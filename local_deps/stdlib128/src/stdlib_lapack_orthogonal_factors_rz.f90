        ...
        ! =====================================================================
           ...
           if( nb>=nbmin .and. nb<m .and. nx<m ) then
              ...
              do i = m - kk + ki + 1, m - kk + 1, -nb
                 ...
                 ! compute the tz factorization of the current block
                 ! a(i:i+ib-1,i:n)
                 call stdlib_qlatrz( ib, n-i+1, n-m, a( i, i ), lda, tau( i ),work )
                 if( i>1_ilp ) then
                    ! form the triangular factor of the block reflector
                    ! h = h(i+ib-1) . . . h(i+1) h(i)
                    call stdlib_qlarzt( 'BACKWARD', 'ROWWISE', n-m, ib, a( i, m1 ),lda, tau( i ), &
                              work, ldwork )
                    ! apply h to a(1:i-1,i:n) from the right
           ...
           ! use unblocked code to factor the last or only block
           if( mu>0_ilp )call stdlib_qlatrz( mu, n, n-m, a, lda, tau, work )
           work( 1_ilp ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( nb>=nbmin .and. nb<m .and. nx<m ) then
              ...
              do i = m - kk + ki + 1, m - kk + 1, -nb
                 ...
                 ! compute the tz factorization of the current block
                 ! a(i:i+ib-1,i:n)
                 call stdlib_wlatrz( ib, n-i+1, n-m, a( i, i ), lda, tau( i ),work )
                 if( i>1_ilp ) then
                    ! form the triangular factor of the block reflector
                    ! h = h(i+ib-1) . . . h(i+1) h(i)
                    call stdlib_wlarzt( 'BACKWARD', 'ROWWISE', n-m, ib, a( i, m1 ),lda, tau( i ), &
                              work, ldwork )
                    ! apply h to a(1:i-1,i:n) from the right
           ...
           ! use unblocked code to factor the last or only block
           if( mu>0_ilp )call stdlib_wlatrz( mu, n, n-m, a, lda, tau, work )
           work( 1_ilp ) = lwkopt
           return
        ...
        ! =====================================================================
           ...
           if( nb<nbmin .or. nb>=k ) then
              ...
              do i = i1, i2, i3
                 ...
                 ! form the triangular factor of the block reflector
                 ! h = h(i+ib-1) . . . h(i+1) h(i)
                 call stdlib_wlarzt( 'BACKWARD', 'ROWWISE', l, ib, a( i, ja ), lda,tau( i ), work(&
                            iwt ), ldt )
                 if( left ) then
                    ! h or h**h is applied to c(i:m,1:n)
                    mi = m - i + 1_ilp
                 ...
                 ! apply h or h**h
                 call stdlib_wlarzb( side, transt, 'BACKWARD', 'ROWWISE', mi, ni,ib, l, a( i, ja )&
                           , lda, work( iwt ), ldt,c( ic, jc ), ldc, work, ldwork )
              end do
           end if
           work( 1_ilp ) = lwkopt
        ...
        ! =====================================================================
           ...
           if( nb<nbmin .or. nb>=k ) then
              ...
              do i = i1, i2, i3
                 ...
                 ! form the triangular factor of the block reflector
                 ! h = h(i+ib-1) . . . h(i+1) h(i)
                 call stdlib_qlarzt( 'BACKWARD', 'ROWWISE', l, ib, a( i, ja ), lda,tau( i ), work(&
                            iwt ), ldt )
                 if( left ) then
                    ! h or h**t is applied to c(i:m,1:n)
                    mi = m - i + 1_ilp
                 ...
                 ! apply h or h**t
                 call stdlib_qlarzb( side, transt, 'BACKWARD', 'ROWWISE', mi, ni,ib, l, a( i, ja )&
                           , lda, work( iwt ), ldt,c( ic, jc ), ldc, work, ldwork )
              end do
           end if
           work( 1_ilp ) = lwkopt
     ...
     !! if SIDE = 'R'.
        ...
        ! =====================================================================
           ...
           do i = i1, i2, i3
              ...
              ! apply h(i) or h(i)**t
              call stdlib_qlarz( side, mi, ni, l, a( i, ja ), lda, tau( i ),c( ic, jc ), ldc, &
                        work )
           end do
           return
     ...
     !! H is a product of k elementary reflectors as returned by DTZRZF.
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( side, 'L' ) ) then
              ...
              if( tau/=zero ) then
                 ! w( 1:n ) = c( 1, 1:n )
                 call stdlib_qcopy( n, c, ldc, work, 1_ilp )
                 ! w( 1:n ) = w( 1:n ) + c( m-l+1:m, 1:n )**t * v( 1:l )
                 call stdlib_qgemv( 'TRANSPOSE', l, n, one, c( m-l+1, 1_ilp ), ldc, v,incv, one, work,&
                            1_ilp )
                 ! c( 1, 1:n ) = c( 1, 1:n ) - tau * w( 1:n )
                 call stdlib_qaxpy( n, -tau, work, 1_ilp, c, ldc )
                 ! c( m-l+1:m, 1:n ) = c( m-l+1:m, 1:n ) - ...
                                     ! tau * v( 1:l ) * w( 1:n )**t
                 call stdlib_qger( l, n, -tau, v, incv, work, 1_ilp, c( m-l+1, 1_ilp ),ldc )
              end if
           else
              ! form  c * h
              if( tau/=zero ) then
                 ! w( 1:m ) = c( 1:m, 1 )
                 call stdlib_qcopy( m, c, 1_ilp, work, 1_ilp )
                 ! w( 1:m ) = w( 1:m ) + c( 1:m, n-l+1:n, 1:n ) * v( 1:l )
                 call stdlib_qgemv( 'NO TRANSPOSE', m, l, one, c( 1_ilp, n-l+1 ), ldc,v, incv, one, &
                           work, 1_ilp )
                 ! c( 1:m, 1 ) = c( 1:m, 1 ) - tau * w( 1:m )
                 call stdlib_qaxpy( m, -tau, work, 1_ilp, c, 1_ilp )
                 ! c( 1:m, n-l+1:n ) = c( 1:m, n-l+1:n ) - ...
                                     ! tau * w( 1:m ) * v( 1:l )**t
                 call stdlib_qger( m, l, -tau, work, 1_ilp, v, incv, c( 1_ilp, n-l+1 ),ldc )
              end if
           end if
           return
     ...
     !! H is a product of k elementary reflectors as returned by ZTZRZF.
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( side, 'L' ) ) then
              ...
              if( tau/=czero ) then
                 ! w( 1:n ) = conjg( c( 1, 1:n ) )
                 call stdlib_wcopy( n, c, ldc, work, 1_ilp )
                 call stdlib_wlacgv( n, work, 1_ilp )
                 ! w( 1:n ) = conjg( w( 1:n ) + c( m-l+1:m, 1:n )**h * v( 1:l ) )
                 call stdlib_wgemv( 'CONJUGATE TRANSPOSE', l, n, cone, c( m-l+1, 1_ilp ),ldc, v, incv,&
                            cone, work, 1_ilp )
                 call stdlib_wlacgv( n, work, 1_ilp )
                 ! c( 1, 1:n ) = c( 1, 1:n ) - tau * w( 1:n )
                 call stdlib_waxpy( n, -tau, work, 1_ilp, c, ldc )
                 ! c( m-l+1:m, 1:n ) = c( m-l+1:m, 1:n ) - ...
                                     ! tau * v( 1:l ) * w( 1:n )**h
                 call stdlib_wgeru( l, n, -tau, v, incv, work, 1_ilp, c( m-l+1, 1_ilp ),ldc )
              end if
           else
              ! form  c * h
              if( tau/=czero ) then
                 ! w( 1:m ) = c( 1:m, 1 )
                 call stdlib_wcopy( m, c, 1_ilp, work, 1_ilp )
                 ! w( 1:m ) = w( 1:m ) + c( 1:m, n-l+1:n, 1:n ) * v( 1:l )
                 call stdlib_wgemv( 'NO TRANSPOSE', m, l, cone, c( 1_ilp, n-l+1 ), ldc,v, incv, cone, &
                           work, 1_ilp )
                 ! c( 1:m, 1 ) = c( 1:m, 1 ) - tau * w( 1:m )
                 call stdlib_waxpy( m, -tau, work, 1_ilp, c, 1_ilp )
                 ! c( 1:m, n-l+1:n ) = c( 1:m, n-l+1:n ) - ...
                                     ! tau * w( 1:m ) * v( 1:l )**h
                 call stdlib_wgerc( m, l, -tau, work, 1_ilp, v, incv, c( 1_ilp, n-l+1 ),ldc )
              end if
           end if
           return
     ...
     !! Currently, only STOREV = 'R' and DIRECT = 'B' are supported.
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( side, 'L' ) ) then
              ...
              ! w( 1:n, 1:k ) = w( 1:n, 1:k ) + ...
                              ! c( m-l+1:m, 1:n )**t * v( 1:k, 1:l )**t
              if( l>0_ilp )call stdlib_qgemm( 'TRANSPOSE', 'TRANSPOSE', n, k, l, one,c( m-l+1, 1_ilp ), &
                        ldc, v, ldv, one, work, ldwork )
              ! w( 1:n, 1:k ) = w( 1:n, 1:k ) * t**t  or  w( 1:m, 1:k ) * t
              call stdlib_qtrmm( 'RIGHT', 'LOWER', transt, 'NON-UNIT', n, k, one, t,ldt, work, &
                        ldwork )
              ! c( 1:k, 1:n ) = c( 1:k, 1:n ) - w( 1:n, 1:k )**t
              do j = 1, n
                 do i = 1, k
              ...
              ! c( m-l+1:m, 1:n ) = c( m-l+1:m, 1:n ) - ...
                                  ! v( 1:k, 1:l )**t * w( 1:n, 1:k )**t
              if( l>0_ilp )call stdlib_qgemm( 'TRANSPOSE', 'TRANSPOSE', l, n, k, -one, v, ldv,work, &
                        ldwork, one, c( m-l+1, 1_ilp ), ldc )
           else if( stdlib_lsame( side, 'R' ) ) then
              ! form  c * h  or  c * h**t
              ! w( 1:m, 1:k ) = c( 1:m, 1:k )
              ...
              ! w( 1:m, 1:k ) = w( 1:m, 1:k ) + ...
                              ! c( 1:m, n-l+1:n ) * v( 1:k, 1:l )**t
              if( l>0_ilp )call stdlib_qgemm( 'NO TRANSPOSE', 'TRANSPOSE', m, k, l, one,c( 1_ilp, n-l+1 ),&
                         ldc, v, ldv, one, work, ldwork )
              ! w( 1:m, 1:k ) = w( 1:m, 1:k ) * t  or  w( 1:m, 1:k ) * t**t
              call stdlib_qtrmm( 'RIGHT', 'LOWER', trans, 'NON-UNIT', m, k, one, t,ldt, work, &
                        ldwork )
              ! c( 1:m, 1:k ) = c( 1:m, 1:k ) - w( 1:m, 1:k )
              do j = 1, k
                 do i = 1, m
              ...
              ! c( 1:m, n-l+1:n ) = c( 1:m, n-l+1:n ) - ...
                                  ! w( 1:m, 1:k ) * v( 1:k, 1:l )
              if( l>0_ilp )call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', m, l, k, -one,work, &
                        ldwork, v, ldv, one, c( 1_ilp, n-l+1 ), ldc )
           end if
           return
     ...
     !! Currently, only STOREV = 'R' and DIRECT = 'B' are supported.
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( side, 'L' ) ) then
              ...
              ! w( 1:n, 1:k ) = w( 1:n, 1:k ) + ...
                              ! c( m-l+1:m, 1:n )**h * v( 1:k, 1:l )**t
              if( l>0_ilp )call stdlib_wgemm( 'TRANSPOSE', 'CONJUGATE TRANSPOSE', n, k, l,cone, c( m-&
                        l+1, 1_ilp ), ldc, v, ldv, cone, work,ldwork )
              ! w( 1:n, 1:k ) = w( 1:n, 1:k ) * t**t  or  w( 1:m, 1:k ) * t
              call stdlib_wtrmm( 'RIGHT', 'LOWER', transt, 'NON-UNIT', n, k, cone, t,ldt, work, &
                        ldwork )
              ! c( 1:k, 1:n ) = c( 1:k, 1:n ) - w( 1:n, 1:k )**h
              do j = 1, n
                 do i = 1, k
              ...
              ! c( m-l+1:m, 1:n ) = c( m-l+1:m, 1:n ) - ...
                                  ! v( 1:k, 1:l )**h * w( 1:n, 1:k )**h
              if( l>0_ilp )call stdlib_wgemm( 'TRANSPOSE', 'TRANSPOSE', l, n, k, -cone, v, ldv,work, &
                        ldwork, cone, c( m-l+1, 1_ilp ), ldc )
           else if( stdlib_lsame( side, 'R' ) ) then
              ! form  c * h  or  c * h**h
              ! w( 1:m, 1:k ) = c( 1:m, 1:k )
              ...
              ! w( 1:m, 1:k ) = w( 1:m, 1:k ) + ...
                              ! c( 1:m, n-l+1:n ) * v( 1:k, 1:l )**h
              if( l>0_ilp )call stdlib_wgemm( 'NO TRANSPOSE', 'TRANSPOSE', m, k, l, cone,c( 1_ilp, n-l+1 )&
                        , ldc, v, ldv, cone, work, ldwork )
              ! w( 1:m, 1:k ) = w( 1:m, 1:k ) * conjg( t )  or
                              ! w( 1:m, 1:k ) * t**h
              do j = 1, k
                 call stdlib_wlacgv( k-j+1, t( j, j ), 1_ilp )
              end do
              call stdlib_wtrmm( 'RIGHT', 'LOWER', trans, 'NON-UNIT', m, k, cone, t,ldt, work, &
                        ldwork )
              do j = 1, k
                 call stdlib_wlacgv( k-j+1, t( j, j ), 1_ilp )
              end do
              ...
              ! c( 1:m, n-l+1:n ) = c( 1:m, n-l+1:n ) - ...
                                  ! w( 1:m, 1:k ) * conjg( v( 1:k, 1:l ) )
              do j = 1, l
                 call stdlib_wlacgv( k, v( 1_ilp, j ), 1_ilp )
              end do
              if( l>0_ilp )call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', m, l, k, -cone,work, &
                        ldwork, v, ldv, cone, c( 1_ilp, n-l+1 ), ldc )
              do j = 1, l
                 call stdlib_wlacgv( k, v( 1_ilp, j ), 1_ilp )
              end do
        ...
        ! =====================================================================
           ...
           do i = k, 1, -1
              if( tau( i )==zero ) then
                 ...
                 if( i<k ) then
                    ! t(i+1:k,i) = - tau(i) * v(i+1:k,1:n) * v(i,1:n)**t
                    call stdlib_qgemv( 'NO TRANSPOSE', k-i, n, -tau( i ),v( i+1, 1_ilp ), ldv, v( i, &
                              1_ilp ), ldv, zero,t( i+1, i ), 1_ilp )
                    ! t(i+1:k,i) = t(i+1:k,i+1:k) * t(i+1:k,i)
        ...
        ! =====================================================================
           ...
           do i = k, 1, -1
              if( tau( i )==czero ) then
                 ...
                 if( i<k ) then
                    ! t(i+1:k,i) = - tau(i) * v(i+1:k,1:n) * v(i,1:n)**h
                    call stdlib_wlacgv( n, v( i, 1_ilp ), ldv )
                    call stdlib_wgemv( 'NO TRANSPOSE', k-i, n, -tau( i ),v( i+1, 1_ilp ), ldv, v( i, &
                              1_ilp ), ldv, czero,t( i+1, i ), 1_ilp )
                    call stdlib_wlacgv( n, v( i, 1_ilp ), ldv )
                    ! t(i+1:k,i) = t(i+1:k,i+1:k) * t(i+1:k,i)
     ...
     !! matrix and, R and A1 are M-by-M upper triangular matrices.
        ...
        ! =====================================================================
           ...
           do i = m, 1, -1
              ! generate elementary reflector h(i) to annihilate
              ! [ a(i,i) a(i,n-l+1:n) ]
              call stdlib_qlarfg( l+1, a( i, i ), a( i, n-l+1 ), lda, tau( i ) )
              ! apply h(i) to a(1:i-1,i:n) from the right
              call stdlib_qlarz( 'RIGHT', i-1, n-i+1, l, a( i, n-l+1 ), lda,tau( i ), a( 1_ilp, i ), &
                        lda, work )
           end do
           return
     ...
     !! matrix and, R and A1 are M-by-M upper triangular matrices.
        ...
        ! =====================================================================
           ...
           do i = m, 1, -1
              ! generate elementary reflector h(i) to annihilate
              ! [ a(i,i) a(i,n-l+1:n) ]
              call stdlib_wlacgv( l, a( i, n-l+1 ), lda )
              alpha = conjg( a( i, i ) )
              call stdlib_wlarfg( l+1, alpha, a( i, n-l+1 ), lda, tau( i ) )
              tau( i ) = conjg( tau( i ) )
              ! apply h(i) to a(1:i-1,i:n) from the right
              call stdlib_wlarz( 'RIGHT', i-1, n-i+1, l, a( i, n-l+1 ), lda,conjg( tau( i ) ), a( &
                        1_ilp, i ), lda, work )
              a( i, i ) = conjg( alpha )
           end do
           return
