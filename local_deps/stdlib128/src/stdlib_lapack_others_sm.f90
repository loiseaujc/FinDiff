submodule(stdlib_lapack_others) stdlib_lapack_others_sm
  ...
  contains
     ...
     real(qp) module function stdlib_qla_syrpvgrw( uplo, n, info, a, lda, af,ldaf, ipiv, work )
     !! DLA_SYRPVGRW: computes the reciprocal pivot growth factor
     !! norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !! much less than 1, the stability of the LU factorization of the
     !! (equilibrated) matrix A could be poor. This also means that the
     ...
     real(qp) module function stdlib_wla_syrpvgrw( uplo, n, info, a, lda, af,ldaf, ipiv, work )
     !! ZLA_SYRPVGRW: computes the reciprocal pivot growth factor
     !! norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !! much less than 1, the stability of the LU factorization of the
     !! (equilibrated) matrix A could be poor. This also means that the
     ...
     pure real(qp) module function stdlib_qla_gerpvgrw( n, ncols, a, lda, af,ldaf )
     !! DLA_GERPVGRW: computes the reciprocal pivot growth factor
     !! norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !! much less than 1, the stability of the LU factorization of the
     !! (equilibrated) matrix A could be poor. This also means that the
     ...
     pure real(qp) module function stdlib_wla_gerpvgrw( n, ncols, a, lda, af,ldaf )
     !! ZLA_GERPVGRW: computes the reciprocal pivot growth factor
     !! norm(A)/norm(U). The "max absolute element" norm is used. If this is
     !! much less than 1, the stability of the LU factorization of the
     !! (equilibrated) matrix A could be poor. This also means that the
     ...
     real(qp) module function stdlib_wla_gbrcond_c( trans, n, kl, ku, ab,ldab, afb, ldafb, ipiv,c, &
     !! ZLA_GBRCOND_C: Computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector.
               capply, info, work,rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==2_ilp ) then
                 ...
                 if ( notrans ) then
                    call stdlib_wgbtrs( 'NO TRANSPOSE', n, kl, ku, 1_ilp, afb, ldafb,ipiv, work, n, &
                              info )
                 else
                    call stdlib_wgbtrs( 'CONJUGATE TRANSPOSE', n, kl, ku, 1_ilp, afb,ldafb, ipiv, &
                              work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
     ...
     real(qp) module function stdlib_wla_gercond_c( trans, n, a, lda, af,ldaf, ipiv, c, capply,info, &
     !! ZLA_GERCOND_C: computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector.
               work, rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==2_ilp ) then
                 ...
                 if (notrans) then
                    call stdlib_wgetrs( 'NO TRANSPOSE', n, 1_ilp, af, ldaf, ipiv,work, n, info )
                              
                 else
                    call stdlib_wgetrs( 'CONJUGATE TRANSPOSE', n, 1_ilp, af, ldaf, ipiv,work, n, info &
                              )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
     ...
     real(qp) module function stdlib_wla_hercond_c( uplo, n, a, lda, af,ldaf, ipiv, c, capply,info, work,&
     !! ZLA_HERCOND_C: computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector.
                rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==2_ilp ) then
                 ...
                 if ( up ) then
                    call stdlib_whetrs( 'U', n, 1_ilp, af, ldaf, ipiv,work, n, info )
                 else
                    call stdlib_whetrs( 'L', n, 1_ilp, af, ldaf, ipiv,work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
                    do i = 1, n
     ...
     module subroutine stdlib_qla_syamv( uplo, n, alpha, a, lda, x, incx, beta, y,incy )
     !! DLA_SYAMV:  performs the matrix-vector operation
     !! y := alpha*abs(A)*abs(x) + beta*abs(y),
     !! where alpha and beta are scalars, x and y are vectors and A is an
     !! n by n symmetric matrix.
     ...
     !! in computing that entry have at least one zero multiplicand.
        ...
        ! =====================================================================
           ...
           ! set safe1 essentially to be the underflow threshold times the
           ! number of additions in each row.
           safe1 = stdlib_qlamch( 'SAFE MINIMUM' )
           safe1 = (n+1)*safe1
           ! form  y := alpha*abs(a)*abs(x) + beta*abs(y).
     ...
     module subroutine stdlib_wla_syamv( uplo, n, alpha, a, lda, x, incx, beta, y,incy )
     !! ZLA_SYAMV:  performs the matrix-vector operation
     !! y := alpha*abs(A)*abs(x) + beta*abs(y),
     !! where alpha and beta are scalars, x and y are vectors and A is an
     !! n by n symmetric matrix.
     ...
     real(qp) module function stdlib_qla_syrcond( uplo, n, a, lda, af, ldaf,ipiv, cmode, c, info, work,&
     !! DLA_SYRCOND: estimates the Skeel condition number of  op(A) * op2(C)
     !! where op2 is determined by CMODE as follows
     !! CMODE =  1    op2(C) = C
     !! CMODE =  0    op2(C) = I
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==2_ilp ) then
                 ...
                 if ( up ) then
                    call stdlib_qsytrs( 'U', n, 1_ilp, af, ldaf, ipiv, work, n, info )
                 else
                    call stdlib_qsytrs( 'L', n, 1_ilp, af, ldaf, ipiv, work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( cmode == 1_ilp ) then
                    do i = 1, n
     ...
     real(qp) module function stdlib_wla_syrcond_c( uplo, n, a, lda, af,ldaf, ipiv, c, capply,info, work,&
     !! ZLA_SYRCOND_C: Computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector.
                rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==2_ilp ) then
                 ...
                 if ( up ) then
                    call stdlib_wsytrs( 'U', n, 1_ilp, af, ldaf, ipiv,work, n, info )
                 else
                    call stdlib_wsytrs( 'L', n, 1_ilp, af, ldaf, ipiv,work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
                    do i = 1, n
     ...
     real(qp) module function stdlib_wla_porcond_c( uplo, n, a, lda, af,ldaf, c, capply, info,work, &
     !! ZLA_PORCOND_C: Computes the infinity norm condition number of
     !! op(A) * inv(diag(C)) where C is a DOUBLE PRECISION vector
               rwork )
        ! -- lapack computational routine --
        ...
        ! =====================================================================
           ...
           if( kase/=0_ilp ) then
              if( kase==2_ilp ) then
                 ...
                 if ( up ) then
                    call stdlib_wpotrs( 'U', n, 1_ilp, af, ldaf,work, n, info )
                 else
                    call stdlib_wpotrs( 'L', n, 1_ilp, af, ldaf,work, n, info )
                 endif
                 ! multiply by inv(c).
                 if ( capply ) then
                    do i = 1, n
