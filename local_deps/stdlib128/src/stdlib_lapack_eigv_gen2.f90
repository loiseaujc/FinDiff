  ...
  contains
     ...
     !! by the orthogonal matrix Q:  A = Q*H*Q**T = (QZ)*T*(QZ)**T.
        ...
        ! =====================================================================
           ...
           initz = stdlib_lsame( compz, 'I' )
           wantz = initz .or. stdlib_lsame( compz, 'V' )
           work( 1_ilp ) = real( max( 1_ilp, n ),KIND=qp)
           lquery = lwork==-1_ilp
           info = 0_ilp
           if( .not.stdlib_lsame( job, 'E' ) .and. .not.wantt ) then
              info = -1_ilp
           ...
           if( info/=0_ilp ) then
              ...
              ! ==== quick return in case of a workspace query ====
              call stdlib_qlaqr0( wantt, wantz, n, ilo, ihi, h, ldh, wr, wi, ilo,ihi, z, ldz, &
                        work, lwork, info )
              ! ==== ensure reported workspace size is backward-compatible with
              ! .    previous lapack versions. ====
              work( 1_ilp ) = max( real( max( 1_ilp, n ),KIND=qp), work( 1_ilp ) )
              return
           else
              ! ==== copy eigenvalues isolated by stdlib_qgebal ====
              do i = 1, ilo - 1
                 wr( i ) = h( i, i )
                 wi( i ) = zero
              end do
              ...
              ! ==== initialize z, if requested ====
              if( initz )call stdlib_qlaset( 'A', n, n, zero, one, z, ldz )
              ! ==== quick return if possible ====
              if( ilo==ihi ) then
                 wr( ilo ) = h( ilo, ilo )
                 wi( ilo ) = zero
              ...
              if( n>nmin ) then
                 ...
                 ! ==== small matrix ====
                 call stdlib_qlahqr( wantt, wantz, n, ilo, ihi, h, ldh, wr, wi, ilo,ihi, z, ldz, &
                           info )
                 if( info>0_ilp ) then
                    ! ==== a rare stdlib_qlahqr failure!  stdlib_qlaqr0 sometimes succeeds
                    ! .    when stdlib_qlahqr fails. ====
                    kbot = info
                    if( n>=nl ) then
                       ! ==== larger matrices have enough subdiagonal scratch
                       ! .    space to call stdlib_qlaqr0 directly. ====
                       call stdlib_qlaqr0( wantt, wantz, n, ilo, kbot, h, ldh, wr,wi, ilo, ihi, z,&
                                  ldz, work, lwork, info )
                    else
                       ! ==== tiny matrices don't have enough subdiagonal
                       ! .    scratch space to benefit from stdlib_qlaqr0.  hence,
                       ! .    tiny matrices must be copied into a larger
                       ! .    array before calling stdlib_qlaqr0. ====
                       call stdlib_qlacpy( 'A', n, n, h, ldh, hl, nl )
                       hl( n+1, n ) = zero
                       call stdlib_qlaset( 'A', nl, nl-n, zero, zero, hl( 1_ilp, n+1 ),nl )
                       call stdlib_qlaqr0( wantt, wantz, nl, ilo, kbot, hl, nl, wr,wi, ilo, ihi, &
                                 z, ldz, workl, nl, info )
                       if( wantt .or. info/=0_ilp )call stdlib_qlacpy( 'A', n, n, hl, nl, h, ldh )
                                 
                    end if
                 end if
              end if
              ! ==== clear out the trash, if necessary. ====
              if( ( wantt .or. info/=0_ilp ) .and. n>2_ilp )call stdlib_qlaset( 'L', n-2, n-2, zero, zero,&
                         h( 3_ilp, 1_ilp ), ldh )
              ! ==== ensure reported workspace size is backward-compatible with
              ! .    previous lapack versions. ====
              work( 1_ilp ) = max( real( max( 1_ilp, n ),KIND=qp), work( 1_ilp ) )
     ...
     !! by the unitary matrix Q:  A = Q*H*Q**H = (QZ)*T*(QZ)**H.
        ...
        ! =====================================================================
           ...
           initz = stdlib_lsame( compz, 'I' )
           wantz = initz .or. stdlib_lsame( compz, 'V' )
           work( 1_ilp ) = cmplx( real( max( 1_ilp, n ),KIND=qp), zero,KIND=qp)
           lquery = lwork==-1_ilp
           info = 0_ilp
           if( .not.stdlib_lsame( job, 'E' ) .and. .not.wantt ) then
              info = -1_ilp
           ...
           if( info/=0_ilp ) then
              ...
              ! ==== quick return in case of a workspace query ====
              call stdlib_wlaqr0( wantt, wantz, n, ilo, ihi, h, ldh, w, ilo, ihi, z,ldz, work, &
                        lwork, info )
              ! ==== ensure reported workspace size is backward-compatible with
              ! .    previous lapack versions. ====
              work( 1_ilp ) = cmplx( max( real( work( 1_ilp ),KIND=qp), real( max( 1_ilp,n ),KIND=qp) ), &
                        zero,KIND=qp)
              return
           else
              ! ==== copy eigenvalues isolated by stdlib_wgebal ====
              if( ilo>1_ilp )call stdlib_wcopy( ilo-1, h, ldh+1, w, 1_ilp )
              if( ihi<n )call stdlib_wcopy( n-ihi, h( ihi+1, ihi+1 ), ldh+1, w( ihi+1 ), 1_ilp )
              ...
              if( n>nmin ) then
                 ...
                 if( info>0_ilp ) then
                    ...
                    if( n>=nl ) then
                       ! ==== larger matrices have enough subdiagonal scratch
                       ! .    space to call stdlib_wlaqr0 directly. ====
                       call stdlib_wlaqr0( wantt, wantz, n, ilo, kbot, h, ldh, w,ilo, ihi, z, ldz,&
                                  work, lwork, info )
                    else
                       ! ==== tiny matrices don't have enough subdiagonal
                       ! .    scratch space to benefit from stdlib_wlaqr0.  hence,
                       ! .    tiny matrices must be copied into a larger
                       ! .    array before calling stdlib_wlaqr0. ====
                       call stdlib_wlacpy( 'A', n, n, h, ldh, hl, nl )
                       hl( n+1, n ) = czero
                       call stdlib_wlaset( 'A', nl, nl-n, czero, czero, hl( 1_ilp, n+1 ),nl )
                       call stdlib_wlaqr0( wantt, wantz, nl, ilo, kbot, hl, nl, w,ilo, ihi, z, &
                                 ldz, workl, nl, info )
                       if( wantt .or. info/=0_ilp )call stdlib_wlacpy( 'A', n, n, hl, nl, h, ldh )
                                 
                    end if
                 end if
              end if
              ! ==== clear out the trash, if necessary. ====
              if( ( wantt .or. info/=0_ilp ) .and. n>2_ilp )call stdlib_wlaset( 'L', n-2, n-2, czero, &
                        czero, h( 3_ilp, 1_ilp ), ldh )
              ! ==== ensure reported workspace size is backward-compatible with
              ! .    previous lapack versions. ====
              work( 1_ilp ) = cmplx( max( real( max( 1_ilp, n ),KIND=qp),real( work( 1_ilp ),KIND=qp) ), &
                        zero,KIND=qp)
           end if
     ...
     !! where y**h denotes the conjugate transpose of the vector y.
        ...
        ! =====================================================================
           ...
           if( n==0 )return
           ! set machine-dependent constants.
           unfl = stdlib_qlamch( 'SAFE MINIMUM' )
           ...
           loop_120: do k = 1, n
              if( select( k ) ) then
                 ...
                 if( kl/=kln ) then
                    ...
                    ! compute infinity-norm of submatrix h(kl:kr,kl:kr) if it
                    ! has not ben computed before.
                    hnorm = stdlib_qlanhs( 'I', kr-kl+1, h( kl, kl ), ldh, work )
                    if( stdlib_qisnan( hnorm ) ) then
                       info = -6_ilp
                       return
                    else if( hnorm>zero ) then
                       eps3 = hnorm*ulp
                 ...
                 if( leftv ) then
                    ! compute left eigenvector.
                    call stdlib_qlaein( .false., noinit, n-kl+1, h( kl, kl ), ldh,wkr, wki, vl( &
                    kl, ksr ), vl( kl, ksi ),work, ldwork, work( n*n+n+1 ), eps3, smlnum,bignum, &
                              iinfo )
                    if( iinfo>0_ilp ) then
                       if( pair ) then
                 ...
                 if( rightv ) then
                    ! compute right eigenvector.
                    call stdlib_qlaein( .true., noinit, kr, h, ldh, wkr, wki,vr( 1_ilp, ksr ), vr( 1_ilp, &
                              ksi ), work, ldwork,work( n*n+n+1 ), eps3, smlnum, bignum,iinfo )
                    if( iinfo>0_ilp ) then
                       if( pair ) then
                          info = info + 2_ilp
           ...
           loop_100: do k = 1, n
              if( select( k ) ) then
                 ...
                 if( kl/=kln ) then
                    ...
                    ! compute infinity-norm of submatrix h(kl:kr,kl:kr) if it
                    ! has not ben computed before.
                    hnorm = stdlib_wlanhs( 'I', kr-kl+1, h( kl, kl ), ldh, rwork )
                    if( stdlib_qisnan( hnorm ) ) then
                       info = -6_ilp
                       return
                    else if( hnorm>zero ) then
                 ...
                 if( leftv ) then
                    ! compute left eigenvector.
                    call stdlib_wlaein( .false., noinit, n-kl+1, h( kl, kl ), ldh,wk, vl( kl, ks )&
                              , work, ldwork, rwork, eps3,smlnum, iinfo )
                    if( iinfo>0_ilp ) then
                       info = info + 1_ilp
                       ifaill( ks ) = k
                 ...
                 if( rightv ) then
                    ! compute right eigenvector.
                    call stdlib_wlaein( .true., noinit, kr, h, ldh, wk, vr( 1_ilp, ks ),work, ldwork, &
                              rwork, eps3, smlnum, iinfo )
                    if( iinfo>0_ilp ) then
                       info = info + 1_ilp
                       ifailr( ks ) = k
              ...
              loop_140: do ki = n, 1, -1
                 ...
                 if( ip==0_ilp ) then
                    ...
                    loop_60: do j = ki - 1, 1, -1
                       ...
                       if( j1==j2 ) then
                          ! 1-by-1 diagonal block
                          call stdlib_qlaln2( .false., 1_ilp, 1_ilp, smin, one, t( j, j ),ldt, one, one, &
                                    work( j+n ), n, wr,zero, x, 2_ilp, scale, xnorm, ierr )
                          ! scale x(1,1) to avoid overflow when updating
                          ! the right-hand side.
                          if( xnorm>one ) then
                          ...
                          ! scale if necessary
                          if( scale/=one )call stdlib_qscal( ki, scale, work( 1_ilp+n ), 1_ilp )
                          work( j+n ) = x( 1_ilp, 1_ilp )
                          ! update right-hand side
                          call stdlib_qaxpy( j-1, -x( 1_ilp, 1_ilp ), t( 1_ilp, j ), 1_ilp,work( 1_ilp+n ), 1_ilp )
                                    
                       else
                          ! 2-by-2 diagonal block
                          call stdlib_qlaln2( .false., 2_ilp, 1_ilp, smin, one,t( j-1, j-1 ), ldt, one, &
                                    one,work( j-1+n ), n, wr, zero, x, 2_ilp,scale, xnorm, ierr )
                          ! scale x(1,1) and x(2,1) to avoid overflow when
                          ! updating the right-hand side.
                          if( xnorm>one ) then
                          ...
                          work( j+n ) = x( 2_ilp, 1_ilp )
                          ! update right-hand side
                          call stdlib_qaxpy( j-2, -x( 1_ilp, 1_ilp ), t( 1_ilp, j-1 ), 1_ilp,work( 1_ilp+n ), 1_ilp )
                                    
                          call stdlib_qaxpy( j-2, -x( 2_ilp, 1_ilp ), t( 1_ilp, j ), 1_ilp,work( 1_ilp+n ), 1_ilp )
                                    
                       end if
                    end do loop_60
                    ! copy the vector x or q*x to vr and normalize.
                    if( .not.over ) then
                       call stdlib_qcopy( ki, work( 1_ilp+n ), 1_ilp, vr( 1_ilp, is ), 1_ilp )
                       ii = stdlib_iqamax( ki, vr( 1_ilp, is ), 1_ilp )
                       remax = one / abs( vr( ii, is ) )
                       call stdlib_qscal( ki, remax, vr( 1_ilp, is ), 1_ilp )
                       do k = ki + 1, n
                          vr( k, is ) = zero
                       end do
                    else
                       if( ki>1_ilp )call stdlib_qgemv( 'N', n, ki-1, one, vr, ldvr,work( 1_ilp+n ), 1_ilp, &
                                 work( ki+n ),vr( 1_ilp, ki ), 1_ilp )
                       ii = stdlib_iqamax( n, vr( 1_ilp, ki ), 1_ilp )
                       remax = one / abs( vr( ii, ki ) )
                       call stdlib_qscal( n, remax, vr( 1_ilp, ki ), 1_ilp )
                    end if
                 else
                    ! complex right eigenvector.
                    ! initial solve
                    ...
                    loop_90: do j = ki - 2, 1, -1
                       ...
                       if( j1==j2 ) then
                          ! 1-by-1 diagonal block
                          call stdlib_qlaln2( .false., 1_ilp, 2_ilp, smin, one, t( j, j ),ldt, one, one, &
                                    work( j+n ), n, wr, wi,x, 2_ilp, scale, xnorm, ierr )
                          ! scale x(1,1) and x(1,2) to avoid overflow when
                          ! updating the right-hand side.
                          if( xnorm>one ) then
                          ...
                          ! scale if necessary
                          if( scale/=one ) then
                             call stdlib_qscal( ki, scale, work( 1_ilp+n ), 1_ilp )
                             call stdlib_qscal( ki, scale, work( 1_ilp+n2 ), 1_ilp )
                          end if
                          work( j+n ) = x( 1_ilp, 1_ilp )
                          work( j+n2 ) = x( 1_ilp, 2_ilp )
                          ! update the right-hand side
                          ...
                          call stdlib_qaxpy( j-1, -x( 1_ilp, 2_ilp ), t( 1_ilp, j ), 1_ilp,work( 1_ilp+n2 ), 1_ilp )
                                    
                       else
                          ! 2-by-2 diagonal block
                          call stdlib_qlaln2( .false., 2_ilp, 2_ilp, smin, one,t( j-1, j-1 ), ldt, one, &
                                    one,work( j-1+n ), n, wr, wi, x, 2_ilp, scale,xnorm, ierr )
                          ! scale x to avoid overflow when updating
                          ! the right-hand side.
                          if( xnorm>one ) then
                          ...
                          call stdlib_qaxpy( j-2, -x( 1_ilp, 2_ilp ), t( 1_ilp, j-1 ), 1_ilp,work( 1_ilp+n2 ), 1_ilp )
                                    
                          call stdlib_qaxpy( j-2, -x( 2_ilp, 2_ilp ), t( 1_ilp, j ), 1_ilp,work( 1_ilp+n2 ), 1_ilp )
                                    
                       end if
                    end do loop_90
                    ! copy the vector x or q*x to vr and normalize.
                    if( .not.over ) then
                       call stdlib_qcopy( ki, work( 1_ilp+n ), 1_ilp, vr( 1_ilp, is-1 ), 1_ilp )
                       call stdlib_qcopy( ki, work( 1_ilp+n2 ), 1_ilp, vr( 1_ilp, is ), 1_ilp )
                       emax = zero
                       do k = 1, ki
                          emax = max( emax, abs( vr( k, is-1 ) )+abs( vr( k, is ) ) )
                       end do
                       remax = one / emax
                       call stdlib_qscal( ki, remax, vr( 1_ilp, is-1 ), 1_ilp )
                       call stdlib_qscal( ki, remax, vr( 1_ilp, is ), 1_ilp )
                       do k = ki + 1, n
                          vr( k, is-1 ) = zero
                          vr( k, is ) = zero
                       ...
                       if( ki>2_ilp ) then
                          call stdlib_qgemv( 'N', n, ki-2, one, vr, ldvr,work( 1_ilp+n ), 1_ilp, work( ki-&
                                    1_ilp+n ),vr( 1_ilp, ki-1 ), 1_ilp )
                          call stdlib_qgemv( 'N', n, ki-2, one, vr, ldvr,work( 1_ilp+n2 ), 1_ilp, work( &
                                    ki+n2 ),vr( 1_ilp, ki ), 1_ilp )
                       else
                          call stdlib_qscal( n, work( ki-1+n ), vr( 1_ilp, ki-1 ), 1_ilp )
                          call stdlib_qscal( n, work( ki+n2 ), vr( 1_ilp, ki ), 1_ilp )
                       end if
                       emax = zero
                       do k = 1, n
                          emax = max( emax, abs( vr( k, ki-1 ) )+abs( vr( k, ki ) ) )
                       ...
                       remax = one / emax
                       call stdlib_qscal( n, remax, vr( 1_ilp, ki-1 ), 1_ilp )
                       call stdlib_qscal( n, remax, vr( 1_ilp, ki ), 1_ilp )
                    end if
                 end if
                 is = is - 1_ilp
                 ...
                 if( ip==0_ilp ) then
                    ...
                    loop_170: do j = ki + 1, n
                       ...
                       if( j1==j2 ) then
                          ...
                          if( work( j )>vcrit ) then
                             rec = one / vmax
                             call stdlib_qscal( n-ki+1, rec, work( ki+n ), 1_ilp )
                             vmax = one
                             vcrit = bignum
                          end if
                          work( j+n ) = work( j+n ) -stdlib_qdot( j-ki-1, t( ki+1, j ), 1_ilp,work( &
                                    ki+1+n ), 1_ilp )
                          ! solve (t(j,j)-wr)**t*x = work
                          call stdlib_qlaln2( .false., 1_ilp, 1_ilp, smin, one, t( j, j ),ldt, one, one, &
                                    work( j+n ), n, wr,zero, x, 2_ilp, scale, xnorm, ierr )
                          ! scale if necessary
                          if( scale/=one )call stdlib_qscal( n-ki+1, scale, work( ki+n ), 1_ilp )
                          ...
                          work( j+n ) = work( j+n ) -stdlib_qdot( j-ki-1, t( ki+1, j ), 1_ilp,work( &
                                    ki+1+n ), 1_ilp )
                          work( j+1+n ) = work( j+1+n ) -stdlib_qdot( j-ki-1, t( ki+1, j+1 ), 1_ilp,&
                                    work( ki+1+n ), 1_ilp )
                          ! solve
                            ! [t(j,j)-wr   t(j,j+1)     ]**t * x = scale*( work1 )
                            ! [t(j+1,j)    t(j+1,j+1)-wr]                ( work2 )
                          call stdlib_qlaln2( .true., 2_ilp, 1_ilp, smin, one, t( j, j ),ldt, one, one, &
                                    work( j+n ), n, wr,zero, x, 2_ilp, scale, xnorm, ierr )
                          ! scale if necessary
                          if( scale/=one )call stdlib_qscal( n-ki+1, scale, work( ki+n ), 1_ilp )
                    ...
                    ! copy the vector x or q*x to vl and normalize.
                    if( .not.over ) then
                       call stdlib_qcopy( n-ki+1, work( ki+n ), 1_ilp, vl( ki, is ), 1_ilp )
                       ii = stdlib_iqamax( n-ki+1, vl( ki, is ), 1_ilp ) + ki - 1_ilp
                       remax = one / abs( vl( ii, is ) )
                       call stdlib_qscal( n-ki+1, remax, vl( ki, is ), 1_ilp )
                       do k = 1, ki - 1
                          vl( k, is ) = zero
                       end do
                    else
                       if( ki<n )call stdlib_qgemv( 'N', n, n-ki, one, vl( 1_ilp, ki+1 ), ldvl,work( &
                                 ki+1+n ), 1_ilp, work( ki+n ),vl( 1_ilp, ki ), 1_ilp )
                       ii = stdlib_iqamax( n, vl( 1_ilp, ki ), 1_ilp )
                       remax = one / abs( vl( ii, ki ) )
                       call stdlib_qscal( n, remax, vl( 1_ilp, ki ), 1_ilp )
                    end if
                 else
                    ! complex left eigenvector.
                     ! initial solve:
                    ...
                    loop_200: do j = ki + 2, n
                       ...
                       if( j1==j2 ) then
                          ...
                          if( work( j )>vcrit ) then
                             rec = one / vmax
                             call stdlib_qscal( n-ki+1, rec, work( ki+n ), 1_ilp )
                             call stdlib_qscal( n-ki+1, rec, work( ki+n2 ), 1_ilp )
                             vmax = one
                             vcrit = bignum
                          end if
                          work( j+n ) = work( j+n ) -stdlib_qdot( j-ki-2, t( ki+2, j ), 1_ilp,work( &
                                    ki+2+n ), 1_ilp )
                          work( j+n2 ) = work( j+n2 ) -stdlib_qdot( j-ki-2, t( ki+2, j ), 1_ilp,work( &
                                    ki+2+n2 ), 1_ilp )
                          ! solve (t(j,j)-(wr-i*wi))*(x11+i*x12)= wk+i*wk2
                          call stdlib_qlaln2( .false., 1_ilp, 2_ilp, smin, one, t( j, j ),ldt, one, one, &
                                    work( j+n ), n, wr,-wi, x, 2_ilp, scale, xnorm, ierr )
                          ! scale if necessary
                          if( scale/=one ) then
                             call stdlib_qscal( n-ki+1, scale, work( ki+n ), 1_ilp )
                             call stdlib_qscal( n-ki+1, scale, work( ki+n2 ), 1_ilp )
                          end if
                          work( j+n ) = x( 1_ilp, 1_ilp )
                          work( j+n2 ) = x( 1_ilp, 2_ilp )
                          vmax = max( abs( work( j+n ) ),abs( work( j+n2 ) ), vmax )
                          ...
                          work( j+n2 ) = work( j+n2 ) -stdlib_qdot( j-ki-2, t( ki+2, j ), 1_ilp,work( &
                                    ki+2+n2 ), 1_ilp )
                          work( j+1+n ) = work( j+1+n ) -stdlib_qdot( j-ki-2, t( ki+2, j+1 ), 1_ilp,&
                                    work( ki+2+n ), 1_ilp )
                          work( j+1+n2 ) = work( j+1+n2 ) -stdlib_qdot( j-ki-2, t( ki+2, j+1 ), 1_ilp,&
                                    work( ki+2+n2 ), 1_ilp )
                          ! solve 2-by-2 complex linear equation
                            ! ([t(j,j)   t(j,j+1)  ]**t-(wr-i*wi)*i)*x = scale*b
                            ! ([t(j+1,j) t(j+1,j+1)]               )
                          call stdlib_qlaln2( .true., 2_ilp, 2_ilp, smin, one, t( j, j ),ldt, one, one, &
                                    work( j+n ), n, wr,-wi, x, 2_ilp, scale, xnorm, ierr )
                          ! scale if necessary
                          if( scale/=one ) then
                             call stdlib_qscal( n-ki+1, scale, work( ki+n ), 1_ilp )
                    ...
                    if( .not.over ) then
                       call stdlib_qcopy( n-ki+1, work( ki+n ), 1_ilp, vl( ki, is ), 1_ilp )
                       call stdlib_qcopy( n-ki+1, work( ki+n2 ), 1_ilp, vl( ki, is+1 ),1_ilp )
                       emax = zero
                       do k = ki, n
                          emax = max( emax, abs( vl( k, is ) )+abs( vl( k, is+1 ) ) )
                       end do
                       remax = one / emax
                       call stdlib_qscal( n-ki+1, remax, vl( ki, is ), 1_ilp )
                       call stdlib_qscal( n-ki+1, remax, vl( ki, is+1 ), 1_ilp )
                       do k = 1, ki - 1
                          vl( k, is ) = zero
                          vl( k, is+1 ) = zero
                       end do
                       ...
                       if( ki<n-1 ) then
                          call stdlib_qgemv( 'N', n, n-ki-1, one, vl( 1_ilp, ki+2 ),ldvl, work( ki+2+&
                                    n ), 1_ilp, work( ki+n ),vl( 1_ilp, ki ), 1_ilp )
                          call stdlib_qgemv( 'N', n, n-ki-1, one, vl( 1_ilp, ki+2 ),ldvl, work( ki+2+&
                                    n2 ), 1_ilp,work( ki+1+n2 ), vl( 1_ilp, ki+1 ), 1_ilp )
                       else
                          call stdlib_qscal( n, work( ki+n ), vl( 1_ilp, ki ), 1_ilp )
                          call stdlib_qscal( n, work( ki+1+n2 ), vl( 1_ilp, ki+1 ), 1_ilp )
                       end if
                       emax = zero
                       do k = 1, n
                          emax = max( emax, abs( vl( k, ki ) )+abs( vl( k, ki+1 ) ) )
                       ...
                       remax = one / emax
                       call stdlib_qscal( n, remax, vl( 1_ilp, ki ), 1_ilp )
                       call stdlib_qscal( n, remax, vl( 1_ilp, ki+1 ), 1_ilp )
                    end if
                 end if
                 is = is + 1_ilp
                 if( ip/=0_ilp )is = is + 1_ilp
     ...
     !! eigenvectors of A.
        ...
        ! =====================================================================
           ...
           rwork( 1_ilp ) = zero
           do j = 2, n
              rwork( j ) = stdlib_qzasum( j-1, t( 1_ilp, j ), 1_ilp )
           end do
           if( rightv ) then
              ! compute right eigenvectors.
              is = m
              loop_80: do ki = n, 1, -1
                 ...
                 if( ki>1_ilp ) then
                    call stdlib_wlatrs( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', 'Y',ki-1, t, ldt, &
                              work( 1_ilp ), scale, rwork,info )
                    work( ki ) = scale
                 end if
                 ! copy the vector x or q*x to vr and normalize.
                 if( .not.over ) then
                    call stdlib_wcopy( ki, work( 1_ilp ), 1_ilp, vr( 1_ilp, is ), 1_ilp )
                    ii = stdlib_iwamax( ki, vr( 1_ilp, is ), 1_ilp )
                    remax = one / cabs1( vr( ii, is ) )
                    call stdlib_wdscal( ki, remax, vr( 1_ilp, is ), 1_ilp )
                    do k = ki + 1, n
                       vr( k, is ) = cmzero
                    end do
                 else
                    if( ki>1_ilp )call stdlib_wgemv( 'N', n, ki-1, cmone, vr, ldvr, work( 1_ilp ),1_ilp, &
                              cmplx( scale,KIND=qp), vr( 1_ilp, ki ), 1_ilp )
                    ii = stdlib_iwamax( n, vr( 1_ilp, ki ), 1_ilp )
                    remax = one / cabs1( vr( ii, ki ) )
                    call stdlib_wdscal( n, remax, vr( 1_ilp, ki ), 1_ilp )
                 end if
                 ! set back the original diagonal elements of t.
                 do k = 1, ki - 1
                    t( k, k ) = work( k+n )
           ...
           if( leftv ) then
              ...
              loop_130: do ki = 1, n
                 ...
                 if( ki<n ) then
                    call stdlib_wlatrs( 'UPPER', 'CONJUGATE TRANSPOSE', 'NON-UNIT','Y', n-ki, t( &
                              ki+1, ki+1 ), ldt,work( ki+1 ), scale, rwork, info )
                    work( ki ) = scale
                 end if
                 ! copy the vector x or q*x to vl and normalize.
                 if( .not.over ) then
                    call stdlib_wcopy( n-ki+1, work( ki ), 1_ilp, vl( ki, is ), 1_ilp )
                    ii = stdlib_iwamax( n-ki+1, vl( ki, is ), 1_ilp ) + ki - 1_ilp
                    remax = one / cabs1( vl( ii, is ) )
                    call stdlib_wdscal( n-ki+1, remax, vl( ki, is ), 1_ilp )
                    do k = 1, ki - 1
                       vl( k, is ) = cmzero
                    end do
                 else
                    if( ki<n )call stdlib_wgemv( 'N', n, n-ki, cmone, vl( 1_ilp, ki+1 ), ldvl,work( &
                              ki+1 ), 1_ilp, cmplx( scale,KIND=qp),vl( 1_ilp, ki ), 1_ilp )
                    ii = stdlib_iwamax( n, vl( 1_ilp, ki ), 1_ilp )
                    remax = one / cabs1( vl( ii, ki ) )
                    call stdlib_wdscal( n, remax, vl( 1_ilp, ki ), 1_ilp )
                 end if
                 ! set back the original diagonal elements of t.
                 do k = ki + 1, n
                    t( k, k ) = work( k+n )
     ...
     !! This uses a Level 3 BLAS version of the back transformation.
        ...
        ! =====================================================================
           ...
           if( over .and. lwork >= n + 2_ilp*n*nbmin ) then
              nb = (lwork - n) / (2_ilp*n)
              nb = min( nb, nbmax )
              call stdlib_qlaset( 'F', n, 1_ilp+2*nb, zero, zero, work, n )
           else
              nb = 1_ilp
           end if
           ! set the constants to control overflow.
           ...
           if( rightv ) then
              ...
              loop_140: do ki = n, 1, -1
                 ...
                 if( ip==0_ilp ) then
                    ...
                    loop_60: do j = ki - 1, 1, -1
                       ...
                       if( j1==j2 ) then
                          ...
                          ! scale if necessary
                          if( scale/=one )call stdlib_qscal( ki, scale, work( 1_ilp+iv*n ), 1_ilp )
                                    
                          work( j+iv*n ) = x( 1_ilp, 1_ilp )
                          ! update right-hand side
                          call stdlib_qaxpy( j-1, -x( 1_ilp, 1_ilp ), t( 1_ilp, j ), 1_ilp,work( 1_ilp+iv*n ), 1_ilp )
                          ...
                          work( j  +iv*n ) = x( 2_ilp, 1_ilp )
                          ! update right-hand side
                          call stdlib_qaxpy( j-2, -x( 1_ilp, 1_ilp ), t( 1_ilp, j-1 ), 1_ilp,work( 1_ilp+iv*n ), 1_ilp )
                                    
                          call stdlib_qaxpy( j-2, -x( 2_ilp, 1_ilp ), t( 1_ilp, j ), 1_ilp,work( 1_ilp+iv*n ), 1_ilp )
                    ...
                    if( .not.over ) then
                       ! ------------------------------
                       ! no back-transform: copy x to vr and normalize.
                       call stdlib_qcopy( ki, work( 1_ilp + iv*n ), 1_ilp, vr( 1_ilp, is ), 1_ilp )
                       ii = stdlib_iqamax( ki, vr( 1_ilp, is ), 1_ilp )
                       remax = one / abs( vr( ii, is ) )
                       call stdlib_qscal( ki, remax, vr( 1_ilp, is ), 1_ilp )
                       do k = ki + 1, n
                       ...
                       ! ------------------------------
                       ! version 1: back-transform each vector with gemv, q*x.
                       if( ki>1_ilp )call stdlib_qgemv( 'N', n, ki-1, one, vr, ldvr,work( 1_ilp + iv*n ), &
                                 1_ilp, work( ki + iv*n ),vr( 1_ilp, ki ), 1_ilp )
                       ii = stdlib_iqamax( n, vr( 1_ilp, ki ), 1_ilp )
                       remax = one / abs( vr( ii, ki ) )
                       call stdlib_qscal( n, remax, vr( 1_ilp, ki ), 1_ilp )
                    ...
                    loop_90: do j = ki - 2, 1, -1
                       ...
                       if( j1==j2 ) then
                          ...
                          ! scale if necessary
                          if( scale/=one ) then
                             call stdlib_qscal( ki, scale, work( 1_ilp+(iv-1)*n ), 1_ilp )
                             call stdlib_qscal( ki, scale, work( 1_ilp+(iv  )*n ), 1_ilp )
                          end if
                          work( j+(iv-1)*n ) = x( 1_ilp, 1_ilp )
                          work( j+(iv  )*n ) = x( 1_ilp, 2_ilp )
                          ! update the right-hand side
                          call stdlib_qaxpy( j-1, -x( 1_ilp, 1_ilp ), t( 1_ilp, j ), 1_ilp,work( 1_ilp+(iv-1)*n ), 1_ilp )
                                    
                          call stdlib_qaxpy( j-1, -x( 1_ilp, 2_ilp ), t( 1_ilp, j ), 1_ilp,work( 1_ilp+(iv  )*n ), 1_ilp )
                          ...
                          work( j  +(iv  )*n ) = x( 2_ilp, 2_ilp )
                          ! update the right-hand side
                          call stdlib_qaxpy( j-2, -x( 1_ilp, 1_ilp ), t( 1_ilp, j-1 ), 1_ilp,work( 1_ilp+(iv-1)*n   ),&
                                     1_ilp )
                          call stdlib_qaxpy( j-2, -x( 2_ilp, 1_ilp ), t( 1_ilp, j ), 1_ilp,work( 1_ilp+(iv-1)*n   ), &
                                    1_ilp )
                          call stdlib_qaxpy( j-2, -x( 1_ilp, 2_ilp ), t( 1_ilp, j-1 ), 1_ilp,work( 1_ilp+(iv  )*n ), &
                                    1_ilp )
                          call stdlib_qaxpy( j-2, -x( 2_ilp, 2_ilp ), t( 1_ilp, j ), 1_ilp,work( 1_ilp+(iv  )*n ), 1_ilp )
                    ...
                    if( .not.over ) then
                       ! ------------------------------
                       ! no back-transform: copy x to vr and normalize.
                       call stdlib_qcopy( ki, work( 1_ilp+(iv-1)*n ), 1_ilp, vr(1_ilp,is-1), 1_ilp )
                       call stdlib_qcopy( ki, work( 1_ilp+(iv  )*n ), 1_ilp, vr(1_ilp,is  ), 1_ilp )
                       emax = zero
                       do k = 1, ki
                          emax = max( emax, abs( vr( k, is-1 ) )+abs( vr( k, is   ) ) )
                       ...
                       ! version 1: back-transform each vector with gemv, q*x.
                       if( ki>2_ilp ) then
                          call stdlib_qgemv( 'N', n, ki-2, one, vr, ldvr,work( 1_ilp    + (iv-1)*n ), &
                                    1_ilp,work( ki-1 + (iv-1)*n ), vr(1_ilp,ki-1), 1_ilp)
                          call stdlib_qgemv( 'N', n, ki-2, one, vr, ldvr,work( 1_ilp  + (iv)*n ), 1_ilp,&
                                    work( ki + (iv)*n ), vr( 1_ilp, ki ), 1_ilp )
                       else
                          call stdlib_qscal( n, work(ki-1+(iv-1)*n), vr(1_ilp,ki-1), 1_ilp)
                          call stdlib_qscal( n, work(ki  +(iv  )*n), vr(1_ilp,ki  ), 1_ilp)
                       end if
                       emax = zero
                       do k = 1, n
                          emax = max( emax, abs( vr( k, ki-1 ) )+abs( vr( k, ki   ) ) )
                 ...
                 if( nb>1_ilp ) then
                    ...
                    ! or if this was last vector, do the gemm
                    if( (iv<=2_ilp) .or. (ki2==1_ilp) ) then
                       call stdlib_qgemm( 'N', 'N', n, nb-iv+1, ki2+nb-iv, one,vr, ldvr,work( 1_ilp + &
                                 (iv)*n    ), n,zero,work( 1_ilp + (nb+iv)*n ), n )
                       ! normalize vectors
                       do k = iv, nb
                          if( iscomplex(k)==0_ilp ) then
                             ! real eigenvector
                             ii = stdlib_iqamax( n, work( 1_ilp + (nb+k)*n ), 1_ilp )
                             remax = one / abs( work( ii + (nb+k)*n ) )
                          else if( iscomplex(k)==1_ilp ) then
                             ! first eigenvector of conjugate pair
                             emax = zero
                          ...
                          call stdlib_qscal( n, remax, work( 1_ilp + (nb+k)*n ), 1_ilp )
                       end do
                       call stdlib_qlacpy( 'F', n, nb-iv+1,work( 1_ilp + (nb+iv)*n ), n,vr( 1_ilp, ki2 ), &
                                 ldvr )
                       iv = nb
                    else
                       iv = iv - 1_ilp
           ...
           if( leftv ) then
              ...
              loop_260: do ki = 1, n
                 ...
                 if( ip==0_ilp ) then
                    ...
                    loop_170: do j = ki + 1, n
                       ...
                       if( j1==j2 ) then
                          ...
                          if( work( j )>vcrit ) then
                             rec = one / vmax
                             call stdlib_qscal( n-ki+1, rec, work( ki+iv*n ), 1_ilp )
                             vmax = one
                             vcrit = bignum
                          end if
                          work( j+iv*n ) = work( j+iv*n ) -stdlib_qdot( j-ki-1, t( ki+1, j ), 1_ilp,&
                                    work( ki+1+iv*n ), 1_ilp )
                          ! solve [ t(j,j) - wr ]**t * x = work
                          call stdlib_qlaln2( .false., 1_ilp, 1_ilp, smin, one, t( j, j ),ldt, one, one, &
                                    work( j+iv*n ), n, wr,zero, x, 2_ilp, scale, xnorm, ierr )
                          ! scale if necessary
                          if( scale/=one )call stdlib_qscal( n-ki+1, scale, work( ki+iv*n ), 1_ilp )
                          ...
                          work( j+iv*n ) = work( j+iv*n ) -stdlib_qdot( j-ki-1, t( ki+1, j ), 1_ilp,&
                                    work( ki+1+iv*n ), 1_ilp )
                          work( j+1+iv*n ) = work( j+1+iv*n ) -stdlib_qdot( j-ki-1, t( ki+1, j+1 )&
                                    , 1_ilp,work( ki+1+iv*n ), 1_ilp )
                          ! solve
                          ! [ t(j,j)-wr   t(j,j+1)      ]**t * x = scale*( work1 )
                          ! [ t(j+1,j)    t(j+1,j+1)-wr ]                ( work2 )
                    ...
                    if( .not.over ) then
                       ! ------------------------------
                       ! no back-transform: copy x to vl and normalize.
                       call stdlib_qcopy( n-ki+1, work( ki + iv*n ), 1_ilp,vl( ki, is ), 1_ilp )
                       ii = stdlib_iqamax( n-ki+1, vl( ki, is ), 1_ilp ) + ki - 1_ilp
                       remax = one / abs( vl( ii, is ) )
                       call stdlib_qscal( n-ki+1, remax, vl( ki, is ), 1_ilp )
                       do k = 1, ki - 1
                    ...
                    loop_200: do j = ki + 2, n
                       ...
                       if( j1==j2 ) then
                          ...
                          if( work( j )>vcrit ) then
                             rec = one / vmax
                             call stdlib_qscal( n-ki+1, rec, work(ki+(iv  )*n), 1_ilp )
                             call stdlib_qscal( n-ki+1, rec, work(ki+(iv+1)*n), 1_ilp )
                             vmax = one
                             vcrit = bignum
                          end if
                          work( j+(iv  )*n ) = work( j+(iv)*n ) -stdlib_qdot( j-ki-2, t( ki+2, j )&
                                    , 1_ilp,work( ki+2+(iv)*n ), 1_ilp )
                          work( j+(iv+1)*n ) = work( j+(iv+1)*n ) -stdlib_qdot( j-ki-2, t( ki+2, &
                                    j ), 1_ilp,work( ki+2+(iv+1)*n ), 1_ilp )
                          ! solve [ t(j,j)-(wr-i*wi) ]*(x11+i*x12)= wk+i*wk2
                          call stdlib_qlaln2( .false., 1_ilp, 2_ilp, smin, one, t( j, j ),ldt, one, one, &
                                    work( j+iv*n ), n, wr,-wi, x, 2_ilp, scale, xnorm, ierr )
                          ! scale if necessary
                          if( scale/=one ) then
                             call stdlib_qscal( n-ki+1, scale, work(ki+(iv  )*n), 1_ilp)
                             call stdlib_qscal( n-ki+1, scale, work(ki+(iv+1)*n), 1_ilp)
                          end if
                          work( j+(iv  )*n ) = x( 1_ilp, 1_ilp )
                          work( j+(iv+1)*n ) = x( 1_ilp, 2_ilp )
                          vmax = max( abs( work( j+(iv  )*n ) ),abs( work( j+(iv+1)*n ) ), vmax )
                          ...
                          work( j  +(iv  )*n ) = work( j+(iv)*n ) -stdlib_qdot( j-ki-2, t( ki+2, &
                                    j ), 1_ilp,work( ki+2+(iv)*n ), 1_ilp )
                          work( j  +(iv+1)*n ) = work( j+(iv+1)*n ) -stdlib_qdot( j-ki-2, t( ki+2,&
                                     j ), 1_ilp,work( ki+2+(iv+1)*n ), 1_ilp )
                          work( j+1+(iv  )*n ) = work( j+1+(iv)*n ) -stdlib_qdot( j-ki-2, t( ki+2,&
                                     j+1 ), 1_ilp,work( ki+2+(iv)*n ), 1_ilp )
                          work( j+1+(iv+1)*n ) = work( j+1+(iv+1)*n ) -stdlib_qdot( j-ki-2, t( ki+&
                                    2_ilp, j+1 ), 1_ilp,work( ki+2+(iv+1)*n ), 1_ilp )
                          ! solve 2-by-2 complex linear equation
                          ! [ (t(j,j)   t(j,j+1)  )**t - (wr-i*wi)*i ]*x = scale*b
                          ! [ (t(j+1,j) t(j+1,j+1))                  ]
                    ...
                    if( .not.over ) then
                       ...
                       call stdlib_qcopy( n-ki+1, work( ki + (iv+1)*n ), 1_ilp,vl( ki, is+1 ), 1_ilp )
                       ...
                       if( ki<n-1 ) then
                          ...
                          call stdlib_qscal( n, work(ki+  (iv  )*n), vl(1_ilp, ki  ), 1_ilp)
                          call stdlib_qscal( n, work(ki+1+(iv+1)*n), vl(1_ilp, ki+1), 1_ilp)
                       end if
                       emax = zero
                       do k = 1, n
                          emax = max( emax, abs( vl( k, ki   ) )+abs( vl( k, ki+1 ) ) )
                 ...
                 if( nb>1_ilp ) then
                    ...
                    ! or if this was last vector, do the gemm
                    if( (iv>=nb-1) .or. (ki2==n) ) then
                       call stdlib_qgemm( 'N', 'N', n, iv, n-ki2+iv, one,vl( 1_ilp, ki2-iv+1 ), ldvl,&
                                 work( ki2-iv+1 + (1_ilp)*n ), n,zero,work( 1_ilp + (nb+1)*n ), n )
                       ! normalize vectors
                       do k = 1, iv
                          if( iscomplex(k)==0_ilp) then
                       ...
                       call stdlib_qlacpy( 'F', n, iv,work( 1_ilp + (nb+1)*n ), n,vl( 1_ilp, ki2-iv+1 ), &
                                 ldvl )
                       iv = 1_ilp
                    else
                       iv = iv + 1_ilp
     ...
     !! This uses a Level 3 BLAS version of the back transformation.
        ...
        ! =====================================================================
           ...
           if( over .and. lwork >= n + 2_ilp*n*nbmin ) then
              nb = (lwork - n) / (2_ilp*n)
              nb = min( nb, nbmax )
              call stdlib_wlaset( 'F', n, 1_ilp+2*nb, czero, czero, work, n )
           else
              nb = 1_ilp
           end if
           ! set the constants to control overflow.
           ...
           if( rightv ) then
              ...
              loop_80: do ki = n, 1, -1
                 ...
                 if( .not.over ) then
                    ! ------------------------------
                    ! no back-transform: copy x to vr and normalize.
                    call stdlib_wcopy( ki, work( 1_ilp + iv*n ), 1_ilp, vr( 1_ilp, is ), 1_ilp )
                    ii = stdlib_iwamax( ki, vr( 1_ilp, is ), 1_ilp )
                    remax = one / cabs1( vr( ii, is ) )
                    call stdlib_wdscal( ki, remax, vr( 1_ilp, is ), 1_ilp )
                    do k = ki + 1, n
                    ...
                    ! ------------------------------
                    ! version 1: back-transform each vector with gemv, q*x.
                    if( ki>1_ilp )call stdlib_wgemv( 'N', n, ki-1, cone, vr, ldvr,work( 1_ilp + iv*n ), 1_ilp,&
                               cmplx( scale,KIND=qp),vr( 1_ilp, ki ), 1_ilp )
                    ii = stdlib_iwamax( n, vr( 1_ilp, ki ), 1_ilp )
                    remax = one / cabs1( vr( ii, ki ) )
                    call stdlib_wdscal( n, remax, vr( 1_ilp, ki ), 1_ilp )
                    ...
                    ! or if this was last vector, do the gemm
                    if( (iv==1_ilp) .or. (ki==1_ilp) ) then
                       call stdlib_wgemm( 'N', 'N', n, nb-iv+1, ki+nb-iv, cone,vr, ldvr,work( 1_ilp + &
                                 (iv)*n    ), n,czero,work( 1_ilp + (nb+iv)*n ), n )
                       ! normalize vectors
                       do k = iv, nb
                          ii = stdlib_iwamax( n, work( 1_ilp + (nb+k)*n ), 1_ilp )
                          remax = one / cabs1( work( ii + (nb+k)*n ) )
                          call stdlib_wdscal( n, remax, work( 1_ilp + (nb+k)*n ), 1_ilp )
                       end do
                       call stdlib_wlacpy( 'F', n, nb-iv+1,work( 1_ilp + (nb+iv)*n ), n,vr( 1_ilp, ki ), &
                                 ldvr )
                       iv = nb
                    else
                       iv = iv - 1_ilp
           ...
           if( leftv ) then
              ...
              loop_130: do ki = 1, n
                 ...
                 if( .not.over ) then
                    ! ------------------------------
                    ! no back-transform: copy x to vl and normalize.
                    call stdlib_wcopy( n-ki+1, work( ki + iv*n ), 1_ilp, vl(ki,is), 1_ilp )
                    ii = stdlib_iwamax( n-ki+1, vl( ki, is ), 1_ilp ) + ki - 1_ilp
                    remax = one / cabs1( vl( ii, is ) )
                    call stdlib_wdscal( n-ki+1, remax, vl( ki, is ), 1_ilp )
                    do k = 1, ki - 1
                    ...
                    ! ------------------------------
                    ! version 1: back-transform each vector with gemv, q*x.
                    if( ki<n )call stdlib_wgemv( 'N', n, n-ki, cone, vl( 1_ilp, ki+1 ), ldvl,work( ki+&
                              1_ilp + iv*n ), 1_ilp, cmplx( scale,KIND=qp),vl( 1_ilp, ki ), 1_ilp )
                    ii = stdlib_iwamax( n, vl( 1_ilp, ki ), 1_ilp )
                    remax = one / cabs1( vl( ii, ki ) )
                    call stdlib_wdscal( n, remax, vl( 1_ilp, ki ), 1_ilp )
                 else
                    ...
                    ! or if this was last vector, do the gemm
                    if( (iv==nb) .or. (ki==n) ) then
                       call stdlib_wgemm( 'N', 'N', n, iv, n-ki+iv, cone,vl( 1_ilp, ki-iv+1 ), ldvl,&
                                 work( ki-iv+1 + (1_ilp)*n ), n,czero,work( 1_ilp + (nb+1)*n ), n )
                       ! normalize vectors
                       do k = 1, iv
                          ii = stdlib_iwamax( n, work( 1_ilp + (nb+k)*n ), 1_ilp )
                       ...
                       call stdlib_wlacpy( 'F', n, iv,work( 1_ilp + (nb+1)*n ), n,vl( 1_ilp, ki-iv+1 ), &
                                 ldvl )
                       iv = 1_ilp
                    else
                       iv = iv + 1_ilp
     ...
     !! by a reasonable factor.  (See BIGNUM.)
       ...
       ! =====================================================================
           ...
           logical(lk) :: rswap(4_ilp), zswap(4_ilp)
           integer(ilp) :: ipivot(4_ilp,4_ilp)
           real(qp) :: ci(2_ilp,2_ilp), civ(4_ilp), cr(2_ilp,2_ilp), crv(4_ilp)
           ! Intrinsic Functions 
           ! Equivalences 
           equivalence        ( ci( 1_ilp, 1_ilp ), civ( 1_ilp ) ),( cr( 1_ilp, 1_ilp ), crv( 1_ilp ) )
           ! Data Statements 
           ...
           ! Executable Statements 
           ! compute bignum
           smlnum = two*stdlib_qlamch( 'SAFE MINIMUM' )
           bignum = one / smlnum
           smini = max( smin, smlnum )
           ! don't check for input errors
           info = 0_ilp
           ...
           if( na==1_ilp ) then
              ...
              if( nw==1_ilp ) then
                 ...
                 ! compute x
                 call stdlib_qladiv( scale*b( 1_ilp, 1_ilp ), scale*b( 1_ilp, 2_ilp ), csr, csi,x( 1_ilp, 1_ilp ), x( 1_ilp, &
                           2_ilp ) )
                 xnorm = abs( x( 1_ilp, 1_ilp ) ) + abs( x( 1_ilp, 2_ilp ) )
              end if
           else
              ...
              if( nw==1_ilp ) then
                 ...
                 call stdlib_qladiv( br2, bi2, ur22, ui22, xr2, xi2 )
                 xr1 = ur11r*br1 - ui11r*bi1 - ur12s*xr2 + ui12s*xi2
                 xi1 = ui11r*br1 + ur11r*bi1 - ui12s*xr2 - ur12s*xi2
                 if( zswap( icmax ) ) then
                    x( 1_ilp, 1_ilp ) = xr2
     ...
     !! off-diagonal elements of opposite sign.
        ...
        ! =====================================================================
           ...
           real(qp) :: a11, bignum, da11, db, eps, scaloc, sgn, smin, smlnum, suml, sumr, &
                     xnorm
           ! Local Arrays 
           real(qp) :: dum(1_ilp), vec(2_ilp,2_ilp), x(2_ilp,2_ilp)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! decode and test input parameters
           notrna = stdlib_lsame( trana, 'N' )
           ...
           ! set constants to control overflow
           eps = stdlib_qlamch( 'P' )
           smlnum = stdlib_qlamch( 'S' )
           bignum = one / smlnum
           call stdlib_qlabad( smlnum, bignum )
           smlnum = smlnum*real( m*n,KIND=qp) / eps
           bignum = one / smlnum
           smin = max( smlnum, eps*stdlib_qlange( 'M', m, m, a, lda, dum ),eps*stdlib_qlange( 'M',&
                      n, n, b, ldb, dum ) )
           sgn = isgn
           if( notrna .and. notrnb ) then
              ! solve    a*x + isgn*x*b = scale*c.
              ...
              loop_60: do l = 1, n
                 ...
                 loop_50: do k = m, 1, -1
                    ...
                    if( l1==l2 .and. k1==k2 ) then
                       suml = stdlib_qdot( m-k1, a( k1, min( k1+1, m ) ), lda,c( min( k1+1, m ), &
                                 l1 ), 1_ilp )
                       sumr = stdlib_qdot( l1-1, c( k1, 1_ilp ), ldc, b( 1_ilp, l1 ), 1_ilp )
                       vec( 1_ilp, 1_ilp ) = c( k1, l1 ) - ( suml+sgn*sumr )
                       scaloc = one
                       a11 = a( k1, k1 ) + sgn*b( l1, l1 )
                       da11 = abs( a11 )
                       ...
                       if( scaloc/=one ) then
                          do j = 1, n
                             call stdlib_qscal( m, scaloc, c( 1_ilp, j ), 1_ilp )
                          end do
                          scale = scale*scaloc
                       end if
                       c( k1, l1 ) = x( 1_ilp, 1_ilp )
                    else if( l1==l2 .and. k1/=k2 ) then
                       suml = stdlib_qdot( m-k2, a( k1, min( k2+1, m ) ), lda,c( min( k2+1, m ), &
                                 l1 ), 1_ilp )
                       sumr = stdlib_qdot( l1-1, c( k1, 1_ilp ), ldc, b( 1_ilp, l1 ), 1_ilp )
                       vec( 1_ilp, 1_ilp ) = c( k1, l1 ) - ( suml+sgn*sumr )
                       suml = stdlib_qdot( m-k2, a( k2, min( k2+1, m ) ), lda,c( min( k2+1, m ), &
                                 l1 ), 1_ilp )
                       sumr = stdlib_qdot( l1-1, c( k2, 1_ilp ), ldc, b( 1_ilp, l1 ), 1_ilp )
                       vec( 2_ilp, 1_ilp ) = c( k2, l1 ) - ( suml+sgn*sumr )
                       call stdlib_qlaln2( .false., 2_ilp, 1_ilp, smin, one, a( k1, k1 ),lda, one, one, &
                                 vec, 2_ilp, -sgn*b( l1, l1 ),zero, x, 2_ilp, scaloc, xnorm, ierr )
                       if( ierr/=0_ilp )info = 1_ilp
                       if( scaloc/=one ) then
                          do j = 1, n
                       ...
                       suml = stdlib_qdot( m-k1, a( k1, min( k1+1, m ) ), lda,c( min( k1+1, m ), &
                                 l2 ), 1_ilp )
                       sumr = stdlib_qdot( l1-1, c( k1, 1_ilp ), ldc, b( 1_ilp, l2 ), 1_ilp )
                       vec( 2_ilp, 1_ilp ) = sgn*( c( k1, l2 )-( suml+sgn*sumr ) )
                       call stdlib_qlaln2( .true., 2_ilp, 1_ilp, smin, one, b( l1, l1 ),ldb, one, one, &
                                 vec, 2_ilp, -sgn*a( k1, k1 ),zero, x, 2_ilp, scaloc, xnorm, ierr )
                       if( ierr/=0_ilp )info = 1_ilp
                       if( scaloc/=one ) then
                          do j = 1, n
                       ...
                       suml = stdlib_qdot( m-k2, a( k2, min( k2+1, m ) ), lda,c( min( k2+1, m ), &
                                 l2 ), 1_ilp )
                       sumr = stdlib_qdot( l1-1, c( k2, 1_ilp ), ldc, b( 1_ilp, l2 ), 1_ilp )
                       vec( 2_ilp, 2_ilp ) = c( k2, l2 ) - ( suml+sgn*sumr )
                       call stdlib_qlasy2( .false., .false., isgn, 2_ilp, 2_ilp,a( k1, k1 ), lda, b( l1, &
                                 l1 ), ldb, vec,2_ilp, scaloc, x, 2_ilp, xnorm, ierr )
                       if( ierr/=0_ilp )info = 1_ilp
                       if( scaloc/=one ) then
                          do j = 1, n
              ...
              loop_120: do l = 1, n
                 ...
                 loop_110: do k = 1, m
                    ...
                    if( l1==l2 .and. k1==k2 ) then
                       suml = stdlib_qdot( k1-1, a( 1_ilp, k1 ), 1_ilp, c( 1_ilp, l1 ), 1_ilp )
                       sumr = stdlib_qdot( l1-1, c( k1, 1_ilp ), ldc, b( 1_ilp, l1 ), 1_ilp )
                       vec( 1_ilp, 1_ilp ) = c( k1, l1 ) - ( suml+sgn*sumr )
                       scaloc = one
                       a11 = a( k1, k1 ) + sgn*b( l1, l1 )
                       ...
                       sumr = stdlib_qdot( l1-1, c( k1, 1_ilp ), ldc, b( 1_ilp, l1 ), 1_ilp )
                       vec( 1_ilp, 1_ilp ) = c( k1, l1 ) - ( suml+sgn*sumr )
                       suml = stdlib_qdot( k1-1, a( 1_ilp, k2 ), 1_ilp, c( 1_ilp, l1 ), 1_ilp )
                       sumr = stdlib_qdot( l1-1, c( k2, 1_ilp ), ldc, b( 1_ilp, l1 ), 1_ilp )
                       vec( 2_ilp, 1_ilp ) = c( k2, l1 ) - ( suml+sgn*sumr )
                       call stdlib_qlaln2( .true., 2_ilp, 1_ilp, smin, one, a( k1, k1 ),lda, one, one, &
                                 vec, 2_ilp, -sgn*b( l1, l1 ),zero, x, 2_ilp, scaloc, xnorm, ierr )
                       if( ierr/=0_ilp )info = 1_ilp
                       if( scaloc/=one ) then
                          do j = 1, n
                       ...
                       sumr = stdlib_qdot( l1-1, c( k1, 1_ilp ), ldc, b( 1_ilp, l1 ), 1_ilp )
                       vec( 1_ilp, 1_ilp ) = sgn*( c( k1, l1 )-( suml+sgn*sumr ) )
                       suml = stdlib_qdot( k1-1, a( 1_ilp, k1 ), 1_ilp, c( 1_ilp, l2 ), 1_ilp )
                       sumr = stdlib_qdot( l1-1, c( k1, 1_ilp ), ldc, b( 1_ilp, l2 ), 1_ilp )
                       vec( 2_ilp, 1_ilp ) = sgn*( c( k1, l2 )-( suml+sgn*sumr ) )
                       call stdlib_qlaln2( .true., 2_ilp, 1_ilp, smin, one, b( l1, l1 ),ldb, one, one, &
                                 vec, 2_ilp, -sgn*a( k1, k1 ),zero, x, 2_ilp, scaloc, xnorm, ierr )
                       ...
                       sumr = stdlib_qdot( l1-1, c( k2, 1_ilp ), ldc, b( 1_ilp, l1 ), 1_ilp )
                       vec( 2_ilp, 1_ilp ) = c( k2, l1 ) - ( suml+sgn*sumr )
                       suml = stdlib_qdot( k1-1, a( 1_ilp, k2 ), 1_ilp, c( 1_ilp, l2 ), 1_ilp )
                       sumr = stdlib_qdot( l1-1, c( k2, 1_ilp ), ldc, b( 1_ilp, l2 ), 1_ilp )
                       vec( 2_ilp, 2_ilp ) = c( k2, l2 ) - ( suml+sgn*sumr )
                       call stdlib_qlasy2( .true., .false., isgn, 2_ilp, 2_ilp, a( k1, k1 ),lda, b( l1, &
                                 l1 ), ldb, vec, 2_ilp, scaloc, x,2_ilp, xnorm, ierr )
                       if( ierr/=0_ilp )info = 1_ilp
                       if( scaloc/=one ) then
                          do j = 1, n
              ...
              loop_180: do l = n, 1, -1
                 ...
                 loop_170: do k = 1, m
                    ...
                    if( l1==l2 .and. k1==k2 ) then
                       suml = stdlib_qdot( k1-1, a( 1_ilp, k1 ), 1_ilp, c( 1_ilp, l1 ), 1_ilp )
                       sumr = stdlib_qdot( n-l1, c( k1, min( l1+1, n ) ), ldc,b( l1, min( l1+1, n &
                                 ) ), ldb )
                       vec( 1_ilp, 1_ilp ) = c( k1, l1 ) - ( suml+sgn*sumr )
                       scaloc = one
                       a11 = a( k1, k1 ) + sgn*b( l1, l1 )
                       ...
                       suml = stdlib_qdot( k1-1, a( 1_ilp, k1 ), 1_ilp, c( 1_ilp, l1 ), 1_ilp )
                       sumr = stdlib_qdot( n-l2, c( k1, min( l2+1, n ) ), ldc,b( l1, min( l2+1, n &
                                 ) ), ldb )
                       vec( 1_ilp, 1_ilp ) = c( k1, l1 ) - ( suml+sgn*sumr )
                       suml = stdlib_qdot( k1-1, a( 1_ilp, k2 ), 1_ilp, c( 1_ilp, l1 ), 1_ilp )
                       sumr = stdlib_qdot( n-l2, c( k2, min( l2+1, n ) ), ldc,b( l1, min( l2+1, n &
                                 ) ), ldb )
                       vec( 2_ilp, 1_ilp ) = c( k2, l1 ) - ( suml+sgn*sumr )
                       call stdlib_qlaln2( .true., 2_ilp, 1_ilp, smin, one, a( k1, k1 ),lda, one, one, &
                                 vec, 2_ilp, -sgn*b( l1, l1 ),zero, x, 2_ilp, scaloc, xnorm, ierr )
                       ...
                       vec( 1_ilp, 1_ilp ) = sgn*( c( k1, l1 )-( suml+sgn*sumr ) )
                       suml = stdlib_qdot( k1-1, a( 1_ilp, k1 ), 1_ilp, c( 1_ilp, l2 ), 1_ilp )
                       sumr = stdlib_qdot( n-l2, c( k1, min( l2+1, n ) ), ldc,b( l2, min( l2+1, n &
                                 ) ), ldb )
                       vec( 2_ilp, 1_ilp ) = sgn*( c( k1, l2 )-( suml+sgn*sumr ) )
                       call stdlib_qlaln2( .false., 2_ilp, 1_ilp, smin, one, b( l1, l1 ),ldb, one, one, &
                                 vec, 2_ilp, -sgn*a( k1, k1 ),zero, x, 2_ilp, scaloc, xnorm, ierr )
                       if( ierr/=0_ilp )info = 1_ilp
                       if( scaloc/=one ) then
                          do j = 1, n
                       ...
                       vec( 2_ilp, 1_ilp ) = c( k2, l1 ) - ( suml+sgn*sumr )
                       suml = stdlib_qdot( k1-1, a( 1_ilp, k2 ), 1_ilp, c( 1_ilp, l2 ), 1_ilp )
                       sumr = stdlib_qdot( n-l2, c( k2, min( l2+1, n ) ), ldc,b( l2, min( l2+1, n &
                                 ) ), ldb )
                       vec( 2_ilp, 2_ilp ) = c( k2, l2 ) - ( suml+sgn*sumr )
                       call stdlib_qlasy2( .true., .true., isgn, 2_ilp, 2_ilp, a( k1, k1 ),lda, b( l1, l1 &
                                 ), ldb, vec, 2_ilp, scaloc, x,2_ilp, xnorm, ierr )
                       if( ierr/=0_ilp )info = 1_ilp
                       if( scaloc/=one ) then
                          do j = 1, n
              ...
              loop_240: do l = n, 1, -1
                 ...
                 loop_230: do k = m, 1, -1
                    ...
                    if( l1==l2 .and. k1==k2 ) then
                       ...
                       vec( 2_ilp, 2_ilp ) = c( k2, l2 ) - ( suml+sgn*sumr )
                       call stdlib_qlasy2( .false., .true., isgn, 2_ilp, 2_ilp, a( k1, k1 ),lda, b( l1, &
                                 l1 ), ldb, vec, 2_ilp, scaloc, x,2_ilp, xnorm, ierr )
                       if( ierr/=0_ilp )info = 1_ilp
                       if( scaloc/=one ) then
                          do j = 1, n
     ...
     !! overflow in X.
        ...
        ! =====================================================================
           ...
           smlnum = smlnum*real( m*n,KIND=qp) / eps
           bignum = one / smlnum
           smin = max( smlnum, eps*stdlib_wlange( 'M', m, m, a, lda, dum ),eps*stdlib_wlange( 'M',&
                      n, n, b, ldb, dum ) )
           sgn = isgn
           if( notrna .and. notrnb ) then
              ! solve    a*x + isgn*x*b = scale*c.
              ...
              loop_30: do l = 1, n
                 do k = m, 1, -1
                    suml = stdlib_wdotu( m-k, a( k, min( k+1, m ) ), lda,c( min( k+1, m ), l ), 1_ilp &
                              )
                    sumr = stdlib_wdotu( l-1, c( k, 1_ilp ), ldc, b( 1_ilp, l ), 1_ilp )
                    vec = c( k, l ) - ( suml+sgn*sumr )
                    scaloc = one
                    a11 = a( k, k ) + sgn*b( l, l )
                    ...
                    x11 = stdlib_wladiv( vec*cmplx( scaloc,KIND=qp), a11 )
                    if( scaloc/=one ) then
                       do j = 1, n
                          call stdlib_wdscal( m, scaloc, c( 1_ilp, j ), 1_ilp )
                       end do
                       scale = scale*scaloc
                    end if
                    c( k, l ) = x11
              ...
              loop_60: do l = 1, n
                 do k = 1, m
                    suml = stdlib_wdotc( k-1, a( 1_ilp, k ), 1_ilp, c( 1_ilp, l ), 1_ilp )
                    sumr = stdlib_wdotu( l-1, c( k, 1_ilp ), ldc, b( 1_ilp, l ), 1_ilp )
                    vec = c( k, l ) - ( suml+sgn*sumr )
                    scaloc = one
                    a11 = conjg( a( k, k ) ) + sgn*b( l, l )
              ...
              loop_90: do l = n, 1, -1
                 do k = 1, m
                    suml = stdlib_wdotc( k-1, a( 1_ilp, k ), 1_ilp, c( 1_ilp, l ), 1_ilp )
                    sumr = stdlib_wdotc( n-l, c( k, min( l+1, n ) ), ldc,b( l, min( l+1, n ) ), &
                              ldb )
                    vec = c( k, l ) - ( suml+sgn*conjg( sumr ) )
                    scaloc = one
                    a11 = conjg( a( k, k )+sgn*b( l, l ) )
     ...
     !! -1.  op(T) = T or T**T, where T**T denotes the transpose of T.
       ...
       ! =====================================================================
           ...
           logical(lk) :: bswpiv(4_ilp), xswpiv(4_ilp)
           integer(ilp) :: jpiv(4_ilp), locl21(4_ilp), locu12(4_ilp), locu22(4_ilp)
           real(qp) :: btmp(4_ilp), t16(4_ilp,4_ilp), tmp(4_ilp), x2(2_ilp)
           ! Intrinsic Functions 
           ! Data Statements 
           locu12 = [3_ilp,4_ilp,1_ilp,2_ilp]
           locl21 = [2_ilp,1_ilp,4_ilp,3_ilp]
           ...
           ! solve 2 by 2 system using complete pivoting.
           ! set pivots less than smin to smin.
           ipiv = stdlib_iqamax( 4_ilp, tmp, 1_ilp )
           u11 = tmp( ipiv )
           if( abs( u11 )<=smin ) then
              info = 1_ilp
              u11 = smin
           ...
           smin = max( eps*smin, smlnum )
           btmp( 1_ilp ) = zero
           call stdlib_qcopy( 16_ilp, btmp, 0_ilp, t16, 1_ilp )
           t16( 1_ilp, 1_ilp ) = tl( 1_ilp, 1_ilp ) + sgn*tr( 1_ilp, 1_ilp )
           t16( 2_ilp, 2_ilp ) = tl( 2_ilp, 2_ilp ) + sgn*tr( 1_ilp, 1_ilp )
           t16( 3_ilp, 3_ilp ) = tl( 1_ilp, 1_ilp ) + sgn*tr( 2_ilp, 2_ilp )
           t16( 4_ilp, 4_ilp ) = tl( 2_ilp, 2_ilp ) + sgn*tr( 2_ilp, 2_ilp )
           ...
           loop_100: do i = 1, 3
              ...
              if( ipsv/=i ) then
                 call stdlib_qswap( 4_ilp, t16( ipsv, 1_ilp ), 4_ilp, t16( i, 1_ilp ), 4_ilp )
                 temp = btmp( i )
                 btmp( i ) = btmp( ipsv )
                 btmp( ipsv ) = temp
              end if
              if( jpsv/=i )call stdlib_qswap( 4_ilp, t16( 1_ilp, jpsv ), 1_ilp, t16( 1_ilp, i ), 1_ilp )
              jpiv( i ) = jpsv
              if( abs( t16( i, i ) )<smin ) then
                 info = 1_ilp
                 t16( i, i ) = smin
        ...
        ! =====================================================================
           ...
           loop_60: do k = 1, n
              ...
              if( wants ) then
                 ...
                 if( .not.pair ) then
                    ! real eigenvalue.
                    prod = stdlib_qdot( n, vr( 1_ilp, ks ), 1_ilp, vl( 1_ilp, ks ), 1_ilp )
                    rnrm = stdlib_qnrm2( n, vr( 1_ilp, ks ), 1_ilp )
                    lnrm = stdlib_qnrm2( n, vl( 1_ilp, ks ), 1_ilp )
                    s( ks ) = abs( prod ) / ( rnrm*lnrm )
                 else
                    ! complex eigenvalue.
                    prod1 = stdlib_qdot( n, vr( 1_ilp, ks ), 1_ilp, vl( 1_ilp, ks ), 1_ilp )
                    prod1 = prod1 + stdlib_qdot( n, vr( 1_ilp, ks+1 ), 1_ilp, vl( 1_ilp, ks+1 ),1_ilp )
                    prod2 = stdlib_qdot( n, vl( 1_ilp, ks ), 1_ilp, vr( 1_ilp, ks+1 ), 1_ilp )
                    prod2 = prod2 - stdlib_qdot( n, vl( 1_ilp, ks+1 ), 1_ilp, vr( 1_ilp, ks ),1_ilp )
                    rnrm = stdlib_qlapy2( stdlib_qnrm2( n, vr( 1_ilp, ks ), 1_ilp ),stdlib_qnrm2( n, vr( &
                              1_ilp, ks+1 ), 1_ilp ) )
                    lnrm = stdlib_qlapy2( stdlib_qnrm2( n, vl( 1_ilp, ks ), 1_ilp ),stdlib_qnrm2( n, vl( &
                              1_ilp, ks+1 ), 1_ilp ) )
                    cond = stdlib_qlapy2( prod1, prod2 ) / ( rnrm*lnrm )
                    s( ks ) = cond
                    s( ks+1 ) = cond
                 end if
              end if
              if( wantsp ) then
                 ...
                 ! copy the matrix t to the array work and swap the diagonal
                 ! block beginning at t(k,k) to the (1,1) position.
                 call stdlib_qlacpy( 'FULL', n, n, t, ldt, work, ldwork )
                 ifst = k
                 ilst = 1_ilp
                 call stdlib_qtrexc( 'NO Q', n, work, ldwork, dummy, 1_ilp, ifst, ilst,work( 1_ilp, n+1 ),&
                            ierr )
                 if( ierr==1_ilp .or. ierr==2_ilp ) then
                    ! could not swap because blocks not well separated
                    scale = one
                    ...
                    if( work( 2_ilp, 1_ilp )==zero ) then
                       ...
                       ! with negative imaginary  part.
                       mu = sqrt( abs( work( 1_ilp, 2_ilp ) ) )*sqrt( abs( work( 2_ilp, 1_ilp ) ) )
                       delta = stdlib_qlapy2( mu, work( 2_ilp, 1_ilp ) )
                       cs = mu / delta
                       sn = -work( 2_ilp, 1_ilp ) / delta
                       ! form
                       ! c**t = work(2:n,2:n) + i*[rwork(1) ..... rwork(n-1) ]
                    ...
                    kase = 0_ilp
                    50 continue
                    call stdlib_qlacn2( nn, work( 1_ilp, n+2 ), work( 1_ilp, n+4 ), iwork,est, kase, &
                              isave )
                    if( kase/=0_ilp ) then
                       if( kase==1_ilp ) then
                          if( n2==1_ilp ) then
                             ! real eigenvalue: solve c**t*x = scale*c.
                             call stdlib_qlaqtr( .true., .true., n-1, work( 2_ilp, 2_ilp ),ldwork, dummy, &
                                       dumm, scale,work( 1_ilp, n+4 ), work( 1_ilp, n+6 ),ierr )
                          else
                             ! complex eigenvalue: solve
                             ! c**t*(p+iq) = scale*(c+id) in real arithmetic.
                             call stdlib_qlaqtr( .true., .false., n-1, work( 2_ilp, 2_ilp ),ldwork, work( &
                                       1_ilp, n+1 ), mu, scale,work( 1_ilp, n+4 ), work( 1_ilp, n+6 ),ierr )
                          end if
                       else
                          if( n2==1_ilp ) then
                             ! real eigenvalue: solve c*x = scale*c.
                             call stdlib_qlaqtr( .false., .true., n-1, work( 2_ilp, 2_ilp ),ldwork, dummy,&
                                        dumm, scale,work( 1_ilp, n+4 ), work( 1_ilp, n+6 ),ierr )
                          else
                             ! complex eigenvalue: solve
                             ! c*(p+iq) = scale*(c+id) in real arithmetic.
                             call stdlib_qlaqtr( .false., .false., n-1,work( 2_ilp, 2_ilp ), ldwork,work( &
                                       1_ilp, n+1 ), mu, scale,work( 1_ilp, n+4 ), work( 1_ilp, n+6 ),ierr )
                          end if
                       end if
                       go to 50
        ...
        ! =====================================================================
           ...
           loop_50: do k = 1, n
              ...
              if( wants ) then
                 ! compute the reciprocal condition number of the k-th
                 ! eigenvalue.
                 prod = stdlib_wdotc( n, vr( 1_ilp, ks ), 1_ilp, vl( 1_ilp, ks ), 1_ilp )
                 rnrm = stdlib_qznrm2( n, vr( 1_ilp, ks ), 1_ilp )
                 lnrm = stdlib_qznrm2( n, vl( 1_ilp, ks ), 1_ilp )
                 s( ks ) = abs( prod ) / ( rnrm*lnrm )
              end if
              if( wantsp ) then
                 ! estimate the reciprocal condition number of the k-th
                 ...
                 ! copy the matrix t to the array work and swap the k-th
                 ! diagonal element to the (1,1) position.
                 call stdlib_wlacpy( 'FULL', n, n, t, ldt, work, ldwork )
                 call stdlib_wtrexc( 'NO Q', n, work, ldwork, dummy, 1_ilp, k, 1_ilp, ierr )
                 ! form  c = t22 - lambda*i in work(2:n,2:n).
                 do i = 2, n
                    work( i, i ) = work( i, i ) - work( 1_ilp, 1_ilp )
                 end do
                 ...
                 normin = 'N'
                 30 continue
                 call stdlib_wlacn2( n-1, work( 1_ilp, n+1 ), work, est, kase, isave )
                 if( kase/=0_ilp ) then
                    if( kase==1_ilp ) then
                       ! solve c**h*x = scale*b
                       call stdlib_wlatrs( 'UPPER', 'CONJUGATE TRANSPOSE','NONUNIT', normin, n-1, &
                                 work( 2_ilp, 2_ilp ),ldwork, work, scale, rwork, ierr )
                    else
                       ! solve c*x = scale*b
                       call stdlib_wlatrs( 'UPPER', 'NO TRANSPOSE', 'NONUNIT',normin, n-1, work( &
                                 2_ilp, 2_ilp ), ldwork, work,scale, rwork, ierr )
                    end if
                    normin = 'Y'
                    if( scale/=one ) then
                       ! multiply by 1/scale if doing so will not cause
                       ! overflow.
                       ix = stdlib_iwamax( n-1, work, 1_ilp )
                       xnorm = cabs1( work( ix, 1_ilp ) )
                       if( scale<xnorm*smlnum .or. scale==zero )go to 40
                       call stdlib_wdrscl( n, scale, work, 1_ilp )
                    end if
                    go to 30
                 end if
                 sep( ks ) = one / max( est, smlnum )
        ...
        ! =====================================================================
           ...
           if( ifst<ilst ) then
              ...
              if( nbf==1_ilp .or. nbf==2_ilp ) then
                 ...
                 call stdlib_qlaexc( wantq, n, t, ldt, q, ldq, here+1, 1_ilp, nbnext,work, info )
                 ...
                 if( nbnext==1_ilp ) then
                    ! swap two 1 by 1 blocks, no problems possible
                    call stdlib_qlaexc( wantq, n, t, ldt, q, ldq, here, 1_ilp, nbnext,work, info )
                    ...
                    if( nbnext==2_ilp ) then
                       ...
                       ! 2 by 2 block did split
                       call stdlib_qlaexc( wantq, n, t, ldt, q, ldq, here, 1_ilp, 1_ilp,work, info )
                                 
                       call stdlib_qlaexc( wantq, n, t, ldt, q, ldq, here+1, 1_ilp, 1_ilp,work, info )
              ...
              if( nbf==1_ilp .or. nbf==2_ilp ) then
                 ...
                 call stdlib_qlaexc( wantq, n, t, ldt, q, ldq, here-nbnext, nbnext,nbf, work, &
                           info )
                 if( info/=0_ilp ) then
                    ilst = here
                    return
                 ...
                 call stdlib_qlaexc( wantq, n, t, ldt, q, ldq, here-nbnext, nbnext,1_ilp, work, info )
                 ...
                 if( nbnext==1_ilp ) then
                    ! swap two 1 by 1 blocks, no problems possible
                    call stdlib_qlaexc( wantq, n, t, ldt, q, ldq, here, nbnext, 1_ilp,work, info )
                    ...
                    if( nbnext==2_ilp ) then
                       ! 2 by 2 block did not split
                       call stdlib_qlaexc( wantq, n, t, ldt, q, ldq, here-1, 2_ilp, 1_ilp,work, info )
     ...
     !! postmultplying it with Z.
        ...
        ! =====================================================================
           ...
           do k = ifst + m1, ilst + m2, m3
              ...
              call stdlib_wrot( k-1, t( 1_ilp, k ), 1_ilp, t( 1_ilp, k+1 ), 1_ilp, cs,conjg( sn ) )
              t( k, k ) = t22
              t( k+1, k+1 ) = t11
              if( wantq ) then
                 ! accumulate transformation in the matrix q.
                 call stdlib_wrot( n, q( 1_ilp, k ), 1_ilp, q( 1_ilp, k+1 ), 1_ilp, cs,conjg( sn ) )
              end if
           end do
           return
     ...
     !! off-diagonal elements of opposite sign.
        ...
        ! =====================================================================
           ...
           if( m==n .or. m==0_ilp ) then
              if( wants )s = one
              if( wantsp )sep = stdlib_qlange( '1', n, n, t, ldt, work )
              go to 40
           end if
           ! collect the selected blocks at the top-left corner of t.
           ks = 0_ilp
           ...
           if( wants ) then
              ! solve sylvester equation for r:
                 ! t11*r - r*t22 = scale*t12
              call stdlib_qlacpy( 'F', n1, n2, t( 1_ilp, n1+1 ), ldt, work, n1 )
              call stdlib_qtrsyl( 'N', 'N', -1_ilp, n1, n2, t, ldt, t( n1+1, n1+1 ),ldt, work, n1, &
                        scale, ierr )
              ! estimate the reciprocal of the condition number of the cluster
              ! of eigenvalues.
              rnorm = stdlib_qlange( 'F', n1, n2, work, n1, work )
              if( rnorm==zero ) then
                 s = one
              else
                 s = scale / ( sqrt( scale*scale / rnorm+rnorm )*sqrt( rnorm ) )
           ...
           if( wantsp ) then
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! solve  t11*r - r*t22 = scale*x.
                    call stdlib_qtrsyl( 'N', 'N', -1_ilp, n1, n2, t, ldt,t( n1+1, n1+1 ), ldt, work, &
                              n1, scale,ierr )
                 else
                    ! solve t11**t*r - r*t22**t = scale*x.
                    call stdlib_qtrsyl( 'T', 'T', -1_ilp, n1, n2, t, ldt,t( n1+1, n1+1 ), ldt, work, &
                              n1, scale,ierr )
                 end if
                 go to 30
              end if
     ...
     !! the cluster of eigenvalues and/or the invariant subspace.
        ...
        ! =====================================================================
           ...
           if( m==n .or. m==0_ilp ) then
              if( wants )s = one
              if( wantsp )sep = stdlib_wlange( '1', n, n, t, ldt, rwork )
              go to 40
           end if
           ! collect the selected eigenvalues at the top left corner of t.
           ks = 0_ilp
           do k = 1, n
              if( select( k ) ) then
                 ks = ks + 1_ilp
                 ! swap the k-th eigenvalue to position ks.
                 if( k/=ks )call stdlib_wtrexc( compq, n, t, ldt, q, ldq, k, ks, ierr )
              end if
           end do
           if( wants ) then
              ! solve the sylvester equation for r:
                 ! t11*r - r*t22 = scale*t12
              call stdlib_wlacpy( 'F', n1, n2, t( 1_ilp, n1+1 ), ldt, work, n1 )
              call stdlib_wtrsyl( 'N', 'N', -1_ilp, n1, n2, t, ldt, t( n1+1, n1+1 ),ldt, work, n1, &
                        scale, ierr )
              ! estimate the reciprocal of the condition number of the cluster
              ! of eigenvalues.
              rnorm = stdlib_wlange( 'F', n1, n2, work, n1, rwork )
              if( rnorm==zero ) then
                 s = one
              else
                 s = scale / ( sqrt( scale*scale / rnorm+rnorm )*sqrt( rnorm ) )
           ...
           if( wantsp ) then
              ...
              if( kase/=0_ilp ) then
                 if( kase==1_ilp ) then
                    ! solve t11*r - r*t22 = scale*x.
                    call stdlib_wtrsyl( 'N', 'N', -1_ilp, n1, n2, t, ldt,t( n1+1, n1+1 ), ldt, work, &
                              n1, scale,ierr )
                 else
                    ! solve t11**h*r - r*t22**h = scale*x.
                    call stdlib_wtrsyl( 'C', 'C', -1_ilp, n1, n2, t, ldt,t( n1+1, n1+1 ), ldt, work, &
                              n1, scale,ierr )
                 end if
                 go to 30
              end if
     ...
     !! opposite sign.
        ...
        ! =====================================================================
           ...
           real(qp) :: cs, dnorm, eps, scale, smlnum, sn, t11, t22, t33, tau, tau1, tau2, temp, &
                     thresh, wi1, wi2, wr1, wr2, xnorm
           ! Local Arrays 
           real(qp) :: d(ldd,4_ilp), u(3_ilp), u1(3_ilp), u2(3_ilp), x(ldx,2_ilp)
           ! Intrinsic Functions 
           ! Executable Statements 
           info = 0_ilp
           ! quick return if possible
           ...
           if( n1==1_ilp .and. n2==1_ilp ) then
              ...
              call stdlib_qrot( j1-1, t( 1_ilp, j1 ), 1_ilp, t( 1_ilp, j2 ), 1_ilp, cs, sn )
              t( j1, j1 ) = t22
              t( j2, j2 ) = t11
              if( wantq ) then
                 ! accumulate transformation in the matrix q.
                 call stdlib_qrot( n, q( 1_ilp, j1 ), 1_ilp, q( 1_ilp, j2 ), 1_ilp, cs, sn )
              end if
           else
              ! swapping involves at least one 2-by-2 block.
              ! copy the diagonal block of order n1+n2 to the local array d
              ! and compute its norm.
              nd = n1 + n2
              call stdlib_qlacpy( 'FULL', nd, nd, t( j1, j1 ), ldt, d, ldd )
              dnorm = stdlib_qlange( 'MAX', nd, nd, d, ldd, work )
              ! compute machine-dependent threshold for test for accepting
              ! swap.
              eps = stdlib_qlamch( 'P' )
              smlnum = stdlib_qlamch( 'S' ) / eps
              thresh = max( ten*eps*dnorm, smlnum )
              ! solve t11*x - x*t22 = scale*t12 for x.
              call stdlib_qlasy2( .false., .false., -1_ilp, n1, n2, d, ldd,d( n1+1, n1+1 ), ldd, d( 1_ilp,&
                         n1+1 ), ldd, scale, x,ldx, xnorm, ierr )
              ! swap the adjacent diagonal blocks.
              k = n1 + n1 + n2 - 3_ilp
              go to ( 10, 20, 30 )k
              ...
              u( 2_ilp ) = x( 1_ilp, 1_ilp )
              u( 3_ilp ) = x( 1_ilp, 2_ilp )
              call stdlib_qlarfg( 3_ilp, u( 3_ilp ), u, 1_ilp, tau )
              u( 3_ilp ) = one
              t11 = t( j1, j1 )
              ! perform swap provisionally on diagonal block in d.
              call stdlib_qlarfx( 'L', 3_ilp, 3_ilp, u, tau, d, ldd, work )
              call stdlib_qlarfx( 'R', 3_ilp, 3_ilp, u, tau, d, ldd, work )
              ! test whether to reject swap.
              if( max( abs( d( 3, 1 ) ), abs( d( 3, 2 ) ), abs( d( 3,3 )-t11 ) )>thresh ) goto 50
              ! accept swap: apply transformation to the entire matrix t.
              call stdlib_qlarfx( 'L', 3_ilp, n-j1+1, u, tau, t( j1, j1 ), ldt, work )
              call stdlib_qlarfx( 'R', j2, 3_ilp, u, tau, t( 1_ilp, j1 ), ldt, work )
              t( j3, j1 ) = zero
              t( j3, j2 ) = zero
              t( j3, j3 ) = t11
              if( wantq ) then
                 ! accumulate transformation in the matrix q.
                 call stdlib_qlarfx( 'R', n, 3_ilp, u, tau, q( 1_ilp, j1 ), ldq, work )
              end if
              go to 40
              20 continue
              ! n1 = 2, n2 = 1: generate elementary reflector h so that:
              ...
              u( 2_ilp ) = -x( 2_ilp, 1_ilp )
              u( 3_ilp ) = scale
              call stdlib_qlarfg( 3_ilp, u( 1_ilp ), u( 2_ilp ), 1_ilp, tau )
              u( 1_ilp ) = one
              t33 = t( j3, j3 )
              ! perform swap provisionally on diagonal block in d.
              call stdlib_qlarfx( 'L', 3_ilp, 3_ilp, u, tau, d, ldd, work )
              ...
              ! accept swap: apply transformation to the entire matrix t.
              call stdlib_qlarfx( 'R', j3, 3_ilp, u, tau, t( 1_ilp, j1 ), ldt, work )
              call stdlib_qlarfx( 'L', 3_ilp, n-j1, u, tau, t( j1, j2 ), ldt, work )
              t( j1, j1 ) = t33
              t( j2, j1 ) = zero
              t( j3, j1 ) = zero
              if( wantq ) then
              ...
              u1( 2_ilp ) = -x( 2_ilp, 1_ilp )
              u1( 3_ilp ) = scale
              call stdlib_qlarfg( 3_ilp, u1( 1_ilp ), u1( 2_ilp ), 1_ilp, tau1 )
              u1( 1_ilp ) = one
              temp = -tau1*( x( 1_ilp, 2_ilp )+u1( 2_ilp )*x( 2_ilp, 2_ilp ) )
              u2( 1_ilp ) = -temp*u1( 2_ilp ) - x( 2_ilp, 2_ilp )
              u2( 2_ilp ) = -temp*u1( 3_ilp )
              u2( 3_ilp ) = scale
              call stdlib_qlarfg( 3_ilp, u2( 1_ilp ), u2( 2_ilp ), 1_ilp, tau2 )
              u2( 1_ilp ) = one
              ! perform swap provisionally on diagonal block in d.
              call stdlib_qlarfx( 'L', 3_ilp, 4_ilp, u1, tau1, d, ldd, work )
              call stdlib_qlarfx( 'R', 4_ilp, 3_ilp, u1, tau1, d, ldd, work )
              call stdlib_qlarfx( 'L', 3_ilp, 4_ilp, u2, tau2, d( 2_ilp, 1_ilp ), ldd, work )
              call stdlib_qlarfx( 'R', 4_ilp, 3_ilp, u2, tau2, d( 1_ilp, 2_ilp ), ldd, work )
              ! test whether to reject swap.
              if( max( abs( d( 3_ilp, 1_ilp ) ), abs( d( 3_ilp, 2_ilp ) ), abs( d( 4_ilp, 1_ilp ) ),abs( d( 4_ilp, 2_ilp ) ) )&
                        >thresh )go to 50
              ! accept swap: apply transformation to the entire matrix t.
              call stdlib_qlarfx( 'L', 3_ilp, n-j1+1, u1, tau1, t( j1, j1 ), ldt, work )
              call stdlib_qlarfx( 'R', j4, 3_ilp, u1, tau1, t( 1_ilp, j1 ), ldt, work )
              call stdlib_qlarfx( 'L', 3_ilp, n-j1+1, u2, tau2, t( j2, j1 ), ldt, work )
              call stdlib_qlarfx( 'R', j4, 3_ilp, u2, tau2, t( 1_ilp, j2 ), ldt, work )
              t( j3, j1 ) = zero
              t( j3, j2 ) = zero
              t( j4, j1 ) = zero
              t( j4, j2 ) = zero
              if( wantq ) then
                 ! accumulate transformation in the matrix q.
                 call stdlib_qlarfx( 'R', n, 3_ilp, u1, tau1, q( 1_ilp, j1 ), ldq, work )
                 call stdlib_qlarfx( 'R', n, 3_ilp, u2, tau2, q( 1_ilp, j2 ), ldq, work )
              end if
              40 continue
              if( n2==2_ilp ) then
                 ! standardize new 2-by-2 block t11
                 call stdlib_qlanv2( t( j1, j1 ), t( j1, j2 ), t( j2, j1 ),t( j2, j2 ), wr1, wi1, &
                           wr2, wi2, cs, sn )
                 call stdlib_qrot( n-j1-1, t( j1, j1+2 ), ldt, t( j2, j1+2 ), ldt,cs, sn )
                 call stdlib_qrot( j1-1, t( 1_ilp, j1 ), 1_ilp, t( 1_ilp, j2 ), 1_ilp, cs, sn )
                 if( wantq )call stdlib_qrot( n, q( 1_ilp, j1 ), 1_ilp, q( 1_ilp, j2 ), 1_ilp, cs, sn )
              end if
              if( n1==2_ilp ) then
                 ! standardize new 2-by-2 block t22
                 j3 = j1 + n2
                 j4 = j3 + 1_ilp
                 call stdlib_qlanv2( t( j3, j3 ), t( j3, j4 ), t( j4, j3 ),t( j4, j4 ), wr1, wi1, &
                           wr2, wi2, cs, sn )
                 if( j3+2<=n )call stdlib_qrot( n-j3-1, t( j3, j3+2 ), ldt, t( j4, j3+2 ),ldt, cs,&
                            sn )
                 call stdlib_qrot( j3-1, t( 1_ilp, j3 ), 1_ilp, t( 1_ilp, j4 ), 1_ilp, cs, sn )
                 if( wantq )call stdlib_qrot( n, q( 1_ilp, j3 ), 1_ilp, q( 1_ilp, j4 ), 1_ilp, cs, sn )
              end if
           end if
           return
           ! exit with info = 1 if swap was rejected.
        ...
        ! =====================================================================
           ...
           if( wi==zero ) then
              ...
              if( noinit ) then
                 ...
                 ! scale supplied initial vector.
                 vnorm = stdlib_qnrm2( n, vr, 1_ilp )
                 call stdlib_qscal( n, ( eps3*rootn ) / max( vnorm, nrmsml ), vr,1_ilp )
              end if
              if( rightv ) then
                 ! lu decomposition with partial pivoting of b, replacing zero
                 ! pivots by eps3.
              ...
              do its = 1, n
                 ! solve u*x = scale*v for a right eigenvector
                   ! or u**t*x = scale*v for a left eigenvector,
                 ! overwriting x on v.
                 call stdlib_qlatrs( 'UPPER', trans, 'NONUNIT', normin, n, b, ldb,vr, scale, work,&
                            ierr )
                 normin = 'Y'
                 ! test for sufficient growth in the norm of v.
                 vnorm = stdlib_qasum( n, vr, 1_ilp )
                 if( vnorm>=growto*scale )go to 120
                 ! choose new orthogonal starting vector and try again.
                 temp = eps3 / ( rootn+one )
                 vr( 1_ilp ) = eps3
              ...
              120 continue
              ! normalize eigenvector.
              i = stdlib_iqamax( n, vr, 1_ilp )
              call stdlib_qscal( n, one / abs( vr( i ) ), vr, 1_ilp )
           else
              ! complex eigenvalue.
              if( noinit ) then
                 ! set initial vector.
                 ...
                 ! scale supplied initial vector.
                 norm = stdlib_qlapy2( stdlib_qnrm2( n, vr, 1_ilp ), stdlib_qnrm2( n, vi, 1_ilp ) )
                           
                 rec = ( eps3*rootn ) / max( norm, nrmsml )
                 call stdlib_qscal( n, rec, vr, 1_ilp )
                 call stdlib_qscal( n, rec, vi, 1_ilp )
              end if
              if( rightv ) then
                 ! lu decomposition with partial pivoting of b, replacing zero
                 ! pivots by eps3.
                 ...
                 loop_170: do i = 1, n - 1
                    absbii = stdlib_qlapy2( b( i, i ), b( i+1, i ) )
                    ei = h( i+1, i )
                    if( absbii<abs( ei ) ) then
                       ! interchange rows and eliminate.
                       xr = b( i, i ) / ei
                    ...
                    ! compute 1-norm of offdiagonal elements of i-th row.
                    work( i ) = stdlib_qasum( n-i, b( i, i+1 ), ldb ) +stdlib_qasum( n-i, b( i+2, &
                              i ), 1_ilp )
                 end do loop_170
                 if( b( n, n )==zero .and. b( n+1, n )==zero )b( n, n ) = eps3
                 work( n ) = zero
                 ...
                 loop_210: do j = n, 2, -1
                    ej = h( j, j-1 )
                    absbjj = stdlib_qlapy2( b( j, j ), b( j+1, j ) )
                    if( absbjj<abs( ej ) ) then
                       ! interchange columns and eliminate
                       xr = b( j, j ) / ej
                       xi = b( j+1, j ) / ej
                    ...
                    ! compute 1-norm of offdiagonal elements of j-th column.
                    work( j ) = stdlib_qasum( j-1, b( 1_ilp, j ), 1_ilp ) +stdlib_qasum( j-1, b( j+1, 1_ilp ),&
                               ldb )
                 end do loop_210
                 if( b( 1_ilp, 1_ilp )==zero .and. b( 2_ilp, 1_ilp )==zero )b( 1_ilp, 1_ilp ) = eps3
                 work( 1_ilp ) = zero
              ...
              loop_270: do its = 1, n
                 ...
                 ! test for sufficient growth in the norm of (vr,vi).
                 vnorm = stdlib_qasum( n, vr, 1_ilp ) + stdlib_qasum( n, vi, 1_ilp )
                 if( vnorm>=growto*scale )go to 280
                 ! choose a new orthogonal starting vector and try again.
                 y = eps3 / ( rootn+one )
                 vr( 1_ilp ) = eps3
              ...
              call stdlib_qscal( n, one / vnorm, vr, 1_ilp )
              call stdlib_qscal( n, one / vnorm, vi, 1_ilp )
           end if
           return
     ...
     !! matrix H.
        ...
        ! =====================================================================
           ...
           if( noinit ) then
              ...
              ! scale supplied initial vector.
              vnorm = stdlib_qznrm2( n, v, 1_ilp )
              call stdlib_wdscal( n, ( eps3*rootn ) / max( vnorm, nrmsml ), v, 1_ilp )
           end if
           if( rightv ) then
              ! lu decomposition with partial pivoting of b, replacing czero
              ! pivots by eps3.
           ...
           do its = 1, n
              ! solve u*x = scale*v for a right eigenvector
                ! or u**h *x = scale*v for a left eigenvector,
              ! overwriting x on v.
              call stdlib_wlatrs( 'UPPER', trans, 'NONUNIT', normin, n, b, ldb, v,scale, rwork, &
                        ierr )
              normin = 'Y'
              ! test for sufficient growth in the norm of v.
              vnorm = stdlib_qzasum( n, v, 1_ilp )
              if( vnorm>=growto*scale )go to 120
              ! choose new orthogonal starting vector and try again.
              rtemp = eps3 / ( rootn+one )
              v( 1_ilp ) = eps3
           ...
           120 continue
           ! normalize eigenvector.
           i = stdlib_iwamax( n, v, 1_ilp )
           call stdlib_wdscal( n, one / cabs1( v( i ) ), v, 1_ilp )
           return
