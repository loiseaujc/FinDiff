...
pure module function stdlib_sum_1d_qp(a) result(s)
    integer(ilp), parameter :: chunk = 64
    real(qp), intent(in) :: a(:)
    real(qp) :: s
    real(qp) :: abatch(chunk)
    integer(ilp) :: i, n, r
    ! -----------------------------
    n = size(a,kind=ilp)
    r = mod(n,chunk)
...
pure module function stdlib_sum_1d_qp_mask(a,mask) result(s)
    integer(ilp), parameter :: chunk = 64
    real(qp), intent(in) :: a(:)
    logical, intent(in) :: mask(:)
    real(qp) :: s
    ...
    abatch(1:r)       = merge( zero_qp , a(1:r) , mask(1:r) )
    abatch(r+1:chunk) = zero_qp
    do i = r+1, n-r, chunk
        abatch(1:chunk) = abatch(1:chunk) + merge( zero_qp , a(i:i+chunk-1), mask(i:i+chunk-1) )
    end do
...
pure module function stdlib_sum_1d_cqp(a) result(s)
    integer(ilp), parameter :: chunk = 64
    complex(qp), intent(in) :: a(:)
    complex(qp) :: s
    complex(qp) :: abatch(chunk)
    integer(ilp) :: i, n, r
    ! -----------------------------
    n = size(a,kind=ilp)
    r = mod(n,chunk)
...
pure module function stdlib_sum_1d_cqp_mask(a,mask) result(s)
    integer(ilp), parameter :: chunk = 64
    complex(qp), intent(in) :: a(:)
    logical, intent(in) :: mask(:)
    complex(qp) :: s
    ...
    abatch(1:r)       = merge( zero_cqp , a(1:r) , mask(1:r) )
    abatch(r+1:chunk) = zero_cqp
    do i = r+1, n-r, chunk
        abatch(1:chunk) = abatch(1:chunk) + merge( zero_cqp , a(i:i+chunk-1), mask(i:i+chunk-1) )
    end do
...
pure module function stdlib_sum_kahan_1d_qp(a) result(s)
    integer(ilp), parameter :: chunk = 64
    real(qp), intent(in) :: a(:)
    real(qp) :: s
    real(qp) :: sbatch(chunk)
    real(qp) :: cbatch(chunk)
    integer(ilp) :: i, n, r
    ! -----------------------------
    n = size(a,kind=ilp)
    r = mod(n,chunk)
...
pure module function stdlib_sum_kahan_1d_qp_mask(a,mask) result(s)
    integer(ilp), parameter :: chunk = 64
    real(qp), intent(in) :: a(:)
    logical, intent(in) :: mask(:)
    real(qp) :: s
    ...
    sbatch(1:r) = merge( zero_qp , a(1:r) , mask(1:r) )
    sbatch(r+1:chunk)  = zero_qp
    cbatch = zero_qp
    do i = r+1, n-r, chunk
      call kahan_kernel( a(i:i+chunk-1) , sbatch(1:chunk) , cbatch(1:chunk), mask(i:i+chunk-1) )
...
pure module function stdlib_sum_kahan_1d_cqp(a) result(s)
    integer(ilp), parameter :: chunk = 64
    complex(qp), intent(in) :: a(:)
    complex(qp) :: s
    complex(qp) :: sbatch(chunk)
    complex(qp) :: cbatch(chunk)
    integer(ilp) :: i, n, r
    ! -----------------------------
    n = size(a,kind=ilp)
    r = mod(n,chunk)
...
pure module function stdlib_sum_kahan_1d_cqp_mask(a,mask) result(s)
    integer(ilp), parameter :: chunk = 64
    complex(qp), intent(in) :: a(:)
    logical, intent(in) :: mask(:)
    complex(qp) :: s
    ...
    sbatch(1:r) = merge( zero_cqp , a(1:r) , mask(1:r) )
    sbatch(r+1:chunk)  = zero_cqp
    cbatch = zero_cqp
    do i = r+1, n-r, chunk
      call kahan_kernel( a(i:i+chunk-1) , sbatch(1:chunk) , cbatch(1:chunk), mask(i:i+chunk-1) )
...
pure module function stdlib_sum_2d_qp( x , mask ) result( s )
    real(qp), intent(in) :: x(:,:)
    logical, intent(in), optional :: mask(:,:)
    real(qp) :: s
    if(.not.present(mask)) then
...
contains
    pure real(qp) function sum_recast(b,n)
      integer(ilp), intent(in) :: n
      real(qp), intent(in) :: b(n)
      sum_recast = stdlib_sum(b)
    end function
    pure real(qp) function sum_recast_mask(b,m,n)
      integer(ilp), intent(in) :: n
      real(qp), intent(in) :: b(n)
      logical, intent(in) :: m(n)
      sum_recast_mask = stdlib_sum(b,m)
...
pure module function stdlib_sum_2d_dim_qp( x , dim, mask ) result( s )
    real(qp), intent(in) :: x(:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:)
    real(qp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim))
    integer :: j 
...
pure module function stdlib_sum_3d_qp( x , mask ) result( s )
    real(qp), intent(in) :: x(:,:,:)
    logical, intent(in), optional :: mask(:,:,:)
    real(qp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_3d_dim_qp( x , dim, mask ) result( s )
    real(qp), intent(in) :: x(:,:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:,:)
    real(qp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim))
    integer :: j 
...
pure module function stdlib_sum_4d_qp( x , mask ) result( s )
    real(qp), intent(in) :: x(:,:,:,:)
    logical, intent(in), optional :: mask(:,:,:,:)
    real(qp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_4d_dim_qp( x , dim, mask ) result( s )
    real(qp), intent(in) :: x(:,:,:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:,:,:)
    real(qp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim), merge(size(x, 3), size(x,&
        & 4), mask=3<dim))
    integer :: j 
...
pure module function stdlib_sum_2d_cqp( x , mask ) result( s )
...
contains
    pure complex(qp) function sum_recast(b,n)
      integer(ilp), intent(in) :: n
      complex(qp), intent(in) :: b(n)
      sum_recast = stdlib_sum(b)
    end function
    pure complex(qp) function sum_recast_mask(b,m,n)
      integer(ilp), intent(in) :: n
      complex(qp), intent(in) :: b(n)
      logical, intent(in) :: m(n)
      sum_recast_mask = stdlib_sum(b,m)
...
pure module function stdlib_sum_2d_dim_cqp( x , dim, mask ) result( s )
    complex(qp), intent(in) :: x(:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:)
    complex(qp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim))
    integer :: j 
...
pure module function stdlib_sum_3d_cqp( x , mask ) result( s )
    complex(qp), intent(in) :: x(:,:,:)
    logical, intent(in), optional :: mask(:,:,:)
    complex(qp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_3d_dim_cqp( x , dim, mask ) result( s )
    complex(qp), intent(in) :: x(:,:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:,:)
    complex(qp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim))
    integer :: j 
...
pure module function stdlib_sum_4d_cqp( x , mask ) result( s )
    complex(qp), intent(in) :: x(:,:,:,:)
    logical, intent(in), optional :: mask(:,:,:,:)
    complex(qp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_4d_dim_cqp( x , dim, mask ) result( s )
    complex(qp), intent(in) :: x(:,:,:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:,:,:)
    complex(qp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim), merge(size(x, 3),&
        & size(x, 4), mask=3<dim))
    integer :: j 
...
pure module function stdlib_sum_kahan_2d_qp( x , mask ) result( s )
    real(qp), intent(in) :: x(:,:)
    logical, intent(in), optional :: mask(:,:)
    real(qp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_kahan_2d_dim_qp( x , dim, mask ) result( s )
    real(qp), intent(in) :: x(:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:)
    real(qp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim))
...
pure module function stdlib_sum_kahan_3d_qp( x , mask ) result( s )
    real(qp), intent(in) :: x(:,:,:)
    logical, intent(in), optional :: mask(:,:,:)
    real(qp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_kahan_3d_dim_qp( x , dim, mask ) result( s )
    real(qp), intent(in) :: x(:,:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:,:)
    real(qp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim))
...
pure module function stdlib_sum_kahan_4d_qp( x , mask ) result( s )
    real(qp), intent(in) :: x(:,:,:,:)
    logical, intent(in), optional :: mask(:,:,:,:)
    real(qp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_kahan_4d_dim_qp( x , dim, mask ) result( s )
    real(qp), intent(in) :: x(:,:,:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:,:,:)
    real(qp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim), merge(size(x, 3), size(x,&
...
pure module function stdlib_sum_kahan_2d_cqp( x , mask ) result( s )
    complex(qp), intent(in) :: x(:,:)
    logical, intent(in), optional :: mask(:,:)
    complex(qp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_kahan_2d_dim_cqp( x , dim, mask ) result( s )
    complex(qp), intent(in) :: x(:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:)
    complex(qp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim))
...
pure module function stdlib_sum_kahan_3d_cqp( x , mask ) result( s )
    complex(qp), intent(in) :: x(:,:,:)
    logical, intent(in), optional :: mask(:,:,:)
    complex(qp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_kahan_3d_dim_cqp( x , dim, mask ) result( s )
    complex(qp), intent(in) :: x(:,:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:,:)
    complex(qp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim))
...
pure module function stdlib_sum_kahan_4d_cqp( x , mask ) result( s )
    complex(qp), intent(in) :: x(:,:,:,:)
    logical, intent(in), optional :: mask(:,:,:,:)
    complex(qp) :: s
    if(.not.present(mask)) then
...
pure module function stdlib_sum_kahan_4d_dim_cqp( x , dim, mask ) result( s )
    complex(qp), intent(in) :: x(:,:,:,:)
    integer, intent(in):: dim
    logical, intent(in), optional :: mask(:,:,:,:)
    complex(qp) :: s(merge(size(x, 1), size(x, 2), mask=1<dim), merge(size(x, 2), size(x, 3), mask=2<dim), merge(size(x, 3),&
