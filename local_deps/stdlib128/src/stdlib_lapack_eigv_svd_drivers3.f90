  ...
  contains
     ...
     !! for a detailed description of the algorithm.
        ...
        ! =====================================================================
           ...
           ! if no singular vectors desired, use qd algorithm
           if( .not.rotate ) then
              call stdlib_qlasq1( n, d, e, work, info )
           ! if info equals 2, dqds didn't finish, try to finish
              if( info /= 2 ) return
              info = 0_ilp
           end if
           ...
           if( lower ) then
              ...
              ! update singular vectors if desired
              if( nru>0_ilp )call stdlib_qlasr( 'R', 'V', 'F', nru, n, work( 1_ilp ), work( n ), u,ldu )
                        
              if( ncc>0_ilp )call stdlib_qlasr( 'L', 'V', 'F', n, ncc, work( 1_ilp ), work( n ), c,ldc )
           ...
           if( ll==m-1 ) then
              ! 2 by 2 block, handle separately
              call stdlib_qlasv2( d( m-1 ), e( m-1 ), d( m ), sigmn, sigmx, sinr,cosr, sinl, cosl &
                        )
              d( m-1 ) = sigmx
              e( m-1 ) = zero
              d( m ) = sigmn
              ! compute singular vectors, if desired
              if( ncvt>0_ilp )call stdlib_qrot( ncvt, vt( m-1, 1_ilp ), ldvt, vt( m, 1_ilp ), ldvt, cosr,sinr &
                        )
              if( nru>0_ilp )call stdlib_qrot( nru, u( 1_ilp, m-1 ), 1_ilp, u( 1_ilp, m ), 1_ilp, cosl, sinl )
              if( ncc>0_ilp )call stdlib_qrot( ncc, c( m-1, 1_ilp ), ldc, c( m, 1_ilp ), ldc, cosl,sinl )
           ...
           if( tol>=zero .and. n*tol*( sminl / smax )<=max( eps, hndrth*tol ) ) then
              ...
              if( idir==1_ilp ) then
                 sll = abs( d( ll ) )
                 call stdlib_qlas2( d( m-1 ), e( m-1 ), d( m ), shift, r )
              else
                 sll = abs( d( m ) )
                 call stdlib_qlas2( d( ll ), e( ll ), d( ll+1 ), shift, r )
              end if
              ! test if shift negligible, and if so set to zero
              if( sll>zero ) then
                 if( ( shift / sll )**2_ilp<eps )shift = zero
           ...
           if( shift==zero ) then
              if( idir==1_ilp ) then
                 ...
                 oldcs = one
                 do i = ll, m - 1
                    call stdlib_qlartg( d( i )*cs, e( i ), cs, sn, r )
                    if( i>ll )e( i-1 ) = oldsn*r
                    call stdlib_qlartg( oldcs*r, d( i+1 )*sn, oldcs, oldsn, d( i ) )
                    work( i-ll+1 ) = cs
                    work( i-ll+1+nm1 ) = sn
                    work( i-ll+1+nm12 ) = oldcs
                    work( i-ll+1+nm13 ) = oldsn
                 ...
                 e( m-1 ) = h*oldsn
                 ! update singular vectors
                 if( ncvt>0_ilp )call stdlib_qlasr( 'L', 'V', 'F', m-ll+1, ncvt, work( 1_ilp ),work( n ), &
                           vt( ll, 1_ilp ), ldvt )
                 if( nru>0_ilp )call stdlib_qlasr( 'R', 'V', 'F', nru, m-ll+1, work( nm12+1 ),work( &
                           nm13+1 ), u( 1_ilp, ll ), ldu )
                 if( ncc>0_ilp )call stdlib_qlasr( 'L', 'V', 'F', m-ll+1, ncc, work( nm12+1 ),work( &
                           nm13+1 ), c( ll, 1_ilp ), ldc )
                 ! test convergence
                 if( abs( e( m-1 ) )<=thresh )e( m-1 ) = zero
              else
                 ...
                 oldcs = one
                 do i = m, ll + 1, -1
                    call stdlib_qlartg( d( i )*cs, e( i-1 ), cs, sn, r )
                    if( i<m )e( i ) = oldsn*r
                    call stdlib_qlartg( oldcs*r, d( i-1 )*sn, oldcs, oldsn, d( i ) )
                    work( i-ll ) = cs
                    work( i-ll+nm1 ) = -sn
                 ...
                 e( ll ) = h*oldsn
                 ! update singular vectors
                 if( ncvt>0_ilp )call stdlib_qlasr( 'L', 'V', 'B', m-ll+1, ncvt, work( nm12+1 ),work( &
                           nm13+1 ), vt( ll, 1_ilp ), ldvt )
                 if( nru>0_ilp )call stdlib_qlasr( 'R', 'V', 'B', nru, m-ll+1, work( 1_ilp ),work( n ), u(&
                            1_ilp, ll ), ldu )
                 if( ncc>0_ilp )call stdlib_qlasr( 'L', 'V', 'B', m-ll+1, ncc, work( 1_ilp ),work( n ), c(&
                            ll, 1_ilp ), ldc )
                 ! test convergence
                 if( abs( e( ll ) )<=thresh )e( ll ) = zero
              end if
              ...
              if( idir==1_ilp ) then
                 ...
                 g = e( ll )
                 do i = ll, m - 1
                    call stdlib_qlartg( f, g, cosr, sinr, r )
                    if( i>ll )e( i-1 ) = r
                    f = cosr*d( i ) + sinr*e( i )
                    e( i ) = cosr*e( i ) - sinr*d( i )
                    g = sinr*d( i+1 )
                    d( i+1 ) = cosr*d( i+1 )
                    call stdlib_qlartg( f, g, cosl, sinl, r )
                    d( i ) = r
                    f = cosl*e( i ) + sinl*d( i+1 )
                    d( i+1 ) = cosl*d( i+1 ) - sinl*e( i )
                    if( i<m-1 ) then
           ...
           do i = 1, n
              if( d( i )<zero ) then
                 d( i ) = -d( i )
                 ! change sign of singular vectors, if desired
                 if( ncvt>0_ilp )call stdlib_qscal( ncvt, negone, vt( i, 1_ilp ), ldvt )
              end if
           end do
           ! sort the singular values into decreasing order (insertion sort on
           ! singular values, but only one transposition per singular vector)
           do i = 1, n - 1
              ...
              if( isub/=n+1-i ) then
                 ...
                 d( isub ) = d( n+1-i )
                 d( n+1-i ) = smin
                 if( ncvt>0_ilp )call stdlib_qswap( ncvt, vt( isub, 1_ilp ), ldvt, vt( n+1-i, 1_ilp ),ldvt )
                           
                 if( nru>0_ilp )call stdlib_qswap( nru, u( 1_ilp, isub ), 1_ilp, u( 1_ilp, n+1-i ), 1_ilp )
                 if( ncc>0_ilp )call stdlib_qswap( ncc, c( isub, 1_ilp ), ldc, c( n+1-i, 1_ilp ), ldc )
     ...
     !! for a detailed description of the algorithm.
        ...
        ! =====================================================================
           ...
           ! if no singular vectors desired, use qd algorithm
           if( .not.rotate ) then
              call stdlib_qlasq1( n, d, e, rwork, info )
           ! if info equals 2, dqds didn't finish, try to finish
              if( info /= 2 ) return
              info = 0_ilp
           end if
           ...
           if( lower ) then
              ...
              ! update singular vectors if desired
              if( nru>0_ilp )call stdlib_wlasr( 'R', 'V', 'F', nru, n, rwork( 1_ilp ), rwork( n ),u, ldu )
                        
              if( ncc>0_ilp )call stdlib_wlasr( 'L', 'V', 'F', n, ncc, rwork( 1_ilp ), rwork( n ),c, ldc )
           ...
           if( ll==m-1 ) then
              ...
              d( m ) = sigmn
              ! compute singular vectors, if desired
              if( ncvt>0_ilp )call stdlib_wdrot( ncvt, vt( m-1, 1_ilp ), ldvt, vt( m, 1_ilp ), ldvt,cosr, &
                        sinr )
              if( nru>0_ilp )call stdlib_wdrot( nru, u( 1_ilp, m-1 ), 1_ilp, u( 1_ilp, m ), 1_ilp, cosl, sinl )
                        
              if( ncc>0_ilp )call stdlib_wdrot( ncc, c( m-1, 1_ilp ), ldc, c( m, 1_ilp ), ldc, cosl,sinl )
           ...
           if( shift==zero ) then
              if( idir==1_ilp ) then
                 ...
                 e( m-1 ) = h*oldsn
                 ! update singular vectors
                 if( ncvt>0_ilp )call stdlib_wlasr( 'L', 'V', 'F', m-ll+1, ncvt, rwork( 1_ilp ),rwork( n )&
                           , vt( ll, 1_ilp ), ldvt )
                 if( nru>0_ilp )call stdlib_wlasr( 'R', 'V', 'F', nru, m-ll+1, rwork( nm12+1 ),rwork( &
                           nm13+1 ), u( 1_ilp, ll ), ldu )
                 if( ncc>0_ilp )call stdlib_wlasr( 'L', 'V', 'F', m-ll+1, ncc, rwork( nm12+1 ),rwork( &
                           nm13+1 ), c( ll, 1_ilp ), ldc )
                 ! test convergence
                 if( abs( e( m-1 ) )<=thresh )e( m-1 ) = zero
              else
                 ...
                 e( ll ) = h*oldsn
                 ! update singular vectors
                 if( ncvt>0_ilp )call stdlib_wlasr( 'L', 'V', 'B', m-ll+1, ncvt, rwork( nm12+1 ),&
                           rwork( nm13+1 ), vt( ll, 1_ilp ), ldvt )
                 if( nru>0_ilp )call stdlib_wlasr( 'R', 'V', 'B', nru, m-ll+1, rwork( 1_ilp ),rwork( n ), &
                           u( 1_ilp, ll ), ldu )
                 if( ncc>0_ilp )call stdlib_wlasr( 'L', 'V', 'B', m-ll+1, ncc, rwork( 1_ilp ),rwork( n ), &
                           c( ll, 1_ilp ), ldc )
                 ! test convergence
                 if( abs( e( ll ) )<=thresh )e( ll ) = zero
              end if
           ...
           do i = 1, n
              if( d( i )<zero ) then
                 d( i ) = -d( i )
                 ! change sign of singular vectors, if desired
                 if( ncvt>0_ilp )call stdlib_wdscal( ncvt, negone, vt( i, 1_ilp ), ldvt )
              end if
           end do
           ! sort the singular values into decreasing order (insertion sort on
           ! singular values, but only one transposition per singular vector)
           do i = 1, n - 1
              ...
              if( isub/=n+1-i ) then
                 ...
                 d( isub ) = d( n+1-i )
                 d( n+1-i ) = smin
                 if( ncvt>0_ilp )call stdlib_wswap( ncvt, vt( isub, 1_ilp ), ldvt, vt( n+1-i, 1_ilp ),ldvt )
                           
                 if( nru>0_ilp )call stdlib_wswap( nru, u( 1_ilp, isub ), 1_ilp, u( 1_ilp, n+1-i ), 1_ilp )
                 if( ncc>0_ilp )call stdlib_wswap( ncc, c( isub, 1_ilp ), ldc, c( n+1-i, 1_ilp ), ldc )
     ...
     !! using the divide and conquer method.
        ...
        ! =====================================================================
           ...
           qstart = 3_ilp
           if( icompq==1_ilp ) then
              call stdlib_qcopy( n, d, 1_ilp, q( 1_ilp ), 1_ilp )
              call stdlib_qcopy( n-1, e, 1_ilp, q( n+1 ), 1_ilp )
           end if
           if( iuplo==2_ilp ) then
              qstart = 5_ilp
              if( icompq == 2_ilp ) wstart = 2_ilp*n - 1_ilp
           ...
           ! if icompq = 0, use stdlib_qlasdq to compute the singular values.
           if( icompq==0_ilp ) then
              ! ignore wstart, instead using work( 1 ), since the two vectors
              ! for cs and -sn above are added only if icompq == 2,
              ! and adding them exceeds documented work size of 4*n.
              call stdlib_qlasdq( 'U', 0_ilp, n, 0_ilp, 0_ilp, 0_ilp, d, e, vt, ldvt, u, ldu, u,ldu, work( 1_ilp ), &
                        info )
              go to 40
           end if
           ! if n is smaller than the minimum divide size smlsiz, then solve
           ...
           if( n<=smlsiz ) then
              if( icompq==2_ilp ) then
                 call stdlib_qlaset( 'A', n, n, zero, one, u, ldu )
                 call stdlib_qlaset( 'A', n, n, zero, one, vt, ldvt )
                 call stdlib_qlasdq( 'U', 0_ilp, n, n, n, 0_ilp, d, e, vt, ldvt, u, ldu, u,ldu, work( &
                           wstart ), info )
              else if( icompq==1_ilp ) then
                 iu = 1_ilp
                 ivt = iu + n
                 call stdlib_qlaset( 'A', n, n, zero, one, q( iu+( qstart-1 )*n ),n )
                 call stdlib_qlaset( 'A', n, n, zero, one, q( ivt+( qstart-1 )*n ),n )
                 call stdlib_qlasdq( 'U', 0_ilp, n, n, n, 0_ilp, d, e,q( ivt+( qstart-1 )*n ), n,q( iu+( &
                           qstart-1 )*n ), n,q( iu+( qstart-1 )*n ), n, work( wstart ),info )
              end if
              go to 40
           end if
           ...
           if( orgnrm==zero )return
           call stdlib_qlascl( 'G', 0_ilp, 0_ilp, orgnrm, one, n, 1_ilp, d, n, ierr )
           call stdlib_qlascl( 'G', 0_ilp, 0_ilp, orgnrm, one, nm1, 1_ilp, e, nm1, ierr )
           ...
           loop_30: do i = 1, nm1
              if( ( abs( e( i ) )<eps ) .or. ( i==nm1 ) ) then
                 ...
                 if( icompq==2_ilp ) then
                    call stdlib_qlasd0( nsize, sqre, d( start ), e( start ),u( start, start ), &
                              ldu, vt( start, start ),ldvt, smlsiz, iwork, work( wstart ), info )
                 else
                    call stdlib_qlasda( icompq, smlsiz, nsize, sqre, d( start ),e( start ), q( &
                    start+( iu+qstart-2 )*n ), n,q( start+( ivt+qstart-2 )*n ),iq( start+k*n ), q(&
                     start+( difl+qstart-2 )*n ), q( start+( difr+qstart-2 )*n ),q( start+( z+&
                     qstart-2 )*n ),q( start+( poles+qstart-2 )*n ),iq( start+givptr*n ), iq( &
                     start+givcol*n ),n, iq( start+perm*n ),q( start+( givnum+qstart-2 )*n ),q( &
           ...
           ! unscale
           call stdlib_qlascl( 'G', 0_ilp, 0_ilp, one, orgnrm, n, 1_ilp, d, n, ierr )
           40 continue
           ! use selection sort to minimize swaps of singular vectors
           do ii = 2, n
              i = ii - 1_ilp
              ...
              if( kk/=i ) then
                 ...
                 if( icompq==1_ilp ) then
                    ...
                    call stdlib_qswap( n, u( 1_ilp, i ), 1_ilp, u( 1_ilp, kk ), 1_ilp )
                    call stdlib_qswap( n, vt( i, 1_ilp ), ldvt, vt( kk, 1_ilp ), ldvt )
                 end if
              else if( icompq==1_ilp ) then
                 iq( i ) = i
              end if
           ...
           ! if b is lower bidiagonal, update u by those givens rotations
           ! which rotated b to be upper bidiagonal
           if( ( iuplo==2_ilp ) .and. ( icompq==2_ilp ) )call stdlib_qlasr( 'L', 'V', 'B', n, n, work( 1_ilp )&
                     , work( n ), u, ldu )
           return
