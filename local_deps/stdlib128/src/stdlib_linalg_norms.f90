...
submodule(stdlib_linalg) stdlib_linalg_norms
     ...
     interface stride_1d
        ...
        module procedure stride_1d_q
        module procedure stride_1d_c
        module procedure stride_1d_z
        module procedure stride_1d_w
    ...
    ! Compute stride of a 1d array
    pure integer(ilp) function stride_1d_q(a) result(stride)
        !> Input 1-d array 
    ...
    end function stride_1d_q
    
    ! Private internal 1D implementation. This has to be used only internally, 
    ! when all inputs are already checked for correctness.
    pure subroutine internal_norm_1D_q(sze, a, nrm, norm_request)
        !> Input matrix length
        integer(ilp), intent(in) :: sze
        !> Input contiguous 1-d matrix a(*)
        real(qp), intent(in) :: a(sze)
        !> Norm of the matrix.
        ...
        select case(norm_request)
            ...
            case(NORM_TWO)            
                nrm = nrm2(sze,a,incx=1_ilp)
            case(NORM_INF)
                i = stdlib_iqamax(sze,a,incx=1_ilp)
                nrm = abs(a(i))
            case(NORM_MINUSINF)
                nrm = minval( abs(a) )
            case (NORM_POW_FIRST:NORM_POW_LAST)
    ...
    end subroutine internal_norm_1D_q     
    ...
    pure module subroutine norm_1D_char_q(a, nrm, order, err)
        ...
        ! Get norm
        call internal_norm_1D_q(sze, a, nrm, norm_request)
        call linalg_error_handling(err_,err)
    ...
    pure module subroutine norm_2D_to_1D_char_q(a, nrm, order, dim, err)
        ...
        if (.not.contiguous_data) then 
            ...
            do j2 = lbound(apack, 2), ubound(apack, 2)
                call internal_norm_1D_q(lda, apack(:, j2), &
                    nrm(j2), norm_request)
            enddo            
            ...
            do j2 = lbound(a, 2), ubound(a, 2)
                call internal_norm_1D_q(lda, a(:, j2), &
                    nrm(j2), norm_request)
            enddo               
    ...
    pure module subroutine norm_3D_to_2D_char_q(a, nrm, order, dim, err)
        ...
        if (.not.contiguous_data) then 
            ...
            do j3 = lbound(apack, 3), ubound(apack, 3)
            do j2 = lbound(apack, 2), ubound(apack, 2)
                call internal_norm_1D_q(lda, apack(:, j2, j3), &
                    nrm(j2, j3), norm_request)
            enddo; enddo            
            ...
            do j3 = lbound(a, 3), ubound(a, 3)
            do j2 = lbound(a, 2), ubound(a, 2)
                call internal_norm_1D_q(lda, a(:, j2, j3), &
                    nrm(j2, j3), norm_request)
            enddo; enddo               
    ...
    pure module subroutine norm_4D_to_3D_char_q(a, nrm, order, dim, err)
        ...
        if (.not.contiguous_data) then 
            ...
            do j3 = lbound(apack, 3), ubound(apack, 3)
            do j2 = lbound(apack, 2), ubound(apack, 2)
                call internal_norm_1D_q(lda, apack(:, j2, j3, j4), &
                    nrm(j2, j3, j4), norm_request)
            enddo; enddo; enddo            
            ...
            do j3 = lbound(a, 3), ubound(a, 3)
            do j2 = lbound(a, 2), ubound(a, 2)
                call internal_norm_1D_q(lda, a(:, j2, j3, j4), &
                    nrm(j2, j3, j4), norm_request)
            enddo; enddo; enddo               
    ...
    ! Compute stride of a 1d array
    pure integer(ilp) function stride_1d_w(a) result(stride)
        !> Input 1-d array 
    ...
    end function stride_1d_w
    
    ! Private internal 1D implementation. This has to be used only internally, 
    ! when all inputs are already checked for correctness.
    pure subroutine internal_norm_1D_w(sze, a, nrm, norm_request)
        !> Input matrix length
        integer(ilp), intent(in) :: sze
        !> Input contiguous 1-d matrix a(*)
        complex(qp), intent(in) :: a(sze)
        !> Norm of the matrix.
        real(qp), intent(out) :: nrm
        !> Internal matrix request 
        integer(ilp), intent(in) :: norm_request
        ...
        select case(norm_request)
            ...
            case(NORM_TWO)            
                nrm = nrm2(sze,a,incx=1_ilp)
            case(NORM_INF)
                ! The default BLAS stdlib_iwamax uses |Re(.)|+|Im(.)|. Do not use it.
                i = stdlib_iwmax1(sze,a,incx=1_ilp)
                nrm = abs(a(i))
            case(NORM_MINUSINF)
                nrm = minval( abs(a) )
            case (NORM_POW_FIRST:NORM_POW_LAST)
    ...
    end subroutine internal_norm_1D_w     
    ...
    pure module subroutine norm_1D_char_w(a, nrm, order, err)
        ...
        ! Get norm
        call internal_norm_1D_w(sze, a, nrm, norm_request)
        call linalg_error_handling(err_,err)
    ...
    pure module subroutine norm_2D_to_1D_char_w(a, nrm, order, dim, err)
        ...
        if (.not.contiguous_data) then 
            ...
            do j2 = lbound(apack, 2), ubound(apack, 2)
                call internal_norm_1D_w(lda, apack(:, j2), &
                    nrm(j2), norm_request)
            enddo            
            ...
            do j2 = lbound(a, 2), ubound(a, 2)
                call internal_norm_1D_w(lda, a(:, j2), &
                    nrm(j2), norm_request)
            enddo               
    ...
    pure module subroutine norm_3D_to_2D_char_w(a, nrm, order, dim, err)
        ...
        if (.not.contiguous_data) then 
            ...
            do j3 = lbound(apack, 3), ubound(apack, 3)
            do j2 = lbound(apack, 2), ubound(apack, 2)
                call internal_norm_1D_w(lda, apack(:, j2, j3), &
                    nrm(j2, j3), norm_request)
            enddo; enddo            
            ...
            do j3 = lbound(a, 3), ubound(a, 3)
            do j2 = lbound(a, 2), ubound(a, 2)
                call internal_norm_1D_w(lda, a(:, j2, j3), &
                    nrm(j2, j3), norm_request)
            enddo; enddo               
    ...
    pure module subroutine norm_4D_to_3D_char_w(a, nrm, order, dim, err)
        ...
        if (.not.contiguous_data) then 
            ...
            do j3 = lbound(apack, 3), ubound(apack, 3)
            do j2 = lbound(apack, 2), ubound(apack, 2)
                call internal_norm_1D_w(lda, apack(:, j2, j3, j4), &
                    nrm(j2, j3, j4), norm_request)
            enddo; enddo; enddo            
            ...
            do j3 = lbound(a, 3), ubound(a, 3)
            do j2 = lbound(a, 2), ubound(a, 2)
                call internal_norm_1D_w(lda, a(:, j2, j3, j4), &
                    nrm(j2, j3, j4), norm_request)
            enddo; enddo; enddo               
