...
module stdlib_sparse_kinds
    ...
    type, public, extends(COO_type) :: COO_qp_type
        real(qp), allocatable :: data(:) 
    contains
        procedure, non_overridable :: at => at_value_coo_qp
        procedure, non_overridable :: add_value => add_value_coo_qp
        procedure, non_overridable :: add_block => add_block_coo_qp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(COO_type) :: COO_cqp_type
        complex(qp), allocatable :: data(:) 
    contains
        procedure, non_overridable :: at => at_value_coo_cqp
        procedure, non_overridable :: add_value => add_value_coo_cqp
        procedure, non_overridable :: add_block => add_block_coo_cqp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(CSR_type) :: CSR_qp_type
        real(qp), allocatable :: data(:) 
    contains
        procedure, non_overridable :: at => at_value_csr_qp
        procedure, non_overridable :: add_value => add_value_csr_qp
        procedure, non_overridable :: add_block => add_block_csr_qp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(CSR_type) :: CSR_cqp_type
        complex(qp), allocatable :: data(:) 
    contains
        procedure, non_overridable :: at => at_value_csr_cqp
        procedure, non_overridable :: add_value => add_value_csr_cqp
        procedure, non_overridable :: add_block => add_block_csr_cqp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(CSC_type) :: CSC_qp_type
        real(qp), allocatable :: data(:) 
    contains
        procedure, non_overridable :: at => at_value_csc_qp
        procedure, non_overridable :: add_value => add_value_csc_qp
        procedure, non_overridable :: add_block => add_block_csc_qp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(CSC_type) :: CSC_cqp_type
        complex(qp), allocatable :: data(:) 
    contains
        procedure, non_overridable :: at => at_value_csc_cqp
        procedure, non_overridable :: add_value => add_value_csc_cqp
        procedure, non_overridable :: add_block => add_block_csc_cqp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(ELL_type) :: ELL_qp_type
        real(qp), allocatable :: data(:,:) 
    contains
        procedure, non_overridable :: at => at_value_ell_qp
        procedure, non_overridable :: add_value => add_value_ell_qp
        procedure, non_overridable :: add_block => add_block_ell_qp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(ELL_type) :: ELL_cqp_type
        complex(qp), allocatable :: data(:,:) 
    contains
        procedure, non_overridable :: at => at_value_ell_cqp
        procedure, non_overridable :: add_value => add_value_ell_cqp
        procedure, non_overridable :: add_block => add_block_ell_cqp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(SELLC_type) :: SELLC_qp_type
        real(qp), allocatable :: data(:,:)
    contains
        procedure, non_overridable :: at => at_value_sellc_qp
        procedure, non_overridable :: add_value => add_value_sellc_qp
        procedure, non_overridable :: add_block => add_block_sellc_qp
        generic :: add => add_value, add_block
    end type
    ...
    type, public, extends(SELLC_type) :: SELLC_cqp_type
        complex(qp), allocatable :: data(:,:)
    contains
        procedure, non_overridable :: at => at_value_sellc_cqp
        procedure, non_overridable :: add_value => add_value_sellc_cqp
        procedure, non_overridable :: add_block => add_block_sellc_cqp
        generic :: add => add_value, add_block
    end type
...
contains
    ...
    subroutine malloc_coo(self,num_rows,num_cols,nnz)
        ...
        select type(self)
            ...
            type is(COO_qp_type)
                block
                real(qp), allocatable :: temp_data_qp(:)
                if(.not.allocated(self%data)) then
                    allocate(temp_data_qp(nnz) , source = zero_qp )
                else
                    allocate(temp_data_qp(nnz) , source = self%data )
                end if
                call move_alloc(from=temp_data_qp,to=self%data)
                end block
            ...
            type is(COO_cqp_type)
                block
                complex(qp), allocatable :: temp_data_cqp(:)
                if(.not.allocated(self%data)) then
                    allocate(temp_data_cqp(nnz) , source = zero_cqp )
                else
                    allocate(temp_data_cqp(nnz) , source = self%data )
                end if
                call move_alloc(from=temp_data_cqp,to=self%data)
                end block
        end select
    end subroutine
    ...
    subroutine malloc_ell(self,num_rows,num_cols,num_nz_rows)
        ...
        select type(self)
            ...
            type is(ELL_qp_type)
                ...
                real(qp), allocatable :: temp_data_qp(:,:)
                if(.not.allocated(self%data)) then
                    allocate(temp_data_qp(num_rows,num_nz_rows) , source = zero_qp )
                else
                    allocate(temp_data_qp(num_rows,num_nz_rows) , source = self%data )
                end if
                call move_alloc(from=temp_data_qp,to=self%data)
                end block
            ...
            type is(ELL_cqp_type)
                block
                complex(qp), allocatable :: temp_data_cqp(:,:)
                if(.not.allocated(self%data)) then
                    allocate(temp_data_cqp(num_rows,num_nz_rows) , source = zero_cqp )
                else
                    allocate(temp_data_cqp(num_rows,num_nz_rows) , source = self%data )
                end if
                call move_alloc(from=temp_data_cqp,to=self%data)
                end block
        end select
    ...
    pure real(qp) function at_value_coo_qp(self,ik,jk) result(val)
        class(COO_qp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_
        logical :: transpose
        ! naive implementation
        if( (ik<1 .or. ik>self%nrows) .or. (jk<1 .or. jk>self%ncols) ) then
            val = ieee_value( 0._qp , ieee_quiet_nan)
            return
        end if
        ik_ = ik; jk_ = jk
        transpose = (self%storage == sparse_lower .and. ik > jk) .or. (self%storage == sparse_upper .and. ik < jk)
    ...
    subroutine add_value_coo_qp(self,ik,jk,val)
        class(COO_qp_type), intent(inout) :: self
    ...
    subroutine add_block_coo_qp(self,ik,jk,val)
        class(COO_qp_type), intent(inout) :: self
        real(qp), intent(in) :: val(:,:)
        integer(ilp), intent(in) :: ik(:), jk(:)
        integer(ilp) :: k, i, j
    ...
    pure complex(qp) function at_value_coo_cqp(self,ik,jk) result(val)
        class(COO_cqp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_coo_cqp(self,ik,jk,val)
        class(COO_cqp_type), intent(inout) :: self
    ...
    subroutine add_block_coo_cqp(self,ik,jk,val)
        class(COO_cqp_type), intent(inout) :: self
        complex(qp), intent(in) :: val(:,:)
        integer(ilp), intent(in) :: ik(:), jk(:)
        integer(ilp) :: k, i, j
    ...
    pure real(qp) function at_value_csr_qp(self,ik,jk) result(val)
        class(CSR_qp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_csr_qp(self,ik,jk,val)
        class(CSR_qp_type), intent(inout) :: self
        real(qp), intent(in) :: val
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k
        ! naive implementation
    ...
    subroutine add_block_csr_qp(self,ik,jk,val)
        class(CSR_qp_type), intent(inout) :: self
        real(qp), intent(in) :: val(:,:)
        integer(ilp), intent(in) :: ik(:), jk(:)
        integer(ilp) :: k, i, j
    ...
    pure complex(qp) function at_value_csr_cqp(self,ik,jk) result(val)
        class(CSR_cqp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_csr_cqp(self,ik,jk,val)
        class(CSR_cqp_type), intent(inout) :: self
        complex(qp), intent(in) :: val
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k
        ! naive implementation
    ...
    subroutine add_block_csr_cqp(self,ik,jk,val)
        class(CSR_cqp_type), intent(inout) :: self
        complex(qp), intent(in) :: val(:,:)
        integer(ilp), intent(in) :: ik(:), jk(:)
        integer(ilp) :: k, i, j
    ...
    pure real(qp) function at_value_csc_qp(self,ik,jk) result(val)
        class(CSC_qp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_csc_qp(self,ik,jk,val)
        class(CSC_qp_type), intent(inout) :: self
        real(qp), intent(in) :: val
        integer(ilp), intent(in)  :: ik, jk
        integer(ilp) :: k
        ! naive implementation
    ...
    subroutine add_block_csc_qp(self,ik,jk,val)
        class(CSC_qp_type), intent(inout) :: self
        real(qp), intent(in) :: val(:,:)
        integer(ilp), intent(in)  :: ik(:), jk(:)
        integer(ilp) :: k, i, j
    ...
    pure complex(qp) function at_value_csc_cqp(self,ik,jk) result(val)
        class(CSC_cqp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_csc_cqp(self,ik,jk,val)
        class(CSC_cqp_type), intent(inout) :: self
        complex(qp), intent(in) :: val
        integer(ilp), intent(in)  :: ik, jk
        integer(ilp) :: k
        ! naive implementation
    ...
    subroutine add_block_csc_cqp(self,ik,jk,val)
        class(CSC_cqp_type), intent(inout) :: self
        complex(qp), intent(in) :: val(:,:)
        integer(ilp), intent(in)  :: ik(:), jk(:)
        integer(ilp) :: k, i, j
    ...
    pure real(qp) function at_value_ell_qp(self,ik,jk) result(val)
        class(ELL_qp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_ell_qp(self,ik,jk,val)
        class(ELL_qp_type), intent(inout) :: self
        real(qp), intent(in) :: val
        integer(ilp), intent(in)  :: ik, jk
        integer(ilp) :: k
        ! naive implementation
    ...
    subroutine add_block_ell_qp(self,ik,jk,val)
        class(ELL_qp_type), intent(inout) :: self
        real(qp), intent(in) :: val(:,:)
        integer(ilp), intent(in)  :: ik(:), jk(:)
        integer(ilp) :: k, i, j
    ...
    pure complex(qp) function at_value_ell_cqp(self,ik,jk) result(val)
        class(ELL_cqp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_ell_cqp(self,ik,jk,val)
        class(ELL_cqp_type), intent(inout) :: self
        complex(qp), intent(in) :: val
        integer(ilp), intent(in)  :: ik, jk
        integer(ilp) :: k
        ! naive implementation
    ...
    subroutine add_block_ell_cqp(self,ik,jk,val)
        class(ELL_cqp_type), intent(inout) :: self
        complex(qp), intent(in) :: val(:,:)
        integer(ilp), intent(in)  :: ik(:), jk(:)
        integer(ilp) :: k, i, j
    ...
    pure real(qp) function at_value_sellc_qp(self,ik,jk) result(val)
        class(SELLC_qp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_, idx
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_sellc_qp(self,ik,jk,val)
        class(SELLC_qp_type), intent(inout) :: self
        real(qp), intent(in) :: val
        integer(ilp), intent(in)  :: ik, jk
        integer(ilp) :: k, idx
        ! naive implementation
    ...
    subroutine add_block_sellc_qp(self,ik,jk,val)
        class(SELLC_qp_type), intent(inout) :: self
        real(qp), intent(in) :: val(:,:)
        integer(ilp), intent(in)  :: ik(:), jk(:)
        integer(ilp) :: k, i, j, idx
    ...
    pure complex(qp) function at_value_sellc_cqp(self,ik,jk) result(val)
        class(SELLC_cqp_type), intent(in) :: self
        integer(ilp), intent(in) :: ik, jk
        integer(ilp) :: k, ik_, jk_, idx
        logical :: transpose
        ! naive implementation
    ...
    subroutine add_value_sellc_cqp(self,ik,jk,val)
        class(SELLC_cqp_type), intent(inout) :: self
        complex(qp), intent(in) :: val
        integer(ilp), intent(in)  :: ik, jk
        integer(ilp) :: k, idx
        ! naive implementation
    ...
    subroutine add_block_sellc_cqp(self,ik,jk,val)
        class(SELLC_cqp_type), intent(inout) :: self
        complex(qp), intent(in) :: val(:,:)
        integer(ilp), intent(in)  :: ik(:), jk(:)
        integer(ilp) :: k, i, j, idx
