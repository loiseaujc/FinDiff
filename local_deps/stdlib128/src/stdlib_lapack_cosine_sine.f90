submodule(stdlib_lapack_eig_svd_lsq) stdlib_lapack_cosine_sine
  ...
  contains
     ...
     ldu1, u2, ldu2, v1t, ldv1t,v2t, ldv2t, b11d, b11e, b12d, b12e, b21d, b21e,b22d, b22e, work, &
        ...
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           use stdlib_blas_constants_qp, only: negone, zero, one, ten, cnegone
           ! Scalar Arguments 
           character, intent(in) :: jobu1, jobu2, jobv1t, jobv2t, trans
           integer(ilp), intent(out) :: info
           integer(ilp), intent(in) :: ldu1, ldu2, ldv1t, ldv2t, lwork, m, p, q
        ...
        ! ===================================================================
           ...
           do while( imax > 1 )
              ...
              ! rotate to produce bulges in b11 and b21
              if( mu <= nu ) then
                 call stdlib_qlartgs( b11d(imin), b11e(imin), mu,work(iv1tcs+imin-1), work(iv1tsn+&
                           imin-1) )
              else
                 call stdlib_qlartgs( b21d(imin), b21e(imin), nu,work(iv1tcs+imin-1), work(iv1tsn+&
                           imin-1) )
              end if
              temp = work(iv1tcs+imin-1)*b11d(imin) +work(iv1tsn+imin-1)*b11e(imin)
              b11e(imin) = work(iv1tcs+imin-1)*b11e(imin) -work(iv1tsn+imin-1)*b11d(imin)
              ...
              ! chase the bulges in b11(imin+1,imin) and b21(imin+1,imin)
              if( b11d(imin)**2_ilp+b11bulge**2_ilp > thresh**2_ilp ) then
                 call stdlib_qlartgp( b11bulge, b11d(imin), work(iu1sn+imin-1),work(iu1cs+imin-1),&
                            r )
              else if( mu <= nu ) then
                 call stdlib_qlartgs( b11e( imin ), b11d( imin + 1_ilp ), mu,work(iu1cs+imin-1), work(&
                           iu1sn+imin-1) )
              else
                 call stdlib_qlartgs( b12d( imin ), b12e( imin ), nu,work(iu1cs+imin-1), work(&
                           iu1sn+imin-1) )
              end if
              if( b21d(imin)**2_ilp+b21bulge**2_ilp > thresh**2_ilp ) then
                 call stdlib_qlartgp( b21bulge, b21d(imin), work(iu2sn+imin-1),work(iu2cs+imin-1),&
                            r )
              else if( nu < mu ) then
                 call stdlib_qlartgs( b21e( imin ), b21d( imin + 1_ilp ), nu,work(iu2cs+imin-1), work(&
                           iu2sn+imin-1) )
              else
                 call stdlib_qlartgs( b22d(imin), b22e(imin), mu,work(iu2cs+imin-1), work(iu2sn+&
                           imin-1) )
              end if
              work(iu2cs+imin-1) = -work(iu2cs+imin-1)
              work(iu2sn+imin-1) = -work(iu2sn+imin-1)
              ...
              do i = imin+1, imax-1
                 ...
                 ! chasing by applying the original shift again.
                 if( .not. restart11 .and. .not. restart21 ) then
                    call stdlib_qlartgp( x2, x1, work(iv1tsn+i-1), work(iv1tcs+i-1),r )
                 else if( .not. restart11 .and. restart21 ) then
                    call stdlib_qlartgp( b11bulge, b11e(i-1), work(iv1tsn+i-1),work(iv1tcs+i-1), &
                              r )
                 else if( restart11 .and. .not. restart21 ) then
                    call stdlib_qlartgp( b21bulge, b21e(i-1), work(iv1tsn+i-1),work(iv1tcs+i-1), &
                              r )
                 else if( mu <= nu ) then
                 ...
                 if( .not. restart12 .and. .not. restart22 ) then
                    ...
                    call stdlib_qlartgp( b22bulge, b22d(i-1), work(iv2tsn+i-1-1),work(iv2tcs+i-1-&
                              1_ilp), r )
                 else if( nu < mu ) then
                    call stdlib_qlartgs( b12e(i-1), b12d(i), nu, work(iv2tcs+i-1-1),work(iv2tsn+i-&
                              1_ilp-1) )
                 else
                    call stdlib_qlartgs( b22e(i-1), b22d(i), mu, work(iv2tcs+i-1-1),work(iv2tsn+i-&
                              1_ilp-1) )
                 end if
                 temp = work(iv1tcs+i-1)*b11d(i) + work(iv1tsn+i-1)*b11e(i)
                 b11e(i) = work(iv1tcs+i-1)*b11e(i) -work(iv1tsn+i-1)*b11d(i)
              ...
              if( .not. restart12 .and. .not. restart22 ) then
                 ...
                 call stdlib_qlartgp( b22bulge, b22d(imax-1), work(iv2tsn+imax-1-1),work(iv2tcs+&
                           imax-1-1), r )
              else if( nu < mu ) then
                 call stdlib_qlartgs( b12e(imax-1), b12d(imax), nu,work(iv2tcs+imax-1-1), work(&
                           iv2tsn+imax-1-1) )
              else
                 call stdlib_qlartgs( b22e(imax-1), b22d(imax), mu,work(iv2tcs+imax-1-1), work(&
                           iv2tsn+imax-1-1) )
              end if
              temp = work(iv2tcs+imax-1-1)*b12e(imax-1) +work(iv2tsn+imax-1-1)*b12d(imax)
              b12d(imax) = work(iv2tcs+imax-1-1)*b12d(imax) -work(iv2tsn+imax-1-1)*b12e(imax-1)
              ...
              if( wantu1 ) then
                 if( colmajor ) then
                    call stdlib_qlasr( 'R', 'V', 'F', p, imax-imin+1,work(iu1cs+imin-1), work(&
                              iu1sn+imin-1),u1(1_ilp,imin), ldu1 )
                 else
                    call stdlib_qlasr( 'L', 'V', 'F', imax-imin+1, p,work(iu1cs+imin-1), work(&
                              iu1sn+imin-1),u1(imin,1_ilp), ldu1 )
                 end if
              end if
              if( wantu2 ) then
                 if( colmajor ) then
                    call stdlib_qlasr( 'R', 'V', 'F', m-p, imax-imin+1,work(iu2cs+imin-1), work(&
                              iu2sn+imin-1),u2(1_ilp,imin), ldu2 )
                 else
                    call stdlib_qlasr( 'L', 'V', 'F', imax-imin+1, m-p,work(iu2cs+imin-1), work(&
                              iu2sn+imin-1),u2(imin,1_ilp), ldu2 )
                 end if
              end if
              if( wantv1t ) then
                 if( colmajor ) then
                    call stdlib_qlasr( 'L', 'V', 'F', imax-imin+1, q,work(iv1tcs+imin-1), work(&
                              iv1tsn+imin-1),v1t(imin,1_ilp), ldv1t )
                 else
                    call stdlib_qlasr( 'R', 'V', 'F', q, imax-imin+1,work(iv1tcs+imin-1), work(&
                              iv1tsn+imin-1),v1t(1_ilp,imin), ldv1t )
                 end if
              end if
              if( wantv2t ) then
                 if( colmajor ) then
                    call stdlib_qlasr( 'L', 'V', 'F', imax-imin+1, m-q,work(iv2tcs+imin-1), work(&
                              iv2tsn+imin-1),v2t(imin,1_ilp), ldv2t )
                 else
                    call stdlib_qlasr( 'R', 'V', 'F', m-q, imax-imin+1,work(iv2tcs+imin-1), work(&
                              iv2tsn+imin-1),v2t(1_ilp,imin), ldv2t )
                 end if
              end if
              ! fix signs on b11(imax-1,imax) and b21(imax-1,imax)
              if( b11e(imax-1)+b21e(imax-1) > 0_ilp ) then
                 ...
                 if( wantv1t ) then
                    if( colmajor ) then
                       call stdlib_qscal( q, negone, v1t(imax,1_ilp), ldv1t )
                    else
                       call stdlib_qscal( q, negone, v1t(1_ilp,imax), 1_ilp )
                    end if
                 end if
              end if
              ! compute theta(imax)
              ...
              if( b11d(imax)+b12e(imax-1) < 0_ilp ) then
                 ...
                 if( wantu1 ) then
                    if( colmajor ) then
                       call stdlib_qscal( p, negone, u1(1_ilp,imax), 1_ilp )
                    else
                       call stdlib_qscal( p, negone, u1(imax,1_ilp), ldu1 )
                    end if
                 end if
              end if
              if( b21d(imax)+b22e(imax-1) > 0_ilp ) then
                 ...
                 if( wantu2 ) then
                    if( colmajor ) then
                       call stdlib_qscal( m-p, negone, u2(1_ilp,imax), 1_ilp )
                    else
                       call stdlib_qscal( m-p, negone, u2(imax,1_ilp), ldu2 )
                    end if
                 end if
              end if
              ! fix signs on b12(imax,imax) and b22(imax,imax)
              if( b12d(imax)+b22d(imax) < 0_ilp ) then
                 if( wantv2t ) then
                    if( colmajor ) then
                       call stdlib_qscal( m-q, negone, v2t(imax,1_ilp), ldv2t )
                    else
                       call stdlib_qscal( m-q, negone, v2t(1_ilp,imax), 1_ilp )
                    end if
                 end if
              end if
              ! test for negligible sines or cosines
           ...
           do i = 1, q
              ...
              if( mini /= i ) then
                 ...
                 theta(i) = thetamin
                 if( colmajor ) then
                    if( wantu1 )call stdlib_qswap( p, u1(1_ilp,i), 1_ilp, u1(1_ilp,mini), 1_ilp )
                    if( wantu2 )call stdlib_qswap( m-p, u2(1_ilp,i), 1_ilp, u2(1_ilp,mini), 1_ilp )
                    if( wantv1t )call stdlib_qswap( q, v1t(i,1_ilp), ldv1t, v1t(mini,1_ilp), ldv1t )
                              
                    if( wantv2t )call stdlib_qswap( m-q, v2t(i,1_ilp), ldv2t, v2t(mini,1_ilp),ldv2t )
                              
                 else
                    if( wantu1 )call stdlib_qswap( p, u1(i,1_ilp), ldu1, u1(mini,1_ilp), ldu1 )
                    if( wantu2 )call stdlib_qswap( m-p, u2(i,1_ilp), ldu2, u2(mini,1_ilp), ldu2 )
                    if( wantv1t )call stdlib_qswap( q, v1t(1_ilp,i), 1_ilp, v1t(1_ilp,mini), 1_ilp )
                    if( wantv2t )call stdlib_qswap( m-q, v2t(1_ilp,i), 1_ilp, v2t(1_ilp,mini), 1_ilp )
                 end if
              end if
           end do
           return
     ...
     ldu1, u2, ldu2, v1t, ldv1t,v2t, ldv2t, b11d, b11e, b12d, b12e, b21d, b21e,b22d, b22e, rwork, &
        ...
        ! ===================================================================
           ...
           do while( imax > 1 )
              ...
              ! rotate to produce bulges in b11 and b21
              if( mu <= nu ) then
                 call stdlib_qlartgs( b11d(imin), b11e(imin), mu,rwork(iv1tcs+imin-1), rwork(&
                           iv1tsn+imin-1) )
              else
                 call stdlib_qlartgs( b21d(imin), b21e(imin), nu,rwork(iv1tcs+imin-1), rwork(&
                           iv1tsn+imin-1) )
              end if
              temp = rwork(iv1tcs+imin-1)*b11d(imin) +rwork(iv1tsn+imin-1)*b11e(imin)
              b11e(imin) = rwork(iv1tcs+imin-1)*b11e(imin) -rwork(iv1tsn+imin-1)*b11d(imin)
              ...
              ! chase the bulges in b11(imin+1,imin) and b21(imin+1,imin)
              if( b11d(imin)**2_ilp+b11bulge**2_ilp > thresh**2_ilp ) then
                 call stdlib_qlartgp( b11bulge, b11d(imin), rwork(iu1sn+imin-1),rwork(iu1cs+imin-&
                           1_ilp), r )
              else if( mu <= nu ) then
                 call stdlib_qlartgs( b11e( imin ), b11d( imin + 1_ilp ), mu,rwork(iu1cs+imin-1), &
                           rwork(iu1sn+imin-1) )
              else
                 call stdlib_qlartgs( b12d( imin ), b12e( imin ), nu,rwork(iu1cs+imin-1), rwork(&
                           iu1sn+imin-1) )
              end if
              if( b21d(imin)**2_ilp+b21bulge**2_ilp > thresh**2_ilp ) then
                 call stdlib_qlartgp( b21bulge, b21d(imin), rwork(iu2sn+imin-1),rwork(iu2cs+imin-&
                           1_ilp), r )
              else if( nu < mu ) then
                 call stdlib_qlartgs( b21e( imin ), b21d( imin + 1_ilp ), nu,rwork(iu2cs+imin-1), &
                           rwork(iu2sn+imin-1) )
              else
                 call stdlib_qlartgs( b22d(imin), b22e(imin), mu,rwork(iu2cs+imin-1), rwork(iu2sn+&
                           imin-1) )
              end if
              rwork(iu2cs+imin-1) = -rwork(iu2cs+imin-1)
              rwork(iu2sn+imin-1) = -rwork(iu2sn+imin-1)
              ...
              do i = imin+1, imax-1
                 ...
                 ! chasing by applying the original shift again.
                 if( .not. restart11 .and. .not. restart21 ) then
                    call stdlib_qlartgp( x2, x1, rwork(iv1tsn+i-1),rwork(iv1tcs+i-1), r )
                 else if( .not. restart11 .and. restart21 ) then
                    call stdlib_qlartgp( b11bulge, b11e(i-1), rwork(iv1tsn+i-1),rwork(iv1tcs+i-1),&
                               r )
                 else if( restart11 .and. .not. restart21 ) then
                    call stdlib_qlartgp( b21bulge, b21e(i-1), rwork(iv1tsn+i-1),rwork(iv1tcs+i-1),&
                               r )
                 else if( mu <= nu ) then
                    call stdlib_qlartgs( b11d(i), b11e(i), mu, rwork(iv1tcs+i-1),rwork(iv1tsn+i-1)&
                               )
                 else
                    call stdlib_qlartgs( b21d(i), b21e(i), nu, rwork(iv1tcs+i-1),rwork(iv1tsn+i-1)&
                               )
                 end if
                 rwork(iv1tcs+i-1) = -rwork(iv1tcs+i-1)
                 rwork(iv1tsn+i-1) = -rwork(iv1tsn+i-1)
                 if( .not. restart12 .and. .not. restart22 ) then
                    ...
                    call stdlib_qlartgp( b22bulge, b22d(i-1), rwork(iv2tsn+i-1-1),rwork(iv2tcs+i-&
                              1_ilp-1), r )
                 else if( nu < mu ) then
                    call stdlib_qlartgs( b12e(i-1), b12d(i), nu,rwork(iv2tcs+i-1-1), rwork(iv2tsn+&
                              i-1-1) )
                 else
                    call stdlib_qlartgs( b22e(i-1), b22d(i), mu,rwork(iv2tcs+i-1-1), rwork(iv2tsn+&
                              i-1-1) )
                 end if
                 temp = rwork(iv1tcs+i-1)*b11d(i) + rwork(iv1tsn+i-1)*b11e(i)
                 b11e(i) = rwork(iv1tcs+i-1)*b11e(i) -rwork(iv1tsn+i-1)*b11d(i)
                 ...
                 if( .not. restart11 .and. .not. restart12 ) then
                    ...
                    call stdlib_qlartgs( b11e(i), b11d(i+1), mu, rwork(iu1cs+i-1),rwork(iu1sn+i-1)&
                               )
                 else
                 ...
                 if( .not. restart21 .and. .not. restart22 ) then
                    ...
                    call stdlib_qlartgs( b21e(i), b21e(i+1), nu, rwork(iu2cs+i-1),rwork(iu2sn+i-1)&
                               )
                 else
              ...
              if( .not. restart12 .and. .not. restart22 ) then
                 ...
                 call stdlib_qlartgp( b22bulge, b22d(imax-1),rwork(iv2tsn+imax-1-1),rwork(iv2tcs+&
                           imax-1-1), r )
              else if( nu < mu ) then
                 call stdlib_qlartgs( b12e(imax-1), b12d(imax), nu,rwork(iv2tcs+imax-1-1),rwork(&
                           iv2tsn+imax-1-1) )
              else
                 call stdlib_qlartgs( b22e(imax-1), b22d(imax), mu,rwork(iv2tcs+imax-1-1),rwork(&
                           iv2tsn+imax-1-1) )
              end if
              temp = rwork(iv2tcs+imax-1-1)*b12e(imax-1) +rwork(iv2tsn+imax-1-1)*b12d(imax)
              ...
              if( wantu1 ) then
                 if( colmajor ) then
                    call stdlib_wlasr( 'R', 'V', 'F', p, imax-imin+1,rwork(iu1cs+imin-1), rwork(&
                              iu1sn+imin-1),u1(1_ilp,imin), ldu1 )
                 else
                    call stdlib_wlasr( 'L', 'V', 'F', imax-imin+1, p,rwork(iu1cs+imin-1), rwork(&
                              iu1sn+imin-1),u1(imin,1_ilp), ldu1 )
                 end if
              end if
              if( wantu2 ) then
                 if( colmajor ) then
                    call stdlib_wlasr( 'R', 'V', 'F', m-p, imax-imin+1,rwork(iu2cs+imin-1), rwork(&
                              iu2sn+imin-1),u2(1_ilp,imin), ldu2 )
                 else
                    call stdlib_wlasr( 'L', 'V', 'F', imax-imin+1, m-p,rwork(iu2cs+imin-1), rwork(&
                              iu2sn+imin-1),u2(imin,1_ilp), ldu2 )
                 end if
              end if
              if( wantv1t ) then
                 if( colmajor ) then
                    call stdlib_wlasr( 'L', 'V', 'F', imax-imin+1, q,rwork(iv1tcs+imin-1), rwork(&
                              iv1tsn+imin-1),v1t(imin,1_ilp), ldv1t )
                 else
                    call stdlib_wlasr( 'R', 'V', 'F', q, imax-imin+1,rwork(iv1tcs+imin-1), rwork(&
                              iv1tsn+imin-1),v1t(1_ilp,imin), ldv1t )
                 end if
              end if
              if( wantv2t ) then
                 if( colmajor ) then
                    call stdlib_wlasr( 'L', 'V', 'F', imax-imin+1, m-q,rwork(iv2tcs+imin-1), &
                              rwork(iv2tsn+imin-1),v2t(imin,1_ilp), ldv2t )
                 else
                    call stdlib_wlasr( 'R', 'V', 'F', m-q, imax-imin+1,rwork(iv2tcs+imin-1), &
                              rwork(iv2tsn+imin-1),v2t(1_ilp,imin), ldv2t )
                 end if
              end if
              ! fix signs on b11(imax-1,imax) and b21(imax-1,imax)
              if( b11e(imax-1)+b21e(imax-1) > 0_ilp ) then
                 ...
                 if( wantv1t ) then
                    if( colmajor ) then
                       call stdlib_wscal( q, cnegone, v1t(imax,1_ilp), ldv1t )
                    else
                       call stdlib_wscal( q, cnegone, v1t(1_ilp,imax), 1_ilp )
                    end if
                 end if
              end if
              ! compute theta(imax)
              ...
              if( b11d(imax)+b12e(imax-1) < 0_ilp ) then
                 ...
                 if( wantu1 ) then
                    if( colmajor ) then
                       call stdlib_wscal( p, cnegone, u1(1_ilp,imax), 1_ilp )
                    else
                       call stdlib_wscal( p, cnegone, u1(imax,1_ilp), ldu1 )
                    end if
                 end if
              end if
              if( b21d(imax)+b22e(imax-1) > 0_ilp ) then
                 ...
                 if( wantu2 ) then
                    if( colmajor ) then
                       call stdlib_wscal( m-p, cnegone, u2(1_ilp,imax), 1_ilp )
                    else
                       call stdlib_wscal( m-p, cnegone, u2(imax,1_ilp), ldu2 )
                    end if
                 end if
              end if
              ! fix signs on b12(imax,imax) and b22(imax,imax)
              if( b12d(imax)+b22d(imax) < 0_ilp ) then
                 if( wantv2t ) then
                    if( colmajor ) then
                       call stdlib_wscal( m-q, cnegone, v2t(imax,1_ilp), ldv2t )
                    else
                       call stdlib_wscal( m-q, cnegone, v2t(1_ilp,imax), 1_ilp )
                    end if
                 end if
              end if
              ! test for negligible sines or cosines
           ...
           do i = 1, q
              ...
              if( mini /= i ) then
                 ...
                 theta(i) = thetamin
                 if( colmajor ) then
                    if( wantu1 )call stdlib_wswap( p, u1(1_ilp,i), 1_ilp, u1(1_ilp,mini), 1_ilp )
                    if( wantu2 )call stdlib_wswap( m-p, u2(1_ilp,i), 1_ilp, u2(1_ilp,mini), 1_ilp )
                    if( wantv1t )call stdlib_wswap( q, v1t(i,1_ilp), ldv1t, v1t(mini,1_ilp), ldv1t )
                              
                    if( wantv2t )call stdlib_wswap( m-q, v2t(i,1_ilp), ldv2t, v2t(mini,1_ilp),ldv2t )
                              
                 else
                    if( wantu1 )call stdlib_wswap( p, u1(i,1_ilp), ldu1, u1(mini,1_ilp), ldu1 )
                    if( wantu2 )call stdlib_wswap( m-p, u2(i,1_ilp), ldu2, u2(mini,1_ilp), ldu2 )
                    if( wantv1t )call stdlib_wswap( q, v1t(1_ilp,i), 1_ilp, v1t(1_ilp,mini), 1_ilp )
                    if( wantv2t )call stdlib_wswap( m-q, v2t(1_ilp,i), 1_ilp, v2t(1_ilp,mini), 1_ilp )
                 end if
              end if
           end do
           return
     ...
     ldx11, x12,ldx12, x21, ldx21, x22, ldx22, theta,u1, ldu1, u2, ldu2, v1t, ldv1t, v2t,ldv2t, &
        ...
        ! ===================================================================
           ...
           if( info == 0_ilp .and. min( p, m-p ) < min( q, m-q ) ) then
              ...
              call stdlib_wuncsd( jobv1t, jobv2t, jobu1, jobu2, transt, signst, m,q, p, x11, &
              ldx11, x21, ldx21, x12, ldx12, x22,ldx22, theta, v1t, ldv1t, v2t, ldv2t, u1, ldu1,&
                        u2, ldu2, work, lwork, rwork, lrwork, iwork,info )
              return
           end if
           ...
           if( info == 0_ilp .and. m-q < q ) then
              ...
              call stdlib_wuncsd( jobu2, jobu1, jobv2t, jobv1t, trans, signst, m,m-p, m-q, x22, &
              ldx22, x21, ldx21, x12, ldx12, x11,ldx11, theta, u2, ldu2, u1, ldu1, v2t, ldv2t, &
                        v1t,ldv1t, work, lwork, rwork, lrwork, iwork, info )
              return
           end if
           ...
           if( info == 0_ilp ) then
              ...
              ib22e = ib22d + max( 1_ilp, q )
              ibbcsd = ib22e + max( 1_ilp, q - 1_ilp )
              call stdlib_wbbcsd( jobu1, jobu2, jobv1t, jobv2t, trans, m, p, q,theta, theta, u1, &
              ldu1, u2, ldu2, v1t, ldv1t,v2t, ldv2t, theta, theta, theta, theta, theta,theta, &
                        theta, theta, rwork, -1_ilp, childinfo )
              lbbcsdworkopt = int( rwork(1_ilp),KIND=ilp)
              lbbcsdworkmin = lbbcsdworkopt
              ...
              itauq2 = itauq1 + max( 1_ilp, q )
              iorgqr = itauq2 + max( 1_ilp, m - q )
              call stdlib_wungqr( m-q, m-q, m-q, u1, max(1_ilp,m-q), u1, work, -1_ilp,childinfo )
              lorgqrworkopt = int( work(1_ilp),KIND=ilp)
              lorgqrworkmin = max( 1_ilp, m - q )
              iorglq = itauq2 + max( 1_ilp, m - q )
              call stdlib_wunglq( m-q, m-q, m-q, u1, max(1_ilp,m-q), u1, work, -1_ilp,childinfo )
              lorglqworkopt = int( work(1_ilp),KIND=ilp)
              lorglqworkmin = max( 1_ilp, m - q )
              iorbdb = itauq2 + max( 1_ilp, m - q )
              call stdlib_wunbdb( trans, signs, m, p, q, x11, ldx11, x12, ldx12,x21, ldx21, x22, &
                        ldx22, theta, theta, u1, u2,v1t, v2t, work, -1_ilp, childinfo )
              lorbdbworkopt = int( work(1_ilp),KIND=ilp)
              lorbdbworkmin = lorbdbworkopt
              lworkopt = max( iorgqr + lorgqrworkopt, iorglq + lorglqworkopt,iorbdb + &
           ...
           if( colmajor ) then
              if( wantu1 .and. p > 0_ilp ) then
                 call stdlib_wlacpy( 'L', p, q, x11, ldx11, u1, ldu1 )
                 call stdlib_wungqr( p, p, q, u1, ldu1, work(itaup1), work(iorgqr),lorgqrwork, &
                           info)
              end if
              if( wantu2 .and. m-p > 0_ilp ) then
                 call stdlib_wlacpy( 'L', m-p, q, x21, ldx21, u2, ldu2 )
                 call stdlib_wungqr( m-p, m-p, q, u2, ldu2, work(itaup2),work(iorgqr), lorgqrwork,&
                            info )
              end if
              if( wantv1t .and. q > 0_ilp ) then
                 call stdlib_wlacpy( 'U', q-1, q-1, x11(1_ilp,2_ilp), ldx11, v1t(2_ilp,2_ilp),ldv1t )
                 v1t(1_ilp, 1_ilp) = cone
                 do j = 2, q
                    v1t(1_ilp,j) = czero
                    v1t(j,1_ilp) = czero
                 end do
                 call stdlib_wunglq( q-1, q-1, q-1, v1t(2_ilp,2_ilp), ldv1t, work(itauq1),work(iorglq), &
                           lorglqwork, info )
              end if
              if( wantv2t .and. m-q > 0_ilp ) then
                 call stdlib_wlacpy( 'U', p, m-q, x12, ldx12, v2t, ldv2t )
                 if( m-p > q) then
                    call stdlib_wlacpy( 'U', m-p-q, m-p-q, x22(q+1,p+1), ldx22,v2t(p+1,p+1), &
                              ldv2t )
                 end if
                 if( m > q ) then
                    call stdlib_wunglq( m-q, m-q, m-q, v2t, ldv2t, work(itauq2),work(iorglq), &
                              lorglqwork, info )
                 end if
              end if
           else
              if( wantu1 .and. p > 0_ilp ) then
                 call stdlib_wlacpy( 'U', q, p, x11, ldx11, u1, ldu1 )
                 call stdlib_wunglq( p, p, q, u1, ldu1, work(itaup1), work(iorglq),lorglqwork, &
                           info)
              end if
              if( wantu2 .and. m-p > 0_ilp ) then
                 call stdlib_wlacpy( 'U', q, m-p, x21, ldx21, u2, ldu2 )
                 call stdlib_wunglq( m-p, m-p, q, u2, ldu2, work(itaup2),work(iorglq), lorglqwork,&
                            info )
              end if
              if( wantv1t .and. q > 0_ilp ) then
                 call stdlib_wlacpy( 'L', q-1, q-1, x11(2_ilp,1_ilp), ldx11, v1t(2_ilp,2_ilp),ldv1t )
                 v1t(1_ilp, 1_ilp) = cone
                 do j = 2, q
                    v1t(1_ilp,j) = czero
                    v1t(j,1_ilp) = czero
                 end do
                 call stdlib_wungqr( q-1, q-1, q-1, v1t(2_ilp,2_ilp), ldv1t, work(itauq1),work(iorgqr), &
                           lorgqrwork, info )
              end if
              if( wantv2t .and. m-q > 0_ilp ) then
                 p1 = min( p+1, m )
           ...
           ! compute the csd of the matrix in bidiagonal-block form
           call stdlib_wbbcsd( jobu1, jobu2, jobv1t, jobv2t, trans, m, p, q, theta,rwork(iphi), &
           u1, ldu1, u2, ldu2, v1t, ldv1t, v2t,ldv2t, rwork(ib11d), rwork(ib11e), rwork(ib12d),&
           rwork(ib12e), rwork(ib21d), rwork(ib21e),rwork(ib22d), rwork(ib22e), rwork(ibbcsd),&
                     lbbcsdwork, info )
           ! permute rows and columns to place identity submatrices in top-
           ...
           if( q > 0_ilp .and. wantu2 ) then
              ...
              if( colmajor ) then
                 call stdlib_wlapmt( .false., m-p, m-p, u2, ldu2, iwork )
              else
                 call stdlib_wlapmr( .false., m-p, m-p, u2, ldu2, iwork )
              end if
           end if
           if( m > 0_ilp .and. wantv2t ) then
              do i = 1, p
              ...
              if( .not. colmajor ) then
                 call stdlib_wlapmt( .false., m-q, m-q, v2t, ldv2t, iwork )
              else
                 call stdlib_wlapmr( .false., m-q, m-q, v2t, ldv2t, iwork )
              end if
           end if
           return
           ! end stdlib_wuncsd
     ...
     !! K2-by-K2 identity matrix, where K1 = MAX(Q+P-M,0), K2 = MAX(Q-P,0).
        ...
        ! =====================================================================
           ...
           ! | tauq1 (max(1,q))                        |
           ! |-----------------------------------------|
           ! | stdlib_wunbdb work | stdlib_wungqr work | stdlib_wunglq work |
           ! |             |             |             |
           ! |             |             |             |
           ! |             |             |             |
           ! |             |             |             |
           ...
           if( info == 0_ilp ) then
              ...
              lorglqopt = 1_ilp
              if( r == q ) then
                 call stdlib_wunbdb1( m, p, q, x11, ldx11, x21, ldx21, theta, dum,cdum, cdum, &
                           cdum, work, -1_ilp, childinfo )
                 lorbdb = int( work(1_ilp),KIND=ilp)
                 if( wantu1 .and. p > 0_ilp ) then
                    call stdlib_wungqr( p, p, q, u1, ldu1, cdum, work(1_ilp), -1_ilp,childinfo )
                    lorgqrmin = max( lorgqrmin, p )
                    lorgqropt = max( lorgqropt, int( work(1_ilp),KIND=ilp) )
                 endif
                 if( wantu2 .and. m-p > 0_ilp ) then
                    call stdlib_wungqr( m-p, m-p, q, u2, ldu2, cdum, work(1_ilp), -1_ilp,childinfo )
                              
                    lorgqrmin = max( lorgqrmin, m-p )
                    lorgqropt = max( lorgqropt, int( work(1_ilp),KIND=ilp) )
                 end if
                 if( wantv1t .and. q > 0_ilp ) then
                    call stdlib_wunglq( q-1, q-1, q-1, v1t, ldv1t,cdum, work(1_ilp), -1_ilp, childinfo )
                              
                    lorglqmin = max( lorglqmin, q-1 )
                    lorglqopt = max( lorglqopt, int( work(1_ilp),KIND=ilp) )
                 end if
                 call stdlib_wbbcsd( jobu1, jobu2, jobv1t, 'N', 'N', m, p, q, theta,dum, u1, ldu1,&
                  u2, ldu2, v1t, ldv1t, cdum, 1_ilp,dum, dum, dum, dum, dum, dum, dum, dum,rwork(1_ilp), -&
                            1_ilp, childinfo )
                 lbbcsd = int( rwork(1_ilp),KIND=ilp)
              else if( r == p ) then
                 call stdlib_wunbdb2( m, p, q, x11, ldx11, x21, ldx21, theta, dum,cdum, cdum, &
                           cdum, work(1_ilp), -1_ilp, childinfo )
                 lorbdb = int( work(1_ilp),KIND=ilp)
                 if( wantu1 .and. p > 0_ilp ) then
                    call stdlib_wungqr( p-1, p-1, p-1, u1(2_ilp,2_ilp), ldu1, cdum, work(1_ilp),-1_ilp, childinfo &
                              )
                    lorgqrmin = max( lorgqrmin, p-1 )
                    lorgqropt = max( lorgqropt, int( work(1_ilp),KIND=ilp) )
                 end if
                 ...
                 if( wantv1t .and. q > 0_ilp ) then
                    call stdlib_wunglq( q, q, r, v1t, ldv1t, cdum, work(1_ilp), -1_ilp,childinfo )
                    lorglqmin = max( lorglqmin, q )
                    lorglqopt = max( lorglqopt, int( work(1_ilp),KIND=ilp) )
                 end if
                 call stdlib_wbbcsd( jobv1t, 'N', jobu1, jobu2, 'T', m, q, p, theta,dum, v1t, &
                 ldv1t, cdum, 1_ilp, u1, ldu1, u2, ldu2,dum, dum, dum, dum, dum, dum, dum, dum,rwork(&
                           1_ilp), -1_ilp, childinfo )
                 lbbcsd = int( rwork(1_ilp),KIND=ilp)
              else if( r == m-p ) then
                 call stdlib_wunbdb3( m, p, q, x11, ldx11, x21, ldx21, theta, dum,cdum, cdum, &
                           cdum, work(1_ilp), -1_ilp, childinfo )
                 lorbdb = int( work(1_ilp),KIND=ilp)
                 if( wantu1 .and. p > 0_ilp ) then
                    call stdlib_wungqr( p, p, q, u1, ldu1, cdum, work(1_ilp), -1_ilp,childinfo )
                 ...
                 if( wantu2 .and. m-p > 0_ilp ) then
                    call stdlib_wungqr( m-p-1, m-p-1, m-p-1, u2(2_ilp,2_ilp), ldu2, cdum,work(1_ilp), -1_ilp, &
                              childinfo )
                    lorgqrmin = max( lorgqrmin, m-p-1 )
                    lorgqropt = max( lorgqropt, int( work(1_ilp),KIND=ilp) )
                 end if
                 ...
                 call stdlib_wbbcsd( 'N', jobv1t, jobu2, jobu1, 'T', m, m-q, m-p,theta, dum, cdum,&
                  1_ilp, v1t, ldv1t, u2, ldu2, u1,ldu1, dum, dum, dum, dum, dum, dum, dum, dum,rwork(&
                            1_ilp), -1_ilp, childinfo )
                 lbbcsd = int( rwork(1_ilp),KIND=ilp)
              else
                 call stdlib_wunbdb4( m, p, q, x11, ldx11, x21, ldx21, theta, dum,cdum, cdum, &
                           cdum, cdum, work(1_ilp), -1_ilp, childinfo)
                 lorbdb = m + int( work(1_ilp),KIND=ilp)
                 if( wantu1 .and. p > 0_ilp ) then
                    call stdlib_wungqr( p, p, m-q, u1, ldu1, cdum, work(1_ilp), -1_ilp,childinfo )
                    lorgqrmin = max( lorgqrmin, p )
                    lorgqropt = max( lorgqropt, int( work(1_ilp),KIND=ilp) )
                 end if
                 if( wantu2 .and. m-p > 0_ilp ) then
                    call stdlib_wungqr( m-p, m-p, m-q, u2, ldu2, cdum, work(1_ilp), -1_ilp,childinfo )
                              
                    lorgqrmin = max( lorgqrmin, m-p )
                    lorgqropt = max( lorgqropt, int( work(1_ilp),KIND=ilp) )
                 end if
                 if( wantv1t .and. q > 0_ilp ) then
                    call stdlib_wunglq( q, q, q, v1t, ldv1t, cdum, work(1_ilp), -1_ilp,childinfo )
                    lorglqmin = max( lorglqmin, q )
                    lorglqopt = max( lorglqopt, int( work(1_ilp),KIND=ilp) )
                 end if
                 call stdlib_wbbcsd( jobu2, jobu1, 'N', jobv1t, 'N', m, m-p, m-q,theta, dum, u2, &
                 ldu2, u1, ldu1, cdum, 1_ilp, v1t,ldv1t, dum, dum, dum, dum, dum, dum, dum, dum,rwork(&
                           1_ilp), -1_ilp, childinfo )
                 lbbcsd = int( rwork(1_ilp),KIND=ilp)
              end if
           ...
           if( r == q ) then
              ! case 1: r = q
              ! simultaneously bidiagonalize x11 and x21
              call stdlib_wunbdb1( m, p, q, x11, ldx11, x21, ldx21, theta,rwork(iphi), work(&
                        itaup1), work(itaup2),work(itauq1), work(iorbdb), lorbdb, childinfo )
              ! accumulate householder reflectors
              if( wantu1 .and. p > 0_ilp ) then
                 call stdlib_wlacpy( 'L', p, q, x11, ldx11, u1, ldu1 )
                 call stdlib_wungqr( p, p, q, u1, ldu1, work(itaup1), work(iorgqr),lorgqr, &
                           childinfo )
              end if
              if( wantu2 .and. m-p > 0_ilp ) then
                 call stdlib_wlacpy( 'L', m-p, q, x21, ldx21, u2, ldu2 )
                 call stdlib_wungqr( m-p, m-p, q, u2, ldu2, work(itaup2),work(iorgqr), lorgqr, &
                           childinfo )
              end if
              if( wantv1t .and. q > 0_ilp ) then
                 v1t(1_ilp,1_ilp) = cone
                 ...
                 call stdlib_wlacpy( 'U', q-1, q-1, x21(1_ilp,2_ilp), ldx21, v1t(2_ilp,2_ilp),ldv1t )
                 call stdlib_wunglq( q-1, q-1, q-1, v1t(2_ilp,2_ilp), ldv1t, work(itauq1),work(iorglq), &
                           lorglq, childinfo )
              end if
              ! simultaneously diagonalize x11 and x21.
              call stdlib_wbbcsd( jobu1, jobu2, jobv1t, 'N', 'N', m, p, q, theta,rwork(iphi), u1, &
              ldu1, u2, ldu2, v1t, ldv1t, cdum,1_ilp, rwork(ib11d), rwork(ib11e), rwork(ib12d),rwork(&
              ib12e), rwork(ib21d), rwork(ib21e),rwork(ib22d), rwork(ib22e), rwork(ibbcsd),lrwork-&
                        ibbcsd+1, childinfo )
              ! permute rows and columns to place czero submatrices in
              ...
              ! case 2: r = p
              ! simultaneously bidiagonalize x11 and x21
              call stdlib_wunbdb2( m, p, q, x11, ldx11, x21, ldx21, theta,rwork(iphi), work(&
                        itaup1), work(itaup2),work(itauq1), work(iorbdb), lorbdb, childinfo )
              ! accumulate householder reflectors
              if( wantu1 .and. p > 0_ilp ) then
                 u1(1_ilp,1_ilp) = cone
                 ...
                 call stdlib_wlacpy( 'L', p-1, p-1, x11(2_ilp,1_ilp), ldx11, u1(2_ilp,2_ilp), ldu1 )
                 call stdlib_wungqr( p-1, p-1, p-1, u1(2_ilp,2_ilp), ldu1, work(itaup1),work(iorgqr), &
                           lorgqr, childinfo )
              end if
              if( wantu2 .and. m-p > 0_ilp ) then
                 call stdlib_wlacpy( 'L', m-p, q, x21, ldx21, u2, ldu2 )
              ...
              if( wantv1t .and. q > 0_ilp ) then
                 call stdlib_wlacpy( 'U', p, q, x11, ldx11, v1t, ldv1t )
                 call stdlib_wunglq( q, q, r, v1t, ldv1t, work(itauq1),work(iorglq), lorglq, &
                           childinfo )
              end if
              ! simultaneously diagonalize x11 and x21.
              call stdlib_wbbcsd( jobv1t, 'N', jobu1, jobu2, 'T', m, q, p, theta,rwork(iphi), v1t,&
               ldv1t, cdum, 1_ilp, u1, ldu1, u2,ldu2, rwork(ib11d), rwork(ib11e), rwork(ib12d),rwork(&
               ib12e), rwork(ib21d), rwork(ib21e),rwork(ib22d), rwork(ib22e), rwork(ibbcsd), &
                         lbbcsd,childinfo )
              ! permute rows and columns to place identity submatrices in
              ...
              ! case 3: r = m-p
              ! simultaneously bidiagonalize x11 and x21
              call stdlib_wunbdb3( m, p, q, x11, ldx11, x21, ldx21, theta,rwork(iphi), work(&
                        itaup1), work(itaup2),work(itauq1), work(iorbdb), lorbdb, childinfo )
              ! accumulate householder reflectors
              if( wantu1 .and. p > 0_ilp ) then
                 call stdlib_wlacpy( 'L', p, q, x11, ldx11, u1, ldu1 )
              ...
              if( wantu2 .and. m-p > 0_ilp ) then
                 ...
                 call stdlib_wlacpy( 'L', m-p-1, m-p-1, x21(2_ilp,1_ilp), ldx21, u2(2_ilp,2_ilp),ldu2 )
                 call stdlib_wungqr( m-p-1, m-p-1, m-p-1, u2(2_ilp,2_ilp), ldu2,work(itaup2), work(iorgqr)&
                           , lorgqr, childinfo )
              end if
              if( wantv1t .and. q > 0_ilp ) then
                 call stdlib_wlacpy( 'U', m-p, q, x21, ldx21, v1t, ldv1t )
                 call stdlib_wunglq( q, q, r, v1t, ldv1t, work(itauq1),work(iorglq), lorglq, &
                           childinfo )
              end if
              ! simultaneously diagonalize x11 and x21.
              call stdlib_wbbcsd( 'N', jobv1t, jobu2, jobu1, 'T', m, m-q, m-p,theta, rwork(iphi), &
              cdum, 1_ilp, v1t, ldv1t, u2, ldu2,u1, ldu1, rwork(ib11d), rwork(ib11e),rwork(ib12d), &
              rwork(ib12e), rwork(ib21d),rwork(ib21e), rwork(ib22d), rwork(ib22e),rwork(ibbcsd), &
                        lbbcsd, childinfo )
              ! permute rows and columns to place identity submatrices in
              ...
              if( q > r ) then
                 ...
                 if( wantu1 ) then
                    call stdlib_wlapmt( .false., p, q, u1, ldu1, iwork )
                 end if
                 if( wantv1t ) then
                    call stdlib_wlapmr( .false., q, q, v1t, ldv1t, iwork )
                 end if
              end if
           else
              ! case 4: r = m-q
              ! simultaneously bidiagonalize x11 and x21
              call stdlib_wunbdb4( m, p, q, x11, ldx11, x21, ldx21, theta,rwork(iphi), work(&
              itaup1), work(itaup2),work(itauq1), work(iorbdb), work(iorbdb+m),lorbdb-m, &
                        childinfo )
              ! accumulate householder reflectors
              if( wantu2 .and. m-p > 0_ilp ) then
                 call stdlib_wcopy( m-p, work(iorbdb+p), 1_ilp, u2, 1_ilp )
              end if
              if( wantu1 .and. p > 0_ilp ) then
                 call stdlib_wcopy( p, work(iorbdb), 1_ilp, u1, 1_ilp )
                 do j = 2, p
                    u1(1_ilp,j) = czero
                 end do
                 call stdlib_wlacpy( 'L', p-1, m-q-1, x11(2_ilp,1_ilp), ldx11, u1(2_ilp,2_ilp),ldu1 )
                 call stdlib_wungqr( p, p, m-q, u1, ldu1, work(itaup1),work(iorgqr), lorgqr, &
                           childinfo )
              end if
              if( wantu2 .and. m-p > 0_ilp ) then
                 do j = 2, m-p
                 ...
                 call stdlib_wlacpy( 'L', m-p-1, m-q-1, x21(2_ilp,1_ilp), ldx21, u2(2_ilp,2_ilp),ldu2 )
                 call stdlib_wungqr( m-p, m-p, m-q, u2, ldu2, work(itaup2),work(iorgqr), lorgqr, &
                           childinfo )
              end if
              if( wantv1t .and. q > 0_ilp ) then
                 call stdlib_wlacpy( 'U', m-q, q, x21, ldx21, v1t, ldv1t )
              ...
              ! simultaneously diagonalize x11 and x21.
              call stdlib_wbbcsd( jobu2, jobu1, 'N', jobv1t, 'N', m, m-p, m-q,theta, rwork(iphi), &
              u2, ldu2, u1, ldu1, cdum, 1_ilp,v1t, ldv1t, rwork(ib11d), rwork(ib11e),rwork(ib12d), &
              rwork(ib12e), rwork(ib21d),rwork(ib21e), rwork(ib22d), rwork(ib22e),rwork(ibbcsd), &
                        lbbcsd, childinfo )
              ! permute rows and columns to place identity submatrices in
              ...
              if( p > r ) then
                 ...
                 if( wantu1 ) then
                    call stdlib_wlapmt( .false., p, p, u1, ldu1, iwork )
                 end if
                 if( wantv1t ) then
                    call stdlib_wlapmr( .false., p, q, v1t, ldv1t, iwork )
                 end if
              end if
           end if
           return
     ...
     !! implicitly by angles THETA, PHI.
        ...
        ! ====================================================================
           ...
           if( colmajor ) then
              ...
              do i = 1, q
                 if( i == 1_ilp ) then
                    call stdlib_wscal( p-i+1, cmplx( z1, 0.0_qp,KIND=qp), x11(i,i), 1_ilp )
                 else
                 ...
                 if( i == 1_ilp ) then
                    call stdlib_wscal( m-p-i+1, cmplx( z2, 0.0_qp,KIND=qp), x21(i,i), 1_ilp )
                 else
                 ...
                 theta(i) = atan2( stdlib_qznrm2( m-p-i+1, x21(i,i), 1_ilp ),stdlib_qznrm2( p-i+1, &
                           x11(i,i), 1_ilp ) )
                 if( p > i ) then
                    call stdlib_wlarfgp( p-i+1, x11(i,i), x11(i+1,i), 1_ilp, taup1(i) )
                 else if ( p == i ) then
                    call stdlib_wlarfgp( p-i+1, x11(i,i), x11(i,i), 1_ilp, taup1(i) )
                 end if
                 x11(i,i) = cone
                 if ( m-p > i ) then
                    call stdlib_wlarfgp( m-p-i+1, x21(i,i), x21(i+1,i), 1_ilp,taup2(i) )
                 else if ( m-p == i ) then
                    call stdlib_wlarfgp( m-p-i+1, x21(i,i), x21(i,i), 1_ilp,taup2(i) )
                 end if
                 x21(i,i) = cone
                 if ( q > i ) then
                    call stdlib_wlarf( 'L', p-i+1, q-i, x11(i,i), 1_ilp,conjg(taup1(i)), x11(i,i+1), &
                              ldx11, work )
                    call stdlib_wlarf( 'L', m-p-i+1, q-i, x21(i,i), 1_ilp,conjg(taup2(i)), x21(i,i+1),&
                               ldx21, work )
                 end if
                 if ( m-q+1 > i ) then
                    call stdlib_wlarf( 'L', p-i+1, m-q-i+1, x11(i,i), 1_ilp,conjg(taup1(i)), x12(i,i),&
                               ldx12, work )
                    call stdlib_wlarf( 'L', m-p-i+1, m-q-i+1, x21(i,i), 1_ilp,conjg(taup2(i)), x22(i,&
                              i), ldx22, work )
                 end if
                 if( i < q ) then
                 ...
                 if( i < q )phi(i) = atan2( stdlib_qznrm2( q-i, x11(i,i+1), ldx11 ),stdlib_qznrm2(&
                            m-q-i+1, x12(i,i), ldx12 ) )
                 if( i < q ) then
                    call stdlib_wlacgv( q-i, x11(i,i+1), ldx11 )
                    if ( i == q-1 ) then
                       call stdlib_wlarfgp( q-i, x11(i,i+1), x11(i,i+1), ldx11,tauq1(i) )
                    else
                       call stdlib_wlarfgp( q-i, x11(i,i+1), x11(i,i+2), ldx11,tauq1(i) )
                    end if
                    x11(i,i+1) = cone
                 end if
                 if ( m-q+1 > i ) then
                 ...
                 x12(i,i) = cone
                 if( i < q ) then
                    call stdlib_wlarf( 'R', p-i, q-i, x11(i,i+1), ldx11, tauq1(i),x11(i+1,i+1), &
                              ldx11, work )
                    call stdlib_wlarf( 'R', m-p-i, q-i, x11(i,i+1), ldx11, tauq1(i),x21(i+1,i+1), &
                              ldx21, work )
                 end if
                 if ( p > i ) then
                    call stdlib_wlarf( 'R', p-i, m-q-i+1, x12(i,i), ldx12, tauq2(i),x12(i+1,i), &
                              ldx12, work )
                 end if
                 if ( m-p > i ) then
                    call stdlib_wlarf( 'R', m-p-i, m-q-i+1, x12(i,i), ldx12,tauq2(i), x22(i+1,i), &
                              ldx22, work )
                 end if
                 if( i < q )call stdlib_wlacgv( q-i, x11(i,i+1), ldx11 )
                 call stdlib_wlacgv( m-q-i+1, x12(i,i), ldx12 )
              end do
              ! reduce columns q + 1, ..., p of x12, x22
              do i = q + 1, p
                 ...
                 if( m-p-q >= 1_ilp )call stdlib_wlarf( 'R', m-p-q, m-q-i+1, x12(i,i), ldx12,tauq2(i),&
                            x22(q+1,i), ldx22, work )
                 call stdlib_wlacgv( m-q-i+1, x12(i,i), ldx12 )
              end do
              ! reduce columns p + 1, ..., m - q of x12, x22
              ...
              do i = 1, q
                 ...
                 call stdlib_wlarfgp( p-i+1, x11(i,i), x11(i,i+1), ldx11, taup1(i) )
                 x11(i,i) = cone
                 if ( i == m-p ) then
                    call stdlib_wlarfgp( m-p-i+1, x21(i,i), x21(i,i), ldx21,taup2(i) )
                 else
                    call stdlib_wlarfgp( m-p-i+1, x21(i,i), x21(i,i+1), ldx21,taup2(i) )
                 end if
                 x21(i,i) = cone
                 call stdlib_wlarf( 'R', q-i, p-i+1, x11(i,i), ldx11, taup1(i),x11(i+1,i), ldx11, &
                           work )
                 call stdlib_wlarf( 'R', m-q-i+1, p-i+1, x11(i,i), ldx11, taup1(i),x12(i,i), &
                           ldx12, work )
                 call stdlib_wlarf( 'R', q-i, m-p-i+1, x21(i,i), ldx21, taup2(i),x21(i+1,i), &
                           ldx21, work )
                 call stdlib_wlarf( 'R', m-q-i+1, m-p-i+1, x21(i,i), ldx21,taup2(i), x22(i,i), &
                           ldx22, work )
                 call stdlib_wlacgv( p-i+1, x11(i,i), ldx11 )
                 call stdlib_wlacgv( m-p-i+1, x21(i,i), ldx21 )
                 if( i < q ) then
                 ...
                 call stdlib_waxpy( m-q-i+1, cmplx( z2*z4*cos(theta(i)), 0.0_qp,KIND=qp),x22(i,i),&
                            1_ilp, x12(i,i), 1_ilp )
                 if( i < q )phi(i) = atan2( stdlib_qznrm2( q-i, x11(i+1,i), 1_ilp ),stdlib_qznrm2( m-&
                           q-i+1, x12(i,i), 1_ilp ) )
                 if( i < q ) then
                    call stdlib_wlarfgp( q-i, x11(i+1,i), x11(i+2,i), 1_ilp, tauq1(i) )
                    x11(i+1,i) = cone
                 end if
                 call stdlib_wlarfgp( m-q-i+1, x12(i,i), x12(i+1,i), 1_ilp, tauq2(i) )
                 x12(i,i) = cone
                 if( i < q ) then
                    call stdlib_wlarf( 'L', q-i, p-i, x11(i+1,i), 1_ilp,conjg(tauq1(i)), x11(i+1,i+1),&
                               ldx11, work )
                    call stdlib_wlarf( 'L', q-i, m-p-i, x11(i+1,i), 1_ilp,conjg(tauq1(i)), x21(i+1,i+&
                              1_ilp), ldx21, work )
                 end if
                 call stdlib_wlarf( 'L', m-q-i+1, p-i, x12(i,i), 1_ilp,conjg(tauq2(i)), x12(i,i+1), &
                           ldx12, work )
                 if ( m-p > i ) then
                    call stdlib_wlarf( 'L', m-q-i+1, m-p-i, x12(i,i), 1_ilp,conjg(tauq2(i)), x22(i,i+&
                              1_ilp), ldx22, work )
                 end if
              end do
              ! reduce columns q + 1, ..., p of x12, x22
              do i = q + 1, p
                 call stdlib_wscal( m-q-i+1, cmplx( -z1*z4, 0.0_qp,KIND=qp), x12(i,i), 1_ilp )
                 call stdlib_wlarfgp( m-q-i+1, x12(i,i), x12(i+1,i), 1_ilp, tauq2(i) )
                 x12(i,i) = cone
                 if ( p > i ) then
                    call stdlib_wlarf( 'L', m-q-i+1, p-i, x12(i,i), 1_ilp,conjg(tauq2(i)), x12(i,i+1),&
                 ...
                 if( m-p-q >= 1_ilp )call stdlib_wlarf( 'L', m-q-i+1, m-p-q, x12(i,i), 1_ilp,conjg(tauq2(&
                           i)), x22(i,q+1), ldx22, work )
              end do
              ! reduce columns p + 1, ..., m - q of x12, x22
              do i = 1, m - p - q
                 call stdlib_wscal( m-p-q-i+1, cmplx( z2*z4, 0.0_qp,KIND=qp),x22(p+i,q+i), 1_ilp )
                           
                 call stdlib_wlarfgp( m-p-q-i+1, x22(p+i,q+i), x22(p+i+1,q+i), 1_ilp,tauq2(p+i) )
                           
                 x22(p+i,q+i) = cone
                 if ( m-p-q /= i ) then
                    call stdlib_wlarf( 'L', m-p-q-i+1, m-p-q-i, x22(p+i,q+i), 1_ilp,conjg(tauq2(p+i)),&
                               x22(p+i,q+i+1), ldx22,work )
                 end if
              end do
           end if
     ...
     !! angles THETA, PHI.
        ...
        ! ====================================================================
           ...
           do i = 1, q
              ...
              x11(i,i) = cone
              x21(i,i) = cone
              call stdlib_wlarf( 'L', p-i+1, q-i, x11(i,i), 1_ilp, conjg(taup1(i)),x11(i,i+1), ldx11, &
                        work(ilarf) )
              call stdlib_wlarf( 'L', m-p-i+1, q-i, x21(i,i), 1_ilp, conjg(taup2(i)),x21(i,i+1), &
                        ldx21, work(ilarf) )
              if( i < q ) then
                 call stdlib_wdrot( q-i, x11(i,i+1), ldx11, x21(i,i+1), ldx21, c,s )
                 call stdlib_wlacgv( q-i, x21(i,i+1), ldx21 )
                 call stdlib_wlarfgp( q-i, x21(i,i+1), x21(i,i+2), ldx21, tauq1(i) )
                 s = real( x21(i,i+1),KIND=qp)
                 x21(i,i+1) = cone
                 call stdlib_wlarf( 'R', p-i, q-i, x21(i,i+1), ldx21, tauq1(i),x11(i+1,i+1), &
                           ldx11, work(ilarf) )
                 call stdlib_wlarf( 'R', m-p-i, q-i, x21(i,i+1), ldx21, tauq1(i),x21(i+1,i+1), &
                           ldx21, work(ilarf) )
                 call stdlib_wlacgv( q-i, x21(i,i+1), ldx21 )
                 c = sqrt( stdlib_qznrm2( p-i, x11(i+1,i+1), 1_ilp )**2_ilp+ stdlib_qznrm2( m-p-i, x21(i+&
                           1_ilp,i+1), 1_ilp )**2_ilp )
                 phi(i) = atan2( s, c )
                 call stdlib_wunbdb5( p-i, m-p-i, q-i-1, x11(i+1,i+1), 1_ilp,x21(i+1,i+1), 1_ilp, x11(i+1,&
                           i+2), ldx11,x21(i+1,i+2), ldx21, work(iorbdb5), lorbdb5,childinfo )
              end if
           end do
           return
     ...
     !! angles THETA, PHI.
        ...
        ! ====================================================================
           ...
           do i = 1, p
              if( i > 1_ilp ) then
                 call stdlib_wdrot( q-i+1, x11(i,i), ldx11, x21(i-1,i), ldx21, c,s )
              end if
              call stdlib_wlacgv( q-i+1, x11(i,i), ldx11 )
              call stdlib_wlarfgp( q-i+1, x11(i,i), x11(i,i+1), ldx11, tauq1(i) )
              c = real( x11(i,i),KIND=qp)
              x11(i,i) = cone
              call stdlib_wlarf( 'R', p-i, q-i+1, x11(i,i), ldx11, tauq1(i),x11(i+1,i), ldx11, &
                        work(ilarf) )
              call stdlib_wlarf( 'R', m-p-i+1, q-i+1, x11(i,i), ldx11, tauq1(i),x21(i,i), ldx21, &
                        work(ilarf) )
              call stdlib_wlacgv( q-i+1, x11(i,i), ldx11 )
              s = sqrt( stdlib_qznrm2( p-i, x11(i+1,i), 1_ilp )**2_ilp+ stdlib_qznrm2( m-p-i+1, x21(i,i), &
                        1_ilp )**2_ilp )
              theta(i) = atan2( s, c )
              call stdlib_wunbdb5( p-i, m-p-i+1, q-i, x11(i+1,i), 1_ilp, x21(i,i), 1_ilp,x11(i+1,i+1), &
                        ldx11, x21(i,i+1), ldx21,work(iorbdb5), lorbdb5, childinfo )
              call stdlib_wscal( p-i, cnegone, x11(i+1,i), 1_ilp )
              call stdlib_wlarfgp( m-p-i+1, x21(i,i), x21(i+1,i), 1_ilp, taup2(i) )
              if( i < p ) then
                 call stdlib_wlarfgp( p-i, x11(i+1,i), x11(i+2,i), 1_ilp, taup1(i) )
                 phi(i) = atan2( real( x11(i+1,i),KIND=qp), real( x21(i,i),KIND=qp) )
                 c = cos( phi(i) )
                 s = sin( phi(i) )
                 x11(i+1,i) = cone
                 call stdlib_wlarf( 'L', p-i, q-i, x11(i+1,i), 1_ilp, conjg(taup1(i)),x11(i+1,i+1), &
                           ldx11, work(ilarf) )
              end if
              x21(i,i) = cone
              call stdlib_wlarf( 'L', m-p-i+1, q-i, x21(i,i), 1_ilp, conjg(taup2(i)),x21(i,i+1), &
     ...
     !! implicitly by angles THETA, PHI.
        ...
        ! ====================================================================
           ...
           do i = 1, m-p
              if( i > 1_ilp ) then
                 call stdlib_wdrot( q-i+1, x11(i-1,i), ldx11, x21(i,i), ldx11, c,s )
              end if
              call stdlib_wlacgv( q-i+1, x21(i,i), ldx21 )
              call stdlib_wlarfgp( q-i+1, x21(i,i), x21(i,i+1), ldx21, tauq1(i) )
              s = real( x21(i,i),KIND=qp)
              x21(i,i) = cone
              call stdlib_wlarf( 'R', p-i+1, q-i+1, x21(i,i), ldx21, tauq1(i),x11(i,i), ldx11, &
                        work(ilarf) )
              call stdlib_wlarf( 'R', m-p-i, q-i+1, x21(i,i), ldx21, tauq1(i),x21(i+1,i), ldx21, &
                        work(ilarf) )
              call stdlib_wlacgv( q-i+1, x21(i,i), ldx21 )
              c = sqrt( stdlib_qznrm2( p-i+1, x11(i,i), 1_ilp )**2_ilp+ stdlib_qznrm2( m-p-i, x21(i+1,i), &
                        1_ilp )**2_ilp )
              theta(i) = atan2( s, c )
              call stdlib_wunbdb5( p-i+1, m-p-i, q-i, x11(i,i), 1_ilp, x21(i+1,i), 1_ilp,x11(i,i+1), &
                        ldx11, x21(i+1,i+1), ldx21,work(iorbdb5), lorbdb5, childinfo )
              call stdlib_wlarfgp( p-i+1, x11(i,i), x11(i+1,i), 1_ilp, taup1(i) )
              if( i < m-p ) then
                 call stdlib_wlarfgp( m-p-i, x21(i+1,i), x21(i+2,i), 1_ilp, taup2(i) )
                 phi(i) = atan2( real( x21(i+1,i),KIND=qp), real( x11(i,i),KIND=qp) )
                 c = cos( phi(i) )
                 s = sin( phi(i) )
                 x21(i+1,i) = cone
                 call stdlib_wlarf( 'L', m-p-i, q-i, x21(i+1,i), 1_ilp,conjg(taup2(i)), x21(i+1,i+1), &
                           ldx21,work(ilarf) )
              end if
              x11(i,i) = cone
              call stdlib_wlarf( 'L', p-i+1, q-i, x11(i,i), 1_ilp, conjg(taup1(i)),x11(i,i+1), ldx11, &
        ...
        ! ====================================================================
           ...
           do i = 1, m-q
              if( i == 1_ilp ) then
                 ...
                 call stdlib_wunbdb5( p, m-p, q, phantom(1_ilp), 1_ilp, phantom(p+1), 1_ilp,x11, ldx11, x21, &
                           ldx21, work(iorbdb5),lorbdb5, childinfo )
                 call stdlib_wscal( p, cnegone, phantom(1_ilp), 1_ilp )
                 call stdlib_wlarfgp( p, phantom(1_ilp), phantom(2_ilp), 1_ilp, taup1(1_ilp) )
                 call stdlib_wlarfgp( m-p, phantom(p+1), phantom(p+2), 1_ilp, taup2(1_ilp) )
                 theta(i) = atan2( real( phantom(1_ilp),KIND=qp), real( phantom(p+1),KIND=qp) )
                           
                 c = cos( theta(i) )
                 s = sin( theta(i) )
                 phantom(1_ilp) = cone
                 phantom(p+1) = cone
                 call stdlib_wlarf( 'L', p, q, phantom(1_ilp), 1_ilp, conjg(taup1(1_ilp)), x11,ldx11, work(&
                           ilarf) )
                 call stdlib_wlarf( 'L', m-p, q, phantom(p+1), 1_ilp, conjg(taup2(1_ilp)),x21, ldx21, &
                           work(ilarf) )
              else
                 call stdlib_wunbdb5( p-i+1, m-p-i+1, q-i+1, x11(i,i-1), 1_ilp,x21(i,i-1), 1_ilp, x11(i,i)&
                           , ldx11, x21(i,i),ldx21, work(iorbdb5), lorbdb5, childinfo )
                 call stdlib_wscal( p-i+1, cnegone, x11(i,i-1), 1_ilp )
                 call stdlib_wlarfgp( p-i+1, x11(i,i-1), x11(i+1,i-1), 1_ilp, taup1(i) )
                 call stdlib_wlarfgp( m-p-i+1, x21(i,i-1), x21(i+1,i-1), 1_ilp,taup2(i) )
                 theta(i) = atan2( real( x11(i,i-1),KIND=qp), real( x21(i,i-1),KIND=qp) )
                 c = cos( theta(i) )
                 s = sin( theta(i) )
                 x11(i,i-1) = cone
                 x21(i,i-1) = cone
                 call stdlib_wlarf( 'L', p-i+1, q-i+1, x11(i,i-1), 1_ilp,conjg(taup1(i)), x11(i,i), &
                           ldx11, work(ilarf) )
                 call stdlib_wlarf( 'L', m-p-i+1, q-i+1, x21(i,i-1), 1_ilp,conjg(taup2(i)), x21(i,i), &
                           ldx21, work(ilarf) )
              end if
              call stdlib_wdrot( q-i+1, x11(i,i), ldx11, x21(i,i), ldx21, s, -c )
              call stdlib_wlacgv( q-i+1, x21(i,i), ldx21 )
              call stdlib_wlarfgp( q-i+1, x21(i,i), x21(i,i+1), ldx21, tauq1(i) )
              c = real( x21(i,i),KIND=qp)
              x21(i,i) = cone
              call stdlib_wlarf( 'R', p-i, q-i+1, x21(i,i), ldx21, tauq1(i),x11(i+1,i), ldx11, &
                        work(ilarf) )
              call stdlib_wlarf( 'R', m-p-i, q-i+1, x21(i,i), ldx21, tauq1(i),x21(i+1,i), ldx21, &
                        work(ilarf) )
              call stdlib_wlacgv( q-i+1, x21(i,i), ldx21 )
              if( i < m-q ) then
                 s = sqrt( stdlib_qznrm2( p-i, x11(i+1,i), 1_ilp )**2_ilp+ stdlib_qznrm2( m-p-i, x21(i+1,&
                           i), 1_ilp )**2_ilp )
                 phi(i) = atan2( s, c )
              end if
           end do
           ...
           do i = m - q + 1, p
              ...
              call stdlib_wlarf( 'R', p-i, q-i+1, x11(i,i), ldx11, tauq1(i),x11(i+1,i), ldx11, &
                        work(ilarf) )
              call stdlib_wlarf( 'R', q-p, q-i+1, x11(i,i), ldx11, tauq1(i),x21(m-q+1,i), ldx21, &
                        work(ilarf) )
              call stdlib_wlacgv( q-i+1, x11(i,i), ldx11 )
           end do
           ! reduce the bottom-right portion of x21 to [ 0 i ]
     ...
     !! way.
        ...
        ! =====================================================================
           ...
           ! project x onto the orthogonal complement of q
           call stdlib_wunbdb6( m1, m2, n, x1, incx1, x2, incx2, q1, ldq1, q2, ldq2,work, lwork, &
                     childinfo )
           ! if the projection is nonzero, then return
           if( stdlib_qznrm2(m1,x1,incx1) /= czero.or. stdlib_qznrm2(m2,x2,incx2) /= czero ) &
                     then
              return
           end if
           ! project each standard basis vector e_1,...,e_m1 in turn, stopping
           ...
           do i = 1, m1
              ...
              call stdlib_wunbdb6( m1, m2, n, x1, incx1, x2, incx2, q1, ldq1, q2,ldq2, work, &
                        lwork, childinfo )
              if( stdlib_qznrm2(m1,x1,incx1) /= czero.or. stdlib_qznrm2(m2,x2,incx2) /= czero ) &
                        then
                 return
     ...
     !! criterion, then the zero vector is returned.
        ...
        ! =====================================================================
           ...
           scl2 = zero
           ssq2 = one
           call stdlib_wlassq( m2, x2, incx2, scl2, ssq2 )
           normsq1 = scl1**2_ilp*ssq1 + scl2**2_ilp*ssq2
           if( m1 == 0_ilp ) then
              do i = 1, n
                 work(i) = czero
              ...
              call stdlib_wgemv( 'C', m1, n, cone, q1, ldq1, x1, incx1, czero, work,1_ilp )
           end if
           call stdlib_wgemv( 'C', m2, n, cone, q2, ldq2, x2, incx2, cone, work, 1_ilp )
           call stdlib_wgemv( 'N', m1, n, cnegone, q1, ldq1, work, 1_ilp, cone, x1,incx1 )
           call stdlib_wgemv( 'N', m2, n, cnegone, q2, ldq2, work, 1_ilp, cone, x2,incx2 )
           scl1 = zero
           ssq1 = one
           call stdlib_wlassq( m1, x1, incx1, scl1, ssq1 )
           scl2 = zero
