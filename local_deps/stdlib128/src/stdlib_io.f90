...
contains
    ...
    subroutine  loadtxt_rqp(filename, d, skiprows, max_rows, fmt, delimiter)
      !! version: experimental
      !!
      !! Loads a 2D array from a text file.
      !!
      ...
      ! Default to format used for savetxt if fmt not specified.
      fmt_ = optval(fmt, "(*"//FMT_REAL_qp(1:len(FMT_REAL_qp)-1)//",:,1x))")
    ...
    subroutine  loadtxt_cqp(filename, d, skiprows, max_rows, fmt, delimiter)
      !! version: experimental
      !!
      !! Loads a 2D array from a text file.
      !!
      ...
      ! Default to format used for savetxt if fmt not specified.
      fmt_ = optval(fmt, "(*"//FMT_COMPLEX_qp(1:len(FMT_COMPLEX_qp)-1)//",:,1x))")
    ...
    subroutine savetxt_rqp(filename, d, delimiter)
      !! version: experimental
      !!
      !! Saves a 2D array into a text file.
      !!
      ...
      delimiter_ = optval(delimiter, delimiter_default)
      delim_str = "'"//delimiter_//"'"
        fmt_ = "(*"//FMT_REAL_qp(1:len(FMT_REAL_qp)-1)//",:,"//delim_str//"))"
    ...
    subroutine savetxt_cqp(filename, d, delimiter)
      !! version: experimental
      !!
      !! Saves a 2D array into a text file.
      !!
      ...
      delimiter_ = optval(delimiter, delimiter_default)
      delim_str = "'"//delimiter_//"'"
        fmt_ = "(*"//FMT_COMPLEX_qp(1:11)//delim_str//FMT_COMPLEX_qp(14:23)//",:,"//delim_str//"))"
