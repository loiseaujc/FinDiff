  ...
  contains
     ...
     !! If tau = 0, then H is taken to be the unit matrix.
        ...
        ! =====================================================================
           ...
           ! look for the last non-zero row in v.
              ...
              if( applyleft ) then
           ! scan for the last non-zero column in c(1:lastv,:).
                 lastc = stdlib_ilaqlc(lastv, n, c, ldc)
              else
           ! scan for the last non-zero row in c(:,1:lastv).
                 lastc = stdlib_ilaqlr(m, lastv, c, ldc)
              end if
           end if
           ...
           if( applyleft ) then
              ...
              if( lastv>0_ilp ) then
                 ! w(1:lastc,1) := c(1:lastv,1:lastc)**t * v(1:lastv,1)
                 call stdlib_qgemv( 'TRANSPOSE', lastv, lastc, one, c, ldc, v, incv,zero, work, 1_ilp &
                           )
                 ! c(1:lastv,1:lastc) := c(...) - v(1:lastv,1) * w(1:lastc,1)**t
                 call stdlib_qger( lastv, lastc, -tau, v, incv, work, 1_ilp, c, ldc )
              end if
           else
              ! form  c * h
              if( lastv>0_ilp ) then
                 ! w(1:lastc,1) := c(1:lastc,1:lastv) * v(1:lastv,1)
                 call stdlib_qgemv( 'NO TRANSPOSE', lastc, lastv, one, c, ldc,v, incv, zero, work,&
                            1_ilp )
                 ! c(1:lastc,1:lastv) := c(...) - w(1:lastc,1) * v(1:lastv,1)**t
                 call stdlib_qger( lastc, lastv, -tau, work, 1_ilp, v, incv, c, ldc )
              end if
           end if
           return
     ...
     !! tau.
        ...
        ! =====================================================================
           ...
           if( applyleft ) then
              ...
              if( lastv>0_ilp ) then
                 ! w(1:lastc,1) := c(1:lastv,1:lastc)**h * v(1:lastv,1)
                 call stdlib_wgemv( 'CONJUGATE TRANSPOSE', lastv, lastc, cone,c, ldc, v, incv, &
                           czero, work, 1_ilp )
                 ! c(1:lastv,1:lastc) := c(...) - v(1:lastv,1) * w(1:lastc,1)**h
                 call stdlib_wgerc( lastv, lastc, -tau, v, incv, work, 1_ilp, c, ldc )
              end if
           else
              ! form  c * h
              if( lastv>0_ilp ) then
                 ! w(1:lastc,1) := c(1:lastc,1:lastv) * v(1:lastv,1)
                 call stdlib_wgemv( 'NO TRANSPOSE', lastc, lastv, cone, c, ldc,v, incv, czero, &
                           work, 1_ilp )
                 ! c(1:lastc,1:lastv) := c(...) - w(1:lastc,1) * v(1:lastv,1)**h
                 call stdlib_wgerc( lastc, lastv, -tau, work, 1_ilp, v, incv, c, ldc )
              end if
           end if
           return
     ...
     !! This version uses inline code if H has order < 11.
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( side, 'L' ) ) then
              ...
              go to ( 10, 30, 50, 70, 90, 110, 130, 150,170, 190 )m
              ! code for general m
              call stdlib_qlarf( side, m, n, v, 1_ilp, tau, c, ldc, work )
              go to 410
              10 continue
              ! special code for 1 x 1 householder
              t1 = one - tau*v( 1_ilp )*v( 1_ilp )
     ...
     !! This version uses inline code if H has order < 11.
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( side, 'L' ) ) then
              ...
              go to ( 10, 30, 50, 70, 90, 110, 130, 150,170, 190 )m
              ! code for general m
              call stdlib_wlarf( side, m, n, v, 1_ilp, tau, c, ldc, work )
              go to 410
              10 continue
              ! special code for 1 x 1 householder
              t1 = cone - tau*v( 1_ilp )*conjg( v( 1_ilp ) )
     ...
     !! If  tau  is  zero, then  H  is taken to be the unit matrix.
        ...
        ! =====================================================================
           ...
           if( tau==zero )return
           ! form  w:= c * v
           call stdlib_qsymv( uplo, n, one, c, ldc, v, incv, zero, work, 1_ilp )
           alpha = -half*tau*stdlib_qdot( n, work, 1_ilp, v, incv )
           call stdlib_qaxpy( n, alpha, v, incv, work, 1_ilp )
           ! c := c - v * w' - w * v'
           call stdlib_qsyr2( uplo, n, -tau, v, incv, work, 1_ilp, c, ldc )
           return
     ...
     !! If  tau  is  zero, then  H  is taken to be the unit matrix.
        ...
        ! =====================================================================
           ...
           if( tau==czero )return
           ! form  w:= c * v
           call stdlib_whemv( uplo, n, cone, c, ldc, v, incv, czero, work, 1_ilp )
           alpha = -chalf*tau*stdlib_wdotc( n, work, 1_ilp, v, incv )
           call stdlib_waxpy( n, alpha, v, incv, work, 1_ilp )
           ! c := c - v * w' - w * v'
           call stdlib_wher2( uplo, n, -tau, v, incv, work, 1_ilp, c, ldc )
           return
           ...
           if( stdlib_lsame( storev, 'C' ) ) then
              if( stdlib_lsame( direct, 'F' ) ) then
                 ...
                 if( stdlib_lsame( side, 'L' ) ) then
                    ...
                    ! w := c1**t
                    do j = 1, k
                       call stdlib_qcopy( n, c( j, 1_ilp ), ldc, work( 1_ilp, j ), 1_ilp )
                    end do
                    ! w := w * v1
                    call stdlib_qtrmm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', 'UNIT', n,k, one, v, ldv,&
                               work, ldwork )
                    if( m>k ) then
                       ! w := w + c2**t * v2
                       call stdlib_qgemm( 'TRANSPOSE', 'NO TRANSPOSE', n, k, m-k,one, c( k+1, 1_ilp ),&
                                  ldc, v( k+1, 1_ilp ), ldv,one, work, ldwork )
                    end if
                    ! w := w * t**t  or  w * t
                    call stdlib_qtrmm( 'RIGHT', 'UPPER', transt, 'NON-UNIT', n, k,one, t, ldt, &
                              work, ldwork )
                    ! c := c - v * w**t
                    if( m>k ) then
                       ! c2 := c2 - v2 * w**t
                       call stdlib_qgemm( 'NO TRANSPOSE', 'TRANSPOSE', m-k, n, k,-one, v( k+1, 1_ilp )&
                                 , ldv, work, ldwork, one,c( k+1, 1_ilp ), ldc )
                    end if
                    ! w := w * v1**t
                    call stdlib_qtrmm( 'RIGHT', 'LOWER', 'TRANSPOSE', 'UNIT', n, k,one, v, ldv, &
                              work, ldwork )
                    ! c1 := c1 - w**t
                    do j = 1, k
                       do i = 1, n
                    ...
                    ! w := c1
                    do j = 1, k
                       call stdlib_qcopy( m, c( 1_ilp, j ), 1_ilp, work( 1_ilp, j ), 1_ilp )
                    end do
                    ! w := w * v1
                    call stdlib_qtrmm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', 'UNIT', m,k, one, v, ldv,&
                               work, ldwork )
                    if( n>k ) then
                       ! w := w + c2 * v2
                       call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', m, k, n-k,one, c( 1_ilp, k+&
                                 1_ilp ), ldc, v( k+1, 1_ilp ), ldv,one, work, ldwork )
                    end if
                    ! w := w * t  or  w * t**t
                    call stdlib_qtrmm( 'RIGHT', 'UPPER', trans, 'NON-UNIT', m, k,one, t, ldt, &
                              work, ldwork )
                    ! c := c - w * v**t
                    if( n>k ) then
                       ! c2 := c2 - w * v2**t
                       call stdlib_qgemm( 'NO TRANSPOSE', 'TRANSPOSE', m, n-k, k,-one, work, &
                                 ldwork, v( k+1, 1_ilp ), ldv, one,c( 1_ilp, k+1 ), ldc )
                    end if
                    ! w := w * v1**t
                    call stdlib_qtrmm( 'RIGHT', 'LOWER', 'TRANSPOSE', 'UNIT', m, k,one, v, ldv, &
                              work, ldwork )
                    ! c1 := c1 - w
                    do j = 1, k
                       do i = 1, m
                 ...
                 if( stdlib_lsame( side, 'L' ) ) then
                    ...
                    ! w := c2**t
                    do j = 1, k
                       call stdlib_qcopy( n, c( m-k+j, 1_ilp ), ldc, work( 1_ilp, j ), 1_ilp )
                    end do
                    ! w := w * v2
                    call stdlib_qtrmm( 'RIGHT', 'UPPER', 'NO TRANSPOSE', 'UNIT', n,k, one, v( m-k+&
                              1_ilp, 1_ilp ), ldv, work, ldwork )
                    if( m>k ) then
                       ! w := w + c1**t * v1
                       call stdlib_qgemm( 'TRANSPOSE', 'NO TRANSPOSE', n, k, m-k,one, c, ldc, v, &
                                 ldv, one, work, ldwork )
                    end if
                    ! w := w * t**t  or  w * t
                    call stdlib_qtrmm( 'RIGHT', 'LOWER', transt, 'NON-UNIT', n, k,one, t, ldt, &
                              work, ldwork )
                    ! c := c - v * w**t
                    if( m>k ) then
                       ! c1 := c1 - v1 * w**t
                       call stdlib_qgemm( 'NO TRANSPOSE', 'TRANSPOSE', m-k, n, k,-one, v, ldv, &
                                 work, ldwork, one, c, ldc )
                    end if
                    ! w := w * v2**t
                    call stdlib_qtrmm( 'RIGHT', 'UPPER', 'TRANSPOSE', 'UNIT', n, k,one, v( m-k+1, &
                              1_ilp ), ldv, work, ldwork )
                    ! c2 := c2 - w**t
                    do j = 1, k
                       do i = 1, n
                    ...
                    ! w := c2
                    do j = 1, k
                       call stdlib_qcopy( m, c( 1_ilp, n-k+j ), 1_ilp, work( 1_ilp, j ), 1_ilp )
                    end do
                    ! w := w * v2
                    call stdlib_qtrmm( 'RIGHT', 'UPPER', 'NO TRANSPOSE', 'UNIT', m,k, one, v( n-k+&
                              1_ilp, 1_ilp ), ldv, work, ldwork )
                    if( n>k ) then
                       ! w := w + c1 * v1
                       call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', m, k, n-k,one, c, ldc, &
                                 v, ldv, one, work, ldwork )
                    end if
                    ! w := w * t  or  w * t**t
                    call stdlib_qtrmm( 'RIGHT', 'LOWER', trans, 'NON-UNIT', m, k,one, t, ldt, &
                              work, ldwork )
                    ! c := c - w * v**t
                    if( n>k ) then
                       ! c1 := c1 - w * v1**t
                    ...
                    ! w := w * v2**t
                    call stdlib_qtrmm( 'RIGHT', 'UPPER', 'TRANSPOSE', 'UNIT', m, k,one, v( n-k+1, &
                              1_ilp ), ldv, work, ldwork )
                    ! c2 := c2 - w
                    do j = 1, k
                       do i = 1, m
              ...
              if( stdlib_lsame( direct, 'F' ) ) then
                 ...
                 if( stdlib_lsame( side, 'L' ) ) then
                    ...
                    ! w := w * v1**t
                    call stdlib_qtrmm( 'RIGHT', 'UPPER', 'TRANSPOSE', 'UNIT', n, k,one, v, ldv, &
                              work, ldwork )
                    if( m>k ) then
                       ! w := w + c2**t * v2**t
                       call stdlib_qgemm( 'TRANSPOSE', 'TRANSPOSE', n, k, m-k, one,c( k+1, 1_ilp ), &
                                 ldc, v( 1_ilp, k+1 ), ldv, one,work, ldwork )
                    end if
                    ! w := w * t**t  or  w * t
                    call stdlib_qtrmm( 'RIGHT', 'UPPER', transt, 'NON-UNIT', n, k,one, t, ldt, &
                    ...
                    if( m>k ) then
                       ! c2 := c2 - v2**t * w**t
                       call stdlib_qgemm( 'TRANSPOSE', 'TRANSPOSE', m-k, n, k, -one,v( 1_ilp, k+1 ), &
                                 ldv, work, ldwork, one,c( k+1, 1_ilp ), ldc )
                    end if
                    ! w := w * v1
                    call stdlib_qtrmm( 'RIGHT', 'UPPER', 'NO TRANSPOSE', 'UNIT', n,k, one, v, ldv,&
                               work, ldwork )
                    ! c1 := c1 - w**t
                    do j = 1, k
                       do i = 1, n
                    ...
                    ! w := w * v1**t
                    call stdlib_qtrmm( 'RIGHT', 'UPPER', 'TRANSPOSE', 'UNIT', m, k,one, v, ldv, &
                              work, ldwork )
                    if( n>k ) then
                       ! w := w + c2 * v2**t
                       call stdlib_qgemm( 'NO TRANSPOSE', 'TRANSPOSE', m, k, n-k,one, c( 1_ilp, k+1 ),&
                                  ldc, v( 1_ilp, k+1 ), ldv,one, work, ldwork )
                    end if
                    ! w := w * t  or  w * t**t
                    call stdlib_qtrmm( 'RIGHT', 'UPPER', trans, 'NON-UNIT', m, k,one, t, ldt, &
                    ...
                    if( n>k ) then
                       ! c2 := c2 - w * v2
                       call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', m, n-k, k,-one, work, &
                                 ldwork, v( 1_ilp, k+1 ), ldv, one,c( 1_ilp, k+1 ), ldc )
                    end if
                    ! w := w * v1
                    call stdlib_qtrmm( 'RIGHT', 'UPPER', 'NO TRANSPOSE', 'UNIT', m,k, one, v, ldv,&
                               work, ldwork )
                    ! c1 := c1 - w
                    do j = 1, k
                       do i = 1, m
                 ...
                 if( stdlib_lsame( side, 'L' ) ) then
                    ...
                    ! w := w * v2**t
                    call stdlib_qtrmm( 'RIGHT', 'LOWER', 'TRANSPOSE', 'UNIT', n, k,one, v( 1_ilp, m-k+&
                              1_ilp ), ldv, work, ldwork )
                    if( m>k ) then
                       ! w := w + c1**t * v1**t
                       call stdlib_qgemm( 'TRANSPOSE', 'TRANSPOSE', n, k, m-k, one,c, ldc, v, ldv,&
                                  one, work, ldwork )
                    end if
                    ! w := w * t**t  or  w * t
                    call stdlib_qtrmm( 'RIGHT', 'LOWER', transt, 'NON-UNIT', n, k,one, t, ldt, &
                    ...
                    if( m>k ) then
                       ! c1 := c1 - v1**t * w**t
                       call stdlib_qgemm( 'TRANSPOSE', 'TRANSPOSE', m-k, n, k, -one,v, ldv, work, &
                                 ldwork, one, c, ldc )
                    end if
                    ! w := w * v2
                    call stdlib_qtrmm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', 'UNIT', n,k, one, v( 1_ilp, &
                              m-k+1 ), ldv, work, ldwork )
                    ! c2 := c2 - w**t
                    do j = 1, k
                       do i = 1, n
                    ...
                    ! w := w * v2**t
                    call stdlib_qtrmm( 'RIGHT', 'LOWER', 'TRANSPOSE', 'UNIT', m, k,one, v( 1_ilp, n-k+&
                              1_ilp ), ldv, work, ldwork )
                    if( n>k ) then
                       ! w := w + c1 * v1**t
                       call stdlib_qgemm( 'NO TRANSPOSE', 'TRANSPOSE', m, k, n-k,one, c, ldc, v, &
                                 ldv, one, work, ldwork )
                    end if
                    ! w := w * t  or  w * t**t
                    call stdlib_qtrmm( 'RIGHT', 'LOWER', trans, 'NON-UNIT', m, k,one, t, ldt, &
                    ...
                    ! w := w * v2
                    call stdlib_qtrmm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', 'UNIT', m,k, one, v( 1_ilp, &
                              n-k+1 ), ldv, work, ldwork )
                    ! c1 := c1 - w
                    do j = 1, k
                       do i = 1, m
           ...
           if( stdlib_lsame( storev, 'C' ) ) then
              if( stdlib_lsame( direct, 'F' ) ) then
                 ...
                 if( stdlib_lsame( side, 'L' ) ) then
                    ...
                    ! w := c1**h
                    do j = 1, k
                       call stdlib_wcopy( n, c( j, 1_ilp ), ldc, work( 1_ilp, j ), 1_ilp )
                       call stdlib_wlacgv( n, work( 1_ilp, j ), 1_ilp )
                    end do
                    ! w := w * v1
                    call stdlib_wtrmm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', 'UNIT', n,k, cone, v, &
                              ldv, work, ldwork )
                    if( m>k ) then
                       ! w := w + c2**h * v2
                       call stdlib_wgemm( 'CONJUGATE TRANSPOSE', 'NO TRANSPOSE', n,k, m-k, cone, &
                                 c( k+1, 1_ilp ), ldc,v( k+1, 1_ilp ), ldv, cone, work, ldwork )
                    end if
                    ! w := w * t**h  or  w * t
                    call stdlib_wtrmm( 'RIGHT', 'UPPER', transt, 'NON-UNIT', n, k,cone, t, ldt, &
                              work, ldwork )
                    ! c := c - v * w**h
                    if( m>k ) then
                       ! c2 := c2 - v2 * w**h
                       call stdlib_wgemm( 'NO TRANSPOSE', 'CONJUGATE TRANSPOSE',m-k, n, k, -cone, &
                                 v( k+1, 1_ilp ), ldv, work,ldwork, cone, c( k+1, 1_ilp ), ldc )
                    end if
                    ! w := w * v1**h
                    call stdlib_wtrmm( 'RIGHT', 'LOWER', 'CONJUGATE TRANSPOSE','UNIT', n, k, cone,&
                               v, ldv, work, ldwork )
                    ! c1 := c1 - w**h
                    do j = 1, k
                       do i = 1, n
                    ...
                    ! w := c1
                    do j = 1, k
                       call stdlib_wcopy( m, c( 1_ilp, j ), 1_ilp, work( 1_ilp, j ), 1_ilp )
                    end do
                    ! w := w * v1
                    call stdlib_wtrmm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', 'UNIT', m,k, cone, v, &
                              ldv, work, ldwork )
                    if( n>k ) then
                       ! w := w + c2 * v2
                       call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', m, k, n-k,cone, c( 1_ilp, k+&
                                 1_ilp ), ldc, v( k+1, 1_ilp ), ldv,cone, work, ldwork )
                    end if
                    ! w := w * t  or  w * t**h
                    call stdlib_wtrmm( 'RIGHT', 'UPPER', trans, 'NON-UNIT', m, k,cone, t, ldt, &
                              work, ldwork )
                    ! c := c - w * v**h
                    if( n>k ) then
                       ! c2 := c2 - w * v2**h
                       call stdlib_wgemm( 'NO TRANSPOSE', 'CONJUGATE TRANSPOSE', m,n-k, k, -cone, &
                                 work, ldwork, v( k+1, 1_ilp ),ldv, cone, c( 1_ilp, k+1 ), ldc )
                    end if
                    ! w := w * v1**h
                    call stdlib_wtrmm( 'RIGHT', 'LOWER', 'CONJUGATE TRANSPOSE','UNIT', m, k, cone,&
                               v, ldv, work, ldwork )
                    ! c1 := c1 - w
                    do j = 1, k
                       do i = 1, m
                 ...
                 if( stdlib_lsame( side, 'L' ) ) then
                    ...
                    ! w := c2**h
                    do j = 1, k
                       call stdlib_wcopy( n, c( m-k+j, 1_ilp ), ldc, work( 1_ilp, j ), 1_ilp )
                       call stdlib_wlacgv( n, work( 1_ilp, j ), 1_ilp )
                    end do
                    ! w := w * v2
                    call stdlib_wtrmm( 'RIGHT', 'UPPER', 'NO TRANSPOSE', 'UNIT', n,k, cone, v( m-&
                              k+1, 1_ilp ), ldv, work, ldwork )
                    if( m>k ) then
                       ! w := w + c1**h * v1
                       call stdlib_wgemm( 'CONJUGATE TRANSPOSE', 'NO TRANSPOSE', n,k, m-k, cone, &
                    ...
                    ! w := w * t**h  or  w * t
                    call stdlib_wtrmm( 'RIGHT', 'LOWER', transt, 'NON-UNIT', n, k,cone, t, ldt, &
                              work, ldwork )
                    ! c := c - v * w**h
                    if( m>k ) then
                       ! c1 := c1 - v1 * w**h
                    ...
                    ! w := w * v2**h
                    call stdlib_wtrmm( 'RIGHT', 'UPPER', 'CONJUGATE TRANSPOSE','UNIT', n, k, cone,&
                               v( m-k+1, 1_ilp ), ldv, work,ldwork )
                    ! c2 := c2 - w**h
                    do j = 1, k
                       do i = 1, n
                    ...
                    ! w := c2
                    do j = 1, k
                       call stdlib_wcopy( m, c( 1_ilp, n-k+j ), 1_ilp, work( 1_ilp, j ), 1_ilp )
                    end do
                    ! w := w * v2
                    call stdlib_wtrmm( 'RIGHT', 'UPPER', 'NO TRANSPOSE', 'UNIT', m,k, cone, v( n-&
                              k+1, 1_ilp ), ldv, work, ldwork )
                    if( n>k ) then
                       ! w := w + c1 * v1
                       call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', m, k, n-k,cone, c, ldc, &
                                 v, ldv, cone, work, ldwork )
                    end if
                    ! w := w * t  or  w * t**h
                    call stdlib_wtrmm( 'RIGHT', 'LOWER', trans, 'NON-UNIT', m, k,cone, t, ldt, &
                              work, ldwork )
                    ! c := c - w * v**h
                    if( n>k ) then
                       ! c1 := c1 - w * v1**h
                    ...
                    ! w := w * v2**h
                    call stdlib_wtrmm( 'RIGHT', 'UPPER', 'CONJUGATE TRANSPOSE','UNIT', m, k, cone,&
                               v( n-k+1, 1_ilp ), ldv, work,ldwork )
                    ! c2 := c2 - w
                    do j = 1, k
                       do i = 1, m
              ...
              if( stdlib_lsame( direct, 'F' ) ) then
                 ...
                 if( stdlib_lsame( side, 'L' ) ) then
                    ...
                    if( m>k ) then
                       ! w := w + c2**h * v2**h
                       call stdlib_wgemm( 'CONJUGATE TRANSPOSE','CONJUGATE TRANSPOSE', n, k, m-k, &
                                 cone,c( k+1, 1_ilp ), ldc, v( 1_ilp, k+1 ), ldv, cone,work, ldwork )
                    end if
                    ! w := w * t**h  or  w * t
                    call stdlib_wtrmm( 'RIGHT', 'UPPER', transt, 'NON-UNIT', n, k,cone, t, ldt, &
                    ...
                    if( m>k ) then
                       ! c2 := c2 - v2**h * w**h
                       call stdlib_wgemm( 'CONJUGATE TRANSPOSE','CONJUGATE TRANSPOSE', m-k, n, k, &
                                 -cone,v( 1_ilp, k+1 ), ldv, work, ldwork, cone,c( k+1, 1_ilp ), ldc )
                    end if
                    ! w := w * v1
                    call stdlib_wtrmm( 'RIGHT', 'UPPER', 'NO TRANSPOSE', 'UNIT', n,k, cone, v, &
                              ldv, work, ldwork )
                    ! c1 := c1 - w**h
                    do j = 1, k
                       do i = 1, n
                    ...
                    if( n>k ) then
                       ! c2 := c2 - w * v2
                       call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', m, n-k, k,-cone, work, &
                                 ldwork, v( 1_ilp, k+1 ), ldv, cone,c( 1_ilp, k+1 ), ldc )
                    end if
                    ! w := w * v1
                    call stdlib_wtrmm( 'RIGHT', 'UPPER', 'NO TRANSPOSE', 'UNIT', m,k, cone, v, &
                              ldv, work, ldwork )
                    ! c1 := c1 - w
                    do j = 1, k
                       do i = 1, m
                 ...
                 if( stdlib_lsame( side, 'L' ) ) then
                    ...
                    ! w := w * v2
                    call stdlib_wtrmm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', 'UNIT', n,k, cone, v( 1_ilp, &
                              m-k+1 ), ldv, work, ldwork )
                    ! c2 := c2 - w**h
                    do j = 1, k
                       do i = 1, n
                    ...
                    ! w := w * v2
                    call stdlib_wtrmm( 'RIGHT', 'LOWER', 'NO TRANSPOSE', 'UNIT', m,k, cone, v( 1_ilp, &
                              n-k+1 ), ldv, work, ldwork )
                    ! c1 := c1 - w
                    do j = 1, k
                       do i = 1, m
     ...
     !! Otherwise  1 <= tau <= 2.
        ...
        ! =====================================================================
           ...
           xnorm = stdlib_qnrm2( n-1, x, incx )
           if( xnorm==zero ) then
              ! h  =  i
              tau = zero
           else
              ! general case
              beta = -sign( stdlib_qlapy2( alpha, xnorm ), alpha )
              safmin = stdlib_qlamch( 'S' ) / stdlib_qlamch( 'E' )
              knt = 0_ilp
              if( abs( beta )<safmin ) then
                 ! xnorm, beta may be inaccurate; scale x and recompute them
                 rsafmn = one / safmin
              ...
              tau = ( beta-alpha ) / beta
              call stdlib_qscal( n-1, one / ( alpha-beta ), x, incx )
              ! if alpha is subnormal, it may lose relative accuracy
              do j = 1, knt
                 beta = beta*safmin
              end do
     ...
     !! Otherwise  1 <= real(tau) <= 2  and  abs(tau-1) <= 1 .
        ...
        ! =====================================================================
           ...
           xnorm = stdlib_qznrm2( n-1, x, incx )
           ...
           if( xnorm==zero .and. alphi==zero ) then
              ...
              ! general case
              beta = -sign( stdlib_qlapy3( alphr, alphi, xnorm ), alphr )
              safmin = stdlib_qlamch( 'S' ) / stdlib_qlamch( 'E' )
              rsafmn = one / safmin
              knt = 0_ilp
              if( abs( beta )<safmin ) then
              ...
              alpha = stdlib_wladiv( cmplx( one,KIND=qp), alpha-beta )
              call stdlib_wscal( n-1, alpha, x, incx )
              ! if alpha is subnormal, it may lose relative accuracy
              do j = 1, knt
                 beta = beta*safmin
              end do
     ...
     !! the unit matrix.
        ...
        ! =====================================================================
           ...
           if( xnorm==zero ) then
              ...
              ! general case
              beta = sign( stdlib_qlapy2( alpha, xnorm ), alpha )
              smlnum = stdlib_qlamch( 'S' ) / stdlib_qlamch( 'E' )
              knt = 0_ilp
              if( abs( beta )<smlnum ) then
                 ! xnorm, beta may be inaccurate; scale x and recompute them
                 bignum = one / smlnum
              ...
              if ( abs(tau)<=smlnum ) then
                 ...
                 ! this is the general case.
                 call stdlib_qscal( n-1, one / alpha, x, incx )
              end if
              ! if beta is subnormal, it may lose relative accuracy
              do j = 1, knt
                 beta = beta*smlnum
        ...
        ! =====================================================================
           ...
           if( xnorm==zero ) then
              ...
              if( alphi==zero ) then
                 ...
                 ! only "reflecting" the diagonal entry to be real and non-negative.
                 xnorm = stdlib_qlapy2( alphr, alphi )
              ...
              alpha = stdlib_wladiv( cmplx( one,KIND=qp), alpha )
              if ( abs(tau)<=smlnum ) then
                 ! in the case where the computed tau ends up being a denormalized number,
                 ! it loses relative accuracy. this is a big problem. solution: flush tau
                 ! to zero (or two or whatever makes a nonnegative real number for beta).
              ...
              do i = 1, k
                 ...
                 if( tau( i )==zero ) then
                    ...
                    if( stdlib_lsame( storev, 'C' ) ) then
                       ...
                       j = min( lastv, prevlastv )
                       ! t(1:i-1,i) := - tau(i) * v(i:j,1:i-1)**t * v(i:j,i)
                       call stdlib_qgemv( 'TRANSPOSE', j-i, i-1, -tau( i ),v( i+1, 1_ilp ), ldv, v( i+&
                                 1_ilp, i ), 1_ilp, one,t( 1_ilp, i ), 1_ilp )
                    else
                       ! skip any trailing zeros.
                       do lastv = n, i+1, -1
                       ...
                       j = min( lastv, prevlastv )
                       ! t(1:i-1,i) := - tau(i) * v(1:i-1,i:j) * v(i,i:j)**t
                       call stdlib_qgemv( 'NO TRANSPOSE', i-1, j-i, -tau( i ),v( 1_ilp, i+1 ), ldv, v(&
                                  i, i+1 ), ldv, one,t( 1_ilp, i ), 1_ilp )
                    end if
                    ! t(1:i-1,i) := t(1:i-1,1:i-1) * t(1:i-1,i)
                    call stdlib_qtrmv( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', i-1, t,ldt, t( 1_ilp, i ),&
                               1_ilp )
                    t( i, i ) = tau( i )
                    if( i>1_ilp ) then
                       prevlastv = max( prevlastv, lastv )
                 ...
                 if( tau( i )==zero ) then
                    ...
                    if( i<k ) then
                       if( stdlib_lsame( storev, 'C' ) ) then
                          ...
                          j = max( lastv, prevlastv )
                          ! t(i+1:k,i) = -tau(i) * v(j:n-k+i,i+1:k)**t * v(j:n-k+i,i)
                          call stdlib_qgemv( 'TRANSPOSE', n-k+i-j, k-i, -tau( i ),v( j, i+1 ), &
                                    ldv, v( j, i ), 1_ilp, one,t( i+1, i ), 1_ilp )
                       else
                          ! skip any leading zeros.
                          do lastv = 1, i-1
                          ...
                          j = max( lastv, prevlastv )
                          ! t(i+1:k,i) = -tau(i) * v(i+1:k,j:n-k+i) * v(i,j:n-k+i)**t
                          call stdlib_qgemv( 'NO TRANSPOSE', k-i, n-k+i-j,-tau( i ), v( i+1, j ), &
                                    ldv, v( i, j ), ldv,one, t( i+1, i ), 1_ilp )
                       end if
                       ! t(i+1:k,i) := t(i+1:k,i+1:k) * t(i+1:k,i)
                       call stdlib_qtrmv( 'LOWER', 'NO TRANSPOSE', 'NON-UNIT', k-i,t( i+1, i+1 ), &
                                 ldt, t( i+1, i ), 1_ilp )
                       if( i>1_ilp ) then
                          prevlastv = min( prevlastv, lastv )
                       else
              ...
              do i = 1, k
                 ...
                 if( tau( i )==czero ) then
                    ...
                    if( stdlib_lsame( storev, 'C' ) ) then
                       ...
                       j = min( lastv, prevlastv )
                       ! t(1:i-1,i) := - tau(i) * v(i:j,1:i-1)**h * v(i:j,i)
                       call stdlib_wgemv( 'CONJUGATE TRANSPOSE', j-i, i-1,-tau( i ), v( i+1, 1_ilp ), &
                                 ldv,v( i+1, i ), 1_ilp, cone, t( 1_ilp, i ), 1_ilp )
                    else
                       ! skip any trailing zeros.
                       do lastv = n, i+1, -1
                       ...
                       j = min( lastv, prevlastv )
                       ! t(1:i-1,i) := - tau(i) * v(1:i-1,i:j) * v(i,i:j)**h
                       call stdlib_wgemm( 'N', 'C', i-1, 1_ilp, j-i, -tau( i ),v( 1_ilp, i+1 ), ldv, v( i,&
                                  i+1 ), ldv,cone, t( 1_ilp, i ), ldt )
                    end if
                    ! t(1:i-1,i) := t(1:i-1,1:i-1) * t(1:i-1,i)
                    call stdlib_wtrmv( 'UPPER', 'NO TRANSPOSE', 'NON-UNIT', i-1, t,ldt, t( 1_ilp, i ),&
                               1_ilp )
                    t( i, i ) = tau( i )
                    if( i>1_ilp ) then
                       prevlastv = max( prevlastv, lastv )
                 ...
                 if( tau( i )==czero ) then
                    ...
                    if( i<k ) then
                       if( stdlib_lsame( storev, 'C' ) ) then
                          ...
                          j = max( lastv, prevlastv )
                          ! t(i+1:k,i) = -tau(i) * v(j:n-k+i,i+1:k)**h * v(j:n-k+i,i)
                          call stdlib_wgemv( 'CONJUGATE TRANSPOSE', n-k+i-j, k-i,-tau( i ), v( j, &
                                    i+1 ), ldv, v( j, i ),1_ilp, cone, t( i+1, i ), 1_ilp )
                       else
                          ! skip any leading zeros.
                          do lastv = 1, i-1
                          ...
                          j = max( lastv, prevlastv )
                          ! t(i+1:k,i) = -tau(i) * v(i+1:k,j:n-k+i) * v(i,j:n-k+i)**h
                          call stdlib_wgemm( 'N', 'C', k-i, 1_ilp, n-k+i-j, -tau( i ),v( i+1, j ), &
                                    ldv, v( i, j ), ldv,cone, t( i+1, i ), ldt )
                       end if
                       ! t(i+1:k,i) := t(i+1:k,i+1:k) * t(i+1:k,i)
                       call stdlib_wtrmv( 'LOWER', 'NO TRANSPOSE', 'NON-UNIT', k-i,t( i+1, i+1 ), &
                                 ldt, t( i+1, i ), 1_ilp )
                       if( i>1_ilp ) then
                          prevlastv = min( prevlastv, lastv )
                       else
