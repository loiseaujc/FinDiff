submodule (stdlib_specialmatrices) tridiagonal_matrices
    ...
    pure module function initialize_tridiagonal_pure_qp(dl, dv, du) result(A)
        !! Construct a `tridiagonal` matrix from the rank-1 arrays
        !! `dl`, `dv` and `du`.
        real(qp), intent(in) :: dl(:), dv(:), du(:)
        !! tridiagonal matrix elements.
        type(tridiagonal_qp_type) :: A
        !! Corresponding tridiagonal matrix.
    ...
    pure module function initialize_constant_tridiagonal_pure_qp(dl, dv, du, n) result(A)
        !! Construct a `tridiagonal` matrix with constant elements.
        real(qp), intent(in) :: dl, dv, du
        !! tridiagonal matrix elements.
        integer(ilp), intent(in) :: n
    ...
    module function initialize_tridiagonal_impure_qp(dl, dv, du, err) result(A)
        !! Construct a `tridiagonal` matrix from the rank-1 arrays
        !! `dl`, `dv` and `du`.
        real(qp), intent(in) :: dl(:), dv(:), du(:)
        !! tridiagonal matrix elements.
    ...
    module function initialize_constant_tridiagonal_impure_qp(dl, dv, du, n, err) result(A)
        !! Construct a `tridiagonal` matrix with constant elements.
        real(qp), intent(in) :: dl, dv, du
        !! tridiagonal matrix elements.
        integer(ilp), intent(in) :: n
    ...
    pure module function initialize_tridiagonal_pure_cqp(dl, dv, du) result(A)
        !! Construct a `tridiagonal` matrix from the rank-1 arrays
        !! `dl`, `dv` and `du`.
        complex(qp), intent(in) :: dl(:), dv(:), du(:)
        !! tridiagonal matrix elements.
        type(tridiagonal_cqp_type) :: A
        !! Corresponding tridiagonal matrix.
    ...
    pure module function initialize_constant_tridiagonal_pure_cqp(dl, dv, du, n) result(A)
        !! Construct a `tridiagonal` matrix with constant elements.
        complex(qp), intent(in) :: dl, dv, du
        !! tridiagonal matrix elements.
        integer(ilp), intent(in) :: n
    ...
    module function initialize_tridiagonal_impure_cqp(dl, dv, du, err) result(A)
        !! Construct a `tridiagonal` matrix from the rank-1 arrays
        !! `dl`, `dv` and `du`.
        complex(qp), intent(in) :: dl(:), dv(:), du(:)
        !! tridiagonal matrix elements.
    ...
    module function initialize_constant_tridiagonal_impure_cqp(dl, dv, du, n, err) result(A)
        !! Construct a `tridiagonal` matrix with constant elements.
        complex(qp), intent(in) :: dl, dv, du
        !! tridiagonal matrix elements.
        integer(ilp), intent(in) :: n
    ...
    module subroutine spmv_tridiag_1d_qp(A, x, y, alpha, beta, op)
        type(tridiagonal_qp_type), intent(in) :: A
        real(qp), intent(in), contiguous, target :: x(:)
        real(qp), intent(inout), contiguous, target :: y(:)
        real(qp), intent(in), optional :: alpha
        real(qp), intent(in), optional :: beta
        character(1), intent(in), optional :: op

        ! Internal variables.
        real(qp) :: alpha_, beta_
        integer(ilp) :: n, nrhs, ldx, ldy
        character(1) :: op_
        real(qp), pointer :: xmat(:, :), ymat(:, :)

        ! Deal with optional arguments.
        alpha_ = 1.0_qp ; if (present(alpha)) alpha_ = alpha
        beta_  = 0.0_qp ; if (present(beta))  beta_  = beta
        op_    = "N"        ; if (present(op))    op_    = op

        ! Prepare Lapack arguments.
        n = A%n ; ldx = n ; ldy = n ; y = 0.0_qp
        nrhs =  1 
    ...
    module subroutine spmv_tridiag_2d_qp(A, x, y, alpha, beta, op)
        type(tridiagonal_qp_type), intent(in) :: A
        real(qp), intent(in), contiguous, target :: x(:,:)
        real(qp), intent(inout), contiguous, target :: y(:,:)
        real(qp), intent(in), optional :: alpha
    ...
    module subroutine spmv_tridiag_1d_cqp(A, x, y, alpha, beta, op)
        type(tridiagonal_cqp_type), intent(in) :: A
        complex(qp), intent(in), contiguous, target :: x(:)
        complex(qp), intent(inout), contiguous, target :: y(:)
        real(qp), intent(in), optional :: alpha
        real(qp), intent(in), optional :: beta
        character(1), intent(in), optional :: op
    ...
    module subroutine spmv_tridiag_2d_cqp(A, x, y, alpha, beta, op)
        type(tridiagonal_cqp_type), intent(in) :: A
        complex(qp), intent(in), contiguous, target :: x(:,:)
        complex(qp), intent(inout), contiguous, target :: y(:,:)
        real(qp), intent(in), optional :: alpha
    ...
    pure module function tridiagonal_to_dense_qp(A) result(B)
        !! Convert a `tridiagonal` matrix to its dense representation.
        type(tridiagonal_qp_type), intent(in) :: A
        !! Input tridiagonal matrix.
        real(qp), allocatable :: B(:, :)
        !! Corresponding dense matrix.
    ...
    pure module function tridiagonal_to_dense_cqp(A) result(B)
        !! Convert a `tridiagonal` matrix to its dense representation.
        type(tridiagonal_cqp_type), intent(in) :: A
        !! Input tridiagonal matrix.
        complex(qp), allocatable :: B(:, :)
        !! Corresponding dense matrix.
        ...
        associate (n => A%n)
        allocate(B(n, n), source=zero_cqp)
        B(1, 1) = A%dv(1) ; B(1, 2) = A%du(1)
        do concurrent (i=2:n-1)
            B(i, i-1) = A%dl(i-1)
            B(i, i) = A%dv(i)
    ...
    pure module function transpose_tridiagonal_qp(A) result(B)
        type(tridiagonal_qp_type), intent(in) :: A
        !! Input matrix.
        type(tridiagonal_qp_type) :: B
        B = tridiagonal(A%du, A%dv, A%dl)
    end function
    ...
    pure module function transpose_tridiagonal_cqp(A) result(B)
        type(tridiagonal_cqp_type), intent(in) :: A
        !! Input matrix.
        type(tridiagonal_cqp_type) :: B
        B = tridiagonal(A%du, A%dv, A%dl)
    end function
    ...
    pure module function hermitian_tridiagonal_qp(A) result(B)
        type(tridiagonal_qp_type), intent(in) :: A
        !! Input matrix.
        type(tridiagonal_qp_type) :: B
        B = tridiagonal(A%du, A%dv, A%dl)
    ...
    pure module function hermitian_tridiagonal_cqp(A) result(B)
        type(tridiagonal_cqp_type), intent(in) :: A
        !! Input matrix.
        type(tridiagonal_cqp_type) :: B
        B = tridiagonal(conjg(A%du), conjg(A%dv), conjg(A%dl))
    ...
    pure module function scalar_multiplication_tridiagonal_qp(alpha, A) result(B)
    ...
    pure module function scalar_multiplication_bis_tridiagonal_qp(A, alpha) result(B)
        type(tridiagonal_qp_type), intent(in) :: A
        real(qp), intent(in) :: alpha
        type(tridiagonal_qp_type) :: B
        B = tridiagonal(A%dl, A%dv, A%du)
    ...
    pure module function scalar_multiplication_tridiagonal_cqp(alpha, A) result(B)
    ...
    pure module function scalar_multiplication_bis_tridiagonal_cqp(A, alpha) result(B)
        type(tridiagonal_cqp_type), intent(in) :: A
        complex(qp), intent(in) :: alpha
        type(tridiagonal_cqp_type) :: B
        B = tridiagonal(A%dl, A%dv, A%du)
    ...
    pure module function matrix_add_tridiagonal_qp(A, B) result(C)
        type(tridiagonal_qp_type), intent(in) :: A
        type(tridiagonal_qp_type), intent(in) :: B
        type(tridiagonal_qp_type) :: C
        C = tridiagonal(A%dl, A%dv, A%du)
        C%dl = C%dl + B%dl; C%dv = C%dv + B%dv; C%du = C%du + B%du
    end function

    pure module function matrix_sub_tridiagonal_qp(A, B) result(C)
        type(tridiagonal_qp_type), intent(in) :: A
        type(tridiagonal_qp_type), intent(in) :: B
        type(tridiagonal_qp_type) :: C
        C = tridiagonal(A%dl, A%dv, A%du)
    ...
    pure module function matrix_add_tridiagonal_cqp(A, B) result(C)
        type(tridiagonal_cqp_type), intent(in) :: A
        type(tridiagonal_cqp_type), intent(in) :: B
        type(tridiagonal_cqp_type) :: C
        C = tridiagonal(A%dl, A%dv, A%du)
        C%dl = C%dl + B%dl; C%dv = C%dv + B%dv; C%du = C%du + B%du
    end function

    pure module function matrix_sub_tridiagonal_cqp(A, B) result(C)
        type(tridiagonal_cqp_type), intent(in) :: A
        type(tridiagonal_cqp_type), intent(in) :: B
        type(tridiagonal_cqp_type) :: C
        C = tridiagonal(A%dl, A%dv, A%du)
