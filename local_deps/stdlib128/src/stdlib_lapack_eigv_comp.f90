  ...
  contains
     ...
     !! generalized eigenvalue problem A*x = lambda*B*x.
        ...
        ! =====================================================================
           ...
           rscale( m ) = j
           if( j==m )go to 180
           call stdlib_qswap( l, a( 1_ilp, j ), 1_ilp, a( 1_ilp, m ), 1_ilp )
           call stdlib_qswap( l, b( 1_ilp, j ), 1_ilp, b( 1_ilp, m ), 1_ilp )
           180 continue
           go to ( 20, 90 )iflow
           190 continue
           ilo = k
           ...
           coef = one / real( 2_ilp*nr,KIND=qp)
           coef2 = coef*coef
           coef5 = half*coef2
           nrp2 = nr + 2_ilp
           beta = zero
           ...
           ! start generalized conjugate gradient iteration
           250 continue
           gamma = stdlib_qdot( nr, work( ilo+4*n ), 1_ilp, work( ilo+4*n ), 1_ilp ) +stdlib_qdot( nr, &
                     work( ilo+5*n ), 1_ilp, work( ilo+5*n ), 1_ilp )
           ew = zero
           ewc = zero
           do i = ilo, ihi
           ...
           t = coef5*( ewc-three*ew )
           tc = coef5*( ew-three*ewc )
           call stdlib_qscal( nr, beta, work( ilo ), 1_ilp )
           call stdlib_qscal( nr, beta, work( ilo+n ), 1_ilp )
           call stdlib_qaxpy( nr, coef, work( ilo+4*n ), 1_ilp, work( ilo+n ), 1_ilp )
           call stdlib_qaxpy( nr, coef, work( ilo+5*n ), 1_ilp, work( ilo ), 1_ilp )
           do i = ilo, ihi
              work( i ) = work( i ) + tc
              work( i+n ) = work( i+n ) + t
           end do
           ...
           sum = stdlib_qdot( nr, work( ilo+n ), 1_ilp, work( ilo+2*n ), 1_ilp ) +stdlib_qdot( nr, work( &
                     ilo ), 1_ilp, work( ilo+3*n ), 1_ilp )
           alpha = gamma / sum
           ! determine correction to current iteration
           cmax = zero
           ...
           if( cmax<half )go to 350
           call stdlib_qaxpy( nr, -alpha, work( ilo+2*n ), 1_ilp, work( ilo+4*n ), 1_ilp )
           call stdlib_qaxpy( nr, -alpha, work( ilo+3*n ), 1_ilp, work( ilo+5*n ), 1_ilp )
           pgamma = gamma
           it = it + 1_ilp
           if( it<=nrp2 )go to 250
           ! end generalized conjugate gradient iteration
           ...
           lsfmax = int( log10( sfmax ) / basl,KIND=ilp)
           do i = ilo, ihi
              irab = stdlib_iqamax( n-ilo+1, a( i, ilo ), lda )
              rab = abs( a( i, irab+ilo-1 ) )
              ...
              ir = min( max( ir, lsfmin ), lsfmax, lsfmax-lrab )
              lscale( i ) = sclfac**ir
              icab = stdlib_iqamax( ihi, a( 1_ilp, i ), 1_ilp )
              cab = abs( a( icab, i ) )
              icab = stdlib_iqamax( ihi, b( 1_ilp, i ), 1_ilp )
              cab = max( cab, abs( b( icab, i ) ) )
              lcab = int( log10( cab+sfmin ) / basl+one,KIND=ilp)
              jc = int(rscale( i ) + sign( half, rscale( i ) ),KIND=ilp)
              jc = min( max( jc, lsfmin ), lsfmax, lsfmax-lcab )
           ...
           ! row scaling of matrices a and b
           do i = ilo, ihi
              call stdlib_qscal( n-ilo+1, lscale( i ), a( i, ilo ), lda )
              call stdlib_qscal( n-ilo+1, lscale( i ), b( i, ilo ), ldb )
           end do
           ! column scaling of matrices a and b
           do j = ilo, ihi
              call stdlib_qscal( ihi, rscale( j ), a( 1_ilp, j ), 1_ilp )
              call stdlib_qscal( ihi, rscale( j ), b( 1_ilp, j ), 1_ilp )
           end do
           return
     ...
     !! generalized eigenvalue problem A*x = lambda*B*x.
        ...
        ! =====================================================================
           ...
           rscale( m ) = j
           if( j==m )go to 180
           call stdlib_wswap( l, a( 1_ilp, j ), 1_ilp, a( 1_ilp, m ), 1_ilp )
           call stdlib_wswap( l, b( 1_ilp, j ), 1_ilp, b( 1_ilp, m ), 1_ilp )
           180 continue
           go to ( 20, 90 )iflow
           190 continue
           ilo = k
           ...
           lsfmax = int( log10( sfmax ) / basl,KIND=ilp)
           do i = ilo, ihi
              irab = stdlib_iwamax( n-ilo+1, a( i, ilo ), lda )
              rab = abs( a( i, irab+ilo-1 ) )
              ...
              ir = min( max( ir, lsfmin ), lsfmax, lsfmax-lrab )
              lscale( i ) = sclfac**ir
              icab = stdlib_iwamax( ihi, a( 1_ilp, i ), 1_ilp )
              cab = abs( a( icab, i ) )
              icab = stdlib_iwamax( ihi, b( 1_ilp, i ), 1_ilp )
              cab = max( cab, abs( b( icab, i ) ) )
              lcab = int( log10( cab+sfmin ) / basl+one,KIND=ilp)
              jc = int(rscale( i ) + sign( half, rscale( i ) ),KIND=ilp)
              jc = min( max( jc, lsfmin ), lsfmax, lsfmax-lcab )
           ...
           ! row scaling of matrices a and b
           do i = ilo, ihi
              call stdlib_wdscal( n-ilo+1, lscale( i ), a( i, ilo ), lda )
              call stdlib_wdscal( n-ilo+1, lscale( i ), b( i, ilo ), ldb )
           end do
           ! column scaling of matrices a and b
           do j = ilo, ihi
              call stdlib_wdscal( ihi, rscale( j ), a( 1_ilp, j ), 1_ilp )
              call stdlib_wdscal( ihi, rscale( j ), b( 1_ilp, j ), 1_ilp )
           end do
           return
     ...
     !! problem to generalized Hessenberg form.
        ...
        ! =====================================================================
           ...
           ! initialize q and z if desired.
           if( icompq==3_ilp )call stdlib_qlaset( 'FULL', n, n, zero, one, q, ldq )
           if( icompz==3_ilp )call stdlib_qlaset( 'FULL', n, n, zero, one, z, ldz )
           ! quick return if possible
           if( n<=1 )return
           ! zero out lower triangle of b
           do jcol = 1, n - 1
           ...
           do jcol = ilo, ihi - 2
              do jrow = ihi, jcol + 2, -1
                 ...
                 call stdlib_qrot( n+2-jrow, b( jrow-1, jrow-1 ), ldb,b( jrow, jrow-1 ), ldb, c, &
                           s )
                 if( ilq )call stdlib_qrot( n, q( 1_ilp, jrow-1 ), 1_ilp, q( 1_ilp, jrow ), 1_ilp, c, s )
                 ! step 2: rotate columns jrow, jrow-1 to kill b(jrow,jrow-1)
                 temp = b( jrow, jrow )
                 call stdlib_qlartg( temp, b( jrow, jrow-1 ), c, s,b( jrow, jrow ) )
                 b( jrow, jrow-1 ) = zero
                 call stdlib_qrot( ihi, a( 1_ilp, jrow ), 1_ilp, a( 1_ilp, jrow-1 ), 1_ilp, c, s )
                 call stdlib_qrot( jrow-1, b( 1_ilp, jrow ), 1_ilp, b( 1_ilp, jrow-1 ), 1_ilp, c,s )
                 if( ilz )call stdlib_qrot( n, z( 1_ilp, jrow ), 1_ilp, z( 1_ilp, jrow-1 ), 1_ilp, c, s )
              end do
           end do
           return
     ...
     !! problem to generalized Hessenberg form.
        ...
        ! =====================================================================
           ...
           ! initialize q and z if desired.
           if( icompq==3_ilp )call stdlib_wlaset( 'FULL', n, n, czero, cone, q, ldq )
           if( icompz==3_ilp )call stdlib_wlaset( 'FULL', n, n, czero, cone, z, ldz )
           ! quick return if possible
           if( n<=1 )return
           ! zero out lower triangle of b
           do jcol = 1, n - 1
           ...
           do jcol = ilo, ihi - 2
              do jrow = ihi, jcol + 2, -1
                 ...
                 call stdlib_wrot( n+2-jrow, b( jrow-1, jrow-1 ), ldb,b( jrow, jrow-1 ), ldb, c, &
                           s )
                 if( ilq )call stdlib_wrot( n, q( 1_ilp, jrow-1 ), 1_ilp, q( 1_ilp, jrow ), 1_ilp, c,conjg( s ) )
                           
                 ! step 2: rotate columns jrow, jrow-1 to kill b(jrow,jrow-1)
                 ctemp = b( jrow, jrow )
                 call stdlib_wlartg( ctemp, b( jrow, jrow-1 ), c, s,b( jrow, jrow ) )
                 b( jrow, jrow-1 ) = czero
                 call stdlib_wrot( ihi, a( 1_ilp, jrow ), 1_ilp, a( 1_ilp, jrow-1 ), 1_ilp, c, s )
                 call stdlib_wrot( jrow-1, b( 1_ilp, jrow ), 1_ilp, b( 1_ilp, jrow-1 ), 1_ilp, c,s )
                 if( ilz )call stdlib_wrot( n, z( 1_ilp, jrow ), 1_ilp, z( 1_ilp, jrow-1 ), 1_ilp, c, s )
              end do
           end do
           return
     ...
     !! multiplications for parts of the computation to enhance performance.
       ...
       ! =====================================================================
           ...
           ! initialize q and z if desired.
           if( initq )call stdlib_qlaset( 'ALL', n, n, zero, one, q, ldq )
           if( initz )call stdlib_qlaset( 'ALL', n, n, zero, one, z, ldz )
           ! zero out lower triangle of b.
           if( n>1_ilp )call stdlib_qlaset( 'LOWER', n-1, n-1, zero, zero, b(2_ilp, 1_ilp), ldb )
           ! quick return if possible
           nh = ihi - ilo + 1_ilp
           if( nh<=1_ilp ) then
              work( 1_ilp ) = one
           ...
           if( nb<nbmin .or. nb>=nh ) then
              ...
              do jcol = ilo, ihi-2, nb
                 ...
                 n2nb = ( ihi-jcol-1 ) / nnb - 1_ilp
                 nblst = ihi - jcol - n2nb*nnb
                 call stdlib_qlaset( 'ALL', nblst, nblst, zero, one, work, nblst )
                 pw = nblst * nblst + 1_ilp
                 do i = 1, n2nb
                    call stdlib_qlaset( 'ALL', 2_ilp*nnb, 2_ilp*nnb, zero, one,work( pw ), 2_ilp*nnb )
                    pw = pw + 4_ilp*nnb*nnb
                 end do
                 ! reduce columns jcol:jcol+nnb-1 of a to hessenberg form.
                 do j = jcol, jcol+nnb-1
                    ...
                    do i = ihi, j+2, -1
                       temp = a( i-1, j )
                       call stdlib_qlartg( temp, a( i, j ), c, s, a( i-1, j ) )
                       a( i, j ) = c
                       b( i, j ) = s
                    end do
                    ! accumulate givens rotations into workspace array.
                    ...
                    do jj = n, j+1, -1
                       ...
                       if( jj<ihi ) then
                          ...
                          b( jj+1, jj ) = zero
                          call stdlib_qrot( jj-top, b( top+1, jj+1 ), 1_ilp,b( top+1, jj ), 1_ilp, c, s )
                    ...
                    ! update a by transformations from right.
                    ! explicit loop unrolling provides better performance
                    ! compared to stdlib_qlasr.
                     ! call stdlib_qlasr( 'right', 'variable', 'backward', ihi-top,
           ! $                     ihi-j, a( j+2, j ), b( j+2, j ),
           ! $                     a( top+1, j+1 ), lda )
                    jj = mod( ihi-j-1, 3_ilp )
                    do i = ihi-j-3, jj+1, -3
                    ...
                    if( jj>0_ilp ) then
                       do i = jj, 1, -1
                          call stdlib_qrot( ihi-top, a( top+1, j+i+1 ), 1_ilp,a( top+1, j+i ), 1_ilp, a( &
                                    j+1+i, j ),-b( j+1+i, j ) )
                       end do
                    end if
                    ! update (j+1)th column of a by transformations from left.
                    if ( j < jcol + nnb - 1_ilp ) then
                       ...
                       ! triangular.
                       jrow = ihi - nblst + 1_ilp
                       call stdlib_qgemv( 'TRANSPOSE', nblst, len, one, work,nblst, a( jrow, j+1 )&
                                 , 1_ilp, zero,work( pw ), 1_ilp )
                       ppw = pw + len
                       do i = jrow, jrow+nblst-len-1
                          work( ppw ) = a( i, j+1 )
                       ...
                       call stdlib_qtrmv( 'LOWER', 'TRANSPOSE', 'NON-UNIT',nblst-len, work( &
                                 len*nblst + 1_ilp ), nblst,work( pw+len ), 1_ilp )
                       ...
                       do jrow = j0, jcol+1, -nnb
                          ...
                          call stdlib_qtrmv( 'UPPER', 'TRANSPOSE', 'NON-UNIT', len,work( ppwo + &
                                    nnb ), 2_ilp*nnb, work( pw ),1_ilp )
                          call stdlib_qtrmv( 'LOWER', 'TRANSPOSE', 'NON-UNIT', nnb,work( ppwo + &
                                    2_ilp*len*nnb ),2_ilp*nnb, work( pw + len ), 1_ilp )
                          call stdlib_qgemv( 'TRANSPOSE', nnb, len, one,work( ppwo ), 2_ilp*nnb, a( &
                                    jrow, j+1 ), 1_ilp,one, work( pw ), 1_ilp )
                          call stdlib_qgemv( 'TRANSPOSE', len, nnb, one,work( ppwo + 2_ilp*len*nnb + &
                                    nnb ), 2_ilp*nnb,a( jrow+nnb, j+1 ), 1_ilp, one,work( pw+len ), 1_ilp )
                          ppw = pw
                          do i = jrow, jrow+len+nnb-1
                             a( i, j+1 ) = work( ppw )
                 ...
                 do j = j0, jcol+1, -nnb
                    if ( blk22 ) then
                       ...
                       ! where all blocks are nnb-by-nnb, u21 is upper
                       ! triangular and u12 is lower triangular.
                       call stdlib_qorm22( 'LEFT', 'TRANSPOSE', 2_ilp*nnb, cola, nnb,nnb, work( ppwo )&
                                 , 2_ilp*nnb,a( j, jcol+nnb ), lda, work( pw ),lwork-pw+1, ierr )
                    else
                       ! ignore the structure of u.
                       call stdlib_qgemm( 'TRANSPOSE', 'NO TRANSPOSE', 2_ilp*nnb,cola, 2_ilp*nnb, one, &
                                 work( ppwo ), 2_ilp*nnb,a( j, jcol+nnb ), lda, zero, work( pw ),2_ilp*nnb )
                       call stdlib_qlacpy( 'ALL', 2_ilp*nnb, cola, work( pw ), 2_ilp*nnb,a( j, jcol+nnb ),&
                                  lda )
                    end if
                    ppwo = ppwo + 4_ilp*nnb*nnb
                 end do
                 ...
                 if( wantq ) then
                    ...
                    do j = j0, jcol+1, -nnb
                       ...
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_qorm22( 'RIGHT', 'NO TRANSPOSE', nh, 2_ilp*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp*nnb,q( topq, j ), ldq, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,2_ilp*nnb, 2_ilp*nnb, one,&
                                     q( topq, j ), ldq,work( ppwo ), 2_ilp*nnb, zero, work( pw ),nh )
                          call stdlib_qlacpy( 'ALL', nh, 2_ilp*nnb, work( pw ), nh,q( topq, j ), ldq )
                 ...
                 if ( wantz .or. top>0_ilp ) then
                    ...
                    call stdlib_qlaset( 'LOWER', ihi - jcol - 1_ilp, nnb, zero, zero,a( jcol + 2_ilp, &
                              jcol ), lda )
                    call stdlib_qlaset( 'LOWER', ihi - jcol - 1_ilp, nnb, zero, zero,b( jcol + 2_ilp, &
                              jcol ), ldb )
                 end if
                 ! apply accumulated orthogonal matrices to a and b.
                 if ( top>0_ilp ) then
                    ...
                    call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,nblst, nblst, one, a( &
                              1_ilp, j ), lda,work, nblst, zero, work( pw ), top )
                    call stdlib_qlacpy( 'ALL', top, nblst, work( pw ), top,a( 1_ilp, j ), lda )
                    ...
                    do j = j0, jcol+1, -nnb
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_qorm22( 'RIGHT', 'NO TRANSPOSE', top, 2_ilp*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp*nnb,a( 1_ilp, j ), lda, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,2_ilp*nnb, 2_ilp*nnb, &
                                    one, a( 1_ilp, j ), lda,work( ppwo ), 2_ilp*nnb, zero,work( pw ), top )
                          call stdlib_qlacpy( 'ALL', top, 2_ilp*nnb, work( pw ), top,a( 1_ilp, j ), lda )
                    ...
                    call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,nblst, nblst, one, b( &
                              1_ilp, j ), ldb,work, nblst, zero, work( pw ), top )
                    call stdlib_qlacpy( 'ALL', top, nblst, work( pw ), top,b( 1_ilp, j ), ldb )
                    ...
                    do j = j0, jcol+1, -nnb
                       if ( blk22 ) then
                          ...
                          call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,2_ilp*nnb, 2_ilp*nnb, &
                                    one, b( 1_ilp, j ), ldb,work( ppwo ), 2_ilp*nnb, zero,work( pw ), top )
                          call stdlib_qlacpy( 'ALL', top, 2_ilp*nnb, work( pw ), top,b( 1_ilp, j ), ldb )
                 ...
                 if( wantz ) then
                    ...
                    do j = j0, jcol+1, -nnb
                       ...
                       if ( blk22 ) then
                          ...
                          call stdlib_qgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,2_ilp*nnb, 2_ilp*nnb, one,&
                                     z( topq, j ), ldz,work( ppwo ), 2_ilp*nnb, zero, work( pw ),nh )
                          call stdlib_qlacpy( 'ALL', nh, 2_ilp*nnb, work( pw ), nh,z( topq, j ), ldz )
           ...
           if ( jcol<ihi )call stdlib_qgghrd( compq2, compz2, n, jcol, ihi, a, lda, b, ldb, q,ldq,&
                      z, ldz, ierr )
           work( 1_ilp ) = real( lwkopt,KIND=qp)
           return
     ...
     !! multiplications for parts of the computation to enhance performance.
        ...
        ! =====================================================================
           ...
           ! initialize q and z if desired.
           if( initq )call stdlib_wlaset( 'ALL', n, n, czero, cone, q, ldq )
           if( initz )call stdlib_wlaset( 'ALL', n, n, czero, cone, z, ldz )
           ! zero out lower triangle of b.
           if( n>1_ilp )call stdlib_wlaset( 'LOWER', n-1, n-1, czero, czero, b(2_ilp, 1_ilp), ldb )
           ! quick return if possible
           nh = ihi - ilo + 1_ilp
           if( nh<=1_ilp ) then
              work( 1_ilp ) = cone
           ...
           if( nb<nbmin .or. nb>=nh ) then
              ...
              do jcol = ilo, ihi-2, nb
                 ...
                 n2nb = ( ihi-jcol-1 ) / nnb - 1_ilp
                 nblst = ihi - jcol - n2nb*nnb
                 call stdlib_wlaset( 'ALL', nblst, nblst, czero, cone, work, nblst )
                 pw = nblst * nblst + 1_ilp
                 do i = 1, n2nb
                    call stdlib_wlaset( 'ALL', 2_ilp*nnb, 2_ilp*nnb, czero, cone,work( pw ), 2_ilp*nnb )
                 ...
                 do j = jcol, jcol+nnb-1
                    ...
                    do i = ihi, j+2, -1
                       temp = a( i-1, j )
                       call stdlib_wlartg( temp, a( i, j ), c, s, a( i-1, j ) )
                    ...
                    do jj = n, j+1, -1
                       ...
                       if( jj<ihi ) then
                          ...
                          b( jj+1, jj ) = czero
                          call stdlib_wrot( jj-top, b( top+1, jj+1 ), 1_ilp,b( top+1, jj ), 1_ilp, c, s )
                    ...
                    if( jj>0_ilp ) then
                       do i = jj, 1, -1
                          ...
                          call stdlib_wrot( ihi-top, a( top+1, j+i+1 ), 1_ilp,a( top+1, j+i ), 1_ilp, c,-&
                                    conjg( b( j+1+i, j ) ) )
                       end do
                    end if
                    ! update (j+1)th column of a by transformations from left.
                    if ( j < jcol + nnb - 1_ilp ) then
                       ...
                       ! triangular.
                       jrow = ihi - nblst + 1_ilp
                       call stdlib_wgemv( 'CONJUGATE', nblst, len, cone, work,nblst, a( jrow, j+1 &
                                 ), 1_ilp, czero,work( pw ), 1_ilp )
                       ppw = pw + len
                       do i = jrow, jrow+nblst-len-1
                          work( ppw ) = a( i, j+1 )
                       ...
                       call stdlib_wtrmv( 'LOWER', 'CONJUGATE', 'NON-UNIT',nblst-len, work( &
                                 len*nblst + 1_ilp ), nblst,work( pw+len ), 1_ilp )
                       ...
                       do jrow = j0, jcol+1, -nnb
                          ...
                          call stdlib_wtrmv( 'UPPER', 'CONJUGATE', 'NON-UNIT', len,work( ppwo + &
                                    nnb ), 2_ilp*nnb, work( pw ),1_ilp )
                          call stdlib_wtrmv( 'LOWER', 'CONJUGATE', 'NON-UNIT', nnb,work( ppwo + &
                                    2_ilp*len*nnb ),2_ilp*nnb, work( pw + len ), 1_ilp )
                          call stdlib_wgemv( 'CONJUGATE', nnb, len, cone,work( ppwo ), 2_ilp*nnb, a( &
                                    jrow, j+1 ), 1_ilp,cone, work( pw ), 1_ilp )
                          call stdlib_wgemv( 'CONJUGATE', len, nnb, cone,work( ppwo + 2_ilp*len*nnb + &
                                    nnb ), 2_ilp*nnb,a( jrow+nnb, j+1 ), 1_ilp, cone,work( pw+len ), 1_ilp )
                          ppw = pw
                          do i = jrow, jrow+len+nnb-1
                             a( i, j+1 ) = work( ppw )
                 ...
                 do j = j0, jcol+1, -nnb
                    if ( blk22 ) then
                       ...
                       ! where all blocks are nnb-by-nnb, u21 is upper
                       ! triangular and u12 is lower triangular.
                       call stdlib_wunm22( 'LEFT', 'CONJUGATE', 2_ilp*nnb, cola, nnb,nnb, work( ppwo )&
                                 , 2_ilp*nnb,a( j, jcol+nnb ), lda, work( pw ),lwork-pw+1, ierr )
                    else
                       ! ignore the structure of u.
                       call stdlib_wgemm( 'CONJUGATE', 'NO TRANSPOSE', 2_ilp*nnb,cola, 2_ilp*nnb, cone, &
                       work( ppwo ), 2_ilp*nnb,a( j, jcol+nnb ), lda, czero, work( pw ),2_ilp*nnb )
                                 
                       call stdlib_wlacpy( 'ALL', 2_ilp*nnb, cola, work( pw ), 2_ilp*nnb,a( j, jcol+nnb ),&
                                  lda )
                    end if
                    ppwo = ppwo + 4_ilp*nnb*nnb
                 end do
                 ...
                 if( wantq ) then
                    ...
                    do j = j0, jcol+1, -nnb
                       ...
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_wunm22( 'RIGHT', 'NO TRANSPOSE', nh, 2_ilp*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp*nnb,q( topq, j ), ldq, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', nh,2_ilp*nnb, 2_ilp*nnb, &
                          cone, q( topq, j ), ldq,work( ppwo ), 2_ilp*nnb, czero, work( pw ),nh )
                                    
                          call stdlib_wlacpy( 'ALL', nh, 2_ilp*nnb, work( pw ), nh,q( topq, j ), ldq )
                 ...
                 if ( wantz .or. top>0_ilp ) then
                    ...
                    call stdlib_wlaset( 'LOWER', ihi - jcol - 1_ilp, nnb, czero, czero,a( jcol + 2_ilp, &
                              jcol ), lda )
                    call stdlib_wlaset( 'LOWER', ihi - jcol - 1_ilp, nnb, czero, czero,b( jcol + 2_ilp, &
                              jcol ), ldb )
                 end if
                 ! apply accumulated unitary matrices to a and b.
                 if ( top>0_ilp ) then
                    ...
                    call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,nblst, nblst, cone, a( &
                              1_ilp, j ), lda,work, nblst, czero, work( pw ), top )
                    call stdlib_wlacpy( 'ALL', top, nblst, work( pw ), top,a( 1_ilp, j ), lda )
                    ...
                    do j = j0, jcol+1, -nnb
                       if ( blk22 ) then
                          ! exploit the structure of u.
                          call stdlib_wunm22( 'RIGHT', 'NO TRANSPOSE', top, 2_ilp*nnb,nnb, nnb, work( &
                                    ppwo ), 2_ilp*nnb,a( 1_ilp, j ), lda, work( pw ),lwork-pw+1, ierr )
                       else
                          ! ignore the structure of u.
                          call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,2_ilp*nnb, 2_ilp*nnb, &
                          cone, a( 1_ilp, j ), lda,work( ppwo ), 2_ilp*nnb, czero,work( pw ), top )
                                    
                          call stdlib_wlacpy( 'ALL', top, 2_ilp*nnb, work( pw ), top,a( 1_ilp, j ), lda )
                    ...
                    call stdlib_wgemm( 'NO TRANSPOSE', 'NO TRANSPOSE', top,nblst, nblst, cone, b( &
                              1_ilp, j ), ldb,work, nblst, czero, work( pw ), top )
                    call stdlib_wlacpy( 'ALL', top, nblst, work( pw ), top,b( 1_ilp, j ), ldb )
                    ...
                    do j = j0, jcol+1, -nnb
                       if ( blk22 ) then
                          ...
                          call stdlib_wlacpy( 'ALL', top, 2_ilp*nnb, work( pw ), top,b( 1_ilp, j ), ldb )
                 ...
                 if( wantz ) then
                    ...
                    do j = j0, jcol+1, -nnb
                       ...
                       if ( blk22 ) then
                          ...
                          call stdlib_wlacpy( 'ALL', nh, 2_ilp*nnb, work( pw ), nh,z( topq, j ), ldz )
           ...
           if ( jcol<ihi )call stdlib_wgghrd( compq2, compz2, n, jcol, ihi, a, lda, b, ldb, q,ldq,&
                      z, ldz, ierr )
           work( 1_ilp ) = cmplx( lwkopt,KIND=qp)
           return
     ...
     !! pp. 241--256.
        ...
        ! =====================================================================
          ...
          ! $                     safety = one )
           ...
           safmax = one / safmin
           ulp = stdlib_qlamch( 'E' )*stdlib_qlamch( 'B' )
           anorm = stdlib_qlanhs( 'F', in, h( ilo, ilo ), ldh, work )
           bnorm = stdlib_qlanhs( 'F', in, t( ilo, ilo ), ldt, work )
           atol = max( safmin, ulp*anorm )
           btol = max( safmin, ulp*bnorm )
           ascale = one / max( safmin, anorm )
           bscale = one / max( safmin, bnorm )
           ...
           loop_360: do jiter = 1, maxit
              ...
              loop_60: do j = ilast - 1, ilo, -1
                 ...
                 if( abs( t( j, j ) )<max( safmin,ulp*temp ) ) then
                    ...
                    if( ilazro .or. ilazr2 ) then
                       do jch = j, ilast - 1
                          temp = h( jch, jch )
                          call stdlib_qlartg( temp, h( jch+1, jch ), c, s,h( jch, jch ) )
                          h( jch+1, jch ) = zero
                          call stdlib_qrot( ilastm-jch, h( jch, jch+1 ), ldh,h( jch+1, jch+1 ), &
                                    ldh, c, s )
                          call stdlib_qrot( ilastm-jch, t( jch, jch+1 ), ldt,t( jch+1, jch+1 ), &
                                    ldt, c, s )
                          if( ilq )call stdlib_qrot( n, q( 1_ilp, jch ), 1_ilp, q( 1_ilp, jch+1 ), 1_ilp,c, s )
                       ...
                       do jch = j, ilast - 1
                          ...
                          h( jch+1, jch-1 ) = zero
                          call stdlib_qrot( jch+1-ifrstm, h( ifrstm, jch ), 1_ilp,h( ifrstm, jch-1 ), &
                                    1_ilp, c, s )
                          call stdlib_qrot( jch-ifrstm, t( ifrstm, jch ), 1_ilp,t( ifrstm, jch-1 ), 1_ilp,&
                                     c, s )
                          if( ilz )call stdlib_qrot( n, z( 1_ilp, jch ), 1_ilp, z( 1_ilp, jch-1 ), 1_ilp,c, s )
              ...
              h( ilast, ilast-1 ) = zero
              call stdlib_qrot( ilast-ifrstm, h( ifrstm, ilast ), 1_ilp,h( ifrstm, ilast-1 ), 1_ilp, c, s &
                        )
              call stdlib_qrot( ilast-ifrstm, t( ifrstm, ilast ), 1_ilp,t( ifrstm, ilast-1 ), 1_ilp, c, s &
                        )
              if( ilz )call stdlib_qrot( n, z( 1_ilp, ilast ), 1_ilp, z( 1_ilp, ilast-1 ), 1_ilp, c, s )
              ! h(ilast,ilast-1)=0 -- standardize b, set alphar, alphai,
                                    ! and beta
                                    80 continue
              if( t( ilast, ilast )<zero ) then
              ...
              if( ( iiter / 10_ilp )*10_ilp==iiter ) then
                 ...
                 ! shifts based on the generalized eigenvalues of the
                 ! bottom-right 2x2 block of a and b. the first eigenvalue
                 ! returned by stdlib_qlag2 is the wilkinson shift (aep p.512_qp),
                 call stdlib_qlag2( h( ilast-1, ilast-1 ), ldh,t( ilast-1, ilast-1 ), ldt, &
                           safmin*safety, s1,s2, wr, wr2, wi )
                 if ( abs( (wr/s1)*t( ilast, ilast ) - h( ilast, ilast ) )> abs( (wr2/s2)*t( &
                           ilast, ilast )- h( ilast, ilast ) ) ) then
                    temp = wr
              ...
              temp = s1*h( istart, istart ) - wr*t( istart, istart )
              temp2 = s1*h( istart+1, istart )
              call stdlib_qlartg( temp, temp2, c, s, tempr )
              ! sweep
              loop_190: do j = istart, ilast - 1
                 if( j>istart ) then
                    temp = h( j, j-1 )
                    call stdlib_qlartg( temp, h( j+1, j-1 ), c, s, h( j, j-1 ) )
                    h( j+1, j-1 ) = zero
                 end if
                 do jc = j, ilastm
                    temp = c*h( j, jc ) + s*h( j+1, jc )
                 ...
                 temp = t( j+1, j+1 )
                 call stdlib_qlartg( temp, t( j+1, j ), c, s, t( j+1, j+1 ) )
                 t( j+1, j ) = zero
                 do jr = ifrstm, min( j+2, ilast )
                    temp = c*h( jr, j+1 ) + s*h( jr, j )
                    h( jr, j ) = -s*h( jr, j+1 ) + c*h( jr, j )
              ...
              if( ifirst+1==ilast ) then
                 ...
                 ! step 1: standardize, that is, rotate so that
                         ...
                         ! b = (         )  with b11 non-negative.
                             ! (  0  b22 )
                 call stdlib_qlasv2( t( ilast-1, ilast-1 ), t( ilast-1, ilast ),t( ilast, ilast ),&
                            b22, b11, sr, cr, sl, cl )
                 if( b11<zero ) then
                    cr = -cr
                    sr = -sr
                 ...
                 call stdlib_qrot( ilastm+1-ifirst, h( ilast-1, ilast-1 ), ldh,h( ilast, ilast-1 )&
                           , ldh, cl, sl )
                 call stdlib_qrot( ilast+1-ifrstm, h( ifrstm, ilast-1 ), 1_ilp,h( ifrstm, ilast ), 1_ilp, &
                           cr, sr )
                 if( ilast<ilastm )call stdlib_qrot( ilastm-ilast, t( ilast-1, ilast+1 ), ldt,t( &
                           ilast, ilast+1 ), ldt, cl, sl )
                 if( ifrstm<ilast-1 )call stdlib_qrot( ifirst-ifrstm, t( ifrstm, ilast-1 ), 1_ilp,t( &
                           ifrstm, ilast ), 1_ilp, cr, sr )
                 if( ilq )call stdlib_qrot( n, q( 1_ilp, ilast-1 ), 1_ilp, q( 1_ilp, ilast ), 1_ilp, cl,sl )
                           
                 if( ilz )call stdlib_qrot( n, z( 1_ilp, ilast-1 ), 1_ilp, z( 1_ilp, ilast ), 1_ilp, cr,sr )
                 ...
                 if( abs( c11r )+abs( c11i )+abs( c12 )>abs( c21 )+abs( c22r )+abs( c22i ) ) &
                           then
                    t1 = stdlib_qlapy3( c12, c11r, c11i )
                    cz = c12 / t1
                    szr = -c11r / t1
                    szi = -c11i / t1
                 else
                    cz = stdlib_qlapy2( c22r, c22i )
                    if( cz<=safmin ) then
                       cz = zero
                       szr = one
                       szi = zero
                 ...
                 b1r = cq*cz*b11 + tempr*b22
                 b1i = tempi*b22
                 b1a = stdlib_qlapy2( b1r, b1i )
                 b2r = cq*cz*b22 + tempr*b11
                 b2i = -tempi*b11
                 b2a = stdlib_qlapy2( b2r, b2i )
                 ! normalize so beta > 0, and im( alpha1 ) > 0
                 beta( ilast-1 ) = b1a
                 beta( ilast ) = b2a
                 alphar( ilast-1 ) = ( wr*b1a )*s1inv
                 ...
                 v( 3_ilp ) = ad32l*ad21l
                 istart = ifirst
                 call stdlib_qlarfg( 3_ilp, v( 1_ilp ), v( 2_ilp ), 1_ilp, tau )
                 v( 1_ilp ) = one
                 ! sweep
                 loop_290: do j = istart, ilast - 2
                    ! all but last elements: use 3x3 householder transforms.
                    ...
                    if( j>istart ) then
                       ...
                       v( 2_ilp ) = h( j+1, j-1 )
                       v( 3_ilp ) = h( j+2, j-1 )
                       call stdlib_qlarfg( 3_ilp, h( j, j-1 ), v( 2_ilp ), 1_ilp, tau )
                       v( 1_ilp ) = one
                       h( j+1, j-1 ) = zero
                       h( j+2, j-1 ) = zero
                    end if
     ...
     !! pp. 241--256.
        ...
        ! =====================================================================
           ...
           if( n<=0_ilp ) then
              work( 1_ilp ) = cmplx( 1_ilp,KIND=qp)
              return
           end if
           ! initialize q and z
           if( icompq==3_ilp )call stdlib_wlaset( 'FULL', n, n, czero, cone, q, ldq )
           ...
           safmin = stdlib_qlamch( 'S' )
           ulp = stdlib_qlamch( 'E' )*stdlib_qlamch( 'B' )
           anorm = stdlib_wlanhs( 'F', in, h( ilo, ilo ), ldh, rwork )
           bnorm = stdlib_wlanhs( 'F', in, t( ilo, ilo ), ldt, rwork )
           atol = max( safmin, ulp*anorm )
           btol = max( safmin, ulp*bnorm )
           ascale = one / max( safmin, anorm )
           bscale = one / max( safmin, bnorm )
           ...
           do j = ihi + 1, n
              ...
              if( absb>safmin ) then
                 ...
                 t( j, j ) = absb
                 if( ilschr ) then
                    call stdlib_wscal( j-1, signbc, t( 1_ilp, j ), 1_ilp )
                    call stdlib_wscal( j, signbc, h( 1_ilp, j ), 1_ilp )
                 else
                    call stdlib_wscal( 1_ilp, signbc, h( j, j ), 1_ilp )
                 end if
                 if( ilz )call stdlib_wscal( n, signbc, z( 1_ilp, j ), 1_ilp )
              else
                 t( j, j ) = czero
              end if
              alpha( j ) = h( j, j )
           ...
           loop_170: do jiter = 1, maxit
              ...
              loop_40: do j = ilast - 1, ilo, -1
                 ...
                 if( abs( t( j, j ) )<max( safmin,ulp*temp ) ) then
                    ...
                    if( ilazro .or. ilazr2 ) then
                       do jch = j, ilast - 1
                          ctemp = h( jch, jch )
                          call stdlib_wlartg( ctemp, h( jch+1, jch ), c, s,h( jch, jch ) )
                          h( jch+1, jch ) = czero
                          call stdlib_wrot( ilastm-jch, h( jch, jch+1 ), ldh,h( jch+1, jch+1 ), &
                                    ldh, c, s )
                          call stdlib_wrot( ilastm-jch, t( jch, jch+1 ), ldt,t( jch+1, jch+1 ), &
                                    ldt, c, s )
                          if( ilq )call stdlib_wrot( n, q( 1_ilp, jch ), 1_ilp, q( 1_ilp, jch+1 ), 1_ilp,c, conjg(&
                                     s ) )
                          if( ilazr2 )h( jch, jch-1 ) = h( jch, jch-1 )*c
                          ilazr2 = .false.
                          if( abs1( t( jch+1, jch+1 ) )>=btol ) then
                       ...
                       do jch = j, ilast - 1
                          ...
                          if( jch<ilastm-1 )call stdlib_wrot( ilastm-jch-1, t( jch, jch+2 ), ldt,&
                                    t( jch+1, jch+2 ), ldt, c, s )
                          call stdlib_wrot( ilastm-jch+2, h( jch, jch-1 ), ldh,h( jch+1, jch-1 ), &
                                    ldh, c, s )
                          if( ilq )call stdlib_wrot( n, q( 1_ilp, jch ), 1_ilp, q( 1_ilp, jch+1 ), 1_ilp,c, conjg(&
                                     s ) )
                          ctemp = h( jch+1, jch )
                          ...
                          h( jch+1, jch-1 ) = czero
                          call stdlib_wrot( jch+1-ifrstm, h( ifrstm, jch ), 1_ilp,h( ifrstm, jch-1 ), &
                                    1_ilp, c, s )
                          call stdlib_wrot( jch-ifrstm, t( ifrstm, jch ), 1_ilp,t( ifrstm, jch-1 ), 1_ilp,&
                                     c, s )
                          if( ilz )call stdlib_wrot( n, z( 1_ilp, jch ), 1_ilp, z( 1_ilp, jch-1 ), 1_ilp,c, s )
              ...
              h( ilast, ilast-1 ) = czero
              call stdlib_wrot( ilast-ifrstm, h( ifrstm, ilast ), 1_ilp,h( ifrstm, ilast-1 ), 1_ilp, c, s &
                        )
              call stdlib_wrot( ilast-ifrstm, t( ifrstm, ilast ), 1_ilp,t( ifrstm, ilast-1 ), 1_ilp, c, s &
                        )
              if( ilz )call stdlib_wrot( n, z( 1_ilp, ilast ), 1_ilp, z( 1_ilp, ilast-1 ), 1_ilp, c, s )
              ! h(ilast,ilast-1)=0 -- standardize b, set alpha and beta
              60 continue
              absb = abs( t( ilast, ilast ) )
              if( absb>safmin ) then
                 ...
                 t( ilast, ilast ) = absb
                 if( ilschr ) then
                    call stdlib_wscal( ilast-ifrstm, signbc, t( ifrstm, ilast ), 1_ilp )
                    call stdlib_wscal( ilast+1-ifrstm, signbc, h( ifrstm, ilast ),1_ilp )
                 else
                    call stdlib_wscal( 1_ilp, signbc, h( ilast, ilast ), 1_ilp )
                 end if
                 if( ilz )call stdlib_wscal( n, signbc, z( 1_ilp, ilast ), 1_ilp )
              else
                 t( ilast, ilast ) = czero
              end if
              alpha( ilast ) = h( ilast, ilast )
              ...
              ! initial q
              ctemp2 = ascale*h( istart+1, istart )
              call stdlib_wlartg( ctemp, ctemp2, c, s, ctemp3 )
              ! sweep
              loop_150: do j = istart, ilast - 1
                 if( j>istart ) then
                    ctemp = h( j, j-1 )
                    call stdlib_wlartg( ctemp, h( j+1, j-1 ), c, s, h( j, j-1 ) )
                    h( j+1, j-1 ) = czero
                 end if
                 do jc = j, ilastm
                    ctemp = c*h( j, jc ) + s*h( j+1, jc )
                 ...
                 ctemp = t( j+1, j+1 )
                 call stdlib_wlartg( ctemp, t( j+1, j ), c, s, t( j+1, j+1 ) )
                 t( j+1, j ) = czero
                 do jr = ifrstm, min( j+2, ilast )
                    ctemp = c*h( jr, j+1 ) + s*h( jr, j )
                    h( jr, j ) = -conjg( s )*h( jr, j+1 ) + c*h( jr, j )
     ...
     !! DGGBAL.
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( job, 'S' ) .or. stdlib_lsame( job, 'B' ) ) then
              ...
              if( rightv ) then
                 do i = ilo, ihi
                    call stdlib_qscal( m, rscale( i ), v( i, 1_ilp ), ldv )
                 end do
              end if
              ! backward transformation on left eigenvectors
              if( leftv ) then
                 do i = ilo, ihi
                    call stdlib_qscal( m, lscale( i ), v( i, 1_ilp ), ldv )
                 end do
              end if
           end if
           ! backward permutation
           ...
           if( stdlib_lsame( job, 'P' ) .or. stdlib_lsame( job, 'B' ) ) then
              ...
              if( rightv ) then
                 ...
                 loop_40: do i = ilo - 1, 1, -1
                    k = int(rscale( i ),KIND=ilp)
                    if( k==i )cycle loop_40
                    call stdlib_qswap( m, v( i, 1_ilp ), ldv, v( k, 1_ilp ), ldv )
                 end do loop_40
                 50 continue
                 if( ihi==n )go to 70
                 loop_60: do i = ihi + 1, n
     ...
     !! ZGGBAL.
        ...
        ! =====================================================================
           ...
           if( stdlib_lsame( job, 'S' ) .or. stdlib_lsame( job, 'B' ) ) then
              ...
              if( rightv ) then
                 do i = ilo, ihi
                    call stdlib_wdscal( m, rscale( i ), v( i, 1_ilp ), ldv )
                 end do
              end if
              ! backward transformation on left eigenvectors
              if( leftv ) then
                 do i = ilo, ihi
                    call stdlib_wdscal( m, lscale( i ), v( i, 1_ilp ), ldv )
                 end do
              end if
           end if
           ! backward permutation
           ...
           if( stdlib_lsame( job, 'P' ) .or. stdlib_lsame( job, 'B' ) ) then
              ...
              if( rightv ) then
                 ...
                 loop_40: do i = ilo - 1, 1, -1
                    k = int(rscale( i ),KIND=ilp)
                    if( k==i )cycle loop_40
                    call stdlib_wswap( m, v( i, 1_ilp ), ldv, v( k, 1_ilp ), ldv )
                 end do loop_40
                 50 continue
                 if( ihi==n )go to 70
                 loop_60: do i = ihi + 1, n
