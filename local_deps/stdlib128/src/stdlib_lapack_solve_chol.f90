  ...
  contains
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_qpoequ( n, a, lda, s, scond, amax, infequ )
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_qlaqsy( uplo, n, a, lda, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right hand side.
           ...
           if( nofact .or. equil ) then
              ...
              call stdlib_qpotrf( uplo, n, af, ldaf, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_qlansy( '1', uplo, n, a, lda, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_qpocon( uplo, n, af, ldaf, anorm, rcond, work, iwork, info )
           ! compute the solution matrix x.
           call stdlib_qlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_qpotrs( uplo, n, nrhs, af, ldaf, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_qporfs( uplo, n, nrhs, a, lda, af, ldaf, b, ldb, x, ldx,ferr, berr, work, &
                     iwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_wpoequ( n, a, lda, s, scond, amax, infequ )
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_wlaqhe( uplo, n, a, lda, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right hand side.
           ...
           if( nofact .or. equil ) then
              ...
              call stdlib_wpotrf( uplo, n, af, ldaf, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_wlanhe( '1', uplo, n, a, lda, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_wpocon( uplo, n, af, ldaf, anorm, rcond, work, rwork, info )
           ! compute the solution matrix x.
           call stdlib_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_wpotrs( uplo, n, nrhs, af, ldaf, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_wporfs( uplo, n, nrhs, a, lda, af, ldaf, b, ldb, x, ldx,ferr, berr, work, &
                     rwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**t*u or a = l*l**t.
           call stdlib_qpptrf( uplo, n, ap, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
     ...
     !! equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**h *u or a = l*l**h.
           call stdlib_wpptrf( uplo, n, ap, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_qppequ( uplo, n, ap, s, scond, amax, infequ )
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_qlaqsp( uplo, n, ap, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right-hand side.
           ...
           if( nofact .or. equil ) then
              ...
              call stdlib_qpptrf( uplo, n, afp, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the reciprocal of the condition number of a.
           call stdlib_qppcon( uplo, n, afp, anorm, rcond, work, iwork, info )
           ! compute the solution matrix x.
           call stdlib_qlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_qpptrs( uplo, n, nrhs, afp, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_qpprfs( uplo, n, nrhs, ap, afp, b, ldb, x, ldx, ferr, berr,work, iwork, &
                     info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_wppequ( uplo, n, ap, s, scond, amax, infequ )
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_wlaqhp( uplo, n, ap, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right-hand side.
           ...
           if( nofact .or. equil ) then
              ...
              call stdlib_wpptrf( uplo, n, afp, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the reciprocal of the condition number of a.
           call stdlib_wppcon( uplo, n, afp, anorm, rcond, work, rwork, info )
           ! compute the solution matrix x.
           call stdlib_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_wpptrs( uplo, n, nrhs, afp, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_wpprfs( uplo, n, nrhs, ap, afp, b, ldb, x, ldx, ferr, berr,work, rwork, &
                     info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**t*u or a = l*l**t.
           call stdlib_qpbtrf( uplo, n, kd, ab, ldab, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
     ...
     !! system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the cholesky factorization a = u**h *u or a = l*l**h.
           call stdlib_wpbtrf( uplo, n, kd, ab, ldab, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_qpbequ( uplo, n, kd, ab, ldab, s, scond, amax, infequ )
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_qlaqsb( uplo, n, kd, ab, ldab, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right-hand side.
           ...
           if( nofact .or. equil ) then
              ...
              if( upper ) then
                 do j = 1, n
                    j1 = max( j-kd, 1_ilp )
                    call stdlib_qcopy( j-j1+1, ab( kd+1-j+j1, j ), 1_ilp,afb( kd+1-j+j1, j ), 1_ilp )
                              
                 end do
              else
                 do j = 1, n
                    j2 = min( j+kd, n )
                    call stdlib_qcopy( j2-j+1, ab( 1_ilp, j ), 1_ilp, afb( 1_ilp, j ), 1_ilp )
                 end do
              end if
              call stdlib_qpbtrf( uplo, n, kd, afb, ldafb, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_qlansb( '1', uplo, n, kd, ab, ldab, work )
           ! compute the reciprocal of the condition number of a.
           call stdlib_qpbcon( uplo, n, kd, afb, ldafb, anorm, rcond, work, iwork,info )
           ! compute the solution matrix x.
           call stdlib_qlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_qpbtrs( uplo, n, kd, nrhs, afb, ldafb, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_qpbrfs( uplo, n, kd, nrhs, ab, ldab, afb, ldafb, b, ldb, x,ldx, ferr, berr,&
                      work, iwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_wpbequ( uplo, n, kd, ab, ldab, s, scond, amax, infequ )
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_wlaqhb( uplo, n, kd, ab, ldab, s, scond, amax, equed )
                 rcequ = stdlib_lsame( equed, 'Y' )
              end if
           end if
           ! scale the right-hand side.
           ...
           if( nofact .or. equil ) then
              ...
              if( upper ) then
                 do j = 1, n
                    j1 = max( j-kd, 1_ilp )
                    call stdlib_wcopy( j-j1+1, ab( kd+1-j+j1, j ), 1_ilp,afb( kd+1-j+j1, j ), 1_ilp )
                              
                 end do
              else
                 do j = 1, n
                    j2 = min( j+kd, n )
                    call stdlib_wcopy( j2-j+1, ab( 1_ilp, j ), 1_ilp, afb( 1_ilp, j ), 1_ilp )
                 end do
              end if
              call stdlib_wpbtrf( uplo, n, kd, afb, ldafb, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_wlanhb( '1', uplo, n, kd, ab, ldab, rwork )
           ! compute the reciprocal of the condition number of a.
           call stdlib_wpbcon( uplo, n, kd, afb, ldafb, anorm, rcond, work, rwork,info )
           ! compute the solution matrix x.
           call stdlib_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_wpbtrs( uplo, n, kd, nrhs, afb, ldafb, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_wpbrfs( uplo, n, kd, nrhs, ab, ldab, afb, ldafb, b, ldb, x,ldx, ferr, berr,&
                      work, rwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( rcequ ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the l*d*l**t (or u**t*d*u) factorization of a.
              call stdlib_qcopy( n, d, 1_ilp, df, 1_ilp )
              if( n>1_ilp )call stdlib_qcopy( n-1, e, 1_ilp, ef, 1_ilp )
              call stdlib_qpttrf( n, df, ef, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_qlanst( '1', n, d, e )
           ! compute the reciprocal of the condition number of a.
           call stdlib_qptcon( n, df, ef, anorm, rcond, work, info )
           ! compute the solution vectors x.
           call stdlib_qlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_qpttrs( n, nrhs, df, ef, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the l*d*l**h (or u**h*d*u) factorization of a.
              call stdlib_qcopy( n, d, 1_ilp, df, 1_ilp )
              if( n>1_ilp )call stdlib_wcopy( n-1, e, 1_ilp, ef, 1_ilp )
              call stdlib_wpttrf( n, df, ef, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! compute the norm of the matrix a.
           anorm = stdlib_wlanht( '1', n, d, e )
           ! compute the reciprocal of the condition number of a.
           call stdlib_wptcon( n, df, ef, anorm, rcond, rwork, info )
           ! compute the solution vectors x.
           call stdlib_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_wpttrs( 'LOWER', n, nrhs, df, ef, x, ldx, info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_wptrfs( 'LOWER', n, nrhs, d, e, df, ef, b, ldb, x, ldx, ferr,berr, work, &
                     rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_qlamch( 'EPSILON' ) )info = n + 1_ilp
           return
