...
module stdlib_linalg_iterative_solvers
    ...
    type, public :: stdlib_linop_qp_type
        procedure(vector_sub_qp), nopass, pointer    :: matvec => null()
        procedure(reduction_sub_qp), nopass, pointer :: inner_product => default_dot_qp
    end type
    ...
    type, public :: stdlib_solver_workspace_qp_type
        real(qp), allocatable :: tmp(:,:)
        procedure(logger_sub_qp), pointer, nopass :: callback => null()
    end type 
    ...
    abstract interface
        ...
        subroutine vector_sub_qp(x,y,alpha,beta,op)
            import :: qp
            real(qp), intent(in)  :: x(:)
        ...
        pure real(qp) function reduction_sub_qp(x,y) result(r)
            import :: qp
            real(qp), intent(in) :: x(:)
            real(qp), intent(in) :: y(:)
        end function
        subroutine logger_sub_qp(x,norm_sq,iter)
            import :: qp
            real(qp), intent(in) :: x(:)
            real(qp), intent(in) :: norm_sq
            integer, intent(in) :: iter
        end subroutine
    end interface
    ...
    interface stdlib_solve_cg_kernel
        ...
        module subroutine stdlib_solve_cg_kernel_qp(A,b,x,rtol,atol,maxiter,workspace)
            class(stdlib_linop_qp_type), intent(in) :: A !! linear operator
            real(qp), intent(in) :: b(:) !! right-hand side vector
            real(qp), intent(inout) :: x(:) !! solution vector and initial guess
            real(qp), intent(in) :: rtol !! relative tolerance for convergence
            real(qp), intent(in) :: atol !! absolut tolerance for convergence
            integer, intent(in) :: maxiter !! maximum number of iterations
            type(stdlib_solver_workspace_qp_type), intent(inout) :: workspace !! workspace for the solver
        end subroutine
    end interface
    ...
    interface stdlib_solve_cg
        ...
        module subroutine stdlib_solve_cg_dense_qp(A,b,x,di,rtol,atol,maxiter,restart,workspace)
            !! linear operator matrix
            real(qp), intent(in) :: A(:,:) 
            real(qp), intent(in) :: b(:) !! right-hand side vector
            real(qp), intent(inout) :: x(:) !! solution vector and initial guess
            real(qp), intent(in), optional :: rtol !! relative tolerance for convergence
            real(qp), intent(in), optional :: atol !! absolute tolerance for convergence
            logical(int8), intent(in), optional, target  :: di(:) !! dirichlet conditions mask
            integer, intent(in), optional :: maxiter !! maximum number of iterations
            logical, intent(in), optional :: restart !! restart flag
            type(stdlib_solver_workspace_qp_type), optional, intent(inout), target :: workspace !! workspace for the solver
        end subroutine
        ...
        module subroutine stdlib_solve_cg_CSR_qp(A,b,x,di,rtol,atol,maxiter,restart,workspace)
            !! linear operator matrix
            type(CSR_qp_type), intent(in) :: A
            real(qp), intent(in) :: b(:) !! right-hand side vector
            real(qp), intent(inout) :: x(:) !! solution vector and initial guess
            real(qp), intent(in), optional :: rtol !! relative tolerance for convergence
            real(qp), intent(in), optional :: atol !! absolute tolerance for convergence
    ...
    interface stdlib_solve_pcg_kernel
        ...
        module subroutine stdlib_solve_pcg_kernel_qp(A,M,b,x,rtol,atol,maxiter,workspace)
            class(stdlib_linop_qp_type), intent(in) :: A !! linear operator
            class(stdlib_linop_qp_type), intent(in) :: M !! preconditioner linear operator
            real(qp), intent(in) :: b(:) !! right-hand side vector
            real(qp), intent(inout) :: x(:) !! solution vector and initial guess
            real(qp), intent(in) :: rtol !! relative tolerance for convergence
            real(qp), intent(in) :: atol !! absolute tolerance for convergence
            integer, intent(in) :: maxiter !! maximum number of iterations
            type(stdlib_solver_workspace_qp_type), intent(inout) :: workspace !! workspace for the solver
        end subroutine
    end interface
    ...
    interface stdlib_solve_bicgstab_kernel
        ...
        module subroutine stdlib_solve_bicgstab_kernel_qp(A,M,b,x,rtol,atol,maxiter,workspace)
            class(stdlib_linop_qp_type), intent(in) :: A !! linear operator
            class(stdlib_linop_qp_type), intent(in) :: M !! preconditioner linear operator
            real(qp), intent(in) :: b(:) !! right-hand side vector
            real(qp), intent(inout) :: x(:) !! solution vector and initial guess
    ...
    interface stdlib_solve_pcg
        ...
        module subroutine stdlib_solve_pcg_dense_qp(A,b,x,di,rtol,atol,maxiter,restart,precond,M,workspace)
            !! linear operator matrix
            real(qp), intent(in) :: A(:,:)
            real(qp), intent(in) :: b(:) !! right-hand side vector
            real(qp), intent(inout) :: x(:) !! solution vector and initial guess
            ...
            logical, intent(in), optional :: restart !! restart flag
            integer, intent(in), optional  :: precond !! preconditioner method enumerator
            class(stdlib_linop_qp_type), optional , intent(in), target :: M !! preconditioner linear operator
            type(stdlib_solver_workspace_qp_type), optional, intent(inout), target :: workspace !! workspace for the solver
        end subroutine
        ...
        module subroutine stdlib_solve_pcg_CSR_qp(A,b,x,di,rtol,atol,maxiter,restart,precond,M,workspace)
            !! linear operator matrix
            type(CSR_qp_type), intent(in) :: A
            real(qp), intent(in) :: b(:) !! right-hand side vector
            real(qp), intent(inout) :: x(:) !! solution vector and initial guess
    ...
    interface stdlib_solve_bicgstab
        ...
        module subroutine stdlib_solve_bicgstab_dense_qp(A,b,x,di,rtol,atol,maxiter,restart,precond,M,workspace)
            !! linear operator matrix
            real(qp), intent(in) :: A(:,:)
            real(qp), intent(in) :: b(:) !! right-hand side vector
            real(qp), intent(inout) :: x(:) !! solution vector and initial guess
        ...
        module subroutine stdlib_solve_bicgstab_CSR_qp(A,b,x,di,rtol,atol,maxiter,restart,precond,M,workspace)
            !! linear operator matrix
            type(CSR_qp_type), intent(in) :: A
            real(qp), intent(in) :: b(:) !! right-hand side vector
            real(qp), intent(inout) :: x(:) !! solution vector and initial guess
...
contains
    ...
    pure real(qp) function default_dot_qp(x,y) result(r)
        use stdlib_intrinsics, only: stdlib_dot_product
        real(qp), intent(in) :: x(:)
        real(qp), intent(in) :: y(:)
        r = stdlib_dot_product(x,y)
