...
module stdlib_str2num
    ...
    interface to_num_from_stream
        ...
        module procedure to_qp_from_stream
    end interface
    ...
    function to_qp_from_stream(s,mold,stat) result(v)
        ! -- In/out Variables
        character(len=:), pointer :: s !! input string
        real(qp), intent(in) :: mold !! dummy argument to disambiguate at compile time the generic interface
        real(qp) :: v !! Output real(qp) value
