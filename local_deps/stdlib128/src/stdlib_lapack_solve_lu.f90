  ...
  contains
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_qgeequ( n, n, a, lda, r, c, rowcnd, colcnd, amax, infequ )
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_qlaqge( n, n, a, lda, r, c, rowcnd, colcnd, amax,equed )
                 rowequ = stdlib_lsame( equed, 'R' ) .or. stdlib_lsame( equed, 'B' )
                 colequ = stdlib_lsame( equed, 'C' ) .or. stdlib_lsame( equed, 'B' )
              end if
           end if
           ...
           if( nofact .or. equil ) then
              ! compute the lu factorization of a.
              call stdlib_qlacpy( 'FULL', n, n, a, lda, af, ldaf )
              call stdlib_qgetrf( n, n, af, ldaf, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp ) then
                 ! compute the reciprocal pivot growth factor of the
                 ! leading rank-deficient info columns of a.
                 rpvgrw = stdlib_qlantr( 'M', 'U', 'N', info, info, af, ldaf,work )
                 if( rpvgrw==zero ) then
                    rpvgrw = one
                 else
                    rpvgrw = stdlib_qlange( 'M', n, info, a, lda, work ) / rpvgrw
                 end if
                 work( 1_ilp ) = rpvgrw
                 rcond = zero
                 return
           ...
           anorm = stdlib_qlange( norm, n, n, a, lda, work )
           rpvgrw = stdlib_qlantr( 'M', 'U', 'N', n, n, af, ldaf, work )
           if( rpvgrw==zero ) then
              rpvgrw = one
           else
              rpvgrw = stdlib_qlange( 'M', n, n, a, lda, work ) / rpvgrw
           end if
           ! compute the reciprocal of the condition number of a.
           call stdlib_qgecon( norm, n, af, ldaf, anorm, rcond, work, iwork, info )
           ! compute the solution matrix x.
           call stdlib_qlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_qgetrs( trans, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_qgerfs( trans, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, iwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( notran ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( equil ) then
              ! compute row and column scalings to equilibrate the matrix a.
              call stdlib_wgeequ( n, n, a, lda, r, c, rowcnd, colcnd, amax, infequ )
              if( infequ==0_ilp ) then
                 ! equilibrate the matrix.
                 call stdlib_wlaqge( n, n, a, lda, r, c, rowcnd, colcnd, amax,equed )
                 rowequ = stdlib_lsame( equed, 'R' ) .or. stdlib_lsame( equed, 'B' )
                 colequ = stdlib_lsame( equed, 'C' ) .or. stdlib_lsame( equed, 'B' )
              end if
           end if
           ...
           if( nofact .or. equil ) then
              ! compute the lu factorization of a.
              call stdlib_wlacpy( 'FULL', n, n, a, lda, af, ldaf )
              call stdlib_wgetrf( n, n, af, ldaf, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp ) then
                 ! compute the reciprocal pivot growth factor of the
                 ! leading rank-deficient info columns of a.
                 rpvgrw = stdlib_wlantr( 'M', 'U', 'N', info, info, af, ldaf,rwork )
                 if( rpvgrw==zero ) then
                    rpvgrw = one
                 else
                    rpvgrw = stdlib_wlange( 'M', n, info, a, lda, rwork ) /rpvgrw
                 end if
                 rwork( 1_ilp ) = rpvgrw
                 rcond = zero
                 return
           ...
           anorm = stdlib_wlange( norm, n, n, a, lda, rwork )
           rpvgrw = stdlib_wlantr( 'M', 'U', 'N', n, n, af, ldaf, rwork )
           if( rpvgrw==zero ) then
              rpvgrw = one
           else
              rpvgrw = stdlib_wlange( 'M', n, n, a, lda, rwork ) / rpvgrw
           end if
           ! compute the reciprocal of the condition number of a.
           call stdlib_wgecon( norm, n, af, ldaf, anorm, rcond, work, rwork, info )
           ! compute the solution matrix x.
           call stdlib_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_wgetrs( trans, n, nrhs, af, ldaf, ipiv, x, ldx, info )
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_wgerfs( trans, n, nrhs, a, lda, af, ldaf, ipiv, b, ldb, x,ldx, ferr, berr, &
                     work, rwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( notran ) then
     ...
     !! is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the lu factorization of the band matrix a.
           call stdlib_qgbtrf( n, n, kl, ku, ab, ldab, ipiv, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
     ...
     !! is then used to solve the system of equations A * X = B.
        ...
        ! =====================================================================
           ...
           ! compute the lu factorization of the band matrix a.
           call stdlib_wgbtrf( n, n, kl, ku, ab, ldab, ipiv, info )
           if( info==0_ilp ) then
              ! solve the system a*x = b, overwriting b with x.
        ...
        ! =====================================================================
           ...
           if( nofact .or. equil ) then
              ...
              do j = 1, n
                 j1 = max( j-ku, 1_ilp )
                 j2 = min( j+kl, n )
                 call stdlib_qcopy( j2-j1+1, ab( ku+1-j+j1, j ), 1_ilp,afb( kl+ku+1-j+j1, j ), 1_ilp )
              ...
              if( info>0_ilp ) then
                 ...
                 rpvgrw = stdlib_qlantb( 'M', 'U', 'N', info, min( info-1, kl+ku ),afb( max( 1_ilp, &
                           kl+ku+2-info ), 1_ilp ), ldafb,work )
                 if( rpvgrw==zero ) then
                    rpvgrw = one
                 else
           ...
           anorm = stdlib_qlangb( norm, n, kl, ku, ab, ldab, work )
           rpvgrw = stdlib_qlantb( 'M', 'U', 'N', n, kl+ku, afb, ldafb, work )
           if( rpvgrw==zero ) then
              rpvgrw = one
           else
              rpvgrw = stdlib_qlangb( 'M', n, kl, ku, ab, ldab, work ) / rpvgrw
           end if
           ! compute the reciprocal of the condition number of a.
           ...
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_qgbrfs( trans, n, kl, ku, nrhs, ab, ldab, afb, ldafb, ipiv,b, ldb, x, ldx, &
                     ferr, berr, work, iwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( notran ) then
        ...
        ! =====================================================================
           ...
           if( nofact .or. equil ) then
              ...
              do j = 1, n
                 j1 = max( j-ku, 1_ilp )
                 j2 = min( j+kl, n )
                 call stdlib_wcopy( j2-j1+1, ab( ku+1-j+j1, j ), 1_ilp,afb( kl+ku+1-j+j1, j ), 1_ilp )
              ...
              if( info>0_ilp ) then
                 ...
                 rpvgrw = stdlib_wlantb( 'M', 'U', 'N', info, min( info-1, kl+ku ),afb( max( 1_ilp, &
                           kl+ku+2-info ), 1_ilp ), ldafb,rwork )
                 if( rpvgrw==zero ) then
                    rpvgrw = one
                 else
           ...
           anorm = stdlib_wlangb( norm, n, kl, ku, ab, ldab, rwork )
           rpvgrw = stdlib_wlantb( 'M', 'U', 'N', n, kl+ku, afb, ldafb, rwork )
           if( rpvgrw==zero ) then
              rpvgrw = one
           else
              rpvgrw = stdlib_wlangb( 'M', n, kl, ku, ab, ldab, rwork ) / rpvgrw
           end if
           ! compute the reciprocal of the condition number of a.
           ...
           ! use iterative refinement to improve the computed solution and
           ! compute error bounds and backward error estimates for it.
           call stdlib_wgbrfs( trans, n, kl, ku, nrhs, ab, ldab, afb, ldafb, ipiv,b, ldb, x, ldx, &
                     ferr, berr, work, rwork, info )
           ! transform the solution matrix x to a solution of the original
           ! system.
           if( notran ) then
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ...
              if( n>1_ilp ) then
                 call stdlib_qcopy( n-1, dl, 1_ilp, dlf, 1_ilp )
                 call stdlib_qcopy( n-1, du, 1_ilp, duf, 1_ilp )
              end if
              call stdlib_qgttrf( n, dlf, df, duf, du2, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_qgtrfs( trans, n, nrhs, dl, d, du, dlf, df, duf, du2, ipiv,b, ldb, x, ldx, &
                     ferr, berr, work, iwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_qlamch( 'EPSILON' ) )info = n + 1_ilp
           return
     ...
     !! provided.
        ...
        ! =====================================================================
           ...
           if( nofact ) then
              ! compute the lu factorization of a.
              call stdlib_wcopy( n, d, 1_ilp, df, 1_ilp )
              if( n>1_ilp ) then
                 call stdlib_wcopy( n-1, dl, 1_ilp, dlf, 1_ilp )
                 call stdlib_wcopy( n-1, du, 1_ilp, duf, 1_ilp )
              end if
              call stdlib_wgttrf( n, dlf, df, duf, du2, ipiv, info )
              ! return if info is non-zero.
              if( info>0_ilp )then
                 rcond = zero
                 return
           ...
           anorm = stdlib_wlangt( norm, n, dl, d, du )
           ! compute the reciprocal of the condition number of a.
           call stdlib_wgtcon( norm, n, dlf, df, duf, du2, ipiv, anorm, rcond, work,info )
           ! compute the solution vectors x.
           call stdlib_wlacpy( 'FULL', n, nrhs, b, ldb, x, ldx )
           call stdlib_wgttrs( trans, n, nrhs, dlf, df, duf, du2, ipiv, x, ldx,info )
           ! use iterative refinement to improve the computed solutions and
           ! compute error bounds and backward error estimates for them.
           call stdlib_wgtrfs( trans, n, nrhs, dl, d, du, dlf, df, duf, du2, ipiv,b, ldb, x, ldx, &
                     ferr, berr, work, rwork, info )
           ! set info = n+1 if the matrix is singular to working precision.
           if( rcond<stdlib_qlamch( 'EPSILON' ) )info = n + 1_ilp
           return
