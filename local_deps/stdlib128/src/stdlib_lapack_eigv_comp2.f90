  ...
  contains
        ...
        ! =====================================================================
           ...
           if( m==n .or. m==0_ilp ) then
              ...
              if( wantd ) then
                 ...
                 dsum = one
                 do i = 1, n
                    call stdlib_qlassq( n, a( 1_ilp, i ), 1_ilp, dscale, dsum )
                    call stdlib_qlassq( n, b( 1_ilp, i ), 1_ilp, dscale, dsum )
                 end do
                 dif( 1_ilp ) = dscale*sqrt( dsum )
                 dif( 2_ilp ) = dif( 1_ilp )
              end if
           ...
           loop_30: do k = 1, n
              if( pair ) then
                 ...
                 if( swap ) then
                    ...
                    ! q and z accordingly (if requested):
                    kk = k
                    if( k/=ks )call stdlib_qtgexc( wantq, wantz, n, a, lda, b, ldb, q, ldq,z, ldz,&
                               kk, ks, work, lwork, ierr )
                    if( ierr>0_ilp ) then
                       ! swap is rejected: exit.
                       info = 1_ilp
           ...
           if( wantp ) then
              ...
              i = n1 + 1_ilp
              ijb = 0_ilp
              call stdlib_qlacpy( 'FULL', n1, n2, a( 1_ilp, i ), lda, work, n1 )
              call stdlib_qlacpy( 'FULL', n1, n2, b( 1_ilp, i ), ldb, work( n1*n2+1 ),n1 )
              call stdlib_qtgsyl( 'N', ijb, n1, n2, a, lda, a( i, i ), lda, work,n1, b, ldb, b( i,&
               i ), ldb, work( n1*n2+1 ), n1,dscale, dif( 1_ilp ), work( n1*n2*2_ilp+1 ),lwork-2*n1*n2, &
                         iwork, ierr )
              ! estimate the reciprocal of norms of "projections" onto left
              ! and right eigenspaces.
              rdscal = zero
              dsum = one
              call stdlib_qlassq( n1*n2, work, 1_ilp, rdscal, dsum )
              pl = rdscal*sqrt( dsum )
              if( pl==zero ) then
                 pl = one
              else
              ...
              rdscal = zero
              dsum = one
              call stdlib_qlassq( n1*n2, work( n1*n2+1 ), 1_ilp, rdscal, dsum )
              pr = rdscal*sqrt( dsum )
              if( pr==zero ) then
                 pr = one
              else
           ...
           if( wantd ) then
              ...
              if( wantd1 ) then
                 ...
                 ijb = idifjb
                 ! frobenius norm-based difu-estimate.
                 call stdlib_qtgsyl( 'N', ijb, n1, n2, a, lda, a( i, i ), lda, work,n1, b, ldb, b(&
                  i, i ), ldb, work( n1*n2+1 ),n1, dscale, dif( 1_ilp ), work( 2_ilp*n1*n2+1 ),lwork-&
                            2_ilp*n1*n2, iwork, ierr )
                 ! frobenius norm-based difl-estimate.
                 call stdlib_qtgsyl( 'N', ijb, n2, n1, a( i, i ), lda, a, lda, work,n2, b( i, i ),&
                  ldb, b, ldb, work( n1*n2+1 ),n2, dscale, dif( 2_ilp ), work( 2_ilp*n1*n2+1 ),lwork-&
                            2_ilp*n1*n2, iwork, ierr )
              else
                 ! compute 1-norm-based estimates of difu and difl using
                 ! reversed communication with stdlib_qlacn2. in each step a
                 ! generalized sylvester equation or a transposed variant
                 ! is solved.
                 kase = 0_ilp
                 n1 = m
                 ...
                 ! 1-norm-based estimate of difu.
                 40 continue
                 call stdlib_qlacn2( mn2, work( mn2+1 ), work, iwork, dif( 1_ilp ),kase, isave )
                           
                 if( kase/=0_ilp ) then
                    if( kase==1_ilp ) then
                       ! solve generalized sylvester equation.
                       call stdlib_qtgsyl( 'N', ijb, n1, n2, a, lda, a( i, i ), lda,work, n1, b, &
                       ldb, b( i, i ), ldb,work( n1*n2+1 ), n1, dscale, dif( 1_ilp ),work( 2_ilp*n1*n2+1 )&
                                 , lwork-2*n1*n2, iwork,ierr )
                    else
                       ! solve the transposed variant.
                       call stdlib_qtgsyl( 'T', ijb, n1, n2, a, lda, a( i, i ), lda,work, n1, b, &
                       ldb, b( i, i ), ldb,work( n1*n2+1 ), n1, dscale, dif( 1_ilp ),work( 2_ilp*n1*n2+1 )&
                                 , lwork-2*n1*n2, iwork,ierr )
                    end if
                    go to 40
                 ...
                 ! 1-norm-based estimate of difl.
                 50 continue
                 call stdlib_qlacn2( mn2, work( mn2+1 ), work, iwork, dif( 2_ilp ),kase, isave )
                           
                 if( kase/=0_ilp ) then
                    if( kase==1_ilp ) then
                       ! solve generalized sylvester equation.
                       call stdlib_qtgsyl( 'N', ijb, n2, n1, a( i, i ), lda, a, lda,work, n2, b( &
                       i, i ), ldb, b, ldb,work( n1*n2+1 ), n2, dscale, dif( 2_ilp ),work( 2_ilp*n1*n2+1 )&
                                 , lwork-2*n1*n2, iwork,ierr )
                    else
                       ! solve the transposed variant.
                       call stdlib_qtgsyl( 'T', ijb, n2, n1, a( i, i ), lda, a, lda,work, n2, b( &
                       i, i ), ldb, b, ldb,work( n1*n2+1 ), n2, dscale, dif( 2_ilp ),work( 2_ilp*n1*n2+1 )&
                                 , lwork-2*n1*n2, iwork,ierr )
                    end if
                    go to 50
              ...
              if( pair ) then
                 ...
                 if( pair ) then
                   ! compute the eigenvalue(s) at position k.
                    ...
                    work( 7_ilp ) = b( k, k+1 )
                    work( 8_ilp ) = b( k+1, k+1 )
                    call stdlib_qlag2( work, 2_ilp, work( 5_ilp ), 2_ilp, smlnum*eps, beta( k ),beta( k+1 ), &
                              alphar( k ), alphar( k+1 ),alphai( k ) )
                    alphai( k+1 ) = -alphai( k )
                 else
                    if( sign( one, b( k, k ) )<zero ) then
        ...
        ! =====================================================================
           ...
           if( m==n .or. m==0_ilp ) then
              ...
              if( wantd ) then
                 ...
                 dsum = one
                 do i = 1, n
                    call stdlib_wlassq( n, a( 1_ilp, i ), 1_ilp, dscale, dsum )
                    call stdlib_wlassq( n, b( 1_ilp, i ), 1_ilp, dscale, dsum )
                 end do
                 dif( 1_ilp ) = dscale*sqrt( dsum )
                 dif( 2_ilp ) = dif( 1_ilp )
              end if
           ...
           do k = 1, n
              ...
              if( swap ) then
                 ...
                 ! swap the k-th block to position ks. compute unitary q
                 ! and z that will swap adjacent diagonal blocks in (a, b).
                 if( k/=ks )call stdlib_wtgexc( wantq, wantz, n, a, lda, b, ldb, q, ldq, z,ldz, k,&
                            ks, ierr )
                 if( ierr>0_ilp ) then
                    ! swap is rejected: exit.
                    info = 1_ilp
           ...
           if( wantp ) then
              ...
              n2 = n - m
              i = n1 + 1_ilp
              call stdlib_wlacpy( 'FULL', n1, n2, a( 1_ilp, i ), lda, work, n1 )
              call stdlib_wlacpy( 'FULL', n1, n2, b( 1_ilp, i ), ldb, work( n1*n2+1 ),n1 )
              ijb = 0_ilp
              call stdlib_wtgsyl( 'N', ijb, n1, n2, a, lda, a( i, i ), lda, work,n1, b, ldb, b( i,&
               i ), ldb, work( n1*n2+1 ), n1,dscale, dif( 1_ilp ), work( n1*n2*2_ilp+1 ),lwork-2*n1*n2, &
                         iwork, ierr )
              ! estimate the reciprocal of norms of "projections" onto
              ! left and right eigenspaces
              rdscal = zero
              dsum = one
              call stdlib_wlassq( n1*n2, work, 1_ilp, rdscal, dsum )
              pl = rdscal*sqrt( dsum )
              if( pl==zero ) then
                 pl = one
              else
              ...
              rdscal = zero
              dsum = one
              call stdlib_wlassq( n1*n2, work( n1*n2+1 ), 1_ilp, rdscal, dsum )
              pr = rdscal*sqrt( dsum )
              if( pr==zero ) then
                 pr = one
              else
           ...
           if( wantd ) then
              ...
              if( wantd1 ) then
                 ...
                 ijb = idifjb
                 ! frobenius norm-based difu estimate.
                 call stdlib_wtgsyl( 'N', ijb, n1, n2, a, lda, a( i, i ), lda, work,n1, b, ldb, b(&
                  i, i ), ldb, work( n1*n2+1 ),n1, dscale, dif( 1_ilp ), work( n1*n2*2_ilp+1 ),lwork-&
                            2_ilp*n1*n2, iwork, ierr )
                 ! frobenius norm-based difl estimate.
                 call stdlib_wtgsyl( 'N', ijb, n2, n1, a( i, i ), lda, a, lda, work,n2, b( i, i ),&
                  ldb, b, ldb, work( n1*n2+1 ),n2, dscale, dif( 2_ilp ), work( n1*n2*2_ilp+1 ),lwork-&
                            2_ilp*n1*n2, iwork, ierr )
              else
                 ! compute 1-norm-based estimates of difu and difl using
                 ! reversed communication with stdlib_wlacn2. in each step a
                 ! generalized sylvester equation or a transposed variant
                 ! is solved.
                 kase = 0_ilp
                 n1 = m
                 ...
                 ! 1-norm-based estimate of difu.
                 40 continue
                 call stdlib_wlacn2( mn2, work( mn2+1 ), work, dif( 1_ilp ), kase,isave )
                 if( kase/=0_ilp ) then
                    if( kase==1_ilp ) then
                       ! solve generalized sylvester equation
                       call stdlib_wtgsyl( 'N', ijb, n1, n2, a, lda, a( i, i ), lda,work, n1, b, &
                       ldb, b( i, i ), ldb,work( n1*n2+1 ), n1, dscale, dif( 1_ilp ),work( n1*n2*2_ilp+1 )&
                                 , lwork-2*n1*n2, iwork,ierr )
                    else
                       ! solve the transposed variant.
                       call stdlib_wtgsyl( 'C', ijb, n1, n2, a, lda, a( i, i ), lda,work, n1, b, &
                       ldb, b( i, i ), ldb,work( n1*n2+1 ), n1, dscale, dif( 1_ilp ),work( n1*n2*2_ilp+1 )&
                                 , lwork-2*n1*n2, iwork,ierr )
                    end if
                    go to 40
                 ...
                 ! 1-norm-based estimate of difl.
                 50 continue
                 call stdlib_wlacn2( mn2, work( mn2+1 ), work, dif( 2_ilp ), kase,isave )
                 if( kase/=0_ilp ) then
                    if( kase==1_ilp ) then
                       ! solve generalized sylvester equation
                       call stdlib_wtgsyl( 'N', ijb, n2, n1, a( i, i ), lda, a, lda,work, n2, b( &
                       i, i ), ldb, b, ldb,work( n1*n2+1 ), n2, dscale, dif( 2_ilp ),work( n1*n2*2_ilp+1 )&
                                 , lwork-2*n1*n2, iwork,ierr )
                    else
                       ! solve the transposed variant.
                       call stdlib_wtgsyl( 'C', ijb, n2, n1, a( i, i ), lda, a, lda,work, n2, b, &
                       ldb, b( i, i ), ldb,work( n1*n2+1 ), n2, dscale, dif( 2_ilp ),work( n1*n2*2_ilp+1 )&
                                 , lwork-2*n1*n2, iwork,ierr )
                    end if
                    go to 50
           ...
           do k = 1, n
              ...
              if( dscale>safmin ) then
                 ...
                 temp2 = b( k, k ) / dscale
                 b( k, k ) = dscale
                 call stdlib_wscal( n-k, temp1, b( k, k+1 ), ldb )
                 call stdlib_wscal( n-k+1, temp1, a( k, k ), lda )
                 if( wantq )call stdlib_wscal( n, temp2, q( 1_ilp, k ), 1_ilp )
              else
     ...
     !! blocks. B is upper triangular.
        ...
        ! =====================================================================
           ...
           real(qp) :: alphai, alphar, alprqt, beta, c1, c2, cond, eps, lnrm, rnrm, root1, root2, &
                     scale, smlnum, tmpii, tmpir, tmpri, tmprr, uhav, uhavi, uhbv, uhbvi
           ! Local Arrays 
           real(qp) :: dummy(1_ilp), dummy1(1_ilp)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! decode and test the input parameters
           wantbh = stdlib_lsame( job, 'B' )
           ...
           loop_20: do k = 1, n
              ...
              if( wants ) then
                 ...
                 if( pair ) then
                    ...
                    lnrm = stdlib_qlapy2( stdlib_qnrm2( n, vl( 1_ilp, ks ), 1_ilp ),stdlib_qnrm2( n, vl( &
                              1_ilp, ks+1 ), 1_ilp ) )
                    call stdlib_qgemv( 'N', n, n, one, a, lda, vr( 1_ilp, ks ), 1_ilp, zero,work, 1_ilp )
                              
                    tmprr = stdlib_qdot( n, work, 1_ilp, vl( 1_ilp, ks ), 1_ilp )
                    tmpri = stdlib_qdot( n, work, 1_ilp, vl( 1_ilp, ks+1 ), 1_ilp )
                    call stdlib_qgemv( 'N', n, n, one, a, lda, vr( 1_ilp, ks+1 ), 1_ilp,zero, work, 1_ilp )
                              
                    tmpii = stdlib_qdot( n, work, 1_ilp, vl( 1_ilp, ks+1 ), 1_ilp )
                    tmpir = stdlib_qdot( n, work, 1_ilp, vl( 1_ilp, ks ), 1_ilp )
                    uhav = tmprr + tmpii
                    uhavi = tmpir - tmpri
                    call stdlib_qgemv( 'N', n, n, one, b, ldb, vr( 1_ilp, ks ), 1_ilp, zero,work, 1_ilp )
                              
                    tmprr = stdlib_qdot( n, work, 1_ilp, vl( 1_ilp, ks ), 1_ilp )
                    tmpri = stdlib_qdot( n, work, 1_ilp, vl( 1_ilp, ks+1 ), 1_ilp )
                    call stdlib_qgemv( 'N', n, n, one, b, ldb, vr( 1_ilp, ks+1 ), 1_ilp,zero, work, 1_ilp )
                              
                    tmpii = stdlib_qdot( n, work, 1_ilp, vl( 1_ilp, ks+1 ), 1_ilp )
                    tmpir = stdlib_qdot( n, work, 1_ilp, vl( 1_ilp, ks ), 1_ilp )
                    uhbv = tmprr + tmpii
                    uhbvi = tmpir - tmpri
                    uhav = stdlib_qlapy2( uhav, uhavi )
                    uhbv = stdlib_qlapy2( uhbv, uhbvi )
                    ...
                    uhav = stdlib_qdot( n, work, 1_ilp, vl( 1_ilp, ks ), 1_ilp )
                    call stdlib_qgemv( 'N', n, n, one, b, ldb, vr( 1_ilp, ks ), 1_ilp, zero,work, 1_ilp )
                              
                    uhbv = stdlib_qdot( n, work, 1_ilp, vl( 1_ilp, ks ), 1_ilp )
                    cond = stdlib_qlapy2( uhav, uhbv )
                    if( cond==zero ) then
                       s( ks ) = -one
                    else
              ...
              if( wantdf ) then
                 if( n==1_ilp ) then
                    dif( ks ) = stdlib_qlapy2( a( 1_ilp, 1_ilp ), b( 1_ilp, 1_ilp ) )
                    cycle loop_20
                 end if
                 ! estimate the reciprocal condition number of the k-th
                 ! eigenvectors.
                 if( pair ) then
                    ...
                    work( 7_ilp ) = b( k, k+1 )
                    work( 8_ilp ) = b( k+1, k+1 )
                    call stdlib_qlag2( work, 2_ilp, work( 5_ilp ), 2_ilp, smlnum*eps, beta,dummy1( 1_ilp ), &
                              alphar, dummy( 1_ilp ), alphai )
                    alprqt = one
                    c1 = two*( alphar*alphar+alphai*alphai+beta*beta )
                    c2 = four*beta*beta*alphai*alphai
                 ...
                 ! copy the matrix (a, b) to the array work and swap the
                 ! diagonal block beginning at a(k,k) to the (1,1) position.
                 call stdlib_qlacpy( 'FULL', n, n, a, lda, work, n )
                 call stdlib_qlacpy( 'FULL', n, n, b, ldb, work( n*n+1 ), n )
                 ifst = k
                 ilst = 1_ilp
                 call stdlib_qtgexc( .false., .false., n, work, n, work( n*n+1 ), n,dummy, 1_ilp, &
                           dummy1, 1_ilp, ifst, ilst,work( n*n*2_ilp+1 ), lwork-2*n*n, ierr )
                 if( ierr>0_ilp ) then
                    ! ill-conditioned problem - swap rejected.
                    dif( ks ) = zero
                    ...
                    if( n2==0_ilp ) then
                       ...
                       i = n*n + 1_ilp
                       iz = 2_ilp*n*n + 1_ilp
                       call stdlib_qtgsyl( 'N', difdri, n2, n1, work( n*n1+n1+1 ),n, work, n, &
                       work( n1+1 ), n,work( n*n1+n1+i ), n, work( i ), n,work( n1+i ), n, scale, &
                                 dif( ks ),work( iz+1 ), lwork-2*n*n, iwork, ierr )
                       if( pair )dif( ks ) = min( max( one, alprqt )*dif( ks ),cond )
                    end if
     ...
     !! B are both upper triangular.
        ...
        ! =====================================================================
           ...
           ! Local Arrays 
           complex(qp) :: dummy(1_ilp), dummy1(1_ilp)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! decode and test the input parameters
           wantbh = stdlib_lsame( job, 'B' )
           ...
           loop_20: do k = 1, n
              ...
              if( wants ) then
                 ...
                 call stdlib_wgemv( 'N', n, n, cmplx( one, zero,KIND=qp), a, lda,vr( 1_ilp, ks ), 1_ilp, &
                           cmplx( zero, zero,KIND=qp), work, 1_ilp )
                 yhax = stdlib_wdotc( n, work, 1_ilp, vl( 1_ilp, ks ), 1_ilp )
                 call stdlib_wgemv( 'N', n, n, cmplx( one, zero,KIND=qp), b, ldb,vr( 1_ilp, ks ), 1_ilp, &
                           cmplx( zero, zero,KIND=qp), work, 1_ilp )
                 yhbx = stdlib_wdotc( n, work, 1_ilp, vl( 1_ilp, ks ), 1_ilp )
                 cond = stdlib_qlapy2( abs( yhax ), abs( yhbx ) )
                 if( cond==zero ) then
                    s( ks ) = -one
                 else
                    s( ks ) = cond / ( rnrm*lnrm )
              ...
              if( wantdf ) then
                 if( n==1_ilp ) then
                    dif( ks ) = stdlib_qlapy2( abs( a( 1_ilp, 1_ilp ) ), abs( b( 1_ilp, 1_ilp ) ) )
                 else
                    ! estimate the reciprocal condition number of the k-th
                    ! eigenvectors.
                    ! copy the matrix (a, b) to the array work and move the
                    ! (k,k)th pair to the (1,1) position.
                    call stdlib_wlacpy( 'FULL', n, n, a, lda, work, n )
                    call stdlib_wlacpy( 'FULL', n, n, b, ldb, work( n*n+1 ), n )
                    ifst = k
                    ilst = 1_ilp
                    call stdlib_wtgexc( .false., .false., n, work, n, work( n*n+1 ),n, dummy, 1_ilp, &
                              dummy1, 1_ilp, ifst, ilst, ierr )
                    if( ierr>0_ilp ) then
                       ! ill-conditioned problem - swap rejected.
                       dif( ks ) = zero
                       ...
                       n2 = n - n1
                       i = n*n + 1_ilp
                       call stdlib_wtgsyl( 'N', idifjb, n2, n1, work( n*n1+n1+1 ),n, work, n, &
                       work( n1+1 ), n,work( n*n1+n1+i ), n, work( i ), n,work( n1+i ), n, scale, &
                                 dif( ks ), dummy,1_ilp, iwork, ierr )
                    end if
                 end if
     ...
     !! This is a level 3 BLAS algorithm.
        ...
        ! sven hammarling, 1/5/02.
           ...
           if( notran ) then
              if( ijob>=3_ilp ) then
                 ifunc = ijob - 2_ilp
                 call stdlib_qlaset( 'F', m, n, zero, zero, c, ldc )
                 call stdlib_qlaset( 'F', m, n, zero, zero, f, ldf )
              else if( ijob>=1_ilp ) then
                 isolve = 2_ilp
              end if
           end if
           if( ( mb<=1_ilp .and. nb<=1_ilp ) .or. ( mb>=m .and. nb>=n ) )then
              loop_30: do iround = 1, isolve
                 ...
                 dsum = one
                 pq = 0_ilp
                 call stdlib_qtgsy2( trans, ifunc, m, n, a, lda, b, ldb, c, ldc, d,ldd, e, lde, f,&
                            ldf, scale, dsum, dscale,iwork, pq, info )
                 if( dscale/=zero ) then
                    if( ijob==1_ilp .or. ijob==3_ilp ) then
                       dif = sqrt( real( 2_ilp*m*n,KIND=qp) ) / ( dscale*sqrt( dsum ) )
                    else
                 ...
                 if( isolve==2_ilp .and. iround==1_ilp ) then
                    ...
                    scale2 = scale
                    call stdlib_qlacpy( 'F', m, n, c, ldc, work, m )
                    call stdlib_qlacpy( 'F', m, n, f, ldf, work( m*n+1 ), m )
                    call stdlib_qlaset( 'F', m, n, zero, zero, c, ldc )
                    call stdlib_qlaset( 'F', m, n, zero, zero, f, ldf )
                 else if( isolve==2_ilp .and. iround==2_ilp ) then
                    call stdlib_qlacpy( 'F', m, n, work, m, c, ldc )
                    call stdlib_qlacpy( 'F', m, n, work( m*n+1 ), m, f, ldf )
                    scale = scale2
                 end if
              end do loop_30
              return
           ...
           if( notran ) then
              loop_150: do iround = 1, isolve
                 ...
                 loop_130: do j = p + 2, q
                    ...
                    loop_120: do i = p, 1, -1
                       ...
                       mb = ie - is + 1_ilp
                       ppqq = 0_ilp
                       call stdlib_qtgsy2( trans, ifunc, mb, nb, a( is, is ), lda,b( js, js ), &
                       ldb, c( is, js ), ldc,d( is, is ), ldd, e( js, js ), lde,f( is, js ), ldf, &
                                 scaloc, dsum, dscale,iwork( q+2 ), ppqq, linfo )
                       if( linfo>0_ilp )info = linfo
                       pq = pq + ppqq
                       if( scaloc/=one ) then
                          do k = 1, js - 1
                             call stdlib_qscal( m, scaloc, c( 1_ilp, k ), 1_ilp )
                             call stdlib_qscal( m, scaloc, f( 1_ilp, k ), 1_ilp )
                          end do
                          do k = js, je
                             call stdlib_qscal( is-1, scaloc, c( 1_ilp, k ), 1_ilp )
                             call stdlib_qscal( is-1, scaloc, f( 1_ilp, k ), 1_ilp )
                          end do
                          do k = js, je
                             call stdlib_qscal( m-ie, scaloc, c( ie+1, k ), 1_ilp )
                             call stdlib_qscal( m-ie, scaloc, f( ie+1, k ), 1_ilp )
                          end do
                          do k = je + 1, n
                             call stdlib_qscal( m, scaloc, c( 1_ilp, k ), 1_ilp )
                             call stdlib_qscal( m, scaloc, f( 1_ilp, k ), 1_ilp )
                       ...
                       ! equation.
                       if( i>1_ilp ) then
                          call stdlib_qgemm( 'N', 'N', is-1, nb, mb, -one,a( 1_ilp, is ), lda, c( is, &
                                    js ), ldc, one,c( 1_ilp, js ), ldc )
                          call stdlib_qgemm( 'N', 'N', is-1, nb, mb, -one,d( 1_ilp, is ), ldd, c( is, &
                                    js ), ldc, one,f( 1_ilp, js ), ldf )
                       end if
                       if( j<q ) then
                          call stdlib_qgemm( 'N', 'N', mb, n-je, nb, one,f( is, js ), ldf, b( js, &
                                    je+1 ), ldb,one, c( is, je+1 ), ldc )
                          call stdlib_qgemm( 'N', 'N', mb, n-je, nb, one,f( is, js ), ldf, e( js, &
                                    je+1 ), lde,one, f( is, je+1 ), ldf )
                       end if
                    end do loop_120
                 end do loop_130
              ...
              loop_210: do i = 1, p
                 ...
                 loop_200: do j = q, p + 2, -1
                    ...
                    je = iwork( j+1 ) - 1_ilp
                    nb = je - js + 1_ilp
                    call stdlib_qtgsy2( trans, ifunc, mb, nb, a( is, is ), lda,b( js, js ), ldb, &
                    c( is, js ), ldc,d( is, is ), ldd, e( js, js ), lde,f( is, js ), ldf, scaloc, &
                              dsum, dscale,iwork( q+2 ), ppqq, linfo )
                    if( linfo>0_ilp )info = linfo
                    if( scaloc/=one ) then
                    ...
                    ! substitute r(i, j) and l(i, j) into remaining equation.
                    if( j>p+2 ) then
                       call stdlib_qgemm( 'N', 'T', mb, js-1, nb, one, c( is, js ),ldc, b( 1_ilp, js )&
                                 , ldb, one, f( is, 1_ilp ),ldf )
                       call stdlib_qgemm( 'N', 'T', mb, js-1, nb, one, f( is, js ),ldf, e( 1_ilp, js )&
                                 , lde, one, f( is, 1_ilp ),ldf )
                    end if
                    if( i<p ) then
                       call stdlib_qgemm( 'T', 'N', m-ie, nb, mb, -one,a( is, ie+1 ), lda, c( is, &
                                 js ), ldc, one,c( ie+1, js ), ldc )
                       call stdlib_qgemm( 'T', 'N', m-ie, nb, mb, -one,d( is, ie+1 ), ldd, f( is, &
                                 js ), ldf, one,c( ie+1, js ), ldc )
                    end if
                 end do loop_200
              end do loop_210
     ...
     !! This is a level-3 BLAS algorithm.
        ...
        ! sven hammarling, 1/5/02.
           ...
           if( notran ) then
              if( ijob>=3_ilp ) then
                 ifunc = ijob - 2_ilp
                 call stdlib_wlaset( 'F', m, n, czero, czero, c, ldc )
                 call stdlib_wlaset( 'F', m, n, czero, czero, f, ldf )
              else if( ijob>=1_ilp .and. notran ) then
                 isolve = 2_ilp
              end if
           end if
           if( ( mb<=1_ilp .and. nb<=1_ilp ) .or. ( mb>=m .and. nb>=n ) )then
              ...
              loop_30: do iround = 1, isolve
                 ...
                 dsum = one
                 pq = m*n
                 call stdlib_wtgsy2( trans, ifunc, m, n, a, lda, b, ldb, c, ldc, d,ldd, e, lde, f,&
                            ldf, scale, dsum, dscale,info )
                 if( dscale/=zero ) then
                    if( ijob==1_ilp .or. ijob==3_ilp ) then
                       dif = sqrt( real( 2_ilp*m*n,KIND=qp) ) / ( dscale*sqrt( dsum ) )
                 ...
                 if( isolve==2_ilp .and. iround==1_ilp ) then
                    ...
                    scale2 = scale
                    call stdlib_wlacpy( 'F', m, n, c, ldc, work, m )
                    call stdlib_wlacpy( 'F', m, n, f, ldf, work( m*n+1 ), m )
                    call stdlib_wlaset( 'F', m, n, czero, czero, c, ldc )
                    call stdlib_wlaset( 'F', m, n, czero, czero, f, ldf )
                 else if( isolve==2_ilp .and. iround==2_ilp ) then
                    call stdlib_wlacpy( 'F', m, n, work, m, c, ldc )
                    call stdlib_wlacpy( 'F', m, n, work( m*n+1 ), m, f, ldf )
                    scale = scale2
                 end if
              end do loop_30
              return
           ...
           if( notran ) then
              loop_150: do iround = 1, isolve
                 ...
                 loop_130: do j = p + 2, q
                    ...
                    loop_120: do i = p, 1, -1
                       ...
                       ie = iwork( i+1 ) - 1_ilp
                       mb = ie - is + 1_ilp
                       call stdlib_wtgsy2( trans, ifunc, mb, nb, a( is, is ), lda,b( js, js ), &
                       ldb, c( is, js ), ldc,d( is, is ), ldd, e( js, js ), lde,f( is, js ), ldf, &
                                 scaloc, dsum, dscale,linfo )
                       if( linfo>0_ilp )info = linfo
                       pq = pq + mb*nb
                       if( scaloc/=one ) then
                          do k = 1, js - 1
                             call stdlib_wscal( m, cmplx( scaloc, zero,KIND=qp),c( 1_ilp, k ), 1_ilp )
                                       
                             call stdlib_wscal( m, cmplx( scaloc, zero,KIND=qp),f( 1_ilp, k ), 1_ilp )
                                       
                          end do
                          do k = js, je
                             call stdlib_wscal( is-1, cmplx( scaloc, zero,KIND=qp),c( 1_ilp, k ), 1_ilp )
                                       
                             call stdlib_wscal( is-1, cmplx( scaloc, zero,KIND=qp),f( 1_ilp, k ), 1_ilp )
                                       
                          end do
                          do k = js, je
                             call stdlib_wscal( m-ie, cmplx( scaloc, zero,KIND=qp),c( ie+1, k ), &
                                       1_ilp )
                             call stdlib_wscal( m-ie, cmplx( scaloc, zero,KIND=qp),f( ie+1, k ), &
                                       1_ilp )
                          end do
                          do k = je + 1, n
                             call stdlib_wscal( m, cmplx( scaloc, zero,KIND=qp),c( 1_ilp, k ), 1_ilp )
                       ...
                       if( i>1_ilp ) then
                          call stdlib_wgemm( 'N', 'N', is-1, nb, mb,cmplx( -one, zero,KIND=qp), a(&
                           1_ilp, is ), lda,c( is, js ), ldc, cmplx( one, zero,KIND=qp),c( 1_ilp, js ), &
                                     ldc )
                          call stdlib_wgemm( 'N', 'N', is-1, nb, mb,cmplx( -one, zero,KIND=qp), d(&
                           1_ilp, is ), ldd,c( is, js ), ldc, cmplx( one, zero,KIND=qp),f( 1_ilp, js ), &
                                     ldf )
                       end if
                       if( j<q ) then
              ...
              loop_210: do i = 1, p
                 ...
                 loop_200: do j = q, p + 2, -1
                    ...
                    je = iwork( j+1 ) - 1_ilp
                    nb = je - js + 1_ilp
                    call stdlib_wtgsy2( trans, ifunc, mb, nb, a( is, is ), lda,b( js, js ), ldb, &
                    c( is, js ), ldc,d( is, is ), ldd, e( js, js ), lde,f( is, js ), ldf, scaloc, &
                              dsum, dscale,linfo )
                    if( linfo>0_ilp )info = linfo
                    if( scaloc/=one ) then
                       ...
                       do k = js, je
                          call stdlib_wscal( m-ie, cmplx( scaloc, zero,KIND=qp),c( ie+1, k ), 1_ilp )
                                    
                          call stdlib_wscal( m-ie, cmplx( scaloc, zero,KIND=qp),f( ie+1, k ), 1_ilp )
                    ...
                    if( j>p+2 ) then
                       call stdlib_wgemm( 'N', 'C', mb, js-1, nb,cmplx( one, zero,KIND=qp), c( is,&
                        js ), ldc,b( 1_ilp, js ), ldb, cmplx( one, zero,KIND=qp),f( is, 1_ilp ), ldf )
                                  
                       call stdlib_wgemm( 'N', 'C', mb, js-1, nb,cmplx( one, zero,KIND=qp), f( is,&
                        js ), ldf,e( 1_ilp, js ), lde, cmplx( one, zero,KIND=qp),f( is, 1_ilp ), ldf )
              ...
              loop_120: do j = p + 2, q
                 ...
                 loop_110: do i = p, 1, -1
                    ...
                    if( ( mb==1_ilp ) .and. ( nb==1_ilp ) ) then
                       ...
                       rhs( 2_ilp ) = f( is, js )
                       ! solve z * x = rhs
                       call stdlib_qgetc2( zdim, z, ldz, ipiv, jpiv, ierr )
                       if( ierr>0_ilp )info = ierr
                       if( ijob==0_ilp ) then
                          call stdlib_qgesc2( zdim, z, ldz, rhs, ipiv, jpiv,scaloc )
                          if( scaloc/=one ) then
                             do k = 1, n
                                call stdlib_qscal( m, scaloc, c( 1_ilp, k ), 1_ilp )
                                call stdlib_qscal( m, scaloc, f( 1_ilp, k ), 1_ilp )
                       ...
                       if( i>1_ilp ) then
                          alpha = -rhs( 1_ilp )
                          call stdlib_qaxpy( is-1, alpha, a( 1_ilp, is ), 1_ilp, c( 1_ilp, js ),1_ilp )
                          call stdlib_qaxpy( is-1, alpha, d( 1_ilp, is ), 1_ilp, f( 1_ilp, js ),1_ilp )
                       end if
                       if( j<q ) then
                          call stdlib_qaxpy( n-je, rhs( 2_ilp ), b( js, je+1 ), ldb,c( is, je+1 ), &
                                    ldc )
                          call stdlib_qaxpy( n-je, rhs( 2_ilp ), e( js, je+1 ), lde,f( is, je+1 ), &
                                    ldf )
                       end if
                    else if( ( mb==1_ilp ) .and. ( nb==2_ilp ) ) then
                       ! build a 4-by-4 system z * x = rhs
                       ...
                       ! equation.
                       if( i>1_ilp ) then
                          call stdlib_qger( is-1, nb, -one, a( 1_ilp, is ), 1_ilp, rhs( 1_ilp ),1_ilp, c( 1_ilp, js ),&
                                     ldc )
                          call stdlib_qger( is-1, nb, -one, d( 1_ilp, is ), 1_ilp, rhs( 1_ilp ),1_ilp, f( 1_ilp, js ),&
                                     ldf )
                       end if
                       if( j<q ) then
                          call stdlib_qaxpy( n-je, rhs( 3_ilp ), b( js, je+1 ), ldb,c( is, je+1 ), &
                                    ldc )
                          call stdlib_qaxpy( n-je, rhs( 3_ilp ), e( js, je+1 ), lde,f( is, je+1 ), &
                                    ldf )
                          call stdlib_qaxpy( n-je, rhs( 4_ilp ), b( jsp1, je+1 ), ldb,c( is, je+1 ), &
                                    ldc )
                          call stdlib_qaxpy( n-je, rhs( 4_ilp ), e( jsp1, je+1 ), lde,f( is, je+1 ), &
                                    ldf )
                       end if
                    else if( ( mb==2_ilp ) .and. ( nb==1_ilp ) ) then
                       ! build a 4-by-4 system z * x = rhs
                       ...
                       ! equation.
                       if( i>1_ilp ) then
                          call stdlib_qgemv( 'N', is-1, mb, -one, a( 1_ilp, is ), lda,rhs( 1_ilp ), 1_ilp, &
                                    one, c( 1_ilp, js ), 1_ilp )
                          call stdlib_qgemv( 'N', is-1, mb, -one, d( 1_ilp, is ), ldd,rhs( 1_ilp ), 1_ilp, &
                                    one, f( 1_ilp, js ), 1_ilp )
                       end if
                       if( j<q ) then
                          call stdlib_qger( mb, n-je, one, rhs( 3_ilp ), 1_ilp,b( js, je+1 ), ldb, c( is, &
                                    je+1 ), ldc )
                          call stdlib_qger( mb, n-je, one, rhs( 3_ilp ), 1_ilp,e( js, je+1 ), lde, f( is, &
                                    je+1 ), ldf )
                       end if
                    else if( ( mb==2_ilp ) .and. ( nb==2_ilp ) ) then
                       ! build an 8-by-8 system z * x = rhs
                       call stdlib_qlaset( 'F', ldz, ldz, zero, zero, z, ldz )
                       z( 1_ilp, 1_ilp ) = a( is, is )
                       z( 2_ilp, 1_ilp ) = a( isp1, is )
                       z( 5_ilp, 1_ilp ) = d( is, is )
                       z( 1_ilp, 2_ilp ) = a( is, isp1 )
                       ...
                       ii = mb*nb + 1_ilp
                       do jj = 0, nb - 1
                          call stdlib_qcopy( mb, c( is, js+jj ), 1_ilp, rhs( k ), 1_ilp )
                          call stdlib_qcopy( mb, f( is, js+jj ), 1_ilp, rhs( ii ), 1_ilp )
                          k = k + mb
                          ii = ii + mb
                       end do
                       ! solve z * x = rhs
                       ...
                       ii = mb*nb + 1_ilp
                       do jj = 0, nb - 1
                          call stdlib_qcopy( mb, rhs( k ), 1_ilp, c( is, js+jj ), 1_ilp )
                          call stdlib_qcopy( mb, rhs( ii ), 1_ilp, f( is, js+jj ), 1_ilp )
                          k = k + mb
                          ii = ii + mb
                       end do
                       ! substitute r(i, j) and l(i, j) into remaining
                       ! equation.
                       if( i>1_ilp ) then
                          call stdlib_qgemm( 'N', 'N', is-1, nb, mb, -one,a( 1_ilp, is ), lda, rhs( 1_ilp &
                                    ), mb, one,c( 1_ilp, js ), ldc )
                          call stdlib_qgemm( 'N', 'N', is-1, nb, mb, -one,d( 1_ilp, is ), ldd, rhs( 1_ilp &
                                    ), mb, one,f( 1_ilp, js ), ldf )
                       end if
                       if( j<q ) then
                          k = mb*nb + 1_ilp
                          call stdlib_qgemm( 'N', 'N', mb, n-je, nb, one, rhs( k ),mb, b( js, je+&
                                    1_ilp ), ldb, one,c( is, je+1 ), ldc )
                          call stdlib_qgemm( 'N', 'N', mb, n-je, nb, one, rhs( k ),mb, e( js, je+&
                                    1_ilp ), lde, one,f( is, je+1 ), ldf )
                       end if
                    end if
                 end do loop_110
              ...
              loop_200: do i = 1, p
                 ...
                 loop_190: do j = q, p + 2, -1
                    ...
                    if( ( mb==1_ilp ) .and. ( nb==1_ilp ) ) then
                       ...
                       if( j>p+2 ) then
                          alpha = rhs( 1_ilp )
                          call stdlib_qaxpy( js-1, alpha, b( 1_ilp, js ), 1_ilp, f( is, 1_ilp ),ldf )
                          alpha = rhs( 2_ilp )
                          call stdlib_qaxpy( js-1, alpha, e( 1_ilp, js ), 1_ilp, f( is, 1_ilp ),ldf )
                       end if
                       if( i<p ) then
                          alpha = -rhs( 1_ilp )
                          call stdlib_qaxpy( m-ie, alpha, a( is, ie+1 ), lda,c( ie+1, js ), 1_ilp )
                                    
                          alpha = -rhs( 2_ilp )
                          call stdlib_qaxpy( m-ie, alpha, d( is, ie+1 ), ldd,c( ie+1, js ), 1_ilp )
                       ...
                       ! equation.
                       if( j>p+2 ) then
                          call stdlib_qaxpy( js-1, rhs( 1_ilp ), b( 1_ilp, js ), 1_ilp,f( is, 1_ilp ), ldf )
                                    
                          call stdlib_qaxpy( js-1, rhs( 2_ilp ), b( 1_ilp, jsp1 ), 1_ilp,f( is, 1_ilp ), ldf )
                                    
                          call stdlib_qaxpy( js-1, rhs( 3_ilp ), e( 1_ilp, js ), 1_ilp,f( is, 1_ilp ), ldf )
                                    
                          call stdlib_qaxpy( js-1, rhs( 4_ilp ), e( 1_ilp, jsp1 ), 1_ilp,f( is, 1_ilp ), ldf )
                                    
                       end if
                       if( i<p ) then
                          call stdlib_qger( m-ie, nb, -one, a( is, ie+1 ), lda,rhs( 1_ilp ), 1_ilp, c( ie+&
                                    1_ilp, js ), ldc )
                          call stdlib_qger( m-ie, nb, -one, d( is, ie+1 ), ldd,rhs( 3_ilp ), 1_ilp, c( ie+&
                                    1_ilp, js ), ldc )
                       end if
                    else if( ( mb==2_ilp ) .and. ( nb==1_ilp ) ) then
                       ! build a 4-by-4 system z**t * x = rhs
                       ...
                       ! equation.
                       if( j>p+2 ) then
                          call stdlib_qger( mb, js-1, one, rhs( 1_ilp ), 1_ilp, b( 1_ilp, js ),1_ilp, f( is, 1_ilp ), &
                                    ldf )
                          call stdlib_qger( mb, js-1, one, rhs( 3_ilp ), 1_ilp, e( 1_ilp, js ),1_ilp, f( is, 1_ilp ), &
                                    ldf )
                       end if
                       if( i<p ) then
                          call stdlib_qgemv( 'T', mb, m-ie, -one, a( is, ie+1 ),lda, rhs( 1_ilp ), 1_ilp, &
                                    one, c( ie+1, js ),1_ilp )
                          call stdlib_qgemv( 'T', mb, m-ie, -one, d( is, ie+1 ),ldd, rhs( 3_ilp ), 1_ilp, &
                                    one, c( ie+1, js ),1_ilp )
                       end if
                    else if( ( mb==2_ilp ) .and. ( nb==2_ilp ) ) then
                       ! build an 8-by-8 system z**t * x = rhs
                       ...
                       ! equation.
                       if( j>p+2 ) then
                          call stdlib_qgemm( 'N', 'T', mb, js-1, nb, one,c( is, js ), ldc, b( 1_ilp, &
                                    js ), ldb, one,f( is, 1_ilp ), ldf )
                          call stdlib_qgemm( 'N', 'T', mb, js-1, nb, one,f( is, js ), ldf, e( 1_ilp, &
                                    js ), lde, one,f( is, 1_ilp ), ldf )
                       end if
                       if( i<p ) then
                          call stdlib_qgemm( 'T', 'N', m-ie, nb, mb, -one,a( is, ie+1 ), lda, c( &
                                    is, js ), ldc,one, c( ie+1, js ), ldc )
                          call stdlib_qgemm( 'T', 'N', m-ie, nb, mb, -one,d( is, ie+1 ), ldd, f( &
                                    is, js ), ldf,one, c( ie+1, js ), ldc )
                       end if
                    end if
                 end do loop_190
           ...
           if( notran ) then
              ...
              loop_30: do j = 1, n
                 loop_20: do i = m, 1, -1
                    ...
                    rhs( 2_ilp ) = f( i, j )
                    ! solve z * x = rhs
                    call stdlib_wgetc2( ldz, z, ldz, ipiv, jpiv, ierr )
                    if( ierr>0_ilp )info = ierr
                    if( ijob==0_ilp ) then
                       call stdlib_wgesc2( ldz, z, ldz, rhs, ipiv, jpiv, scaloc )
                       if( scaloc/=one ) then
                          do k = 1, n
                             call stdlib_wscal( m, cmplx( scaloc, zero,KIND=qp),c( 1_ilp, k ), 1_ilp )
                    ...
                    if( i>1_ilp ) then
                       alpha = -rhs( 1_ilp )
                       call stdlib_waxpy( i-1, alpha, a( 1_ilp, i ), 1_ilp, c( 1_ilp, j ), 1_ilp )
                       call stdlib_waxpy( i-1, alpha, d( 1_ilp, i ), 1_ilp, f( 1_ilp, j ), 1_ilp )
                    end if
                    if( j<n ) then
                       call stdlib_waxpy( n-j, rhs( 2_ilp ), b( j, j+1 ), ldb,c( i, j+1 ), ldc )
                                 
                       call stdlib_waxpy( n-j, rhs( 2_ilp ), e( j, j+1 ), lde,f( i, j+1 ), ldf )
     ...
     !! where b11 >= b22 > 0.
        ...
        ! =====================================================================
           ...
           ! check if b is singular
              ...
              call stdlib_qlartg( a( 1_ilp, 1_ilp ), a( 2_ilp, 1_ilp ), csl, snl, r )
              csr = one
              snr = zero
              call stdlib_qrot( 2_ilp, a( 1_ilp, 1_ilp ), lda, a( 2_ilp, 1_ilp ), lda, csl, snl )
              call stdlib_qrot( 2_ilp, b( 1_ilp, 1_ilp ), ldb, b( 2_ilp, 1_ilp ), ldb, csl, snl )
              a( 2_ilp, 1_ilp ) = zero
              b( 1_ilp, 1_ilp ) = zero
              b( 2_ilp, 1_ilp ) = zero
              wi = zero
           else if( abs( b( 2_ilp, 2_ilp ) )<=ulp ) then
              call stdlib_qlartg( a( 2_ilp, 2_ilp ), a( 2_ilp, 1_ilp ), csr, snr, t )
              snr = -snr
              call stdlib_qrot( 2_ilp, a( 1_ilp, 1_ilp ), 1_ilp, a( 1_ilp, 2_ilp ), 1_ilp, csr, snr )
              call stdlib_qrot( 2_ilp, b( 1_ilp, 1_ilp ), 1_ilp, b( 1_ilp, 2_ilp ), 1_ilp, csr, snr )
              csl = one
              snl = zero
              a( 2_ilp, 1_ilp ) = zero
              b( 2_ilp, 1_ilp ) = zero
              ...
              ! b is nonsingular, first compute the eigenvalues of (a,b)
              call stdlib_qlag2( a, lda, b, ldb, safmin, scale1, scale2, wr1, wr2,wi )
              if( wi==zero ) then
                 ! two real eigenvalues, compute s*a-w*b
                 h1 = scale1*a( 1_ilp, 1_ilp ) - wr1*b( 1_ilp, 1_ilp )
                 h2 = scale1*a( 1_ilp, 2_ilp ) - wr1*b( 1_ilp, 2_ilp )
                 h3 = scale1*a( 2_ilp, 2_ilp ) - wr1*b( 2_ilp, 2_ilp )
                 rr = stdlib_qlapy2( h1, h2 )
                 qq = stdlib_qlapy2( scale1*a( 2_ilp, 1_ilp ), h3 )
                 if( rr>qq ) then
                    ! find right rotation matrix to zero 1,1 element of
                    ! (sa - wb)
                    call stdlib_qlartg( h2, h1, csr, snr, t )
                 else
                    ! find right rotation matrix to zero 2,1 element of
                    ! (sa - wb)
                    call stdlib_qlartg( h3, scale1*a( 2_ilp, 1_ilp ), csr, snr, t )
                 end if
                 snr = -snr
                 call stdlib_qrot( 2_ilp, a( 1_ilp, 1_ilp ), 1_ilp, a( 1_ilp, 2_ilp ), 1_ilp, csr, snr )
                 call stdlib_qrot( 2_ilp, b( 1_ilp, 1_ilp ), 1_ilp, b( 1_ilp, 2_ilp ), 1_ilp, csr, snr )
                 ...
                 if( ( scale1*h1 )>=abs( wr1 )*h2 ) then
                    ! find left rotation matrix q to zero out b(2,1)
                    call stdlib_qlartg( b( 1_ilp, 1_ilp ), b( 2_ilp, 1_ilp ), csl, snl, r )
                 else
                    ! find left rotation matrix q to zero out a(2,1)
                    call stdlib_qlartg( a( 1_ilp, 1_ilp ), a( 2_ilp, 1_ilp ), csl, snl, r )
                 end if
                 ...
                 ! a pair of complex conjugate eigenvalues
                 ! first compute the svd of the matrix b
                 call stdlib_qlasv2( b( 1_ilp, 1_ilp ), b( 1_ilp, 2_ilp ), b( 2_ilp, 2_ilp ), r, t, snr,csr, snl, csl )
     ...
     !! are the matrices of right and left eigenvectors of (A,B).
        ...
        ! =====================================================================
           ...
           bscale, cim2a, cim2b, cimaga, cimagb, cre2a, cre2b, creala, crealb, dmin, safmin, &
                     salfar, sbeta, scale, small, temp, temp2, temp2i, temp2r, ulp, xmax, xscale
           ! Local Arrays 
           real(qp) :: bdiag(2_ilp), sum(2_ilp,2_ilp), sums(2_ilp,2_ilp), sump(2_ilp,2_ilp)
           ! Intrinsic Functions 
           ! Executable Statements 
           ! decode and test the input parameters
           if( stdlib_lsame( howmny, 'A' ) ) then
           ...
           big = one / safmin
           call stdlib_qlabad( safmin, big )
           ulp = stdlib_qlamch( 'EPSILON' )*stdlib_qlamch( 'BASE' )
           small = safmin*n / ulp
           big = one / small
           bignum = one / ( safmin*n )
           ! compute the 1-norm of each column of the strictly upper triangular
           ...
           if( compl ) then
              ...
              loop_220: do je = 1, n
                 ...
                 if( .not.ilcplx ) then
                    ...
                    ! complex eigenvalue
                    call stdlib_qlag2( s( je, je ), lds, p( je, je ), ldp,safmin*safety, acoef, &
                              temp, bcoefr, temp2,bcoefi )
                    bcoefi = -bcoefi
                    if( bcoefi==zero ) then
                       info = je
                 ...
                 loop_160: do j = je + nw, n
                    ...
                    ! solve  ( a a - b b )  y = sum(,)
                    ! with scaling and perturbation of the denominator
                    call stdlib_qlaln2( .true., na, nw, dmin, acoef, s( j, j ), lds,bdiag( 1_ilp ), &
                    bdiag( 2_ilp ), sum, 2_ilp, bcoefr,bcoefi, work( 2_ilp*n+j ), n, scale, temp,iinfo )
                 ...
                 if( ilback ) then
                    do jw = 0, nw - 1
                       call stdlib_qgemv( 'N', n, n+1-je, one, vl( 1_ilp, je ), ldvl,work( ( jw+2 )*n+&
                                 je ), 1_ilp, zero,work( ( jw+4 )*n+1 ), 1_ilp )
                    end do
                    call stdlib_qlacpy( ' ', n, nw, work( 4_ilp*n+1 ), n, vl( 1_ilp, je ),ldvl )
                    ibeg = 1_ilp
                 else
                    call stdlib_qlacpy( ' ', n, nw, work( 2_ilp*n+1 ), n, vl( 1_ilp, ieig ),ldvl )
                    ibeg = je
                 end if
                 ! scale eigenvector
                 xmax = zero
           ...
           if( compr ) then
              ...
              loop_500: do je = n, 1, -1
                 ...
                 if( .not.ilcplx ) then
                    ...
                    ! complex eigenvalue
                    call stdlib_qlag2( s( je-1, je-1 ), lds, p( je-1, je-1 ), ldp,safmin*safety, &
                              acoef, temp, bcoefr, temp2,bcoefi )
                    if( bcoefi==zero ) then
                       info = je - 1_ilp
                       return
                 ...
                 loop_370: do j = je - nw, 1, -1
                    ...
                    ! compute x(j) (and x(j+1), if 2-by-2 block)
                    call stdlib_qlaln2( .false., na, nw, dmin, acoef, s( j, j ),lds, bdiag( 1_ilp ), &
                    bdiag( 2_ilp ), work( 2_ilp*n+j ),n, bcoefr, bcoefi, sum, 2_ilp, scale, temp,iinfo )
              ...
              loop_140: do je = 1, n
                 ...
                 if( ilcomp ) then
                    ...
                    loop_100: do j = je + 1, n
                       ...
                       work( j ) = stdlib_wladiv( -sum, d )
                       xmax = max( xmax, abs1( work( j ) ) )
                    end do loop_100
                    ! back transform eigenvector if howmny='b'.
                    if( ilback ) then
                       call stdlib_wgemv( 'N', n, n+1-je, cone, vl( 1_ilp, je ), ldvl,work( je ), 1_ilp, &
                                 czero, work( n+1 ), 1_ilp )
                       isrc = 2_ilp
                       ibeg = 1_ilp
                    else
           ...
           if( compr ) then
              ...
              loop_250: do je = n, 1, -1
                 ...
                 if( ilcomp ) then
                    ...
                    ! back transform eigenvector if howmny='b'.
                    if( ilback ) then
                       call stdlib_wgemv( 'N', n, je, cone, vr, ldvr, work, 1_ilp,czero, work( n+1 ), &
                                 1_ilp )
                       isrc = 2_ilp
                       iend = n
                    else
        ...
        ! =====================================================================
           ...
           if( ifst<ilst ) then
              ...
              if( nbf==1_ilp .or. nbf==2_ilp ) then
                 ...
                 call stdlib_qtgex2( wantq, wantz, n, a, lda, b, ldb, q, ldq, z,ldz, here, nbf, &
                           nbnext, work, lwork, info )
                 if( info/=0_ilp ) then
                    ilst = here
                    return
                 ...
                 call stdlib_qtgex2( wantq, wantz, n, a, lda, b, ldb, q, ldq, z,ldz, here+1, 1_ilp, &
                           nbnext, work, lwork, info )
                 if( info/=0_ilp ) then
                    ilst = here
                    return
                 ...
                 if( nbnext==1_ilp ) then
                    ! swap two 1-by-1 blocks.
                    call stdlib_qtgex2( wantq, wantz, n, a, lda, b, ldb, q, ldq, z,ldz, here, 1_ilp, &
                              1_ilp, work, lwork, info )
                    if( info/=0_ilp ) then
                       ilst = here
                       return
                    ...
                    if( nbnext==2_ilp ) then
                       ! 2-by-2 block did not split.
                       call stdlib_qtgex2( wantq, wantz, n, a, lda, b, ldb, q, ldq,z, ldz, here, &
                                 1_ilp, nbnext, work, lwork,info )
                       if( info/=0_ilp ) then
                          ilst = here
                          return
              ...
              if( nbf==1_ilp .or. nbf==2_ilp ) then
                 ...
                 call stdlib_qtgex2( wantq, wantz, n, a, lda, b, ldb, q, ldq, z,ldz, here-nbnext, &
                           nbnext, nbf, work, lwork,info )
                 if( info/=0_ilp ) then
                    ilst = here
                    return
                 ...
                 if( nbnext==1_ilp ) then
                   ...
                   ! recompute nbnext in case of 2-by-2 split.
                    ...
                    if( nbnext==2_ilp ) then
                       ! 2-by-2 block did not split.
                       call stdlib_qtgex2( wantq, wantz, n, a, lda, b, ldb, q, ldq,z, ldz, here-1,&
                                  2_ilp, 1_ilp, work, lwork, info )
                       if( info/=0_ilp ) then
                          ilst = here
                          return
     ...
     !! Q(in) * B(in) * Z(in)**T = Q(out) * B(out) * Z(out)**T
        ...
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           ...
           real(qp), intent(out) :: work(*)
        ! =====================================================================
        ! replaced various illegal calls to stdlib_qcopy by calls to stdlib_qlaset, or by do
        ! loops. sven hammarling, 1/5/02.
           ! Parameters 
           ...
           ! Local Arrays 
           integer(ilp) :: iwork(ldst)
           real(qp) :: ai(2_ilp), ar(2_ilp), be(2_ilp), ir(ldst,ldst), ircop(ldst,ldst), li(ldst,ldst), licop(&
           ldst,ldst), s(ldst,ldst), scpy(ldst,ldst), t(ldst,ldst), taul(ldst), taur(ldst), tcpy(&
                     ldst,ldst)
           ! Intrinsic Functions 
           ! Executable Statements 
           ...
           strong = .false.
           ! make a local copy of selected block
           call stdlib_qlaset( 'FULL', ldst, ldst, zero, zero, li, ldst )
           call stdlib_qlaset( 'FULL', ldst, ldst, zero, zero, ir, ldst )
           call stdlib_qlacpy( 'FULL', m, m, a( j1, j1 ), lda, s, ldst )
           call stdlib_qlacpy( 'FULL', m, m, b( j1, j1 ), ldb, t, ldst )
           ! compute threshold for testing acceptance of swapping.
           eps = stdlib_qlamch( 'P' )
           smlnum = stdlib_qlamch( 'S' ) / eps
           dscale = zero
           dsum = one
           call stdlib_qlacpy( 'FULL', m, m, s, ldst, work, m )
           call stdlib_qlassq( m*m, work, 1_ilp, dscale, dsum )
           dnorma = dscale*sqrt( dsum )
           dscale = zero
           dsum = one
           call stdlib_qlacpy( 'FULL', m, m, t, ldst, work, m )
           call stdlib_qlassq( m*m, work, 1_ilp, dscale, dsum )
           dnormb = dscale*sqrt( dsum )
           ! thres has been changed from
              ! thresh = max( ten*eps*sa, smlnum )
           ...
           if( m==2_ilp ) then
              ...
              sa = abs( s( 2_ilp, 2_ilp ) ) * abs( t( 1_ilp, 1_ilp ) )
              sb = abs( s( 1_ilp, 1_ilp ) ) * abs( t( 2_ilp, 2_ilp ) )
              call stdlib_qlartg( f, g, ir( 1_ilp, 2_ilp ), ir( 1_ilp, 1_ilp ), ddum )
              ir( 2_ilp, 1_ilp ) = -ir( 1_ilp, 2_ilp )
              ir( 2_ilp, 2_ilp ) = ir( 1_ilp, 1_ilp )
              call stdlib_qrot( 2_ilp, s( 1_ilp, 1_ilp ), 1_ilp, s( 1_ilp, 2_ilp ), 1_ilp, ir( 1_ilp, 1_ilp ),ir( 2_ilp, 1_ilp ) )
              call stdlib_qrot( 2_ilp, t( 1_ilp, 1_ilp ), 1_ilp, t( 1_ilp, 2_ilp ), 1_ilp, ir( 1_ilp, 1_ilp ),ir( 2_ilp, 1_ilp ) )
              if( sa>=sb ) then
                 call stdlib_qlartg( s( 1_ilp, 1_ilp ), s( 2_ilp, 1_ilp ), li( 1_ilp, 1_ilp ), li( 2_ilp, 1_ilp ),ddum )
              else
                 call stdlib_qlartg( t( 1_ilp, 1_ilp ), t( 2_ilp, 1_ilp ), li( 1_ilp, 1_ilp ), li( 2_ilp, 1_ilp ),ddum )
              end if
              call stdlib_qrot( 2_ilp, s( 1_ilp, 1_ilp ), ldst, s( 2_ilp, 1_ilp ), ldst, li( 1_ilp, 1_ilp ),li( 2_ilp, 1_ilp ) )
                        
              call stdlib_qrot( 2_ilp, t( 1_ilp, 1_ilp ), ldst, t( 2_ilp, 1_ilp ), ldst, li( 1_ilp, 1_ilp ),li( 2_ilp, 1_ilp ) )
              ...
              if( wands ) then
                 ...
                 dscale = zero
                 dsum = one
                 call stdlib_qlassq( m*m, work( m*m+1 ), 1_ilp, dscale, dsum )
                 sa = dscale*sqrt( dsum )
              ...
              ! update (a(j1:j1+m-1, m+j1:n), b(j1:j1+m-1, m+j1:n)) and
                     ! (a(1:j1-1, j1:j1+m), b(1:j1-1, j1:j1+m)).
              call stdlib_qrot( j1+1, a( 1_ilp, j1 ), 1_ilp, a( 1_ilp, j1+1 ), 1_ilp, ir( 1_ilp, 1_ilp ),ir( 2_ilp, 1_ilp ) )
                        
              call stdlib_qrot( j1+1, b( 1_ilp, j1 ), 1_ilp, b( 1_ilp, j1+1 ), 1_ilp, ir( 1_ilp, 1_ilp ),ir( 2_ilp, 1_ilp ) )
                        
              call stdlib_qrot( n-j1+1, a( j1, j1 ), lda, a( j1+1, j1 ), lda,li( 1_ilp, 1_ilp ), li( 2_ilp, 1_ilp &
                        ) )
              call stdlib_qrot( n-j1+1, b( j1, j1 ), ldb, b( j1+1, j1 ), ldb,li( 1_ilp, 1_ilp ), li( 2_ilp, 1_ilp &
                        ) )
              ! set  n1-by-n2 (2,1) - blocks to zero.
              a( j1+1, j1 ) = zero
              b( j1+1, j1 ) = zero
              ! accumulate transformations into q and z if requested.
              if( wantz )call stdlib_qrot( n, z( 1_ilp, j1 ), 1_ilp, z( 1_ilp, j1+1 ), 1_ilp, ir( 1_ilp, 1_ilp ),ir( 2_ilp, 1_ilp &
                        ) )
              if( wantq )call stdlib_qrot( n, q( 1_ilp, j1 ), 1_ilp, q( 1_ilp, j1+1 ), 1_ilp, li( 1_ilp, 1_ilp ),li( 2_ilp, 1_ilp &
                        ) )
              ! exit with info = 0 if swap was successfully performed.
              return
           else
              ...
              ! solve the generalized sylvester equation
                       ...
                       ! t11 * r - l * t22 = scale * t12
              ! for r and l. solutions in li and ir.
              call stdlib_qlacpy( 'FULL', n1, n2, t( 1_ilp, n1+1 ), ldst, li, ldst )
              call stdlib_qlacpy( 'FULL', n1, n2, s( 1_ilp, n1+1 ), ldst,ir( n2+1, n1+1 ), ldst )
                        
              call stdlib_qtgsy2( 'N', 0_ilp, n1, n2, s, ldst, s( n1+1, n1+1 ), ldst,ir( n2+1, n1+1 ),&
               ldst, t, ldst, t( n1+1, n1+1 ),ldst, li, ldst, scale, dsum, dscale, iwork, idum,&
                         linfo )
              if( linfo/=0 )go to 70
              ! compute orthogonal matrix ql:
              ...
              ! where
                          ! li =  [      -l              ]
                                ! [ scale * identity(n2) ]
              do i = 1, n2
                 call stdlib_qscal( n1, -one, li( 1_ilp, i ), 1_ilp )
                 li( n1+i, i ) = scale
              end do
              call stdlib_qgeqr2( m, n2, li, ldst, taul, work, linfo )
              if( linfo/=0 )go to 70
              call stdlib_qorg2r( m, m, n2, li, ldst, taul, work, linfo )
              if( linfo/=0 )go to 70
              ! compute orthogonal matrix rq:
                          ! ir * rq**t =   [ 0  tr],
               ! where ir = [ scale * identity(n1), r ]
              ...
              call stdlib_qgerq2( n1, m, ir( n2+1, 1_ilp ), ldst, taur, work, linfo )
              if( linfo/=0 )go to 70
              call stdlib_qorgr2( m, m, n1, ir, ldst, taur, work, linfo )
              if( linfo/=0 )go to 70
              ! perform the swapping tentatively:
              call stdlib_qgemm( 'T', 'N', m, m, m, one, li, ldst, s, ldst, zero,work, m )
              call stdlib_qgemm( 'N', 'T', m, m, m, one, work, m, ir, ldst, zero, s,ldst )
              call stdlib_qgemm( 'T', 'N', m, m, m, one, li, ldst, t, ldst, zero,work, m )
              call stdlib_qgemm( 'N', 'T', m, m, m, one, work, m, ir, ldst, zero, t,ldst )
              call stdlib_qlacpy( 'F', m, m, s, ldst, scpy, ldst )
              call stdlib_qlacpy( 'F', m, m, t, ldst, tcpy, ldst )
              call stdlib_qlacpy( 'F', m, m, ir, ldst, ircop, ldst )
              call stdlib_qlacpy( 'F', m, m, li, ldst, licop, ldst )
              ! triangularize the b-part by an rq factorization.
              ! apply transformation (from left) to a-part, giving s.
              call stdlib_qgerq2( m, m, t, ldst, taur, work, linfo )
              if( linfo/=0 )go to 70
              call stdlib_qormr2( 'R', 'T', m, m, m, t, ldst, taur, s, ldst, work,linfo )
              if( linfo/=0 )go to 70
              call stdlib_qormr2( 'L', 'N', m, m, m, t, ldst, taur, ir, ldst, work,linfo )
              if( linfo/=0 )go to 70
              ! compute f-norm(s21) in brqa21. (t21 is 0.)
              dscale = zero
              dsum = one
              do i = 1, n2
                 call stdlib_qlassq( n1, s( n2+1, i ), 1_ilp, dscale, dsum )
              end do
              brqa21 = dscale*sqrt( dsum )
              ! triangularize the b-part by a qr factorization.
              ! apply transformation (from right) to a-part, giving s.
              ...
              dsum = one
              do i = 1, n2
                 call stdlib_qlassq( n1, scpy( n2+1, i ), 1_ilp, dscale, dsum )
              end do
              bqra21 = dscale*sqrt( dsum )
              ! decide which method to use.
                ! weak stability test:
                   ! f-norm(s21) <= o(eps * f-norm((s)))
              if( bqra21<=brqa21 .and. bqra21<=thresha ) then
                 call stdlib_qlacpy( 'F', m, m, scpy, ldst, s, ldst )
                 call stdlib_qlacpy( 'F', m, m, tcpy, ldst, t, ldst )
                 call stdlib_qlacpy( 'F', m, m, ircop, ldst, ir, ldst )
                 call stdlib_qlacpy( 'F', m, m, licop, ldst, li, ldst )
              else if( brqa21>=thresha ) then
                 go to 70
              end if
              ! set lower triangle of b-part to zero
              if (m>1_ilp) call stdlib_qlaset( 'LOWER', m-1, m-1, zero, zero, t(2_ilp,1_ilp), ldst )
              if( wands ) then
                 ! strong stability test:
                     ! f-norm((a-ql**h*s*qr)) <= o(eps*f-norm((a)))
                     ! and
              ...
              ! if the swap is accepted ("weakly" and "strongly"), apply the
              ! transformations and set n1-by-n2 (2,1)-block to zero.
              call stdlib_qlaset( 'FULL', n1, n2, zero, zero, s(n2+1,1_ilp), ldst )
              ! copy back m-by-m diagonal block starting at index j1 of (a, b)
              call stdlib_qlacpy( 'F', m, m, s, ldst, a( j1, j1 ), lda )
              call stdlib_qlacpy( 'F', m, m, t, ldst, b( j1, j1 ), ldb )
              call stdlib_qlaset( 'FULL', ldst, ldst, zero, zero, t, ldst )
              ! standardize existing 2-by-2 blocks.
              call stdlib_qlaset( 'FULL', m, m, zero, zero, work, m )
              work( 1_ilp ) = one
              t( 1_ilp, 1_ilp ) = one
              idum = lwork - m*m - 2_ilp
              if( n2>1_ilp ) then
                 call stdlib_qlagv2( a( j1, j1 ), lda, b( j1, j1 ), ldb, ar, ai, be,work( 1_ilp ), &
                           work( 2_ilp ), t( 1_ilp, 1_ilp ), t( 2_ilp, 1_ilp ) )
                 work( m+1 ) = -work( 2_ilp )
                 work( m+2 ) = work( 1_ilp )
                 t( n2, n2 ) = t( 1_ilp, 1_ilp )
              ...
              call stdlib_qgemm( 'T', 'N', n2, n1, n2, one, work, m, a( j1, j1+n2 ),lda, zero, &
                        work( m*m+1 ), n2 )
              call stdlib_qlacpy( 'FULL', n2, n1, work( m*m+1 ), n2, a( j1, j1+n2 ),lda )
              call stdlib_qgemm( 'T', 'N', n2, n1, n2, one, work, m, b( j1, j1+n2 ),ldb, zero, &
                        work( m*m+1 ), n2 )
              call stdlib_qlacpy( 'FULL', n2, n1, work( m*m+1 ), n2, b( j1, j1+n2 ),ldb )
              call stdlib_qgemm( 'N', 'N', m, m, m, one, li, ldst, work, m, zero,work( m*m+1 ), m &
                        )
              call stdlib_qlacpy( 'FULL', m, m, work( m*m+1 ), m, li, ldst )
              call stdlib_qgemm( 'N', 'N', n2, n1, n1, one, a( j1, j1+n2 ), lda,t( n2+1, n2+1 ), &
                        ldst, zero, work, n2 )
              call stdlib_qlacpy( 'FULL', n2, n1, work, n2, a( j1, j1+n2 ), lda )
              call stdlib_qgemm( 'N', 'N', n2, n1, n1, one, b( j1, j1+n2 ), ldb,t( n2+1, n2+1 ), &
                        ldst, zero, work, n2 )
              call stdlib_qlacpy( 'FULL', n2, n1, work, n2, b( j1, j1+n2 ), ldb )
              call stdlib_qgemm( 'T', 'N', m, m, m, one, ir, ldst, t, ldst, zero,work, m )
              call stdlib_qlacpy( 'FULL', m, m, work, m, ir, ldst )
              ! accumulate transformations into q and z if requested.
              if( wantq ) then
                 call stdlib_qgemm( 'N', 'N', n, m, m, one, q( 1_ilp, j1 ), ldq, li,ldst, zero, work, &
                           n )
                 call stdlib_qlacpy( 'FULL', n, m, work, n, q( 1_ilp, j1 ), ldq )
              end if
              if( wantz ) then
                 call stdlib_qgemm( 'N', 'N', n, m, m, one, z( 1_ilp, j1 ), ldz, ir,ldst, zero, work, &
                           n )
                 call stdlib_qlacpy( 'FULL', n, m, work, n, z( 1_ilp, j1 ), ldz )
              end if
              ! update (a(j1:j1+m-1, m+j1:n), b(j1:j1+m-1, m+j1:n)) and
                      ! (a(1:j1-1, j1:j1+m), b(1:j1-1, j1:j1+m)).
              i = j1 + m
              if( i<=n ) then
                 call stdlib_qgemm( 'T', 'N', m, n-i+1, m, one, li, ldst,a( j1, i ), lda, zero, &
                           work, m )
                 call stdlib_qlacpy( 'FULL', m, n-i+1, work, m, a( j1, i ), lda )
                 call stdlib_qgemm( 'T', 'N', m, n-i+1, m, one, li, ldst,b( j1, i ), ldb, zero, &
                           work, m )
                 call stdlib_qlacpy( 'FULL', m, n-i+1, work, m, b( j1, i ), ldb )
              end if
              i = j1 - 1_ilp
              if( i>0_ilp ) then
                 call stdlib_qgemm( 'N', 'N', i, m, m, one, a( 1_ilp, j1 ), lda, ir,ldst, zero, work, &
                           i )
                 call stdlib_qlacpy( 'FULL', i, m, work, i, a( 1_ilp, j1 ), lda )
                 call stdlib_qgemm( 'N', 'N', i, m, m, one, b( 1_ilp, j1 ), ldb, ir,ldst, zero, work, &
                           i )
                 call stdlib_qlacpy( 'FULL', i, m, work, i, b( 1_ilp, j1 ), ldb )
              end if
              ! exit with info = 0 if swap was successfully performed.
              return
           end if
     ...
     !! Q(in) * B(in) * Z(in)**H = Q(out) * B(out) * Z(out)**H
        ...
        ! =====================================================================
           ...
           ! Local Arrays 
           complex(qp) :: s(ldst,ldst), t(ldst,ldst), work(8_ilp)
           ! Intrinsic Functions 
           ! Executable Statements 
           info = 0_ilp
           ! quick return if possible
           ...
           strong = .false.
           ! make a local copy of selected block in (a, b)
           call stdlib_wlacpy( 'FULL', m, m, a( j1, j1 ), lda, s, ldst )
           call stdlib_wlacpy( 'FULL', m, m, b( j1, j1 ), ldb, t, ldst )
           ! compute the threshold for testing the acceptance of swapping.
           eps = stdlib_qlamch( 'P' )
           smlnum = stdlib_qlamch( 'S' ) / eps
           scale = real( czero,KIND=qp)
           sum = real( cone,KIND=qp)
           call stdlib_wlacpy( 'FULL', m, m, s, ldst, work, m )
           call stdlib_wlacpy( 'FULL', m, m, t, ldst, work( m*m+1 ), m )
           call stdlib_wlassq( m*m, work, 1_ilp, scale, sum )
           sa = scale*sqrt( sum )
           scale = real( czero,KIND=qp)
           sum = real( cone,KIND=qp)
           call stdlib_wlassq( m*m, work(m*m+1), 1_ilp, scale, sum )
           sb = scale*sqrt( sum )
           ! thres has been changed from
              ! thresh = max( ten*eps*sa, smlnum )
           ! to
           ...
           sa = abs( s( 2_ilp, 2_ilp ) ) * abs( t( 1_ilp, 1_ilp ) )
           sb = abs( s( 1_ilp, 1_ilp ) ) * abs( t( 2_ilp, 2_ilp ) )
           call stdlib_wlartg( g, f, cz, sz, cdum )
           sz = -sz
           call stdlib_wrot( 2_ilp, s( 1_ilp, 1_ilp ), 1_ilp, s( 1_ilp, 2_ilp ), 1_ilp, cz, conjg( sz ) )
           call stdlib_wrot( 2_ilp, t( 1_ilp, 1_ilp ), 1_ilp, t( 1_ilp, 2_ilp ), 1_ilp, cz, conjg( sz ) )
           if( sa>=sb ) then
              call stdlib_wlartg( s( 1_ilp, 1_ilp ), s( 2_ilp, 1_ilp ), cq, sq, cdum )
           else
              call stdlib_wlartg( t( 1_ilp, 1_ilp ), t( 2_ilp, 1_ilp ), cq, sq, cdum )
           end if
           call stdlib_wrot( 2_ilp, s( 1_ilp, 1_ilp ), ldst, s( 2_ilp, 1_ilp ), ldst, cq, sq )
           call stdlib_wrot( 2_ilp, t( 1_ilp, 1_ilp ), ldst, t( 2_ilp, 1_ilp ), ldst, cq, sq )
           ! weak stability test: |s21| <= o(eps f-norm((a)))
                                ! and  |t21| <= o(eps f-norm((b)))
           weak = abs( s( 2_ilp, 1_ilp ) )<=thresha .and.abs( t( 2_ilp, 1_ilp ) )<=threshb
           if( .not.weak )go to 20
           if( wands ) then
              ...
              call stdlib_wlacpy( 'FULL', m, m, s, ldst, work, m )
              call stdlib_wlacpy( 'FULL', m, m, t, ldst, work( m*m+1 ), m )
              call stdlib_wrot( 2_ilp, work, 1_ilp, work( 3_ilp ), 1_ilp, cz, -conjg( sz ) )
              call stdlib_wrot( 2_ilp, work( 5_ilp ), 1_ilp, work( 7_ilp ), 1_ilp, cz, -conjg( sz ) )
              call stdlib_wrot( 2_ilp, work, 2_ilp, work( 2_ilp ), 2_ilp, cq, -sq )
              call stdlib_wrot( 2_ilp, work( 5_ilp ), 2_ilp, work( 6_ilp ), 2_ilp, cq, -sq )
              do i = 1, 2
                 work( i ) = work( i ) - a( j1+i-1, j1 )
                 work( i+2 ) = work( i+2 ) - a( j1+i-1, j1+1 )
                 work( i+4 ) = work( i+4 ) - b( j1+i-1, j1 )
           ...
           ! if the swap is accepted ("weakly" and "strongly"), apply the
           ! equivalence transformations to the original matrix pair (a,b)
           call stdlib_wrot( j1+1, a( 1_ilp, j1 ), 1_ilp, a( 1_ilp, j1+1 ), 1_ilp, cz,conjg( sz ) )
           call stdlib_wrot( j1+1, b( 1_ilp, j1 ), 1_ilp, b( 1_ilp, j1+1 ), 1_ilp, cz,conjg( sz ) )
           call stdlib_wrot( n-j1+1, a( j1, j1 ), lda, a( j1+1, j1 ), lda, cq, sq )
           call stdlib_wrot( n-j1+1, b( j1, j1 ), ldb, b( j1+1, j1 ), ldb, cq, sq )
           ! set  n1 by n2 (2,1) blocks to 0
           a( j1+1, j1 ) = czero
           b( j1+1, j1 ) = czero
           ! accumulate transformations into q and z if requested.
           if( wantz )call stdlib_wrot( n, z( 1_ilp, j1 ), 1_ilp, z( 1_ilp, j1+1 ), 1_ilp, cz,conjg( sz ) )
                     
           if( wantq )call stdlib_wrot( n, q( 1_ilp, j1 ), 1_ilp, q( 1_ilp, j1+1 ), 1_ilp, cq,conjg( sq ) )
