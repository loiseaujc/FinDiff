submodule(stdlib_lapack_base) stdlib_lapack_blas_like_base
  ...
  contains
     ...
     !! normal distribution.
        ...
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           use stdlib_blas_constants_qp, only: one, two
           ! Scalar Arguments 
           integer(ilp), intent(in) :: idist, n
           ! Array Arguments 
           integer(ilp), intent(inout) :: iseed(4_ilp)
        ...
        ! =====================================================================
           ...
           do 40 iv = 1, n, lv / 2
              ...
              ! call stdlib_qlaruv to generate il2 numbers from a uniform (0,1)
              ! distribution (il2 <= lv)
              call stdlib_qlaruv( iseed, il2, u )
              if( idist==1_ilp ) then
                 ! copy generated numbers
                 do i = 1, il
                    x( iv+i-1 ) = u( i )
     ...
     !! normal distribution.
        ...
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           use stdlib_blas_constants_qp, only: zero, one, two
           ! Scalar Arguments 
           integer(ilp), intent(in) :: idist, n
           ! Array Arguments 
           integer(ilp), intent(inout) :: iseed(4_ilp)
        ...
        ! =====================================================================
           ...
           do 60 iv = 1, n, lv / 2
              ...
              ! distribution (2*il <= lv)
              call stdlib_qlaruv( iseed, 2_ilp*il, u )
              if( idist==1_ilp ) then
                 ! copy generated numbers
                 do i = 1, il
                    x( iv+i-1 ) = cmplx( u( 2_ilp*i-1 ), u( 2_ilp*i ),KIND=qp)
                 end do
              else if( idist==2_ilp ) then
                 ! convert generated numbers to uniform (-1,1) distribution
                 do i = 1, il
                    x( iv+i-1 ) = cmplx( two*u( 2_ilp*i-1 )-one,two*u( 2_ilp*i )-one,KIND=qp)
                 end do
              else if( idist==3_ilp ) then
                 ! convert generated numbers to normal (0,1) distribution
                 do i = 1, il
                 ...
                 ! distributed on the unit disk
                 do i = 1, il
                    x( iv+i-1 ) = sqrt( u( 2_ilp*i-1 ) )*exp( cmplx( zero, twopi*u( 2_ilp*i ),KIND=qp) )
                 ...
                 ! distributed on the unit circle
                 do i = 1, il
                    x( iv+i-1 ) = exp( cmplx( zero, twopi*u( 2_ilp*i ),KIND=qp) )
                 end do
              end if
              60 continue
           return
     ...
     !! This is an auxiliary routine called by DLARNV and ZLARNV.
        ...
        ! -- lapack is a software package provided by univ. of tennessee,    --
        ! -- univ. of california berkeley, univ. of colorado denver and nag ltd..--
           use stdlib_blas_constants_qp, only: one
           ! Scalar Arguments 
           integer(ilp), intent(in) :: n
           ! Array Arguments 
           integer(ilp), intent(inout) :: iseed(4_ilp)
        ...
        ! =====================================================================
           ...
           20 continue
              ...
              if (x( i )==one) then
                 ...
                 ! will occur about once every 2**n calls), then x( i ) will
                 ! be rounded to exactly one.
                 ! since x( i ) is not supposed to return exactly 0.0_qp or one,
                 ! the statistically correct thing to do in this situation is
                 ! simply to iterate again.
